<?php
/**
 * Handle Trackbacks and Pingbacks Sent to JKPress
 *
 * @since 0.71
 *
 * @package JKPress
...
$title     = isset( $_POST['title'] ) ? jk_unslash( $_POST['title'] ) : '';
$excerpt   = isset( $_POST['excerpt'] ) ? jk_unslash( $_POST['excerpt'] ) : '';
$blog_name = isset( $_POST['blog_name'] ) ? jk_unslash( $_POST['blog_name'] ) : '';
...
$title     = jk_slash( $title );
$excerpt   = jk_slash( $excerpt );
$blog_name = jk_slash( $blog_name );
...
if ( empty( $title ) && empty( $trackback_url ) && empty( $blog_name ) ) {
	...
	jk_redirect( get_permalink( $post_id ) );
	exit;
}
...
if ( ! empty( $trackback_url ) && ! empty( $title ) ) {
	...
	$title   = jk_html_excerpt( $title, 250, '&#8230;' );
	...
	$dupe = $jkdb->get_results(
		$jkdb->prepare(
			"SELECT * FROM $jkdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s",
			$comment_post_id,
			$comment_author_url
		)
	);
	...
	$result = jk_new_comment( $commentdata );

	if ( is_jk_error( $result ) ) {
		trackback_response( 1, $result->get_error_message() );
	}

	$trackback_id = $jkdb->insert_id;
