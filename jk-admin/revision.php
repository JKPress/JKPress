 ...
 *
 * Requires jk-admin/includes/revision.php.
 *
 * @package JKPress
...
switch ( $action ) {
	case 'restore':
		...
		if ( ! jk_revisions_enabled( $post ) && ! jk_is_post_autosave( $revision ) ) {
		 ...
		 * Because jk_insert_post() and jk_transition_post_status() are called
		 * during the process, plugins can unexpectedly modify $post.
		 */
		$backup_global_post = clone $post;

		jk_restore_post_revision( $revision->ID );
...
jk_enqueue_script( 'revisions' );
jk_localize_script( 'revisions', '_jkRevisionsSettings', jk_prepare_revisions_for_js( $post, $revision_id, $from ) );
