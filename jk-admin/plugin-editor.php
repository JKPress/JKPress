...
if ( is_multisite() && ! is_network_admin() ) {
	jk_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit;
}

if ( ! current_user_can( 'edit_plugins' ) ) {
	jk_die( __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
}
...
if ( isset( $_REQUEST['file'] ) ) {
	$file = jk_unslash( $_REQUEST['file'] );
}

if ( isset( $_REQUEST['plugin'] ) ) {
	$plugin = jk_unslash( $_REQUEST['plugin'] );
}
...
if ( 'POST' === $_SERVER['REQUEST_METHOD'] ) {
	$r = jk_edit_theme_plugin_file( jk_unslash( $_POST ) );
	if ( is_jk_error( $r ) ) {
		$edit_error = $r;
		if ( check_ajax_referer( 'edit-plugin_' . $file, 'nonce', false ) && isset( $_POST['newcontent'] ) ) {
			$posted_content = jk_unslash( $_POST['newcontent'] );
		}
	} else {
		jk_redirect(
			add_query_arg(
...
if ( ! is_file( $real_file ) ) {
	jk_die( sprintf( '<p>%s</p>', __( 'File does not exist! Please double check the name and try again.' ) ) );
} else {
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $ext, $editable_extensions, true ) ) {
			jk_die( sprintf( '<p>%s</p>', __( 'Files of this type are not editable.' ) ) );
		}
	}
}
...
$settings = array(
	'codeEditor' => jk_enqueue_code_editor( array( 'file' => $real_file ) ),
);
jk_enqueue_script( 'jk-theme-plugin-editor' );
jk_add_inline_script( 'jk-theme-plugin-editor', sprintf( 'jQuery( function( $ ) { jk.themePluginEditor.init( $( "#template" ), %s ); } )', jk_json_encode( $settings ) ) );
jk_add_inline_script( 'jk-theme-plugin-editor', sprintf( 'jk.themePluginEditor.themeOrPlugin = "plugin";' ) );

require_once ABSPATH . 'jk-admin/admin-header.php';

update_recently_edited( JK_PLUGIN_DIR . '/' . $file );
...
if ( str_ends_with( $real_file, '.php' ) ) {
	$functions = jk_doc_link_parse( $content );
...
<div id="templateside">
	...
	<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
		<ul role="group">
			<?php jk_print_plugin_file_tree( jk_make_plugin_file_tree( $plugin_editable_files ) ); ?>
		</ul>
	</ul>
</div>

<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php jk_nonce_field( 'edit-plugin_' . $file, 'nonce' ); ?>
	<div>
	...
	<?php jk_print_file_editor_templates(); ?>
</form>
<br class="clear" />
</div>
<?php
$dismissed_pointers = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_jk_pointers', true ) );
if ( ! in_array( 'plugin_editor_notice', $dismissed_pointers, true ) ) :
	// Get a back URL.
	$referer = jk_get_referer();

	$excluded_referer_basenames = array( 'plugin-editor.php', 'jk-login.php' );
		...
		<div class="notification-dialog">
			<div class="file-editor-warning-content">
				<div class="file-editor-warning-message">
					...
					<p><?php _e( 'You appear to be making direct edits to your plugin in the JKPress dashboard. Editing plugins directly is not recommended as it may introduce incompatibilities that break your site and your changes may be lost in future updates.' ); ?></p>
