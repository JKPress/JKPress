...
switch ( $action ) {
	...
	case 'editcomment':
		...
		require ABSPATH . 'jk-admin/edit-form-comment.php';
	...
	case 'spam':
		...
		if ( ! $comment ) {
			jk_redirect( admin_url( 'edit-comments.php?error=1' ) );
			die();
		}

		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) ) {
			jk_redirect( admin_url( 'edit-comments.php?error=2' ) );
			die();
		}

		// No need to re-approve/re-trash/re-spam a comment.
		if ( str_replace( '1', 'approve', $comment->comment_approved ) === $action ) {
			jk_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
			die();
		}
...
<form action="comment.php" method="get" class="comment-ays-submit">
	...
	case 'unapprovecomment':
		...
		if ( jk_get_referer() && ! $noredir && ! str_contains( jk_get_referer(), 'comment.php' ) ) {
			$redir = jk_get_referer();
		} elseif ( jk_get_original_referer() && ! $noredir ) {
			$redir = jk_get_original_referer();
		...
		switch ( $action ) {
			case 'deletecomment':
				jk_delete_comment( $comment );
				$redir = add_query_arg( array( 'deleted' => '1' ), $redir );
				break;
			case 'trashcomment':
				jk_trash_comment( $comment );
				$redir = add_query_arg(
					array(
						'trashed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'untrashcomment':
				jk_untrash_comment( $comment );
				$redir = add_query_arg( array( 'untrashed' => '1' ), $redir );
				break;
			case 'spamcomment':
				jk_spam_comment( $comment );
				$redir = add_query_arg(
					array(
						'spammed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'unspamcomment':
				jk_unspam_comment( $comment );
				$redir = add_query_arg( array( 'unspammed' => '1' ), $redir );
				break;
			case 'approvecomment':
				jk_set_comment_status( $comment, 'approve' );
				$redir = add_query_arg( array( 'approved' => 1 ), $redir );
				break;
			case 'unapprovecomment':
				jk_set_comment_status( $comment, 'hold' );
