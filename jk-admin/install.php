...
<body class="jk-core-ui">
	<p id="logo"><a href="https://wordpress.org/">JKPress</a></p>
	<h1>Error: PHP is not running</h1>
	<p>JKPress requires that your web server is running PHP. Your server does not have PHP installed, or PHP is turned off.</p>
</body>
</html>
	<?php
}
...
/** Load JKPress Translation Install API */
...
<head>
	...
	<title><?php _e( 'JKPress &rsaquo; Installation' ); ?></title>
	<?php jk_admin_css( 'install', true ); ?>
</head>
<body class="jk-core-ui<?php echo $body_classes; ?>">
...
function display_setup_form( $error = null ) {
	...
	$user_table = ( $jkdb->get_var( $jkdb->prepare( 'SHOW TABLES LIKE %s', $jkdb->esc_like( $jkdb->users ) ) ) !== null );
	...
	$weblog_title = isset( $_POST['weblog_title'] ) ? trim( jk_unslash( $_POST['weblog_title'] ) ) : '';
	$user_name    = isset( $_POST['user_name'] ) ? trim( jk_unslash( $_POST['user_name'] ) ) : '';
	$admin_email  = isset( $_POST['admin_email'] ) ? trim( jk_unslash( $_POST['admin_email'] ) ) : '';
...
<form id="setup" method="post" action="install.php?step=2" novalidate="novalidate">
	<table class="form-table" role="presentation">
		...
		<tr class="form-field form-required user-pass1-wrap">
			...
			<td>
				<div class="jk-pwd">
					<?php $initial_password = isset( $_POST['admin_password'] ) ? stripslashes( $_POST['admin_password'] ) : jk_generate_password( 18 ); ?>
					...
					<button type="button" class="button jk-hide-pw hide-if-no-js" data-start-masked="<?php echo (int) isset( $_POST['admin_password'] ); ?>" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
			...
			<td>
				<fieldset>
					...
					if ( has_action( 'blog_privacy_selector' ) ) {
						...
						<?php
						/** This action is documented in jk-admin/options-reading.php */
	...
	<p class="step"><?php submit_button( __( 'Install JKPress' ), 'large', 'Submit', false, array( 'id' => 'submit' ) ); ?></p>
	<input type="hidden" name="language" value="<?php echo isset( $_REQUEST['language'] ) ? esc_attr( $_REQUEST['language'] ) : ''; ?>" />
</form>
	<?php
} // End display_setup_form().

// Let's check to make sure JK isn't already installed.
if ( is_blog_installed() ) {
	display_header();
	die(
		'<h1>' . __( 'Already Installed' ) . '</h1>' .
		'<p>' . __( 'You appear to have already installed JKPress. To reinstall please clear your old database tables first.' ) . '</p>' .
		'<p class="step"><a href="' . esc_url( jk_login_url() ) . '">' . __( 'Log In' ) . '</a></p>' .
		'</body></html>'
	);
}
 ...
 * @global jkdb   $jkdb                   JKPress database abstraction object.
 */
global $jk_version, $required_php_version, $required_mysql_version, $jkdb;

$php_version   = PHP_VERSION;
$mysql_version = $jkdb->db_version();
$php_compat    = version_compare( $php_version, $required_php_version, '>=' );
$mysql_compat  = version_compare( $mysql_version, $required_mysql_version, '>=' ) || file_exists( JK_CONTENT_DIR . '/db.php' );

$version_url = sprintf(
	/* translators: %s: JKPress version. */
	esc_url( __( 'https://wordpress.org/documentation/wordpress-version/version-%s/' ) ),
	sanitize_title( $jk_version )
);
...
if ( ! $mysql_compat && ! $php_compat ) {
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">JKPress %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
		$version_url,
		$jk_version,
	...
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">JKPress %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
		$version_url,
		$jk_version,
		$required_php_version,
		$php_version
	...
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">JKPress %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
		$version_url,
		$jk_version,
		$required_mysql_version,
		$mysql_version
...
if ( ! is_string( $jkdb->base_prefix ) || '' === $jkdb->base_prefix ) {
	display_header();
	die(
...
if ( defined( 'DO_NOT_UPGRADE_GLOBAL_TABLES' ) ) {
	...
	die(
		...
		'<p>' . sprintf(
			...
			__( 'The constant %s cannot be defined when installing JKPress.' ),
...
switch ( $step ) {
	...
	case 1: // Step 1, direct link or from language chooser.
		...
		?>
<h1><?php _ex( 'Welcome', 'Howdy' ); ?></h1>
<p><?php _e( 'Welcome to the famous five-minute JKPress installation process! Just fill in the information below and you&#8217;ll be on your way to using the most extendable and powerful personal publishing platform in the world.' ); ?></p>
...
<p><?php _e( 'Please provide the following information. Do not worry, you can always change these settings later.' ); ?></p>
	...
	case 2:
		...
		if ( ! empty( $jkdb->error ) ) {
			jk_die( $jkdb->error->get_error_message() );
		}
		...
		$weblog_title         = isset( $_POST['weblog_title'] ) ? trim( jk_unslash( $_POST['weblog_title'] ) ) : '';
		$user_name            = isset( $_POST['user_name'] ) ? trim( jk_unslash( $_POST['user_name'] ) ) : '';
		$admin_password       = isset( $_POST['admin_password'] ) ? jk_unslash( $_POST['admin_password'] ) : '';
		$admin_password_check = isset( $_POST['admin_password2'] ) ? jk_unslash( $_POST['admin_password2'] ) : '';
		$admin_email          = isset( $_POST['admin_email'] ) ? trim( jk_unslash( $_POST['admin_email'] ) ) : '';
		...
		if ( false === $error ) {
			$jkdb->show_errors();
			$result = jk_install( $weblog_title, $user_name, $admin_email, $public, '', jk_slash( $admin_password ), $loaded_language );
			?>

<h1><?php _e( 'Success!' ); ?></h1>

<p><?php _e( 'JKPress has been installed. Thank you, and enjoy!' ); ?></p>
...
<p class="step"><a href="<?php echo esc_url( jk_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
...
jk_print_scripts( $scripts_to_print );
?>
