...
jk_enqueue_script( 'post' );

$_jk_editor_expand   = false;
...
if ( post_type_supports( $post_type, 'editor' )
	 ...
	 */
	if ( apply_filters( 'jk_editor_expand', true, $post_type ) ) {
		jk_enqueue_script( 'editor-expand' );
		$_content_editor_dfw = true;
		$_jk_editor_expand   = ( 'on' === get_user_setting( 'editor_expand', 'on' ) );
	}
}
...
if ( (int) get_option( 'page_for_posts' ) === $post->ID && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_jk_posts_page_notice' );
...
if ( $thumbnail_support ) {
	add_thickbox();
	jk_enqueue_media( array( 'post' => $post->ID ) );
}
...
$messages['post']       = array(
	...
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Post restored to revision from %s.' ), jk_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page']       = array(
	...
	4  => __( 'Page updated.' ),
	/* translators: %s: Date and time of the revision. */
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Page restored to revision from %s.' ), jk_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _jk_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
...
if ( 'post' === $post_type ) {
	...
	$discussion_settings  = '<p>' . __( '<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other JKPress sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.' ) . '</p>';
...
?>
>
<?php jk_nonce_field( $nonce_action ); ?>
...
<div class="inside">
	...
	if ( $viewable ) :
		...
		if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
			$shortlink = jk_get_shortlink( $post->ID, 'post' );
...
if ( post_type_supports( $post_type, 'editor' ) ) {
	$_jk_editor_expand_class = '';
	if ( $_jk_editor_expand ) {
		$_jk_editor_expand_class = ' jk-editor-expand';
	}
	?>
<div id="postdivrich" class="postarea<?php echo $_jk_editor_expand_class; ?>">
	...
	jk_editor(
		...
		array(
			...
			'tinymce'             => array(
				'resize'                  => false,
				'jk_autoresize_on'        => $_jk_editor_expand,
...
<table id="post-status-info" role="presentation"><tbody><tr>
	<td id="jk-word-count" class="hide-if-no-js">
	<?php
	printf(
...
<?php if ( ! jk_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
