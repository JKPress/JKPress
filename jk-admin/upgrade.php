...
require dirname( __DIR__ ) . '/jk-load.php';
...
<head>
	...
	<title><?php _e( 'JKPress &rsaquo; Update' ); ?></title>
...
<?php if ( (int) get_option( 'db_version' ) === $jk_db_version || ! is_blog_installed() ) : ?>

<h1><?php _e( 'No Update Required' ); ?></h1>
<p><?php _e( 'Your JKPress database is already up to date!' ); ?></p>
...
<p><?php _e( 'JKPress has been updated! Next and final step is to update your database to the newest version.' ); ?></p>
<p><?php _e( 'The database update process may take a little while, so please be patient.' ); ?></p>
<p class="step"><a class="button button-large button-primary" href="upgrade.php?step=1&amp;backto=<?php echo $goback; ?>"><?php _e( 'Update JKPress Database' ); ?></a></p>
			<?php
			break;
		case 1:
			jk_upgrade();

			$backto = ! empty( $_GET['backto'] ) ? jk_unslash( urldecode( $_GET['backto'] ) ) : __get_option( 'home' ) . '/';
			$backto = esc_url( $backto );
			$backto = jk_validate_redirect( $backto, __get_option( 'home' ) . '/' );
			?>
	<h1><?php _e( 'Update Complete' ); ?></h1>
	<p><?php _e( 'Your JKPress database has been successfully updated!' ); ?></p>
