...
if ( ! current_user_can( 'export_others_personal_data' ) ) {
	jk_die( __( 'Sorry, you are not allowed to export personal data on this site.' ) );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p><strong>' . __( 'Note: Since this tool only gathers data from JKPress and participating plugins, you may need to do more to comply with export requests. For example, you should also send the requester some of the data collected from or stored with the 3rd party services your organization uses.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the JKPress database or remotely. Any Export Personal Data request should include data from plugins as well.' ) . '</p>' .
...
$requests_table = _get_list_table( 'JK_Privacy_Data_Export_Requests_List_Table', $_list_table_args );
...
<div class="wrap nosubsub">
	...
	<form action="<?php echo esc_url( admin_url( 'export-personal-data.php' ) ); ?>" method="post" class="jk-privacy-request-form">
