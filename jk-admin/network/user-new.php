...
if ( ! current_user_can( 'create_users' ) ) {
	jk_die( __( 'Sorry, you are not allowed to add users to this network.' ) );
}
...
if ( isset( $_REQUEST['action'] ) && 'add-user' === $_REQUEST['action'] ) {
	...
	if ( ! is_array( $_POST['user'] ) ) {
		jk_die( __( 'Cannot create an empty user.' ) );
	}

	$user = jk_unslash( $_POST['user'] );

	$user_details = jkmu_validate_user_signup( $user['username'], $user['email'] );

	if ( is_jk_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
		$add_user_errors = $user_details['errors'];
	} else {
		$password = jk_generate_password( 12, false );
		$user_id  = jkmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, sanitize_email( $user['email'] ) );

		if ( ! $user_id ) {
			$add_user_errors = new JK_Error( 'add_user_fail', __( 'Cannot add user.' ) );
		} else {
			/**
...
?>
	<form action="<?php echo esc_url( network_admin_url( 'user-new.php?action=add-user' ) ); ?>" id="adduser" method="post" novalidate="novalidate">
		...
		<table class="form-table" role="presentation">
			<tr class="form-field form-required">
				<th scope="row"><label for="username"><?php _e( 'Username' ); ?> <?php echo jk_required_field_indicator(); ?></label></th>
			...
			<tr class="form-field form-required">
				<th scope="row"><label for="email"><?php _e( 'Email' ); ?> <?php echo jk_required_field_indicator(); ?></label></th>
	...
	jk_nonce_field( 'add-user', '_jknonce_add-user' );
