...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
			...
			'<p>' . __( 'You can set the language, and JKPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['JKLANG'] ) && current_user_can( 'install_languages' ) && jk_can_install_language_pack() ) {
		$language = jk_download_language_pack( $_POST['JKLANG'] );
		if ( $language ) {
			$_POST['JKLANG'] = $language;
		}
	}
	...
	foreach ( $options as $option_name ) {
		...
		$value = jk_unslash( $_POST[ $option_name ] );
	...
	jk_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit;
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php jk_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					if ( $new_admin_email && get_site_option( 'admin_email' ) !== $new_admin_email ) :
						...
						$notice_message .= sprintf(
							' <a href="%1$s">%2$s</a>',
							esc_url( jk_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
							__( 'Cancel' )
						);
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						printf(
							/* translators: 1: NOBLOGREDIRECT, 2: jk-config.php */
			...
			<tr>
				...
				<td>
					...
					if ( empty( $limited_email_domains ) ) {
						...
						// Convert from an input field. Back-compat for JKMU < 1.0.
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ); ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table" role="presentation">
				<tr>
					<th><label for="JKLANG"><?php _e( 'Default Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
					<td>
						<?php
