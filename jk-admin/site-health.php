...
if ( ! current_user_can( 'view_site_health_checks' ) ) {
	jk_die( __( 'Sorry, you are not allowed to access site health information.' ), '', 403 );
}
...
if ( 'update_https' === $action ) {
	check_admin_referer( 'jk_update_https' );

	if ( ! current_user_can( 'update_https' ) ) {
		jk_die( __( 'Sorry, you are not allowed to update this site to HTTPS.' ), 403 );
	}

	if ( ! jk_is_https_supported() ) {
		jk_die( __( 'It looks like HTTPS is not supported for your website at this point.' ) );
	}

	$result = jk_update_urls_to_https();

	jk_redirect( add_query_arg( 'https_updated', (int) $result, jk_get_referer() ) );
	exit;
}

$health_check_site_status = JK_Site_Health::get_instance();
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				...
				'<p>' . __( 'In the Status tab, you can see critical information about your JKPress configuration, along with anything else that requires your attention.' ) . '</p>' .
				'<p>' . __( 'In the Info tab, you will find all the details about the configuration of your JKPress site, server, and database. There is also an export feature that allows you to copy all of the information about your site to the clipboard, to help solve problems on your site when obtaining support.' ) . '</p>',
	)
);
...
$health_check_site_status->check_jk_version_check_exists();
...
<div class="health-check-body health-check-status-tab hide-if-no-js">
	...
	<div class="site-status-has-issues">
		...
		<p><?php _e( 'The site health check shows information about your JKPress configuration and items that may need your attention.' ); ?></p>
