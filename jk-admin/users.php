...
if ( empty( $_REQUEST ) ) {
	$referer = '<input type="hidden" name="jk_http_referer" value="' . esc_attr( jk_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset( $_REQUEST['jk_http_referer'] ) ) {
	$redirect = remove_query_arg( array( 'jk_http_referer', 'updated', 'delete_count' ), jk_unslash( $_REQUEST['jk_http_referer'] ) );
	$referer  = '<input type="hidden" name="jk_http_referer" value="' . esc_attr( $redirect ) . '" />';
} else {
	$redirect = 'users.php';
	$referer  = '';
}
...
switch ( $jk_list_table->current_action() ) {
	...
	case 'promote':
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			jk_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			...
			if ( $id === $current_user->ID
				&& ! $jk_roles->role_objects[ $role ]->has_cap( 'promote_users' )
		...
		jk_redirect( add_query_arg( 'update', $update, $redirect ) );
		exit;

	case 'dodelete':
		if ( is_multisite() ) {
			jk_die( __( 'User deletion is not allowed from this screen.' ), 400 );
		}
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', jk_nonce_url( $url, 'bulk-users' ) );
			jk_redirect( $url );
			exit;
		}

		if ( ! current_user_can( 'delete_users' ) ) {
			jk_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			if ( ! current_user_can( 'delete_user', $id ) ) {
				jk_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
			}
			...
			switch ( $_REQUEST['delete_option'] ) {
				case 'delete':
					...
					break;
				case 'reassign':
					jk_delete_user( $id, $_REQUEST['reassign_user'] );
					break;
			}
	...
	case 'resetpassword':
		...
		if ( ! current_user_can( 'edit_users' ) ) {
			$errors = new JK_Error( 'edit_users', __( 'Sorry, you are not allowed to edit users.' ) );
		}
	...
	case 'delete':
		...
		if ( ! current_user_can( 'delete_users' ) ) {
			$errors = new JK_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
		}
		...
		if ( $user_ids && ! $users_have_content ) {
			if ( $jkdb->get_var(
				"SELECT ID FROM {$jkdb->posts}
				WHERE post_author IN( " . implode( ',', $user_ids ) . ' )
				LIMIT 1'
			) ) {
				$users_have_content = true;
			} elseif ( $jkdb->get_var(
				"SELECT link_id FROM {$jkdb->links}
		...
		<?php jk_nonce_field( 'delete-users' ); ?>
		<?php echo $referer; ?>
		...
		if ( $go_delete ) :
			 ...
			 *
			 * @param JK_User $current_user JK_User object for the current user.
			 * @param int[]   $user_ids     Array of IDs for users being deleted.
			 */
	...
	case 'doremove':
		...
		if ( ! is_multisite() ) {
			jk_die( __( 'You cannot remove users.' ), 400 );
		}
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			jk_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
		}
	...
	case 'remove':
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			$error = new JK_Error( 'edit_users', __( 'Sorry, you are not allowed to remove users.' ) );
		}
		...
		?>
		<form method="post" name="updateusers" id="updateusers">
		<?php jk_nonce_field( 'remove-users' ); ?>
		<?php echo $referer; ?>
	...
	default:
		if ( ! empty( $_GET['_jk_http_referer'] ) ) {
			jk_redirect( remove_query_arg( array( '_jk_http_referer', '_jknonce' ), jk_unslash( $_SERVER['REQUEST_URI'] ) ) );
			exit;
		}

		if ( $jk_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			$screen   = get_current_screen()->id;
			$sendback = jk_get_referer();
			$user_ids = array_map( 'intval', (array) $_REQUEST['users'] );

			/** This action is documented in jk-admin/edit.php */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $jk_list_table->current_action(), $user_ids ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
		...
		$total_pages = $jk_list_table->get_pagination_arg( 'total_pages' );

		if ( $pagenum > $total_pages && $total_pages > 0 ) {
			jk_redirect( add_query_arg( 'paged', $total_pages ) );
			exit;
		}
					...
					if ( $user_id && current_user_can( 'edit_user', $user_id ) ) {
						$message .= sprintf(
							...
							esc_url(
								add_query_arg(
									'jk_http_referer',
									urlencode( jk_unslash( $_SERVER['REQUEST_URI'] ) ),
		...
		<?php $jk_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
