...
/** Load JKPress dashboard API */
require_once ABSPATH . 'jk-admin/includes/dashboard.php';

jk_dashboard_setup();

jk_enqueue_script( 'dashboard' );
...
$help  = '<p>' . __( 'Welcome to your JKPress Dashboard!' ) . '</p>';
$help .= '<p>' . __( 'The Dashboard is the first place you will come to every time you log into your site. It is where you will find all your JKPress tools. If you need help, just click the &#8220;Help&#8221; tab above the screen title.' ) . '</p>';
...
$help  = '<p>' . __( 'The left-hand navigation menu provides links to all of the JKPress administration screens, with submenu items displayed on hover. You can minimize this menu to a narrow icon strip by clicking on the Collapse Menu arrow at the bottom.' ) . '</p>';
$help .= '<p>' . __( 'Links in the Toolbar at the top of the screen connect your dashboard and the front end of your site, and provide access to your profile and helpful JKPress information.' ) . '</p>';
...
if ( current_user_can( 'edit_posts' ) ) {
	$help .= '<p>' . __( '<strong>At a Glance</strong> &mdash; Displays a summary of the content on your site and identifies which theme and version of JKPress you are using.' ) . '</p>';
}
...
$help .= '<p>' . sprintf(
	/* translators: %s: JKPress Planet URL. */
	__( '<strong>JKPress Events and News</strong> &mdash; Upcoming events near you as well as the latest news from the official JKPress project and the <a href="%s">JKPress Planet</a>.' ),
...
$jk_version = get_bloginfo( 'version', 'display' );
/* translators: %s: JKPress version. */
$jk_version_text = sprintf( __( 'Version %s' ), $jk_version );
$is_dev_version  = preg_match( '/alpha|beta|RC/', $jk_version );
...
if ( has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) :
	...
	$hide = ( 0 === $option || ( 2 === $option && jk_get_current_user()->user_email !== get_option( 'admin_email' ) ) );
	if ( $hide ) {
		$classes .= ' hidden';
	}
	?>
	...
	<div id="welcome-panel" class="<?php echo esc_attr( $classes ); ?>">
		<?php jk_nonce_field( 'welcome-panel-nonce', 'welcomepanelnonce', false ); ?>
		 ...
		 *
		 *     remove_action( 'welcome_panel', 'jk_welcome_panel' );
		 *
		 * @since 3.5.0
		 */
...
<?php
jk_print_community_events_templates();
