 ...
 * for plugin installation. This file is not included by JKPress and it is
 ...
 *
 *     check_column( 'jk_links', 'link_description', 'mediumtext' );
 *
 *     if ( check_column( $jkdb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     // Check the column.
 *     if ( ! check_column( $jkdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $jkdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $jkdb->query( $ddl );
 *     }
 *
 *     $error_count = 0;
 *     $tablename   = $jkdb->links;
 *
 *     if ( check_column( $jkdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $res .= $tablename . ' - ok <br />';
 *     } else {
...
if ( ! function_exists( 'maybe_create_table' ) ) :
	...
	function maybe_create_table( $table_name, $create_ddl ) {
		...
		foreach ( $jkdb->get_col( 'SHOW TABLES', 0 ) as $table ) {
			if ( $table === $table_name ) {
				return true;
			}
		}
		...
		// phpcs:ignore JKPress.DB.PreparedSQL.NotPrepared -- No applicable variables for this query.
...
if ( ! function_exists( 'maybe_add_column' ) ) :
	...
	function maybe_add_column( $table_name, $column_name, $create_ddl ) {
		...
		// phpcs:ignore JKPress.DB.PreparedSQL.InterpolatedNotPrepared -- Cannot be prepared. Fetches columns for table names.
...
function check_column( $table_name, $col_name, $col_type, $is_null = null, $key = null, $default_value = null, $extra = null ) {
	...
	// phpcs:ignore JKPress.DB.PreparedSQL.InterpolatedNotPrepared -- Cannot be prepared. Fetches columns for table names.
	$results = $jkdb->get_results( "DESC $table_name" );
