 ...
 * base prefix, JKPress core can no longer account for the possible collision.
 */
...
if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['category_base'] ) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$jk_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$jk_rewrite->set_tag_base( $tag_base );
	}
}
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' )
		...
		$writable       = false;
		$existing_rules = array_filter( extract_from_markers( $home_path . '.htaccess', 'JKPress' ) );
...
$using_index_permalinks = $jk_rewrite->using_index_permalinks();
...
if ( $structure_updated ) {
	...
	jk_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}
...
<?php jk_nonce_field( 'update-permalink' ); ?>

<p>
<?php
printf(
	/* translators: %s: Documentation URL. */
	__( 'JKPress offers you the ability to create a custom URL structure for your permalinks and archives. Custom URL structures can improve the aesthetics, usability, and forward-compatibility of your links. A <a href="%s">number of tags are available</a>, and here are some examples to get you started.' ),
...
$default_structure_values = jk_list_pluck( $default_structures, 'value' );
			...
			if ( file_exists( $home_path . 'web.config' ) ) :
				...
				<form action="options-permalink.php" method="post">
					...
					<p>
						...
						><?php echo esc_textarea( $jk_rewrite->iis7_url_rewrite_rules() ); ?></textarea>
					</p>
				</form>
				<p>
				<?php
			...
			<?php else : ?>
				...
				<form action="options-permalink.php" method="post">
					...
					<p>
						...
						><?php echo esc_textarea( $jk_rewrite->iis7_url_rewrite_rules( true ) ); ?></textarea>
					</p>
				</form>
				<p>
				<?php
		...
		<?php if ( $permalink_structure && ! $using_index_permalinks && ! $writable && $htaccess_update_required ) : ?>
			...
			<form action="options-permalink.php" method="post">
				...
				<p>
					...
					><?php echo esc_textarea( $jk_rewrite->mod_rewrite_rules() ); ?></textarea>
				</p>
			</form>
