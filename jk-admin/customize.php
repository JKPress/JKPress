 ...
 */
global $jk_scripts, $jk_customize;

if ( $jk_customize->changeset_post_id() ) {
	$changeset_post = get_post( $jk_customize->changeset_post_id() );
	...
	if ( $missed_schedule ) {
		/*
		 * Note that an Ajax request spawns here instead of just calling `jk_publish_post( $changeset_post->ID )`.
		 *
		 * Because JK_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		 ...
		 */
		$nonces       = $jk_customize->get_nonces();
		$request_args = array(
			'nonce'                      => $nonces['save'],
			'customize_changeset_uuid'   => $jk_customize->changeset_uuid(),
			'jk_customize'               => 'on',
			'customize_changeset_status' => 'publish',
		);
		ob_start();
		?>
		<?php jk_print_scripts( array( 'jk-util' ) ); ?>
		<script>
			jk.ajax.post( 'customize_save', <?php echo jk_json_encode( $request_args ); ?> );
		</script>
		<?php
		$script = ob_get_clean();
...
$url       = ! empty( $_REQUEST['url'] ) ? sanitize_text_field( jk_unslash( $_REQUEST['url'] ) ) : '';
$return    = ! empty( $_REQUEST['return'] ) ? sanitize_text_field( jk_unslash( $_REQUEST['return'] ) ) : '';
$autofocus = ! empty( $_REQUEST['autofocus'] ) && is_array( $_REQUEST['autofocus'] )
	? array_map( 'sanitize_text_field', jk_unslash( $_REQUEST['autofocus'] ) )
	: array();

if ( ! empty( $url ) ) {
	$jk_customize->set_preview_url( $url );
}
if ( ! empty( $return ) ) {
	$jk_customize->set_return_url( $return );
}
if ( ! empty( $autofocus ) ) {
	$jk_customize->set_autofocus( $autofocus );
}
...
$registered             = $jk_scripts->registered;
$jk_scripts             = new JK_Scripts();
$jk_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts', 'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_jk_footer_scripts' );
...
jk_enqueue_script( 'customize-controls' );
jk_enqueue_style( 'customize-controls' );
...
$body_class = 'jk-core-ui jk-customizer js';
...
if ( jk_use_widgets_block_editor() ) {
	$body_class .= ' jk-embed-responsive';
}

$admin_title = sprintf( $jk_customize->get_document_title_template(), __( 'Loading&hellip;' ) );

?>
<title><?php echo esc_html( $admin_title ); ?></title>

<script type="text/javascript">
var ajaxurl = <?php echo jk_json_encode( admin_url( 'admin-ajax.php', 'relative' ) ); ?>,
	pagenow = 'customize';
</script>
...
<div class="jk-full-overlay expanded">
	<form id="customize-controls" class="wrap jk-full-overlay-sidebar">
		<div id="customize-header-actions" class="jk-full-overlay-header">
			<?php
			$compatible_jk  = is_jk_version_compatible( $jk_customize->theme()->get( 'RequiresWP' ) );
			$compatible_php = is_php_version_compatible( $jk_customize->theme()->get( 'RequiresPHP' ) );
			?>
			<?php if ( $compatible_jk && $compatible_php ) : ?>
				<?php $save_text = $jk_customize->is_theme_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url( $jk_customize->get_return_url() ); ?>">
		...
		<div id="widgets-right" class="jk-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="jk-full-overlay-sidebar-content" tabindex="-1">
				<div id="customize-info" class="accordion-section customize-info" data-block-theme="<?php echo (int) jk_is_block_theme(); ?>">
		...
		<div id="customize-footer-actions" class="jk-full-overlay-footer">
			...
			<?php $previewable_devices = $jk_customize->get_previewable_devices(); ?>
	...
	<div id="customize-preview" class="jk-full-overlay-main"></div>
	<?php
