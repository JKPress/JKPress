...
if ( isset( $_GET['page'] ) && ! empty( $_POST ) ) {
	...
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'jk-admin/export-personal-data.php';
		return;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'jk-admin/erase-personal-data.php';
		return;
	}
}

// The privacy policy guide used to be outputted from here. Since JK 5.3 it is in jk-admin/privacy-policy-guide.php.
if ( isset( $_GET['jk-privacy-policy-guide'] ) ) {
	require_once dirname( __DIR__ ) . '/jk-load.php';
	jk_redirect( admin_url( 'options-privacy.php?tab=policyguide' ), 301 );
	exit;
} elseif ( isset( $_GET['page'] ) ) {
	// These were also moved to files in JK 5.3.
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once dirname( __DIR__ ) . '/jk-load.php';
		jk_redirect( admin_url( 'export-personal-data.php' ), 301 );
		exit;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once dirname( __DIR__ ) . '/jk-load.php';
		jk_redirect( admin_url( 'erase-personal-data.php' ), 301 );
		exit;
	}
}
