...
if ( 'on' === $widgets_access ) {
	add_filter( 'admin_body_class', 'jk_widgets_access_body_class' );
} else {
...
if ( empty( $sidebars_widgets ) ) {
	$sidebars_widgets = jk_get_widget_defaults();
}

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'jk_inactive_widgets' === $sidebar_id ) {
		continue;
	}
...
if ( isset( $_POST['savewidget'] ) || isset( $_POST['removewidget'] ) ) {
	...
	if ( isset( $_POST['removewidget'] ) && $_POST['removewidget'] ) {
		...
		if ( ! in_array( $widget_id, $sidebar, true ) ) {
			jk_redirect( admin_url( 'widgets.php?error=0' ) );
			exit;
		}
	...
	jk_redirect( admin_url( 'widgets.php?message=0' ) );
	exit;
}

// Remove inactive widgets without JS.
if ( isset( $_POST['removeinactivewidgets'] ) ) {
	check_admin_referer( 'remove-inactive-widgets', '_jknonce_remove_inactive_widgets' );
...
if ( isset( $_GET['editwidget'] ) && $_GET['editwidget'] ) {
	...
	if ( isset( $_GET['addnew'] ) ) {
		...
		$keys    = array_keys( $jk_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset( $_GET['base'] ) && isset( $_GET['num'] ) ) { // Multi-widget.
			// Copy minimal info from an existing instance of this widget to a new instance.
			foreach ( $jk_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					...
					$jk_registered_widget_controls[ $control['id'] ] = $control;
					break;
				}
			}
		}
	...
	if ( isset( $jk_registered_widget_controls[ $widget_id ] ) && ! isset( $control ) ) {
		$control          = $jk_registered_widget_controls[ $widget_id ];
		$control_callback = $control['callback'];
	} elseif ( ! isset( $jk_registered_widget_controls[ $widget_id ] ) && isset( $jk_registered_widgets[ $widget_id ] ) ) {
		$name = esc_html( strip_tags( $jk_registered_widgets[ $widget_id ]['name'] ) );
	}
	...
	if ( ! isset( $sidebar ) ) {
		$sidebar = isset( $_GET['sidebar'] ) ? $_GET['sidebar'] : 'jk_inactive_widgets';
	}
	...
	<?php
	foreach ( $jk_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr( $sbname ) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'jk_inactive_widgets' === $sbname || str_starts_with( $sbname, 'orphaned_widgets' ) ) {
			echo '&nbsp;';
		} else {
	...
	<div class="widget-control-actions">
		...
		<div class="alignright">
			...
			<?php jk_nonce_field( "save-delete-widget-$widget_id" ); ?>
		</div>
		<br class="clear" />
	</div>
...
$nonce = jk_create_nonce( 'widgets-access' );
?>
<div class="widget-access-link">
	<a id="access-on" href="widgets.php?widgets-access=on&_jknonce=<?php echo urlencode( $nonce ); ?>"><?php _e( 'Enable accessibility mode' ); ?></a><a id="access-off" href="widgets.php?widgets-access=off&_jknonce=<?php echo urlencode( $nonce ); ?>"><?php _e( 'Disable accessibility mode' ); ?></a>
</div>
...
foreach ( $jk_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( str_contains( $registered_sidebar['class'], 'inactive-sidebar' ) || str_starts_with( $sidebar, 'orphaned_widgets' ) ) {
		...
		$is_inactive_widgets = 'jk_inactive_widgets' === $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php jk_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form method="post">
						<p>
							...
							if ( empty( $sidebars_widgets['jk_inactive_widgets'] ) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php jk_nonce_field( 'remove-inactive-widgets', '_jknonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
...
<div id="widgets-right" class="jk-clearfix<?php echo $single_sidebar_class; ?>">
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		...
		jk_list_widget_controls( $sidebar, $registered_sidebar['name'] );
		?>
	</div>
	<?php
...
<form method="post">
<?php jk_nonce_field( 'save-sidebar-widgets', '_jknonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
