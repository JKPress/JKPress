...
if ( ! current_user_can( 'manage_options' ) ) {
	jk_die( __( 'Sorry, you are not allowed to manage options for this site.' ) );
}
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'Two terms you will want to know are the JKPress URL and the site URL. The JKPress URL is where the core JKPress installation files are, and the site URL is the address a visitor uses in the browser to go to your site.' ) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s: Documentation URL. */
			__( 'Though the terms refer to two different concepts, in practice, they can be the same address or different. For example, you can have the core JKPress installation files in the root directory (<code>https://example.com</code>), in which case the two URLs would be the same. Or the <a href="%s">JKPress files can be in a subdirectory</a> (<code>https://example.com/wordpress</code>). In that case, the JKPress URL and the site URL would be different.' ),
			__( 'https://developer.wordpress.org/advanced-administration/server/wordpress-in-directory/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: 1: http://, 2: https:// */
			__( 'Both JKPress URL and site URL can start with either %1$s or %2$s. A URL starting with %2$s requires an SSL certificate, so be sure that you have one before changing to %2$s. With %2$s, a padlock will appear next to the address in the browser address bar. Both %2$s and the padlock signal that your site meets some basic security requirements, which can build trust with your visitors and with search engines.' ),
			'<code>http://</code>',
			'<code>https://</code>'
		) . '</p>' .
...
$options_help .= '<p>' . __( 'You can set the language, and JKPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
if ( ! is_multisite() ) {
	...
	$sample_tagline = __( 'Just another JKPress site' );
} else {
...
<td>
	...
	if ( $site_icon_id ) {
		$img_alt            = get_post_meta( $site_icon_id, '_jk_attachment_image_alt', true );
		$filename           = jk_basename( $site_icon_url );
	...
	<p class="description">
		<?php
			printf(
				...
				__( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the JKPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ),
				512,
				512
			);
		?>
...
if ( ! is_multisite() ) {
	...
	if ( defined( 'JK_SITEURL' ) ) {
		$jk_site_url_class = ' disabled';
	}
	if ( defined( 'JK_HOME' ) ) {
		$jk_home_class = ' disabled';
	}
	?>

<tr>
<th scope="row"><label for="siteurl"><?php _e( 'JKPress Address (URL)' ); ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'JK_SITEURL' ) ); ?> class="regular-text code<?php echo $jk_site_url_class; ?>" /></td>
</tr>

<tr>
<th scope="row"><label for="home"><?php _e( 'Site Address (URL)' ); ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'JK_HOME' ) ); ?> class="regular-text code<?php echo $jk_home_class; ?>" />
	<?php if ( ! defined( 'JK_HOME' ) ) : ?>
<p class="description" id="home-description">
		<?php
		printf(
			/* translators: %s: Documentation URL. */
			__( 'Enter the same address here unless you <a href="%s">want your site home page to be different from your JKPress installation directory</a>.' ),
			__( 'https://developer.wordpress.org/advanced-administration/server/wordpress-in-directory/' )
		);
		?>
</p>
...
if ( $new_admin_email && get_option( 'admin_email' ) !== $new_admin_email ) {
	...
	$pending_admin_email_message .= sprintf(
		' <a href="%1$s">%2$s</a>',
		esc_url( jk_nonce_url( admin_url( 'options.php?dismiss=new_admin_email' ), 'dismiss-' . get_current_blog_id() . '-new_admin_email' ) ),
		__( 'Cancel' )
	);
	jk_admin_notice(
...
<td>
<select name="default_role" id="default_role"><?php jk_dropdown_roles( get_option( 'default_role' ) ); ?></select>
</td>
</tr>
...
if ( ! is_multisite() && defined( 'JKLANG' ) && '' !== JKLANG && 'en_US' !== JKLANG && ! in_array( JKLANG, $languages, true ) ) {
	$languages[] = JKLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th scope="row"><label for="JKLANG"><?php _e( 'Site Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			// Add note about deprecated JKLANG constant.
			if ( defined( 'JKLANG' ) && ( '' !== JKLANG ) && JKLANG !== $locale ) {
				_deprecated_argument(
					'define()',
					'4.0.0',
					/* translators: 1: JKLANG, 2: jk-config.php */
					sprintf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), 'JKLANG', 'jk-config.php' )
				);
			}
			?>
		</td>
...
<select id="timezone_string" name="timezone_string" aria-describedby="timezone-description">
	<?php echo jk_timezone_choice( $tzstring, get_user_locale() ); ?>
</select>
...
<span>
	...
	if ( in_array( $tzstring, timezone_identifiers_list( DateTimeZone::ALL_WITH_BC ), true ) ) {
		...
		if ( ! empty( $transitions[1] ) ) {
			...
			printf(
				$message,
				'<code>' . jk_date( __( 'F j, Y' ) . ' ' . __( 'g:i a' ), $transitions[1]['ts'] ) . '</code>'
			);
		} else {
...
for ( $day_index = 0; $day_index <= 6; $day_index++ ) :
	...
	echo "\n\t<option value='" . esc_attr( $day_index ) . "' $selected>" . $jk_locale->get_weekday( $day_index ) . '</option>';
endfor;
?>
</select></td>
</tr>
