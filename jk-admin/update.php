...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'update-selected' === $action ) {
		if ( ! current_user_can( 'update_plugins' ) ) {
			jk_die( __( 'Sorry, you are not allowed to update plugins for this site.' ) );
		}
		...
		if ( ! isset( $_GET['failure'] ) && ! isset( $_GET['success'] ) ) {
			jk_redirect( admin_url( 'update.php?action=activate-plugin&failure=true&plugin=' . urlencode( $plugin ) . '&_jknonce=' . $_GET['_jknonce'] ) );
			activate_plugin( $plugin, '', ! empty( $_GET['networkwide'] ), true );
			jk_redirect( admin_url( 'update.php?action=activate-plugin&success=true&plugin=' . urlencode( $plugin ) . '&_jknonce=' . $_GET['_jknonce'] ) );
			die();
		}
		...
		require_once ABSPATH . 'jk-admin/includes/plugin-install.php'; // For plugins_api().
		...
		if ( isset( $_FILES['pluginzip']['name'] ) && ! str_ends_with( strtolower( $_FILES['pluginzip']['name'] ), '.zip' ) ) {
			jk_die( __( 'Only .zip archives may be uploaded.' ) );
		}
		...
		// Make sure the attachment still exists, or File_Upload_Upgrader will call jk_die()
		...
		jk_redirect( self_admin_url( 'plugin-install.php' ) );
		exit;
	} elseif ( 'upgrade-theme' === $action ) {

		if ( ! current_user_can( 'update_themes' ) ) {
			jk_die( __( 'Sorry, you are not allowed to update themes for this site.' ) );
		}
		...
		require_once ABSPATH . 'jk-admin/includes/class-jk-upgrader.php'; // For themes_api().
		...
		jk_redirect( self_admin_url( 'theme-install.php' ) );
		exit;
	} else {
		/**
		 ...
		 * provided in the request for jk-admin/update.php. Can be used to
		 * provide custom update functionality for themes and plugins.
		 *
		 * @since 2.8.0
		 */
		do_action( "update-custom_{$action}" ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	}
}
