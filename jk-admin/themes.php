...
if ( current_user_can( 'switch_themes' ) && isset( $_GET['action'] ) ) {
	if ( 'activate' === $_GET['action'] ) {
		...
		$theme = jk_get_theme( $_GET['stylesheet'] );
		...
		jk_redirect( admin_url( 'themes.php?activated=true' ) );
		exit;
		...
		jk_redirect( admin_url( 'themes.php?resumed=true' ) );
		exit;
		...
		if ( $active->get( 'Template' ) === $_GET['stylesheet'] ) {
			jk_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			jk_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	} elseif ( 'enable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && jk_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			jk_die( __( 'Sorry, you are not allowed to enable themes automatic updates.' ) );
		}
		...
		jk_redirect( admin_url( 'themes.php?enabled-auto-update=true' ) );

		exit;
	} elseif ( 'disable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && jk_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			jk_die( __( 'Sorry, you are not allowed to disable themes automatic updates.' ) );
		}
		...
		jk_redirect( admin_url( 'themes.php?disabled-auto-update=true' ) );
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your JKPress installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'update_themes' ) && jk_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		'<p>' . __( 'Auto-updates can be enabled or disabled for each individual theme. Themes with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the JK-Cron task scheduling system.' ) . '</p>' .
...
} // End if 'update_themes' && 'jk_is_auto_update_enabled_for_type'.
...
if ( current_user_can( 'switch_themes' ) ) {
	$themes = jk_prepare_themes_for_js();
} else {
	$themes = jk_prepare_themes_for_js( array( jk_get_theme() ) );
}
...
<div class="wrap">
	<h1 class="jk-heading-inline"><?php esc_html_e( 'Themes' ); ?>
...
if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
	$forbidden_paths = array(
		...
		'theme-editor.php',
		'site-editor.php',
		'edit.php?post_type=jk_navigation',
	);
	...
	foreach ( (array) $submenu['themes.php'] as $item ) {
		...
		if ( ! empty( $submenu[ $item[2] ] ) ) {
			...
			if ( file_exists( JK_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) || ! empty( $menu_hook ) ) {
			...
			if ( file_exists( ABSPATH . "jk-admin/$menu_file" ) ) {
...
<div class="theme<?php echo $active_class; ?>">
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			$message = __( 'This theme does not work with your versions of JKPress and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$message .= sprintf(
					/* translators: 1: URL to JKPress Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update JKPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$message .= jk_update_php_annotation( '</p><p><em>', '</em>', false );
			} elseif ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to JKPress Updates screen. */
					' ' . __( '<a href="%s">Please update JKPress</a>.' ),
			...
			$message .= __( 'This theme does not work with your version of JKPress.' );
			if ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to JKPress Updates screen. */
					' ' . __( '<a href="%s">Please update JKPress</a>.' ),
...
$broken_themes = jk_get_themes( array( 'errors' => true ) );
		...
		<tr>
			...
			if ( $can_resume ) {
				if ( 'theme_paused' === $broken_theme->errors()->get_error_code() ) {
					...
					$resume_url = jk_nonce_url( $resume_url, 'resume-theme_' . $stylesheet );
					?>
			...
			if ( $can_delete ) {
				...
				$delete_url = jk_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
			...
			if ( $can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code() ) {
				...
				if ( ! is_jk_error( $parent_theme ) ) {
					...
					$install_url = jk_nonce_url( $install_url, 'install-theme_' . $parent_theme_name );
					?>
 ...
 */
function jk_theme_auto_update_setting_template() {
	$notice   = jk_get_admin_notice(
		'',
		array(
			'type'               => 'error',
		...
		<div class="theme-autoupdate">
			<# if ( data.autoupdate.supported ) { #>
				...
				<# } else if ( data.autoupdate.enabled ) { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-jk-action="disable">
						<span class="dashicons dashicons-update spin hidden" aria-hidden="true"></span><span class="label">' . __( 'Disable auto-updates' ) . '</span>
					</button>
				<# } else { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-jk-action="enable">
			...
			<# if ( data.hasUpdate ) { #>
				...
				<# } #>
				<br />' . jk_get_auto_update_message() . '</span>
			<# } #>
			' . $notice . '
		</div>
	';
	 ...
	 *
	 * See {@see jk_prepare_themes_for_js()} for the properties of the `data` object.
	 *
	 * @since 5.5.0
	 *
		...
		<div class="theme-about jk-clearfix">
			...
			<div class="theme-info">
				...
				<# if ( data.actions.autoupdate ) { #>
					<?php echo jk_theme_auto_update_setting_template(); ?>
				<# } #>
