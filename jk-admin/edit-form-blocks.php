 ...
 */
global $post_type, $post_type_object, $post, $title, $jk_meta_boxes;

$block_editor_context = new JK_Block_Editor_Context( array( 'post' => $post ) );
...
jk_enqueue_script( 'heartbeat' );
jk_enqueue_script( 'jk-edit-post' );
...
$preload_paths = array(
	...
	'/jk/v2/taxonomies?context=view',
	add_query_arg( 'context', 'edit', $rest_path ),
	sprintf( '/jk/v2/types/%s?context=edit', $post_type ),
	'/jk/v2/users/me',
	array( rest_get_route_for_post_type_items( 'attachment' ), 'OPTIONS' ),
	array( rest_get_route_for_post_type_items( 'page' ), 'OPTIONS' ),
	array( rest_get_route_for_post_type_items( 'jk_block' ), 'OPTIONS' ),
	array( rest_get_route_for_post_type_items( 'jk_template' ), 'OPTIONS' ),
	sprintf( '%s/autosaves?context=edit', $rest_path ),
	'/jk/v2/settings',
	array( '/jk/v2/settings', 'OPTIONS' ),
	...
	array( '/jk/v2/global-styles/' . JK_Theme_JSON_Resolver::get_user_global_styles_post_id(), 'OPTIONS' ),
	'/jk/v2/global-styles/' . JK_Theme_JSON_Resolver::get_user_global_styles_post_id() . '?context=edit',
);
...
jk_add_inline_script(
	'jk-blocks',
	sprintf( 'jk.blocks.setCategories( %s );', jk_json_encode( get_block_categories( $post ) ) ),
	'after'
);
...
$meta_box_url = add_query_arg(
	array(
		...
		'meta-box-loader-nonce' => jk_create_nonce( 'meta-box-loader' ),
	),
	$meta_box_url
);
jk_add_inline_script(
	'jk-editor',
	sprintf( 'var _jkMetaBoxUrl = %s;', jk_json_encode( $meta_box_url ) ),
	'before'
);
 ...
 */
$available_templates = jk_get_theme()->get_page_templates( get_post( $post->ID ) );
...
$editor_settings = array(
	...
	'postLockUtils'        => array(
		'nonce'       => jk_create_nonce( 'lock-post_' . $post->ID ),
		'unlockNonce' => jk_create_nonce( 'update-post_' . $post->ID ),
...
$editor_settings['__experimentalAdditionalBlockPatterns']          = JK_Block_Patterns_Registry::get_instance()->get_all_registered( true );
$editor_settings['__experimentalAdditionalBlockPatternCategories'] = JK_Block_Pattern_Categories_Registry::get_instance()->get_all_registered( true );

$autosave = jk_get_post_autosave( $post->ID );
if ( $autosave ) {
	if ( mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
		...
		jk_delete_post_revision( $autosave->ID );
	}
}
...
if ( jk_is_block_theme() && $editor_settings['supportsTemplateMode'] ) {
...
jk_tinymce_inline_scripts();
jk_enqueue_editor();

/**
 * Styles
 */
jk_enqueue_style( 'jk-edit-post' );
 ...
 * `jk_enqueue_style` to add your functionality to the block editor.
 *
 * @since 5.0.0
 */
...
require_once ABSPATH . 'jk-admin/includes/meta-boxes.php';
register_and_do_post_meta_boxes( $post );

// Check if the Custom Fields meta box has been removed at some point.
$core_meta_boxes = $jk_meta_boxes[ $current_screen->id ]['normal']['core'];
...
$init_script = <<<JS
( function() {
	window._jkLoadBlockEditor = new Promise( function( resolve ) {
		jk.domReady( function() {
			resolve( jk.editPost.initializeEditor( 'editor', "%s", %d, %s, %s ) );
		} );
	} );
} )();
JS;
...
$script = sprintf(
	...
	jk_json_encode( $initial_edits )
);
jk_add_inline_script( 'jk-edit-post', $script );

if ( (int) get_option( 'page_for_posts' ) === $post->ID ) {
	add_action( 'admin_enqueue_scripts', '_jk_block_editor_posts_page_notice' );
}
...
<div class="block-editor">
	...
	<div class="wrap hide-if-js block-editor-no-js">
		...
		<?php
		if ( file_exists( JK_PLUGIN_DIR . '/classic-editor/classic-editor.php' ) ) {
			// If Classic Editor is already installed, provide a link to activate the plugin.
			$installed           = true;
			$plugin_activate_url = jk_nonce_url( 'plugins.php?action=activate&amp;plugin=classic-editor/classic-editor.php', 'activate-plugin_classic-editor/classic-editor.php' );
			$message             = sprintf(
			...
			$installed          = false;
			$plugin_install_url = jk_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=classic-editor' ), 'install-plugin_classic-editor' );
			$message            = sprintf(
