...
if ( is_multisite() ) {
	add_filter( 'jkmu_signup_user_notification_email', 'admin_created_user_email' );
}

if ( isset( $_REQUEST['action'] ) && 'adduser' === $_REQUEST['action'] ) {
	check_admin_referer( 'add-user', '_jknonce_add-user' );

	$user_details = null;
	$user_email   = jk_unslash( $_REQUEST['email'] );
	...
	if ( str_contains( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			jk_redirect( add_query_arg( array( 'update' => 'enter_email' ), 'user-new.php' ) );
			die();
		}
	}

	if ( ! $user_details ) {
		jk_redirect( add_query_arg( array( 'update' => 'does_not_exist' ), 'user-new.php' ) );
		die();
	}
	...
	if ( array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
			...
			$newuser_key = jk_generate_password( 20, false );
			add_option(
...
%4$s'
	...
	check_admin_referer( 'create-user', '_jknonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = jk_unslash( $_REQUEST['email'] );
		$user_details   = jkmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );

		if ( is_jk_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			$add_user_errors = $user_details['errors'];
		} else {
			/** This filter is documented in jk-includes/user.php */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( jk_unslash( $_REQUEST['user_login'] ), true ) );

			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'jkmu_signup_user_notification', '__return_false' );  // Disable confirmation email.
				add_filter( 'jkmu_welcome_user_notification', '__return_false' ); // Disable welcome email.
			}
			...
			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				$key      = $jkdb->get_var( $jkdb->prepare( "SELECT activation_key FROM {$jkdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				$new_user = jkmu_activate_signup( $key );
				if ( is_jk_error( $new_user ) ) {
...
jk_enqueue_script( 'jk-ajax-response' );
...
if ( isset( $_GET['update'] ) ) {
	...
	if ( is_multisite() ) {
		...
		if ( ( isset( $_GET['user_id'] ) ) ) {
			...
			if ( $user_id_new ) {
				$edit_link = esc_url( add_query_arg( 'jk_http_referer', urlencode( jk_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_id_new ) ) );
			}
		}
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new JK_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new JK_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new JK_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new JK_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<?php
if ( isset( $add_user_errors ) && is_jk_error( $add_user_errors ) ) :
	$error_message = '';
...
<input name="action" type="hidden" value="adduser" />
	<?php jk_nonce_field( 'add-user', '_jknonce_add-user' ); ?>

<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo esc_html( $label ); ?></label></th>
		<td><input name="email" type="<?php echo esc_attr( $type ); ?>" id="adduser-email" class="jk-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e( 'Role' ); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php jk_dropdown_roles( get_option( 'default_role' ) ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"
	<?php
	/** This action is documented in jk-admin/user-new.php */
	do_action( 'user_new_form_tag' );
	?>
>
<input name="action" type="hidden" value="createuser" />
	<?php jk_nonce_field( 'create-user', '_jknonce_create-user' ); ?>
	<?php
	// Load up the passed data, else set to a default.
	$creating = isset( $_POST['createuser'] );

	$new_user_login             = $creating && isset( $_POST['user_login'] ) ? jk_unslash( $_POST['user_login'] ) : '';
	$new_user_firstname         = $creating && isset( $_POST['first_name'] ) ? jk_unslash( $_POST['first_name'] ) : '';
	$new_user_lastname          = $creating && isset( $_POST['last_name'] ) ? jk_unslash( $_POST['last_name'] ) : '';
	$new_user_email             = $creating && isset( $_POST['email'] ) ? jk_unslash( $_POST['email'] ) : '';
	$new_user_uri               = $creating && isset( $_POST['url'] ) ? jk_unslash( $_POST['url'] ) : '';
	$new_user_role              = $creating && isset( $_POST['role'] ) ? jk_unslash( $_POST['role'] ) : '';
	$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
	$new_user_ignore_pass       = $creating && isset( $_POST['noconfirmation'] ) ? jk_unslash( $_POST['noconfirmation'] ) : '';
...
<table class="form-table" role="presentation">
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button jk-generate-pw hide-if-no-js"><?php _e( 'Generate password' ); ?></button>
			<div class="jk-pwd">
				<?php $initial_password = jk_generate_password( 24 ); ?>
	...
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			jk_dropdown_roles( $new_user_role );
			?>
			</select>
		</td>
	</tr>
