/**
 * @output jk-admin/js/user-profile.js
 */
...
(function($) {
	...
	function bindToggleButton() {
		...
		$toggleButton = $pass1Row.find('.jk-hide-pw');
	...
	function bindPasswordResetLink() {
		$( '#generate-reset-link' ).on( 'click', function() {
			var $this  = $(this),
				...
				var resetAction =  jk.ajax.post( 'send-password-reset', data );
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #jk-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.jk-pwd' );
		$generateButton  = $pass1Row.find( 'button.jk-generate-pw' );
		...
		$generateButton.on( 'click', function () {
			...
			jk.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
		} );

		$cancelButton = $pass1Row.find( 'button.jk-cancel-pw' );
	...
	function check_pass_strength() {
		...
		strength = jk.passwordStrength.meter( pass1, jk.passwordStrength.userInputDisallowedList(), pass1 );
	...
	clipboard.on( 'success', function( e ) {
		...
		jk.a11y.speak( __( 'Application password has been copied to your clipboard.' ) );
	} );
	...
	$( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#jk-admin-bar-my-account' ).find( '.display-name' );
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// Repaint icons.
				if ( typeof jk !== 'undefined' && jk.svgPainter ) {
					try {
						colors = JSON.parse( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						jk.svgPainter.setColors( colors );
						jk.svgPainter.paint();
					}
				}
	...
	$( function() {
		if ( $( '.reset-pass-submit' ).length ) {
			$( '.reset-pass-submit button.jk-generate-pw' ).trigger( 'click' );
		}
	});
