/**
 * @output jk-admin/js/code-editor.js
 */

if ( 'undefined' === typeof window.jk ) {
	/**
	 * @namespace jk
	 */
	window.jk = {};
}
if ( 'undefined' === typeof window.jk.codeEditor ) {
	/**
	 * @namespace jk.codeEditor
	 */
	window.jk.codeEditor = {};
}
...
( function( $, jk ) {
	...
	/**
	 * @typedef {object} jk.codeEditor~CodeEditorInstance
	 ...
	 */
	jk.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, jk.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = jk.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = jk.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
