/**
 * @output jk-admin/js/customize-controls.js
 */

/* global _jkCustomizeHeader, _jkCustomizeBackground, _jkMediaViewsL10n, MediaElementPlayer, console, confirm */
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = jk.customize;
	...
	api.OverlayNotification = api.Notification.extend(/** @lends jk.customize.OverlayNotification.prototype */{
		 ...
		 *
		 * @constructs jk.customize.OverlayNotification
		 * @augments   jk.customize.Notification
		 *
		 * @since 4.9.0
		 *
	...
	api.Notifications = api.Values.extend(/** @lends jk.customize.Notifications.prototype */{
		 ...
		 * @since 4.9.0
		 *
		 * @constructs jk.customize.Notifications
		 * @augments   jk.customize.Values
		 *
		 ...
		 * @since 4.9.0
		 *
		 * @param {string|jk.customize.Notification} notification - Notification object to add. Alternatively code may be supplied, and in that case the second notificationObject argument must be supplied.
		 * @param {jk.customize.Notification} [notificationObject] - Notification to add when first argument is the code string.
		 * @return {jk.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		 ...
		 * @return {Array.<jk.customize.Notification>} Notifications.
		 */
		get: function( args ) {
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( jk.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					jk.a11y.speak( notification.message, 'assertive' );
				}
	...
	api.Setting = api.Value.extend(/** @lends jk.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is JKPress data (theme mod, option, menu, etc.) that the user can
		 * draft changes to in the Customizer.
		 *
		 * @see PHP class JK_Customize_Setting.
		 *
		 * @constructs jk.customize.Setting
		 * @augments   jk.customize.Value
		 *
		 * @since 3.4.0
		 *
		 ...
		 * @param {Object}  [options.previewer]         - The Previewer instance to sync with. Defaults to jk.customize.previewer.
		 */
		 ...
		 *
		 * @since 4.6.0
		 * @return {jk.customize.Control[]} Controls associated with setting.
		 */
	 ...
	 *
	 * @alias jk.customize._latestRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias jk.customize._lastSavedRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias jk.customize._latestSettingRevisions
	 *
	 * @since 4.7.0
	 * @type {object}
	 * @protected
	 ...
	 *
	 * @alias jk.customize.dirtyValues
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 ...
	 *
	 * @alias jk.customize.requestChangesetUpdate
	 *
	 * @since 4.7.0
	 * @access public
	 *
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// A status would cause a revision to be made, and for this jk.customize.previewer.save() should be used.
		...
		request = jk.ajax.post( 'customize_save', data );
	 ...
	 *
	 * @alias jk.customize.utils.bubbleChildValueChanges
	 *
	 * @since 4.1.0
	 *
	 * @param {jk.customize.Class} instance
	 ...
	 *
	 * @alias jk.customize.utils.prioritySort
	 *
	 * @since 4.1.0
	 *
	 * @param {(jk.customize.Panel|jk.customize.Section|jk.customize.Control)} a
	 * @param {(jk.customize.Panel|jk.customize.Section|jk.customize.Control)} b
	 * @return {number}
	 */
	 ...
	 *
	 * @alias jk.customize.utils.isKeydownButNotEnterEvent
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias jk.customize.utils.areElementListsEqual
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias jk.customize.utils.highlightButton
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias jk.customize.utils.getCurrentTimestamp
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias jk.customize.utils.getRemainingTime
	 *
	 * @since 4.9.0
	 *
	...
	Container = api.Class.extend(/** @lends jk.customize~Container.prototype */{
		 ...
		 *
		 * @constructs jk.customize~Container
		 * @augments   jk.customize.Class
		 *
		 * @since 4.1.0
		 *
		 * @borrows jk.customize~focus as focus
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the panel. See jk.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.jk-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.jk-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = jk.template( container.templateSelector );
			} else {
				template = jk.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend(/** @lends jk.customize.Section.prototype */{
		...
		/**
		 * @constructs jk.customize.Section
		 * @augments   jk.customize~Container
		 *
		 * @since 4.1.0
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the section. See jk.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.jk-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.jk-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends jk.customize.ThemesSection.prototype */{
		currentTheme: '',
		overlay: '',
		template: '',
		...
		ready: function() {
			...
			section.template = jk.template( 'customize-themes-details-view' );
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.jk-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.jk-filter-search', function() {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.jk-filter-search' ).val();
							section.contentContainer.find( '.jk-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.jk-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-themes' ).hide();
			request = jk.ajax.post( 'customize_load_themes', params );
			request.done(function( data ) {
				...
				if ( 0 !== themes.length ) {
					...
					if ( 1 === page ) {
						...
						if ( 'local' !== section.params.filter_type ) {
							jk.a11y.speak( api.settings.l10n.themeSearchResults.replace( '%d', data.info.results ) );
						}
					}
					...
					if ( 0 === section.loaded ) {
						...
						jk.a11y.speak( section.container.find( '.no-themes' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				...
				section.headContainer.closest( '.jk-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					jk.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		filterSearch: function( term ) {
			...
			if ( 0 === count ) {
				...
				jk.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		 ...
		 * @since 4.9.0
		 *
		 * @param {jk.customize.ThemesSection} section - The current theme section, passed through the debouncer.
		 * @return {void}
		 */
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.jk-filter-search' ).val();
		...
		updateCount: function( count ) {
			...
			if ( 0 === count ) {
				...
				jk.a11y.speak( api.settings.l10n.announceThemeCount.replace( '%d', count ) );
			}
		},
		 ...
		 * @since 4.2.0
		 *
		 * @return {jk.customize.ThemeControl|boolean} Next theme.
		 */
		 ...
		 *
		 * @since 4.2.0
		 * @return {jk.customize.ThemeControl|boolean} Previous theme.
		 */
		...
		showDetails: function ( theme, callback ) {
			...
			jk.a11y.speak( api.settings.l10n.announceThemeDetails.replace( '%s', theme.name ) );
			if ( callback ) {
				callback();
			}
		},
	...
	api.OuterSection = api.Section.extend(/** @lends jk.customize.OuterSection.prototype */{

		/**
		 * Class jk.customize.OuterSection.
		 *
		 * Creates section outside of the sidebar, there is no ui to trigger collapse/expand so
		 * it would require custom handling.
		 *
		 * @constructs jk.customize.OuterSection
		 * @augments   jk.customize.Section
		 *
		 * @since 4.9.0
		 *
	...
	api.Panel = Container.extend(/** @lends jk.customize.Panel.prototype */{
		containerType: 'panel',

		/**
		 * @constructs jk.customize.Panel
		 * @augments   jk.customize~Container
		 *
		 * @since 4.1.0
		 *
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.jk-full-overlay' ),
				container = accordionSection.closest( '.jk-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = jk.template( panel.templateSelector + '-content' );
			} else {
				template = jk.template( 'customize-panel-default-content' );
			}
	...
	api.ThemesPanel = api.Panel.extend(/** @lends jk.customize.ThemsPanel.prototype */{

		/**
		 *  Class jk.customize.ThemesPanel.
		 *
		 * Custom section for themes that displays without the customize preview.
		 *
		 * @constructs jk.customize.ThemesPanel
		 * @augments   jk.customize.Panel
		 *
		 * @since 4.9.0
		 *
		 * @param {string} id - The ID for the panel.
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.jk-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.jk-full-overlay' );
		...
		/**
		 * Install a theme via jk.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		installTheme: function( event ) {
			...
			jk.updates.maybeRequestFilesystemCredentials( event );
			...
			request = jk.updates.installTheme( {
				slug: slug
			} );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'jk-theme-update-success', function( e, response ) {
			...
			jk.updates.updateTheme( {
				slug: $( event.target ).closest( '.notice' ).data( 'slug' )
			} );
		},

		/**
		 * Delete a theme via jk.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'jk-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends jk.customize.Control.prototype */{
		defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
		 ...
		 *
		 * @see PHP class JK_Customize_Control.
		 *
		 * @constructs jk.customize.Control
		 * @augments   jk.customize.Class
		 *
		 * @borrows jk.customize~focus as this#focus
		 * @borrows jk.customize~Container#activate as this#activate
		 * @borrows jk.customize~Container#deactivate as this#deactivate
		 * @borrows jk.customize~Container#_toggleActive as this#_toggleActive
		 *
		 ...
		 * @since 4.6.0
		 * @this {jk.customize.Control}
		 */
		renderNotifications: function() {
			var control = this, container, notifications, hasError = false;

			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] jk.customize.Control.prototype.renderNotifications() is deprecated in favor of instantiating a jk.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = jk.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = jk.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = jk.template( 'available-menu-item' );
	 ...
	 *
	 * @class    jk.customize.ColorControl
	 * @augments jk.customize.Control
	 */
	api.ColorControl = api.Control.extend(/** @lends jk.customize.ColorControl.prototype */{
		ready: function() {
			var control = this,
	 ...
	 *
	 * @class    jk.customize.MediaControl
	 * @augments jk.customize.Control
	 */
	api.MediaControl = api.Control.extend(/** @lends jk.customize.MediaControl.prototype */{
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					jk.media.attachment( value ).fetch().done( function() {
						...
						jk.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && jk.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = jk.media({
				...
				states: [
					new jk.media.controller.Library({
						...
						library:   jk.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._jkmejsSettings || {};
	 ...
	 *
	 * @class    jk.customize.UploadControl
	 * @augments jk.customize.MediaControl
	 */
	api.UploadControl = api.MediaControl.extend(/** @lends jk.customize.UploadControl.prototype */{
	 ...
	 *
	 * @class    jk.customize.ImageControl
	 * @augments jk.customize.UploadControl
	 */
	api.ImageControl = api.UploadControl.extend(/** @lends jk.customize.ImageControl.prototype */{
		// @deprecated
	 ...
	 *
	 * @class    jk.customize.BackgroundControl
	 * @augments jk.customize.UploadControl
	 */
	api.BackgroundControl = api.UploadControl.extend(/** @lends jk.customize.BackgroundControl.prototype */{
		...
		select: function() {
			...
			jk.ajax.post( 'custom-background-add', {
				nonce: _jkCustomizeBackground.nonces.add,
				jk_customize: 'on',
				customize_theme: api.settings.theme.stylesheet,
	 ...
	 * @since 4.7.0
	 *
	 * @class    jk.customize.BackgroundPositionControl
	 * @augments jk.customize.Control
	 */
	api.BackgroundPositionControl = api.Control.extend(/** @lends jk.customize.BackgroundPositionControl.prototype */{
	 ...
	 *
	 * @class    jk.customize.CroppedImageControl
	 * @augments jk.customize.MediaControl
	 */
	api.CroppedImageControl = api.MediaControl.extend(/** @lends jk.customize.CroppedImageControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = jk.media({
				...
				states: [
					...
					new jk.media.controller.CustomizeImageCropper({
		 ...
		 * @param {jk.media.controller.Cropper} controller
	 ...
	 *
	 * @class    jk.customize.SiteIconControl
	 * @augments jk.customize.CroppedImageControl
	 */
	api.SiteIconControl = api.CroppedImageControl.extend(/** @lends jk.customize.SiteIconControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = jk.media({
				...
				states: [
					...
					new jk.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	/**
	 * @class    jk.customize.HeaderControl
	 * @augments jk.customize.Control
	 */
	api.HeaderControl = api.Control.extend(/** @lends jk.customize.HeaderControl.prototype */{
		ready: function() {
			...
			jk.media.controller.Cropper.prototype.defaults.doCropArgs.jk_customize = 'on';
			jk.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _jkCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_jkCustomizeHeader.data.width, 10),
				yInit = parseInt(_jkCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_jkCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_jkCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = jk.media({
				...
				states: [
					new jk.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _jkCustomizeHeader.data.width,
						suggestedHeight: _jkCustomizeHeader.data.height
					}),
					new jk.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new jk.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * jk.customize.ThemeControl
	 *
	 * @class    jk.customize.ThemeControl
	 * @augments jk.customize.Control
	 */
	api.ThemeControl = api.Control.extend(/** @lends jk.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in JKPress 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	...
	/**
	 * Class jk.customize.CodeEditorControl
	 *
	 * @since 4.9.0
	 *
	 * @class    jk.customize.CodeEditorControl
	 * @augments jk.customize.Control
	 */
	api.CodeEditorControl = api.Control.extend(/** @lends jk.customize.CodeEditorControl.prototype */{
		...
		initEditor: function() {
			...
			if ( jk.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = jk.codeEditor.defaultSettings ? _.clone( jk.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = jk.codeEditor.initialize( $textarea, settings );
	...
	/**
	 * Class jk.customize.DateTimeControl.
	 *
	 * @since 4.9.0
	 * @class    jk.customize.DateTimeControl
	 * @augments jk.customize.Control
	 */
	api.DateTimeControl = api.Control.extend(/** @lends jk.customize.DateTimeControl.prototype */{
		 ...
		 * @return {jk.customize.DateTimeControl}
		 */
	 ...
	 *
	 * @since 4.9.0
	 * @class    jk.customize.PreviewLinkControl
	 * @augments jk.customize.Control
	 */
	api.PreviewLinkControl = api.Control.extend(/** @lends jk.customize.PreviewLinkControl.prototype */{
	 ...
	 *
	 * @alias jk.customize.defaultConstructor
	 */
	 ...
	 *
	 * @callback jk.customize.deferredControlsCallback
	 * @param {jk.customize.Control[]} controls Resolved controls.
	 */

	/**
	 * Collection of all registered controls.
	 *
	 * @alias jk.customize.control
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {jk.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * jk.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = jk.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = jk.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * jk.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = jk.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * jk.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = jk.customize.control( 'background_color ').setting.get();
	 * value = jk.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * jk.customize.control.add( new jk.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * jk.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * jk.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * jk.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 *
	 * @callback jk.customize.deferredSectionsCallback
	 * @param {jk.customize.Section[]} sections Resolved sections.
	 */

	/**
	 * Collection of all registered sections.
	 *
	 * @alias jk.customize.section
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {jk.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested sections.
	 *
	 * @example <caption>Loop over all registered sections.</caption>
	 * jk.customize.section.each( function( section ) { ... } )
	 *
	 * @example <caption>Getting `title_tagline` section instance.</caption>
	 * section = jk.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * jk.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link jk.customize.control} for further examples of how to interact with {@link jk.customize.Values} instances.
	 */
	 ...
	 *
	 * @callback jk.customize.deferredPanelsCallback
	 * @param {jk.customize.Panel[]} panels Resolved panels.
	 */

	/**
	 * Collection of all registered panels.
	 *
	 * @alias jk.customize.panel
	 *
	 * @since 4.0.0
	 *
	 * @type {Function}
	 ...
	 * @return {jk.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param),
	 *                                                       or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * jk.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = jk.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * jk.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link jk.customize.control} for further examples of how to interact with {@link jk.customize.Values} instances.
	 ...
	 *
	 * @callback jk.customize.deferredNotificationsCallback
	 * @param {jk.customize.Notification[]} notifications Resolved notifications.
	 */

	/**
	 * Collection of all global notifications.
	 *
	 * @alias jk.customize.notifications
	 *
	 * @since 4.9.0
	 *
	 * @type {Function}
	 ...
	 * @return {jk.customize.Notification|undefined|jQuery.promise} Notification instance or undefined (if function called with one code param),
	 *                                                              or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = jk.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = jk.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * jk.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * jk.customize.notifications.add( new jk.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * jk.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link jk.customize.control} for further examples of how to interact with {@link jk.customize.Values} instances.
	 */
	api.notifications = new api.Notifications();

	api.PreviewFrame = api.Messenger.extend(/** @lends jk.customize.PreviewFrame.prototype */{
		 ...
		 *
		 * @constructs jk.customize.PreviewFrame
		 * @augments   jk.customize.Messenger
		 *
		...
		run: function( deferred ) {
			...
			previewFrame.iframe.attr( 'onmousewheel', '' ); // Workaround for Safari bug. See JK Trac #38149.
	 ...
	 *
	 * @alias jk.customize.setDocumentTitle
	 *
	 * @since 4.1.0
	 *
	...
	api.Previewer = api.Messenger.extend(/** @lends jk.customize.Previewer.prototype */{
		refreshBuffer: null, // Will get set to api.settings.timeouts.windowRefresh.

		/**
		 * @constructs jk.customize.Previewer
		 * @augments   jk.customize.Messenger
		 *
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in jk-includes or jk-content.
				if ( /\/jk-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
		...
		refreshNonces: function() {
			...
			request = jk.ajax.post( 'customize_refresh_nonces', {
				jk_customize: 'on',
				customize_theme: api.settings.theme.stylesheet
			});
	 ...
	 *
	 * @alias jk.customize._handleSettingValidities
	 *
	 * @since 4.6.0
	 * @private
	 *
	 ...
	 *
	 * @alias jk.customize.findControlsForSettings
	 *
	 * @since 4.6.0
	 * @param {string[]} settingIds Setting IDs.
	 * @return {Object<string, jk.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	 ...
	 *
	 * @alias jk.customize.reflowPaneContents
	 *
	 * @since 4.1.0
	 */
	...
	$( function() {
		api.settings = window._jkCustomizeSettings;
		api.l10n = window._jkCustomizeControlsL10n;
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.jk-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = jk.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = jk.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /jk_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.jk-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via jk.customizer.previewer.save() will update selection.
			saved( true );
		...
		( function checkAndDisplayLockNotice() {
			...
			var LockedNotification = api.OverlayNotification.extend(/** @lends jk.customize~LockedNotification.prototype */{
				 ...
				 *
				 * @constructs jk.customize~LockedNotification
				 * @augments   jk.customize.OverlayNotification
				 *
				 * @since 4.9.0
				 *
				 * @param {string} [code] - Code.
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = jk.ajax.post( 'customize_override_changeset_lock', {
							jk_customize: 'on',
							customize_theme: api.settings.theme.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				jk.ajax.post( 'customize_dismiss_autosave_or_lock', {
					jk_customize: 'on',
					customize_theme: api.settings.theme.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.jk-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {jk.customize.Panel|jk.customize.Section} container Construct.
			 * @return {void}
			 */
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.jk-customize-changeset-update' );
					$( window ).off( 'beforeunload.jk-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					jk.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_jkCustomizeBackground.defaults['default-position-x'],
				_jkCustomizeBackground.defaults['default-position-y'],
				_jkCustomizeBackground.defaults['default-size'],
				_jkCustomizeBackground.defaults['default-repeat'],
				_jkCustomizeBackground.defaults['default-attachment']
			];
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.jk-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.jk-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
