 ...
 *
 * @output jk-admin/js/plugin-install.js
 */
...
jQuery( function( $ ) {
	 ...
	 * @todo Consider to implement a JKPress general utility for this and don't use jQuery UI.
	 */
	function handleTabbables() {
		...
		$firstAndLast.off( 'keydown.jk-plugin-details' );
		// Attach again the keydown event on the first and last focusable elements.
		$firstAndLast.on( 'keydown.jk-plugin-details', function( event ) {
	...
	$( '.wrap' ).on( 'click', '.thickbox.open-plugin-details-modal', function( e ) {
		...
		tbWindow
			.attr({
				'role': 'dialog',
				'aria-label': jk.i18n.__( 'Plugin details' )
			})
