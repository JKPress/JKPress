/**
 * @output jk-admin/js/customize-nav-menus.js
 */

/* global menus, _jkCustomizeNavMenusSettings, jkNavMenu, console */
( function( api, jk, $ ) {
	'use strict';

	/**
	 * Set up jkNavMenu for drag and drop.
	 */
	jkNavMenu.originalInit = jkNavMenu.init;
	jkNavMenu.options.menuItemDepthPerLevel = 20;
	jkNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	jkNavMenu.options.targetTolerance       = 10;
	jkNavMenu.init = function() {
		this.jQueryExtensions();
	};

	/**
	 * @namespace jk.customize.Menus
	 */
	api.Menus = api.Menus || {};
	...
	if ( 'undefined' !== typeof _jkCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _jkCustomizeNavMenusSettings );
	}
	 ...
	 *
	 * @alias jk.customize.Menus.generatePlaceholderAutoIncrementId
	 *
	 * @return {number}
	 */
	...
	/**
	 * jk.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's JK_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @class    jk.customize.Menus.AvailableItemModel
	...
	/**
	 * jk.customize.Menus.AvailableItemCollection
	 *
	 * Collection for available menu item models.
	 *
	 * @class    jk.customize.Menus.AvailableItemCollection
	 * @augments Backbone.Collection
	 */
	api.Menus.AvailableItemCollection = Backbone.Collection.extend(/** @lends jk.customize.Menus.AvailableItemCollection.prototype */{
	 ...
	 *
	 * @since 4.7.0
	 * @alias jk.customize.Menus.insertAutoDraftPost
	 *
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = jk.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	api.Menus.AvailableMenuItemsPanelView = jk.Backbone.View.extend(/** @lends jk.customize.Menus.AvailableMenuItemsPanelView.prototype */{
		...
		/**
		 * jk.customize.Menus.AvailableMenuItemsPanelView
		 *
		 * View class for the available menu items panel.
		 *
		 * @constructs jk.customize.Menus.AvailableMenuItemsPanelView
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				jk.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = jk.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					jk.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					jk.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = jk.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( jk.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * jk.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the JK_Customize_Menu_Panel::$type.
	 *
	 * @class    jk.customize.Menus.MenusPanel
	 * @augments jk.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend(/** @lends jk.customize.Menus.MenusPanel.prototype */{
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					jk.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = jk.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * jk.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the JK_Customize_Menu_Section::$type.
	 *
	 * @class    jk.customize.Menus.MenuSection
	 * @augments jk.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend(/** @lends jk.customize.Menus.MenuSection.prototype */{
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				jkNavMenu.menuList = section.contentContainer;
				jkNavMenu.targetList = jkNavMenu.menuList;

				// Add attributes needed by jkNavMenu.
				$( '#menu-to-edit' ).removeAttr( 'id' );
				jkNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						jkNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
						section.deferred.initSortables.resolve( jkNavMenu.menuList ); // Now MenuControl can extend the sortable.

						// @todo Note that jk.customize.reflowPaneContents() is debounced,
	 ...
	 * @return {jk.customize.Menus.MenuSection} Added nav menu.
	 */
	...
	/**
	 * jk.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 *
	 * @class    jk.customize.Menus.NewMenuSection
	 * @augments jk.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend(/** @lends jk.customize.Menus.NewMenuSection.prototype */{
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				jk.template( 'nav-menu-create-menu-section-title' )
			);
			 ...
			 *
			 * @since 4.9.0
			 * @param {jk.customize.Setting} setting - Added setting.
			 * @return {void}
			 */
			 ...
			 *
			 * @since 4.9.0
			 * @param {jk.customize.Setting} setting - Removed setting.
			 * @return {void}
			 */
		...
		submit: function() {
			...
			jk.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * jk.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the JK_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @class    jk.customize.Menus.MenuLocationControl
	 * @augments jk.customize.Control
	 */
	api.Menus.MenuLocationControl = api.Control.extend(/** @lends jk.customize.Menus.MenuLocationControl.prototype */{
		initialize: function( id, options ) {
			var control = this,
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends jk.customize.Menus.MenuItemControl.prototype */{

		/**
		 * jk.customize.Menus.MenuItemControl
		 *
		 * Customizer control for menu items.
		 * Note that 'menu_item' must match the JK_Customize_Menu_Item_Control::$type.
		 *
		 * @constructs jk.customize.Menus.MenuItemControl
		 * @augments   jk.customize.Control
		 *
		 * @inheritDoc
		 */
		...
		_setupReorderUI: function() {
			...
			template = jk.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					jk.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {jk.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			jk.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			jk.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			jk.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			jk.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * jk.customize.Menus.MenuNameControl
	 *
	 * Customizer control for a nav menu's name.
	 *
	 * @class    jk.customize.Menus.MenuNameControl
	 * @augments jk.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend(/** @lends jk.customize.Menus.MenuNameControl.prototype */{
	...
	/**
	 * jk.customize.Menus.MenuLocationsControl
	 *
	 * Customizer control for a nav menu's locations.
	 *
	 * @since 4.9.0
	 * @class    jk.customize.Menus.MenuLocationsControl
	 * @augments jk.customize.Control
	 */
	api.Menus.MenuLocationsControl = api.Control.extend(/** @lends jk.customize.Menus.MenuLocationsControl.prototype */{
	...
	/**
	 * jk.customize.Menus.MenuAutoAddControl
	 *
	 * Customizer control for a nav menu's auto add.
	 *
	 * @class    jk.customize.Menus.MenuAutoAddControl
	 * @augments jk.customize.Control
	 */
	api.Menus.MenuAutoAddControl = api.Control.extend(/** @lends jk.customize.Menus.MenuAutoAddControl.prototype */{
	...
	/**
	 * jk.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the JK_Menu_Customize_Control::$type
	 *
	 * @class    jk.customize.Menus.MenuControl
	 * @augments jk.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend(/** @lends jk.customize.Menus.MenuControl.prototype */{
		/**
		 * Set up the control.
		 */
		ready: function() {
		 ...
		 *
		 * Notice that the UI aspects here are handled by jkNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that jkNavMenu will be setting this .menu-item-data-parent-id input's value.
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						jk.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				jk.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				jk.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {jk.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @return {jk.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			jk.a11y.speak( api.Menus.data.l10n.itemAdded );
		 ...
		 * @since 4.9.0
		 *
		 * @param {jk.customize.controlConstructor.nav_menu_item[]} optionalMenuItemControls
		 */
	...
	/**
	 * Extends jk.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends jk.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends jk.customize.sectionConstructor with section constructor for menu.
	 */
	 ...
	 *
	 * @alias jk.customize.Menus.applySavedData
	 *
	 * @param {Object} data
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in JK_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @alias jk.customize.Menus.focusMenuItemControl
	 *
	 ...
	 *
	 * @alias jk.customize.Menus.getMenuControl
	 *
	 * @param menuId
	 * @return {jk.customize.controlConstructor.menus[]}
	 */
	 ...
	 *
	 * @alias jk.customize.Menus.getMenuItemControl
	 *
	 * @param {string} menuItemId
	 * @return {Object|null}
	 */
	...
	/**
	 * @alias jk.customize.Menus~menuItemIdToSettingId
	 *
	 * @param {string} menuItemId
	 */
	 ...
	 *
	 * @alias jk.customize.Menus~displayNavMenuName
	 *
	 * @param {string} name
	 * @return {string}
	 */
	function displayNavMenuName( name ) {
		name = name || '';
		name = jk.sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = name.toString().trim();
		return name || api.Menus.data.l10n.unnamed;
	}

})( jk.customize, jk, jQuery );
