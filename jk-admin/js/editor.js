...
( function( $, jk ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'jk-switch-editor' ) ) {
						id = target.attr( 'data-jk-editor-id' );
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = jk.shortcode.regexp( allShortcodes.join( '|' ) ),
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#jk-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#jk-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viewport (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.jk_autoresize_on ) {
		 ...
		 * `jk-includes/js/tinymce/plugins/wordpress/plugin.js`.
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<jk-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<jk-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<jk-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<jk-preserve>/g, function() {
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<jk-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<jk-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		jk.editor.autop = wpautop;
		jk.editor.removep = pre_wpautop;
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with jk_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Text editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, jk-before-tinymce-init and jk-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * }
	 */
	jk.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! jk.editor.getDefaultSettings ) {
			return;
		}

		defaults = jk.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'jk-core-ui jk-editor-wrap tmce-active',
					id: 'jk-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="jk-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-jk-editor-id': id
				} );

			var $editorTools = $( '<div class="jk-editor-tools">' );

			if ( settings.mediaButtons ) {
				var buttonText = 'Add Media';

				if ( window._jkMediaViewsL10n && window._jkMediaViewsL10n.addMedia ) {
					buttonText = window._jkMediaViewsL10n.addMedia;
				}

				var $addMediaButton = $( '<button type="button" class="button insert-media add_media">' );

				$addMediaButton.append( '<span class="jk-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="jk-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="jk-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'jk-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'jk-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'jk-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'jk-before-quicktags-init', init );
			window.quicktags( init );

			if ( ! window.jkActiveEditor ) {
				window.jkActiveEditor = init.id;
			}
		}
	};
	 ...
	 *
	 * Intended for use with editors that were initialized with jk.editor.initialize().
	 *
	 * @since 4.8.0
	 *
	 * @param {string} id The HTML id of the editor textarea.
	 */
	jk.editor.remove = function( id ) {
	 ...
	 */
	jk.editor.getContent = function( id ) {
		var editor;
