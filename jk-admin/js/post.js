...
( function( $ ) {
		...
		get : function(total, num) {
			...
			$.post(
				...
				function(r) {
					...
					$('#the-comment-list').append('<tr><td colspan="2">'+jkAjax.broken+'</td></tr>');
				}
			);
	 ...
	 * @global
	 */
	window.JKSetThumbnailHTML = function(html){
	 ...
	 * @global
	 */
	window.JKSetThumbnailID = function(id){
	 ...
	 * @global
	 */
	window.JKRemoveThumbnail = function(nonce){
		$.post(
			ajaxurl, {
		...
		if ( data['jk-refresh-post-lock'] ) {
			...
			if ( received.lock_error ) {
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					...
					wrap.find('.jk-tab-first').trigger( 'focus' );
				}
...
(function($) {
	...
	$( function() {
		schedule();
	}).on( 'heartbeat-send.jk-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#jk-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_jknonce').val() ) {
				data['jk-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.jk-refresh-nonces', function( e, data ) {
		var nonces = data['jk-refresh-post-nonces'];
...
jQuery( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('jk-tab-first') && e.shiftKey ) {
			$(this).find('.jk-tab-last').trigger( 'focus' );
			e.preventDefault();
		// [Tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('jk-tab-last') && ! e.shiftKey ) {
			$(this).find('.jk-tab-first').trigger( 'focus' );
			e.preventDefault();
		}
	}).filter(':visible').find('.jk-tab-first').trigger( 'focus' );

	// Set the heartbeat interval to 10 seconds if post lock dialogs are enabled.
	if ( jk.heartbeat && $('#post-lock-dialog').length ) {
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#jk-preview'),
			target = $this.attr('target') || 'jk-preview',
		...
		if ( jk.autosave ) {
			jk.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! jk.heartbeat || ! jk.heartbeat.hasConnectionError() ) {
	...
	$( window ).on( 'beforeunload.edit-post', function( event ) {
		...
		if ( jk.autosave ) {
			changed = jk.autosave.server.postChanged();
		} else if ( editor ) {
		...
		var data = {
			action: 'jk-remove-post-lock',
			_jknonce: $('#_jknonce').val(),
			post_ID: postID,
	...
	$('.categorydiv').each( function(){
		...
		// @todo Move to jQuery 1.3+, support for multiple hierarchical taxonomies, see jk-lists.js.
		...
		$('#' + taxonomy + '-add-toggle').on( 'click', function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'jk-hidden-children' );
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			$('#post-status-display').text(
				...
				jk.sanitize.stripTagsAndEncodeText( $('option:selected', postStatus).text() )
			);
		...
		buttons.children( '.save' ).on( 'click', function() {
			...
			$.post(
				...
				function(data) {
					...
					jk.a11y.speak( __( 'Permalink saved' ) );
				}
			);
		});
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'jk-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#jk-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.jk-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.jk-editor-resize', dragging )
				.on( 'mouseup.jk-editor-resize mouseleave.jk-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.jk-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.jk-autosave', function( event ) {
		// Key [S] has code 83.
		if ( event.which === 83 ) {
			if (
				event.shiftKey ||
			...
			jk.autosave && jk.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'jk-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#jk-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new jk.utils.WordCounter() );
