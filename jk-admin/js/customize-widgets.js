/**
 * @output jk-admin/js/customize-widgets.js
 */

/* global _jkCustomizeWidgetsSettings */
(function( jk, $ ){
	...
	/**
	 * @namespace jk.customize.Widgets
	 */
	api.Widgets = api.Widgets || {};
	api.Widgets.savedWidgetIds = {};

	// Link settings.
	api.Widgets.data = _jkCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;

	/**
	 * jk.customize.Widgets.WidgetModel
	 *
	 * A single widget model.
	 *
	 * @class    jk.customize.Widgets.WidgetModel
	 * @augments Backbone.Model
	 */
	api.Widgets.WidgetModel = Backbone.Model.extend(/** @lends jk.customize.Widgets.WidgetModel.prototype */{
		id: null,
		temp_id: null,
		classname: null,
		control_tpl: null,
	...
	/**
	 * jk.customize.Widgets.WidgetCollection
	 *
	 * Collection for widget models.
	 *
	 * @class    jk.customize.Widgets.WidgetCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.WidgetCollection = Backbone.Collection.extend(/** @lends jk.customize.Widgets.WidgetCollection.prototype */{
	...
	/**
	 * jk.customize.Widgets.SidebarModel
	 *
	 * A single sidebar model.
	 *
	 * @class    jk.customize.Widgets.SidebarModel
	 * @augments Backbone.Model
	 */
	api.Widgets.SidebarModel = Backbone.Model.extend(/** @lends jk.customize.Widgets.SidebarModel.prototype */{
		after_title: null,
		after_widget: null,
		before_title: null,
		before_widget: null,
	...
	/**
	 * jk.customize.Widgets.SidebarCollection
	 *
	 * Collection for sidebar models.
	 *
	 * @class    jk.customize.Widgets.SidebarCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.SidebarCollection = Backbone.Collection.extend(/** @lends jk.customize.Widgets.SidebarCollection.prototype */{
	...
	api.Widgets.AvailableWidgetsPanelView = jk.Backbone.View.extend(/** @lends jk.customize.Widgets.AvailableWidgetsPanelView.prototype */{
		 ...
		 *
		 * @constructs jk.customize.Widgets.AvailableWidgetsPanelView
	 ...
	 *
	 * @alias    jk.customize.Widgets.formSyncHandlers
	 */
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends jk.customize.Widgets.WidgetControl.prototype */{
		...
		/**
		 * jk.customize.Widgets.WidgetControl
		 *
		 * Customizer control for widgets.
		 * Note that 'widget_form' must match the JK_Widget_Form_Customize_Control::$type
		 *
		 * @since 4.1.0
		 *
		 * @constructs jk.customize.Widgets.WidgetControl
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.jk-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						jk.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						jk.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // jk_widget_control() outputs the button as "Delete".
		...
		/**
		 * @return {jk.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( jk.ajax.settings.url, data );
	...
	/**
	 * jk.customize.Widgets.WidgetsPanel
	 *
	 * Customizer panel containing the widget area sections.
	 *
	 * @since 4.4.0
	 *
	 * @class    jk.customize.Widgets.WidgetsPanel
	 * @augments jk.customize.Panel
	 */
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends jk.customize.Widgets.WigetsPanel.prototype */{
	...
	/**
	 * jk.customize.Widgets.SidebarSection
	 *
	 * Customizer section representing a widget area widget
	 *
	 * @since 4.1.0
	 *
	 * @class    jk.customize.Widgets.SidebarSection
	 * @augments jk.customize.Section
	 */
	api.Widgets.SidebarSection = api.Section.extend(/** @lends jk.customize.Widgets.SidebarSection.prototype */{
	...
	/**
	 * jk.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the JK_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @class    jk.customize.Widgets.SidebarControl
	 * @augments jk.customize.Control
	 */
	api.Widgets.SidebarControl = api.Control.extend(/** @lends jk.customize.Widgets.SidebarControl.prototype */{
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[jk_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[jk_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[jk_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				jk.a11y.speak( l10n.reorderModeOn );
				...
				jk.a11y.speak( l10n.reorderModeOff );
		 ...
		 *
		 * @since 3.9.0
		 * @return {jk.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo This can use widgetIdToSettingId(), then pass into jk.customize.control( x ).getSidebarWidgetsControl().
	 ...
	 *
	 * @param {jk.customize.Section|jk.customize.Panel|jk.customize.Control} focusConstruct - The object to initially focus.
	 * @param {jk.customize.Section|jk.customize.Panel|jk.customize.Control} returnConstruct - The object to return focus.
	 */
