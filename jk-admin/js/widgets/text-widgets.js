/**
 * @output jk-admin/js/widgets/text-widgets.js
 */
...
/**
 * @namespace jk.textWidgets
 */
jk.textWidgets = ( function( $ ) {
	'use strict';
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends jk.textWidgets.TextWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs jk.textWidgets.TextWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( jk.template( 'widget-text-control-fields' ) );

			control.customHtmlWidgetPointer = control.$el.find( '.jk-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.jk-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				jk.ajax.post( 'dismiss-jk-pointer', {
					pointer: pointer
				});
		...
		openAvailableWidgetsPanel: function openAvailableWidgetsPanel() {
			...
			jk.customize.section.each( function( section ) {
				if ( section.extended( jk.customize.Widgets.SidebarSection ) && section.expanded() ) {
					sidebarControl = jk.customize.control( 'sidebars_widgets[' + section.params.sidebarId + ']' );
				}
			});
			if ( ! sidebarControl ) {
				return;
			...
			setTimeout( function() { // Timeout to prevent click event from causing panel to immediately collapse.
				jk.customize.Widgets.availableWidgetsPanel.open( sidebarControl );
				jk.customize.Widgets.availableWidgetsPanel.$search.val( 'HTML' ).trigger( 'keyup' );
			});
		},
		...
		updateFields: function updateFields() {
			...
			if ( control.fields.text.is( ':visible' ) ) {
				...
				control.editor.setContent( jk.oldEditor.autop( syncInput.val() ) );
			}
		},
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				var updateWidgetBuffer = 300; // See jk.customize.Widgets.WidgetControl._setupUpdateUI() which uses 250ms for updateWidgetDebounced.
				if ( control.editor.isDirty() ) {
					 ...
					 * having to make server round-trips to call the respective JK_Widget::update()
					 * callbacks. See <https://core.trac.wordpress.org/ticket/33507>.
					 */
					if ( jk.customize && jk.customize.state ) {
						jk.customize.state( 'processing' ).set( jk.customize.state( 'processing' ).get() + 1 );
						_.delay( function() {
							jk.customize.state( 'processing' ).set( jk.customize.state( 'processing' ).get() - 1 );
						}, updateWidgetBuffer );
					}
			...
			function buildEditor() {
				...
				if ( tinymce.get( id ) ) {
					...
					jk.oldEditor.remove( id );
				}

				// Add or enable the `wpview` plugin.
				$( document ).one( 'jk-before-tinymce-init.text-widget-init', function( event, init ) {
				...
				showPointerElement = function( pointerElement ) {
					...
					jk.a11y.speak( pointerElement.find( 'h3, p' ).map( function() {
						return $( this ).text();
					} ).get().join( '\n\n' ) );
				};
	 ...
	 *
	 * @memberOf jk.textWidgets
	 *
	 * @type {Object.<string, jk.textWidgets.TextWidgetControl>}
	 */
	 ...
	 * attached which calls jk.textWidgets.init().
	 *
	 * @memberOf jk.textWidgets
	 *
	 * @return {void}
