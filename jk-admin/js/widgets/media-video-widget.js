/**
 * @output jk-admin/js/widgets/media-video-widget.js
 */
...
(function( component ) {
	 ...
	 *
	 * @class    jk.mediaWidgets.controlConstructors~VideoDetailsMediaFrame
	 * @augments jk.media.view.MediaFrame.VideoDetails
	 *
	 * @private
	 */
	VideoDetailsMediaFrame = jk.media.view.MediaFrame.VideoDetails.extend(/** @lends jk.mediaWidgets.controlConstructors~VideoDetailsMediaFrame.prototype */{
		...
		createStates: function createStates() {
			this.states.add([
				...
				new jk.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: jk.media.view.l10n.videoAddSourceTitle,
				...
				new jk.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: jk.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See JK_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    jk.mediaWidgets.modelConstructors.media_video
	 ...
	 * See JK_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    jk.mediaWidgets.controlConstructors.media_video
	 * @augments jk.mediaWidgets.MediaWidgetControl
	 */
	VideoWidgetControl = component.MediaWidgetControl.extend(/** @lends jk.mediaWidgets.controlConstructors.media_video.prototype */{
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = jk.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( jk.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( jk.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = jk.template( 'jk-media-widget-video-preview' );
