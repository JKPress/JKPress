/**
 * @output jk-admin/js/widgets/media-widgets.js
 */

/* eslint consistent-this: [ "error", "control" ] */

/**
 * @namespace jk.mediaWidgets
 * @memberOf  jk
 */
jk.mediaWidgets = ( function( $ ) {
	'use strict';
	 ...
	 *
	 * @memberOf jk.mediaWidgets
	 *
	 * @type {Object.<string, jk.mediaWidgets.MediaWidgetModel>}
	 */
	...
	component.PersistentDisplaySettingsLibrary = jk.media.controller.Library.extend(/** @lends jk.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */{

		/**
		 * Library which persists the customized display settings across selections.
		 *
		 * @constructs jk.mediaWidgets.PersistentDisplaySettingsLibrary
		...
		initialize: function initialize( options ) {
			...
			jk.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = jk.media.controller.Library.prototype.display.call( this, model );
	 ...
	 *
	 * @class    jk.mediaWidgets.MediaEmbedView
	 * @augments jk.media.view.Embed
	 */
	component.MediaEmbedView = jk.media.view.Embed.extend(/** @lends jk.mediaWidgets.MediaEmbedView.prototype */{
		...
		initialize: function( options ) {
			...
			jk.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {jk.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
		refresh: function refresh() {
			/**
			 * @class jk.mediaWidgets~Constructor
			 */
			var Constructor;
			...
			if ( 'image' === this.controller.options.mimeType ) {
				...
				Constructor = jk.media.view.EmbedLink.extend(/** @lends jk.mediaWidgets~Constructor.prototype */{
					...
					updateoEmbed: function() {
						...
						jk.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! jk.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== jk.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = jk.apiRequest({
	 ...
	 *
	 * @class    jk.mediaWidgets.MediaFrameSelect
	 * @augments jk.media.view.MediaFrame.Post
	 */
	component.MediaFrameSelect = jk.media.view.MediaFrame.Post.extend(/** @lends jk.mediaWidgets.MediaFrameSelect.prototype */{
		...
		createStates: function createStates() {
			...
			_.each( jk.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new jk.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new jk.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {jk.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {jk.Backbone.View} view - Toolbar view.
		 * @this {jk.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {jk.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {jk.Backbone.View} toolbar - Toolbar view.
		 * @this {jk.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Embed content.
		 *
		 * Forked override of {jk.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends jk.mediaWidgets.MediaWidgetControl.prototype */{

		/**
		 * Translation strings.
		 *
		 * The mapping of translation strings is handled by media widget subclasses,
		 * exported from PHP to JS such as is done in JK_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in JK_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in JK_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		 ...
		 *
		 * @constructs jk.mediaWidgets.MediaWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new jk.media.model.Attachment();
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( jk.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new jk.media.model.Attachment({
		...
		template: function template() {
			...
			return jk.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new jk.media.model.Selection( selectionModels, { multiple: false } );
			...
			jk.media.frame = mediaFrame; // See jk.media().
			...
			defaultSync = jk.media.model.Attachment.prototype.sync;
			jk.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				jk.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {jk.media.view.MediaFrame.Select} mediaFrame - Select frame.
		 * @return {Object} Props.
		 */
	 ...
	 * Media widget model.
	 *
	 * @class    jk.mediaWidgets.MediaWidgetModel
	 * @augments Backbone.Model
	 */
	component.MediaWidgetModel = Backbone.Model.extend(/** @lends jk.mediaWidgets.MediaWidgetModel.prototype */{
		 ...
		 * @return {jk.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 * @memberOf jk.mediaWidgets
	 *
	 * @type {Object.<string, jk.mediaWidgets.MediaWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of JK_Widget::form() which is
	 ...
	 * When JKPress enqueues this script, it should have an inline script
	 * attached which calls jk.mediaWidgets.init().
	 *
	 * @memberOf jk.mediaWidgets
	 *
	 * @return {void}
