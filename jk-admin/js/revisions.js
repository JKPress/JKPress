 ...
 *
 * @output jk-admin/js/revisions.js
 */
...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.jk.revisions.
	 */
	revisions = jk.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._jkRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			jk.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				jk.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = jk.ajax.send( options ),
	...
	/**
	 * jk.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see jk.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = jk.Backbone.View.extend({
		className: 'revisions',
		template: jk.template('revisions-frame'),
		...
		render: function() {
			jk.Backbone.View.prototype.render.apply( this, arguments );
	...
	revisions.view.Controls = jk.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.jk.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.jk.revisions.pinning click.jk.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.jk.revisions.pinning');
	...
	// The tickmarks view.
	revisions.view.Tickmarks = jk.Backbone.View.extend({
	...
	// The metabox view.
	revisions.view.Metabox = jk.Backbone.View.extend({
	...
	revisions.view.Meta = jk.Backbone.View.extend({
		template: jk.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = jk.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: jk.template('revisions-checkbox'),
	...
	// The slider visually hidden help view.
 	revisions.view.SliderHelp = jk.Backbone.View.extend({
		className: 'revisions-slider-hidden-help',
		template:  jk.template( 'revisions-slider-hidden-help' )
	});
   
	// The tooltip view.
	// Encapsulates the tooltip.
	revisions.view.Tooltip = jk.Backbone.View.extend({
	...
	revisions.view.Buttons = jk.Backbone.View.extend({
		className: 'revisions-buttons',
		template: jk.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = jk.Backbone.View.extend({
		className: 'jk-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.jk.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.jk.revisions');
	...
	revisions.view.Diff = jk.Backbone.View.extend({
		className: 'revisions-diff',
		template:  jk.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// jk_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// jk_localize_script does not allow for top-level booleans so do a comparator here.
