/**
 * Interactions used by the User Privacy tools in JKPress.
 *
 * @output jk-admin/js/privacy-tools.js
 */
			...
			$.ajax(
				{
					url: window.ajaxurl,
					data: {
						action: 'jk-privacy-export-personal-data',
	...
	$( '.remove-personal-data-handle' ).on( 'click', function( event ) {
		...
		function doNextErasure( eraserIndex, pageIndex ) {
			$.ajax({
				url: window.ajaxurl,
				data: {
					action: 'jk-privacy-erase-personal-data',
					eraser: eraserIndex,
					id: requestID,
					page: pageIndex,
					security: nonce
		...
		if ( $target.is( 'button.privacy-text-copy' ) ) {
			...
			if ( $parent.length ) {
				try {
					...
					jk.a11y.speak( __( 'The suggested policy text has been copied to your clipboard.' ) );
