/**
 * @output jk-admin/js/editor-expand.js
 */
...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#jk-content-wrap' ),
			$tools = $( '#jk-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.jk.editor && window.jk.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.jk.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.jk && editor.jk.getView && ( view = editor.jk.getView( node ) ) ) {
			 ...
			 * Executes the jkAutoResize on the active editor.
			 *
			 * @since 4.0.0
			 *
			 * @return {void}
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'jkAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'jk-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent jk-autoresize jk-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'jk-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent jk-autoresize jk-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'jk-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .jk-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'jk-editor-expand' );
			 ...
			 */
			$document.on( 'jk-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'jk-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'jkAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.jk_autoresize_on = true;
				mceEditor.execCommand( 'jkAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'jk-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.jk_autoresize_on = false;
				mceEditor.execCommand( 'jkAutoResizeOff' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'jkToggleDFW', toggle );
			editor.addShortcut( 'access+w', '', 'jkToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'jk-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'jk-autoresize', recalcEditorRect );
				};
