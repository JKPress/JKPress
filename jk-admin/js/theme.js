/**
 * @output jk-admin/js/theme.js
 */

/* global _jkThemeSettings, confirm, tb_position */
window.jk = window.jk || {};
...
// Setup app structure.
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: jk.template });
...
themes.view.Appearance = jk.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.find( '.search-box' )
			.append( $.parseHTML( '<label for="jk-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
...
themes.Collection = Backbone.Collection.extend({
	...
	apiCall: function( request, paginated ) {
		return jk.ajax.send( 'query-themes', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 100
...
themes.view.Theme = jk.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'jk-theme-install-success', function( event, response ) {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'jk-theme-update-success', function( event, response ) {
...
themes.view.Details = jk.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.jk-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'jk-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'jk-auto-update-setting-changed', callback );
	},
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( jk.themes.data.settings.confirmDelete ) ) {
			return;
		}

		jk.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'jk-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'jk-full-overlay expanded',
	...
	togglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.jk-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'jk-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = jk.Backbone.View.extend({

	className: 'themes jk-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( jk.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			jk.a11y.speak( l10n.noThemesFound );
		} else {
			jk.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = jk.Backbone.View.extend({

	tagName: 'input',
	className: 'jk-filter-search',
	id: 'jk-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.jk-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.jk-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	applyFilters: function( event ) {
		...
		if ( ! tags ) {
			jk.a11y.speak( l10n.selectFeatureFilter );
			return;
		}
	...
	// Save the user's JKPress.org username and get his favorite themes.
	saveUsername: function ( event ) {
		...
		return jk.ajax.send( 'save-wporg-username', {
			data: {
				_jknonce: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `jk.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.jk-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
...
$( function() {
	...
	$( document.body ).on( 'click', '.load-customize', function() {
		...
		urlParser.search = $.param( _.extend(
			jk.customize.utils.parseQueryString( urlParser.search.substr( 1 ) ),
			{
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _jkThemeSettings.settings.confirmDelete );
	});
});
