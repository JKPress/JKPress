/**
 * Interactions used by the Site Health modules in JKPress.
 *
 * @output jk-admin/js/site-health.js
 */

/* global ajaxurl, ClipboardJS, SiteHealth, jk */
...
jQuery( function( $ ) {
	...
	var __ = jk.i18n.__,
		...
		pathsSizesSection = $( '#health-check-accordion-block-jk-paths-sizes' ),
	...
	clipboard.on( 'success', function( e ) {
		...
		jk.a11y.speak( __( 'Site information has been copied to your clipboard.' ) );
	} );
	...
	function appendIssue( issue ) {
		var template = jk.template( 'health-check-issue' ),
		...
		if ( isStatusTab ) {
			$.post(
				ajaxurl,
				{
					...
					'_jknonce': SiteHealth.nonce.site_status_result,
		...
		if ( 1 <= SiteHealth.site_status.async.length ) {
			$.each( SiteHealth.site_status.async, function() {
				var data = {
					...
					'_jknonce': SiteHealth.nonce.site_status
				};
				...
				if ( 'undefined' !== typeof( this.has_rest ) && this.has_rest ) {
					jk.apiRequest( {
						url: jk.url.addQueryArgs( this.test, { _locale: 'user' } ),
						headers: this.headers
					} )
						.done( function( response ) {
							/** This filter is documented in jk-admin/includes/class-jk-site-health.php */
							appendIssue( jk.hooks.applyFilters( 'site_status_test_result', response ) );
						} )
					...
					$.post(
						...
						/** This filter is documented in jk-admin/includes/class-jk-site-health.php */
						appendIssue( jk.hooks.applyFilters( 'site_status_test_result', response.data ) );
					} ).fail( function( response ) {
						var description;
	...
	function addFailedSiteHealthCheckNotice( url, description ) {
		...
		/** This filter is documented in jk-admin/includes/class-jk-site-health.php */
		appendIssue( jk.hooks.applyFilters( 'site_status_test_result', issue ) );
	}
	...
	function getDirectorySizes() {
		...
		jk.apiRequest( {
			path: '/jk-site-health/v1/directory-sizes'
		} ).done( function( response ) {
			updateDirSizes( response || {} );
		} ).always( function() {
			var delay = ( new Date().getTime() ) - timestamp;

			$( '.health-check-jk-paths-sizes.spinner' ).css( 'visibility', 'hidden' );
	...
	function announceTestsProgression( type ) {
		...
		switch ( type ) {
			case 'good':
				jk.a11y.speak( __( 'All site health tests have finished running. Your site is looking good.' ) );
				break;
			case 'improvable':
				jk.a11y.speak( __( 'All site health tests have finished running. There are items that should be addressed.' ) );
				break;
			case 'waiting-for-directory-sizes':
				jk.a11y.speak( __( 'Running additional tests... please wait.' ) );
				break;
			default:
				return;
		}
