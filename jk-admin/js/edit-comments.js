 ...
 * @since 2.1.0
 * @output jk-admin/js/edit-comments.js
 */

/* global adminCommentsSettings, thousandsSeparator, list_args, QTags, ajaxurl, jkAjax */
...
window.setCommentsList = function() {
	 ...
	 * @param {Object} settings Settings for the jkList object.
	 *
	 * @return {void}
	 */
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			jkListsData = $( settings.target ).attr( 'data-jk-lists' );
		...
		if ( jkListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( jkListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = jkListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_jknonce=' + settings.data._ajax_nonce);
			a.attr('data-jk-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		var total_items_i18n, total, animated, animatedCallback,
			...
			/*
			 * As `jkList` toggles only the `unapproved` class, the approved comment
		...
		theList.get(0).jkList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function() {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).jkList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See JK_Comments_List_Table::prepare_items() in class-jk-comments-list-table.php.
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).jkList.add( response.rows );
			}
		});
	};
	 ...
	 * @global
	 */
	window.theExtraList = $('#the-extra-comment-list').jkList( { alt: '', delColor: 'none', addColor: 'none' } );
	 ...
	 * @global
	 */
	window.theList = $('#the-comment-list').jkList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('jkListDelEnd', function(e, s){
			var jkListsData = $(s.target).attr('data-jk-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( jkListsData.indexOf(':trash=1') != -1 || jkListsData.indexOf(':spam=1') != -1 )
...
window.commentReply = {
	...
	show : function(xml) {
		...
		r = jkAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': jkAjax.broken});
			return false;
		}
...
$( function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'jk-toggle', 1 ).trigger( 'click' ).removeData( 'jk-toggle' );
		};
