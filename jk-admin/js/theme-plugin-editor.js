/**
 * @output jk-admin/js/theme-plugin-editor.js
 */
...
jk.themePluginEditor = (function( $ ) {
	'use strict';
	var component, TreeLinks,
		__ = jk.i18n.__, _n = jk.i18n._n, sprintf = jk.i18n.sprintf;
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = jk.template( 'jk-file-editor-notice' );
		...
		if ( false !== component.codeEditor ) {
			/*
			 * Defer adding notices until after DOM ready as workaround for JK Admin injecting
	...
	component.showWarning = function() {
		...
		setTimeout( function() {
			jk.a11y.speak( jk.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.submit = function( event ) {
		...
		request = jk.ajax.post( 'edit-theme-plugin-file', data );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = jk.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
 ...
 * @type {object}
 */
jk.themePluginEditor.l10n = jk.themePluginEditor.l10n || {
	saveAlert: '',
	saveError: '',
	lintError: {
		alternative: 'jk.i18n',
...
jk.themePluginEditor.l10n = window.jk.deprecateL10nObject( 'jk.themePluginEditor.l10n', jk.themePluginEditor.l10n, '5.5.0' );
