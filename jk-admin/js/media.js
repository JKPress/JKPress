...
/* global ajaxurl, _jkMediaGridSettings, showNotice, findPosts, ClipboardJS */
...
( function( $ ){
	window.findPosts = {
		...
		send: function() {
			...
			$.ajax( ajaxurl, {
				...
				if ( ! x.success ) {
					$( '#find-posts-response' ).text( jk.i18n.__( 'An error has occurred. Please reload the page and try again.' ) );
				}
	...
	$( function() {
		var settings,
			$mediaGridWrap             = $( '#jk-media-grid' ),
			copyAttachmentURLClipboard = new ClipboardJS( '.copy-attachment-url.media-library' ),
			copyAttachmentURLSuccessTimeout,
			previousSuccessElement = null;

		// Opens a manage media frame into the grid.
		if ( $mediaGridWrap.length && window.jk && window.jk.media ) {
			settings = _jkMediaGridSettings;

			var frame = window.jk.media({
				frame: 'manage',
			...
			$mediaGridWrap.trigger( 'jk-media-grid-ready', frame );
		}
		...
		copyAttachmentURLClipboard.on( 'success', function( event ) {
			...
			jk.a11y.speak( jk.i18n.__( 'The file URL has been copied to your clipboard' ) );
		} );
	});
})( jQuery );
