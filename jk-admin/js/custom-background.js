/**
 * @output jk-admin/js/custom-background.js
 */
...
(function($) {
	$( function() {
		...
		/**
		 * Instantiates the JKPress color picker and binds the change and clear events.
		 *
		 * @since 3.5.0
		 *
		 * @return {void}
		 */
		$('#background-color').jkColorPicker({
		...
		/**
		 * Binds the event for opening the JK Media dialog.
		 *
		 * @since 3.5.0
		 *
		 * @return {void}
		...
		$('#choose-from-library-link').on( 'click', function( event ) {
			...
			frame = jk.media.frames.customBackground = jk.media({
