/**
 * @output jk-admin/js/auth-app.js
 */
...
( function( $, authApp ) {
	...
	$approveBtn.on( 'click', function( e ) {
		 ...
		 */
		request = jk.hooks.applyFilters( 'jk_application_passwords_approve_app_request', request, context );

		jk.apiRequest( {
			path: '/jk/v2/users/me/application-passwords?_locale=user',
			method: 'POST',
			data: request
			 ...
			 *
			 * In most cases, this should be used in combination with the {@see 'jk_authorize_application_password_form_approved_no_js'}
			 ...
			 */
			jk.hooks.doAction( 'jk_application_passwords_approve_app_request_success', response, textStatus, jqXHR );
			...
			if ( raw ) {
				...
				message = jk.i18n.sprintf(
					/* translators: %s: Application name. */
					'<label for="new-application-password-value">' + jk.i18n.__( 'Your new password for %s is:' ) + '</label>',
					'<strong></strong>'
				...
				$notice = $( '<div></div>' )
					...
					.append( '<p>' + jk.i18n.__( 'Be sure to save this in a safe location. You will not be able to retrieve it.' ) + '</p>' );
			 ...
			 * @param {jqXHR}       jqXHR       The underlying jqXHR object that made the request.
			 */
			jk.hooks.doAction( 'jk_application_passwords_approve_app_request_error', error, textStatus, errorThrown, jqXHR );
		} );
	} );
	...
	$rejectBtn.on( 'click', function( e ) {
		 ...
		 * @param {string} context.rejectUrl  The URL the user will be redirected to after rejecting the request.
		 */
		jk.hooks.doAction( 'jk_application_passwords_reject_app', context );
