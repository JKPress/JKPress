/**
 * Functions for ajaxified updates, deletions and installs inside the JKPress admin.
 *
 * @version 4.2.0
 * @output jk-admin/js/updates.js
 */

/* global pagenow, _jkThemeSettings */

/**
 * @param {jQuery}  $                                        jQuery object.
 * @param {object}  jk                                       JK object.
 * @param {object}  settings                                 JK Updates settings.
...
(function( $, jk, settings ) {
	...
	/**
	 * The JK Updates object.
	 *
	 * @since 4.2.0
	 *
	 * @namespace jk.updates
	 */
	jk.updates = {};
	...
	jk.updates.l10n = window.jk.deprecateL10nObject( 'jk.updates.l10n', jk.updates.l10n, '5.5.0' );
	 ...
	 * @type {string}
	 */
	jk.updates.ajaxNonce = settings.ajax_nonce;
	 ...
	 * @type {number}
	 */
	jk.updates.searchMinCharacters = 2;
	 ...
	 * @type {bool}
	 */
	jk.updates.shouldRequestFilesystemCredentials = false;
	 ...
	 *                                                             Default 'false'.
	 */
	jk.updates.filesystemCredentials = {
		ftp:       {
			host:           '',
			username:       '',
			password:       '',
	 ...
	 * @type {function}
	 */
	jk.updates.adminNotice = jk.template( 'jk-updates-admin-notice' );
	 ...
	 * @type {jQuery}
	 */
	jk.updates.$elToReturnFocusToFromCredentialsModal = undefined;
	 ...
	 *
	 */
	jk.updates.addAdminNotice = function( data ) {
		var $notice = $( data.selector ),
			$headerEnd = $( '.jk-header-end' ),
			$adminNotice;

		delete data.selector;
		$adminNotice = jk.updates.adminNotice( data );
		...
		$document.trigger( 'jk-updates-notice-added' );
	};

	/**
	 * Handles Ajax requests to JKPress.
	 *
	 * @since 4.6.0
	 *
	 ...
	 */
	jk.updates.ajax = function( action, data ) {
		var options = {};

		if ( jk.updates.ajaxLocked ) {
			jk.updates.queue.push( {
				action: action,
				data:   data
			} );
		...
		options.data = _.extend( data, {
			action:          action,
			_ajax_nonce:     jk.updates.ajaxNonce,
			_fs_nonce:       jk.updates.filesystemCredentials.fsNonce,
			username:        jk.updates.filesystemCredentials.ftp.username,
			password:        jk.updates.filesystemCredentials.ftp.password,
			hostname:        jk.updates.filesystemCredentials.ftp.hostname,
			connection_type: jk.updates.filesystemCredentials.ftp.connectionType,
			public_key:      jk.updates.filesystemCredentials.ssh.publicKey,
			private_key:     jk.updates.filesystemCredentials.ssh.privateKey
		} );

		return jk.ajax.send( options ).always( jk.updates.ajaxAlways );
	};
	 ...
	 */
	jk.updates.ajaxAlways = function( response ) {
		if ( ! response.errorCode || 'unable_to_connect_to_filesystem' !== response.errorCode ) {
			jk.updates.ajaxLocked = false;
			jk.updates.queueChecker();
		}
		...
		if ( 'undefined' !== typeof response.debug && window.console && window.console.log ) {
			_.map( response.debug, function( message ) {
				...
				window.console.log( jk.sanitize.stripTagsAndEncodeText( message ) );
			} );
		}
	};
	 ...
	 * @since 4.7.0
	 */
	jk.updates.refreshCount = function() {
		var $adminBarUpdates              = $( '#jk-admin-bar-updates' ),
	 ...
	 */
	jk.updates.setCardButtonStatus = function( data ) {
	 ...
	 *                      Can be 'plugin', 'theme'.
	 */
	jk.updates.decrementCount = function( type ) {
	 ...
	 * @param {updatePluginSuccess=} args.success Optional. Success callback. Default: jk.updates.updatePluginSuccess
	 * @param {updatePluginError=}   args.error   Optional. Error callback. Default: jk.updates.updatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.updatePlugin = function( args ) {
		var $updateRow, $card, $message, message,
			$adminBarUpdates = $( '#jk-admin-bar-updates' ),
			buttonText = __( 'Updating...' ),
			isPluginInstall = 'plugin-install' === pagenow || 'plugin-install-network' === pagenow;

		args = _.extend( {
			success: jk.updates.updatePluginSuccess,
			error: jk.updates.updatePluginError
		}, args );
		...
		$document.trigger( 'jk-plugin-updating', args );

		if ( isPluginInstall && 'plugin-information-footer' === $card.attr( 'id' ) ) {
			jk.updates.setCardButtonStatus(
				{
		...
		return jk.updates.ajax( 'update-plugin', args );
	};
	 ...
	 */
	jk.updates.updatePluginSuccess = function( response ) {
		...
		jk.a11y.speak( __( 'Update completed successfully.' ) );

		if ( 'plugin_install_from_iframe' !== $updateMessage.attr( 'id' ) ) {
			jk.updates.decrementCount( 'plugin' );
		} else {
			jk.updates.setCardButtonStatus(
				{
		...
		$document.trigger( 'jk-plugin-update-success', response );
	};
	 ...
	 */
	jk.updates.updatePluginError = function( response ) {
		var $pluginRow, $card, $message, errorMessage, buttonText, ariaLabel,
			$adminBarUpdates = $( '#jk-admin-bar-updates' );

		if ( ! jk.updates.isValidResponse( response, 'update' ) ) {
			return;
		}

		if ( jk.updates.maybeHandleCredentialError( response, 'update-plugin' ) ) {
			return;
		}
		...
		jk.a11y.speak( errorMessage, 'assertive' );
		...
		$document.trigger( 'jk-plugin-update-error', response );
	};
	 ...
	 *
	 * @param {Object}                args         Arguments.
	 * @param {string}                args.slug    Plugin identifier in the JKPress.org Plugin repository.
	 * @param {installPluginSuccess=} args.success Optional. Success callback. Default: jk.updates.installPluginSuccess
	 * @param {installPluginError=}   args.error   Optional. Error callback. Default: jk.updates.installPluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.installPlugin = function( args ) {
		...
		args = _.extend( {
			success: jk.updates.installPluginSuccess,
			error: jk.updates.installPluginError
		}, args );
		...
		jk.a11y.speak( __( 'Installing... please wait.' ) );

		// Remove previous error messages, if any.
		$card.removeClass( 'plugin-card-install-failed' ).find( '.notice.notice-error' ).remove();

		$document.trigger( 'jk-plugin-installing', args );
		...
		return jk.updates.ajax( 'install-plugin', args );
	};
	 ...
	 */
	jk.updates.installPluginSuccess = function( response ) {
		...
		jk.a11y.speak( __( 'Installation completed successfully.' ) );

		$document.trigger( 'jk-plugin-install-success', response );

		if ( response.activateUrl ) {
			setTimeout( function() {
				jk.updates.checkPluginDependencies( {
					slug: response.slug
				} );
			}, 1000 );
		}
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	jk.updates.installPluginError = function( response ) {
		...
		if ( ! jk.updates.isValidResponse( response, 'install' ) ) {
			return;
		}

		if ( jk.updates.maybeHandleCredentialError( response, 'install-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'jk-plugin-install-error', response );
	};
	 ...
	 * @param {Object}                          args         Arguments.
	 * @param {string}                          args.slug    Plugin identifier in the JKPress.org Plugin repository.
	 * @param {checkPluginDependenciesSuccess=} args.success Optional. Success callback. Default: jk.updates.checkPluginDependenciesSuccess
	 * @param {checkPluginDependenciesError=}   args.error   Optional. Error callback. Default: jk.updates.checkPluginDependenciesError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.checkPluginDependencies = function( args ) {
		args = _.extend( {
			success: jk.updates.checkPluginDependenciesSuccess,
			error: jk.updates.checkPluginDependenciesError
		}, args );

		jk.a11y.speak( __( 'Checking plugin dependencies... please wait.' ) );
		$document.trigger( 'jk-checking-plugin-dependencies', args );

		return jk.updates.ajax( 'check_plugin_dependencies', args );
	};
	 ...
	 */
	jk.updates.checkPluginDependenciesSuccess = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.install-now' ),
		...
		jk.a11y.speak( __( 'Plugin dependencies check completed successfully.' ) );
		$document.trigger( 'jk-check-plugin-dependencies-success', response );
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	jk.updates.checkPluginDependenciesError = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.install-now' ),
			buttonText = _x( 'Activate', 'plugin' ),
			ariaLabel = sprintf(
		...
		if ( ! jk.updates.isValidResponse( response, 'check-dependencies' ) ) {
			return;
		}
		...
		jk.a11y.speak( errorMessage, 'assertive' );
		$document.trigger( 'jk-check-plugin-dependencies-error', response );
	 ...
	 * @param {activatePluginSuccess=} args.success Optional. Success callback. Default: jk.updates.activatePluginSuccess
	 * @param {activatePluginError=}   args.error   Optional. Error callback. Default: jk.updates.activatePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.activatePlugin = function( args ) {
		var $message = $( '.plugin-card-' + args.slug + ', #plugin-information-footer' ).find( '.activate-now, .activating-message' );

		args = _.extend( {
			success: jk.updates.activatePluginSuccess,
			error: jk.updates.activatePluginError
		}, args );

		jk.a11y.speak( __( 'Activating... please wait.' ) );
		$document.trigger( 'jk-activating-plugin', args );
		...
		return jk.updates.ajax( 'activate-plugin', args );
	};
	 ...
	 * @param {string} response.plugin      The plugin file, relative to the plugins directory.
	 */
	jk.updates.activatePluginSuccess = function( response ) {
		...
		jk.a11y.speak( __( 'Activation completed successfully.' ) );
		$document.trigger( 'jk-plugin-activate-success', response );
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	jk.updates.activatePluginError = function( response ) {
		var $message = $( '.plugin-card-' + response.slug + ', #plugin-information-footer' ).find( '.activating-message' ),
		...
		if ( ! jk.updates.isValidResponse( response, 'activate' ) ) {
			return;
		}
		...
		jk.a11y.speak( errorMessage, 'assertive' );
		$document.trigger( 'jk-plugin-activate-error', response );
	 ...
	 * @param {string} response.activateUrl URL to activate the just installed plugin.
	 */
	jk.updates.installImporterSuccess = function( response ) {
		jk.updates.addAdminNotice( {
			id:        'install-success',
		...
		$document.trigger( 'jk-importer-install-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	jk.updates.installImporterError = function( response ) {
		...
		$document.trigger( 'jk-importer-install-error', response );
	};
	 ...
	 * @param {deletePluginSuccess=} args.success Optional. Success callback. Default: jk.updates.deletePluginSuccess
	 * @param {deletePluginError=}   args.error   Optional. Error callback. Default: jk.updates.deletePluginError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.deletePlugin = function( args ) {
		var $link = $( '[data-plugin="' + args.plugin + '"]' ).find( '.row-actions a.delete' );

		args = _.extend( {
			success: jk.updates.deletePluginSuccess,
			error: jk.updates.deletePluginError
		}, args );
		...
		jk.a11y.speak( __( 'Deleting...' ) );

		$document.trigger( 'jk-plugin-deleting', args );

		return jk.updates.ajax( 'delete-plugin', args );
	};
	 ...
	 */
	jk.updates.deletePluginSuccess = function( response ) {
		...
		$( '[data-plugin="' + response.plugin + '"]' ).css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
			var $form            = $( '#bulk-action-form' ),
				...
				pluginDeletedRow = jk.template( 'item-deleted-row' ),
				/**
		...
		jk.a11y.speak( _x( 'Deleted!', 'plugin' ) );

		$document.trigger( 'jk-plugin-delete-success', response );
	};
	 ...
	 * @param {string}  response.errorMessage The error that occurred.
	 */
	jk.updates.deletePluginError = function( response ) {
		var $plugin, $pluginUpdateRow,
			pluginUpdateRow  = jk.template( 'item-update-row' ),
			noticeContent    = jk.updates.adminNotice( {
		...
		if ( ! jk.updates.isValidResponse( response, 'delete' ) ) {
			return;
		}

		if ( jk.updates.maybeHandleCredentialError( response, 'delete-plugin' ) ) {
			return;
		}
		...
		$document.trigger( 'jk-plugin-delete-error', response );
	};
	 ...
	 * @param {updateThemeSuccess=} args.success Optional. Success callback. Default: jk.updates.updateThemeSuccess
	 * @param {updateThemeError=}   args.error   Optional. Error callback. Default: jk.updates.updateThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.updateTheme = function( args ) {
		var $notice;

		args = _.extend( {
			success: jk.updates.updateThemeSuccess,
			error: jk.updates.updateThemeError
		}, args );
		...
		jk.a11y.speak( __( 'Updating... please wait.' ) );
		$notice.text( __( 'Updating...' ) );

		$document.trigger( 'jk-theme-updating', args );

		return jk.updates.ajax( 'update-theme', args );
	};
	 ...
	 */
	jk.updates.updateThemeSuccess = function( response ) {
		...
		if ( 'customize' === pagenow ) {
			...
			$notice = $( '.theme-info .notice' ).add( jk.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' ).find( '.update-message' ) );
		...
		jk.updates.addAdminNotice( _.extend( { selector: $notice }, updatedMessage ) );
		jk.a11y.speak( __( 'Update completed successfully.' ) );

		jk.updates.decrementCount( 'theme' );

		$document.trigger( 'jk-theme-update-success', response );

		// Show updated message after modal re-rendered.
		if ( isModalOpen && 'customize' !== pagenow ) {
			$( '.theme-info .theme-author' ).after( jk.updates.adminNotice( updatedMessage ) );
		}
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	jk.updates.updateThemeError = function( response ) {
		...
		if ( jk.updates.maybeHandleCredentialError( response, 'update-theme' ) ) {
			return;
		}

		if ( 'customize' === pagenow ) {
			$theme = jk.customize.control( 'installed_theme_' + response.slug ).container.find( '.theme' );
		}
		...
		jk.a11y.speak( errorMessage );

		$document.trigger( 'jk-theme-update-error', response );
	};
	 ...
	 * @param {Object}               args
	 * @param {string}               args.slug    Theme stylesheet.
	 * @param {installThemeSuccess=} args.success Optional. Success callback. Default: jk.updates.installThemeSuccess
	 * @param {installThemeError=}   args.error   Optional. Error callback. Default: jk.updates.installThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.installTheme = function( args ) {
		var $message = $( '.theme-install[data-slug="' + args.slug + '"]' );

		args = _.extend( {
			success: jk.updates.installThemeSuccess,
			error: jk.updates.installThemeError
		}, args );
		...
		$document.trigger( 'jk-theme-installing', args );

		return jk.updates.ajax( 'install-theme', args );
	};
	 ...
	 */
	jk.updates.installThemeSuccess = function( response ) {
		var $card = $( '.jk-full-overlay-header, [data-slug=' + response.slug + ']' ),
			$message;

		$document.trigger( 'jk-theme-install-success', response );
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	jk.updates.installThemeError = function( response ) {
		var $card, $button,
			errorMessage = sprintf(
				/* translators: %s: Error string for a failed installation. */
				__( 'Installation failed: %s' ),
			...
			$message     = jk.updates.adminNotice( {
				className: 'update-message notice-error notice-alt',
				message:   errorMessage
			} );
		...
		if ( jk.updates.maybeHandleCredentialError( response, 'install-theme' ) ) {
			return;
		}
		...
		if ( 'customize' === pagenow ) {
			...
			jk.customize.notifications.remove( 'theme_installing' );
		} else {
		...
		$document.trigger( 'jk-theme-install-error', response );
	};
	 ...
	 * @param {Object}              args
	 * @param {string}              args.slug    Theme stylesheet.
	 * @param {deleteThemeSuccess=} args.success Optional. Success callback. Default: jk.updates.deleteThemeSuccess
	 * @param {deleteThemeError=}   args.error   Optional. Error callback. Default: jk.updates.deleteThemeError
	 * @return {$.promise} A jQuery promise that represents the request,
	 *                     decorated with an abort() method.
	 */
	jk.updates.deleteTheme = function( args ) {
		var $button;
		...
		args = _.extend( {
			success: jk.updates.deleteThemeSuccess,
			error: jk.updates.deleteThemeError
		}, args );
		...
		$document.trigger( 'jk-theme-deleting', args );

		return jk.updates.ajax( 'delete-theme', args );
	};
	 ...
	 */
	jk.updates.deleteThemeSuccess = function( response ) {
		...
		if ( 'themes-network' === pagenow ) {
			...
			$themeRows.css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
				var $views     = $( '.subsubsub' ),
					...
					deletedRow = jk.template( 'item-deleted-row' );
		...
		if ( 'themes' === pagenow ) {
		    var theme = _.find( _jkThemeSettings.themes, { id: response.slug } );
		    if ( theme.hasUpdate ) {
		        jk.updates.decrementCount( 'theme' );
		    }
		}

		jk.a11y.speak( _x( 'Deleted!', 'theme' ) );

		$document.trigger( 'jk-theme-delete-success', response );
	};
	 ...
	 * @param {string} response.errorMessage The error that occurred.
	 */
	jk.updates.deleteThemeError = function( response ) {
		var $themeRow    = $( 'tr.inactive[data-slug="' + response.slug + '"]' ),
			$button      = $( '.theme-actions .delete-theme' ),
			updateRow    = jk.template( 'item-update-row' ),
		...
		if ( jk.updates.maybeHandleCredentialError( response, 'delete-theme' ) ) {
			return;
		}
		...
		$document.trigger( 'jk-theme-delete-error', response );
	};
	 ...
	 */
	jk.updates._addCallbacks = function( data, action ) {
		if ( 'import' === pagenow && 'install-plugin' === action ) {
			data.success = jk.updates.installImporterSuccess;
			data.error   = jk.updates.installImporterError;
		}
	 ...
	 */
	jk.updates.queueChecker = function() {
		var job;

		if ( jk.updates.ajaxLocked || ! jk.updates.queue.length ) {
			return;
		}

		job = jk.updates.queue.shift();

		// Handle a queue job.
		switch ( job.action ) {
			case 'install-plugin':
				jk.updates.installPlugin( job.data );
				break;

			case 'update-plugin':
				jk.updates.updatePlugin( job.data );
				break;

			case 'delete-plugin':
				jk.updates.deletePlugin( job.data );
				break;

			case 'install-theme':
				jk.updates.installTheme( job.data );
				break;

			case 'update-theme':
				jk.updates.updateTheme( job.data );
				break;

			case 'delete-theme':
				jk.updates.deleteTheme( job.data );
				break;
	 ...
	 */
	jk.updates.requestFilesystemCredentials = function( event ) {
		if ( false === jk.updates.filesystemCredentials.available ) {
			/*
			 * After exiting the credentials request modal,
			 * return the focus to the element triggering the request.
			 */
			if ( event && ! jk.updates.$elToReturnFocusToFromCredentialsModal ) {
				jk.updates.$elToReturnFocusToFromCredentialsModal = $( event.target );
			}

			jk.updates.ajaxLocked = true;
			jk.updates.requestForCredentialsModalOpen();
		}
	};
	 ...
	 * @param {Event=} event Optional. Event interface.
	 */
	jk.updates.maybeRequestFilesystemCredentials = function( event ) {
		if ( jk.updates.shouldRequestFilesystemCredentials && ! jk.updates.ajaxLocked ) {
			jk.updates.requestFilesystemCredentials( event );
		}
	};
	 ...
	 */
	jk.updates.keydown = function( event ) {
		if ( 27 === event.keyCode ) {
			jk.updates.requestForCredentialsModalCancel();
	 ...
	 * @since 4.2.0
	 */
	jk.updates.requestForCredentialsModalOpen = function() {
		var $modal = $( '#request-filesystem-credentials-dialog' );

		$( 'body' ).addClass( 'modal-open' );
		$modal.show();
		$modal.find( 'input:enabled:first' ).trigger( 'focus' );
		$modal.on( 'keydown', jk.updates.keydown );
	};
	 ...
	 * @since 4.2.0
	 */
	jk.updates.requestForCredentialsModalClose = function() {
		$( '#request-filesystem-credentials-dialog' ).hide();
		$( 'body' ).removeClass( 'modal-open' );

		if ( jk.updates.$elToReturnFocusToFromCredentialsModal ) {
			jk.updates.$elToReturnFocusToFromCredentialsModal.trigger( 'focus' );
		}
	};
	 ...
	 */
	jk.updates.requestForCredentialsModalCancel = function() {
		...
		_.each( jk.updates.queue, function( job ) {
		...
		jk.updates.requestForCredentialsModalClose();
	};
	 ...
	 */
	jk.updates.showErrorInCredentialsForm = function( message ) {
	 ...
	 * @param {string} action   The type of request to perform.
	 */
	jk.updates.credentialError = function( response, action ) {

		// Restore callbacks.
		response = jk.updates._addCallbacks( response, action );

		jk.updates.queue.unshift( {
			action: action,
		...
		jk.updates.filesystemCredentials.available = false;
		jk.updates.showErrorInCredentialsForm( response.errorMessage );
		jk.updates.requestFilesystemCredentials();
	};
	 ...
	 */
	jk.updates.maybeHandleCredentialError = function( response, action ) {
		if ( jk.updates.shouldRequestFilesystemCredentials && response.errorCode && 'unable_to_connect_to_filesystem' === response.errorCode ) {
			jk.updates.credentialError( response, action );
			return true;
		}
	 ...
	 *                                                'update' or 'install'.
	 */
	jk.updates.isValidResponse = function( response, action ) {
	 ...
	 * @since 4.2.0
	 */
	jk.updates.beforeunload = function() {
		if ( jk.updates.ajaxLocked ) {
	...
	$( function() {
		var $pluginFilter        = $( '#plugin-filter, #plugin-information-footer' ),
			...
			$pluginSearch        = $( '.plugins-php .jk-filter-search' ),
			$pluginInstallSearch = $( '.plugin-install-php .jk-filter-search' );

		settings = _.extend( settings, window._jkUpdatesItemCounts || {} );

		if ( settings.totals ) {
			jk.updates.refreshCount();
		}
		 ...
		 *
		 * @see {jk_print_request_filesystem_credentials_modal() in PHP}
		 */
		jk.updates.shouldRequestFilesystemCredentials = $filesystemModal.length > 0;
		...
		$filesystemModal.on( 'submit', 'form', function( event ) {
			...
			jk.updates.filesystemCredentials.ftp.hostname       = $( '#hostname' ).val();
			jk.updates.filesystemCredentials.ftp.username       = $( '#username' ).val();
			jk.updates.filesystemCredentials.ftp.password       = $( '#password' ).val();
			jk.updates.filesystemCredentials.ftp.connectionType = $( 'input[name="connection_type"]:checked' ).val();
			jk.updates.filesystemCredentials.ssh.publicKey      = $( '#public_key' ).val();
			jk.updates.filesystemCredentials.ssh.privateKey     = $( '#private_key' ).val();
			jk.updates.filesystemCredentials.fsNonce            = $( '#_fs_nonce' ).val();
			jk.updates.filesystemCredentials.available          = true;
		 ...
		 * @since 4.2.0
		 */
		$filesystemModal.on( 'click', '[data-js-action="close"], .notification-dialog-background', jk.updates.requestForCredentialsModalCancel );
		...
		$document.on( 'credential-modal-cancel', function( event, job ) {
			...
			jk.a11y.speak( __( 'Update canceled.' ) );
		} );
		...
		$bulkActionForm.on( 'click', '[data-plugin] .update-link', function( event ) {
			...
			jk.updates.$elToReturnFocusToFromCredentialsModal = $pluginRow.find( '.check-column input' );
			jk.updates.updatePlugin( {
		...
		$document.on( 'click', '.importer-item .install-now', function( event ) {
			...
			jk.updates.installPlugin( {
				slug:    $button.data( 'slug' ),
				pagenow: pagenow,
				success: jk.updates.installImporterSuccess,
				error:   jk.updates.installImporterError
			} );
		} );
		...
		$document.on( 'click', '.themes-php.network-admin .update-link', function( event ) {
			...
			jk.updates.$elToReturnFocusToFromCredentialsModal = $themeRow.find( '.check-column input' );
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			$document.trigger( 'jk-' + type + '-bulk-' + bulkAction, itemsSelected );
			...
			$document.on( 'jk-plugin-update-success jk-plugin-update-error jk-theme-update-success jk-theme-update-error', function( event, response ) {
				var $itemRow = $( '[data-slug="' + response.slug + '"]' ),
					$bulkActionNotice, itemName;

				if ( 'jk-' + response.update + '-update-success' === event.type ) {
					success++;
				} else {
				...
				jk.updates.adminNotice = jk.template( 'jk-bulk-updates-admin-notice' );
				...
				if ( error > 0 && ! jk.updates.queue.length ) {
					$( 'html, body' ).animate( { scrollTop: 0 } );
				}
			} );

			// Reset admin notice template after #bulk-action-notice was added.
			$document.on( 'jk-updates-notice-added', function() {
				jk.updates.adminNotice = jk.template( 'jk-updates-admin-notice' );
			} );
		...
		jk.updates.shouldSearch = function( searchStringLength ) {
			var shouldSearch = searchStringLength >= jk.updates.searchMinCharacters ||
				previousSearchStringLength > jk.updates.searchMinCharacters;
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( jk.updates.shouldSearch( searchStringLength ) ) {
			...
			if ( jk.updates.searchTerm === data.s && 'typechange' !== eventtype ) {
				return;
			} else {
				$pluginFilter.empty();
				jk.updates.searchTerm = data.s;
			}
			...
			if ( ! $searchTab.length ) {
				...
				$( '.jk-filter .filter-links .current' )
					.removeClass( 'current' )
			...
			if ( 'undefined' !== typeof jk.updates.searchRequest ) {
				jk.updates.searchRequest.abort();
			}
			$( 'body' ).addClass( 'loading-content' );

			jk.updates.searchRequest = jk.ajax.post( 'search-install-plugins', data ).done( function( response ) {
				$( 'body' ).removeClass( 'loading-content' );
				$pluginFilter.append( response.items );
				delete jk.updates.searchRequest;

				if ( 0 === response.count ) {
					jk.a11y.speak( __( 'You do not appear to have any plugins available at this time.' ) );
				} else {
					jk.a11y.speak(
						sprintf(
							/* translators: %s: Number of plugins. */
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			if ( jk.updates.searchTerm === data.s ) {
				return;
			} else {
				jk.updates.searchTerm = data.s;
			}
			...
			jk.updates.searchRequest = jk.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				if ( 0 === response.count ) {
					jk.a11y.speak( __( 'No plugins found. Try a different search.'  ) );
				} else {
					jk.a11y.speak(
						sprintf(
							/* translators: %s: Number of plugins. */
		...
		$document.on( 'submit', '.search-plugins', function( event ) {
			...
			$( 'input.jk-filter-search' ).trigger( 'input' );
		} );
		...
		$( window ).on( 'message', function( event ) {
			...
			switch ( message.action ) {
				...
				// Called from `jk-admin/includes/class-jk-upgrader-skins.php`.
				case 'decrementUpdateCount':
					/** @property {string} message.upgradeType */
					jk.updates.decrementCount( message.upgradeType );
					break;
				...
				case 'update-plugin':
					...
					message.data = jk.updates._addCallbacks( message.data, message.action );

					jk.updates.queue.push( message );
					jk.updates.queueChecker();
					break;
			}
		} );
		 ...
		 * @since 4.2.0
		 */
		$( window ).on( 'beforeunload', jk.updates.beforeunload );
		...
		$document.on( 'click keyup', '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update', function( event ) {
			var data, asset, type, $parent,
				$toggler = $( this ),
				action = $toggler.attr( 'data-jk-action' ),
			...
			$.post( window.ajaxurl, data )
				.done( function( response ) {
					...
					if ( ! response.success ) {
						// if JK returns 0 for response (which can happen in a few cases),
					...
					if ( 'enable' === action ) {
						...
						$toggler.attr( 'data-jk-action', 'enable' );

						$label.text( __( 'Enable auto-updates' ) );
						$parent.find( '.auto-update-time' ).addClass( 'hidden' );
						jk.a11y.speak( __( 'Auto-updates disabled' ) );
					}

					$document.trigger( 'jk-auto-update-setting-changed', { state: action, type: type, asset: asset } );
				} )
				.fail( function() {
					...
					jk.a11y.speak( __( 'The request could not be completed.' ), 'assertive' );
				} )
				.always( function() {
...
})( jQuery, window.jk, window._jkUpdatesSettings );
