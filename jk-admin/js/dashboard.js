/**
 * @output jk-admin/js/dashboard.js
 */

/* global pagenow, ajaxurl, postboxes, jkActiveEditor:true, ajaxWidgets */
...
jQuery( function($) {
	var welcomePanel = $( '#welcome-panel' ),
		welcomePanelHide = $('#jk_welcome_panel-hide'),
		updateWelcomePanel;
	...
	$('.welcome-panel-close, .welcome-panel-dismiss a', welcomePanel).on( 'click', function(e) {
		...
		$('#jk_welcome_panel-hide').prop('checked', false);
	});
...
jQuery( function( $ ) {
	...
	var communityEventsData = window.communityEventsData,
		dateI18n = jk.date.dateI18n,
		format = jk.date.format,
	 ...
	 *
	 * @memberOf jk
	 * @namespace jk.communityEvents
	 */
	app = window.jk.communityEvents = /** @lends jk.communityEvents */{
		initialized: false,
		model: null,

		/**
		 * Initializes the jk.communityEvents object.
		 *
		 * @since 4.8.0
		 *
		 * @return {void}
		...
		getEvents: function( requestParams ) {
			...
			requestParams._jknonce = communityEventsData.nonce;
			...
			jk.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		 ...
		 * @since 4.8.0
		 *
		 * @param {Object} templateParams The various parameters that will get passed to jk.template.
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					template = jk.template( 'community-events-event-list' );
					$results.html( template( templateParams ) );
				} else {
					template = jk.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = jk.template( 'community-events-attend-event-near' );
				...
				template = jk.template( 'community-events-could-not-locate' );
				 ...
				 */
				jk.a11y.speak( __( 'An error occurred. Please try again.' ) );
		 ...
		 * @param {string} timeFormat A time format acceptable by `jk.date.dateI18n()`.
		 *
		 * @returns {Array}
		 */
		...
		/**
		 * Returns the user's local/browser time zone, in a form suitable for `jk.date.i18n()`.
		 *
		 * @since 5.5.2
		 *
		 ...
		 * @param {string} timeZone    A time zone string or offset which is parsable by `jk.date.i18n()`.
		 *
		 * @returns {string}
		 */
...
window.communityEventsData.l10n = window.jk.deprecateL10nObject( 'communityEventsData.l10n', window.communityEventsData.l10n, '5.6.0' );
