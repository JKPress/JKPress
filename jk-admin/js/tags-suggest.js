 ...
 *
 * @output jk-admin/js/tags-suggest.js
 */
( function( $ ) {
	var tempID = 0;
	var separator = jk.i18n._x( ',', 'tag delimiter' ) || ',';
	var __ = jk.i18n.__,
	    _n = jk.i18n._n,
	 ...
	 *
	 * Example: `$( element ).jkTagsSuggest( options )`.
	 *
	 * The taxonomy can be passed in a `data-jk-taxonomy` attribute on the element or
	 ...
	 */
	$.fn.jkTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-jk-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'jk-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.jk.a11y.speak( jk.i18n.__( 'Term selected.' ), 'assertive' );
					event.preventDefault();
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="jk-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
