/**
 * @output jk-admin/js/widgets.js
 */

/* global ajaxurl, isRtl, jkWidgets */
...
window.jkWidgets = {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.on( 'click', function() {
				...
				$document.triggerHandler( 'jk-pin-menu' );
			})
			/*
		...
		$( window ).on( 'beforeunload.widgets', function( event ) {
			...
			if ( 0 !== dirtyWidgetIds.length ) {
				...
				event.returnValue = jk.i18n.__( 'The changes you made will be lost if you navigate away from this page.' );
		...
		$(document.body).on('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				if ( ! widget.data( 'dirty-state-initialized' ) ) {
					...
					saveButton.prop( 'disabled', true ).val( jk.i18n.__( 'Saved' ) );
					inside.on( 'input change', function() {
						self.dirtyWidgets[ widgetId ] = true;
						widget.addClass( 'widget-dirty' );
						saveButton.prop( 'disabled', false ).val( jk.i18n.__( 'Save' ) );
					});
				...
				jkWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				jkWidgets.save( target.closest('div.widget'), 1, 1, 0 );
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				toggleBtn.attr( 'aria-expanded', 'false' );
				jkWidgets.close( widget );
			} else if ( target.attr( 'id' ) === 'inactive-widgets-control-remove' ) {
				jkWidgets.removeInactiveWidgets();
				e.preventDefault();
			}
		});
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( jkWidgets.hoveredSidebar && ! $wrap.is( jkWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					jkWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					jkWidgets.hoveredSidebar = $wrap;
					$wrap
						.removeClass( 'closed' )
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				jkWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					jkWidgets.save( $widget, 1, 0, 1 ); // Delete widget.
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					jkWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_jknonce_widgets').val(),
			sidebars: []
		};
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#jk_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'jk_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#jk_inactive_widgets .widget' ).length );
					}
				}
			} else {
				...
				if ( r && r.length > 2 ) {
					...
					jkWidgets.appendTitle( widget );

					// Re-disable the save button.
					widget.find( '.widget-control-save' ).prop( 'disabled', true ).val( jk.i18n.__( 'Saved' ) );
	...
	removeInactiveWidgets : function() {
		...
		a = {
			...
			removeinactivewidgets : $( '#_jknonce_remove_inactive_widgets' ).val()
		};

		data = $.param( a );

		$.post( ajaxurl, data, function() {
			$( '#jk_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
	...
	addWidget: function( chooser ) {
		...
		window.setTimeout( function() {
			...
			window.jk.a11y.speak( jk.i18n.__( 'Widget has been added to the selected sidebar' ), 'assertive' );
		}, 250 );
	},
...
jkWidgets.l10n = window.jk.deprecateL10nObject( 'jkWidgets.l10n', jkWidgets.l10n, '5.5.0' );
