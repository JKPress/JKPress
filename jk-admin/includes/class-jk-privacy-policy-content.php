<?php
/**
 * JK_Privacy_Policy_Content class.
 *
 * @package JKPress
...
final class JK_Privacy_Policy_Content {
	 ...
	 *
	 * Intended for use from `jk_add_privacy_policy_content()`.
	 *
	 * @since 4.9.6
	 *
	...
	public static function text_change_check() {
		...
		$old = (array) get_post_meta( $policy_page_id, '_jk_suggested_privacy_policy_content' );
		...
		$cached = get_option( '_jk_suggested_policy_text_has_changed' );
		...
		if ( $new != $old ) {
			 ...
			 */
			add_action( 'admin_notices', array( 'JK_Privacy_Policy_Content', 'policy_text_changed_notice' ) );
			$state = 'changed';
		} else {
			$state = 'not-changed';
		}
		...
		if ( $cached !== $state ) {
			update_option( '_jk_suggested_policy_text_has_changed', $state, false );
		}
	...
	public static function _policy_page_updated( $post_id ) {
		...
		if ( $update_cache ) {
			delete_post_meta( $policy_page_id, '_jk_suggested_privacy_policy_content' );
			// Update the cache.
			foreach ( $done as $data ) {
				add_post_meta( $policy_page_id, '_jk_suggested_privacy_policy_content', $data );
			}
		}
	}
	 ...
	 *
	 * @param JK_Post|null $post The currently edited post. Default null.
	 */
	public static function notice( $post = null ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			jk_add_inline_script(
				'jk-notices',
				sprintf(
					'jk.data.dispatch( "core/notices" ).createWarningNotice( "%s", { actions: [ %s ], isDismissible: false } )',
					$message,
					jk_json_encode( $action )
				),
				'after'
			);
		} else {
			...
			jk_admin_notice(
				...
				array(
					'type'               => 'warning',
					'additional_classes' => array( 'inline', 'jk-pp-notice' ),
				)
			);
		}
	}
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			$strings[] = '<div class="jk-suggested-text">';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Who we are' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default JKPress does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Comments' ) . '</h2>';

		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which JKPress collects by default.' ) . '</p>';
		} else {
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Media' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default, JKPress does not include a contact form. If you use a contact form plugin, use this subsection to note what personal data is captured when someone submits a contact form, and how long you keep it. For example, you may note that you keep contact form submissions for a certain period for customer service purposes, but you do not use the information submitted through them for marketing purposes.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Cookies' ) . '</h2>';

		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your website uses, including those set by your plugins, social media, and analytics. We have provided the cookies which JKPress installs by default.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		if ( ! $description ) {
			/* translators: Default privacy policy heading. */
			$strings[] = '<h2 class="jk-block-heading">' . __( 'Embedded content from other websites' ) . '</h2>';
			/* translators: Default privacy policy text. */
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default JKPress does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a JKPress plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Who we share your data with' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default JKPress does not share any personal data with anyone.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'How long we retain your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'What rights you have over your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="jk-block-heading">' . __( 'Where your data is sent' ) . '</h2>';
		...
		if ( $blocks ) {
			foreach ( $strings as $key => $string ) {
				if ( str_starts_with( $string, '<p>' ) ) {
					$strings[ $key ] = "<!-- jk:paragraph -->\n" . $string . "\n<!-- /jk:paragraph -->\n";
				}

				if ( str_starts_with( $string, '<h2 ' ) ) {
					$strings[ $key ] = "<!-- jk:heading -->\n" . $string . "\n<!-- /jk:heading -->\n";
				}
			}
		}
		 ...
		 * @deprecated 5.7.0 Use jk_add_privacy_policy_content() instead.
		 *
		...
		return apply_filters_deprecated(
			'jk_get_default_privacy_policy_content',
			array( $content, $strings, $description, $blocks ),
			'5.7.0',
			'jk_add_privacy_policy_content()'
		);
	}
	...
	public static function add_suggested_content() {
		...
		jk_add_privacy_policy_content( __( 'JKPress' ), $content );
	}
}
