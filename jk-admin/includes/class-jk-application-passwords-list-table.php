<?php
/**
 * List Table API: JK_Application_Passwords_List_Table class
 *
 * @package JKPress
 ...
 * @see JK_List_Table
 */
class JK_Application_Passwords_List_Table extends JK_List_Table {
	...
	public function prepare_items() {
		global $user_id;
		$this->items = array_reverse( JK_Application_Passwords::get_user_application_passwords( $user_id ) );
	}
	...
	public function print_js_template_row() {
		...
		foreach ( $columns as $column_name => $display_name ) {
			...
			printf( '<td class="%s" data-colname="%s">', esc_attr( $classes ), esc_attr( jk_strip_all_tags( $display_name ) ) );
			...
			switch ( $column_name ) {
				...
				case 'created':
					...
					echo '<# print( jk.date.dateI18n( ' . jk_json_encode( __( 'F j, Y' ) ) . ', data.created ) ) #>';
					break;
				case 'last_used':
					echo '<# print( data.last_used !== null ? jk.date.dateI18n( ' . jk_json_encode( __( 'F j, Y' ) ) . ", data.last_used ) : '—' ) #>";
					break;
				case 'last_ip':
					echo "{{ data.last_ip || '—' }}";
					break;
