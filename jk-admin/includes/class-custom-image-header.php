...
class Custom_Image_Header {
	...
	public function __construct( $admin_header_callback, $admin_image_div_callback = '' ) {
		...
		add_action( 'jk_ajax_custom-header-crop', array( $this, 'ajax_header_crop' ) );
		add_action( 'jk_ajax_custom-header-add', array( $this, 'ajax_header_add' ) );
		add_action( 'jk_ajax_custom-header-remove', array( $this, 'ajax_header_remove' ) );
	}
		...
		get_current_screen()->add_help_tab(
			array(
				...
				'content' =>
					...
					'<p>' . __( 'If your theme has more than one default header image, or you have uploaded more than one custom header image, you have the option of having JKPress display a randomly different image on each page of your site. Click the &#8220;Random&#8221; radio button next to the Uploaded Images or Default Images section to enable this feature.' ) . '</p>' .
	...
	public function step() {
		...
		$step = (int) $_GET['step'];
		if ( $step < 1 || 3 < $step ||
			( 2 === $step && ! jk_verify_nonce( $_REQUEST['_jknonce-custom-header-upload'], 'custom-header-upload' ) ) ||
			( 3 === $step && ! jk_verify_nonce( $_REQUEST['_jknonce'], 'custom-header-crop-image' ) )
		) {
			return 1;
		}
	...
	public function js_includes() {
		...
		if ( ( 1 === $step || 3 === $step ) ) {
			jk_enqueue_media();
			jk_enqueue_script( 'custom-header' );
			if ( current_theme_supports( 'custom-header', 'header-text' ) ) {
				jk_enqueue_script( 'jk-color-picker' );
			}
		} elseif ( 2 === $step ) {
			jk_enqueue_script( 'imgareaselect' );
		}
	}
	...
	public function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_jknonce-custom-header-options' );
	...
	public function process_default_headers() {
		...
		$this->default_headers    = $_jk_default_headers;
...
(function($){
	...
	$( function() {
		...
		text_color.jkColorPicker({
			change: function( event, ui ) {
				pickColor( text_color.jkColorPicker('color') );
			},
			clear: function() {
				pickColor( '' );
			}
...
<td>
	...
	<form enctype="multipart/form-data" id="upload-form" class="jk-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ); ?>">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php jk_nonce_field( 'custom-header-upload', '_jknonce-custom-header-upload' ); ?>
			...
			$modal_update_href = add_query_arg(
				array(
					'page'                          => 'custom-header',
					'step'                          => 2,
					'_jknonce-custom-header-upload' => jk_create_nonce( 'custom-header-upload' ),
				),
...
<td>
	<p>
		...
		jk_nonce_field( 'custom-header-options', '_jknonce-custom-header-options' );
		?>
	...
	public function step_2() {
		check_admin_referer( 'custom-header-upload', '_jknonce-custom-header-upload' );
		...
		if ( empty( $_POST ) && isset( $_GET['file'] ) ) {
			...
			$url           = jk_get_attachment_image_src( $attachment_id, 'full' );
			$url           = $url[0];
		} elseif ( isset( $_POST ) ) {
		...
		if ( file_exists( $file ) ) {
			list( $width, $height, $type, $attr ) = jk_getimagesize( $file );
		} else {
			$data   = jk_get_attachment_metadata( $attachment_id );
		...
		if ( ! current_theme_supports( 'custom-header', 'flex-height' )
			 ...
			 */
			$file = apply_filters( 'jk_create_file_in_uploads', $file, $attachment_id ); // For replication.
			...
			$image = jk_crop_image(
				...
				$height / $oitar,
				false,
				str_replace( jk_basename( $file ), 'midsize-' . jk_basename( $file ), $file )
			);

			if ( ! $image || is_jk_error( $image ) ) {
				jk_die( __( 'Image could not be processed. Please go back and try again.' ), __( 'Image Processing Error' ) );
			}

			/** This filter is documented in jk-admin/includes/class-custom-image-header.php */
			$image = apply_filters( 'jk_create_file_in_uploads', $image, $attachment_id ); // For replication.

			$url    = str_replace( jk_basename( $url ), jk_basename( $image ), $url );
			$width  = $width / $oitar;
			$height = $height / $oitar;
		} else {
			$oitar = 1;
...
<form method="post" action="<?php echo esc_url( add_query_arg( 'step', 3 ) ); ?>">
	...
	<?php } ?>
		<?php jk_nonce_field( 'custom-header-crop-image' ); ?>
	...
	public function step_2_manage_upload() {
		...
		$jk_filetype   = jk_check_filetype_and_ext( $uploaded_file['tmp_name'], $uploaded_file['name'] );

		if ( ! jk_match_mime_types( 'image', $jk_filetype['type'] ) ) {
			jk_die( __( 'The uploaded file is not a valid image. Please try again.' ) );
		}

		$file = jk_handle_upload( $uploaded_file, $overrides );

		if ( isset( $file['error'] ) ) {
			jk_die( $file['error'], __( 'Image Upload Error' ) );
		}
		...
		// Save the data.
		$attachment_id = jk_insert_attachment( $attachment, $file );
	 ...
	 *
	 * @since 2.1.0
	 * @since 4.4.0 Switched to using jk_get_attachment_url() instead of the guid
	...
	public function step_3() {
		...
		$attachment = jk_copy_parent_attachment_properties( $cropped, $attachment_id, 'custom-header' );
		...
		$url = jk_get_attachment_url( $attachment_id );
		$this->set_header_image( compact( 'url', 'attachment_id', 'width', 'height' ) );

		// Cleanup.
		$medium = str_replace( jk_basename( $original ), 'midsize-' . jk_basename( $original ), $original );
		if ( file_exists( $medium ) ) {
			jk_delete_file( $medium );
		}

		if ( empty( $_POST['create-new-attachment'] ) && empty( $_POST['skip-cropping'] ) ) {
			jk_delete_file( $original );
		}
	...
	public function admin_page() {
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			jk_die( __( 'Sorry, you are not allowed to customize headers.' ) );
		}
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_jk_attachment_is_custom_header', get_stylesheet() );
	...
	final public function create_attachment_object( $cropped, $parent_attachment_id ) {
		_deprecated_function( __METHOD__, '6.5.0', 'jk_copy_parent_attachment_properties()' );
		...
		$attachment = array(
			...
			'post_title'     => jk_basename( $cropped ),
	...
	final public function insert_attachment( $attachment, $cropped ) {
		 ...
		 */
		$metadata = apply_filters( 'jk_header_image_attachment_metadata', $metadata );
	...
	public function ajax_header_crop() {
		...
		if ( ! $cropped || is_jk_error( $cropped ) ) {
			jk_send_json_error( array( 'message' => __( 'Image could not be processed. Please go back and try again.' ) ) );
		}
		...
		$attachment['url']           = jk_get_attachment_url( $new_attachment_id );
	...
	public function ajax_header_add() {
		...
		$key = '_jk_attachment_custom_header_last_used_' . get_stylesheet();
		update_post_meta( $attachment_id, $key, time() );
		update_post_meta( $attachment_id, '_jk_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_remove() {
		...
		delete_post_meta( $attachment_id, '_jk_attachment_is_custom_header', get_stylesheet() );
	 ...
	 * @since 3.9.0
	 *
	 * @param JK_Customize_Manager $jk_customize Customize manager.
	 */
	public function customize_set_last_used( $jk_customize ) {

		$header_image_data_setting = $jk_customize->get_setting( 'header_image_data' );
	...
	public function get_uploaded_header_images() {
		...
		$timestamp_key = '_jk_attachment_custom_header_last_used_' . get_stylesheet();
		$alt_text_key  = '_jk_attachment_image_alt';
