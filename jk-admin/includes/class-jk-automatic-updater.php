<?php
/**
 * Upgrade API: JK_Automatic_Updater class
 *
 * @package JKPress
 * @subpackage Upgrader
 * @since 4.6.0
...
class JK_Automatic_Updater {
	...
	public function is_disabled() {
		...
		if ( ! jk_is_file_mod_allowed( 'automatic_updater' ) ) {
			return true;
		}
		...
		// More fine grained control can be done through the JK_AUTO_UPDATE_CORE constant and filters.
	 ...
	 * the PHP `open_basedir` restrictions, so that JKPress does not try to access
	 ...
	 * jk-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		// Used to see if JK_Filesystem is set up to allow unattended updates.
		...
		if ( 'core' === $type ) {
			...
			if ( ! $update && jk_is_auto_update_enabled_for_type( $type ) ) {
		 ...
		 *
		 * Since JKPress 3.7, minor and development versions of core, and translations have
		 * been auto-updated by default. New installs on JKPress 5.6 or higher will also
		...
		if ( 'core' === $type ) {
			...
			if ( file_exists( JK_CONTENT_DIR . '/db.php' ) && empty( $jkdb->is_mysql ) ) {
				...
				$mysql_compat = version_compare( $jkdb->db_version(), $item->mysql_version, '>=' );
			}
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from JKPress.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, JKPress won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = JK_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for JKMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = JK_CONTENT_DIR; // JK_LANG_DIR;
				break;
		}
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to JKPress %s' ), $item->version );
				/* translators: %s: JKPress version. */
				$item_name = sprintf( __( 'JKPress %s' ), $item->version );
				break;
			case 'theme':
				$upgrader_item = $item->theme;
				$theme         = jk_get_theme( $upgrader_item );
			...
			case 'translation':
				...
				/* translators: %s: Project name (plugin, theme, or JKPress). */
				$item_name = sprintf( __( 'Translations for %s' ), $language_item_name );
				/* translators: 1: Project name (plugin, theme, or JKPress), 2: Language. */
		...
		$is_debug = JK_DEBUG && JK_DEBUG_LOG;
		if ( 'plugin' === $type ) {
		...
		/*
		 * After JK_Upgrader::upgrade() completes, maintenance mode is disabled.
		 *
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new JK_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
		}

		if ( 'core' === $type ) {
			if ( is_jk_error( $upgrade_result )
			...
			if ( is_jk_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'JKPress updated successfully.' ) );
			}
		}
		...
		if ( 'plugin' === $type ) {
			...
			if ( $was_active && ! is_jk_error( $upgrade_result ) ) {
				...
				if ( $this->has_fatal_error() ) {
					...
					if ( is_jk_error( $backup_restored ) ) {
						$upgrade_result->add(
	...
	public function run() {
		...
		if ( ! JK_Upgrader::create_lock( 'auto_updater' ) ) {
			return;
		}
		...
		// Next, plugins.
		jk_update_plugins(); // Check for plugin updates.
		...
		jk_update_themes();  // Check for theme updates.
		...
		jk_version_check(); // Check for core updates.
		...
		jk_update_themes( $theme_stats ); // Check for theme updates.
		...
		jk_update_plugins( $plugin_stats ); // Check for plugin updates.
		...
		if ( ! empty( $this->update_results ) ) {
			$development_version = str_contains( jk_get_jk_version(), '-' );
		...
		JK_Upgrader::release_lock( 'auto_updater' );
	}
	...
	protected function after_core_update( $update_result ) {
		...
		/*
		 * Any of these JK_Error codes are critical failures, as in they occurred after we started to copy core files.
		 * We should not try to perform a background update again until there is a successful one-click update performed by the user.
		 */
		$critical = false;
		if ( 'disk_full' === $error_code || str_contains( $error_code, '__copy_dir' ) ) {
			$critical = true;
		} elseif ( 'rollback_was_required' === $error_code && is_jk_error( $result->get_error_data()->rollback ) ) {
		...
		/*
		 * Any other JK_Error code (like download_failed or files_not_writable) occurs before
		 ...
		 * the issue could actually be on JKPress.org's side.) If that one fails, then email.
		 */
		$send               = true;
		$transient_failures = array( 'incompatible_archive', 'download_failed', 'insane_distro', 'locked' );
		if ( in_array( $error_code, $transient_failures, true ) && ! get_site_option( 'auto_core_update_failed' ) ) {
			jk_schedule_single_event( time() + HOUR_IN_SECONDS, 'jk_maybe_auto_update' );
			$send = false;
		}
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be JK_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be JK_Error.
		 */
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: JKPress version. */
				$subject = __( '[%1$s] Your site has updated to JKPress %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: JKPress version. */
				$subject = __( '[%1$s] JKPress %2$s is available. Please update!' );
				break;
		...
		// If the auto-update is not to the latest version, say that the current version of JK is available instead.
		$version = 'success' === $type ? $core_update->current : $next_user_core_update->current;
		$subject = sprintf( $subject, jk_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $version );
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: JKPress version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to JKPress %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: JKPress version. */
				$body .= sprintf( __( 'For more on version %s, see the About JKPress screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: JKPress latest version. */
					$body .= "\n\n" . sprintf( __( 'JKPress %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: JKPress version. */
					__( 'Please update your site at %1$s to JKPress %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: JKPress version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update JKPress to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: JKPress latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of JKPress, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The JKPress team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
				$core_update->support_email
			);
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the JKPress.org support forums may be able to help.' );
		...
		if ( $critical_support ) {
			$body .= ' ' . __( "Reach out to JKPress Core developers to ensure you'll never have this problem again." );
		}
		...
		if ( 'critical' === $type && is_jk_error( $result ) ) {
			$body .= "\n***\n\n";
			/* translators: %s: JKPress version. */
			 ...
			 * Loop through all errors (the main JK_Error, the update result, the rollback result) for code, data, etc.
			 */
		 ...
		 *
		 * @param array $email {
		 *     Array of email arguments that will be passed to jk_mail().
		 *
		 *     @type string $to      The email recipient. An array of emails
		 *                            can be returned, as handled by jk_mail().
		...
		jk_mail( $email['to'], jk_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( 'If you experience any issues or need support, the volunteers in the JKPress.org support forums may be able to help.' );
		$body[] = __( 'https://wordpress.org/support/forums/' );
		$body[] = "\n" . __( 'The JKPress Team' );

		if ( '' !== get_option( 'blogname' ) ) {
			$site_title = jk_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
		} else {
		...
		$result = jk_mail( $email['to'], jk_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'JKPress site: %s' ), network_home_url( '/' ) );

		// Core.
		if ( isset( $this->update_results['core'] ) ) {
			$result = $this->update_results['core'][0];

			if ( $result->result && ! is_jk_error( $result->result ) ) {
				/* translators: %s: JKPress version. */
				$body[] = sprintf( __( 'SUCCESS: JKPress was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: JKPress version. */
				$body[] = sprintf( __( 'FAILED: JKPress failed to update to %s' ), $result->name );
				++$failures;
			}
		...
		foreach ( array( 'plugin', 'theme', 'translation' ) as $type ) {
			...
			$success_items = jk_list_filter( $this->update_results[ $type ], array( 'result' => true ) );
			...
			if ( $success_items ) {
				...
				$body[] = $messages[ $type ];
				foreach ( jk_list_pluck( $success_items, 'name' ) as $name ) {
			...
			if ( $success_items !== $this->update_results[ $type ] ) {
				...
				foreach ( $this->update_results[ $type ] as $item ) {
					if ( ! $item->result || is_jk_error( $item->result ) ) {
						/* translators: %s: Name of plugin / theme / translation. */
		...
		if ( '' !== get_bloginfo( 'name' ) ) {
			$site_title = jk_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );
		} else {
			$site_title = parse_url( home_url(), PHP_URL_HOST );
		}
...
This debugging email is sent when you are using a development version of JKPress.

If you think these failures might be due to a bug in JKPress, could you report it?
 * Open a thread in the support forums: https://wordpress.org/support/forum/alphabeta
 * Or, if you're comfortable writing a bug report: https://core.trac.wordpress.org/

Thanks! -- The JKPress Team"
				)
			);
			$body[] = '';
...
=========='
	...
	protected function has_fatal_error() {
		...
		$response     = jk_remote_get( $url, compact( 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( $is_debug ) {
			error_log( var_export( substr( $response['body'], strpos( $response['body'], '###### jk_scraping_result_start:' ) ), true ) );
		}

		$body                   = jk_remote_retrieve_body( $response );
