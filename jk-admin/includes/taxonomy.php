 ...
 * @return int|JK_Error
 */
function jk_create_category( $cat_name, $category_parent = 0 ) {
	$id = category_exists( $cat_name, $category_parent );
	if ( $id ) {
		return $id;
	}

	return jk_insert_category(
		array(
			'cat_name'        => $cat_name,
 ...
 */
function jk_create_categories( $categories, $post_id = '' ) {
	$cat_ids = array();
	foreach ( $categories as $category ) {
		...
		if ( $id ) {
			...
			$id = jk_create_category( $category );
			if ( $id ) {
				$cat_ids[] = $id;
			}
		}
	...
	if ( $post_id ) {
		jk_set_post_categories( $post_id, $cat_ids );
	}
 ...
 *
 * @since 2.0.0
 * @since 2.5.0 $jk_error parameter was added.
 ...
 * }
 * @param bool  $jk_error Optional. Default false.
 * @return int|JK_Error The ID number of the new or updated Category on success. Zero or a JK_Error on failure,
 *                      depending on param `$jk_error`.
 */
function jk_insert_category( $catarr, $jk_error = false ) {
	$cat_defaults = array(
		'cat_ID'               => 0,
		'taxonomy'             => 'category',
		'cat_name'             => '',
	...
	$catarr       = jk_parse_args( $catarr, $cat_defaults );

	if ( '' === trim( $catarr['cat_name'] ) ) {
		if ( ! $jk_error ) {
			return 0;
		} else {
			return new JK_Error( 'cat_name', __( 'You did not enter a category name.' ) );
		}
	}
	...
	if ( $update ) {
		$catarr['cat_ID'] = jk_update_term( $catarr['cat_ID'], $catarr['taxonomy'], $args );
	} else {
		$catarr['cat_ID'] = jk_insert_term( $catarr['cat_name'], $catarr['taxonomy'], $args );
	}

	if ( is_jk_error( $catarr['cat_ID'] ) ) {
		if ( $jk_error ) {
...
/**
 * Aliases jk_insert_category() with minimal args.
 *
 ...
 */
function jk_update_category( $catarr ) {
	...
	$category = jk_slash( $category );

	// Merge old and new fields with new fields overwriting old ones.
	$catarr = array_merge( $category, $catarr );

	return jk_insert_category( $catarr );
}
 ...
 */
function jk_create_tag( $tag_name ) {
	return jk_create_term( $tag_name, 'post_tag' );
}
...
function get_terms_to_edit( $post_id, $taxonomy = 'post_tag' ) {
	...
	if ( false === $terms ) {
		$terms = jk_get_object_terms( $post_id, $taxonomy );
		jk_cache_add( $post_id, jk_list_pluck( $terms, 'term_id' ), $taxonomy . '_relationships' );
	}
 ...
 * @return array|JK_Error
 */
function jk_create_term( $tag_name, $taxonomy = 'post_tag' ) {
	$id = term_exists( $tag_name, $taxonomy );
	if ( $id ) {
		return $id;
	}

	return jk_insert_term( $tag_name, $taxonomy );
}
