...
$jk_file_descriptions = array(
	...
	// Deprecated files.
	'jk-layout.css'         => __( 'Stylesheet' ),
	'jk-comments.php'       => __( 'Comments Template' ),
	'jk-comments-popup.php' => __( 'Popup Comments Template' ),
	'comments-popup.php'    => __( 'Popup Comments' ),
);

/**
 * Gets the description for standard JKPress theme files.
 *
 * @since 1.5.0
 *
 * @global array $jk_file_descriptions Theme file descriptions.
 * @global array $allowed_files        List of allowed files.
 *
 * @param string $file Filesystem path or filename.
 * @return string Description of file from $jk_file_descriptions or basename of $file if description doesn't exist.
 *                Appends 'Page Template' to basename of $file if the file is a page template.
 */
function get_file_description( $file ) {
	global $jk_file_descriptions, $allowed_files;

	$dirname   = pathinfo( $file, PATHINFO_DIRNAME );
	$file_path = $allowed_files[ $file ];

	if ( isset( $jk_file_descriptions[ basename( $file ) ] ) && '.' === $dirname ) {
		return $jk_file_descriptions[ basename( $file ) ];
...
/**
 * Gets the absolute filesystem path to the root of the JKPress installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the JKPress installation.
 */
function get_home_path() {
	...
	if ( ! empty( $home ) && 0 !== strcasecmp( $home, $siteurl ) ) {
		$jk_path_rel_to_home = str_ireplace( $home, '', $siteurl ); /* $siteurl - $home */
		$pos                 = strripos( str_replace( '\\', '/', $_SERVER['SCRIPT_FILENAME'] ), trailingslashit( $jk_path_rel_to_home ) );
 ...
 */
function jk_get_plugin_file_editable_extensions( $plugin ) {
 ...
 * @since 4.9.0
 *
 * @param JK_Theme $theme Theme object.
 * @return string[] Array of editable file extensions.
 */
function jk_get_theme_file_editable_extensions( $theme ) {
	 ...
	 * @param JK_Theme $theme         The active theme object.
	 */
	$file_types = apply_filters( 'jk_theme_editor_filetypes', $default_types, $theme );
 ...
 * @since 4.9.0
 */
function jk_print_file_editor_templates() {
	?>
	<script type="text/html" id="tmpl-jk-file-editor-notice">
 ...
 * }
 * @return true|JK_Error True on success or `JK_Error` on failure.
 */
function jk_edit_theme_plugin_file( $args ) {
	if ( empty( $args['file'] ) ) {
		return new JK_Error( 'missing_file' );
	}

	if ( 0 !== validate_file( $args['file'] ) ) {
		return new JK_Error( 'bad_file' );
	}

	if ( ! isset( $args['newcontent'] ) ) {
		return new JK_Error( 'missing_content' );
	}

	if ( ! isset( $args['nonce'] ) ) {
		return new JK_Error( 'missing_nonce' );
	}
	...
	if ( ! empty( $args['plugin'] ) ) {
		...
		if ( ! current_user_can( 'edit_plugins' ) ) {
			return new JK_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
		}

		if ( ! jk_verify_nonce( $args['nonce'], 'edit-plugin_' . $file ) ) {
			return new JK_Error( 'nonce_failure' );
		}

		if ( ! array_key_exists( $plugin, get_plugins() ) ) {
			return new JK_Error( 'invalid_plugin' );
		}

		if ( 0 !== validate_file( $file, get_plugin_files( $plugin ) ) ) {
			return new JK_Error( 'bad_plugin_file_path', __( 'Sorry, that file cannot be edited.' ) );
		}

		$editable_extensions = jk_get_plugin_file_editable_extensions( $plugin );

		$real_file = JK_PLUGIN_DIR . '/' . $file;
		...
		if ( 0 !== validate_file( $stylesheet ) ) {
			return new JK_Error( 'bad_theme_path' );
		}

		if ( ! current_user_can( 'edit_themes' ) ) {
			return new JK_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit templates for this site.' ) );
		}

		$theme = jk_get_theme( $stylesheet );
		if ( ! $theme->exists() ) {
			return new JK_Error( 'non_existent_theme', __( 'The requested theme does not exist.' ) );
		}

		if ( ! jk_verify_nonce( $args['nonce'], 'edit-theme_' . $stylesheet . '_' . $file ) ) {
			return new JK_Error( 'nonce_failure' );
		}
		...
		$editable_extensions = jk_get_theme_file_editable_extensions( $theme );
		...
		if ( 0 !== validate_file( $file, array_keys( $allowed_files ) ) ) {
			return new JK_Error( 'disallowed_theme_file', __( 'Sorry, that file cannot be edited.' ) );
		}
		...
		return new JK_Error( 'missing_theme_or_plugin' );
	}

	// Ensure file is real.
	if ( ! is_file( $real_file ) ) {
		return new JK_Error( 'file_does_not_exist', __( 'File does not exist! Please double check the name and try again.' ) );
	}
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $extension, $editable_extensions, true ) ) {
			return new JK_Error( 'illegal_file_type', __( 'Files of this type are not editable.' ) );
		}
	}
	...
	if ( ! is_writable( $real_file ) ) {
		return new JK_Error( 'file_not_writable' );
	}
	...
	if ( false === $written ) {
		return new JK_Error( 'unable_to_write', __( 'Unable to write to file.' ) );
	}

	jk_opcache_invalidate( $real_file, true );
	...
	if ( $is_active && 'php' === $extension ) {
		...
		$scrape_params = array(
			'jk_scrape_key'   => $scrape_key,
			'jk_scrape_nonce' => $scrape_nonce,
		);
		$headers       = array(
		...
		$needle_start = "###### jk_scraping_result_start:$scrape_key ######";
		$needle_end   = "###### jk_scraping_result_end:$scrape_key ######";
		...
		$body                   = jk_remote_retrieve_body( $r );
		...
		if ( true !== $result ) {
			...
			return new JK_Error( 'php_error', $message, $result );
		}
	}

	if ( $theme instanceof JK_Theme ) {
		$theme->cache_delete();
	}
 ...
 */
function jk_tempnam( $filename = '', $dir = '' ) {
	if ( empty( $dir ) ) {
		$dir = get_temp_dir();
	}
	...
	if ( ! $temp_filename ) {
		return jk_tempnam( dirname( $filename ), $dir );
	}

	// Suffix some random data to avoid filename conflicts.
	$temp_filename .= '-' . jk_generate_password( 6, false );
	$temp_filename .= '.tmp';
	$temp_filename  = jk_unique_filename( $dir, $temp_filename );
	...
	if ( $characters_over_limit > 0 ) {
		...
		return jk_tempnam( $filename, $dir );
	}
...
function validate_file_to_edit( $file, $allowed_files = array() ) {
	...
	switch ( $code ) {
		case 1:
			jk_die( __( 'Sorry, that file cannot be edited.' ) );

			// case 2 :
			// jk_die( __('Sorry, cannot call files with their real path.' ));
...
/**
 * Handles PHP uploads in JKPress.
 *
 ...
 * }
 */
function _jk_handle_upload( &$file, $overrides, $time, $action ) {
	// The default error handler.
	if ( ! function_exists( 'jk_handle_upload_error' ) ) {
		function jk_handle_upload_error( &$file, $message ) {
			return array( 'error' => $message );
		}
	}

	/**
	 * Filters the data for a file before it is uploaded to JKPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `jk_handle_sideload_prefilter`
	 *  - `jk_handle_upload_prefilter`
	 *
	 * @since 2.9.0 as 'jk_handle_upload_prefilter'.
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to JKPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `jk_handle_sideload_overrides`
	 *  - `jk_handle_upload_overrides`
	 *
	 * @since 5.7.0
	 *
	 * @param array|false $overrides An array of override parameters for this file. Boolean false if none are
	 *                               provided. See {@see _jk_handle_upload()}.
	 * @param array       $file      {
	 *     Reference to a single element from `$_FILES`.
	 *
	 *     @type string $name     The original name of the file on the client machine.
	...
	$upload_error_handler = 'jk_handle_upload_error';
	if ( isset( $overrides['upload_error_handler'] ) ) {
		$upload_error_handler = $overrides['upload_error_handler'];
	}

	// You may have had one or more 'jk_handle_upload_prefilter' functions error out the file. Handle that gracefully.
	...
	$test_uploaded_file = 'jk_handle_upload' === $action ? is_uploaded_file( $file['tmp_name'] ) : @is_readable( $file['tmp_name'] );
	if ( ! $test_uploaded_file ) {
		return call_user_func_array( $upload_error_handler, array( &$file, __( 'Specified file failed upload test.' ) ) );
	}

	$test_file_size = 'jk_handle_upload' === $action ? $file['size'] : filesize( $file['tmp_name'] );
	...
	if ( $test_type ) {
		$jk_filetype     = jk_check_filetype_and_ext( $file['tmp_name'], $file['name'], $mimes );
		$ext             = empty( $jk_filetype['ext'] ) ? '' : $jk_filetype['ext'];
		$type            = empty( $jk_filetype['type'] ) ? '' : $jk_filetype['type'];
		$proper_filename = empty( $jk_filetype['proper_filename'] ) ? '' : $jk_filetype['proper_filename'];

		// Check to see if jk_check_filetype_and_ext() determined the filename was incorrect.
		if ( $proper_filename ) {
	 ...
	 * overriding this one.
	 */
	$uploads = jk_upload_dir( $time );
	if ( ! ( $uploads && false === $uploads['error'] ) ) {
		return call_user_func_array( $upload_error_handler, array( &$file, $uploads['error'] ) );
	}

	$filename = jk_unique_filename( $uploads['path'], $file['name'], $unique_filename_callback );
	...
	return apply_filters(
		...
		'jk_handle_sideload' === $action ? 'sideload' : 'upload'
	);
}

/**
 * Wrapper for _jk_handle_upload().
 *
 * Passes the {@see 'jk_handle_upload'} action.
 *
 * @since 2.0.0
 *
 * @see _jk_handle_upload()
 *
 * @param array       $file      Reference to a single element of `$_FILES`.
 *                               Call the function once for each uploaded file.
 *                               See _jk_handle_upload() for accepted values.
 * @param array|false $overrides Optional. An associative array of names => values
 *                               to override default variables. Default false.
 *                               See _jk_handle_upload() for accepted values.
 * @param string|null $time      Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See _jk_handle_upload() for return value.
 */
function jk_handle_upload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'jk_handle_upload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}

	return _jk_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Wrapper for _jk_handle_upload().
 *
 * Passes the {@see 'jk_handle_sideload'} action.
 *
 * @since 2.6.0
 *
 * @see _jk_handle_upload()
 ...
 * @return array See _jk_handle_upload() for return value.
 */
function jk_handle_sideload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'jk_handle_sideload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}
...
/**
 * Downloads a URL to a local temporary file using the JKPress HTTP API.
 *
 * Please note that the calling function must delete or move the file.
 *
 * @since 2.5.0
 ...
 *                                       Default false.
 * @return string|JK_Error Filename on success, JK_Error on failure.
 */
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	// WARNING: The file is not automatically deleted, the script must delete or move the file.
	if ( ! $url ) {
		return new JK_Error( 'http_no_url', __( 'No URL Provided.' ) );
	}
	...
	$tmpfname = jk_tempnam( $url_filename );
	if ( ! $tmpfname ) {
		return new JK_Error( 'http_no_file', __( 'Could not create temporary file.' ) );
	}
	...
	$response_code = jk_remote_retrieve_response_code( $response );
	...
	if ( 200 !== $response_code ) {
		...
		return new JK_Error( 'http_404', trim( jk_remote_retrieve_response_message( $response ) ), $data );
	}

	$content_disposition = jk_remote_retrieve_header( $response, 'Content-Disposition' );
	...
	$content_md5 = jk_remote_retrieve_header( $response, 'Content-MD5' );

	if ( $content_md5 ) {
		$md5_check = verify_file_md5( $tmpfname, $content_md5 );

		if ( is_jk_error( $md5_check ) ) {
			unlink( $tmpfname );
			return $md5_check;
		}
	}
	...
	if ( $signature_verification ) {
		 ...
		 */
		$signed_hostnames = apply_filters( 'jk_signature_hosts', array( 'wordpress.org', 'downloads.wordpress.org', 's.w.org' ) );
	...
	if ( $signature_verification ) {
		$signature = jk_remote_retrieve_header( $response, 'X-Content-Signature' );

		if ( ! $signature ) {
			/*
			 * Retrieve signatures from a file if the header wasn't included.
			 * JKPress.org stores signatures at $package_url.sig.
			 */
			 ...
			 */
			$signature_url = apply_filters( 'jk_signature_url', $signature_url, $url );

			if ( $signature_url ) {
				$signature_request = jk_safe_remote_get(
					$signature_url,
					array(
				...
				if ( ! is_jk_error( $signature_request ) && 200 === jk_remote_retrieve_response_code( $signature_request ) ) {
					$signature = explode( "\n", jk_remote_retrieve_body( $signature_request ) );
				}
			}
		}
	...
	if ( is_jk_error( $signature_verification ) ) {
		if (
			/**
			 ...
			 */
			apply_filters( 'jk_signature_softfail', true, $url )
		) {
 ...
 * @return bool|JK_Error True on success, false when the MD5 format is unknown/unexpected,
 *                       JK_Error on failure.
 */
 ...
 * @return bool|JK_Error True on success, false if verification not attempted,
 *                       or JK_Error describing an error condition.
 */
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	if ( ! $filename_for_errors ) {
		$filename_for_errors = jk_basename( $filename );
	}
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled JKPress one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			/*
			 * Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode,
			 * as that's what JKPress utilizes during signing verifications.
			 */
			// phpcs:disable JKPress.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by JKPress.
 *
 * @since 5.2.0
 *
 * @return string[] Array of base64-encoded signing keys.
 */
function jk_trusted_keys() {
	$trusted_keys = array();

	if ( time() < 1617235200 ) {
		// JKPress.org Key #1 - This key is only valid before April 1st, 2021.
	 ...
	 */
	return apply_filters( 'jk_trusted_keys', $trusted_keys );
}
 ...
 */
function jk_zip_file_is_valid( $file ) {
	...
	require_once ABSPATH . 'jk-admin/includes/class-pclzip.php';
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the JKPress
 * Filesystem Abstraction.
 *
 * Assumes that JK_Filesystem() has already been called and set up. Does not extract
...
function unzip_file( $file, $to ) {
	...
	if ( ! $jk_filesystem || ! is_object( $jk_filesystem ) ) {
		return new JK_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
	}
	...
	if ( ! $jk_filesystem->is_dir( $to ) ) { // Only do parents if no children exist.
 ...
 *
 * Assumes that JK_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @access private
 *
...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen ) {
		return new JK_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );
	}
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( ! $info ) {
			$z->close();
			return new JK_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
		}
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $jk_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $jk_filesystem->is_dir( $_dir ) ) {
			$z->close();
			return new JK_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), $_dir );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param null|true|JK_Error $result         The result of the override. True on success, otherwise JK Error. Default null.
	 * @param string             $file           Full path and filename of ZIP archive.
	 * @param string             $to             Full path on the filesystem to extract archive to.
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents ) {
			$z->close();
			return new JK_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );
		}

		if ( ! $jk_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
			$z->close();
			return new JK_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param true|JK_Error $result         The result of unzipping the archive. True on success, otherwise JK_Error. Default true.
	 * @param string        $file           Full path and filename of ZIP archive.
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	if ( ! is_array( $archive_files ) ) {
		return new JK_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), $archive->errorInfo( true ) );
	}

	if ( 0 === count( $archive_files ) ) {
		return new JK_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $jk_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $jk_filesystem->is_dir( $_dir ) ) {
			return new JK_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), $_dir );
		}
	}

	/** This filter is documented in src/jk-admin/includes/file.php */
	$pre = apply_filters( 'pre_unzip_file', null, $file, $to, $needed_dirs, $required_space );
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $jk_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE ) ) {
			return new JK_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
		}
	}

	/** This action is documented in src/jk-admin/includes/file.php */
	$result = apply_filters( 'unzip_file', true, $file, $to, $needed_dirs, $required_space );
...
/**
 * Copies a directory from one location to another via the JKPress Filesystem
 * Abstraction.
 *
 * Assumes that JK_Filesystem() has already been called and setup.
 *
 * @since 2.5.0
 *
 * @global JK_Filesystem_Base $jk_filesystem JKPress filesystem subclass.
...
function copy_dir( $from, $to, $skip_list = array() ) {
	...
	$dirlist = $jk_filesystem->dirlist( $from );

	if ( false === $dirlist ) {
		return new JK_Error( 'dirlist_failed_copy_dir', __( 'Directory listing failed.' ), basename( $from ) );
	}

	$from = trailingslashit( $from );
	$to   = trailingslashit( $to );

	if ( ! $jk_filesystem->exists( $to ) && ! $jk_filesystem->mkdir( $to ) ) {
		return new JK_Error(
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			if ( ! $jk_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
				// If copy failed, chmod file to 0644 and try again.
				$jk_filesystem->chmod( $to . $filename, FS_CHMOD_FILE );

				if ( ! $jk_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
					return new JK_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
				}
			}

			jk_opcache_invalidate( $to . $filename );
		} elseif ( 'd' === $fileinfo['type'] ) {
			if ( ! $jk_filesystem->is_dir( $to . $filename ) ) {
				if ( ! $jk_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
					return new JK_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
				}
			}
...
function move_dir( $from, $to, $overwrite = false ) {
	...
	if ( trailingslashit( strtolower( $from ) ) === trailingslashit( strtolower( $to ) ) ) {
		return new JK_Error( 'source_destination_same_move_dir', __( 'The source and destination are the same.' ) );
	}

	if ( $jk_filesystem->exists( $to ) ) {
		if ( ! $overwrite ) {
			return new JK_Error( 'destination_already_exists_move_dir', __( 'The destination folder already exists.' ), $to );
		} elseif ( ! $jk_filesystem->delete( $to, true ) ) {
			// Can't overwrite if the destination couldn't be deleted.
			return new JK_Error( 'destination_not_deleted_move_dir', __( 'The destination directory already exists and could not be removed.' ) );
		}
	}

	if ( $jk_filesystem->move( $from, $to ) ) {
		/*
		 ...
		 */
		usleep( 200000 );
		jk_opcache_invalidate_directory( $to );

		return true;
	}

	// Fall back to a recursive copy.
	if ( ! $jk_filesystem->is_dir( $to ) ) {
		if ( ! $jk_filesystem->mkdir( $to, FS_CHMOD_DIR ) ) {
			return new JK_Error( 'mkdir_failed_move_dir', __( 'Could not create directory.' ), $to );
		}
	}
...
/**
 * Initializes and connects the JKPress Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force JKPress to use them by returning
 ...
 *                                                   directly to the `JK_Filesystem_*()` classes.
 *                                                   Default false.
 ...
 */
function JK_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $jk_filesystem;

	require_once ABSPATH . 'jk-admin/includes/class-jk-filesystem-base.php';
	...
	if ( ! class_exists( "JK_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'jk-admin/includes/class-jk-filesystem-' . $method . '.php', $method );
	...
	$method = "JK_Filesystem_$method";

	$jk_filesystem = new $method( $args );
	...
	if ( is_jk_error( $jk_filesystem->errors ) && $jk_filesystem->errors->has_errors() ) {
		return false;
	}

	if ( ! $jk_filesystem->connect() ) {
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `jk-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * @link https://developer.wordpress.org/advanced-administration/wordpress/jk-config/#wordpress-upgrade-constants
 *
 * Plugins may define a custom transport handler, See JK_Filesystem().
 *
 * @since 2.5.0
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = JK_CONTENT_DIR;
	}

	// If the directory doesn't exist (jk-content/languages) then use the parent directory as we'll create it.
	if ( JK_LANG_DIR === $context && ! is_dir( $context ) ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the JKPress files, and that of newly created files.
			$jk_file_owner   = false;
			$temp_file_owner = false;
			if ( function_exists( 'fileowner' ) ) {
				$jk_file_owner   = @fileowner( __FILE__ );
				$temp_file_owner = @fileowner( $temp_file_name );
			}

			if ( false !== $jk_file_owner && $jk_file_owner === $temp_file_owner ) {
				/*
				 * JKPress is creating files as the same owner as the JKPress files,
				 * this means it's safe to modify & create new files via PHP.
				 */
				$method                                  = 'direct';
				$GLOBALS['_jk_filesystem_direct_method'] = 'file_owner';
				 ...
				 */
				$method                                  = 'direct';
				$GLOBALS['_jk_filesystem_direct_method'] = 'relaxed_ownership';
			}
 ...
 * @param bool|JK_Error $error                        Optional. Whether the current request has failed
...
function request_filesystem_credentials( $form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false ) {
	 ...
	 * @param bool|JK_Error $error                        Whether the current request has failed to connect,
	 *                                                    or an error object.
	...
	$submitted_form = jk_unslash( $_POST );

	// Verify nonce, or unset submitted form field values on failure.
	if ( ! isset( $_POST['_fs_nonce'] ) || ! jk_verify_nonce( $_POST['_fs_nonce'], 'filesystem-credentials' ) ) {
		unset(
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, JKPress needs to access your web server.' );
	echo ' ';
...
<p id="auth-keys-desc"><?php _e( 'Enter the location on the server where the public and private keys are located. If a passphrase is needed, enter that in the password field above.' ); ?></p>
	 ...
	 * from JK_Site_Health_Auto_Updates::test_check_jk_filesystem_method().
	 */
	...
	<p class="request-filesystem-credentials-action-buttons">
		<?php jk_nonce_field( 'filesystem-credentials', '_fs_nonce', false, true ); ?>
 ...
 * @since 4.2.0
 */
function jk_print_request_filesystem_credentials_modal() {
 ...
 */
function jk_opcache_invalidate( $filepath, $force = false ) {
	static $can_invalidate = null;

	/*
	 * Check to see if JKPress is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether JKPress will invalidate `$filepath`. Default true.
	 * @param string $filepath        The path to the PHP file to invalidate.
	 */
	if ( apply_filters( 'jk_opcache_invalidate_file', true, $filepath ) ) {
 ...
 */
function jk_opcache_invalidate_directory( $dir ) {
	global $jk_filesystem;
	...
	if ( ! is_string( $dir ) || '' === trim( $dir ) ) {
		if ( JK_DEBUG ) {
			$error_message = sprintf(
				...
				'<code>jk_opcache_invalidate_directory()</code>'
			);
			jk_trigger_error( '', $error_message );
		}
		return;
	}

	$dirlist = $jk_filesystem->dirlist( $dir, false, true );
	 ...
	 *
	 * JK_Filesystem_*::dirlist() returns an array of file and directory information.
	 *
	 * This does not include a path to the file or directory.
	 * To invalidate files within sub-directories, recursion is needed
	 * to prepend an absolute path containing the sub-directory's name.
	 *
	 * @param array  $dirlist Array of file/directory information from JK_Filesystem_Base::dirlist(),
	...
	$invalidate_directory = static function ( $dirlist, $path ) use ( &$invalidate_directory ) {
		...
		foreach ( $dirlist as $name => $details ) {
			if ( 'f' === $details['type'] ) {
				jk_opcache_invalidate( $path . $name, true );
