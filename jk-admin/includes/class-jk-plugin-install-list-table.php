<?php
/**
 * List Table API: JK_Plugin_Install_List_Table class
 *
 * @package JKPress
 ...
 * @see JK_List_Table
 */
class JK_Plugin_Install_List_Table extends JK_List_Table {
	...
	public function prepare_items() {
		...
		switch ( $tab ) {
			case 'search':
				...
				$term = isset( $_REQUEST['s'] ) ? jk_unslash( $_REQUEST['s'] ) : '';
			...
			case 'favorites':
				...
				if ( isset( $_GET['_jknonce'] ) && jk_verify_nonce( jk_unslash( $_GET['_jknonce'] ), $action ) ) {
					$user = isset( $_GET['user'] ) ? jk_unslash( $_GET['user'] ) : get_user_option( 'wporg_favorites' );
		...
		if ( $installed_plugins ) {
			...
			$js_plugins['all'] = array_values( jk_list_pluck( $installed_plugins, 'plugin' ) );
			$upgrade_plugins   = jk_filter_object_list( $installed_plugins, array( 'upgrade' => true ), 'and', 'plugin' );
	...
	public function views() {
		...
		/** This filter is documented in jk-admin/includes/class-jk-list-table.php */
...
<div class="jk-filter">
	...
	public function display() {
		...
		if ( $singular ) {
			$data_attr = " data-jk-lists='list:$singular'";
		}

		$this->display_tablenav( 'top' );

		?>
<div class="jk-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
		<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = jk_kses( $plugin['name'], $plugins_allowedtags );
			...
			$version = jk_kses( $plugin['version'], $plugins_allowedtags );

			$name = strip_tags( $title . ' ' . $version );

			$author = jk_kses( $plugin['author'], $plugins_allowedtags );
			if ( ! empty( $author ) ) {
			...
			$requires_jk  = isset( $plugin['requires'] ) ? $plugin['requires'] : null;

			$compatible_php = is_php_version_compatible( $requires_php );
			$compatible_jk  = is_jk_version_compatible( $requires_jk );
			$tested_jk      = ( empty( $plugin['tested'] ) || version_compare( get_bloginfo( 'version' ), $plugin['tested'], '<=' ) );

			$action_links = array();

			$action_links[] = jk_get_plugin_action_button( $name, $plugin, $compatible_php, $compatible_jk );
		...
		<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
			...
			if ( ! $compatible_php || ! $compatible_jk ) {
				...
				if ( ! $compatible_php && ! $compatible_jk ) {
					$incompatible_notice_message .= __( 'This plugin does not work with your versions of JKPress and PHP.' );
					if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
						...
						$incompatible_notice_message .= jk_update_php_annotation( '</p><p><em>', '</em>', false );
					...
					$incompatible_notice_message .= __( 'This plugin does not work with your version of JKPress.' );
			...
			<div class="plugin-card-bottom">
				...
				<div class="column-compatibility">
					<?php
					if ( ! $tested_jk ) {
						echo '<span class="compatibility-untested">' . __( 'Untested with your version of JKPress' ) . '</span>';
					} elseif ( ! $compatible_jk ) {
						echo '<span class="compatibility-incompatible">' . __( '<strong>Incompatible</strong> with your version of JKPress' ) . '</span>';
					} else {
						echo '<span class="compatibility-compatible">' . __( '<strong>Compatible</strong> with your version of JKPress' ) . '</span>';
					}
					?>
				</div>
			</div>
	...
	protected function get_dependencies_notice( $plugin_data ) {
		...
		foreach ( $plugin_data['requires_plugins'] as $dependency ) {
			$dependency_data = JK_Plugin_Dependencies::get_dependency_data( $dependency );
