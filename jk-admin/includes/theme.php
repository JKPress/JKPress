 ...
 * @return bool|null|JK_Error True on success, false if `$stylesheet` is empty, JK_Error on failure.
...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $redirect ) ) {
		$redirect = jk_nonce_url( 'themes.php?action=delete&stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet );
	}
	...
	$themes_dir = $jk_filesystem->jk_themes_dir();
	if ( empty( $themes_dir ) ) {
		return new JK_Error( 'fs_no_themes_dir', __( 'Unable to locate JKPress theme directory.' ) );
	}
	...
	$deleted    = $jk_filesystem->delete( $theme_dir, true );
	...
	$theme_translations = jk_get_installed_translations( 'themes' );
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$jk_filesystem->delete( JK_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$jk_filesystem->delete( JK_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );
			$jk_filesystem->delete( JK_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.l10n.php' );

			$json_translation_files = glob( JK_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '-*.json' );
	...
	if ( is_multisite() ) {
		JK_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param JK_Post|null $post      Optional. The post being edited, provided for context.
 * @param string       $post_type Optional. Post type to get the templates for. Default 'page'.
 * @return string[] Array of template file names keyed by the template header name.
 */
function get_page_templates( $post = null, $post_type = 'page' ) {
	return array_flip( jk_get_theme()->get_page_templates( $post, $post_type ) );
}
 ...
 *
 * @param JK_Theme $theme Theme data object.
 */
 ...
 * @since 3.8.0
 *
 * @param JK_Theme $theme JK_Theme object.
...
function get_theme_update_available( $theme ) {
	...
	if ( isset( $themes_update->response[ $stylesheet ] ) ) {
		...
		$details_url = add_query_arg(
			...
			$update['url']
		); // Theme browser inside JK? Replace this. Also, theme preview JS will override this on the available list.
		$update_url  = jk_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $stylesheet ) ), 'upgrade-theme_' . $stylesheet );
...
/**
 * Retrieves list of JKPress theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the JKPress.org API. Defaults to true.
...
function get_theme_feature_list( $api = true ) {
	...
	// Loop over the jk.org canonical list and apply translations.
...
/**
 * Retrieves theme installer pages from the JKPress.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the JKPress.org
 ...
 *     }
 * }
 * @return object|array|JK_Error Response object or array on success, JK_Error on failure. See the
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! isset( $args->jk_version ) ) {
		$args->jk_version = substr( jk_get_jk_version(), 0, 3 ); // x.y
	}

	/**
	 * Filters arguments used to query for installer pages from the JKPress.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the JKPress.org Themes API.
	...
	/**
	 * Filters whether to override the JKPress.org Themes API.
	 *
	 * Returning a non-false value will effectively short-circuit the JKPress.org API request.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the JKPress.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		...
		$request   = jk_remote_get( $url, $http_args );
		...
		if ( $ssl && is_jk_error( $request ) ) {
			...
			$request = jk_remote_get( $http_url, $http_args );
		}
		...
		if ( is_jk_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new JK_Error( 'themes_api_failed', $res->error );
			}
		}
	...
	/**
	 * Filters the returned JKPress.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|stdClass|JK_Error $res    JKPress.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the JKPress.org Themes API.
	 */
 ...
 * @since 3.8.0
 *
 * @param JK_Theme[] $themes Optional. Array of theme objects to prepare.
 *                           Defaults to all allowed themes.
 *
 * @return array An associative array of theme data, sorted by name.
 */
function jk_prepare_themes_for_js( $themes = null ) {
	$current_theme = get_stylesheet();

	/**
	 * Filters theme data before it is prepared for JavaScript.
	 *
	 * Passing a non-empty array will result in jk_prepare_themes_for_js() returning
	 * early with that value instead.
	 *
	 * @since 4.2.0
	 *
	 * @param array           $prepared_themes An associative array of theme data. Default empty array.
	 * @param JK_Theme[]|null $themes          An array of theme objects to prepare, if any.
	...
	if ( null === $themes ) {
		...
		if ( ! isset( $themes[ $current_theme ] ) ) {
			$themes[ $current_theme ] = jk_get_theme();
		}
	}
	...
	foreach ( $themes as $theme ) {
		...
		if ( $is_block_theme && $can_edit_theme_options ) {
			...
			if ( $current_theme !== $slug ) {
				$customize_action = add_query_arg( 'jk_theme_preview', $slug, $customize_action );
			}
		} elseif ( ! $is_block_theme && $can_customize && $can_edit_theme_options ) {
			$customize_action = jk_customize_url( $slug );
		}
		if ( null !== $customize_action ) {
			$customize_action = add_query_arg(
				array(
					'return' => urlencode( sanitize_url( remove_query_arg( jk_removable_query_args(), jk_unslash( $_SERVER['REQUEST_URI'] ) ) ) ),
				),
				$customize_action
			);
		...
		$update_requires_jk  = isset( $updates[ $slug ]['requires'] ) ? $updates[ $slug ]['requires'] : null;
		...
		$auto_update_forced = jk_is_auto_update_forced_for_item( 'theme', null, $auto_update_filter_payload );
		...
		$prepared_themes[ $slug ] = array(
			...
			'compatibleWP'   => is_jk_version_compatible( $theme->get( 'RequiresWP' ) ),
			'compatiblePHP'  => is_php_version_compatible( $theme->get( 'RequiresPHP' ) ),
			'updateResponse' => array(
				'compatibleWP'  => is_jk_version_compatible( $update_requires_jk ),
			...
			'actions'        => array(
				'activate'   => current_user_can( 'switch_themes' ) ? jk_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . $encoded_slug ), 'switch-theme_' . $slug ) : null,
				'customize'  => $customize_action,
				'delete'     => ( ! is_multisite() && current_user_can( 'delete_themes' ) ) ? jk_nonce_url( admin_url( 'themes.php?action=delete&amp;stylesheet=' . $encoded_slug ), 'delete-theme_' . $slug ) : null,
				'autoupdate' => jk_is_auto_update_enabled_for_type( 'theme' ) && ! is_multisite() && current_user_can( 'update_themes' )
					? jk_nonce_url( admin_url( 'themes.php?action=' . $auto_update_action . '&amp;stylesheet=' . $encoded_slug ), 'updates' )
					: null,
			),
	 ...
	 */
	$prepared_themes = apply_filters( 'jk_prepare_themes_for_js', $prepared_themes );
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap jk-clearfix" role="document">
			...
			<div class="theme-about jk-clearfix">
 ...
 */
function jk_get_theme_error( $theme ) {
	if ( ! isset( $GLOBALS['_paused_themes'] ) ) {
		return false;
	}
 ...
 * @return bool|JK_Error True on success, false if `$theme` was not paused,
 *                       `JK_Error` on failure.
 */
function resume_theme( $theme, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		$functions_path = '';
		if ( str_contains( $jk_stylesheet_path, $extension ) ) {
			$functions_path = $jk_stylesheet_path . '/functions.php';
		} elseif ( str_contains( $jk_template_path, $extension ) ) {
			$functions_path = $jk_template_path . '/functions.php';
		}
		...
		if ( ! empty( $functions_path ) ) {
			jk_redirect(
				add_query_arg(
					'_error_nonce',
					jk_create_nonce( 'theme-resume-error_' . $theme ),
					$redirect
				)
			);
	...
	$result = jk_paused_themes()->delete( $extension );
