<?php
/**
 * JKPress Network Administration API.
 *
 * @package JKPress
...
function network_domain_check() {
	...
	$sql = $jkdb->prepare( 'SHOW TABLES LIKE %s', $jkdb->esc_like( $jkdb->site ) );
	if ( $jkdb->get_var( $sql ) ) {
		return $jkdb->get_var( "SELECT domain FROM $jkdb->site ORDER BY id ASC LIMIT 1" );
	}
	return false;
}
...
function allow_subdirectory_install() {
	...
	$post = $jkdb->get_row( "SELECT ID FROM $jkdb->posts WHERE post_date < DATE_SUB(NOW(), INTERVAL 1 MONTH) AND post_status = 'publish'" );
	if ( empty( $post ) ) {
		return true;
	}
 ...
 *
 * @param false|JK_Error $errors Optional. Error object. Default false.
 */
function network_step1( $errors = false ) {
	...
	jk_nonce_field( 'install-network-1' );
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of JKPress sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		...
		<p><?php _e( 'Please choose whether you would like sites in your JKPress network to use sub-domains or sub-directories.' ); ?>
	...
	if ( JK_CONTENT_DIR !== ABSPATH . 'jk-content' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		$subdirectory_warning_message  = '<strong>' . __( 'Warning:' ) . '</strong> ';
		$subdirectory_warning_message .= __( 'Subdirectory networks may not be fully compatible with custom jk-content directories.' );
		jk_admin_notice(
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your JKPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your JKPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your JKPress network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	$jk_siteurl_subdir = preg_replace( '#^' . preg_quote( $home_path, '#' ) . '#', '', $abspath_fix );
	$rewrite_base      = ! empty( $jk_siteurl_subdir ) ? ltrim( trailingslashit( $jk_siteurl_subdir ), '/' ) : '';

	$location_of_jk_config = $abspath_fix;
	if ( ! file_exists( ABSPATH . 'jk-config.php' ) && file_exists( dirname( ABSPATH ) . '/jk-config.php' ) ) {
		$location_of_jk_config = dirname( $abspath_fix );
	}
	$location_of_jk_config = trailingslashit( $location_of_jk_config );
	...
	<p><?php _e( 'The original configuration steps are shown here for reference.' ); ?></p>
			...
			$subdomain_install = (bool) $jkdb->get_var( "SELECT meta_value FROM $jkdb->sitemeta WHERE site_id = 1 AND meta_key = 'subdomain_install'" );

			jk_admin_notice(
				'<strong>' . __( 'Warning:' ) . '</strong> ' . __( 'An existing JKPress network was detected.' ),
				array(
					'additional_classes' => array( 'error' ),
				)
			);
	...
	if ( $_POST || ! is_multisite() ) {
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			$notice_message .= sprintf(
				/* translators: 1: jk-config.php, 2: .htaccess */
			...
			$notice_message .= sprintf(
				/* translators: 1: jk-config.php, 2: web.config */
				__( 'You should back up your existing %1$s and %2$s files.' ),
				'<code>jk-config.php</code>',
		...
		jk_admin_notice( $notice_message, $notice_args );
	}
	?>
	<ol>
		...
		<?php
		printf(
			/* translators: 1: jk-config.php, 2: Location of jk-config file, 3: Translated version of "That's all, stop editing! Happy publishing." */
			__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
			'<code>jk-config.php</code>',
			'<code>' . $location_of_jk_config . '</code>',
			/*
		...
		<p class="configuration-rules-label"><label for="network-wpconfig-rules">
			<?php
			printf(
				/* translators: %s: File name (jk-config.php, .htaccess or web.config). */
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
		...
		if ( ! empty( $keys_salts ) ) {
			$keys_salts_str = '';
			$from_api       = jk_remote_get( 'https://api.wordpress.org/secret-key/1.1/salt/' );
			if ( is_jk_error( $from_api ) ) {
				foreach ( $keys_salts as $c => $v ) {
					$keys_salts_str .= "\ndefine( '$c', '" . jk_generate_password( 64, true, true ) . "' );";
				}
			} else {
				$from_api = explode( "\n", jk_remote_retrieve_body( $from_api ) );
				foreach ( $keys_salts as $c => $v ) {
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="JKPress Rule 1" stopProcessing="true">
                    <match url="^index\.php$" ignoreCase="false" />
                    <action type="None" />
                </rule>';
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$web_config_file .= '
                <rule name="JKPress Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . JKINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
		}
			$web_config_file .= '
                <rule name="JKPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'jk-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'jk-admin/" redirectType="Permanent" />
                </rule>
                <rule name="JKPress Rule 3" stopProcessing="true">
                ...
                <rule name="JKPress Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(jk-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="JKPress Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="JKPress Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
			...
			printf(
				...
				__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other JKPress rules:' ),
				'<code>web.config</code>',
				'<code>' . $home_path . '</code>'
			);
		echo '</p>';
		if ( ! $subdomain_install && JK_CONTENT_DIR !== ABSPATH . 'jk-content' ) {
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom jk-content directories.' ) . '</strong></p>';
		}
		?>
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$ms_files_rewriting  = "\n# uploaded files\nRewriteRule ^";
			$ms_files_rewriting .= $subdir_match . "files/(.+) {$rewrite_base}" . JKINC . "/ms-files.php?file={$subdir_replacement_12} [L]" . "\n";
		}
...
{$ms_files_rewriting}
# add a trailing slash to /jk-admin
RewriteRule ^{$subdir_match}jk-admin$ {$subdir_replacement_01}jk-admin/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(jk-(content|admin|includes).*) {$rewrite_base}{$subdir_replacement_12} [L]
...
EOF;
	...
	if ( ! is_multisite() ) {
		?>
		<p><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?> <a href="<?php echo esc_url( jk_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
		<?php
	}
}
