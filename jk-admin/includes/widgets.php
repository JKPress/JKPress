<?php
/**
 * JKPress Widgets Administration API
 *
 * @package JKPress
...
function jk_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset( $jk_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) ) {
		...
		$sidebar_args = jk_list_widget_controls_dynamic_sidebar( $control_args );

		jk_widget_control( ...$sidebar_args );
	}
}
 ...
 */
function jk_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'jk_list_widget_controls_dynamic_sidebar' );

	$description = jk_sidebar_description( $sidebar );
 ...
 * @return array
 */
function jk_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $jk_registered_widgets[ $widget_id ]['callback'] ) ) {
		$jk_registered_widgets[ $widget_id ]['_callback'] = $jk_registered_widgets[ $widget_id ]['callback'];
		$jk_registered_widgets[ $widget_id ]['callback']  = 'jk_widget_control';
	}
 ...
 * @return array
 */
function jk_widget_control( $sidebar_args ) {
	global $jk_registered_widgets, $jk_registered_widget_controls, $sidebars_widgets;

	$widget_id  = $sidebar_args['widget_id'];
	$sidebar_id = isset( $sidebar_args['id'] ) ? $sidebar_args['id'] : false;
	$key        = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[ $sidebar_id ], true ) : '-1'; // Position of widget in sidebar.
	$control    = isset( $jk_registered_widget_controls[ $widget_id ] ) ? $jk_registered_widget_controls[ $widget_id ] : array();
	...
	$jk_registered_widgets[ $widget_id ]['callback'] = $jk_registered_widgets[ $widget_id ]['_callback'];
	unset( $jk_registered_widgets[ $widget_id ]['_callback'] );
	...
	<?php
	$widget_description = jk_widget_description( $widget_id );
 ...
 * @return string
 */
function jk_widgets_access_body_class( $classes ) {
