 ...
 * @return string|JK_Error New filepath on success, JK_Error on failure.
 */
function jk_crop_image( $src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	$src_file = $src;
	...
	$editor = jk_get_image_editor( $src );
	...
	if ( ! $dst_file ) {
		$dst_file = str_replace( jk_basename( $src_file ), 'cropped-' . jk_basename( $src_file ), $src_file );
	}
	 ...
	 */
	jk_mkdir_p( dirname( $dst_file ) );

	$dst_file = dirname( $dst_file ) . '/' . jk_unique_filename( dirname( $dst_file ), jk_basename( $dst_file ) );
 ...
 */
function jk_get_missing_image_subsizes( $attachment_id ) {
	if ( ! jk_attachment_is_image( $attachment_id ) ) {
		return array();
	}

	$registered_sizes = jk_get_registered_image_subsizes();
	...
	if ( ! empty( $image_meta['original_image'] ) ) {
		$image_file = jk_get_original_image_path( $attachment_id );
		$imagesize  = jk_getimagesize( $image_file );
	}
	 ...
	 * @param int     $attachment_id The image attachment post ID.
	 */
	return apply_filters( 'jk_get_missing_image_subsizes', $missing_sizes, $image_meta, $attachment_id );
}
 ...
 *
 * @param int $attachment_id The image attachment post ID.
 * @return array|JK_Error The updated image meta data array or JK_Error object
 *                        if both the image meta and the attached file are missing.
 */
function jk_update_image_subsizes( $attachment_id ) {
	$image_meta = jk_get_attachment_metadata( $attachment_id );
	$image_file = jk_get_original_image_path( $attachment_id );
	...
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
		 ...
		 */
		if ( ! empty( $image_file ) ) {
			$image_meta = jk_create_image_subsizes( $image_file, $attachment_id );
		} else {
			return new JK_Error( 'invalid_attachment', __( 'The attached file cannot be found.' ) );
		}
	} else {
		$missing_sizes = jk_get_missing_image_subsizes( $attachment_id );
		...
		$image_meta = _jk_make_subsizes( $missing_sizes, $image_file, $image_meta, $attachment_id );
	}

	/** This filter is documented in jk-admin/includes/image.php */
	$image_meta = apply_filters( 'jk_generate_attachment_metadata', $image_meta, $attachment_id, 'update' );

	// Save the updated metadata.
	jk_update_attachment_metadata( $attachment_id, $image_meta );
 ...
 * @access private
 *
 * @param array  $saved_data    The data returned from JK_Image_Editor after successfully saving an image.
 * @param string $original_file Path to the original file.
 * @param array  $image_meta    The image meta data.
 * @param int    $attachment_id The attachment post ID.
 * @return array The updated image meta data.
 */
function _jk_image_meta_replace_original( $saved_data, $original_file, $image_meta, $attachment_id ) {
	...
	$image_meta['file'] = _jk_relative_upload_path( $new_file );

	// Add image file size.
	$image_meta['filesize'] = jk_filesize( $new_file );

	// Store the original image file name in image_meta.
	$image_meta['original_image'] = jk_basename( $original_file );
 ...
 */
function jk_create_image_subsizes( $file, $attachment_id ) {
	$imagesize = jk_getimagesize( $file );
	...
	$image_meta = array(
		...
		'filesize' => jk_filesize( $file ),
		'sizes'    => array(),
	);

	// Fetch additional metadata from EXIF/IPTC.
	$exif_meta = jk_read_image_metadata( $file );
	...
	if ( 'image/png' !== $imagesize['mime'] ) {
		 ...
		 * the `_jk_attached_file` post meta value.
		 *
		...
		if ( $threshold && ( $image_meta['width'] > $threshold || $image_meta['height'] > $threshold ) ) {
			...
			$output_format = jk_get_image_editor_output_format( $file, $imagesize['mime'] );
		...
		if ( $scale_down || $convert ) {
			...
			if ( ! is_jk_error( $resized ) && is_array( $exif_meta ) ) {
				$resized = $editor->maybe_exif_rotate();
				$rotated = $resized; // bool true or JK_Error
			}
			...
			if ( ! is_jk_error( $resized ) ) {
				...
				if ( ! is_jk_error( $saved ) ) {
					$image_meta = _jk_image_meta_replace_original( $saved, $file, $image_meta, $attachment_id );
	...
	$new_sizes = jk_get_registered_image_subsizes();
	...
	return _jk_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id );
}
 ...
 */
function _jk_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id ) {
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
 ...
 */
function jk_copy_parent_attachment_properties( $cropped, $parent_attachment_id, $context = '' ) {
	$parent          = get_post( $parent_attachment_id );
	$parent_url      = jk_get_attachment_url( $parent->ID );
	$parent_basename = jk_basename( $parent_url );
	$url             = str_replace( jk_basename( $parent_url ), jk_basename( $cropped ), $parent_url );

	$size       = jk_getimagesize( $cropped );
	...
	$attachment = array(
		'post_title'     => $use_original_title ? $parent->post_title : jk_basename( $cropped ),
	...
	if ( '' !== trim( $parent->_jk_attachment_image_alt ) ) {
		$attachment['meta_input'] = array(
			'_jk_attachment_image_alt' => jk_slash( $parent->_jk_attachment_image_alt ),
		);
	}
 ...
 */
function jk_generate_attachment_metadata( $attachment_id, $file ) {
	...
	if ( 'image/heic' === $mime_type || ( preg_match( '!^image/!', $mime_type ) && file_is_displayable_image( $file ) ) ) {
		...
		$metadata = jk_create_image_subsizes( $file, $attachment_id );
	} elseif ( jk_attachment_is( 'video', $attachment ) ) {
		$metadata = jk_read_video_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:video' ) || post_type_supports( 'attachment:video', 'thumbnail' );
	} elseif ( jk_attachment_is( 'audio', $attachment ) ) {
		$metadata = jk_read_audio_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:audio' ) || post_type_supports( 'attachment:audio', 'thumbnail' );
	}

	/*
	 * jk_read_video_metadata() and jk_read_audio_metadata() return `false`
	...
	if ( $support && ! empty( $metadata['image']['data'] ) ) {
		...
		if ( ! empty( $exists ) ) {
			...
			$basename = str_replace( '.', '-', jk_basename( $file ) ) . '-image' . $ext;
			$uploaded = jk_upload_bits( $basename, '', $metadata['image']['data'] );
			if ( false === $uploaded['error'] ) {
				...
				$sub_attachment_id = jk_insert_attachment( $image_attachment, $uploaded['file'] );
				add_post_meta( $sub_attachment_id, '_cover_hash', $hash );
				$attach_data = jk_generate_attachment_metadata( $sub_attachment_id, $uploaded['file'] );
				jk_update_attachment_metadata( $sub_attachment_id, $attach_data );
		...
		if ( ! empty( $merged_sizes ) ) {
			...
			if ( ! is_jk_error( $editor ) ) { // No support for this type of file.
				/*
				...
				$preview_file = $dirname . jk_unique_filename( $dirname, jk_basename( $file, $ext ) . '-pdf.jpg' );

				$uploaded = $editor->save( $preview_file, 'image/jpeg' );
				unset( $editor );

				// Resize based on the full size image, rather than the source.
				if ( ! is_jk_error( $uploaded ) ) {
					...
					$metadata = _jk_make_subsizes( $merged_sizes, $image_file, $metadata, $attachment_id );
				}
			}
		}
	}
	...
	if ( ! isset( $metadata['filesize'] ) && file_exists( $file ) ) {
		$metadata['filesize'] = jk_filesize( $file );
	}
	 ...
	 */
	return apply_filters( 'jk_generate_attachment_metadata', $metadata, $attachment_id, 'create' );
}
 ...
 */
function jk_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}

	list( , , $image_type ) = jk_getimagesize( $file );
	 ...
	 */
	$exif_image_types = apply_filters( 'jk_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) );
	...
	if ( is_callable( 'exif_read_data' ) && in_array( $image_type, $exif_image_types, true ) ) {
		...
		if ( ! empty( $exif['FNumber'] ) && is_scalar( $exif['FNumber'] ) ) {
			$meta['aperture'] = round( jk_exif_frac2dec( $exif['FNumber'] ), 2 );
		}
		if ( ! empty( $exif['Model'] ) ) {
			$meta['camera'] = trim( $exif['Model'] );
		}
		if ( empty( $meta['created_timestamp'] ) && ! empty( $exif['DateTimeDigitized'] ) ) {
			$meta['created_timestamp'] = jk_exif_date2ts( $exif['DateTimeDigitized'] );
		}
		if ( ! empty( $exif['FocalLength'] ) ) {
			$meta['focal_length'] = (string) $exif['FocalLength'];
			if ( is_scalar( $exif['FocalLength'] ) ) {
				$meta['focal_length'] = (string) jk_exif_frac2dec( $exif['FocalLength'] );
			}
		}
		...
		if ( ! empty( $exif['ExposureTime'] ) ) {
			...
			if ( is_scalar( $exif['ExposureTime'] ) ) {
				$meta['shutter_speed'] = (string) jk_exif_frac2dec( $exif['ExposureTime'] );
			}
		}
	 ...
	 */
	return apply_filters( 'jk_read_image_metadata', $meta, $file, $image_type, $iptc, $exif );
}
...
function _load_image_to_edit_path( $attachment_id, $size = 'full' ) {
	...
	if ( $filepath && file_exists( $filepath ) ) {
		 ...
		 *                                    an array of width and height values in pixels (in that order).
		 */
		$filepath = apply_filters( 'load_image_to_edit_attachmenturl', jk_get_attachment_url( $attachment_id ), $attachment_id, $size );
	}
...
function _copy_image_file( $attachment_id ) {
	...
	if ( $src_file ) {
		$dst_file = str_replace( jk_basename( $dst_file ), 'copy-' . jk_basename( $dst_file ), $dst_file );
		$dst_file = dirname( $dst_file ) . '/' . jk_unique_filename( dirname( $dst_file ), jk_basename( $dst_file ) );
