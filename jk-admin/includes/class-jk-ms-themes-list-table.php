<?php
/**
 * List Table API: JK_MS_Themes_List_Table class
 *
 * @package JKPress
 ...
 * @see JK_List_Table
 */
class JK_MS_Themes_List_Table extends JK_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->show_autoupdates = jk_is_auto_update_enabled_for_type( 'theme' ) &&
	...
	public function prepare_items() {
		...
		$themes = array(
			/**
			 * Filters the full array of JK_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param JK_Theme[] $all Array of JK_Theme objects to display in the list table.
			 */
			'all'      => apply_filters( 'all_themes', jk_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : jk_get_themes( array( 'errors' => true ) ),
		);
		...
		foreach ( (array) $themes['all'] as $key => $theme ) {
			...
			$auto_update_forced = jk_is_auto_update_forced_for_item( 'theme', null, $filter_payload );
		...
		JK_Theme::sort_by_name( $this->items );
	...
	/**
	 * @param JK_Theme $theme
	 * @return bool
	 */
	public function _search_callback( $theme ) {
		static $term = null;
		if ( is_null( $term ) ) {
			$term = jk_unslash( $_REQUEST['s'] );
		}
	 ...
	 *
	 * @param JK_Theme $item The current JK_Theme object.
	 */
	 ...
	 * @global string $s
	 *
	 * @param JK_Theme $theme The current JK_Theme object.
	 */
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf(
					...
					esc_url( jk_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf(
				...
				esc_url( jk_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && ! $this->is_site_themes
			...
			$actions['delete'] = sprintf(
				...
				esc_url( jk_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param JK_Theme $theme       Current JK_Theme object.
		 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'jk_theme_update_row' );
		}
