<?php
/**
 * Administration API: JK_List_Table class
 *
 * @package JKPress
...
class JK_List_Table {
	...
	public function ajax_user_can() {
		die( 'function JK_List_Table::ajax_user_can() must be overridden in a subclass.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 *
	 * @uses JK_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @abstract
	 */
	public function prepare_items() {
		die( 'function JK_List_Table::prepare_items() must be overridden in a subclass.' );
	}
	...
	protected function set_pagination_args( $args ) {
		...
		if ( ! headers_sent() && ! jk_doing_ajax() && $args['total_pages'] > 0 && $this->get_pagenum() > $args['total_pages'] ) {
			jk_redirect( add_query_arg( 'paged', $args['total_pages'] ) );
			exit;
		}
...
<p class="search-box">
	...
	protected function bulk_actions( $which = '' ) {
		if ( is_null( $this->_actions ) ) {
			 ...
			 */
			$this->_actions = apply_filters( "bulk_actions-{$this->screen->id}", $this->_actions ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	...
	protected function months_dropdown( $post_type ) {
		...
		if ( ! is_array( $months ) ) {
			...
			if ( ! isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) {
				...
				$extra_checks = $jkdb->prepare( ' AND post_status = %s', $_GET['post_status'] );
			}
		...
		foreach ( $months as $arc_row ) {
			...
			printf(
				...
				sprintf( __( '%1$s %2$d' ), $jk_locale->get_month( $month ), $year )
			);
		}
		?>
		</select>
	...
	public function get_columns() {
		die( 'function JK_List_Table::get_columns() must be overridden in a subclass.' );
	}
	 ...
	 *
	 * Public wrapper for JK_List_Table::get_default_primary_column_name().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_column_info() {
		...
		if (
			...
			/*
			 * Backward compatibility for `$_column_headers` format prior to JKPress 4.3.
			 *
			 * In JKPress 4.3 the primary column name was added as a fourth item in the
...
<table class="jk-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	...
	/**
	 * Gets a list of CSS classes for the JK_List_Table table tag.
	 *
	 * @since 3.1.0
	 *
	...
	protected function display_tablenav( $which ) {
		if ( 'top' === $which ) {
			jk_nonce_field( 'bulk-' . $this->_args['plural'] );
		}
		?>
