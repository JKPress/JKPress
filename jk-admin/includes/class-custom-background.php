...
class Custom_Background {
	...
	public function __construct( $admin_header_callback = '', $admin_image_div_callback = '' ) {
		...
		add_action( 'jk_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'jk_ajax_set-background-image', array( $this, 'jk_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		jk_enqueue_media();
		jk_enqueue_script( 'custom-background' );
	...
	public function take_action() {
		...
		if ( isset( $_POST['reset-background'] ) ) {
			check_admin_referer( 'custom-background-reset', '_jknonce-custom-background-reset' );
		...
		if ( isset( $_POST['remove-background'] ) ) {
			...
			check_admin_referer( 'custom-background-remove', '_jknonce-custom-background-remove' );

			set_theme_mod( 'background_image', '' );
			set_theme_mod( 'background_image_thumb', '' );

			$this->updated = true;
			jk_safe_redirect( $_POST['_jk_http_referer'] );
			return;
		}
...
<td>
<form method="post">
			<?php jk_nonce_field( 'custom-background-remove', '_jknonce-custom-background-remove' ); ?>
...
<td>
<form method="post">
			<?php jk_nonce_field( 'custom-background-reset', '_jknonce-custom-background-reset' ); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="jk-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php jk_nonce_field( 'custom-background-upload', '_jknonce-custom-background-upload' ); ?>
...
<input type="text" name="background-color" id="background-color" value="#<?php echo esc_attr( get_background_color() ); ?>"<?php echo $default_color; ?>>
		...
		<?php jk_nonce_field( 'custom-background' ); ?>
	...
	public function handle_upload() {
		...
		check_admin_referer( 'custom-background-upload', '_jknonce-custom-background-upload' );
		...
		// Save the data.
		$id = jk_insert_attachment( $attachment, $file );

		// Add the metadata.
		jk_update_attachment_metadata( $id, jk_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_jk_attachment_is_custom_background', get_option( 'stylesheet' ) );

		set_theme_mod( 'background_image', sanitize_url( $url ) );

		$thumbnail = jk_get_attachment_image_src( $id, 'thumbnail' );
		...
		$file = apply_filters( 'jk_create_file_in_uploads', $file, $id ); // For replication.
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_jk_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function jk_set_background_image() {
		...
		update_post_meta( $attachment_id, '_jk_attachment_is_custom_background', get_option( 'stylesheet' ) );

		$url       = jk_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = jk_get_attachment_image_src( $attachment_id, 'thumbnail' );
