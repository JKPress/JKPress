 ...
 *     Requires at least: Optional. Specify the minimum required JKPress version.
 ...
 *     @type string $RequiresWP      Minimum required version of JKPress.
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a JK_PLUGIN_DIR relative path.
		...
		if ( $textdomain ) {
			foreach ( array( 'Name', 'PluginURI', 'Description', 'Author', 'AuthorURI', 'Version' ) as $field ) {
				if ( ! empty( $plugin_data[ $field ] ) ) {
					// phpcs:ignore JKPress.JK.I18n.LowLevelTranslationFunction,JKPress.JK.I18n.NonSingularStringLiteralText,JKPress.JK.I18n.NonSingularStringLiteralDomain
	 ...
	 */
	$plugin_data['Name']   = jk_kses( $plugin_data['Name'], $allowed_tags_in_links );
	$plugin_data['Author'] = jk_kses( $plugin_data['Author'], $allowed_tags );

	$plugin_data['Description'] = jk_kses( $plugin_data['Description'], $allowed_tags );
	$plugin_data['Version']     = jk_kses( $plugin_data['Version'], $allowed_tags );
...
function get_plugin_files( $plugin ) {
	$plugin_file = JK_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && JK_PLUGIN_DIR !== $dir ) {
 ...
 *
 * JKPress only supports plugin files in the base plugins directory
 * (jk-content/plugins) and in one directory above the plugins directory
 * (jk-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_plugins( $plugin_folder = '' ) {
	...
	$cache_plugins = jk_cache_get( 'plugins', 'plugins' );
	if ( ! $cache_plugins ) {
		$cache_plugins = array();
	}
	...
	$jk_plugins  = array();
	$plugin_root = JK_PLUGIN_DIR;
	if ( ! empty( $plugin_folder ) ) {
		$plugin_root .= $plugin_folder;
	}

	// Files in jk-content/plugins directory.
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$jk_plugins[ plugin_basename( $plugin_file ) ] = $plugin_data;
	}

	uasort( $jk_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $jk_plugins;
	jk_cache_set( 'plugins', $cache_plugins, 'plugins' );
 ...
 *
 * JKPress only includes mu-plugin files in the base mu-plugins directory (jk-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	// Files in jk-content/mu-plugins directory.
	$plugins_dir = @opendir( JKMU_PLUGIN_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( JKMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( JKMU_PLUGIN_DIR . "/$plugin_file", false, false );

		if ( empty( $plugin_data['Name'] ) ) {
			$plugin_data['Name'] = $plugin_file;
		}

		$jk_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $jk_plugins['index.php'] ) && filesize( JKMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
		unset( $jk_plugins['index.php'] );
	}
...
/**
 * Checks the jk-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// Files in jk-content directory.
	$plugins_dir = @opendir( JK_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( JK_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( JK_CONTENT_DIR . "/$plugin_file", false, false );
...
/**
 * Returns drop-in plugins that JKPress uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( 'Advanced caching plugin.' ), 'JK_CACHE' ),  // JK_CACHE
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for JKPress version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
 ...
 * @return null|JK_Error Null on success, JK_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ! empty( $redirect ) ) {
			...
			jk_redirect( add_query_arg( '_error_nonce', jk_create_nonce( 'plugin-activation-error_' . $plugin ), $redirect ) );
		}
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new JK_Error( 'unexpected_output', __( 'The plugin generated unexpected output.' ), $output );
		}
...
function deactivate_plugins( $plugins, $silent = false, $network_wide = null ) {
	...
	foreach ( (array) $plugins as $plugin ) {
		...
		if ( $do_blog && jk_is_recovery_mode() ) {
			list( $extension ) = explode( '/', $plugin );
			jk_paused_plugins()->delete( $extension );
		}
 ...
 *
 * When JK_Error is returned, it does not mean that one of the plugins had
 ...
 * @return true|JK_Error True when finished or JK_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new JK_Error( 'plugins_invalid', __( 'One of the plugins is invalid.' ), $errors );
	}
 ...
 * @return bool|null|JK_Error True on success, false if `$plugins` is empty, `JK_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = jk_nonce_url( 'plugins.php?action=delete-selected&verify-delete=1&' . implode( '&', $checked ), 'bulk-plugins' );
	...
	if ( is_jk_error( $jk_filesystem->errors ) && $jk_filesystem->errors->has_errors() ) {
		return new JK_Error( 'fs_error', __( 'Filesystem error.' ), $jk_filesystem->errors );
	}

	// Get the base plugin folder.
	$plugins_dir = $jk_filesystem->jk_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new JK_Error( 'fs_no_plugins_dir', __( 'Unable to locate JKPress plugin directory.' ) );
	}

	$plugins_dir = trailingslashit( $plugins_dir );

	$plugin_translations = jk_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			$deleted = $jk_filesystem->delete( $this_plugin_dir, true );
		} else {
			$deleted = $jk_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$jk_filesystem->delete( JK_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$jk_filesystem->delete( JK_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );
				$jk_filesystem->delete( JK_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.l10n.php' );

				$json_translation_files = glob( JK_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '-*.json' );
				if ( $json_translation_files ) {
					array_map( array( $jk_filesystem, 'delete' ), $json_translation_files );
				}
			}
		}
	}
	...
	if ( ! empty( $errors ) ) {
		...
		return new JK_Error( 'could_not_remove_plugin', sprintf( $message, implode( ', ', $errors ) ) );
	}
 ...
 *
 * @since 2.5.0
 * @return JK_Error[] Array of plugin errors keyed by plugin file name.
 */
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return int|JK_Error 0 on success, JK_Error on failure.
 */
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new JK_Error( 'plugin_invalid', __( 'Invalid plugin path.' ) );
	}
	if ( ! file_exists( JK_PLUGIN_DIR . '/' . $plugin ) ) {
		return new JK_Error( 'plugin_not_found', __( 'Plugin file does not exist.' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new JK_Error( 'no_plugin_header', __( 'The plugin does not have a valid header.' ) );
	}
	return 0;
}

/**
 * Validates the plugin requirements for JKPress version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( JK_PLUGIN_DIR . '/' . $plugin );
	...
	if ( ! $compatible_jk && ! $compatible_php ) {
		return new JK_Error(
			'plugin_jk_php_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current JKPress version, 2: Current PHP version, 3: Plugin name, 4: Required JKPress version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of JKPress (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires JKPress %4$s and PHP %5$s.', 'plugin' ),
		...
		return new JK_Error(
			'plugin_jk_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current JKPress version, 2: Plugin name, 3: Required JKPress version. */
				_x( '<strong>Error:</strong> Current JKPress version (%1$s) does not meet minimum requirements for %2$s. The plugin requires JKPress %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
	...
	JK_Plugin_Dependencies::initialize();

	if ( JK_Plugin_Dependencies::has_unmet_dependencies( $plugin ) ) {
		$dependency_names       = JK_Plugin_Dependencies::get_dependency_names( $plugin );
		$unmet_dependencies     = array();
		$unmet_dependency_names = array();

		foreach ( $dependency_names as $dependency => $dependency_name ) {
			$dependency_file = JK_Plugin_Dependencies::get_dependency_filepath( $dependency );
		...
		$error_message = sprintf(
			...
			$plugin_headers['Name'],
			count( $unmet_dependency_names ),
			implode( jk_get_list_item_separator(), $unmet_dependency_names )
		);
...
function is_uninstallable_plugin( $plugin ) {
	...
	if ( isset( $uninstallable_plugins[ $file ] ) || file_exists( JK_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		return true;
	}
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( JK_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'JK_UNINSTALL_PLUGIN', $file );

		jk_register_plugin_realpath( JK_PLUGIN_DIR . '/' . $file );
		include_once JK_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		jk_register_plugin_realpath( JK_PLUGIN_DIR . '/' . $file );
		include_once JK_PLUGIN_DIR . '/' . $file;
 ...
 *                              * Pass 'none' to leave div.jk-menu-image empty so an icon can be added via CSS.
 ...
 * @global array $menu
 * @global array $_jk_real_parent_file
 * @global bool  $_jk_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) {
	global $submenu, $menu, $_jk_real_parent_file, $_jk_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug   = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug );

	if ( isset( $_jk_real_parent_file[ $parent_slug ] ) ) {
		$parent_slug = $_jk_real_parent_file[ $parent_slug ];
	}

	if ( ! current_user_can( $capability ) ) {
		$_jk_submenu_nopriv[ $parent_slug ][ $menu_slug ] = true;
		return false;
	}
	 ...
	 * See jk-admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 * @global array  $_jk_real_parent_file
 * @global array  $_jk_menu_nopriv
 * @global array  $_jk_submenu_nopriv
 *
 * @param string $parent_page Optional. The slug name for the parent menu (or the file name
 *                            of a standard JKPress admin page). Default empty string.
 * @return string The parent file of the current admin page.
 */
function get_admin_page_parent( $parent_page = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_jk_real_parent_file, $_jk_menu_nopriv, $_jk_submenu_nopriv;

	if ( ! empty( $parent_page ) && 'admin.php' !== $parent_page ) {
		if ( isset( $_jk_real_parent_file[ $parent_page ] ) ) {
			$parent_page = $_jk_real_parent_file[ $parent_page ];
		}
	...
	if ( 'admin.php' === $pagenow && isset( $plugin_page ) ) {
		foreach ( (array) $menu as $parent_menu ) {
			if ( $parent_menu[2] === $plugin_page ) {
				...
				if ( isset( $_jk_real_parent_file[ $parent_file ] ) ) {
					$parent_file = $_jk_real_parent_file[ $parent_file ];
				}

				return $parent_file;
			}
		}
		if ( isset( $_jk_menu_nopriv[ $plugin_page ] ) ) {
			$parent_file = $plugin_page;
	...
	if ( isset( $plugin_page ) && isset( $_jk_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
		$parent_file = $pagenow;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_jk_menu_nopriv, $_jk_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( ! isset( $plugin_page ) && isset( $_jk_submenu_nopriv[ $parent ][ $pagenow ] ) ) {
		return false;
	}

	if ( isset( $plugin_page ) ) {
		if ( isset( $_jk_submenu_nopriv[ $parent ][ $plugin_page ] ) ) {
			return false;
		}
	...
	if ( empty( $parent ) ) {
		if ( isset( $_jk_menu_nopriv[ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $_jk_submenu_nopriv[ $pagenow ][ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_jk_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_jk_menu_nopriv[ $plugin_page ] ) ) {
			return false;
		}

		foreach ( array_keys( $_jk_submenu_nopriv ) as $key ) {
			if ( isset( $_jk_submenu_nopriv[ $key ][ $pagenow ] ) ) {
				return false;
			}
			if ( isset( $plugin_page ) && isset( $_jk_submenu_nopriv[ $key ][ $plugin_page ] ) ) {
				return false;
			}
		}
	...
	if ( isset( $plugin_page ) && $plugin_page === $parent && isset( $_jk_menu_nopriv[ $plugin_page ] ) ) {
		return false;
	}
...
function settings_fields( $option_group ) {
	...
	jk_nonce_field( "$option_group-options" );
}
 ...
 */
function jk_clean_plugins_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_plugins' );
	}
	jk_cache_delete( 'plugins', 'plugins' );
}
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `jk-admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
function plugin_sandbox_scrape( $plugin ) {
	...
	jk_register_plugin_realpath( JK_PLUGIN_DIR . '/' . $plugin );
	include_once JK_PLUGIN_DIR . '/' . $plugin;
}
 ...
 */
function jk_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: admin_init */
				__( 'The suggested privacy policy content should be added only in jk-admin by using the %s (or later) action.' ),
	...
	JK_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 */
function jk_get_plugin_error( $plugin ) {
	if ( ! isset( $GLOBALS['_paused_plugins'] ) ) {
		return false;
	}
 ...
 * @return true|JK_Error True on success, false if `$plugin` was not paused,
 *                       `JK_Error` on failure.
 */
function resume_plugin( $plugin, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		jk_redirect(
			add_query_arg(
				'_error_nonce',
				jk_create_nonce( 'plugin-resume-error_' . $plugin ),
				$redirect
			)
		);
	...
	$result = jk_paused_plugins()->delete( $extension );
 ...
 * upgrade due to incompatibility with the current version of JKPress.
 *
 * @since 5.8.0
 * @access private
 *
...
function deactivated_plugins_notice() {
	...
	$blog_deactivated_plugins = get_option( 'jk_force_deactivated_plugins' );
	$site_deactivated_plugins = array();

	if ( false === $blog_deactivated_plugins ) {
		// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
		update_option( 'jk_force_deactivated_plugins', array(), false );
	}

	if ( is_multisite() ) {
		$site_deactivated_plugins = get_site_option( 'jk_force_deactivated_plugins' );
		if ( false === $site_deactivated_plugins ) {
			// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
			update_site_option( 'jk_force_deactivated_plugins', array() );
		}
	}
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current JK version, 4: Compatible plugin version. */
				__( '%1$s %2$s was deactivated due to incompatibility with JKPress %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				/* translators: 1: Name of deactivated plugin, 2: Plugin version deactivated, 3: Current JK version. */
				__( '%1$s %2$s was deactivated due to incompatibility with JKPress %3$s.' ),
				$plugin['plugin_name'],
		...
		$message = sprintf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during JKPress upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
		...
		jk_admin_notice( $message, array( 'type' => 'warning' ) );
	}
