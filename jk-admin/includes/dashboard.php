<?php
/**
 * JKPress Dashboard Widget Administration Screen API
 *
 * @package JKPress
 ...
 * @global callable[] $jk_dashboard_control_callbacks
 */
function jk_dashboard_setup() {
	global $jk_registered_widgets, $jk_registered_widget_controls, $jk_dashboard_control_callbacks;

	$screen = get_current_screen();

	/* Register Widgets and Controls */
	$jk_dashboard_control_callbacks = array();

	// Browser version
	$check_browser = jk_check_browser_version();

	if ( $check_browser && $check_browser['upgrade'] ) {
		add_filter( 'postbox_classes_dashboard_dashboard_browser_nag', 'dashboard_browser_nag_class' );

		if ( $check_browser['insecure'] ) {
			jk_add_dashboard_widget( 'dashboard_browser_nag', __( 'You are using an insecure browser!' ), 'jk_dashboard_browser_nag' );
		} else {
			jk_add_dashboard_widget( 'dashboard_browser_nag', __( 'Your browser is out of date!' ), 'jk_dashboard_browser_nag' );
		}
	}

	// PHP Version.
	$check_php = jk_check_php_version();
	...
	if ( $check_php && current_user_can( 'update_php' ) ) {
		...
		if ( isset( $check_php['is_acceptable'] ) && ! $check_php['is_acceptable'] ) {
			...
			if ( $check_php['is_lower_than_future_minimum'] ) {
				jk_add_dashboard_widget( 'dashboard_php_nag', __( 'PHP Update Required' ), 'jk_dashboard_php_nag' );
			} else {
				jk_add_dashboard_widget( 'dashboard_php_nag', __( 'PHP Update Recommended' ), 'jk_dashboard_php_nag' );
			}
		}
	}
	...
	if ( current_user_can( 'view_site_health_checks' ) && ! is_network_admin() ) {
		...
		jk_enqueue_style( 'site-health' );
		jk_enqueue_script( 'site-health' );

		jk_add_dashboard_widget( 'dashboard_site_health', __( 'Site Health Status' ), 'jk_dashboard_site_health' );
	}

	// Right Now.
	if ( is_blog_admin() && current_user_can( 'edit_posts' ) ) {
		jk_add_dashboard_widget( 'dashboard_right_now', __( 'At a Glance' ), 'jk_dashboard_right_now' );
	}

	if ( is_network_admin() ) {
		jk_add_dashboard_widget( 'network_dashboard_right_now', __( 'Right Now' ), 'jk_network_dashboard_right_now' );
	}

	// Activity Widget.
	if ( is_blog_admin() ) {
		jk_add_dashboard_widget( 'dashboard_activity', __( 'Activity' ), 'jk_dashboard_site_activity' );
	}
	...
	if ( is_blog_admin() && current_user_can( get_post_type_object( 'post' )->cap->create_posts ) ) {
		...
		jk_add_dashboard_widget( 'dashboard_quick_press', $quick_draft_title, 'jk_dashboard_quick_press' );
	}

	// JKPress Events and News.
	jk_add_dashboard_widget( 'dashboard_primary', __( 'JKPress Events and News' ), 'jk_dashboard_events_news' );
	...
	if ( is_network_admin() ) {
		 ...
		 * @since 3.1.0
		 */
		do_action( 'jk_network_dashboard_setup' );
		 ...
		 */
		$dashboard_widgets = apply_filters( 'jk_network_dashboard_widgets', array() );
		 ...
		 * @since 3.1.0
		 */
		do_action( 'jk_user_dashboard_setup' );
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'jk_user_dashboard_widgets', array() );
	} else {
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'jk_dashboard_widgets', array() );
	}

	foreach ( $dashboard_widgets as $widget_id ) {
		$name = empty( $jk_registered_widgets[ $widget_id ]['all_link'] ) ? $jk_registered_widgets[ $widget_id ]['name'] : $jk_registered_widgets[ $widget_id ]['name'] . " <a href='{$jk_registered_widgets[$widget_id]['all_link']}' class='edit-box open-box'>" . __( 'View all' ) . '</a>';
		jk_add_dashboard_widget( $widget_id, $name, $jk_registered_widgets[ $widget_id ]['callback'], $jk_registered_widget_controls[ $widget_id ]['callback'] );
	}

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		check_admin_referer( 'edit-dashboard-widget_' . $_POST['widget_id'], 'dashboard-widget-nonce' );
		ob_start(); // Hack - but the same hack jk-admin/widgets.php uses.
		jk_dashboard_trigger_widget_control( $_POST['widget_id'] );
		ob_end_clean();
		jk_redirect( remove_query_arg( 'edit' ) );
		exit;
	}

	/** This action is documented in jk-admin/includes/meta-boxes.php */
 ...
 */
function jk_add_dashboard_widget( $widget_id, $widget_name, $callback, $control_callback = null, $callback_args = null, $context = 'normal', $priority = 'core' ) {
	global $jk_dashboard_control_callbacks;
	...
	if ( $control_callback && is_callable( $control_callback ) && current_user_can( 'edit_dashboard' ) ) {
		$jk_dashboard_control_callbacks[ $widget_id ] = $control_callback;

		if ( isset( $_GET['edit'] ) && $widget_id === $_GET['edit'] ) {
			list($url)    = explode( '#', add_query_arg( 'edit', false ), 2 );
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( $url ) . '">' . __( 'Cancel' ) . '</a></span>';
			$callback     = '_jk_dashboard_control_callback';
		} else {
 ...
 */
function _jk_dashboard_control_callback( $dashboard, $meta_box ) {
	echo '<form method="post" class="dashboard-widget-control-form jk-clearfix">';
	jk_dashboard_trigger_widget_control( $meta_box['id'] );
	jk_nonce_field( 'edit-dashboard-widget_' . $meta_box['id'], 'dashboard-widget-nonce' );
 ...
 * @since 2.7.0
 */
function jk_dashboard_right_now() {
	?>
	<div class="main">
	<ul>
	<?php
	...
	foreach ( array( 'post', 'page' ) as $post_type ) {
		$num_posts = jk_count_posts( $post_type );
 ...
 * @since 3.1.0
 */
function jk_network_dashboard_right_now() {
	$actions = array();
 ...
 */
function jk_dashboard_quick_press( $error_msg = false ) {
	global $post_ID;
 ...
 * @since 2.7.0
 *
 * @param JK_Post[]|false $drafts Optional. Array of posts to display. Default false.
 */
function jk_dashboard_recent_drafts( $drafts = false ) {
	if ( ! $drafts ) {
		$query_args = array(
			'post_type'      => 'post',
			'post_status'    => 'draft',
	...
	foreach ( $drafts as $draft ) {
		...
		$the_content = jk_trim_words( $draft->post_content, $draft_length );
 ...
 *
 * @param JK_Comment $comment   The current comment.
 * @param bool       $show_date Optional. Whether to display the date.
 */
function _jk_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		...
		$approve_nonce = esc_html( '_jknonce=' . jk_create_nonce( 'approve-comment_' . $comment->comment_ID ) );
		$del_nonce     = esc_html( '_jknonce=' . jk_create_nonce( 'delete-comment_' . $comment->comment_ID ) );
		...
		$actions['approve'] = sprintf(
			'<a href="%s" data-jk-lists="%s" class="vim-a aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $approve_url ),
		...
		$actions['unapprove'] = sprintf(
			'<a href="%s" data-jk-lists="%s" class="vim-u aria-button-if-js" aria-label="%s">%s</a>',
		...
		$actions['spam'] = sprintf(
			'<a href="%s" data-jk-lists="%s" class="vim-s vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $spam_url ),
		...
		if ( ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = sprintf(
				'<a href="%s" data-jk-lists="%s" class="delete vim-d vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				esc_url( $delete_url ),
		...
		/** This filter is documented in jk-admin/includes/class-jk-comments-list-table.php */
	...
	?>
		...
		<li id="comment-<?php echo $comment->comment_ID; ?>" <?php comment_class( array( 'comment-item', jk_get_comment_status( $comment ) ), $comment ); ?>>
 ...
 * @since 3.8.0
 */
function jk_dashboard_site_activity() {

	echo '<div id="activity-widget">';

	$future_posts = jk_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'future',
			'order'  => 'ASC',
	...
	$recent_posts = jk_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'publish',
			'order'  => 'DESC',
	...
	$recent_comments = jk_dashboard_recent_comments();
 ...
 */
function jk_dashboard_recent_posts( $args ) {
	$query_args = array(
		'post_type'      => 'post',
	 ...
	 * @since 4.2.0
	 *
	 * @param array $query_args The arguments passed to JK_Query to produce the list of posts.
	 */
	$query_args = apply_filters( 'dashboard_recent_posts_query_args', $query_args );

	$posts = new JK_Query( $query_args );
 ...
 */
function jk_dashboard_recent_comments( $total_items = 5 ) {
	...
	if ( $comments ) {
		...
		echo '<ul id="the-comment-list" data-jk-lists="list:comment">';
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			'</h3>';
			_get_list_table( 'JK_Comments_List_Table' )->views();
		}

		jk_comment_reply( -1, false, 'dashboard', false );
		jk_comment_trashnotice();
 ...
 */
function jk_dashboard_rss_output( $widget_id ) {
	$widgets = get_option( 'dashboard_widget_options' );
	echo '<div class="rss-widget">';
	jk_widget_rss_output( $widgets[ $widget_id ] );
	echo '</div>';
}
 ...
 * which is later replaced by Ajax call (see top of /jk-admin/index.php)
 *
 * @since 2.5.0
 ...
 */
function jk_dashboard_cached_rss_widget( $widget_id, $callback, $check_urls = array(), ...$args ) {
	$doing_ajax = jk_doing_ajax();
	$loading    = '<p class="widget-loading hide-if-no-js">' . __( 'Loading&hellip;' ) . '</p>';
	$loading   .= jk_get_admin_notice(
 ...
 */
function jk_dashboard_trigger_widget_control( $widget_control_id = false ) {
	global $jk_dashboard_control_callbacks;

	if ( is_scalar( $widget_control_id ) && $widget_control_id
		&& isset( $jk_dashboard_control_callbacks[ $widget_control_id ] )
		&& is_callable( $jk_dashboard_control_callbacks[ $widget_control_id ] )
	) {
		call_user_func(
			$jk_dashboard_control_callbacks[ $widget_control_id ],
			'',
			array(
				'id'       => $widget_control_id,
				'callback' => $jk_dashboard_control_callbacks[ $widget_control_id ],
			)
		);
	}
}
...
/**
 * Sets up the RSS dashboard widget control and $args to be used as input to jk_widget_rss_form().
 *
 ...
 */
function jk_dashboard_rss_control( $widget_id, $form_inputs = array() ) {
	...
	$number = 1; // Hack to use jk_widget_rss_form().

	$widget_options[ $widget_id ]['number'] = $number;

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget-rss'][ $number ] ) ) {
		$_POST['widget-rss'][ $number ]         = jk_unslash( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]           = jk_widget_rss_process( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]['number'] = $number;
	...
	jk_widget_rss_form( $widget_options[ $widget_id ], $form_inputs );
}
 ...
 * @since 4.8.0
 */
function jk_dashboard_events_news() {
	jk_print_community_events_markup();

	?>

	<div class="wordpress-news hide-if-no-js">
		<?php jk_dashboard_primary(); ?>
	</div>
 ...
 * @since 4.8.0
 */
function jk_print_community_events_markup() {
	 ...
	 * won't be ready until jk.communityEvents.renderEventsTemplate() has run.
	 */
	?>
 ...
 * @since 4.8.0
 */
function jk_print_community_events_templates() {
	?>
	...
	<script id="tmpl-community-events-event-list" type="text/template">
		<# _.each( data.events, function( event ) { #>
			<li class="event event-{{ event.type }} jk-clearfix">
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'There are no events scheduled near %1$s at the moment. Would you like to <a href="%2$s">organize a JKPress event</a>?' ),
			...
			<# } else { #>
				...
				printf(
					...
					__( 'There are no events scheduled near you at the moment. Would you like to <a href="%s">organize a JKPress event</a>?' ),
					__( 'https://make.wordpress.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			<# } #>
...
/**
 * 'JKPress Events and News' dashboard widget.
 *
 * @since 2.7.0
 * @since 4.8.0 Removed popular plugins feed.
 */
function jk_dashboard_primary() {
	$feeds = array(
		'news'   => array(

			/**
			 * Filters the primary link URL for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			...
			/**
			 * Filters the primary feed URL for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the primary link title for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's primary link.
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'JKPress Blog' ) ),
			'items'        => 2,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary feed URL for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary link title for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's secondary link.
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other JKPress News' ) ),

			/**
			 * Filters the number of secondary link items for the 'JKPress Events and News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
	...
	jk_dashboard_cached_rss_widget( 'dashboard_primary', 'jk_dashboard_primary_output', $feeds );
}

/**
 * Displays the JKPress events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
 ...
 */
function jk_dashboard_primary_output( $widget_id, $feeds ) {
	foreach ( $feeds as $type => $args ) {
		$args['type'] = $type;
		echo '<div class="rss-widget">';
			jk_widget_rss_output( $args['url'], $args );
		echo '</div>';
	}
}
 ...
 * @global bool $is_IE
 */
function jk_dashboard_browser_nag() {
	global $is_IE;

	$notice   = '';
	$response = jk_check_browser_version();

	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best JKPress experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best JKPress experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best JKPress experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 *                              false on failure. See jk_check_browser_version().
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function jk_check_browser_version() {
	...
	if ( false === $response ) {
		...
		$options = array(
			...
			'user-agent' => 'JKPress/' . jk_get_jk_version() . '; ' . home_url( '/' ),
		);
		...
		$response = jk_remote_post( $url, $options );
		 ...
		 */
		$response = json_decode( jk_remote_retrieve_body( $response ), true );
...
function jk_dashboard_php_nag() {
	...
	if ( isset( $response['is_secure'] ) && ! $response['is_secure'] ) {
		...
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				...
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by JKPress. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade JKPress.' ),
				PHP_VERSION
			);
		} else {
			$message = sprintf(
		...
		$message = sprintf(
			/* translators: %s: The server PHP version. */
			__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by JKPress. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade JKPress.' ),
			PHP_VERSION
		);
	} else {
		$message = sprintf(
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build JKPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
	...
	jk_update_php_annotation();
	jk_direct_php_update_button();
}
 ...
 * @since 5.4.0
 */
function jk_dashboard_site_health() {
 ...
 * @since 2.5.0
 */
function jk_dashboard_empty() {}

/**
 * Displays a welcome panel to introduce users to JKPress.
 *
 * @since 3.3.0
 * @since 5.9.0 Send users to the Site Editor if the active theme is block-based.
 */
function jk_welcome_panel() {
	list( $display_version ) = explode( '-', jk_get_jk_version() );
	...
	<div class="welcome-panel-header">
		...
		<h2><?php _e( 'Welcome to JKPress!' ); ?></h2>
		<p>
			<a href="<?php echo esc_url( admin_url( 'about.php' ) ); ?>">
			<?php
				/* translators: %s: Current JKPress version. */
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<?php if ( $can_customize ) : ?>
					<a class="load-customize hide-if-no-customize" href="<?php echo jk_customize_url(); ?>"><?php _e( 'Open the Customizer' ); ?></a>
				<?php endif; ?>
			<?php endif; ?>
			</div>
		</div>
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<p><?php _e( 'There is a new kind of JKPress theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?></p>
