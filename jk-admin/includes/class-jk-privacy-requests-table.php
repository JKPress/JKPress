<?php
/**
 * List Table API: JK_Privacy_Requests_Table class
 *
 * @package JKPress
...
abstract class JK_Privacy_Requests_Table extends JK_List_Table {

	/**
	 * Action name for the requests this table will work with. Classes
	 * which inherit from JK_Privacy_Requests_Table should define this.
	 *
	...
	protected function get_request_counts() {
		...
		$results = (array) $jkdb->get_results( $jkdb->prepare( $query, $this->post_type, $this->request_type ), ARRAY_A );
	...
	public function process_bulk_action() {
		...
		$request_ids = isset( $_REQUEST['request_id'] ) ? jk_parse_id_list( jk_unslash( $_REQUEST['request_id'] ) ) : array();
		...
		switch ( $action ) {
			case 'resend':
				foreach ( $request_ids as $request_id ) {
					$resend = _jk_privacy_resend_request( $request_id );

					if ( $resend && ! is_jk_error( $resend ) ) {
						++$count;
					} else {
						++$failures;
					}
			...
			case 'complete':
				foreach ( $request_ids as $request_id ) {
					$result = _jk_privacy_completed_request( $request_id );
			...
			case 'delete':
				foreach ( $request_ids as $request_id ) {
					if ( jk_delete_post( $request_id, true ) ) {
						++$count;
					} else {
						++$failures;
					}
	...
	public function prepare_items() {
		...
		$requests_query = new JK_Query( $args );
		$requests       = $requests_query->posts;

		foreach ( $requests as $request ) {
			$this->items[] = jk_get_user_request( $request->ID );
		}
	 ...
	 * @since 4.9.6
	 *
	 * @param JK_User_Request $item Item being shown.
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param JK_User_Request $item        The item being shown.
		 */
	 ...
	 * @since 4.9.6
	 *
	 * @param JK_User_Request $item The current item.
	 */
