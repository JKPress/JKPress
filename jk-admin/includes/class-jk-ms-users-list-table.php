<?php
/**
 * List Table API: JK_MS_Users_List_Table class
 *
 * @package JKPress
 ...
 * @see JK_List_Table
 */
class JK_MS_Users_List_Table extends JK_List_Table {
	/**
	 * @return bool
	 */
	...
	public function prepare_items() {
		 ...
		 */
		if ( ! $usersearch && jk_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in jk-admin/includes/class-jk-users-list-table.php */
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'jkmu_users_columns', $users_columns );
	}
	 ...
	 *
	 * @param JK_User $item The current JK_User object.
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_User $user The current JK_User object.
	 */
	...
	public function column_username( $user ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			$edit_link = esc_url( add_query_arg( 'jk_http_referer', urlencode( jk_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	public function column_blogs( $user ) {
		...
		foreach ( $blogs as $site ) {
			 ...
			 * @param JK_User  $user         JK_User object.
			 */
	 ...
	 * @since 5.9.0 Renamed `$user` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param JK_User $item        User being acted upon.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins, true ) ) {
			$actions['delete'] = '<a href="' . esc_url( network_admin_url( add_query_arg( '_jk_http_referer', urlencode( jk_unslash( $_SERVER['REQUEST_URI'] ) ), jk_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
