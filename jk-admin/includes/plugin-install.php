<?php
/**
 * JKPress Plugin Install Administration API
 *
 * @package JKPress
...
/**
 * Retrieves plugin installer pages from the JKPress.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the JKPress.org
 ...
 *         @type bool $requires          Whether to return the required JKPress version. Default true.
 ...
 *         @type bool $compatibility     Whether to return the JKPress compatibility list. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the JKPress.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current JKPress.org Plugin Installation API request.
	 *
	...
	if ( false === $res ) {
		...
		if ( is_jk_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new JK_Error( 'plugins_api_failed', $res->error );
			}
		}
	} elseif ( ! is_jk_error( $res ) ) {
		$res->external = true;
	}
	 ...
	 * @since 2.7.0
	 *
	 * @param object|JK_Error $res    Response object or JK_Error.
	 * @param string          $action The type of information being requested from the Plugin Installation API.
	 * @param object          $args   Plugin API arguments.
	 */
...
/**
 * Retrieves popular JKPress plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	if ( is_jk_error( $api_tags ) ) {
		echo $api_tags->get_error_message();
	} else {
		// Set up the tags in a way which can be interpreted by jk_generate_tag_cloud().
		$tags = array();
...
function install_search_form( $deprecated = true ) {
	$type = isset( $_REQUEST['type'] ) ? jk_unslash( $_REQUEST['type'] ) : 'term';
	$term = isset( $_REQUEST['s'] ) ? urldecode( jk_unslash( $_REQUEST['s'] ) ) : '';
	?>
	<form class="search-form search-plugins" method="get">
		<input type="hidden" name="tab" value="search" />
		<label for="search-plugins"><?php _e( 'Search Plugins' ); ?></label>
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr( $term ); ?>" class="jk-filter-search" />
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="jk-upload-form" action="<?php echo esc_url( self_admin_url( 'update.php?action=upload-plugin' ) ); ?>">
		<?php jk_nonce_field( 'plugin-upload' ); ?>
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on JKPress.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your JKPress.org username:' ); ?></label>
			<input type="search" id="user" name="user" value="<?php echo esc_attr( $user ); ?>" />
			<input type="submit" class="button" value="<?php esc_attr_e( 'Get Favorites' ); ?>" />
			<input type="hidden" id="wporg-username-nonce" name="_jknonce" value="<?php echo esc_attr( jk_create_nonce( $action ) ); ?>" />
		</p>
	</form>
	<?php
}
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of JKPress. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
		...
		case 'install_plugins_featured':
			printf(
				...
				'<p>' . __( 'Plugins extend and expand the functionality of JKPress. You may install plugins in the <a href="%s">JKPress Plugin Directory</a> right from here, or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
...
function install_plugin_install_status( $api, $loop = false ) {
		...
		foreach ( (array) $update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can( 'update_plugins' ) ) {
					$url = jk_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . $update_file ), 'upgrade-plugin_' . $update_file );
				}
				break;
			}
		}
	...
	if ( 'install' === $status ) {
		if ( is_dir( JK_PLUGIN_DIR . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins( '/' . $api->slug );
			if ( empty( $installed_plugin ) ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$url = jk_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . $api->slug ), 'install-plugin_' . $api->slug );
				}
			} else {
	...
	if ( isset( $_GET['from'] ) ) {
		$url .= '&amp;from=' . urlencode( jk_unslash( $_GET['from'] ) );
	}
...
function install_plugin_information() {
	...
	$api = plugins_api(
		'plugin_information',
		array(
			'slug' => jk_unslash( $_REQUEST['plugin'] ),
		)
	);
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$api->sections[ $section_name ] = jk_kses( $content, $plugins_allowedtags );
	}

	foreach ( array( 'version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug' ) as $key ) {
		if ( isset( $api->$key ) ) {
			$api->$key = jk_kses( $api->$key, $plugins_allowedtags );
		}
	}
	...
	$section = isset( $_REQUEST['section'] ) ? jk_unslash( $_REQUEST['section'] ) : 'description';
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires JKPress Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://wordpress.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'JKPress.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on JKPress.org or write your own!' ); ?></p>
			<?php
	...
	$requires_jk  = isset( $api->requires ) ? $api->requires : null;

	$compatible_php = is_php_version_compatible( $requires_php );
	$compatible_jk  = is_jk_version_compatible( $requires_jk );
	$tested_jk      = ( empty( $api->tested ) || version_compare( get_bloginfo( 'version' ), $api->tested, '<=' ) );
	...
	if ( ! $compatible_php ) {
		...
		if ( current_user_can( 'update_php' ) ) {
			...
			) . jk_update_php_annotation( '</p><p><em>', '</em>', false );
		} else {
			$compatible_php_notice_message .= '</p>';
		}
	...
	if ( ! $tested_jk ) {
		jk_admin_notice(
			__( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of JKPress.' ),
			array(
				'type'               => 'warning',
				'additional_classes' => array( 'notice-alt' ),
			)
		...
		$compatible_jk_notice_message = __( '<strong>Error:</strong> This plugin <strong>requires a newer version of JKPress</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			$compatible_jk_notice_message .= sprintf(
				/* translators: %s: URL to JKPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update JKPress</a>.' ),
				esc_url( self_admin_url( 'update-core.php' ) )
			);
		}

		jk_admin_notice(
			$compatible_jk_notice_message,
			array(
				'type'               => 'error',
				'additional_classes' => array( 'notice-alt' ),
			)
	...
	if ( ! empty( $api->download_link ) && ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) ) {
		$button = jk_get_plugin_action_button( $api->name, $api, $compatible_php, $compatible_jk );
 ...
 * @param bool         $compatible_jk    The result of a JK compatibility check.
 * @return string The markup for the dependency row button. An empty string if the user does not have capabilities.
 */
function jk_get_plugin_action_button( $name, $data, $compatible_php, $compatible_jk ) {
	$button           = '';
	$data             = (object) $data;
	...
	if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
		switch ( $status['status'] ) {
			case 'install':
				if ( $status['url'] ) {
					if ( $compatible_php && $compatible_jk && $all_plugin_dependencies_installed && ! empty( $data->download_link ) ) {
						$button = sprintf(
			...
			case 'update_available':
				if ( $status['url'] ) {
					if ( $compatible_php && $compatible_jk ) {
						$button = sprintf(
			...
			case 'newer_installed':
				if ( is_plugin_active( $status['file'] ) ) {
					...
					if ( $compatible_php && $compatible_jk && $all_plugin_dependencies_active ) {
						$button_text = _x( 'Activate', 'plugin' );
						/* translators: %s: Plugin name. */
						$button_label = _x( 'Activate %s', 'plugin' );
						$activate_url = add_query_arg(
							array(
								'_jknonce' => jk_create_nonce( 'activate-plugin_' . $status['file'] ),
								'action'   => 'activate',
								'plugin'   => $status['file'],
							),
