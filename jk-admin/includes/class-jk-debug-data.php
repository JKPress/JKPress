<?php
/**
 * Class for providing debug data based on a users JKPress environment.
 *
 * @package JKPress
...
class JK_Debug_Data {
	...
	public static function debug_data() {
		 ...
		 */
		$info = array(
			'jk-core'             => self::get_jk_core(),
			'jk-paths-sizes'      => self::get_jk_paths_sizes(),
			'jk-dropins'          => self::get_jk_dropins(),
			'jk-active-theme'     => self::get_jk_active_theme(),
			'jk-parent-theme'     => self::get_jk_parent_theme(),
			'jk-themes-inactive'  => self::get_jk_themes_inactive(),
			'jk-mu-plugins'       => self::get_jk_mu_plugins(),
			'jk-plugins-active'   => self::get_jk_plugins_active(),
			'jk-plugins-inactive' => self::get_jk_plugins_inactive(),
			'jk-media'            => self::get_jk_media(),
			'jk-server'           => self::get_jk_server(),
			'jk-database'         => self::get_jk_database(),
			'jk-constants'        => self::get_jk_constants(),
			'jk-filesystem'       => self::get_jk_filesystem(),
		);
		 ...
		 *
		 * Array keys for sections added by core are all prefixed with `jk-`. Plugins and themes
	...
	/**
	 * Gets the JKPress core section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_core(): array {
		...
		$environment_type       = jk_get_environment_type();
		$core_version           = jk_get_jk_version();
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest JKPress version number. */
		...
		if ( is_multisite() ) {
			...
			$network_query = new JK_Network_Query();
		...
		// JKPress features requiring processing.
		$jk_dotorg = jk_remote_get( 'https://wordpress.org', array( 'timeout' => 10 ) );

		if ( ! is_jk_error( $jk_dotorg ) ) {
			$fields['dotorg_communication'] = array(
				'label' => __( 'Communication with JKPress.org' ),
				'value' => __( 'JKPress.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$fields['dotorg_communication'] = array(
				...
				/* translators: 1: The IP address JKPress.org resolves to. 2: The error returned by the lookup. */
					__( 'Unable to reach JKPress.org at %1$s: %2$s' ),
					gethostbyname( 'wordpress.org' ),
					$jk_dotorg->get_error_message()
				),
				'debug' => $jk_dotorg->get_error_message(),
			);
		}
	...
	/**
	 * Gets the JKPress drop-in section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_dropins(): array {
		...
		return array(
			...
			'show_count'  => true,
			'description' => sprintf(
				/* translators: %s: jk-content directory name. */
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance JKPress features in ways that are not possible for traditional plugins.' ),
				'<code>' . str_replace( ABSPATH, '', JK_CONTENT_DIR ) . '</code>'
			),
			'fields'      => $fields,
		);
	}
	...
	/**
	 * Gets the JKPress server section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_server(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			if ( JK_Site_Health::get_instance()->php_memory_limit !== ini_get( 'memory_limit' ) ) {
				$fields['memory_limit']       = array(
					'label' => __( 'PHP memory limit' ),
					'value' => JK_Site_Health::get_instance()->php_memory_limit,
				);
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core JKPress rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN JKPress[\s\S]+?# END JKPress/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core JKPress features.' ), '.htaccess' );
			}
		...
		$fields['server-time'] = array(
			...
			'value' => jk_date( 'c', $_SERVER['REQUEST_TIME'] ),
		);
	...
	/**
	 * Gets the JKPress media section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @throws ImagickException
	 * @return array
	 */
	private static function get_jk_media(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			$effective           = min( jk_convert_hr_to_bytes( $post_max_size ), jk_convert_hr_to_bytes( $upload_max_filesize ) );
		...
		if ( 'JK_Image_Editor_Imagick' === _jk_image_editor_choose() && isset( $imagick ) && $imagick instanceof Imagick ) {
			$limits = array(
	...
	/**
	 * Gets the JKPress MU plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_mu_plugins(): array {
	...
	/**
	 * Gets the JKPress paths and sizes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 */
	private static function get_jk_paths_sizes(): ?array {
		if ( is_multisite() ) {
			return null;
		}
		...
		$fields = array(
			'wordpress_path' => array(
				'label' => __( 'JKPress directory location' ),
				'value' => untrailingslashit( ABSPATH ),
			),
			'wordpress_size' => array(
				'label' => __( 'JKPress directory size' ),
				'value' => $loading,
				'debug' => 'loading...',
			),
			'uploads_path'   => array(
				'label' => __( 'Uploads directory location' ),
				'value' => jk_upload_dir()['basedir'],
			),
			'uploads_size'   => array(
	...
	/**
	 * Gets the JKPress active plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_plugins_active(): array {
		return array(
			'label'      => __( 'Active Plugins' ),
			'show_count' => true,
			'fields'     => self::get_jk_plugins_raw_data()['jk-plugins-active'],
		);
	}

	/**
	 * Gets the JKPress inactive plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_plugins_inactive(): array {
		return array(
			'label'      => __( 'Inactive Plugins' ),
			'show_count' => true,
			'fields'     => self::get_jk_plugins_raw_data()['jk-plugins-inactive'],
		);
	}

	/**
	 * Gets the raw plugin data for the JKPress active and inactive sections of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_plugins_raw_data(): array {
		...
		$auto_updates = array();
		$fields       = array(
			'jk-plugins-active'   => array(),
			'jk-plugins-inactive' => array(),
		);

		$auto_updates_enabled = jk_is_auto_update_enabled_for_type( 'plugin' );
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			$plugin_part = ( is_plugin_active( $plugin_path ) ) ? 'jk-plugins-active' : 'jk-plugins-inactive';
			...
			if ( $auto_updates_enabled ) {
				if ( isset( $transient->response[ $plugin_path ] ) ) {
					...
					$item = jk_parse_args( $plugin, $item );
				}

				$auto_update_forced = jk_is_auto_update_forced_for_item( 'plugin', null, (object) $item );
	...
	/**
	 * Gets the JKPress active theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 * @return array
	 */
	private static function get_jk_active_theme(): array {
		global $_jk_theme_features;

		// Populate the section for the currently active theme.
		$theme_features = array();

		if ( ! empty( $_jk_theme_features ) ) {
			foreach ( $_jk_theme_features as $feature => $options ) {
		...
		$auto_updates         = array();
		$auto_updates_enabled = jk_is_auto_update_enabled_for_type( 'theme' );
		if ( $auto_updates_enabled ) {
		...
		$fields = array(
			...
			'author'         => array(
				'label' => __( 'Author' ),
				'value' => jk_kses( $active_theme->author, array() ),
			),
		...
		if ( $auto_updates_enabled ) {
			...
			$auto_update_forced = jk_is_auto_update_forced_for_item( 'theme', null, (object) $item );
			...
			/** This filter is documented in jk-admin/includes/class-jk-debug-data.php */
	...
	/**
	 * Gets the JKPress parent theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_parent_theme(): array {
		$theme_updates = get_theme_updates();
		$transient     = get_site_transient( 'update_themes' );
		...
		if ( $parent_theme ) {
			...
			$fields = array(
				...
				'author'         => array(
					'label' => __( 'Author' ),
					'value' => jk_kses( $parent_theme->author, array() ),
				),
				'author_website' => array(
					'label' => __( 'Author website' ),
	...
	/**
	 * Gets the JKPress inactive themes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_themes_inactive(): array {
		$active_theme  = jk_get_theme();
		$parent_theme  = $active_theme->parent();
		$theme_updates = get_theme_updates();
		...
		foreach ( $all_themes as $theme_slug => $theme ) {
			...
			// Sanitize.
			$theme_author = jk_kses( $theme_author, array() );
			...
			if ( $auto_updates_enabled ) {
				 ...
				 *
				 * @param string   $auto_updates_string The string output for the auto-updates column.
				 * @param JK_Theme $theme               An object of theme data.
				 * @param bool     $enabled             Whether auto-updates are enabled for this item.
				 */
	...
	/**
	 * Gets the JKPress constants section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_jk_constants(): array {
		// Check if JK_DEBUG_LOG is set.
		$jk_debug_log_value = __( 'Disabled' );
		if ( is_string( JK_DEBUG_LOG ) ) {
			$jk_debug_log_value = JK_DEBUG_LOG;
		} elseif ( JK_DEBUG_LOG ) {
			$jk_debug_log_value = __( 'Enabled' );
		}
		...
		// Check JK_ENVIRONMENT_TYPE.
		if ( defined( 'JK_ENVIRONMENT_TYPE' ) ) {
			$jk_environment_type       = JK_ENVIRONMENT_TYPE ? JK_ENVIRONMENT_TYPE : __( 'Empty value' );
			$jk_environment_type_debug = JK_ENVIRONMENT_TYPE;
		} else {
			$jk_environment_type       = __( 'Undefined' );
			$jk_environment_type_debug = 'undefined';
		}
		...
		$fields = array(
			...
			'JK_HOME'             => array(
				'label' => 'JK_HOME',
				'value' => ( defined( 'JK_HOME' ) ? JK_HOME : __( 'Undefined' ) ),
				'debug' => ( defined( 'JK_HOME' ) ? JK_HOME : 'undefined' ),
			),
			'JK_SITEURL'          => array(
				'label' => 'JK_SITEURL',
				'value' => ( defined( 'JK_SITEURL' ) ? JK_SITEURL : __( 'Undefined' ) ),
				'debug' => ( defined( 'JK_SITEURL' ) ? JK_SITEURL : 'undefined' ),
			),
			'JK_CONTENT_DIR'      => array(
				'label' => 'JK_CONTENT_DIR',
				'value' => JK_CONTENT_DIR,
			),
			'JK_PLUGIN_DIR'       => array(
				'label' => 'JK_PLUGIN_DIR',
				'value' => JK_PLUGIN_DIR,
			),
			'JK_MEMORY_LIMIT'     => array(
				'label' => 'JK_MEMORY_LIMIT',
				'value' => JK_MEMORY_LIMIT,
			),
			'JK_MAX_MEMORY_LIMIT' => array(
				'label' => 'JK_MAX_MEMORY_LIMIT',
				'value' => JK_MAX_MEMORY_LIMIT,
			),
			'JK_DEBUG'            => array(
				'label' => 'JK_DEBUG',
				'value' => JK_DEBUG ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => JK_DEBUG,
			),
			'JK_DEBUG_DISPLAY'    => array(
				'label' => 'JK_DEBUG_DISPLAY',
				'value' => JK_DEBUG_DISPLAY ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => JK_DEBUG_DISPLAY,
			),
			'JK_DEBUG_LOG'        => array(
				'label' => 'JK_DEBUG_LOG',
				'value' => $jk_debug_log_value,
				'debug' => JK_DEBUG_LOG,
			),
			'SCRIPT_DEBUG'        => array(
				'label' => 'SCRIPT_DEBUG',
			...
			'JK_CACHE'            => array(
				'label' => 'JK_CACHE',
				'value' => JK_CACHE ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => JK_CACHE,
			),
			...
			'JK_ENVIRONMENT_TYPE' => array(
				'label' => 'JK_ENVIRONMENT_TYPE',
				'value' => $jk_environment_type,
				'debug' => $jk_environment_type_debug,
			),
			'JK_DEVELOPMENT_MODE' => array(
				'label' => 'JK_DEVELOPMENT_MODE',
				'value' => JK_DEVELOPMENT_MODE ? JK_DEVELOPMENT_MODE : __( 'Disabled' ),
				'debug' => JK_DEVELOPMENT_MODE,
			),
			'DB_CHARSET'          => array(
				'label' => 'DB_CHARSET',
		...
		return array(
			'label'       => __( 'JKPress Constants' ),
			'description' => __( 'These settings alter where and how parts of JKPress are loaded.' ),
			'fields'      => $fields,
		);
	}

	/**
	 * Gets the JKPress database section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 * @return array
	 */
	private static function get_jk_database(): array {
		global $jkdb;

		// Populate the database debug fields.
		if ( is_object( $jkdb->dbh ) ) {
			// mysqli or PDO.
			$extension = get_class( $jkdb->dbh );
		} else {
			// Unknown sql extension.
			$extension = null;
		}

		$server = $jkdb->get_var( 'SELECT VERSION()' );

		$client_version = $jkdb->dbh->client_info;
	 ...
	 * @return array
	 */
	private static function get_jk_filesystem(): array {
		$upload_dir                     = jk_upload_dir();
		$is_writable_abspath            = jk_is_writable( ABSPATH );
		$is_writable_jk_content_dir     = jk_is_writable( JK_CONTENT_DIR );
		$is_writable_upload_dir         = jk_is_writable( $upload_dir['basedir'] );
		$is_writable_jk_plugin_dir      = jk_is_writable( JK_PLUGIN_DIR );
		$is_writable_template_directory = jk_is_writable( get_theme_root( get_template() ) );
		$is_writable_fonts_dir          = jk_is_writable( jk_get_font_dir()['basedir'] );

		$fields = array(
			'wordpress'  => array(
				'label' => __( 'The main JKPress directory' ),
				'value' => ( $is_writable_abspath ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_abspath ? 'writable' : 'not writable' ),
			),
			'jk-content' => array(
				'label' => __( 'The jk-content directory' ),
				'value' => ( $is_writable_jk_content_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_jk_content_dir ? 'writable' : 'not writable' ),
			),
			'uploads'    => array(
			...
			'plugins'    => array(
				...
				'value' => ( $is_writable_jk_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_jk_plugin_dir ? 'writable' : 'not writable' ),
			),
			'themes'     => array(
		...
		if ( defined( 'JKMU_PLUGIN_DIR' ) && is_dir( JKMU_PLUGIN_DIR ) ) {
			$is_writable_wpmu_plugin_dir = jk_is_writable( JKMU_PLUGIN_DIR );
		...
		return array(
			...
			'description' => __( 'Shows whether JKPress is able to write to the directories it needs access to.' ),
			'fields'      => $fields,
		);
	}
	...
	public static function get_mysql_var( $mysql_var ) {
		...
		$result = $jkdb->get_row(
			$jkdb->prepare( 'SHOW VARIABLES LIKE %s', $mysql_var ),
			ARRAY_A
		);
	 ...
	 * @since 5.2.0
	 *
	 * @param array  $info_array Information gathered from the `JK_Debug_Data::debug_data()` function.
	...
	public static function get_database_size() {
		global $jkdb;
		$size = 0;
		$rows = $jkdb->get_results( 'SHOW TABLE STATUS', ARRAY_A );
	...
	/**
	 * Fetches the sizes of the JKPress directories: `wordpress` (ABSPATH), `plugins`, `themes`, and `uploads`.
	 * Intended to supplement the array returned by `JK_Debug_Data::debug_data()`.
	 *
	 * @since 5.2.0
	 *
	...
	public static function get_sizes() {
		...
		$paths = array(
			...
			'plugins_size'   => JK_PLUGIN_DIR,
			'uploads_size'   => $upload_dir['basedir'],
			'fonts_size'     => jk_get_font_dir()['basedir'],
		);
		...
		foreach ( $paths as $name => $path ) {
			...
			if ( microtime( true ) - JK_START_TIMESTAMP < $max_execution_time ) {
