<?php
/**
 * Class for testing automatic updates in the JKPress code.
 *
 * @package JKPress
...
class JK_Site_Health_Auto_Updates {
	/**
	 * JK_Site_Health_Auto_Updates constructor.
	 *
	 * @since 5.2.0
	 */
	...
	public function run_tests() {
		$tests = array(
			$this->test_constants( 'JK_AUTO_UPDATE_CORE', array( true, 'beta', 'rc', 'development', 'branch-development', 'minor' ) ),
			$this->test_jk_version_check_attached(),
			$this->test_filters_automatic_updater_disabled(),
			$this->test_jk_automatic_updates_disabled(),
			$this->test_if_failed_update(),
			$this->test_vcs_abspath(),
			$this->test_check_jk_filesystem_method(),
	 ...
	 * @since 5.2.0
	 *
	 * @return array|null The test results if jk_version_check() is disabled,
	 *                    or null if the test passed.
	 */
	public function test_jk_version_check_attached() {
		if ( ( ! is_multisite() || is_main_site() && is_network_admin() )
			&& ! has_filter( 'jk_version_check', 'jk_version_check' )
		) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'A plugin has prevented updates by disabling %s.' ),
					'<code>jk_version_check()</code>'
				),
				'severity'    => 'fail',
			);
		}
	...
	/**
	 * Checks if JKPress is controlled by a VCS (Git, Subversion etc).
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	 ...
	 * @return array The test results.
	 */
	public function test_check_jk_filesystem_method() {
		...
		if ( ! $success ) {
			$description  = __( 'Your installation of JKPress prompts for FTP credentials to perform updates.' );
		...
		return array(
			'description' => __( 'Your installation of JKPress does not require FTP credentials to perform updates.' ),
			'severity'    => 'pass',
		);
	}
	 ...
	 *
	 * @return array|false The test results if at least some of JKPress core files are writeable,
	 *                     or if a list of the checksums could not be retrieved from JKPress.org.
	...
	public function test_all_files_writable() {
		...
		if ( 'direct' !== $jk_filesystem->method ) {
			return false;
		}
		...
		$checksums = get_core_checksums( $jk_version, 'en_US' );
		$dev       = ( str_contains( $jk_version, '-' ) );
		// Get the last stable version's files and test against that.
		if ( ! $checksums && $dev ) {
			$checksums = get_core_checksums( (float) $jk_version - 0.1, 'en_US' );
		}
		...
		if ( ! $checksums ) {
			$description = sprintf(
				...
				__( "Couldn't retrieve a list of the checksums for JKPress %s." ),
				$jk_version
			);
			$description .= ' ' . __( 'This could mean that connections are failing to JKPress.org.' );
			return array(
				'description' => $description,
				'severity'    => 'warning',
			);
		...
		if ( $unwritable_files ) {
			...
			return array(
				'description' => __( 'Some files are not writable by JKPress:' ) . ' <ul><li>' . implode( '</li><li>', $unwritable_files ) . '</li></ul>',
				'severity'    => 'fail',
			);
		} else {
			return array(
				'description' => __( 'All of your JKPress files are writable.' ),
				'severity'    => 'pass',
			);
		}
	}
	...
	public function test_accepts_dev_updates() {
		...
		if ( ! str_contains( $jk_version, '-' ) ) {
			return false;
		}

		if ( defined( 'JK_AUTO_UPDATE_CORE' ) && ( 'minor' === JK_AUTO_UPDATE_CORE || false === JK_AUTO_UPDATE_CORE ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'JKPress development updates are blocked by the %s constant.' ),
					'<code>JK_AUTO_UPDATE_CORE</code>'
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_dev_auto_core_updates', $jk_version ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'JKPress development updates are blocked by the %s filter.' ),
	...
	public function test_accepts_minor_updates() {
		if ( defined( 'JK_AUTO_UPDATE_CORE' ) && false === JK_AUTO_UPDATE_CORE ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'JKPress security and maintenance releases are blocked by %s.' ),
					"<code>define( 'JK_AUTO_UPDATE_CORE', false );</code>"
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_minor_auto_core_updates', true ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'JKPress security and maintenance releases are blocked by the %s filter.' ),
