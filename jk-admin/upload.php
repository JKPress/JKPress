...
if ( ! current_user_can( 'upload_files' ) ) {
	jk_die( __( 'Sorry, you are not allowed to upload files.' ) );
}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message .= sprintf(
		' <a href="%1$s">%2$s</a>',
		esc_url( jk_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ),
		__( 'Undo' )
	);
...
if ( 'grid' === $mode ) {
	jk_enqueue_media();
	jk_enqueue_script( 'media-grid' );
	jk_enqueue_script( 'media' );

	// Remove the error parameter added by deprecation of jk-admin/media.php.
	add_filter(
	...
	// Let JS handle this.
	unset( $q['s'] );
	$vars   = jk_edit_attachments_query_vars( $q );
	...
	?>
	<div class="wrap" id="jk-media-grid" data-search="<?php _admin_search_query(); ?>">
...
$jk_list_table = _get_list_table( 'JK_Media_List_Table' );
...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction ) {
		$post_ids = $jkdb->get_col( "SELECT ID FROM $jkdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			jk_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			jk_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( $post_ids as $post_id_del ) {
				...
				if ( ! jk_delete_attachment( $post_id_del ) ) {
		...
		default:
			...
			$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $doaction, $post_ids ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	}
