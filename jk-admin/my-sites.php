...
if ( ! current_user_can( 'read' ) ) {
	jk_die( __( 'Sorry, you are not allowed to access this page.' ) );
}
...
if ( 'updateblogsettings' === $action && isset( $_POST['primary_blog'] ) ) {
	...
	if ( $blog && isset( $blog->domain ) ) {
		...
		jk_die( __( 'The primary site you chose does not exist.' ) );
	}
}
...
if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ), true ) ) {
	...
	$sign_up_url = apply_filters( 'jk_signup_location', network_site_url( 'jk-signup.php' ) );
...
<form id="myblogs" method="post">
	...
	foreach ( $blogs as $user_blog ) {
		...
		/** This filter is documented in jk-admin/my-sites.php */
	...
	if ( count( $blogs ) > 1 || has_action( 'myblogs_allblogs_options' ) || has_filter( 'myblogs_options' ) ) {
		...
		<?php
		jk_nonce_field( 'update-my-sites' );
		submit_button();
	}
	?>
	</form>
