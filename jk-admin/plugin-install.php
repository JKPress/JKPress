...
if ( ! current_user_can( 'install_plugins' ) ) {
	jk_die( __( 'Sorry, you are not allowed to install plugins on this site.' ) );
}

if ( is_multisite() && ! is_network_admin() ) {
	jk_redirect( network_admin_url( 'plugin-install.php' ) );
	exit;
}

$jk_list_table = _get_list_table( 'JK_Plugin_Install_List_Table' );
$pagenum       = $jk_list_table->get_pagenum();

if ( ! empty( $_REQUEST['_jk_http_referer'] ) ) {
	$location = remove_query_arg( '_jk_http_referer', jk_unslash( $_SERVER['REQUEST_URI'] ) );
 ...
 */
if ( 'upload' !== $tab ) {
	/** This action is documented in jk-admin/plugin-install.php */
	...
	array(
		...
		'content' =>
				'<p>' . sprintf(
					...
					__( 'Plugins hook into JKPress to extend its functionality with custom features. Plugins are developed independently from the core JKPress application by thousands of developers all over the world. All plugins in the official <a href="%s">JKPress Plugin Directory</a> are compatible with the license JKPress uses.' ),
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'If you know what you are looking for, Search is your best bet. The Search screen has options to search the JKPress Plugin Directory for a particular Term, Author, or Tag. You can also search the directory by selecting popular tags. Tags in larger type mean more plugins have been labeled with that tag.' ) . '</p>' .
				...
				'<p>' . __( 'You can also browse a user&#8217;s favorite plugins, by using the Favorites link above the plugins list and entering their JKPress.org username.' ) . '</p>' .
...
JK_Plugin_Dependencies::display_admin_notice_for_unmet_dependencies();
JK_Plugin_Dependencies::display_admin_notice_for_circular_dependencies();
?>
...
/**
 * JKPress Administration Template Footer.
 */
