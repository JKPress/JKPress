...
if ( 'attachment' === $typenow ) {
	if ( jk_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$jk_list_table = _get_list_table( 'JK_Posts_List_Table' );
$pagenum       = $jk_list_table->get_pagenum();

// Back-compat for viewing comments of an entry.
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		jk_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'locked', 'ids' ), jk_get_referer() );
	if ( ! $sendback ) {
	...
	if ( 'delete_all' === $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			...
			$post_ids = $jkdb->get_col( $jkdb->prepare( "SELECT ID FROM $jkdb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		...
		case 'untrash':
			...
			if ( isset( $_GET['doaction'] ) && ( 'undo' === $_GET['doaction'] ) ) {
				add_filter( 'jk_untrash_post_status', 'jk_untrash_post_set_previous_status', 10, 3 );
			}
			...
			remove_filter( 'jk_untrash_post_status', 'jk_untrash_post_set_previous_status', 10 );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( 'attachment' === $post_del->post_type ) {
					if ( ! jk_delete_attachment( $post_id ) ) {
						jk_die( __( 'Error in deleting the attachment.' ) );
					}
				} else {
					if ( ! jk_delete_post( $post_id ) ) {
		...
		default:
			 ...
			 */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $doaction, $post_ids ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
			break;
	}
	...
	jk_redirect( $sendback );
	exit;
} elseif ( ! empty( $_REQUEST['_jk_http_referer'] ) ) {
...
jk_enqueue_script( 'inline-edit-post' );
jk_enqueue_script( 'heartbeat' );

if ( 'jk_block' === $post_type ) {
	jk_enqueue_script( 'jk-list-reusable-blocks' );
	jk_enqueue_style( 'jk-list-reusable-blocks' );
}
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( 'trashed' === $message && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_url( jk_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", 'bulk-posts' ) ),
			__( 'Undo' )
		);
	}
...
<?php $jk_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
