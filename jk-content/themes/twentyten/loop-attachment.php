...
if ( have_posts() ) {
	while ( have_posts() ) :
		...
		if ( jk_attachment_is_image() ) :
			...
			?>
						<p class="attachment"><a href="<?php echo esc_url( $next_attachment_url ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment">
							...
							echo jk_get_attachment_image( $post->ID, array( $attachment_width, $attachment_height ) );
							?>
							</a></p>
		...
		<?php else : ?>
						<a href="<?php echo esc_url( jk_get_attachment_url() ); ?>" title="<?php the_title_attribute(); ?>" rel="attachment"><?php echo esc_html( jk_basename( get_permalink() ) ); ?></a>
	<?php endif; ?>
