 ...
 * hooks in JKPress to change core functionality.
 *
...
/**
 * Twenty Thirteen only works in JKPress 3.6 or later.
 */
if ( version_compare( $GLOBALS['jk_version'], '3.6-alpha', '<' ) ) {
 ...
 *
 * Sets up theme defaults and registers the various JKPress features that
...
function twentythirteen_setup() {
	 ...
	 *
	 * Translations can be filed at JKPress.org. See: https://translate.wordpress.org/projects/jk-themes/twentythirteen
	 * If you're building a theme based on Twenty Thirteen, use a find and
	 * replace to change 'twentythirteen' to the name of your theme in all
	 * template files.
	 *
	 * Manual loading of text domain is not required after the introduction of
	 * just in time translation loading in JKPress version 4.6.
	 *
	 * @ticket 58318
	 */
	if ( version_compare( $GLOBALS['jk_version'], '4.6', '<' ) ) {
	...
	// This theme uses jk_nav_menu() in one location.
...
function twentythirteen_scripts_styles() {
	...
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		jk_enqueue_script( 'jquery-masonry' );
	}
	...
	jk_enqueue_style( 'twentythirteen-fonts', twentythirteen_fonts_url(), array(), $font_version );

	// Add Genericons font, used in the main stylesheet.
	jk_enqueue_style( 'genericons', get_template_directory_uri() . '/genericons/genericons.css', array(), '3.0.3' );

	// Loads our main stylesheet.
	jk_enqueue_style( 'twentythirteen-style', get_stylesheet_uri(), array(), '20241112' );

	// Theme block stylesheet.
	jk_enqueue_style( 'twentythirteen-block-style', get_template_directory_uri() . '/css/blocks.css', array( 'twentythirteen-style' ), '20240520' );

	// Registers the Internet Explorer specific stylesheet.
	jk_register_style( 'twentythirteen-ie', get_template_directory_uri() . '/css/ie.css', array( 'twentythirteen-style' ), '20150214' );
	jk_style_add_data( 'twentythirteen-ie', 'conditional', 'lt IE 9' );
}
add_action( 'jk_enqueue_scripts', 'twentythirteen_scripts_styles' );
...
function twentythirteen_resource_hints( $urls, $relation_type ) {
	if ( jk_style_is( 'twentythirteen-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		if ( version_compare( $GLOBALS['jk_version'], '4.7-alpha', '>=' ) ) {
			$urls[] = array(
...
// add_filter( 'jk_resource_hints', 'twentythirteen_resource_hints', 10, 2 );
...
function twentythirteen_block_editor_styles() {
	// Block styles.
	jk_enqueue_style( 'twentythirteen-block-editor-style', get_template_directory_uri() . '/css/editor-blocks.css', array(), '20240716' );
	// Add custom fonts.
	$font_version = ( 0 === strpos( (string) twentythirteen_fonts_url(), get_template_directory_uri() . '/' ) ) ? '20230328' : null;
	jk_enqueue_style( 'twentythirteen-fonts', twentythirteen_fonts_url(), array(), $font_version );
}
 ...
 * @since Twenty Thirteen 1.0
 *
 * @global int $paged JKPress archive pagination page count.
 * @global int $page  JKPress paginated post page count.
 *
 * @param string $title Default title text for current view.
 * @param string $sep   Optional separator.
 * @return string The filtered title.
 */
function twentythirteen_jk_title( $title, $sep ) {
	global $paged, $page;
...
add_filter( 'jk_title', 'twentythirteen_jk_title', 10, 2 );
...
if ( ! function_exists( 'jk_get_list_item_separator' ) ) :
	/**
	 * Retrieves the list item separator based on the locale.
	 *
	 * Added for backward compatibility to support pre-6.0.0 JKPress versions.
	 *
	 * @since 6.0.0
	 */
...
if ( ! function_exists( 'twentythirteen_entry_meta' ) ) :
	...
	function twentythirteen_entry_meta() {
		...
		$categories_list = get_the_category_list( jk_get_list_item_separator() );
		if ( $categories_list ) {
			echo '<span class="categories-links">' . $categories_list . '</span>';
		}

		$tags_list = get_the_tag_list( '', jk_get_list_item_separator() );
		if ( $tags_list && ! is_jk_error( $tags_list ) ) {
...
if ( ! function_exists( 'twentythirteen_the_attached_image' ) ) :
	...
	function twentythirteen_the_attached_image() {
		...
		$next_attachment_url = jk_get_attachment_url();
		$post                = get_post();
		...
		printf(
			...
			jk_get_attachment_image( $post->ID, $attachment_size )
		);
	}
endif;
...
/**
 * Extend the default JKPress body classes.
 *
 ...
 * @since Twenty Thirteen 1.0
 *
 * @param JK_Customize_Manager $jk_customize Customizer object.
 */
function twentythirteen_customize_register( $jk_customize ) {
...
function twentythirteen_customize_preview_js() {
	jk_enqueue_script( 'twentythirteen-customizer', get_template_directory_uri() . '/js/theme-customizer.js', array( 'customize-preview' ), '20200516', array( 'in_footer' => true ) );
}
...
function twentythirteen_author_bio_template( $template ) {
	if ( is_author() ) {
		...
		if ( $author instanceof JK_User && 'bio' === $author->user_nicename ) {
...
if ( ! function_exists( 'jk_body_open' ) ) :
	/**
	 * Fire the jk_body_open action.
	 *
	 * Added for backward compatibility to support pre-5.2.0 JKPress versions.
	 *
