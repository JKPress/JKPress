...
if ( is_singular() && get_option( 'thread_comments' ) ) {
	...
	/*
	 * Always have jk_head() just before the closing </head>
	...
	<header id="branding">
			...
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
				...
				if ( is_singular() && has_post_thumbnail( $post->ID ) ) {
					$image = jk_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), array( $header_image_width, $header_image_width ) );
				}
			...
			<nav id="access">
				...
				<?php
				/*
				 * Our navigation menu. If one isn't filled out, jk_nav_menu() falls back to jk_page_menu().
				 * The menu assigned to the primary location is the one used.
				 * If one isn't assigned, the menu with the lowest ID is used.
				 */
				jk_nav_menu( array( 'theme_location' => 'primary' ) );
				?>
			</nav><!-- #access -->
	</header><!-- #branding -->
