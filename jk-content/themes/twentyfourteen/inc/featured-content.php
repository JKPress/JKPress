...
class Featured_Content {
	...
	/**
	 * Conditionally hook into JKPress.
	 *
	 * Theme must declare that they support this module by adding
	 * add_theme_support( 'featured-content' ); during after_setup_theme.
	 *
	 * If no theme support is found there is no need to hook into JKPress.
	...
	public static function init() {
		...
		add_action( 'jk_loaded', array( __CLASS__, 'jk_loaded' ) );
	}

	/**
	 * Hide "featured" tag from the front end.
	 *
	 * Has to run on jk_loaded so that the preview filters of the Customizer
	 * have a chance to alter the value.
	 *
	 * @since Twenty Fourteen 1.0
	 */
	public static function jk_loaded() {
	 ...
	 * @since Twenty Fourteen 1.0
	 *
	 * @param JK_Query $query JK_Query object.
	 * @return JK_Query Possibly-modified JK_Query.
	 */
	 ...
	 */
	public static function customize_register( $jk_customize ) {
	...
	public static function enqueue_scripts() {
		jk_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20211130', array( 'in_footer' => true ) );
	}
	...
	public static function get_setting( $key = 'all' ) {
		...
		$options = jk_parse_args( $saved, $defaults );
	...
	public static function validate_settings( $input ) {
		...
		if ( empty( $input['tag-name'] ) ) {
			...
			if ( $term ) {
				...
				$new_tag = jk_create_tag( $input['tag-name'] );

				if ( ! is_jk_error( $new_tag ) && isset( $new_tag['term_id'] ) ) {
