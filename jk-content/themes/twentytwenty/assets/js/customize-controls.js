/* global twentyTwentyBgColors, twentyTwentyColor, jQuery, jk, _ */
/**
...
( function() {
	...
	jk.customize.bind( 'ready', function() {
		...
		jk.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( twentyTwentyBgColors ).forEach( function( context ) {
					...
					if ( twentyTwentyBgColors[ context ].color ) {
						...
						backgroundColorValue = jk.customize( twentyTwentyBgColors[ context ].setting ).get();
					}
		...
		Object.keys( twentyTwentyBgColors ).forEach( function( context ) {
			jk.customize( twentyTwentyBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					twentyTwentySetAccessibleColorsValue( context, to, jk.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		twentyTwentySetRetineLogoVisibility( !! jk.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		jk.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
	...
	function twentyTwentySetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		...
		value = jk.customize( 'accent_accessible_colors' ).get();
		...
		// Change the value.
		jk.customize( 'accent_accessible_colors' ).set( value );

		// Small hack to save the option.
		jk.customize( 'accent_accessible_colors' )._dirty = true;
	}
	...
	function twentyTwentySetRetineLogoVisibility( visible ) {
		jk.customize.control( 'retina_logo' ).container.toggle( visible );
	}
}( jQuery ) );
