...
function twentytwenty_site_logo( $args = array(), $display = true ) {
	...
	echo $html; // phpcs:ignore JKPress.Security.EscapeOutput.OutputNotEscaped
}
...
function twentytwenty_the_post_meta( $post_id = null, $location = 'single-top' ) {
	...
	echo twentytwenty_get_post_meta( $post_id, $location ); // phpcs:ignore JKPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in twentytwenty_get_post_meta().
}
...
/**
 * Filters classes of jk_list_pages items to match menu items.
 *
 * Filter the class applied to jk_list_pages() items with children to match the menu class, to simplify.
 ...
 */
function twentytwenty_filter_jk_list_pages_item_classes( $css_class, $page, $depth, $args ) {

	// Only apply to jk_list_pages() calls with match_menu_classes set to true.
...
add_filter( 'page_css_class', 'twentytwenty_filter_jk_list_pages_item_classes', 10, 4 );
 ...
 * @return stdClass An object of jk_nav_menu() arguments.
 */
...
add_action( 'jk_head', 'twentytwenty_no_js_class' );
