...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			$time_string // phpcs:ignore JKPress.Security.EscapeOutput
		);
		echo '</span>';
	}
}
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( 'aside' === $post_format || 'status' === $post_format ) {
				echo '<p><a href="' . esc_url( get_permalink() ) . '">' . twenty_twenty_one_continue_reading_text() . '</a></p>'; // phpcs:ignore JKPress.Security.EscapeOutput
			}
			...
			if ( has_category() || has_tag() ) {
				...
				if ( $categories_list ) {
					printf(
						...
						$categories_list // phpcs:ignore JKPress.Security.EscapeOutput
					);
				}
				...
				if ( $tags_list && ! is_jk_error( $tags_list ) ) {
					printf(
						...
						$tags_list // phpcs:ignore JKPress.Security.EscapeOutput
					);
				}
				echo '</div>';
			}
	...
	function twenty_twenty_one_post_thumbnail() {
		...
		<?php if ( is_singular() ) : ?>
			...
			<figure class="post-thumbnail">
				...
				?>
				<?php if ( jk_get_attachment_caption( get_post_thumbnail_id() ) ) : ?>
					<figcaption class="jk-caption-text"><?php echo jk_kses_post( jk_get_attachment_caption( get_post_thumbnail_id() ) ); ?></figcaption>
				<?php endif; ?>
