...
class Twenty_Twenty_One_Dark_Mode {
	...
	public function __construct() {
		...
		add_action( 'jk_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		...
		add_action( 'jk_footer', array( $this, 'the_switch' ) );
	...
	public function enqueue_scripts() {
		...
		jk_enqueue_style( 'tt1-dark-mode', $url, array( 'twenty-twenty-one-style' ), jk_get_theme()->get( 'Version' ) ); // @phpstan-ignore-line. Version is always a string.
	}
	 ...
	 * @return void
	 */
	public function customizer_controls( $jk_customize ) {

		$colors_section = $jk_customize->get_section( 'colors' );
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'twentytwentyone' );
		}

		// Custom notice control.
		require_once get_theme_file_path( 'classes/class-twenty-twenty-one-customize-notice-control.php' ); // phpcs:ignore JKThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	...
	public function the_html( $attrs = array() ) {
		...
		<style>
			...
			<?php if ( is_admin() || jk_is_json_request() ) : ?>
	...
	public function the_script() {
		echo '<script>';
		include get_template_directory() . '/assets/js/dark-mode-toggler.js'; // phpcs:ignore JKThemeReview.CoreFunctionality.FileInclude
		echo '</script>';
	}
	...
	public function add_privacy_policy_content() {
		if ( ! function_exists( 'jk_add_privacy_policy_content' ) ) {
			return;
		}
		...
		jk_add_privacy_policy_content( __( 'Twenty Twenty-One', 'twentytwentyone' ), jk_kses_post( wpautop( $content, false ) ) );
	}
}
