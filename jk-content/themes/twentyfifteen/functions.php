...
/**
 * Twenty Fifteen only works in JKPress 4.1 or later.
 */
if ( version_compare( $GLOBALS['jk_version'], '4.1-alpha', '<' ) ) {
...
if ( ! function_exists( 'twentyfifteen_setup' ) ) :
	...
	function twentyfifteen_setup() {
		 ...
		 * Translations can be filed at JKPress.org. See: https://translate.wordpress.org/projects/jk-themes/twentyfifteen
		...
		// Setup the JKPress core custom background feature.
...
add_action( 'jk_head', 'twentyfifteen_javascript_detection', 0 );
...
function twentyfifteen_scripts() {
	...
	jk_enqueue_style( 'twentyfifteen-fonts', twentyfifteen_fonts_url(), array(), $font_version );

	// Add Genericons, used in the main stylesheet.
	jk_enqueue_style( 'genericons', get_template_directory_uri() . '/genericons/genericons.css', array(), '20201026' );

	// Load our main stylesheet.
	jk_enqueue_style( 'twentyfifteen-style', get_stylesheet_uri(), array(), '20241112' );

	// Theme block stylesheet.
	jk_enqueue_style( 'twentyfifteen-block-style', get_template_directory_uri() . '/css/blocks.css', array( 'twentyfifteen-style' ), '20240715' );

	// Register the Internet Explorer specific stylesheet.
	jk_register_style( 'twentyfifteen-ie', get_template_directory_uri() . '/css/ie.css', array( 'twentyfifteen-style' ), '20220908' );
	jk_style_add_data( 'twentyfifteen-ie', 'conditional', 'lt IE 9' );

	// Register the Internet Explorer 7 specific stylesheet.
	jk_register_style( 'twentyfifteen-ie7', get_template_directory_uri() . '/css/ie7.css', array( 'twentyfifteen-style' ), '20141210' );
	jk_style_add_data( 'twentyfifteen-ie7', 'conditional', 'lt IE 8' );

	// Skip-link fix is no longer enqueued by default.
	jk_register_script( 'twentyfifteen-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20230526', array( 'in_footer' => true ) );
	...
	if ( is_singular() && jk_attachment_is_image() ) {
		jk_enqueue_script( 'twentyfifteen-keyboard-image-navigation', get_template_directory_uri() . '/js/keyboard-image-navigation.js', array( 'jquery' ), '20141210' );
	}
...
add_action( 'jk_enqueue_scripts', 'twentyfifteen_scripts' );
...
function twentyfifteen_block_editor_styles() {
	// Block styles.
	jk_enqueue_style( 'twentyfifteen-block-editor-style', get_template_directory_uri() . '/css/editor-blocks.css', array(), '20240720' );
	// Add custom fonts.
	$font_version = ( 0 === strpos( (string) twentyfifteen_fonts_url(), get_template_directory_uri() . '/' ) ) ? '20230328' : null;
	jk_enqueue_style( 'twentyfifteen-fonts', twentyfifteen_fonts_url(), array(), $font_version );
}
...
function twentyfifteen_resource_hints( $urls, $relation_type ) {
	if ( jk_style_is( 'twentyfifteen-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
...
// add_filter( 'jk_resource_hints', 'twentyfifteen_resource_hints', 10, 2 );
...
function twentyfifteen_post_nav_background() {
	...
	if ( $previous && has_post_thumbnail( $previous->ID ) ) {
		$prevthumb = jk_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), 'post-thumbnail' );
		$css      .= '
	...
	if ( $next && has_post_thumbnail( $next->ID ) ) {
		$nextthumb = jk_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), 'post-thumbnail' );
		$css      .= '
	...
	jk_add_inline_style( 'twentyfifteen-style', $css );
}
add_action( 'jk_enqueue_scripts', 'twentyfifteen_post_nav_background' );
 ...
 * @param JK_Post  $item        Menu item data object.
 * @param int      $depth       Depth of the menu. Used for padding.
 * @param stdClass $args        An object of jk_nav_menu() arguments.
