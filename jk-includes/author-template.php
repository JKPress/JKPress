 ...
 *
 * These functions must be used within the JKPress Loop.
 *
 ...
 *
 * @global JK_User $authordata The current author's data.
 *
...
function get_the_author_link() {
	if ( get_the_author_meta( 'url' ) ) {
		 ...
		 * @param JK_User $authordata Author user data.
		 */
...
function get_author_posts_url( $author_id, $author_nicename = '' ) {
	...
	$link      = $jk_rewrite->get_author_permastruct();
 ...
 *
 * @link https://developer.wordpress.org/reference/functions/jk_list_authors/
 *
 * @since 1.2.0
 *
...
function jk_list_authors( $args = '' ) {
	 ...
	 * @param array $parsed_args The arguments passed to jk_list_authors() combined with the defaults.
	 */
	$query_args = apply_filters( 'jk_list_authors_args', $query_args, $parsed_args );
	 ...
	 * @param array       $parsed_args The arguments passed to jk_list_authors() combined with the defaults.
	 */
	$post_counts = apply_filters( 'pre_jk_list_authors_post_counts_query', false, $parsed_args );

	if ( ! is_array( $post_counts ) ) {
		$post_counts       = array();
		$post_counts_query = $jkdb->get_results(
...
function is_multi_author() {
	...
	if ( false === $is_multi_author ) {
		$rows            = (array) $jkdb->get_col( "SELECT DISTINCT post_author FROM $jkdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2" );
 ...
 * @access private
 */
function __clear_multi_author_cache() { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
