<?php
/**
 * Locale API: JK_Textdomain_Registry class.
 *
...
class JK_Textdomain_Registry {
	...
	public function get_language_files_from_path( $path ) {
		...
		$cache_key = md5( $path );
		$files     = jk_cache_get( $cache_key, 'translation_files' );
		...
		if ( false === $files ) {
			...
			jk_cache_set( $cache_key, $files, 'translation_files', HOUR_IN_SECONDS );
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param JK_Upgrader $upgrader   Unused. JK_Upgrader instance. In other contexts this might be a
	...
	public function invalidate_mo_files_cache( $upgrader, $hook_extra ) {
		...
		$translation_types = array_unique( jk_list_pluck( $hook_extra['translations'], 'type' ) );

		foreach ( $translation_types as $type ) {
			switch ( $type ) {
				case 'plugin':
					jk_cache_delete( md5( JK_LANG_DIR . '/plugins/' ), 'translation_files' );
					break;
				case 'theme':
					jk_cache_delete( md5( JK_LANG_DIR . '/themes/' ), 'translation_files' );
					break;
				default:
					jk_cache_delete( md5( JK_LANG_DIR . '/' ), 'translation_files' );
					break;
			}
		}
	}
