 ...
 *
 * JK_Date_Query is a helper that allows primary query classes, such as JK_Query, to filter
 * their results by date columns, by generating `WHERE` subclauses to be attached to the
 * primary SQL query string.
 *
 * Attempting to filter by an invalid date value (eg month=13) will generate SQL that will
 * return no results. In these cases, a _doing_it_wrong() error notice is also thrown.
 * See JK_Date_Query::validate_date_values().
 *
...
class JK_Date_Query {
	 ...
	 *
	 * See JK_Date_Query::__construct() for information on date query arguments.
	 *
	 * @since 3.7.0
	 * @var array
	 */
	 ...
	 *                                the `$default_column` parameter. See JK_Date_Query::validate_column() and
	 ...
	 *                                               See JK_Date_Query::validate_column() and
	 ...
	 *     }
	 * }
	 * @param string $default_column Optional. Default column to query against. See JK_Date_Query::validate_column()
	...
	public function validate_date_values( $date_query = array() ) {
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! jk_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! jk_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
				$day_month_year_error_msg = sprintf(
	 ...
	 * allowed and known tables, and then, if found, have a table prefix (such as 'jk_posts.')
	 * prepended. Prefixed column names (such as 'jk_posts.post_date') bypass this allowed
	...
	public function get_sql() {
		 ...
		 * @param JK_Date_Query $query The JK_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public JK_Date_Query::get_sql(), this method is abstracted
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) ) {
			$where_parts[] = $jkdb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );
		}
		if ( ! empty( $query['before'] ) ) {
			$where_parts[] = $jkdb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		}
			...
			foreach ( $query_parts as $query_part ) {
				if ( isset( $query[ $query_part ] ) ) {
					...
					if ( $value ) {
						switch ( $sql_part ) {
							case '_jk_mysql_week':
								$where_parts[] = _jk_mysql_week( $column ) . " $compare $value";
								break;
							case 'WEEKDAY':
	...
	public function build_mysql_datetime( $datetime, $default_to_max = false ) {
		if ( ! is_array( $datetime ) ) {
			...
			if ( ! is_array( $datetime ) ) {
				...
				$dt = date_create( $datetime, $jk_timezone );

				if ( false === $dt ) {
					return gmdate( 'Y-m-d H:i:s', false );
				}

				return $dt->setTimezone( $jk_timezone )->format( 'Y-m-d H:i:s' );
			}
		}
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $jkdb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}
