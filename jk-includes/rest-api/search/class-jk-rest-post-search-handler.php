<?php
/**
 * REST API: JK_REST_Post_Search_Handler class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class JK_REST_Post_Search_Handler extends JK_REST_Search_Handler {
	...
	public function search_items( JK_REST_Request $request ) {
		...
		$post_types = $request[ JK_REST_Search_Controller::PROP_SUBTYPE ];
		if ( in_array( JK_REST_Search_Controller::TYPE_ANY, $post_types, true ) ) {
		...
		$found_ids = jk_list_pluck( $posts, 'ID' );
	...
	public function prepare_item( $id, array $fields ) {
		...
		if ( in_array( JK_REST_Search_Controller::PROP_ID, $fields, true ) ) {
			$data[ JK_REST_Search_Controller::PROP_ID ] = (int) $post->ID;
		}
		...
		if ( in_array( JK_REST_Search_Controller::PROP_TITLE, $fields, true ) ) {
			if ( post_type_supports( $post->post_type, 'title' ) ) {
				...
				$data[ JK_REST_Search_Controller::PROP_TITLE ] = get_the_title( $post->ID );
				remove_filter( 'protected_title_format', array( $this, 'protected_title_format' ) );
				remove_filter( 'private_title_format', array( $this, 'protected_title_format' ) );
			} else {
				$data[ JK_REST_Search_Controller::PROP_TITLE ] = '';
			}
		}

		if ( in_array( JK_REST_Search_Controller::PROP_URL, $fields, true ) ) {
			$data[ JK_REST_Search_Controller::PROP_URL ] = get_permalink( $post->ID );
		}

		if ( in_array( JK_REST_Search_Controller::PROP_TYPE, $fields, true ) ) {
			$data[ JK_REST_Search_Controller::PROP_TYPE ] = $this->type;
		}

		if ( in_array( JK_REST_Search_Controller::PROP_SUBTYPE, $fields, true ) ) {
			$data[ JK_REST_Search_Controller::PROP_SUBTYPE ] = $post->post_type;
		}
	...
	public function prepare_item_links( $id ) {
		...
		$links['about'] = array(
			'href' => rest_url( 'jk/v2/types/' . $post->post_type ),
		);
