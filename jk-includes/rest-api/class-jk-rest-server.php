...
/**
 * Core class used to implement the JKPress REST API server.
 *
 * @since 4.4.0
 */
#[AllowDynamicProperties]
class JK_REST_Server {
	 ...
	 * @since 4.4.0
	 *
	 * @return JK_Error|null|true JK_Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a JK_Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A JK_Error instance can be returned if an error occurs, and this should
		 ...
		 * @since 4.4.0
		 *
		 * @param JK_Error|null|true $errors JK_Error if authentication error, null if authentication
	 ...
	 *
	 * Note: This should only be used in JK_REST_Server::serve_request(), as it
	 * cannot handle JK_Error internally. All callbacks and other internal methods
	 * should instead return a JK_Error with the data set to an array that includes
	 * a 'status' key, with the value being the HTTP status to send.
	 *
	 * @since 4.4.0
	 *
	 * @param string $code    JK_Error-style code.
	...
	protected function json_error( $code, $message, $status = null ) {
		...
		return jk_json_encode( $error );
	}

	/**
	 * Gets the encoding options passed to {@see jk_json_encode}.
	 *
	 * @since 6.1.0
	 *
	 * @param \JK_REST_Request $request The current request object.
	 *
	 * @return int The JSON encode options.
	 */
	protected function get_json_encode_options( JK_REST_Request $request ) {
		$options = 0;
		 ...
		 * @param JK_REST_Request $request Current request object.
		 */
	 ...
	 *
	 * @global JK_User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var JK_User|null $current_user */
		global $current_user;

		if ( $current_user instanceof JK_User && ! $current_user->exists() ) {
			/*
		...
		$request = new JK_REST_Request( $_SERVER['REQUEST_METHOD'], $path );

		$request->set_query_params( jk_unslash( $_GET ) );
		$request->set_body_params( jk_unslash( $_POST ) );
		$request->set_file_params( $_FILES );
		$request->set_headers( $this->get_headers( jk_unslash( $_SERVER ) ) );
		...
		$expose_headers = array( 'X-JK-Total', 'X-JK-TotalPages', 'Link' );
		 ...
		 * @param JK_REST_Request $request        The request in context.
		 */
		...
		// Normalize to either JK_Error or JK_REST_Response...
		$result = rest_ensure_response( $result );

		// ...then convert JK_Error across.
		if ( is_jk_error( $result ) ) {
		 ...
		 *
		 * @since 4.4.0
		 * @since 6.3.2 Moved the block to catch the filter added on rest_cookie_check_errors() from jk-includes/rest-api.php.
		 *
		...
		if ( $send_no_cache_headers || ( true === $method_overridden && str_starts_with( $code, '4' ) ) ) {
			foreach ( jk_get_nocache_headers() as $header => $header_value ) {
		...
		if ( ! $served ) {
			...
			$result = jk_json_encode( $result, $this->get_json_encode_options( $request ) );

			$json_error_message = $this->get_json_last_error();

			if ( $json_error_message ) {
				$this->set_status( 500 );
				$json_error_obj = new JK_Error(
					'rest_encode_error',
					$json_error_message,
					array( 'status' => 500 )
				);
				...
				$result = jk_json_encode( $result->data, $this->get_json_encode_options( $request ) );
			}
	 ...
	 *
	 * @param JK_REST_Response $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_REST_Response $response Response to extract links from.
	...
	protected static function get_target_hints_for_link( $link ) {
		...
		$request = JK_REST_Request::from_url( $link['href'] );
		if ( ! $request ) {
			return null;
		}
		...
		if ( is_jk_error( $match ) ) {
			return null;
		}

		if ( is_jk_error( $request->has_valid_params() ) ) {
			return null;
		}

		if ( is_jk_error( $request->sanitize_params() ) ) {
			return null;
		}
		...
		foreach ( $headers as $name => $value ) {
			$name = JK_REST_Request::canonicalize_header_name( $name );
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = JK_REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
					...
					if ( empty( $request['per_page'] ) ) {
						...
						if ( ! is_jk_error( $matched ) && isset( $matched[1]['args']['per_page']['maximum'] ) ) {
	 ...
	 * @param JK_REST_Response $response Response object.
	 * @param bool|string[]    $embed    Whether to embed all links, a filtered list of link relations, or no links.
	 * @return JK_REST_Response New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param JK_REST_Response $response Original response data.
		 */
	...
	public function get_routes( $route_namespace = '' ) {
		...
		if ( $route_namespace ) {
			$endpoints = jk_list_filter( $endpoints, array( 'namespace' => $route_namespace ) );
		}
		...
		foreach ( $endpoints as $route => &$handlers ) {
			...
			foreach ( $handlers as $key => &$handler ) {
				...
				$handler = jk_parse_args( $handler, $defaults );
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_REST_Request $request Request to attempt dispatching.
	 * @return JK_REST_Response Response returned by the callback.
	 */
	public function dispatch( $request ) {
		...
		if ( ! is_jk_error( $error ) ) {
			...
			if ( is_jk_error( $check_required ) ) {
				$error = $check_required;
			} else {
				$check_sanitized = $request->sanitize_params();
				if ( is_jk_error( $check_sanitized ) ) {
	 ...
	 * @return array|JK_Error The route and request handler on success or a JK_Error instance if no handler was found.
	 */
	 ...
	 * @param JK_Error|null   $response The current error object if any.
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_REST_Response|JK_HTTP_Response|JK_Error|mixed $response Result to send to the client.
		 *                                                                   Usually a JK_REST_Response or JK_Error.
		...
		if ( ! is_jk_error( $response ) && ! empty( $handler['permission_callback'] ) ) {
			$permission = call_user_func( $handler['permission_callback'], $request );

			if ( is_jk_error( $permission ) ) {
				$response = $permission;
			} elseif ( false === $permission || null === $permission ) {
				$response = new JK_Error(
					'rest_forbidden',
	 ...
	 * }
	 * @return JK_REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new JK_REST_Response( $available );

		$fields = isset( $request['_fields'] ) ? $request['_fields'] : '';
		$fields = jk_parse_list( $fields );
		if ( empty( $fields ) ) {
			$fields[] = '_links';
		}
		 ...
		 *
		 * @param JK_REST_Response $response Response data.
		 * @param JK_REST_Request  $request  Request data.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param JK_REST_Response $response REST API response.
	 */
	protected function add_active_theme_link_to_index( JK_REST_Response $response ) {
		...
		if ( $should_add ) {
			$theme = jk_get_theme();
			$response->add_link( 'https://api.w.org/active-theme', rest_url( 'jk/v2/themes/' . $theme->get_stylesheet() ) );
		}
	}

	/**
	 * Exposes the site logo through the JKPress REST API.
	 *
	 ...
	 * @param JK_REST_Response $response REST API response.
	 */
	protected function add_site_logo_to_index( JK_REST_Response $response ) {
	...
	/**
	 * Exposes the site icon through the JKPress REST API.
	 *
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 ...
	 * @param JK_REST_Response $response REST API response.
	 */
	protected function add_site_icon_to_index( JK_REST_Response $response ) {
	...
	/**
	 * Exposes an image through the JKPress REST API.
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 * @since 5.9.0
	 ...
	 */
	protected function add_image_to_index( JK_REST_Response $response, $image_id, $type ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_REST_Request $request REST request instance.
	 * @return JK_REST_Response|JK_Error JK_REST_Response instance if the index was found,
	 *                                   JK_Error if the namespace isn't set.
	 */
	public function get_namespace_index( $request ) {
		 ...
		 *
		 * @param JK_REST_Response $response Response data.
		 * @param JK_REST_Request  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
	 ...
	 * @since 5.6.0
	 *
	 * @param JK_REST_Request $batch_request The batch request object.
	 * @return JK_REST_Response The generated response object.
	 */
	public function serve_batch_request_v1( JK_REST_Request $batch_request ) {
		$requests = array();

		foreach ( $batch_request['requests'] as $args ) {
			$parsed_url = jk_parse_url( $args['path'] );

			if ( false === $parsed_url ) {
				$requests[] = new JK_Error( 'parse_path_failed', __( 'Could not parse the path.' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new JK_REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );

			if ( ! empty( $parsed_url['query'] ) ) {
				$query_args = array();
				jk_parse_str( $parsed_url['query'], $query_args );
		...
		return new JK_REST_Response( array( 'responses' => $responses ), JK_Http::MULTI_STATUS );
	}
