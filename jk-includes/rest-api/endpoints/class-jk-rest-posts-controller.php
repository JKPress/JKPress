<?php
/**
 * REST API: JK_REST_Posts_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JK_REST_Posts_Controller extends JK_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function __construct( $post_type ) {
		...
		$this->namespace = ! empty( $obj->rest_namespace ) ? $obj->rest_namespace : 'jk/v2';

		$this->meta = new JK_REST_Post_Meta_Fields( $this->post_type );
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( JK_REST_Server::CREATABLE ),
				),
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => JK_REST_Server::EDITABLE,
					'callback'            => array( $this, 'update_item' ),
					'permission_callback' => array( $this, 'update_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( JK_REST_Server::EDITABLE ),
				),
				array(
					'methods'             => JK_REST_Server::DELETABLE,
	 ...
	 * @return true|JK_Error True if the request has read access, JK_Error object otherwise.
	 */
	 ...
	 * @param JK_Post $post     The post been password checked.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response|JK_Error Response object on success, or JK_Error object on failure.
	 */
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal JK_Query parameter
		 ...
		 */
		foreach ( $parameter_mappings as $api_param => $jk_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$args[ $jk_param ] = $request[ $api_param ];
			}
		}
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * JK_Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters JK_Query arguments when querying posts via the REST API.
		 *
		 ...
		 *
		 * @param array           $args    Array of arguments for JK_Query.
		 * @param JK_REST_Request $request The REST API request.
		 */
		$args       = apply_filters( "rest_{$this->post_type}_query", $args, $request );
		$query_args = $this->prepare_items_query( $args, $request );

		$posts_query  = new JK_Query();
		...
		$response->header( 'X-JK-Total', (int) $total_posts );
		$response->header( 'X-JK-TotalPages', (int) $max_pages );
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return bool|JK_Error True if the request has read access for the item, JK_Error object or false otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param JK_Post         $post    Post to check against.
	 * @param JK_REST_Request $request Request data to check.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to create items, JK_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $prepared_post->post_name )
			...
			/*
			 * `jk_unique_post_slug()` returns the same slug for 'draft' or 'pending' posts.
			 *
			 * To ensure that a unique slug is generated, pass the post data with the 'publish' status.
			 */
			$prepared_post->post_name = jk_unique_post_slug(
		...
		$post_id = jk_insert_post( jk_slash( (array) $prepared_post ), true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Post         $post     Inserted or updated post object.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to update the item, JK_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $post->post_name ) && in_array( $post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$post->post_name = jk_unique_post_slug(
				$post->post_name,
				$post->ID,
				'publish',
				$post->post_type,
		...
		// Convert the post object to an array, otherwise jk_update_post() will expect non-escaped input.
		$post_id = jk_update_post( jk_slash( (array) $post ), true, false );
		...
		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-posts-controller.php */
		...
		// Filter is fired in JK_REST_Attachments_Controller subclass.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to delete the item, JK_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param JK_Post $post           The Post object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			/*
			 * (Note that internally this falls through to `jk_delete_post()`
			 * if the Trash is disabled.)
			 */
			$result   = jk_trash_post( $id );
			$post     = get_post( $id );
			$response = $this->prepare_item_for_response( $post, $request );
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Post          $post     The deleted or trashed post.
		 * @param JK_REST_Response $response The response data.
		 * @param JK_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared JK_Query arguments. Default empty array.
	 * @param JK_REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$query_args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
		}
		...
		// Map to proper JK_Query orderby param.
	 ...
	 *
	 * @param JK_REST_Request $request Request object.
	 * @return stdClass|JK_Error Post object or JK_Error.
	 */
	 ...
	 * @param JK_REST_Request $request The request object.
	 * @param string          $param   The parameter name.
	 * @return true|JK_Error True if the status is valid, or JK_Error if not.
	 */
	public function check_status( $status, $request, $param ) {
		if ( $request['id'] ) {
			$post = $this->get_post( $request['id'] );

			if ( ! is_jk_error( $post ) && $post->post_status === $status ) {
				return true;
			}
		}
	 ...
	 * @param JK_Post_Type $post_type   Post type.
	 * @return string|JK_Error Post status or JK_Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|JK_Error Whether the post thumbnail was successfully deleted, otherwise JK_Error.
	 */
	 ...
	 * @param JK_REST_Request $request  Request.
	 * @return true|JK_Error True if template is still valid or if the same as existing value, or a JK_Error if template not supported.
	 */
	public function check_template( $template, $request ) {
		...
		// If this is a create request, get_post() will return null and jk theme will fallback to the passed post type.
		$allowed_templates = jk_get_theme()->get_page_templates( $post, $this->post_type );
	...
	public function handle_template( $template, $post_id, $validate = false ) {
		...
		if ( $validate && ! array_key_exists( $template, jk_get_theme()->get_page_templates( get_post( $post_id ) ) ) ) {
			$template = '';
		}

		update_post_meta( $post_id, '_jk_page_template', $template );
	}
	 ...
	 * @param JK_REST_Request $request The request object with post and terms data.
	 * @return null|JK_Error JK_Error on an error assigning any of the terms, otherwise null.
	 */
	protected function handle_terms( $post_id, $request ) {
		$taxonomies = jk_list_filter( get_object_taxonomies( $this->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$result = jk_set_object_terms( $post_id, $request[ $base ], $taxonomy->name );
	 ...
	 * @since 4.7.0
	 *
	 * @param JK_Post_Type|string $post_type Post type name or object.
	 ...
	 *
	 * @param JK_Post         $item    Post object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$data[ $base ] = $terms ? array_values( jk_list_pluck( $terms, 'term_id' ) ) : array();
			}
		}
	 ...
	 *
	 * By default, JKPress will show password protected or private posts with a title of
	...
	protected function prepare_links( $post ) {
		...
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( 'jk/v2/types/' . $this->post_type ),
			),
		);
		...
		if ( in_array( $post->post_type, array( 'post', 'page' ), true ) || post_type_supports( $post->post_type, 'revisions' ) ) {
			...
			$revisions_count = ! is_jk_error( $revisions ) ? $revisions['count'] : 0;
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal JK_Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set JK_Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|JK_Error A list of valid statuses, otherwise JK_Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = jk_parse_slug_list( $statuses );

		// The default status is different in JK_REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    JK_Query arguments.
	 * @param JK_REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, JK_REST_Request $request ) {
