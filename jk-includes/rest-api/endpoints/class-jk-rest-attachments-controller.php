<?php
/**
 * REST API: JK_REST_Attachments_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see JK_REST_Posts_Controller
 */
class JK_REST_Attachments_Controller extends JK_REST_Posts_Controller {
	 ...
	 * prepares for JK_Query.
	 *
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Array of prepared arguments. Default empty array.
	 * @param JK_REST_Request $request       Optional. Request to prepare items for.
	 ...
	 * @return true|JK_Error Boolean true if the attachment may be created, or a JK_Error if not.
	 */
	public function create_item_permissions_check( $request ) {
		$ret = parent::create_item_permissions_check( $request );

		if ( ! $ret || is_jk_error( $ret ) ) {
			return $ret;
		}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response|JK_Error Response object on success, JK_Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $attachment_id, '_jk_attachment_image_alt', sanitize_text_field( $request['alt_text'] ) );
		}

		if ( ! empty( $schema['properties']['featured_media'] ) && isset( $request['featured_media'] ) ) {
			$thumbnail_update = $this->handle_featured_media( $request['featured_media'], $attachment_id );

			if ( is_jk_error( $thumbnail_update ) ) {
		 ...
		 * @since 5.0.0
		 *
		 * @param JK_Post         $attachment Inserted or updated attachment object.
		 * @param JK_REST_Request $request    Request object.
		 * @param bool            $creating   True when creating an attachment, false when updating.
		 */
		do_action( 'rest_after_insert_attachment', $attachment, $request, true );

		jk_after_insert_post( $attachment, false, null );
		...
		// Include media and image functions to get access to jk_generate_attachment_metadata().
	...
	protected function insert_attachment( $request ) {
		...
		// Include image functions to get access to jk_read_image_metadata().
		require_once ABSPATH . 'jk-admin/includes/image.php';
		...
		if ( empty( $attachment->post_title ) ) {
			$attachment->post_title = preg_replace( '/\.[^.]+$/', '', jk_basename( $file ) );
		}

		// $post_parent is inherited from $attachment['post_parent'].
		$id = jk_insert_attachment( jk_slash( (array) $attachment ), $file, 0, true, false );
	...
	protected function handle_featured_media( $featured_media, $post_id ) {
		...
		// Similar check as in jk_insert_post().
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $data['id'], '_jk_attachment_image_alt', $request['alt_text'] );
		}
		...
		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-attachments-controller.php */
		do_action( 'rest_after_insert_attachment', $attachment, $request, false );

		jk_after_insert_post( $attachment, true, $attachment_before );
	...
	public function post_process_item( $request ) {
		switch ( $request['action'] ) {
			case 'create-image-subsizes':
				require_once ABSPATH . 'jk-admin/includes/image.php';
				jk_update_image_subsizes( $request['id'] );
				break;
		}
	...
	public function edit_media_item( $request ) {
		...
		if (
			! $image_meta ||
			! $image_file ||
			! jk_image_file_matches_image_meta( $request['src'], $image_meta, $attachment_id )
		) {
			return new JK_Error(
		...
		$image_editor = jk_get_image_editor( $image_file_to_edit );

		if ( is_jk_error( $image_editor ) ) {
			return new JK_Error(
		...
		$image_name = jk_basename( $image_file, ".{$image_ext}" );
		...
		if ( preg_match( '/-edited(-\d+)?$/', $image_name ) ) {
			// Remove any `-1`, `-2`, etc. `jk_unique_filename()` will add the proper number.
		...
		$filename = jk_unique_filename( $uploads['path'], $filename );
		...
		$new_attachment_id = jk_insert_attachment( jk_slash( $new_attachment_post ), $saved['path'], 0, true );

		if ( is_jk_error( $new_attachment_id ) ) {
		...
		$image_alt = get_post_meta( $attachment_id, '_jk_attachment_image_alt', true );

		if ( ! empty( $image_alt ) ) {
			// update_post_meta() expects slashed.
			update_post_meta( $new_attachment_id, '_jk_attachment_image_alt', jk_slash( $image_alt ) );
		}
		...
		if ( jk_is_serving_rest_request() ) {
			 ...
			 * Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			 */
			header( 'X-JK-Upload-Attachment-ID: ' . $new_attachment_id );
		}

		// Generate image sub-sizes and meta.
		$new_image_meta = jk_generate_attachment_metadata( $new_attachment_id, $saved['path'] );
		...
		$new_image_meta['parent_image'] = array(
			...
			'file'          => _jk_relative_upload_path( $image_file ),
		);
		 ...
		 */
		$new_image_meta = apply_filters( 'jk_edited_image_metadata', $new_image_meta, $new_attachment_id, $attachment_id );

		jk_update_attachment_metadata( $new_attachment_id, $new_image_meta );
	 ...
	 *
	 * @param JK_REST_Request $request Request object.
	 * @return stdClass|JK_Error Post object.
	 */
	 ...
	 *
	 * @param JK_Post         $item    Attachment object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'alt_text', $fields, true ) ) {
			$data['alt_text'] = get_post_meta( $post->ID, '_jk_attachment_image_alt', true );
		}

		if ( in_array( 'media_type', $fields, true ) ) {
			$data['media_type'] = jk_attachment_is_image( $post->ID ) ? 'image' : 'file';
		}
		...
		if ( in_array( 'media_details', $fields, true ) ) {
			$data['media_details'] = jk_get_attachment_metadata( $post->ID );
			...
			if ( empty( $data['media_details'] ) ) {
				...
				foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
					...
					$image_src = jk_get_attachment_image_src( $post->ID, $size );
					if ( ! $image_src ) {
						continue;
					}
				...
				$full_src = jk_get_attachment_image_src( $post->ID, 'full' );

				if ( ! empty( $full_src ) ) {
					$data['media_details']['sizes']['full'] = array(
						'file'       => jk_basename( $full_src[0] ),
						'width'      => $full_src[1],
						'height'     => $full_src[2],
		...
		if ( in_array( 'source_url', $fields, true ) ) {
			$data['source_url'] = jk_get_attachment_url( $post->ID );
		}

		if ( in_array( 'missing_image_sizes', $fields, true ) ) {
			require_once ABSPATH . 'jk-admin/includes/image.php';
			$data['missing_image_sizes'] = array_keys( jk_get_missing_image_subsizes( $post->ID ) );
		}
		 ...
		 * @param JK_Post          $post     The original attachment post.
	 ...
	 * @return array|JK_Error Data from jk_handle_sideload().
	 */
	protected function upload_from_data( $data, $headers, $time = null ) {
		...
		// Include filesystem functions to get access to jk_tempnam() and jk_handle_sideload().
		require_once ABSPATH . 'jk-admin/includes/file.php';

		// Save the file.
		$tmpfname = jk_tempnam( $filename );
		...
		$sideloaded = jk_handle_sideload( $file_data, $overrides, $time );
	 ...
	 * @param array       $headers HTTP headers from the request.
	 * @param string|null $time    Optional. Time formatted in 'yyyy/mm'. Default null.
	 * @return array|JK_Error Data from jk_handle_upload().
	 */
	protected function upload_from_file( $files, $headers, $time = null ) {
		...
		// Pass off to JK to handle the actual upload.
		$overrides = array(
			'test_form' => false,
		);
		...
		if ( defined( 'DIR_TESTDATA' ) && DIR_TESTDATA ) {
			$overrides['action'] = 'jk_handle_mock_upload';
		}
		...
		// Include filesystem functions to get access to jk_handle_upload().
		require_once ABSPATH . 'jk-admin/includes/file.php';

		$file = jk_handle_upload( $files['file'], $overrides, $time );
	 ...
	 * @return true|JK_Error True if can upload, error for errors.
	 */
