<?php
/**
 * Block Pattern Directory REST API: JK_REST_Pattern_Directory_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class JK_REST_Pattern_Directory_Controller extends JK_REST_Controller {
	 ...
	 * @return true|JK_Error True if the request has permission, JK_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		$query_args['jk-version']         = jk_get_jk_version();
		...
		if ( ! $raw_patterns ) {
			 ...
			 */
			$cache_ttl      = 5;
			$wporg_response = jk_remote_get( $api_url );
			$raw_patterns   = json_decode( jk_remote_retrieve_body( $wporg_response ) );

			if ( is_jk_error( $wporg_response ) ) {
				$raw_patterns = $wporg_response;

			} elseif ( ! is_array( $raw_patterns ) ) {
				// HTTP request succeeded, but response data is invalid.
				$raw_patterns = new JK_Error(
					'pattern_api_failed',
					sprintf(
					...
					array(
						'response' => jk_remote_retrieve_body( $wporg_response ),
					)
				);
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$prepared_pattern = array(
			...
			'content'        => jk_kses_post( $raw_pattern->pattern_content ),
		...
		$response = new JK_REST_Response( $prepared_pattern );
	...
	protected function get_transient_key( $query_args ) {
		...
		if ( isset( $query_args['slug'] ) ) {
			...
			$query_args['slug'] = jk_parse_list( $query_args['slug'] );
		...
		return 'jk_remote_block_patterns_' . md5( serialize( $query_args ) );
	}
}
