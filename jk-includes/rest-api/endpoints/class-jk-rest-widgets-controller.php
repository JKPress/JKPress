<?php
/**
 * REST API: JK_REST_Widgets_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class JK_REST_Widgets_Controller extends JK_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( jk_get_sidebars_widgets() as $sidebar_id => $widget_ids ) {
			...
			if ( is_jk_error( $permissions_check ) && ! $this->check_read_sidebar_permission( $sidebar_id ) ) {
				continue;
			}
		...
		return new JK_REST_Response( $prepared );
	}
	...
	public function get_item_permissions_check( $request ) {
		...
		$sidebar_id = jk_find_widgets_sidebar( $widget_id );
	...
	protected function check_read_sidebar_permission( $sidebar_id ) {
		$sidebar = jk_get_sidebar( $sidebar_id );
	...
	public function create_item( $request ) {
		...
		if ( is_jk_error( $widget_id ) ) {
			return $widget_id;
		}

		jk_assign_widget_to_sidebar( $widget_id, $sidebar_id );
	...
	public function update_item( $request ) {
		 ...
		 * jk_inactive_widgets sidebar based on the contents of the $sidebars_widgets global.
		 *
		 * When batch requests are processed, this global is not properly updated by previous
		 * calls, resulting in widgets incorrectly being moved to the jk_inactive_widgets
		 * sidebar.
		 *
		...
		// Allow sidebar to be unset or missing when widget is not a JK_Widget.
		$parsed_id     = jk_parse_widget_id( $widget_id );
		$widget_object = $jk_widget_factory->get_widget_object( $parsed_id['id_base'] );
	 ...
	 *
	 * @global JK_Widget_Factory $jk_widget_factory
	 * @global array             $jk_registered_widget_updates The registered widget update functions.
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response|JK_Error Response object on success, or JK_Error object on failure.
	 */
	public function delete_item( $request ) {
		global $jk_widget_factory, $jk_registered_widget_updates;
		...
		if ( $request['force'] ) {
			...
			$callback = $jk_registered_widget_updates[ $id_base ]['callback'];
			$params   = $jk_registered_widget_updates[ $id_base ]['params'];
			...
			if ( $widget_object ) {
				/*
				 * JK_Widget sets `updated = true` after an update to prevent more than one widget
			...
			jk_assign_widget_to_sidebar( $widget_id, '' );
			...
			jk_assign_widget_to_sidebar( $widget_id, 'jk_inactive_widgets' );

			$response = $this->prepare_item_for_response(
				array(
					'sidebar_id' => 'jk_inactive_widgets',
					'widget_id'  => $widget_id,
				),
				$request
			);
		 ...
		 * @param JK_REST_Response|JK_Error $response   The response data, or JK_Error object on failure.
	 ...
	 * @return string|JK_Error The saved widget ID.
	 */
	protected function save_widget( $request, $sidebar_id ) {
		...
		if ( isset( $request['id'] ) ) {
			...
			$id            = $request['id'];
			$parsed_id     = jk_parse_widget_id( $id );
			$id_base       = $parsed_id['id_base'];
		...
		if ( isset( $request['instance'] ) ) {
			if ( ! $widget_object ) {
				return new JK_Error(
					'rest_invalid_widget',
					__( 'Cannot set instance on a widget that does not extend JK_Widget.' ),
					array( 'status' => 400 )
				);
			}
			...
			if ( isset( $request['instance']['raw'] ) ) {
				...
				if ( ! hash_equals( jk_hash( $serialized_instance ), $request['instance']['hash'] ) ) {
					return new JK_Error(
						'rest_invalid_widget',
	...
	public function prepare_item_for_response( $item, $request ) {
		global $jk_widget_factory, $jk_registered_widgets;
		...
		$widget    = $jk_registered_widgets[ $widget_id ];
		$parsed_id = jk_parse_widget_id( $widget_id );
		...
		if (
			...
			'jk_inactive_widgets' !== $sidebar_id
		) {
			$prepared['rendered'] = trim( jk_render_widget( $widget_id, $sidebar_id ) );
		}

		if ( rest_is_field_included( 'rendered_form', $fields ) ) {
			$rendered_form = jk_render_widget_control( $widget_id );
		...
		if ( rest_is_field_included( 'instance', $fields ) ) {
			...
			if ( $widget_object && isset( $parsed_id['number'] ) ) {
				...
				$prepared['instance']['hash']    = jk_hash( $serialized_instance );
		 ...
		 * @since 5.8.0
		 *
		 * @param JK_REST_Response|JK_Error $response The response object, or JK_Error object on failure.
	...
	protected function prepare_links( $prepared ) {
		...
		return array(
			...
			'about'                     => array(
				'href'       => rest_url( sprintf( 'jk/v2/widget-types/%s', $id_base ) ),
				'embeddable' => true,
			),
			'https://api.w.org/sidebar' => array(
				'href' => rest_url( sprintf( 'jk/v2/sidebars/%s/', $prepared['sidebar'] ) ),
			),
		);
	}
