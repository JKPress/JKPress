<?php
/**
 * REST API: JK_REST_Menus_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class JK_REST_Menus_Controller extends JK_REST_Terms_Controller {
	...
	protected function get_term( $id ) {
		...
		$nav_term           = jk_get_nav_menu_object( $term );
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the current user has permission, JK_Error object otherwise.
	 */
	...
	public function prepare_item_for_response( $term, $request ) {
		$nav_menu = jk_get_nav_menu_object( $term );
	...
	protected function prepare_links( $term ) {
		...
		foreach ( $locations as $location ) {
			$url = rest_url( sprintf( 'jk/v2/menu-locations/%s', $location ) );
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new JK_Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = jk_get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new JK_Error( 'rest_term_invalid', __( 'Parent term does not exist.' ), array( 'status' => 400 ) );
			}
		}

		$prepared_term = $this->prepare_item_for_database( $request );

		$term = jk_update_nav_menu_object( 0, jk_slash( (array) $prepared_term ) );
		...
		if ( is_jk_error( $locations_update ) ) {
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			if ( ! isset( $prepared_term->{'menu-name'} ) ) {
				// jk_update_nav_menu_object() requires that the menu-name is always passed.
				$prepared_term->{'menu-name'} = $term->name;
			}

			$update = jk_update_nav_menu_object( $term->term_id, jk_slash( (array) $prepared_term ) );
	...
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new JK_Error( 'rest_trash_not_supported', sprintf( __( "Menus do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		$result = jk_delete_nav_menu( $term );

		if ( ! $result || is_jk_error( $result ) ) {
			return new JK_Error( 'rest_cannot_delete', __( 'The menu cannot be deleted.' ), array( 'status' => 500 ) );
		}
	...
	protected function handle_auto_add( $menu_id, $request ) {
		...
		do_action( 'jk_update_nav_menu', $menu_id );
	 ...
	 * @param int             $menu_id The menu id to update.
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True on success, a JK_Error on an error updating any of the locations.
	 */
