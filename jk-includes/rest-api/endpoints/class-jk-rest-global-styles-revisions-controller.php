<?php
/**
 * REST API: JK_REST_Global_Styles_Revisions_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 6.3.0
 ...
 */
class JK_REST_Global_Styles_Revisions_Controller extends JK_REST_Revisions_Controller {
	/**
	 * Parent controller.
	 *
	 * @since 6.6.0
	 ...
	 *
	 * @since 6.3.0
	 * @since 6.6.0 Extends class from JK_REST_Revisions_Controller.
	 *
	 * @param string $parent_post_type Post type of the parent.
	 */
	public function __construct( $parent_post_type = 'jk_global_styles' ) {
		parent::__construct( $parent_post_type );
		$post_type_object  = get_post_type_object( $parent_post_type );
		$parent_controller = $post_type_object->get_rest_controller();

		if ( ! $parent_controller ) {
			$parent_controller = new JK_REST_Global_Styles_Controller( $parent_post_type );
		}
	 ...
	 *
	 * The bulk of the body is taken from JK_REST_Revisions_Controller->get_items,
	...
	public function get_items( $request ) {
		...
		if ( is_jk_error( $global_styles_config ) ) {
		...
		if ( jk_revisions_enabled( $parent ) ) {
			...
			foreach ( $parameter_mappings as $api_param => $jk_param ) {
				if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
					$query_args[ $jk_param ] = $request[ $api_param ];
				}
			}
	 ...
	 * @return JK_REST_Response|JK_Error Response object.
	 */
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! empty( $global_styles_config['styles'] ) || ! empty( $global_styles_config['settings'] ) ) {
			$theme_json           = new JK_Theme_JSON( $global_styles_config, 'custom' );
