<?php
/**
 * REST API: JK_REST_Global_Styles_Controller class
 *
 * @package    JKPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class JK_REST_Global_Styles_Controller extends JK_REST_Posts_Controller {
	/**
	 ...
	 */
	public function __construct( $post_type = 'jk_global_styles' ) {
	 ...
	 * For example, `jk/v2/global-styles/twentytwentytwo%200.4.0`
	 ...
	 * @return stdClass|JK_Error Prepared item on success. JK_Error on when the custom CSS is not valid.
	 */
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			if ( isset( $request['styles'] ) ) {
				if ( isset( $request['styles']['css'] ) ) {
					...
					if ( is_jk_error( $css_validation_result ) ) {
			...
			$variations = JK_Theme_JSON_Resolver::get_style_variations( 'block' );
			...
			$config['version']                     = JK_Theme_JSON::LATEST_SCHEMA;
			$changes->post_content                 = jk_json_encode( $config );
		}
	 ...
	 *
	 * @param JK_Post         $post    Global Styles post object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new JK_Theme_JSON( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = JK_Theme_JSON_Resolver::get_resolved_theme_uris( $theme_json );
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$revisions                = jk_get_latest_revision_id_and_total_count( $id );
	...
	public function get_theme_item( $request ) {
		...
		$theme  = JK_Theme_JSON_Resolver::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = JK_Theme_JSON_Resolver::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
			$response->add_links( $links );
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = JK_Theme_JSON_Resolver::get_style_variations( 'block' );
		jk_register_block_style_variations_from_theme_json_partials( $partials );

		$variations = JK_Theme_JSON_Resolver::get_style_variations();
		foreach ( $variations as $variation ) {
			$variation_theme_json = new JK_Theme_JSON( $variation );
			$resolved_theme_uris  = JK_Theme_JSON_Resolver::get_resolved_theme_uris( $variation_theme_json );
	 ...
	 * @return true|JK_Error True if the input was validated, otherwise JK_Error.
	 */
