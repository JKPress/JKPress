<?php
/**
 * REST API: JK_REST_URL_Details_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class JK_REST_URL_Details_Controller extends JK_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'url' => array(
							...
							'validate_callback' => 'jk_http_validate_url',
	 ...
	 * @return JK_REST_Response|JK_Error The parsed details as a response object. JK_Error if there are errors.
	 */
	public function parse_url_details( $request ) {
		$url = untrailingslashit( $request['url'] );

		if ( empty( $url ) ) {
			return new JK_Error( 'rest_invalid_url', __( 'Invalid URL' ), array( 'status' => 404 ) );
		}
		...
		if ( ! empty( $cached_response ) ) {
			...
			if ( is_jk_error( $remote_url_response ) || empty( $remote_url_response ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return true|JK_Error True if the request has permission, else JK_Error.
	 */
	 ...
	 * @return string|JK_Error The HTTP response from the remote URL on success.
	 *                         JK_Error if no response or no content.
	 */
	private function get_remote_url( $url ) {

		/*
		 * Provide a modified UA string to workaround web properties which block JKPress "Pingbacks".
		 * Why? The UA string used for pingback requests contains `JKPress/` which is very similar
		 * to that used as the default UA string by the JK HTTP API. Therefore requests from this
		 * REST endpoint are being unintentionally blocked as they are misidentified as pingback requests.
		 * By slightly modifying the UA string, but still retaining the "JKPress" identification (via "JK")
		 * we are able to work around this issue.
		 * Example UA string: `JK-URLDetails/5.9-alpha-51389 (+http://localhost:8888)`.
		*/
		$modified_user_agent = 'JK-URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		 ...
		 *
		 * Can be used to adjust response size limit and other JK_Http::request() args.
		 *
		 * @since 5.9.0
		 *
		...
		$response = jk_safe_remote_get( $url, $args );

		if ( JK_Http::OK !== jk_remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new JK_Error(
				'no_response',
				__( 'URL not found. Response returned a non-200 status code for this URL.' ),
				array( 'status' => JK_Http::NOT_FOUND )
			);
		}

		$remote_body = jk_remote_retrieve_body( $response );
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = JK_Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = JK_Http::make_absolute_url( $image, $root_url );
		}
	...
	private function prepare_metadata_for_output( $metadata ) {
		...
		$metadata = jk_strip_all_tags( $metadata );
		return $metadata;
	}
