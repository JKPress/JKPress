<?php
/**
 * REST API: JK_REST_Application_Passwords_Controller class
 *
 * @package    JKPress
 * @subpackage REST_API
 * @since      5.6.0
 ...
 */
class JK_REST_Application_Passwords_Controller extends JK_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$passwords = JK_Application_Passwords::get_user_application_passwords( $user->ID );
		$response  = array();
	...
	public function create_item( $request ) {
		...
		if ( is_jk_error( $prepared ) ) {
			return $prepared;
		}

		$created = JK_Application_Passwords::create_new_application_password( $user->ID, jk_slash( (array) $prepared ) );

		if ( is_jk_error( $created ) ) {
			return $created;
		}

		$password = $created[0];
		$item     = JK_Application_Passwords::get_user_application_password( $user->ID, $created[1]['uuid'] );

		$item['new_password'] = JK_Application_Passwords::chunk_password( $password );
	...
	public function update_item( $request ) {
		...
		$saved = JK_Application_Passwords::update_application_password( $user->ID, $item['uuid'], jk_slash( (array) $prepared ) );
		...
		$item = JK_Application_Passwords::get_user_application_password( $user->ID, $item['uuid'] );

		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-application-passwords-controller.php */
	...
	public function delete_items( $request ) {
		...
		$deleted = JK_Application_Passwords::delete_all_application_passwords( $user->ID );

		if ( is_jk_error( $deleted ) ) {
			return $deleted;
		}
	...
	public function delete_item( $request ) {
		...
		$deleted  = JK_Application_Passwords::delete_application_password( $user->ID, $password['uuid'] );
	...
	public function get_current_item( $request ) {
		...
		$password = JK_Application_Passwords::get_user_application_password( $user->ID, $uuid );
	 ...
	 *
	 * @param JK_REST_Request $request Request object.
	 * @return object|JK_Error The prepared item, or JK_Error object on failure.
	 */
	 ...
	 * @since 5.6.0
	 *
	 * @param array           $item    JKPress representation of the item.
	 * @param JK_REST_Request $request Request object.
	 * @return JK_REST_Response|JK_Error Response object on success, or JK_Error object on failure.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new JK_REST_Response( $prepared );
	 ...
	 * @since 5.6.0
	 *
	 * @param JK_User $user The requested user.
	 * @param array   $item The application password.
	 * @return array The list of links.
	 */
	protected function prepare_links( JK_User $user, $item ) {
		return array(
			'self' => array(
				'href' => rest_url(
					sprintf(
	 ...
	 *
	 * @param JK_REST_Request $request The request object.
	 * @return JK_User|JK_Error The JKPress user associated with the request, or a JK_Error if none found.
	 */
	protected function get_user( $request ) {
		...
		if ( ! jk_is_application_passwords_available_for_user( $user ) ) {
			return new JK_Error(
	 ...
	 *
	 * @param JK_REST_Request $request The request object.
	 * @return array|JK_Error The application password details if found, a JK_Error otherwise.
	 */
	protected function get_application_password( $request ) {
		...
		$password = JK_Application_Passwords::get_user_application_password( $user->ID, $request['uuid'] );
