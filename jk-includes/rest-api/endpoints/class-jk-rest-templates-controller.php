<?php
/**
 * REST API: JK_REST_Templates_Controller class
 *
 * @package    JKPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class JK_REST_Templates_Controller extends JK_REST_Controller {
	 ...
	 *
	 * @param JK_REST_Request $request The request instance.
	 * @return JK_REST_Response|JK_Error
	 */
	 ...
	 * requires using a path like /jk/v2/templates/twentytwentytwo//home. There
	 * are special cases when JKPress routing corrects the name to contain
	...
	public function get_items( $request ) {
		$query = array();
		if ( isset( $request['jk_id'] ) ) {
			$query['jk_id'] = $request['jk_id'];
		}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has read access for the item, JK_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new JK_Error( 'rest_template_not_found', __( 'No templates exist with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has write access for the item, JK_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		$post_before = get_post( $template->jk_id );

		if ( isset( $request['source'] ) && 'theme' === $request['source'] ) {
			jk_delete_post( $template->jk_id, true );
		...
		if ( is_jk_error( $changes ) ) {
			return $changes;
		}

		if ( 'custom' === $template->source ) {
			$update = true;
			$result = jk_update_post( jk_slash( (array) $changes ), false );
		} else {
			$update      = false;
			$post_before = null;
			$result      = jk_insert_post( jk_slash( (array) $changes ), false );
		}
	...
	public function create_item( $request ) {
		...
		$post_id                  = jk_insert_post( jk_slash( (array) $prepared_post ), true );
		...
		$posts = get_block_templates( array( 'jk_id' => $post_id ), $this->post_type );
		if ( ! count( $posts ) ) {
			return new JK_Error( 'rest_template_insert_error', __( 'No templates exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has delete access for the item, JK_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		...
		if ( 'custom' !== $template->source ) {
			return new JK_Error( 'rest_invalid_template', __( 'Templates based on theme files can\'t be removed.' ), array( 'status' => 400 ) );
		}
	 ...
	 * @return stdClass|JK_Error Changes to pass to jk_update_post.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( null === $template ) {
			...
			$changes->tax_input   = array(
				'jk_theme' => isset( $request['theme'] ) ? $request['theme'] : get_stylesheet(),
			);
		...
		if ( 'jk_template' === $this->post_type && isset( $request['is_jk_suggestion'] ) ) {
			$changes->meta_input     = jk_parse_args(
				array(
					'is_jk_suggestion' => $request['is_jk_suggestion'],
				),
				$changes->meta_input = array()
			);
		}

		if ( 'jk_template_part' === $this->post_type ) {
			if ( isset( $request['area'] ) ) {
				$changes->tax_input['jk_template_part_area'] = _filter_block_template_part_area( $request['area'] );
			} elseif ( null !== $template && 'custom' !== $template->source && $template->area ) {
				$changes->tax_input['jk_template_part_area'] = _filter_block_template_part_area( $template->area );
			} elseif ( empty( $template->area ) ) {
				$changes->tax_input['jk_template_part_area'] = JK_TEMPLATE_PART_AREA_UNCATEGORIZED;
			}
		}
		...
		/** This filter is documented in jk-includes/rest-api/endpoints/class-jk-rest-posts-controller.php */
	 ...
	 *
	 * @param JK_Block_Template $item    Template instance.
	 * @param JK_REST_Request   $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( rest_is_field_included( 'title.rendered', $fields ) ) {
			if ( $template->jk_id ) {
				...
				$data['title']['rendered'] = apply_filters( 'the_title', $template->title, $template->jk_id );
			} else {
		...
		if ( rest_is_field_included( 'jk_id', $fields ) ) {
			$data['jk_id'] = (int) $template->jk_id;
		}
		...
		if ( rest_is_field_included( 'is_custom', $fields ) && 'jk_template' === $template->type ) {
		...
		if ( rest_is_field_included( 'area', $fields ) && 'jk_template_part' === $template->type ) {
		...
		if ( rest_is_field_included( 'author_text', $fields ) ) {
			$data['author_text'] = self::get_jk_templates_author_text_field( $template );
		}

		if ( rest_is_field_included( 'original_source', $fields ) ) {
			$data['original_source'] = self::get_jk_templates_original_source_field( $template );
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param JK_Block_Template $template_object Template instance.
	 * @return string                            Original source of the template one of theme, plugin, site, or user.
	 */
	private static function get_jk_templates_original_source_field( $template_object ) {
		if ( 'jk_template' === $template_object->type || 'jk_template_part' === $template_object->type ) {
			/*
			 * Added by theme.
			 ...
			 * was only added to templates in JKPress 5.9. Fallback to showing the
			 * site logo and title.
			 */
	 ...
	 */
	private static function get_jk_templates_author_text_field( $template_object ) {
		$original_source = self::get_jk_templates_original_source_field( $template_object );
		switch ( $original_source ) {
			case 'theme':
				$theme_name = jk_get_theme( $template_object->theme )->get( 'Name' );
			...
			case 'plugin':
				...
				if ( isset( $template_object->plugin ) ) {
					$plugins = jk_get_active_and_valid_plugins();
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$template = get_block_template( $id, $this->post_type );
			if ( $template instanceof JK_Block_Template && ! empty( $template->jk_id ) ) {
				$revisions       = jk_get_latest_revision_id_and_total_count( $template->jk_id );
