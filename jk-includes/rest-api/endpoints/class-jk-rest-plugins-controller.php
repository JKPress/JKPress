<?php
/**
 * REST API: JK_REST_Plugins_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class JK_REST_Plugins_Controller extends JK_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'slug'   => array(
							'type'        => 'string',
							'required'    => true,
							'description' => __( 'JKPress.org plugin directory slug.' ),
							'pattern'     => '[\w\-]+',
						),
						'status' => array(
	...
	public function get_items( $request ) {
		...
		foreach ( get_plugins() as $file => $data ) {
			if ( is_jk_error( $this->check_read_permission( $file ) ) ) {
				continue;
			}
		...
		return new JK_REST_Response( $plugins );
	}
	 ...
	 * @return true|JK_Error True if can read, a JK_Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'jk-admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new JK_Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_jk_error( $filesystem_available ) ) {
		...
		if ( is_null( $result ) ) {
			...
			if ( $jk_filesystem instanceof JK_Filesystem_Base
				&& is_jk_error( $jk_filesystem->errors ) && $jk_filesystem->errors->has_errors()
			) {
				return new JK_Error(
					'unable_to_connect_to_filesystem',
					$jk_filesystem->errors->get_error_message(),
					array( 'status' => 500 )
				);
			}
		...
		/** This filter is documented in jk-includes/update.php */
	 ...
	 * @return array|JK_Error The plugin data, or a JK_Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param JK_REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( JK_PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|JK_Error True if filesystem is available, JK_Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new JK_Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'requires_jk'  => array(
					'description' => __( 'Minimum required version of JKPress.' ),
					'type'        => 'string',
					'readonly'    => true,
					'context'     => array( 'view', 'edit', 'embed' ),
				),
