<?php
/**
 * REST API: JK_REST_Block_Types_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class JK_REST_Block_Types_Controller extends JK_REST_Controller {

	const NAME_PATTERN = '^[a-z][a-z0-9-]*/[a-z][a-z0-9-]*$';

	/**
	 * Instance of JK_Block_Type_Registry.
	 *
	 * @since 5.5.0
	 * @var JK_Block_Type_Registry
	 */
	protected $block_registry;

	/**
	 * Instance of JK_Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var JK_Block_Styles_Registry
	 */
	...
	public function __construct() {
		...
		$this->block_registry = JK_Block_Type_Registry::get_instance();
		$this->style_registry = JK_Block_Styles_Registry::get_instance();
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|JK_Error True if the block type is visible, JK_Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new JK_Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return JK_Block_Type|JK_Error Block type object if name is valid, JK_Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new JK_Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param JK_Block_Type   $item    Block type data.
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response Block type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		// Fields deprecated in JKPress 6.1, but left in the schema for backwards compatibility.
		...
		if ( rest_is_field_included( 'styles', $fields ) ) {
			...
			$data['styles'] = jk_parse_args( $styles, $data['styles'] );
		 ...
		 * @param JK_Block_Type    $block_type The original block type object.
	 ...
	 * @since 5.5.0
	 *
	 * @param JK_Block_Type $block_type Block type data.
		...
		if ( $block_type->is_dynamic() ) {
			$links['https://api.w.org/render-block'] = array(
				'href' => add_query_arg(
					'context',
					'edit',
					rest_url( sprintf( '%s/%s/%s', 'jk/v2', 'block-renderer', $block_type->name ) )
				),
			);
		}
	...
	public function get_item_schema() {
		...
		// Properties deprecated in JKPress 6.1, but left in the schema for backwards compatibility.
