<?php
/**
 * REST API: JK_REST_Users_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JK_REST_Users_Controller extends JK_REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var JK_REST_User_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new JK_REST_User_Meta_Fields();
	}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has read access, otherwise JK_Error object.
	 */
	...
	public function get_items( $request ) {
		...
		/**
		 * Filters JK_User_Query arguments when querying users via the REST API.
		 *
		 * @link https://developer.wordpress.org/reference/classes/jk_user_query/
		 *
		 * @since 4.7.0
		 *
		 * @param array           $prepared_args Array of arguments for JK_User_Query.
		 * @param JK_REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_user_query', $prepared_args, $request );

		$query = new JK_User_Query( $prepared_args );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new JK_User_Query( $prepared_args );
			$total_users = $count_query->get_total();
		}

		$response->header( 'X-JK-Total', (int) $total_users );
	 ...
	 * @return JK_User|JK_Error True if ID is valid, JK_Error otherwise.
	 */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has read access for the item, otherwise JK_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $request['roles'] ) && ! empty( $schema['properties']['roles'] ) ) {
			...
			if ( is_jk_error( $check_permission ) ) {
		...
		if ( is_multisite() ) {
			$ret = jkmu_validate_user_signup( $user->user_login, $user->user_email );

			if ( is_jk_error( $ret['errors'] ) && $ret['errors']->has_errors() ) {
				$error = new JK_Error(
					'rest_invalid_param',
					__( 'Invalid user parameter(s).' ),
					array( 'status' => 400 )
		...
		if ( is_multisite() ) {
			$user_id = jkmu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			...
			$user->ID = $user_id;
			$user_id  = jk_update_user( jk_slash( (array) $user ) );
			...
			$user_id = jk_insert_user( jk_slash( (array) $user ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_User         $user     Inserted or updated user object.
	...
	public function update_item( $request ) {
		...
		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-users-controller.php */
	...
	public function delete_item( $request ) {
		...
		// Include user admin functions to get access to jk_delete_user().
		require_once ABSPATH . 'jk-admin/includes/user.php';

		$result = jk_delete_user( $id, $reassign );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_User          $user     The user data.
	 ...
	 *
	 * @param JK_User         $item    User object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param JK_User          $user     User object used to create response.
		 * @param JK_REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|JK_Error True if the current user is allowed to make the role change,
	 *                       otherwise a JK_Error object.
	 */
	protected function check_role_update( $user_id, $roles ) {
		...
		foreach ( $roles as $role ) {
			...
			if ( ! isset( $jk_roles->role_objects[ $role ] ) ) {
				return new JK_Error(
			...
			$potential_role = $jk_roles->role_objects[ $role ];
	 ...
	 *
	 * Performs a couple of checks like edit_user() in jk-admin/includes/user.php.
	 *
	 * @since 4.7.0
	 *
	 ...
	 * @return string|JK_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param JK_REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|JK_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal JK_User_Query parameter.  Use the
		 * `rest_user_query` filter to set JK_User_Query arguments.
		 *
		 * @since 4.7.0
		 *
