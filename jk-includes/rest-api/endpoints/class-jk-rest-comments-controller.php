<?php
/**
 * REST API: JK_REST_Comments_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JK_REST_Comments_Controller extends JK_REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var JK_REST_Comment_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		$this->namespace = 'jk/v2';
		$this->rest_base = 'comments';

		$this->meta = new JK_REST_Comment_Meta_Fields();
	}
	 ...
	 * @return true|JK_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response|JK_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		foreach ( $parameter_mappings as $api_param => $jk_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$prepared_args[ $jk_param ] = $request[ $api_param ];
			}
		}
		...
		/**
		 * Filters JK_Comment_Query arguments when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		 * @link https://developer.wordpress.org/reference/classes/jk_comment_query/
		 *
		 * @param array           $prepared_args Array of arguments for JK_Comment_Query.
		...
		$response->header( 'X-JK-Total', $total_comments );
		$response->header( 'X-JK-TotalPages', $max_pages );
	 ...
	 * @return JK_Comment|JK_Error Comment object if ID is valid, JK_Error otherwise.
	 */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has read access for the item, error object otherwise.
	 */
	public function get_item_permissions_check( $request ) {
		$comment = $this->get_comment( $request['id'] );
		if ( is_jk_error( $comment ) ) {
			return $comment;
		}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param JK_REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		...
		if ( is_jk_error( $prepared_comment ) ) {
		...
		// Setting remaining values before jk_insert_comment so we can use jk_allow_comment().
		...
		$check_comment_lengths = jk_check_comment_data_max_lengths( $prepared_comment );

		if ( is_jk_error( $check_comment_lengths ) ) {
		...
		$prepared_comment['comment_approved'] = jk_allow_comment( $prepared_comment, true );

		if ( is_jk_error( $prepared_comment['comment_approved'] ) ) {
		 ...
		 *
		 * Allows modification of the comment right before it is inserted via jk_insert_comment().
		 * Returning a JK_Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a JK_Error to short-circuit insertion.
		 *
		 * @param array|JK_Error  $prepared_comment The prepared comment data for jk_insert_comment().
		 * @param JK_REST_Request $request          Request used to insert the comment.
		 */
		...
		$comment_id = jk_insert_comment( jk_filter_comment( jk_slash( (array) $prepared_comment ) ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to update the item, error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			$check_comment_lengths = jk_check_comment_data_max_lengths( $prepared_args );
			...
			$updated = jk_update_comment( jk_slash( (array) $prepared_args ), true );
		...
		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-comments-controller.php */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param JK_Comment $comment        The comment object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			$result   = jk_delete_comment( $comment->comment_ID, true );
			...
			$result   = jk_trash_comment( $comment->comment_ID );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Comment       $comment  The deleted comment data.
		 * @param JK_REST_Response $response The response returned from the API.
	 ...
	 *
	 * @param JK_Comment      $item    Comment object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param JK_Comment        $comment  The original comment object.
	...
	protected function prepare_links( $comment ) {
		...
		if ( 0 !== (int) $comment->user_id ) {
			$links['author'] = array(
				'href'       => rest_url( 'jk/v2/users/' . $comment->user_id ),
				'embeddable' => true,
			);
		}
	 ...
	 *
	 * @param JK_REST_Request $request Request object.
	 * @return array|JK_Error Prepared comment, otherwise JK_Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['author'] ) ) {
			$user = new JK_User( $request['author'] );
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_comment The prepared comment data for `jk_insert_comment`.
		 * @param JK_REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal JK_Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set JK_Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function handle_status_param( $new_status, $comment_id ) {
		$old_status = jk_get_comment_status( $comment_id );
		...
		switch ( $new_status ) {
			...
			case 'approve':
			case '1':
				$changed = jk_set_comment_status( $comment_id, 'approve' );
				break;
			case 'hold':
			case '0':
				$changed = jk_set_comment_status( $comment_id, 'hold' );
				break;
			case 'spam':
				$changed = jk_spam_comment( $comment_id );
				break;
			case 'unspam':
				$changed = jk_unspam_comment( $comment_id );
				break;
			case 'trash':
				$changed = jk_trash_comment( $comment_id );
				break;
			case 'untrash':
				$changed = jk_untrash_comment( $comment_id );
				break;
			default:
				$changed = false;
				break;
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		/*
		 * Ensure the posts controller is specifically a JK_REST_Posts_Controller instance
		 * before using methods specific to that controller.
		 */
		if ( ! $posts_controller instanceof JK_REST_Posts_Controller ) {
			$posts_controller = new JK_REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|JK_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
	...
	protected function check_is_comment_content_allowed( $prepared_comment ) {
		 ...
		 * comment_content. See jk_handle_comment_submission().
		 */
