<?php
/**
 * REST API: JK_REST_Widget_Types_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class JK_REST_Widget_Types_Controller extends JK_REST_Controller {
	 ...
	 * @since 5.8.0
	 *
	 * @return true|JK_Error True if the widget type is visible, JK_Error otherwise.
	 */
	 ...
	 * @return array|JK_Error The array of widget data if the name is valid, JK_Error otherwise.
	 */
	public function get_widget( $id ) {
		...
		return new JK_Error( 'rest_widget_type_invalid', __( 'Invalid widget type.' ), array( 'status' => 404 ) );
	}
	 ...
	 * @global array             $jk_registered_widgets The list of registered widgets.
	 *
	...
	protected function get_widgets() {
		...
		foreach ( $jk_registered_widgets as $widget ) {
			$parsed_id     = jk_parse_widget_id( $widget['id'] );
	 ...
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Response Widget type data.
	 */
	...
	public function encode_form_data( $request ) {
		...
		$id            = $request['id'];
		$widget_object = $jk_widget_factory->get_widget_object( $id );

		if ( ! $widget_object ) {
			return new JK_Error(
				'rest_invalid_widget',
				__( 'Cannot preview a widget that does not extend JK_Widget.' ),
				array( 'status' => 400 )
			);
		}
		...
		$widget_key          = $jk_widget_factory->get_widget_key( $id );
		...
		$response = array(
			...
			'instance' => array(
				...
				'hash'    => jk_hash( $serialized_instance ),
			),
		);
	...
	/**
	 * Returns the output of JK_Widget::widget() when called with the provided
	...
	/**
	 * Returns the output of JK_Widget::form() when called with the provided
	 * instance. Used by encode_form_data() to preview a widget's form.
	 *
	 * @since 5.8.0
	 *
	 * @param JK_Widget $widget_object Widget object to call widget() on.
	 * @param array     $instance Widget instance settings.
	 * @return string
	 */
