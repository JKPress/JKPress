<?php
/**
 * REST API: JK_REST_Terms_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class JK_REST_Terms_Controller extends JK_REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->namespace = ! empty( $tax_obj->rest_namespace ) ? $tax_obj->rest_namespace : 'jk/v2';

		$this->meta = new JK_REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return bool|JK_Error True if the request has read access, otherwise false or JK_Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * Otherwise, terms will be fetched using `jk_get_object_terms()`,
		...
		if ( ! empty( $prepared_args['post'] ) ) {
			$query_result = jk_get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling jk_count_terms() below.
		...
		$total_terms = jk_count_terms( $count_args );

		// jk_count_terms() can return a falsey value when the term has no children.
		if ( ! $total_terms ) {
			$total_terms = 0;
		}
		...
		$response->header( 'X-JK-Total', (int) $total_terms );
	 ...
	 * @return JK_Term|JK_Error Term object if ID is valid, JK_Error otherwise.
	 */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return bool|JK_Error True if the request has access to create items, otherwise false or JK_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		$term = jk_insert_term( jk_slash( $prepared_term->name ), $this->taxonomy, jk_slash( (array) $prepared_term ) );
		if ( is_jk_error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to update the item, false or JK_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = jk_update_term( $term->term_id, $term->taxonomy, jk_slash( (array) $prepared_term ) );

			if ( is_jk_error( $update ) ) {
				return $update;
			}
		}
		...
		/** This action is documented in jk-includes/rest-api/endpoints/class-jk-rest-terms-controller.php */
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return true|JK_Error True if the request has access to delete the item, otherwise false or JK_Error object.
	 */
	...
	public function delete_item( $request ) {
		...
		$retval = jk_delete_term( $term->term_id, $term->taxonomy );
		 ...
		 * @since 4.7.0
		 *
		 * @param JK_Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof JK_Term ) {
	 ...
	 * @since 4.7.0
	 *
	 * @param JK_Term         $item    Term object.
	 * @param JK_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param JK_Term           $item      The original term object.
	...
	protected function prepare_links( $term ) {
		$links = array(
			...
			'about'      => array(
				'href' => rest_url( sprintf( 'jk/v2/taxonomies/%s', $this->taxonomy ) ),
			),
		);
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal JK_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set JK_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
