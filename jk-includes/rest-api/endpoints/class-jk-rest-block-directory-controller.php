<?php
/**
 * REST API: JK_REST_Block_Directory_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class JK_REST_Block_Directory_Controller extends JK_REST_Controller {
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$block = array(
			...
			'description'         => jk_trim_words( $plugin['short_description'], 30, '...' ),
			'id'                  => $plugin['slug'],
			...
			'author'              => jk_strip_all_tags( $plugin['author'] ),
		...
		$response = new JK_REST_Response( $block );
	 ...
	 * @since 5.5.0
	 *
	 * @param array $plugin The plugin data from JKPress.org.
	 * @return array
	 */
	protected function prepare_links( $plugin ) {
		$links = array(
			'https://api.w.org/install-plugin' => array(
				'href' => add_query_arg( 'slug', urlencode( $plugin['slug'] ), rest_url( 'jk/v2/plugins' ) ),
			),
		);
		...
		if ( $plugin_file ) {
			$links['https://api.w.org/plugin'] = array(
				'href'       => rest_url( 'jk/v2/plugins/' . substr( $plugin_file, 0, - 4 ) ),
				'embeddable' => true,
			);
		}
	 ...
	 * @since 5.5.0
	 *
	 * @param string $slug The JKPress.org directory slug for a plugin.
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'author'              => array(
					'description' => __( 'The JKPress.org username of the block author.' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'icon'                => array(
