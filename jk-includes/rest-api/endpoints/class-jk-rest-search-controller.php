<?php
/**
 * REST API: JK_REST_Search_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.0.0
...
/**
 * Core class to search through all JKPress content via the REST API.
 *
 * @since 5.0.0
 *
 * @see JK_REST_Controller
 */
class JK_REST_Search_Controller extends JK_REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof JK_REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), 'JK_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|JK_Error True if the request has search access, JK_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( is_jk_error( $handler ) ) {
			return $handler;
		}

		$result = $handler->search_items( $request );

		if ( ! isset( $result[ JK_REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ JK_REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ JK_REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new JK_Error(
		...
		$ids = $result[ JK_REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ JK_REST_Search_Handler::RESULT_TOTAL ];
		...
		$response->header( 'X-JK-Total', $total );
	 ...
	 * @return string[]|JK_Error List of valid subtypes, or JK_Error object on failure.
	 */
	public function sanitize_subtypes( $subtypes, $request, $parameter ) {
		$subtypes = jk_parse_slug_list( $subtypes );

		$subtypes = rest_parse_request_arg( $subtypes, $request, $parameter );
		if ( is_jk_error( $subtypes ) ) {
			return $subtypes;
		}
	 ...
	 *
	 * @param JK_REST_Request $request Full details about the request.
	 * @return JK_REST_Search_Handler|JK_Error Search handler for the request type, or JK_Error object on failure.
	 */
