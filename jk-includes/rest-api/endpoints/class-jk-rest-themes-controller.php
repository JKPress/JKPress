<?php
/**
 * REST API: JK_REST_Themes_Controller class
 *
 * @package JKPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class JK_REST_Themes_Controller extends JK_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		$jk_theme      = jk_get_theme( $request['stylesheet'] );
		$current_theme = jk_get_theme();

		if ( $this->is_same_theme( $jk_theme, $current_theme ) ) {
			return $this->check_read_active_theme_permission();
		}
	 ...
	 * @since 5.7.0
	 *
	 * @return true|JK_Error True if the theme can be read, JK_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $jk_theme, $request );
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-JK-Total', count( $themes ) );
		$response->header( 'X-JK-TotalPages', 1 );
	 ...
	 *
	 * @param JK_Theme        $item    Theme object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param JK_Theme         $theme    Theme object used to create response.
		 * @param JK_REST_Request  $request  Request object.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param JK_Theme $theme Theme data.
	...
	protected function prepare_links( $theme ) {
		...
		if ( $this->is_same_theme( $theme, jk_get_theme() ) ) {
			// This creates a record for the active theme if not existent.
			$id = JK_Theme_JSON_Resolver::get_user_global_styles_post_id();
		} else {
			$user_cpt = JK_Theme_JSON_Resolver::get_user_data_from_jk_global_styles( $theme );
			$id       = isset( $user_cpt['ID'] ) ? $user_cpt['ID'] : null;
		}

		if ( $id ) {
			$links['https://api.w.org/user-global-styles'] = array(
				'href' => rest_url( 'jk/v2/global-styles/' . $id ),
			);
		}
	 ...
	 * @since 5.7.0
	 *
	 * @param JK_Theme $theme_a First theme to compare.
	 * @param JK_Theme $theme_b Second theme to compare.
	 * @return bool
	 */
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'requires_jk'    => array(
					'description' => __( 'The minimum JKPress version required for the theme to work.' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'screenshot'     => array(
