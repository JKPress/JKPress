<?php
/**
 * Dependencies API: JK_Dependencies base class
 *
 * @since 2.6.0
 *
 * @package JKPress
...
class JK_Dependencies {
	 ...
	 *
	 * @since 2.1.0
	 * @since 2.6.0 Moved from `JK_Scripts`.
	 * @since 2.8.0 Added the `$group` parameter.
	 *
	 ...
	 *                                 to the JKPress root directory. If source is set to false,
	 ...
	 *                                 equal to current installed JKPress version.
	...
	public function get_etag( $load ) {
		...
		foreach ( $load as $handle ) {
			...
			$ver   = $this->registered[ $handle ]->ver ?? $jk_version;
			$etag .= "{$handle}:{$ver};";
		}
		 ...
		 * load the full JKPress environment, it is not possible to use the salted
		 * jk_hash() function.
		 */
		return 'W/"' . md5( $etag ) . '"';
	}
