<?php
/**
 * JKPress Plugin Administration API: JK_Plugin_Dependencies class
 *
 * @package JKPress
...
class JK_Plugin_Dependencies {
	...
	public static function check_plugin_dependencies_during_ajax() {
		...
		if ( current_user_can( 'activate_plugin', $plugin_file ) && is_plugin_inactive( $plugin_file ) ) {
			$status['activateUrl'] = add_query_arg(
				array(
					'_jknonce' => jk_create_nonce( 'activate-plugin_' . $plugin_file ),
					'action'   => 'activate',
					'plugin'   => $plugin_file,
				),
	...
	protected static function sanitize_dependency_slugs( $slugs ) {
		...
		foreach ( $slugs as $slug ) {
			 ...
			 */
			$slug = apply_filters( 'jk_plugin_dependencies_slug', $slug );

			// Match to JKPress.org slug format.
	...
	/**
	 * Retrieves and stores dependency plugin data from the JKPress.org Plugin API.
	 *
	 * @since 6.5.0
	 *
	...
	protected static function get_dependency_api_data() {
		...
		$plugins                   = self::get_plugins();
		self::$dependency_api_data = (array) get_site_transient( 'jk_plugin_dependencies_plugin_data' );
		foreach ( self::$dependency_slugs as $slug ) {
			// Set transient for individual data, remove from self::$dependency_api_data if transient expired.
			if ( ! get_site_transient( "jk_plugin_dependencies_plugin_timeout_{$slug}" ) ) {
				unset( self::$dependency_api_data[ $slug ] );
				set_site_transient( "jk_plugin_dependencies_plugin_timeout_{$slug}", true, 12 * HOUR_IN_SECONDS );
			}
			...
			set_site_transient( 'jk_plugin_dependencies_plugin_data', self::$dependency_api_data, 0 );
		}
