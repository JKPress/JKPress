 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', JKINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'JKPress/' . $GLOBALS['jk_version']);
...
class MagpieRSS {
	...
	function __construct( $source ) {
		...
		if ( ! function_exists('xml_parser_create') ) {
			jk_trigger_error( '', "PHP's XML extension is not available. Please contact your hosting provider to enable PHP's XML extension." );
			return;
		}
...
/**
 * Retrieve URL headers and content using JK HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = jk_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_jk_error($resp) ) {
	...
	// Also note, JK_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( jk_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = jk_remote_retrieve_response_code( $resp );
	$response->response_code = jk_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = jk_remote_retrieve_body( $resp );
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = JK_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function jk_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with jk_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
