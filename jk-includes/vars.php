<?php
/**
 * Creates common globals for the rest of JKPress
 *
 ...
 *
 * Detects which user environment JKPress is being used on.
 * Only attempts to check for Apache, Nginx and IIS -- three web
 * servers with known pretty permalink capability.
 *
 * Note: Though Nginx is detected, JKPress does not currently
...
if ( is_admin() ) {
	// jk-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/jk-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/jk-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/jk-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /jk-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
...
function jk_is_mobile() {
	 ...
	 */
	return apply_filters( 'jk_is_mobile', $is_mobile );
}
