<?php
/**
 * JKPress Post Thumbnail Template Functions.
 *
 ...
 *
 * @since 2.9.0
 * @since 4.4.0 `$post` can be a post ID or JK_Post object.
 *
...
function has_post_thumbnail( $post = null ) {
	 ...
	 * @param int|JK_Post|null $post          Post ID or JK_Post object. Default is global `$post`.
 ...
 *
 * @param JK_Query|null $jk_query Optional. A JK_Query instance. Defaults to the $jk_query global.
 */
function update_post_thumbnail_cache( $jk_query = null ) {
	if ( ! $jk_query ) {
		$jk_query = $GLOBALS['jk_query'];
	}

	if ( $jk_query->thumbnails_cached ) {
		return;
	}
	...
	/*
	 * $jk_query may contain an array of post objects or post IDs.
	 *
	...
	foreach ( $jk_query->posts as $post ) {
	...
	$jk_query->thumbnails_cached = true;
}
...
function get_the_post_thumbnail( $post = null, $size = 'post-thumbnail', $attr = '' ) {
	...
	if ( $post_thumbnail_id ) {
		 ...
		 *
		 * Provides "just in time" filtering of all filters in jk_get_attachment_image().
		 *
		 * @since 2.9.0
		 *
		 * @param int          $post_id           The post ID.
		...
		$html = jk_get_attachment_image( $post_thumbnail_id, $size, false, $attr );
...
function get_the_post_thumbnail_url( $post = null, $size = 'post-thumbnail' ) {
	...
	$thumbnail_url = jk_get_attachment_image_url( $post_thumbnail_id, $size );
...
function get_the_post_thumbnail_caption( $post = null ) {
	...
	$caption = jk_get_attachment_caption( $post_thumbnail_id );
