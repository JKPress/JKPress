 ...
 *
 * Standardizes the HTTP requests for JKPress. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Returns the initialized JK_Http Object
 *
 * @since 2.7.0
 * @access private
 *
 * @return JK_Http HTTP Transport object.
 */
function _jk_http_get_object() {
	static $http = null;
 ...
 * URL. The URL, and every URL it redirects to, are validated with jk_http_validate_url()
 * to avoid Server Side Request Forgery attacks (SSRF).
 *
 * @since 3.6.0
 *
 * @see jk_remote_request() For more information on the response array format.
 * @see JK_Http::request() For default arguments information.
 * @see jk_http_validate_url() For more information about how the URL is validated.
 *
 * @link https://owasp.org/www-community/attacks/Server_Side_Request_Forgery
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See JK_Http::request() for information on accepted arguments.
 * @return array|JK_Error The response or JK_Error on failure.
 *                        See JK_Http::request() for information on return value.
 */
function jk_safe_remote_request( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _jk_http_get_object();
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_safe_remote_get( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _jk_http_get_object();
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_safe_remote_post( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _jk_http_get_object();
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_safe_remote_head( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _jk_http_get_object();
 ...
 *
 *  - Default 'GET'  for jk_remote_get()
 *  - Default 'POST' for jk_remote_post()
 *  - Default 'HEAD' for jk_remote_head()
 *
 * @since 2.7.0
 *
 * @see JK_Http::request() For information on default arguments.
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See JK_Http::request() for information on accepted arguments.
 * @return array|JK_Error The response array or a JK_Error on failure.
 *                        See JK_Http::request() for information on return value.
 */
function jk_remote_request( $url, $args = array() ) {
	$http = _jk_http_get_object();
	return $http->request( $url, $args );
}
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_remote_get( $url, $args = array() ) {
	$http = _jk_http_get_object();
	return $http->get( $url, $args );
}
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_remote_post( $url, $args = array() ) {
	$http = _jk_http_get_object();
	return $http->post( $url, $args );
}
 ...
 *                        See JK_Http::request() for information on return value.
 */
function jk_remote_head( $url, $args = array() ) {
	$http = _jk_http_get_object();
	return $http->head( $url, $args );
}
 ...
 *
 * @param array|JK_Error $response HTTP response.
 ...
 */
function jk_remote_retrieve_headers( $response ) {
	if ( is_jk_error( $response ) || ! isset( $response['headers'] ) ) {
		return array();
	}
 ...
 */
function jk_remote_retrieve_header( $response, $header ) {
	if ( is_jk_error( $response ) || ! isset( $response['headers'] ) ) {
		return '';
	}
 ...
 */
function jk_remote_retrieve_response_code( $response ) {
	if ( is_jk_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function jk_remote_retrieve_response_message( $response ) {
	if ( is_jk_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function jk_remote_retrieve_body( $response ) {
	if ( is_jk_error( $response ) || ! isset( $response['body'] ) ) {
		return '';
	}
 ...
 *
 * @param array|JK_Error $response HTTP response.
 * @return JK_Http_Cookie[] An array of `JK_Http_Cookie` objects from the response.
 *                          Empty array if there are none, or the response is a JK_Error.
 */
function jk_remote_retrieve_cookies( $response ) {
	if ( is_jk_error( $response ) || empty( $response['cookies'] ) ) {
		return array();
	}
 ...
 * @return JK_Http_Cookie|string The `JK_Http_Cookie` object, or empty string
 *                               if the cookie is not present in the response.
 */
function jk_remote_retrieve_cookie( $response, $name ) {
	$cookies = jk_remote_retrieve_cookies( $response );
 ...
 *                if the cookie is not present in the response.
 */
function jk_remote_retrieve_cookie_value( $response, $name ) {
	$cookie = jk_remote_retrieve_cookie( $response, $name );

	if ( ! ( $cookie instanceof JK_Http_Cookie ) ) {
		return '';
	}
 ...
 * @since 3.2.0
 *
 * @param array  $capabilities Array of capabilities to test or a jk_remote_request() $args array.
 * @param string $url          Optional. If given, will check if the URL requires SSL and adds
 *                             that requirement to the capabilities array.
 *
 * @return bool
 */
function jk_http_supports( $capabilities = array(), $url = null ) {
	$capabilities = jk_parse_args( $capabilities );

	$count = count( $capabilities );

	// If we have a numeric $capabilities array, spoof a jk_remote_request() associative $args array.
 ...
 */
function jk_http_validate_url( $url ) {
	if ( ! is_string( $url ) || '' === $url || is_numeric( $url ) ) {
		return false;
	}

	$original_url = $url;
	$url          = jk_kses_bad_protocol( $url, array( 'http', 'https' ) );
...
function allowed_http_request_hosts( $is_external, $host ) {
	if ( ! $is_external && jk_validate_redirect( 'http://' . $host ) ) {
		$is_external = true;
	}
	return $is_external;
}
...
function ms_allowed_http_request_hosts( $is_external, $host ) {
	...
	$queried[ $host ] = (bool) $jkdb->get_var( $jkdb->prepare( "SELECT domain FROM $jkdb->blogs WHERE domain = %s LIMIT 1", $host ) );
	return $queried[ $host ];
}
 ...
 */
function jk_parse_url( $url, $component = -1 ) {
	$to_unset = array();
	$url      = (string) $url;
...
function _get_component_from_parsed_url_array( $url_parts, $component = -1 ) {
	...
	$key = _jk_translate_php_url_constant_to_key( $component );
 ...
 */
function _jk_translate_php_url_constant_to_key( $constant ) {
	$translation = array(
