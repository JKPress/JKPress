...
class JK_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to JK_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 *
	 * @see JK_Ajax_Response::add()
	 *
	 ...
	 *     @type int|JK_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `JK_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|JK_Error $data         The response content/message. Also accepts a JK_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_jk_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response  .= "<jk_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . ']]></jk_error>';
				...
				$response .= "<jk_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><jk_ajax>";
