 ...
 */
function jk_normalize_remote_block_pattern( $pattern ) {
 ...
 *
 * @param JK_Screen $deprecated Unused. Formerly the screen that the current request was triggered from.
 */
function _load_remote_block_patterns( $deprecated = null ) {
	...
	if ( $supports_core_patterns && $should_load_remote ) {
		$request         = new JK_REST_Request( 'GET', '/jk/v2/pattern-directory/patterns' );
		...
		foreach ( $patterns as $pattern ) {
			...
			$normalized_pattern = jk_normalize_remote_block_pattern( $pattern );
...
function _load_remote_featured_patterns() {
	...
	/** This filter is documented in jk-includes/block-patterns.php */
	$should_load_remote = apply_filters( 'should_load_remote_block_patterns', true );
...
function _register_remote_theme_patterns() {
	...
	$pattern_settings = jk_get_theme_directory_pattern_slugs();
	...
	$patterns          = $response->get_data();
	$patterns_registry = JK_Block_Patterns_Registry::get_instance();
	foreach ( $patterns as $pattern ) {
 ...
 * @since 6.4.0 Uses the `JK_Theme::get_block_patterns` method.
 * @access private
 */
function _register_theme_block_patterns() {
	 ...
	 */
	if ( empty( jk_get_active_and_valid_themes() ) ) {
		return;
	}
	...
	foreach ( $themes as $theme ) {
		...
		foreach ( $patterns as $file => $pattern_data ) {
			...
			// phpcs:ignore JKPress.JK.I18n.NonSingularStringLiteralText,JKPress.JK.I18n.NonSingularStringLiteralDomain,JKPress.JK.I18n.LowLevelTranslationFunction
