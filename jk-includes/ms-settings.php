 ...
 *
 * Allows for some configuration in jk-config.php (see ms-default-constants.php)
 *
 * @package JKPress
...
/** JK_Network class */
require_once ABSPATH . JKINC . '/class-jk-network.php';

/** JK_Site class */
require_once ABSPATH . JKINC . '/class-jk-site.php';

/** Multisite loader */
require_once ABSPATH . JKINC . '/ms-load.php';

/** Default Multisite constants */
require_once ABSPATH . JKINC . '/ms-default-constants.php';

if ( defined( 'SUNRISE' ) ) {
	include_once JK_CONTENT_DIR . '/sunrise.php';
}
...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/jk-admin/.*#', '$1/', $path );
	}
	...
	jk_load_core_site_options( $site_id );
}

$jkdb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php.
$jkdb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix       = $jkdb->get_blog_prefix();
$_jk_switched_stack = array();
$switched           = false;

// Need to init cache again after blog_id is set.
jk_start_object_cache();

if ( ! $current_site instanceof JK_Network ) {
	$current_site = new JK_Network( $current_site );
}

if ( ! $current_blog instanceof JK_Site ) {
	$current_blog = new JK_Site( $current_blog );
}
