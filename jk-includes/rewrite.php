...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$jk_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$jk_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $jk_rewrite;
	$jk_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$jk_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see JK_Rewrite::remove_permastruct()
 * @global JK_Rewrite $jk_rewrite JKPress rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$jk_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $callback ) {
	...
	if ( ! in_array( $feedname, $jk_rewrite->feeds, true ) ) {
		$jk_rewrite->feeds[] = $feedname;
	}
...
function flush_rewrite_rules( $hard = true ) {
	...
	if ( is_callable( array( $jk_rewrite, 'flush_rules' ) ) ) {
		$jk_rewrite->flush_rules( $hard );
	}
}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $jk_rewrite;
	$jk_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _jk_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to JK_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, jk_unique_post_slug() prevents the creation of post slugs
 ...
 *                          JK::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function jk_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof JK_Post ) {
	...
	if ( ! $jk_rewrite->using_index_permalinks() ) {
		$url = str_replace( $jk_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			if ( $jk_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $jk->public_query_vars, true ) ) {
					$query[ $key ] = $value;
			...
			$query = jk_resolve_numeric_slug_conflicts( $query );

			// Do the query.
			$query = new JK_Query( $query );
