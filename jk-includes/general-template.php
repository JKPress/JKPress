 ...
 * form into the sidebar and also by the search widget in JKPress.
 *
 ...
 */
function jk_loginout( $redirect = '', $display = true ) {
	if ( ! is_user_logged_in() ) {
		$link = '<a href="' . esc_url( jk_login_url( $redirect ) ) . '">' . __( 'Log in' ) . '</a>';
	} else {
		$link = '<a href="' . esc_url( jk_logout_url( $redirect ) ) . '">' . __( 'Log out' ) . '</a>';
	}
	...
	if ( $display ) {
		...
		/** This filter is documented in jk-includes/general-template.php */
 ...
 * @return string The logout URL. Note: HTML-encoded via esc_html() in jk_nonce_url().
 */
function jk_logout_url( $redirect = '' ) {
	$args = array();
	if ( ! empty( $redirect ) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg( $args, site_url( 'jk-login.php?action=logout', 'login' ) );
	$logout_url = jk_nonce_url( $logout_url, 'log-out' );
 ...
 */
function jk_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'jk-login.php', 'login' );
...
function jk_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'jk-login.php?action=register', 'login' ) );
}

/**
 * Provides a simple login form for use anywhere within JKPress.
 *
 ...
 *     @type string $id_submit         ID attribute value for the submit button. Default 'jk-submit'.
 ...
 */
function jk_login_form( $args = array() ) {
	$defaults = array(
		'echo'              => true,
	 ...
	 */
	$args = jk_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form =
		sprintf(
			...
			esc_url( site_url( 'jk-login.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
		...
		sprintf(
			'<p class="login-submit">
				<input type="submit" name="jk-submit" id="%1$s" class="button button-primary" value="%2$s" />
 ...
 */
function jk_lostpassword_url( $redirect = '' ) {
	$args = array(
		'action' => 'lostpassword',
	);
	...
	if ( is_multisite() ) {
		...
		$jk_login_path = $blog_details->path . 'jk-login.php';
	} else {
		$jk_login_path = 'jk-login.php';
	}

	$lostpassword_url = add_query_arg( $args, network_site_url( $jk_login_path, 'login' ) );
 ...
 *                     if `$display` is false.
 */
function jk_register( $before = '<li>', $after = '</li>', $display = true ) {
	if ( ! is_user_logged_in() ) {
		if ( get_option( 'users_can_register' ) ) {
			$link = $before . '<a href="' . esc_url( jk_registration_url() ) . '">' . __( 'Register' ) . '</a>' . $after;
		} else {
			$link = '';
		}
...
/**
 * Theme container function for the 'jk_meta' action.
 *
 * The {@see 'jk_meta'} action can have several purposes, depending on how you use it,
 * but one purpose might have been to allow for theme switching.
 *
 * @since 1.5.0
 *
 * @link https://core.trac.wordpress.org/ticket/1458 Explanation of 'jk_meta' action.
 */
function jk_meta() {
	/**
 ...
 * - 'wpurl' - The JKPress address (URL) (set in Settings > General)
 * - 'url' - The Site address (URL) (set in Settings > General)
 * - 'admin_email' - Admin email (set in Settings > General)
 * - 'charset' - The "Encoding for pages and feeds"  (set in Settings > Reading)
 * - 'version' - The current JKPress version
...
function get_site_icon_url( $size = 512, $url = '', $blog_id = 0 ) {
	...
	if ( $site_icon_id ) {
		...
		$url = jk_get_attachment_image_url( $site_icon_id, $size_data );
	}
...
function has_custom_logo( $blog_id = 0 ) {
	...
	$is_image       = ( $custom_logo_id ) ? jk_attachment_is_image( $custom_logo_id ) : false;
...
function get_custom_logo( $blog_id = 0 ) {
	...
	if ( has_custom_logo() ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			 ...
			 * to the attributes used by jk_get_attachment_image().
			 */
			$image_alt = get_post_meta( $custom_logo_id, '_jk_attachment_image_alt', true );
			if ( empty( $image_alt ) ) {
		 ...
		 * because jk_get_attachment_image() already adds the alt attribute.
		 */
		$image = jk_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr );
 ...
 */
function jk_get_document_title() {

	/**
	 * Filters the document title before it is generated.
	 *
	 * Passing a non-empty value will short-circuit jk_get_document_title(),
 ...
 *
 * @since 4.1.0
 * @since 4.4.0 Improved title output replaced `jk_title()`.
 * @access private
 */
function _jk_render_title_tag() {
	if ( ! current_theme_supports( 'title-tag' ) ) {
		return;
	}

	echo '<title>' . jk_get_document_title() . '</title>' . "\n";
}
 ...
 */
function jk_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	global $jk_locale;
	...
	$t_sep = '%JK_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
	...
	if ( is_archive() && ! empty( $m ) ) {
		...
		$title    = $my_year .
			( $my_month ? $t_sep . $jk_locale->get_month( $my_month ) : '' ) .
			( $my_day ? $t_sep . $my_day : '' );
	}
	...
	if ( is_archive() && ! empty( $year ) ) {
		$title = $year;
		if ( ! empty( $monthnum ) ) {
			$title .= $t_sep . $jk_locale->get_month( $monthnum );
		}
		if ( ! empty( $day ) ) {
	 ...
	 */
	$title_array = apply_filters( 'jk_title_parts', explode( $t_sep, $title ) );
	 ...
	 */
	$title = apply_filters( 'jk_title', $title, $sep, $seplocation );
...
function single_post_title( $prefix = '', $display = true ) {
	 ...
	 * @param JK_Post $_post       The current post.
	 */
...
function single_month_title( $prefix = '', $display = true ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		$my_year  = $year;
		$my_month = $jk_locale->get_month( $monthnum );
	} elseif ( ! empty( $m ) ) {
		$my_year  = substr( $m, 0, 4 );
		$my_month = $jk_locale->get_month( substr( $m, 4, 2 ) );
	}
...
function get_the_post_type_description() {
	 ...
	 * @param JK_Post_Type $post_type_obj The post type object.
	 */
 ...
 */
function jk_get_archives( $args = '' ) {
	global $jkdb, $jk_locale;
	...
	$sql_where = $jkdb->prepare( "WHERE post_type = %s AND post_status = 'publish'", $parsed_args['post_type'] );
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $jkdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "jk_get_archives:$key:$last_changed";
		$results = jk_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
			$results = $jkdb->get_results( $query );
			jk_cache_set( $key, $results, 'post-queries' );
		}
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), $jk_locale->get_month( $result->month ), $result->year );
		...
		$query   = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $jkdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "jk_get_archives:$key:$last_changed";
		$results = jk_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $jkdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "jk_get_archives:$key:$last_changed";
		$results = jk_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$week    = _jk_mysql_week( '`post_date`' );
		$query   = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$jkdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` $order $limit";
		$key     = md5( $query );
		$key     = "jk_get_archives:$key:$last_changed";
		$results = jk_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$query   = "SELECT * FROM $jkdb->posts $join $where ORDER BY $orderby $limit";
		$key     = md5( $query );
		$key     = "jk_get_archives:$key:$last_changed";
		$results = jk_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
...
function get_calendar( $initial = true, $display = true ) {
	global $jkdb, $m, $monthnum, $year, $jk_locale, $posts;

	$key   = md5( $m . $monthnum . $year );
	$cache = jk_cache_get( 'get_calendar', 'calendar' );
	...
	if ( ! $posts ) {
		$gotsome = $jkdb->get_var( "SELECT 1 as test FROM $jkdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1" );
		if ( ! $gotsome ) {
			$cache[ $key ] = '';
			jk_cache_set( 'get_calendar', $cache, 'calendar' );
			return;
		}
	}
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$d         = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = $jkdb->get_var( "SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')" );
	} elseif ( ! empty( $m ) ) {
		$thisyear = (int) substr( $m, 0, 4 );
		if ( strlen( $m ) < 6 ) {
			$thismonth = '01';
	...
	$calendar_output  = '<table id="jk-calendar" class="jk-calendar-table">
	<caption>' . sprintf(
		$calendar_caption,
		$jk_locale->get_month( $thismonth ),
		gmdate( 'Y', $unixmonth )
	) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
		$myweek[] = $jk_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
	}

	foreach ( $myweek as $wd ) {
		$day_name         = $initial ? $jk_locale->get_weekday_initial( $wd ) : $jk_locale->get_weekday_abbrev( $wd );
		$wd               = esc_attr( $wd );
	...
	// Get days with posts.
	$dayswithposts = $jkdb->get_results(
		"SELECT DISTINCT DAYOFMONTH(post_date)
		FROM $jkdb->posts WHERE post_date >= '{$thisyear}-{$thismonth}-01 00:00:00'
		AND post_type = 'post' AND post_status = 'publish'
	...
	$calendar_output .= '<nav aria-label="' . __( 'Previous and next months' ) . '" class="jk-calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . '<span class="jk-calendar-nav-prev"><a href="' . get_month_link( $previous->year, $previous->month ) . '">&laquo; ' .
			$jk_locale->get_month_abbrev( $jk_locale->get_month( $previous->month ) ) .
		'</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="jk-calendar-nav-prev">&nbsp;</span>';
	}

	$calendar_output .= "\n\t\t" . '<span class="pad">&nbsp;</span>';

	if ( $next ) {
		$calendar_output .= "\n\t\t" . '<span class="jk-calendar-nav-next"><a href="' . get_month_link( $next->year, $next->month ) . '">' .
			$jk_locale->get_month_abbrev( $jk_locale->get_month( $next->month ) ) .
		' &raquo;</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="jk-calendar-nav-next">&nbsp;</span>';
	}
...
function delete_get_calendar_cache() {
	jk_cache_delete( 'get_calendar', 'calendar' );
}
 ...
 *
 * @param string      $format Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|JK_Post $post   Optional. Post ID or JK_Post object. Default current post.
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param JK_Post|null     $post     JK_Post object or null if no post is found.
	 */
 ...
 *                            was written. Accepts 'G', 'U', or PHP date format.
 *                            Defaults to the 'time_format' option.
 * @param int|JK_Post $post   Post ID or post object. Default is global `$post` object.
...
function get_post_time( $format = 'U', $gmt = false, $post = null, $translate = false ) {
	...
	if ( 'U' === $format || 'G' === $format ) {
		...
		$time = jk_date( $format, $datetime->getTimestamp(), $gmt ? new DateTimeZone( 'UTC' ) : null );
	} else {
		if ( $gmt ) {
 ...
 *
 * The object will be set to the timezone from JKPress settings.
 *
 ...
 * @since 5.3.0
 *
 * @param int|JK_Post $post   Optional. Post ID or post object. Default is global `$post` object.
...
function get_post_datetime( $post = null, $field = 'date', $source = 'local' ) {
	...
	return $datetime->setTimezone( $jk_timezone );
}
...
function the_weekday() {
	...
	$the_weekday = $jk_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function the_weekday_date( $before = '', $after = '' ) {
	global $jk_locale, $currentday, $previousweekday;
	...
	if ( $currentday !== $previousweekday ) {
		...
		$the_weekday_date .= $jk_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
/**
 * Fires the jk_body_open action.
 *
 * See {@see 'jk_body_open'}.
 *
 * @since 5.2.0
 */
function jk_body_open() {
	/**
	 * Triggered after the opening body tag.
	 *
	 * @since 5.2.0
	 */
	do_action( 'jk_body_open' );
}
 ...
 * @since 5.7.0
 */
function jk_strict_cross_origin_referrer() {
	?>
 ...
 * @since 4.6.0
 */
function jk_resource_hints() {
	$hints = array(
		'dns-prefetch' => jk_dependencies_unique_hosts(),
		'preconnect'   => array(),
		'prefetch'     => array(),
		'prerender'    => array(),
	);
	...
	foreach ( $hints as $relation_type => $urls ) {
		 ...
		 */
		$urls = apply_filters( 'jk_resource_hints', $urls, $relation_type );
...
function jk_preload_resources() {
	 ...
	 * }
	 */
	$preload_resources = apply_filters( 'jk_preload_resources', array() );
 ...
 * @global JK_Styles  $jk_styles  The JK_Styles object for printing styles.
 *
 * @return string[] A list of unique hosts of enqueued scripts and styles.
 */
function jk_dependencies_unique_hosts() {
	global $jk_scripts, $jk_styles;

	$unique_hosts = array();

	foreach ( array( $jk_scripts, $jk_styles ) as $dependencies ) {
		if ( $dependencies instanceof JK_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				...
				$parsed     = jk_parse_url( $dependency->src );
 ...
 * @since 2.0.0
 *
 * @global bool $jk_rich_edit Whether the user can access the visual editor.
...
function user_can_richedit() {
	global $jk_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_IE, $is_edge;

	if ( ! isset( $jk_rich_edit ) ) {
		$jk_rich_edit = false;

		if ( 'true' === get_user_option( 'rich_editing' ) || ! is_user_logged_in() ) { // Default to 'true' for logged out users.
			if ( $is_safari ) {
				$jk_rich_edit = ! jk_is_mobile() || ( preg_match( '!AppleWebKit/(\d+)!', $_SERVER['HTTP_USER_AGENT'], $match ) && (int) $match[1] >= 534 );
			} elseif ( $is_IE ) {
				$jk_rich_edit = str_contains( $_SERVER['HTTP_USER_AGENT'], 'Trident/7.0;' );
			} elseif ( $is_gecko || $is_chrome || $is_edge || ( $is_opera && ! jk_is_mobile() ) ) {
				$jk_rich_edit = true;
			}
		}
	}
	 ...
	 * @since 2.1.0
	 *
	 * @param bool $jk_rich_edit Whether the user can access the visual editor.
	 */
	return apply_filters( 'user_can_richedit', $jk_rich_edit );
}
 ...
 */
function jk_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // Defaults.
	if ( jk_get_current_user() ) { // Look for cookie.
	 ...
	 */
	return apply_filters( 'jk_default_editor', $r );
}
 ...
 * running jk_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 ...
 */
function jk_editor( $content, $editor_id, $settings = array() ) {
 ...
 * See jk.editor.initialize() in jk-admin/js/editor.js for initialization options.
 *
 * @uses _WP_Editors
 * @since 4.8.0
 */
function jk_enqueue_editor() {
	if ( ! class_exists( '_WP_Editors', false ) ) {
		require ABSPATH . JKINC . '/class-jk-editor.php';
	}
 ...
 * @since 4.9.0
 *
 * @see jk_enqueue_editor()
 * @see jk_get_code_editor_settings();
 * @see _WP_Editors::parse_settings()
 *
 * @param array $args {
 *     Args.
 ...
 *     @type JK_Theme $theme      Theme being edited when on the theme file editor.
 ...
 */
function jk_enqueue_code_editor( $args ) {
	if ( is_user_logged_in() && 'false' === jk_get_current_user()->syntax_highlighting ) {
		return false;
	}

	$settings = jk_get_code_editor_settings( $args );

	if ( empty( $settings ) || empty( $settings['codemirror'] ) ) {
		return false;
	}

	jk_enqueue_script( 'code-editor' );
	jk_enqueue_style( 'code-editor' );
	...
	if ( isset( $settings['codemirror']['mode'] ) ) {
		...
		if ( ! empty( $settings['codemirror']['lint'] ) ) {
			switch ( $mode['name'] ) {
				...
				case 'text/x-php':
					jk_enqueue_script( 'htmlhint' );
					jk_enqueue_script( 'csslint' );
					jk_enqueue_script( 'jshint' );
					if ( ! current_user_can( 'unfiltered_html' ) ) {
						jk_enqueue_script( 'htmlhint-kses' );
					}
					break;
				case 'javascript':
				...
				case 'application/typescript':
					jk_enqueue_script( 'jshint' );
					jk_enqueue_script( 'jsonlint' );
					break;
			}
		}
	}

	jk_add_inline_script( 'code-editor', sprintf( 'jQuery.extend( jk.codeEditor.defaultSettings, %s );', jk_json_encode( $settings ) ) );
	 ...
	 */
	do_action( 'jk_enqueue_code_editor', $settings );
 ...
 */
function jk_get_code_editor_settings( $args ) {
	$settings = array(
		'codemirror' => array(
			'indentUnit'       => 4,
			'indentWithTabs'   => true,
		...
		'jshint'     => array(
			// The following are copied from <https://github.com/JKPress/wordpress-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
	...
	if ( isset( $args['type'] ) ) {
		...
		// Supply any types that are not matched by jk_get_mime_types().
		if ( empty( $type ) ) {
			switch ( $extension ) {
				case 'conf':
					$type = 'text/nginx';
	...
	if ( in_array( $type, array( 'text/css', 'text/x-scss', 'text/x-less', 'text/x-sass' ), true ) ) {
		...
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			$settings['htmlhint']['kses'] = jk_kses_allowed_html( 'post' );
		}
	...
	foreach ( jk_array_slice_assoc( $args, array( 'codemirror', 'csslint', 'jshint', 'htmlhint' ) ) as $key => $value ) {
	 ...
	 * }
	 */
	return apply_filters( 'jk_code_editor_settings', $settings, $args );
}

/**
 * Retrieves the contents of the search JKPress query variable.
 *
 ...
 *     @type int    $total              The total amount of pages. Default is the value JK_Query's
 *                                      `max_num_pages` or 1.
...
function paginate_links( $args = '' ) {
	...
	$total   = isset( $jk_query->max_num_pages ) ? $jk_query->max_num_pages : 1;
	...
	$format  = $jk_rewrite->using_index_permalinks() && ! strpos( $pagenum_link, 'index.php' ) ? 'index.php/' : '';
	$format .= $jk_rewrite->using_permalinks() ? user_trailingslashit( $jk_rewrite->pagination_base . '/%#%', 'paged' ) : '?paged=%#%';
	...
	if ( isset( $url_parts[1] ) ) {
		...
		jk_parse_str( $format_query, $format_args );

		// Find the query args of the requested URL.
		jk_parse_str( $url_parts[1], $url_query_args );
 ...
 *
 *     jk_admin_css_color( 'classic', __( 'Classic' ), admin_url( "css/colors-classic.css" ), array(
 ...
 * }
 */
function jk_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_jk_admin_css_colors;

	if ( ! isset( $_jk_admin_css_colors ) ) {
		$_jk_admin_css_colors = array();
	}

	$_jk_admin_css_colors[ $key ] = (object) array(
		'name'        => $name,
		'url'         => $url,
		'colors'      => $colors,
		'icon_colors' => $icons,
...
/**
 * Displays the URL of a JKPress admin CSS file.
 *
 * @see JK_Styles::_css_href() and its {@see 'style_loader_src'} filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to jk-admin/ without its ".css" extension.
 * @return string
 */
function jk_admin_css_uri( $file = 'jk-admin' ) {
	...
	/**
	 * Filters the URI of a JKPress admin CSS file.
	 *
	 * @since 2.3.0
	 *
	 ...
	 */
	return apply_filters( 'jk_admin_css_uri', $_file, $file );
}
 ...
 * {@see 'jk_print_styles'} action has *not* yet been called, the CSS file will be
 * enqueued. If the {@see 'jk_print_styles'} action has been called, the CSS link will
 * be printed. Printing may be forced by passing true as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with JKPress 2.3 calling method: If the $file
 * (first) parameter does not correspond to a registered CSS file, we assume
 * $file is a file relative to jk-admin/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 *                           to jk-admin/. Defaults to 'jk-admin'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function jk_admin_css( $file = 'jk-admin', $force_echo = false ) {
	// For backward compatibility.
	$handle = str_starts_with( $file, 'css/' ) ? substr( $file, 4 ) : $file;

	if ( jk_styles()->query( $handle ) ) {
		if ( $force_echo || did_action( 'jk_print_styles' ) ) {
			// We already printed the style queue. Print this one immediately.
			jk_print_styles( $handle );
		} else {
			// Add to style queue.
			jk_enqueue_style( $handle );
		}
		return;
	}
	 ...
	 *                                relative to jk-admin/. Defaults to 'jk-admin'.
	 */
	echo apply_filters( 'jk_admin_css', $stylesheet_link, $file );

	if ( function_exists( 'is_rtl' ) && is_rtl() ) {
		$rtl_stylesheet_link = sprintf(
			"<link rel='stylesheet' href='%s' type='text/css' />\n",
			esc_url( jk_admin_css_uri( "$file-rtl" ) )
		);

		/** This filter is documented in jk-includes/general-template.php */
		echo apply_filters( 'jk_admin_css', $rtl_stylesheet_link, "$file-rtl" );
	}
}
...
function add_thickbox() {
	jk_enqueue_script( 'thickbox' );
	jk_enqueue_style( 'thickbox' );
...
/**
 * Displays the XHTML generator that is generated on the jk_head hook.
 *
 * See {@see 'jk_head'}.
 *
 * @since 2.5.0
 */
function jk_generator() {
	/**
	 ...
	 */
	the_generator( apply_filters( 'jk_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="JKPress ' . esc_attr( get_bloginfo( 'version' ) ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="JKPress ' . esc_attr( get_bloginfo( 'version' ) ) . '" />';
			break;
		case 'atom':
			$gen = '<generator uri="https://wordpress.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '">JKPress</generator>';
			break;
		case 'rss2':
		...
		case 'rdf':
			...
			break;
		case 'comment':
			$gen = '<!-- generator="JKPress/' . esc_attr( get_bloginfo( 'version' ) ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="JKPress/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
	 ...
	 * @since 2.5.0
	 *
	 * @param string $gen  The HTML markup output to jk_head().
 ...
 * @return string HTML attribute or empty string.
 */
function jk_readonly( $readonly_value, $current = true, $display = true ) {
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $display, $type ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
 ...
 */
function jk_required_field_indicator() {
	 ...
	 */
	return apply_filters( 'jk_required_field_indicator', $indicator );
}
 ...
 */
function jk_required_field_message() {
	$message = sprintf(
		'<span class="required-field-message">%s</span>',
		/* translators: %s: Asterisk symbol (*). */
		sprintf( __( 'Required fields are marked %s' ), jk_required_field_indicator() )
	);
	 ...
	 */
	return apply_filters( 'jk_required_field_message', $message );
}
 ...
 */
function jk_heartbeat_settings( $settings ) {
	if ( ! is_admin() ) {
		$settings['ajaxurl'] = admin_url( 'admin-ajax.php', 'relative' );
	}

	if ( is_user_logged_in() ) {
		$settings['nonce'] = jk_create_nonce( 'heartbeat-nonce' );
	}
