 ...
 * }
 */
function jk_style_engine_get_styles( $block_styles, $options = array() ) {
	...
	$parsed_styles = JK_Style_Engine::parse_block_styles( $block_styles, $options );

	// Output.
	$styles_output = array();

	if ( ! empty( $parsed_styles['declarations'] ) ) {
		$styles_output['css']          = JK_Style_Engine::compile_css( $parsed_styles['declarations'], $options['selector'] );
		$styles_output['declarations'] = $parsed_styles['declarations'];
		if ( ! empty( $options['context'] ) ) {
			JK_Style_Engine::store_css_rule( $options['context'], $options['selector'], $parsed_styles['declarations'] );
		}
	}
 ...
 *     );
 *
 *     $css = jk_style_engine_get_stylesheet_from_css_rules( $css_rules );
 *
 * Returns:
 *
 ...
 */
function jk_style_engine_get_stylesheet_from_css_rules( $css_rules, $options = array() ) {
	if ( empty( $css_rules ) ) {
		return '';
	}
	...
	foreach ( $css_rules as $css_rule ) {
		...
		if ( ! empty( $options['context'] ) ) {
			JK_Style_Engine::store_css_rule( $options['context'], $css_rule['selector'], $css_rule['declarations'], $rules_group );
		}

		$css_rule_objects[] = new JK_Style_Engine_CSS_Rule( $css_rule['selector'], $css_rule['declarations'], $rules_group );
	}
	...
	return JK_Style_Engine::compile_stylesheet_from_css_rules( $css_rule_objects, $options );
}
 ...
 */
function jk_style_engine_get_stylesheet_from_context( $context, $options = array() ) {
	return JK_Style_Engine::compile_stylesheet_from_css_rules( JK_Style_Engine::get_store( $context )->get_all_rules(), $options );
}
