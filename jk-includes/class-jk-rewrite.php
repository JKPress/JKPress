 ...
 *
 * The JKPress Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * JKPress Query class.
 *
 * The Rewrite along with JK class function as a front controller for JKPress.
...
class JK_Rewrite {
	 ...
	 * `JK_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see JK_Rewrite::init()
	 * @see JK_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to JKPress' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for JKPress into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see JK_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to JKPress 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells JK::parse_request() to check if a URL matching the page
	 ...
	 * `JK_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in JK_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see JK_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see JK_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	public function page_uri_index() {
		...
		$pages = $jkdb->get_results( "SELECT ID, post_name, post_parent FROM $jkdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'" );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name.
			$uri         = get_page_uri( $id );
			$attachments = $jkdb->get_results( $jkdb->prepare( "SELECT ID, post_name, post_parent FROM $jkdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ) );
	 ...
	 * @since 1.5.0
	 *
	 * @see JK_Rewrite::$rewritecode
	 * @see JK_Rewrite::$rewritereplace
	 * @see JK_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main JK_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of JK_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see JK_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current JK_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*jk-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*jk-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*jk-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*jk-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*jk-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param JK_Rewrite $jk_rewrite Current JK_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and JK_Rewrite::rewrite_rules() is that
	 ...
	 * @return string[] Array of rewrite rules keyed by their regex pattern.
	 */
	public function jk_rewrite_rules() {
	 ...
	 *
	 * If the {@see 'jk_loaded'} action has not occurred yet, will postpone saving to the database.
	 *
	 * @since 6.4.0
	 */
	private function refresh_rewrite_rules() {
		...
		if ( ! did_action( 'jk_loaded' ) ) {
			 ...
			 */
			add_action( 'jk_loaded', array( $this, 'flush_rules' ) );
		} else {
	 ...
	 *
	 * Will add the non_jk_rules property rules to the .htaccess file before
	 * the JKPress rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to JK's index.php (and thus shouldn't be handled by JK at all).
		foreach ( (array) $this->non_jk_rules as $match => $query ) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="JKPress: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the JK_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by JK_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to JK_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how JK_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `JK_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls JK_Rewrite::jk_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * JK_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls JK_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls JK_Rewrite::init() after
