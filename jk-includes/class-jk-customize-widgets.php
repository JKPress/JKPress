<?php
/**
 * JKPress Customize Widgets classes
 *
 * @package JKPress
...
final class JK_Customize_Widgets {
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xwp/jk-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L420-L449
		...
		add_action( 'jk_loaded', array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function get_selective_refreshable_widgets() {
		...
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			...
			foreach ( $jk_widget_factory->widgets as $jk_widget ) {
				$this->selective_refreshable_widgets[ $jk_widget->id_base ] = ! empty( $jk_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( jk_unslash( $_REQUEST['widget-id'] ) );
		}
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the JK_Customize_Setting constructor.
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = jk_get_sidebars_widgets();
		...
		// Reset global cache var used by jk_get_sidebars_widgets().
		unset( $GLOBALS['_jk_sidebars_widgets'] );
	}
	 ...
	 * @since 3.9.0
	 *
	 * @see JK_Customize_Widgets::handle_theme_switch()
	 *
	 ...
	 *
	 * Note: these actions are also fired in jk_ajax_update_widget().
	 *
	 * @since 3.9.0
	 */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after JKPress is loaded
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'jk', array( $this, 'customize_register' ) );
		}
	}
	...
	public function customize_register() {
		global $jk_registered_widgets, $jk_registered_widget_controls, $jk_registered_sidebars;

		$use_widgets_block_editor = jk_use_widgets_block_editor();

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'jk_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $jk_registered_sidebars ), array() ),
			jk_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $jk_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'jk_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title'      => $jk_registered_sidebars[ $sidebar_id ]['name'],
						'priority'   => array_search( $sidebar_id, array_keys( $jk_registered_sidebars ), true ),
						'panel'      => 'widgets',
					...
					if ( $use_widgets_block_editor ) {
						...
						$section_args['description'] = $jk_registered_sidebars[ $sidebar_id ]['description'];
					}
					...
					$section = new JK_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					if ( $use_widgets_block_editor ) {
						$control = new JK_Sidebar_Block_Editor_Control(
							$this->manager,
							$setting_id,
							array(
								'section'     => $section_id,
						...
						$control = new JK_Widget_Area_Customize_Control(
							$this->manager,
							$setting_id,
							array(
								'section'    => $section_id,
			...
			if ( ! $use_widgets_block_editor ) {
				...
				foreach ( $sidebar_widget_ids as $i => $widget_id ) {
					...
					if ( ! $is_active_sidebar || ! isset( $jk_registered_widgets[ $widget_id ] ) ) {
						continue;
					}

					$registered_widget = $jk_registered_widgets[ $widget_id ];
					$setting_id        = $this->get_setting_id( $widget_id );
					$id_base           = $jk_registered_widget_controls[ $widget_id ]['id_base'];

					$control = new JK_Widget_Form_Customize_Control(
						$this->manager,
						$setting_id,
						array(
							...
							'priority'       => $i,
							'width'          => $jk_registered_widget_controls[ $widget_id ]['width'],
							'height'         => $jk_registered_widget_controls[ $widget_id ]['height'],
	 ...
	 * @since 4.4.0
	 *
	 * @see JK_Customize_Panel::$active_callback
	 *
	 * @global array $jk_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $jk_registered_sidebars );
	}
	 ...
	 * @return array|JK_Error Array containing a widget's id_base and number components,
	 *                        or a JK_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new JK_Error( 'widget_setting_invalid_id' );
		}
	...
	public function print_styles() {
		...
		do_action( 'admin_print_styles-widgets.php' ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	...
	public function print_scripts() {
		/** This action is documented in jk-admin/admin-header.php */
		do_action( 'admin_print_scripts-widgets.php' ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	...
	public function enqueue_scripts() {
		global $jk_scripts, $jk_registered_sidebars, $jk_registered_widgets;

		jk_enqueue_style( 'customize-widgets' );
		jk_enqueue_script( 'customize-widgets' );
		...
		$registered_sidebar_count            = count( $jk_registered_sidebars );
		...
		$settings = array(
			'registeredSidebars'          => array_values( $jk_registered_sidebars ),
			'registeredWidgets'           => $jk_registered_widgets,
		...
		$jk_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _jkCustomizeWidgetsSettings = %s;', jk_json_encode( $settings ) )
		);

		/*
		 * TODO: Update 'jk-customize-widgets' to not rely so much on things in
		...
		if ( jk_use_widgets_block_editor() ) {
			$block_editor_context = new JK_Block_Editor_Context(
				array(
			...
			jk_add_inline_script(
				...
				sprintf(
					'jk.domReady( function() {
					   jk.customizeWidgets.initialize( "widgets-customizer", %s );
					} );',
			...
			jk_enqueue_script( 'jk-customize-widgets' );
			jk_enqueue_style( 'jk-customize-widgets' );
	...
	public function print_footer_scripts() {
		...
		do_action( 'admin_print_footer_scripts-widgets.php' ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores

		/** This action is documented in jk-admin/admin-footer.php */
		do_action( 'admin_print_footer_scripts' );

		/** This action is documented in jk-admin/admin-footer.php */
		do_action( 'admin_footer-widgets.php' ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	public function get_available_widgets() {
		...
		global $jk_registered_widgets, $jk_registered_widget_controls;
		require_once ABSPATH . 'jk-admin/includes/widgets.php'; // For next_widget_id_number().

		$sort = $jk_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $jk_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $jk_registered_widget_controls[ $widget['id'] ]['id_base'];
				...
				if ( $sidebar && 'jk_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = jk_list_widget_controls_dynamic_sidebar(
				array(
					0 => $args,
					1 => $widget['params'][0],
				)
			...
			$available_widget = array_merge(
				...
				array(
					...
					'width'        => $jk_registered_widget_controls[ $widget['id'] ]['width'],
					'height'       => $jk_registered_widget_controls[ $widget['id'] ]['height'],
	...
	public function customize_preview_init() {
		add_action( 'jk_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'jk_print_styles', array( $this, 'print_preview_css' ), 1 );
		add_action( 'jk_footer', array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = jk_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because jk_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * jk_convert_widget_settings()) and can set global variable `$_jk_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		jk_enqueue_script( 'customize-preview-widgets' );
	}
	 ...
	 * and at the very end of the jk_footer,
	 *
	 * @since 3.9.0
	 *
	 * @global array $jk_registered_sidebars
	...
	public function export_preview_data() {
		...
		jk_print_inline_script_tag(
			sprintf( 'var _jkWidgetCustomizerPreviewSettings = %s;', jk_json_encode( $settings ) )
		);
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return jk_hash( $serialized_instance );
	}
	...
	public function sanitize_widget_instance( $value, $id_base = null ) {
		...
		if ( isset( $value['raw_instance'] ) && $id_base && jk_use_widgets_block_editor() ) {
			$widget_object = $jk_widget_factory->get_widget_object( $id_base );
			if ( ! empty( $widget_object->widget_options['show_instance_in_rest'] ) ) {
				if ( 'block' === $id_base && ! current_user_can( 'unfiltered_html' ) ) {
					 ...
					 */
					$value['raw_instance']['content'] = jk_kses_post( $value['raw_instance']['content'] );
				}
	...
	public function sanitize_widget_js_instance( $value, $id_base = null ) {
		...
		if ( empty( $value['is_widget_customizer_js_value'] ) ) {
			...
			if ( $id_base && jk_use_widgets_block_editor() ) {
				$widget_object = $jk_widget_factory->get_widget_object( $id_base );
				if ( ! empty( $widget_object->widget_options['show_instance_in_rest'] ) ) {
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $jk_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param string $widget_id Widget ID.
	 * @return array|JK_Error Array containing the updated widget information.
	 *                        A JK_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $jk_registered_widget_updates, $jk_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new JK_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting, $parsed_id['id_base'] );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new JK_Error( 'widget_setting_unsanitized' );
			}
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new JK_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new JK_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from JK_Customize_Setting::preview() will use this value
	 ...
	 *
	 * Most code here copied from jk_ajax_save_widget().
	 *
	 * @since 3.9.0
	 *
	 * @see jk_ajax_save_widget()
	 */
	public function jk_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			jk_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			jk_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_jk_error( $updated_widget ) ) {
			jk_send_json_error( $updated_widget->get_error_code() );
		}

		$form     = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'], $id_base );

		jk_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'jk_kses_allowed_html', array( $this, 'filter_jk_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see JK_Customize_Nav_Menus::filter_jk_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes                   .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( jk_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_jk_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through jk_kses() when printed.
	 *
	 * @since 4.5.0
	 *
	 ...
	 */
	public function filter_jk_kses_allowed_data_attributes( $allowed_html ) {
