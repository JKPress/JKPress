<?php
/**
 * Interactivity API: JK_Interactivity_API class.
 *
 * @package JKPress
 ...
 * @since 6.5.0
 */
final class JK_Interactivity_API {
	/**
	...
	private static $directive_processors = array(
		'data-jk-interactive'   => 'data_jk_interactive_processor',
		'data-jk-router-region' => 'data_jk_router_region_processor',
		'data-jk-context'       => 'data_jk_context_processor',
		'data-jk-bind'          => 'data_jk_bind_processor',
		'data-jk-class'         => 'data_jk_class_processor',
		'data-jk-style'         => 'data_jk_style_processor',
		'data-jk-text'          => 'data_jk_text_processor',
		/*
		 * `data-jk-each` needs to be processed in the last place because it moves
		 * the cursor to the end of the processed items to prevent them to be
		 * processed twice.
		 */
		'data-jk-each'          => 'data_jk_each_processor',
	);
	...
	/**
	 * Flag that indicates whether the `data-jk-router-region` directive has
	 * been found in the HTML and processed.
	 *
	 * The value is saved in a private property of the JK_Interactivity_API
	 ...
	 * `data-jk-router-region` directive or not.
	 *
	 * @since 6.5.0
	 * @var bool
	 */
	...
	/**
	 * Stack of namespaces defined by `data-jk-interactive` directives, in
	...
	/**
	 * Stack of contexts defined by `data-jk-context` directives, in
	 * the order they are processed.
	 *
	 * This is only available during directive processing, otherwise it is `null`.
	 *
	 ...
	 *
	 * @deprecated 6.7.0 Script Modules registration is handled by {@see jk_default_script_modules()}.
	 *
	 * @since 6.5.0
	 */
	public function register_script_modules() {
		_deprecated_function( __METHOD__, '6.7.0', 'jk_default_script_modules' );
	}
	 ...
	 * and updates the markup accordingly.
	 *
	 * It uses the JK_Interactivity_API instance's context and namespace stacks,
	...
	private function _process_directives( string $html ) {
		$p          = new JK_Interactivity_API_Directives_Processor( $html );
		$tag_stack  = array();
		$unbalanced = false;
		...
		while ( $p->next_tag( array( 'tag_closers' => 'visit' ) ) ) {
			...
			if ( 'SVG' === $tag_name || 'MATH' === $tag_name ) {
				if ( $p->get_attribute_names_with_prefix( 'data-jk-' ) ) {
			...
			if ( $p->is_tag_closer() ) {
				...
				if ( 0 !== count( $p->get_attribute_names_with_prefix( 'data-jk-each-child' ) ) ) {
					/*
					 * If the tag has a `data-jk-each-child` directive, jump to its closer
					...
					foreach ( $p->get_attribute_names_with_prefix( 'data-jk-' ) as $attribute_name ) {
	 ...
	 * Example:
	 *
	 *     extract_prefix_and_suffix( 'data-jk-interactive' )   => array( 'data-jk-interactive', null )
	 *     extract_prefix_and_suffix( 'data-jk-bind--src' )     => array( 'data-jk-bind', 'src' )
	 *     extract_prefix_and_suffix( 'data-jk-foo--and--bar' ) => array( 'data-jk-foo', 'and--bar' )
	 *
	 * @since 6.5.0
	 *
	...
	/**
	 * Processes the `data-jk-interactive` directive.
	 *
	 ...
	 * @since 6.5.0
	 *
	 * @param JK_Interactivity_API_Directives_Processor $p    The directives processor instance.
	 * @param string                                    $mode Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_interactive_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		...
		// Tries to decode the `data-jk-interactive` attribute value.
		$attribute_value = $p->get_attribute( 'data-jk-interactive' );

		/*
		 * Pushes the newly defined namespace or the current one if the
		 * `data-jk-interactive` definition was invalid or does not contain a
		 * namespace. It does so because the function pops out the current namespace
		 * from the stack whenever it finds a `data-jk-interactive`'s closing tag,
		 * independently of whether the previous `data-jk-interactive` definition
	...
	/**
	 * Processes the `data-jk-context` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_context_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		...
		$attribute_value = $p->get_attribute( 'data-jk-context' );
		...
		if ( is_string( $namespace_value ) ) {
			 ...
			 * from the stack whenever it finds a `data-jk-context`'s closing tag.
			 */
	...
	/**
	 * Processes the `data-jk-bind` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_bind_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_bind_directives = $p->get_attribute_names_with_prefix( 'data-jk-bind--' );
	...
	/**
	 * Processes the `data-jk-class` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_class_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_class_directives = $p->get_attribute_names_with_prefix( 'data-jk-class--' );
	...
	/**
	 * Processes the `data-jk-style` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_style_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_style_attributes = $p->get_attribute_names_with_prefix( 'data-jk-style--' );
	...
	/**
	 * Processes the `data-jk-text` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_text_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$attribute_value = $p->get_attribute( 'data-jk-text' );
			$result          = $this->evaluate( $attribute_value );
	...
	private function get_router_animation_styles(): string {
		return <<<CSS
			.jk-interactivity-router-loading-bar {
				position: fixed;
				top: 0;
				left: 0;
				margin: 0;
			...
			.jk-interactivity-router-loading-bar.start-animation {
				animation: jk-interactivity-router-loading-bar-start-animation 30s cubic-bezier(0.03, 0.5, 0, 1) forwards
			}
			.jk-interactivity-router-loading-bar.finish-animation {
				animation: jk-interactivity-router-loading-bar-finish-animation 300ms ease-in
			}
			@keyframes jk-interactivity-router-loading-bar-start-animation {
				0% { transform: scaleX(0); transform-origin: 0 0; opacity: 1 }
				100% { transform: scaleX(1); transform-origin: 0 0; opacity: 1 }
			}
			@keyframes jk-interactivity-router-loading-bar-finish-animation {
				0% { opacity: 1 }
				50% { opacity: 1 }
				100% { opacity: 0 }
			}
CSS;
	 ...
	 *
	 * @since 6.5.0
	 * @deprecated 6.7.0 Use {@see JK_Interactivity_API::print_router_markup} instead.
	 */
	public function print_router_loading_and_screen_reader_markup() {
		_deprecated_function( __METHOD__, '6.7.0', 'JK_Interactivity_API::print_router_markup' );
	...
	public function print_router_markup() {
		echo <<<HTML
			<div
				class="jk-interactivity-router-loading-bar"
				data-jk-interactive="core/router"
				data-jk-class--start-animation="state.navigation.hasStarted"
				data-jk-class--finish-animation="state.navigation.hasFinished"
			></div>
HTML;
	}

	/**
	 * Processes the `data-jk-router-region` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_jk_router_region_processor( JK_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode && ! $this->has_processed_router_region ) {
			$this->has_processed_router_region = true;

			// Enqueues as an inline style.
			jk_register_style( 'jk-interactivity-router-animations', false );
			jk_add_inline_style( 'jk-interactivity-router-animations', $this->get_router_animation_styles() );
			jk_enqueue_style( 'jk-interactivity-router-animations' );

			// Adds the necessary markup to the footer.
			add_action( 'jk_footer', array( $this, 'print_router_markup' ) );
		}
	}

	/**
	 * Processes the `data-jk-each` directive.
	 *
	 ...
	 */
	private function data_jk_each_processor( JK_Interactivity_API_Directives_Processor $p, string $mode, array &$tag_stack ) {
		if ( 'enter' === $mode && 'TEMPLATE' === $p->get_tag() ) {
			$attribute_name   = $p->get_attribute_names_with_prefix( 'data-jk-each' )[0];
			...
			$template_end = 'data-jk-each: template end';
			$p->set_bookmark( $template_end );
			$p->next_tag();
			$manual_sdp = $p->get_attribute( 'data-jk-each-child' );
			...
			foreach ( $result as $item ) {
				...
				// Adds the `data-jk-each-child` to each top-level tag.
				$i = new JK_Interactivity_API_Directives_Processor( $processed_item );
				while ( $i->next_tag() ) {
					$i->set_attribute( 'data-jk-each-child', true );
