 ...
 *
 * Used internally by the JK_Embed class, but is designed to be generic.
 *
...
class JK_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://(www\.)?issuu\.com/.+/docs/.+#i'    => array( 'https://issuu.com/oembed_jk', true ),
		 ...
		 *
		 * Since JKPress 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
	 ...
	 *
	 * The provider is added just-in-time when jk_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when jk_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	 ...
	 *                           See jk_oembed_get() for accepted arguments. Default empty.
	...
	public function discover( $url ) {
		...
		// Fetch URL content.
		$request = jk_safe_remote_get( $url, $args );
		$html    = jk_remote_retrieve_body( $request );
		if ( $html ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = jk_parse_args( $args, jk_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_jk_error( $result ) && 'not-implemented' === $result->get_error_code() ) {
				continue;
			}

			return ( $result && ! is_jk_error( $result ) ) ? $result : false;
		}
	 ...
	 * @return object|false|JK_Error The result in the form of an object on success, false on failure.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		$provider_url_with_args = add_query_arg( 'format', $format, $provider_url_with_args );

		/** This filter is documented in jk-includes/class-jk-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = jk_safe_remote_get( $provider_url_with_args, $args );

		if ( 501 === jk_remote_retrieve_response_code( $response ) ) {
			return new JK_Error( 'not-implemented' );
		}
	...
	/**
	 * Converts a data object from JK_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 *
	 ...
	 * @param object $data Data object from JK_oEmbed::data2html()
