...
class JK_Embed {
	 ...
	 * an Ajax request that will call JK_Embed::cache_oembed().
	 */
...
<script type="text/javascript">
	 ...
	 *
	 * Do not use this function directly, use jk_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use jk_embed_unregister_handler() instead.
	 *
	...
	public function get_embed_handler_html( $attr, $url ) {
		$rawattr = $attr;
		$attr    = jk_parse_args( $attr, jk_embed_defaults( $url ) );
	 ...
	 * will be given to the JK_oEmbed class.
	 *
	 * @param array  $attr {
	 *     Shortcode attributes. Optional.
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		...
		// Potentially set by JK_Embed::cache_oembed().
		...
		if ( $post_id ) {
			...
			$has_kses = false !== has_filter( 'content_save_pre', 'jk_filter_post_kses' );
		...
		if ( $html ) {
			/** This filter is documented in jk-includes/class-jk-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to JK_Embed::shortcode() for potential embedding.
	 *
	 * @see JK_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = jk_replace_in_html_tags( $content, array( "\n" => '<!-- jk-line-break -->' ) );
		...
		return str_replace( '<!-- jk-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for JK_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = jk_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new JK_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			jk_cache_set( $cache_key, $oembed_post_id, $cache_group );
