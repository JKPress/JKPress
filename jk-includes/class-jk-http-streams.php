<?php
/**
 * HTTP API: JK_Http_Streams class
 *
 * @package JKPress
 * @subpackage HTTP
 * @since 4.4.0
 ...
 * @deprecated 6.4.0 Use JK_Http
 * @see JK_Http
 */
#[AllowDynamicProperties]
class JK_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see JK_Http::request() For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	 ...
	 * @return array|JK_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A JK_Error instance upon error
	 */
	public function request( $url, $args = array() ) {
		...
		JK_Http::buildCookieHeader( $parsed_args );
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str ) {
				return new JK_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );
			}

			return new JK_Error( 'http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $parsed_args['stream'] ) {
			...
			while ( ! feof( $handle ) && $keep_reading ) {
				...
				if ( ! $body_started ) {
					$response .= $block;
					if ( strpos( $response, "\r\n\r\n" ) ) {
						$processed_response = JK_Http::processResponse( $response );
						$body_started       = true;
				...
				if ( $bytes_written_to_file !== $this_block_size ) {
					...
					return new JK_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$processed_headers = JK_Http::processHeaders( $processed_response['headers'], $url );
		...
		// Handle redirects.
		$redirect_response = JK_Http::handle_redirects( $url, $parsed_args, $response );
		...
		if ( ! empty( $processed_response['body'] )
			...
			$processed_response['body'] = JK_Http::chunkTransferDecode( $processed_response['body'] );
		}

		if ( true === $parsed_args['decompress']
			&& true === JK_Http_Encoding::should_decode( $processed_headers['headers'] )
		) {
			$processed_response['body'] = JK_Http_Encoding::decompress( $processed_response['body'] );
		}
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( JK_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of JK_Http directly through its API.
 *
 * @see JK_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use JK_HTTP::request() directly
 */
class JK_HTTP_Fsockopen extends JK_Http_Streams {
