...
/**
 * JKPress XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional JKPress API for managing comments, pages, posts,
 * options, etc.
 *
 * As of JKPress 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in jk_xmlrpc_server::set_is_enabled().
 *
 * @since 1.5.0
 *
 * @see IXR_Server
...
class jk_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @var array
	...
	/**
	 * Flags that the user authentication has failed in this instance of jk_xmlrpc_server.
	 *
	 * @var bool
	 */
	...
	public function __construct() {
		$this->methods = array(
			// JKPress API.
			'jk.getUsersBlogs'                 => 'this:jk_getUsersBlogs',
			'jk.newPost'                       => 'this:jk_newPost',
			'jk.editPost'                      => 'this:jk_editPost',
			'jk.deletePost'                    => 'this:jk_deletePost',
			'jk.getPost'                       => 'this:jk_getPost',
			'jk.getPosts'                      => 'this:jk_getPosts',
			'jk.newTerm'                       => 'this:jk_newTerm',
			'jk.editTerm'                      => 'this:jk_editTerm',
			'jk.deleteTerm'                    => 'this:jk_deleteTerm',
			'jk.getTerm'                       => 'this:jk_getTerm',
			'jk.getTerms'                      => 'this:jk_getTerms',
			'jk.getTaxonomy'                   => 'this:jk_getTaxonomy',
			'jk.getTaxonomies'                 => 'this:jk_getTaxonomies',
			'jk.getUser'                       => 'this:jk_getUser',
			'jk.getUsers'                      => 'this:jk_getUsers',
			'jk.getProfile'                    => 'this:jk_getProfile',
			'jk.editProfile'                   => 'this:jk_editProfile',
			'jk.getPage'                       => 'this:jk_getPage',
			'jk.getPages'                      => 'this:jk_getPages',
			'jk.newPage'                       => 'this:jk_newPage',
			'jk.deletePage'                    => 'this:jk_deletePage',
			'jk.editPage'                      => 'this:jk_editPage',
			'jk.getPageList'                   => 'this:jk_getPageList',
			'jk.getAuthors'                    => 'this:jk_getAuthors',
			'jk.getCategories'                 => 'this:mw_getCategories',     // Alias.
			'jk.getTags'                       => 'this:jk_getTags',
			'jk.newCategory'                   => 'this:jk_newCategory',
			'jk.deleteCategory'                => 'this:jk_deleteCategory',
			'jk.suggestCategories'             => 'this:jk_suggestCategories',
			'jk.uploadFile'                    => 'this:mw_newMediaObject',    // Alias.
			'jk.deleteFile'                    => 'this:jk_deletePost',        // Alias.
			'jk.getCommentCount'               => 'this:jk_getCommentCount',
			'jk.getPostStatusList'             => 'this:jk_getPostStatusList',
			'jk.getPageStatusList'             => 'this:jk_getPageStatusList',
			'jk.getPageTemplates'              => 'this:jk_getPageTemplates',
			'jk.getOptions'                    => 'this:jk_getOptions',
			'jk.setOptions'                    => 'this:jk_setOptions',
			'jk.getComment'                    => 'this:jk_getComment',
			'jk.getComments'                   => 'this:jk_getComments',
			'jk.deleteComment'                 => 'this:jk_deleteComment',
			'jk.editComment'                   => 'this:jk_editComment',
			'jk.newComment'                    => 'this:jk_newComment',
			'jk.getCommentStatusList'          => 'this:jk_getCommentStatusList',
			'jk.getMediaItem'                  => 'this:jk_getMediaItem',
			'jk.getMediaLibrary'               => 'this:jk_getMediaLibrary',
			'jk.getPostFormats'                => 'this:jk_getPostFormats',
			'jk.getPostType'                   => 'this:jk_getPostType',
			'jk.getPostTypes'                  => 'this:jk_getPostTypes',
			'jk.getRevisions'                  => 'this:jk_getRevisions',
			'jk.restoreRevision'               => 'this:jk_restoreRevision',
	...
	/**
	 * Sets jk_xmlrpc_server::$is_enabled property.
	 *
	 * Determines whether the xmlrpc server is enabled on this JKPress install
	 ...
	 * @return JK_User|false JK_User object if authentication passed, false otherwise.
	 */
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new JK_Error( 'login_prevented' );
		} else {
		...
		if ( is_jk_error( $user ) ) {
			...
			// Flag that authentication has failed once on this jk_xmlrpc_server instance.
			 ...
			 * @param JK_Error  $user  JK_Error object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		jk_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use jk_xmlrpc_server::login()
	 * @see jk_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields( $post_id, $fields ) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset( $meta['id'] ) ) {
				...
				if ( isset( $meta['key'] ) ) {
					$meta['key'] = jk_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key ) {
						continue;
					}
					$meta['value'] = jk_unslash( $meta['value'] );
			...
			} elseif ( current_user_can( 'add_post_meta', $post_id, jk_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function jk_getUsersBlogs( $args ) {
		if ( ! $this->minimum_args( $args, 2 ) ) {
			return $this->error;
		}

		// If this isn't on JKMU then just use blogger_getUsersBlogs().
		if ( ! is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., jk.getUsersBlogs, jk.newPost, etc.
		 *
		 * @since 2.5.0
		 ...
		 * @param jk_xmlrpc_server $server The XML-RPC server instance.
		 */
		do_action( 'xmlrpc_call', 'jk.getUsersBlogs', $args, $this );
	 ...
	 *
	 * @param JK_Taxonomy $taxonomy The unprepared taxonomy data.
	...
	protected function _prepare_taxonomy( $taxonomy, $fields ) {
		 ...
		 * @param JK_Taxonomy $taxonomy  Taxonomy object.
		 * @param array       $fields    The subset of taxonomy fields to return.
		 */
	...
	/**
	 * Converts a JKPress date string to an IXR_Date object.
	 *
	...
	/**
	 * Converts a JKPress GMT date string to an IXR_Date object.
	 *
	 * @param string $date_gmt JKPress GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields, true ) ) {
			...
			$terms                = jk_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms']       = array();
	 ...
	 *
	 * @param JK_Post $media_item     The unprepared media item data.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => jk_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => jk_get_attachment_metadata( $media_item->ID ),
			'type'             => $media_item->post_mime_type,
			'alt'              => get_post_meta( $media_item->ID, '_jk_attachment_image_alt', true ),
		);
		 ...
		 * @param JK_Post $media_item     Media item object.
	 ...
	 *
	 * @param JK_Post $page The unprepared page data.
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( jk_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'jk_slug'                => $page->post_name,
			'jk_password'            => $page->post_password,
			'jk_author'              => $author->display_name,
			'jk_page_parent_id'      => $page->post_parent,
			'jk_page_parent_title'   => $parent_title,
			'jk_page_order'          => $page->menu_order,
			'jk_author_id'           => (string) $author->ID,
			'jk_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'jk_page_template'       => $page_template,
		);
	 ...
	 *
	 * @param JK_Comment $comment The unprepared comment data.
	 ...
	 *
	 * @param JK_User $user   The unprepared user object.
	 ...
	 *     @type array  $3 {
	 *         Content struct for adding a new post. See jk_insert_post() for information on
	 ...
	 */
	public function jk_newPost( $args ) {
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.newPost', $args, $this );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = jk_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for jk_newPost() and jk_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 *
	 * @see jk_insert_post()
	 *
	 * @param JK_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = jk_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = jk_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_jk_insert_post_data', $post_data, $content_struct );
		...
		$post_id = $update ? jk_update_post( $post_data, true ) : jk_insert_post( $post_data, true );
	 ...
	 */
	public function jk_editPost( $args ) {
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.editPost', $args, $this );
	 ...
	 */
	public function jk_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.deletePost', $args, $this );
	 ...
	 *  - 'enclosure'
	 */
	public function jk_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'jk.getPost' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPost', $args, $this );
	 ...
	 * @since 3.4.0
	 *
	 * @see jk_get_recent_posts()
	 * @see jk_getPost() for more on `$fields`
	 ...
	 */
	public function jk_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'jk.getPosts' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPosts', $args, $this );
		...
		$posts_list = jk_get_recent_posts( $query );
	 ...
	 */
	public function jk_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.newTerm', $args, $this );
		...
		$term = jk_insert_term( $term_data['name'], $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function jk_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.editTerm', $args, $this );
		...
		$term = jk_update_term( $term_id, $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function jk_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.deleteTerm', $args, $this );
		...
		$result = jk_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function jk_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getTerm', $args, $this );
	 ...
	 */
	public function jk_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getTerms', $args, $this );
	 ...
	 */
	public function jk_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'jk.getTaxonomy' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getTaxonomy', $args, $this );
	 ...
	 */
	public function jk_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'jk.getTaxonomies' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getTaxonomies', $args, $this );
	 ...
	 *  - 'roles'
	 */
	public function jk_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'jk.getUser' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getUser', $args, $this );
	 ...
	 *
	 * @uses get_users()
	 * @see jk_getUser() for more on $fields and return values
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function jk_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'jk.getUsers' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getUsers', $args, $this );
	 ...
	 * }
	 * @return array|IXR_Error (@see jk_getUser)
	 */
	public function jk_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'jk.getProfile' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getProfile', $args, $this );
	 ...
	 */
	public function jk_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.editProfile', $args, $this );
		...
		$result = jk_update_user( $user_data );
	 ...
	 */
	public function jk_getPage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPages( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPages', $args, $this );
	 ...
	 * @since 2.2.0
	 *
	 * @see jk_xmlrpc_server::mw_newPost()
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function jk_newPage( $args ) {
		// Items not escaped here will be escaped in jk_newPost().
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.newPage', $args, $this );
	 ...
	 */
	public function jk_deletePage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.deletePage', $args, $this );
		 ...
		 */
		do_action( 'xmlrpc_call_success_jk_deletePage', $page_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_editPage( $args ) {
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.editPage', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPageList( $args ) {
		global $jkdb;
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPageList', $args, $this );

		// Get list of page IDs and titles.
		$page_list = $jkdb->get_results(
			"
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getAuthors( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getAuthors', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getKeywords', $args, $this );
	 ...
	 */
	public function jk_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.newCategory', $args, $this );
		 ...
		 * so that JKPress will generate one.
		 */
		...
		$cat_id = jk_insert_category( $new_category, true );
		if ( is_jk_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_jk_newCategory', $cat_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * }
	 * @return bool|IXR_Error See jk_delete_term() for return info.
	 */
	public function jk_deleteCategory( $args ) {
		$this->escape( $args );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_jk_deleteCategory', $category_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.suggestCategories', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getComment', $args, $this );
	 ...
	 * - 'offset'  - Default is 0. See JK_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *                         See jk_xmlrpc_server::jk_getComment() for a description
	 *                         of each item contents.
	 */
	public function jk_getComments( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getComments', $args, $this );
	 ...
	 * See jk_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *     @type int    $3 Comment ID.
	 * }
	 * @return bool|IXR_Error See jk_delete_comment().
	 */
	public function jk_deleteComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.deleteComment', $args, $this );

		$status = jk_delete_comment( $comment_id );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_jk_deleteComment', $comment_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function jk_editComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.editComment', $args, $this );
		$comment = array(
		...
		$result = jk_update_comment( $comment, true );
		if ( is_jk_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_jk_editComment', $comment_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	 ...
	 *     @type array      $4 Content structure.
	 * }
	 * @return int|IXR_Error See jk_new_comment().
	 */
	public function jk_newComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.newComment', $args, $this );

		$comment_id = jk_new_comment( $comment, true );
		 ...
		 */
		do_action( 'xmlrpc_call_success_jk_newComment', $comment_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getCommentStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getCommentStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getCommentCount( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getCommentCount', $args, $this );

		$count = jk_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPostStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPageStatusList', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[ $o_name ]['option'], jk_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function jk_getMediaItem( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getMediaItem', $args, $this );
	 ...
	 *                         See jk_xmlrpc_server::jk_getMediaItem() for a description
	 *                         of each item contents.
	 */
	public function jk_getMediaLibrary( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getMediaLibrary', $args, $this );
	 ...
	 */
	public function jk_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPostFormats', $args, $this );
	 ...
	 *  - 'supports'
	 */
	public function jk_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'jk.getPostType' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPostType', $args, $this );
	 ...
	 * @return array|IXR_Error
	 */
	public function jk_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'jk.getPostTypes' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getPostTypes', $args, $this );
	 ...
	 * @return array|IXR_Error Array containing a collection of posts.
	 */
	public function jk_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'jk.getRevisions' );
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.getRevisions', $args, $this );
		...
		if ( ! jk_revisions_enabled( $post ) ) {
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );
		}

		$revisions = jk_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves.
			if ( jk_is_post_autosave( $revision ) ) {
				continue;
			}
	 ...
	 */
	public function jk_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		/** This action is documented in jk-includes/class-jk-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'jk.restoreRevision', $args, $this );

		$revision = jk_get_post_revision( $revision_id );
		if ( ! $revision ) {
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		}
		...
		$post = jk_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs( $args ) {
		...
		$blogs = $this->jk_getUsersBlogs( $args );
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode( ',', jk_get_post_categories( $post_id ) );

		$content  = '<title>' . jk_unslash( $post_data['post_title'] ) . '</title>';
		$content .= '<category>' . $categories . '</category>';
		$content .= jk_unslash( $post_data['post_content'] );
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = implode( ',', jk_get_post_categories( $entry['ID'] ) );

			$content  = '<title>' . jk_unslash( $entry['post_title'] ) . '</title>';
			$content .= '<category>' . $categories . '</category>';
			$content .= jk_unslash( $entry['post_content'] );
	...
	public function blogger_newPost( $args ) {
		...
		$post_id = jk_insert_post( $post_data );
		if ( is_jk_error( $post_id ) ) {
			return new IXR_Error( 500, $post_id->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_newPost', $post_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_editPost( $args ) {
		...
		$result = jk_update_post( $postdata );
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_editPost', $post_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_deletePost( $args ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_deletePost', $post_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_newPost( $args ) {
		...
		if ( ! empty( $content_struct['post_type'] ) ) {
			if ( 'page' === $content_struct['post_type'] ) {
				...
				$post_type     = 'page';
				if ( ! empty( $content_struct['jk_page_template'] ) ) {
					$page_template = $content_struct['jk_page_template'];
				}
		...
		if ( isset( $content_struct['jk_post_format'] ) ) {
			$content_struct['jk_post_format'] = sanitize_key( $content_struct['jk_post_format'] );
			if ( ! array_key_exists( $content_struct['jk_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format.' ) );
			}
		}

		// Let JKPress generate the 'post_name' (slug) unless
		// one has been provided.
		$post_name = null;
		if ( isset( $content_struct['jk_slug'] ) ) {
			$post_name = $content_struct['jk_slug'];
		}

		// Only use a password if one was given.
		$post_password = '';
		if ( isset( $content_struct['jk_password'] ) ) {
			$post_password = $content_struct['jk_password'];
		}

		// Only set a post parent if one was given.
		$post_parent = 0;
		if ( isset( $content_struct['jk_page_parent_id'] ) ) {
			$post_parent = $content_struct['jk_page_parent_id'];
		}

		// Only set the 'menu_order' if it was given.
		$menu_order = 0;
		if ( isset( $content_struct['jk_page_order'] ) ) {
			$menu_order = $content_struct['jk_page_order'];
		}

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['jk_author_id'] ) && ( $user->ID != $content_struct['jk_author_id'] ) ) {
			switch ( $post_type ) {
				case 'post':
			...
			$author = get_userdata( $content_struct['jk_author_id'] );
			if ( ! $author ) {
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			}
			$post_author = $content_struct['jk_author_id'];
		}
		...
		if ( isset( $content_struct['jk_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_id, $content_struct['jk_post_thumbnail'] ) === false ) {
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
			}

			unset( $content_struct['jk_post_thumbnail'] );
		}
		 ...
		 */
		if ( isset( $content_struct['jk_post_format'] ) ) {
			set_post_format( $post_id, $content_struct['jk_post_format'] );
		}

		$post_id = jk_insert_post( $postdata, true );
		if ( is_jk_error( $post_id ) ) {
			return new IXR_Error( 500, $post_id->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newPost', $post_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function attach_uploads( $post_id, $post_content ) {
		...
		$attachments = $jkdb->get_results( "SELECT ID, guid FROM {$jkdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && str_contains( $post_content, $file->guid ) ) {
					$jkdb->update( $jkdb->posts, array( 'post_parent' => $post_id ), array( 'ID' => $file->ID ) );
				}
			}
		}
	}
	...
	public function mw_editPost( $args ) {
		...
		// Use jk.editPost to edit post types other than post and page.
		...
		// Let JKPress manage slug if none was provided.
		...
		$page_template = '';
		if ( ! empty( $content_struct['jk_page_template'] ) && 'page' === $post_type ) {
			$page_template = $content_struct['jk_page_template'];
		}
		...
		// If an author id was provided then use it instead.
		if ( isset( $content_struct['jk_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['jk_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		$result = jk_update_post( $newpost, true );
		if ( is_jk_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		...
		if ( isset( $content_struct['jk_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['jk_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_id );
			} else {
				if ( set_post_thumbnail( $post_id, $content_struct['jk_post_thumbnail'] ) === false ) {
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_editPost', $post_id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_getPost( $args ) {
		...
		if ( '' !== $postdata['post_date'] ) {
			...
			$categories = array();
			$catids     = jk_get_post_categories( $post_id );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = jk_get_post_tags( $post_id );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$resp = array(
				...
				'jk_more_text'           => $post['more_text'],
				'mt_allow_comments'      => $allow_comments,
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'jk_slug'                => $postdata['post_name'],
				'jk_password'            => $postdata['post_password'],
				'jk_author_id'           => (string) $author->ID,
				'jk_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				'post_status'            => $postdata['post_status'],
				'custom_fields'          => $this->get_custom_fields( $post_id ),
				'jk_post_format'         => $post_format,
				'sticky'                 => $sticky,
			...
			$resp['jk_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = array();
			$catids     = jk_get_post_categories( $entry['ID'] );
			foreach ( $catids as $catid ) {
				$categories[] = get_cat_name( $catid );
			}

			$tagnames = array();
			$tags     = jk_get_post_tags( $entry['ID'] );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'jk_slug'                => $entry['post_name'],
				'jk_password'            => $entry['post_password'],
				'jk_author_id'           => (string) $author->ID,
				'jk_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				...
				'jk_post_thumbnail'      => get_post_thumbnail_id( $entry['ID'] ),
			);
		}
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = jk_upload_bits( $name, null, $bits );
		...
		// Save the data.
		$id = jk_insert_attachment( $attachment, $upload['file'], $post_id );
		jk_update_attachment_metadata( $id, jk_generate_attachment_metadata( $id, $upload['file'] ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newMediaObject', $id, $args ); // phpcs:ignore JKPress.NamingConventions.ValidHookName.NotLowercase
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids     = jk_get_post_categories( (int) $post_id );
	...
	public function mt_setPostCategories( $args ) {
		...
		jk_set_post_categories( $post_id, $catids );
	...
	public function mt_getTrackbackPings( $post_id ) {
		...
		$comments = $jkdb->get_results( $jkdb->prepare( "SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $jkdb->comments WHERE comment_post_ID = %d", $post_id ) );
	...
	public function mt_publishPost( $args ) {
		...
		$postdata['post_category'] = jk_get_post_categories( $post_id );
		$this->escape( $postdata );

		return jk_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		if ( $post_id ) {
			...
			if ( (int) $urltest['fragment'] ) {
				...
				$sql     = $jkdb->prepare( "SELECT ID FROM $jkdb->posts WHERE post_title RLIKE %s", $title );
				$post_id = $jkdb->get_var( $sql );
				if ( ! $post_id ) {
		...
		if ( $jkdb->get_results( $jkdb->prepare( "SELECT * FROM $jkdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_id, $pagelinkedfrom ) ) ) {
		...
		/** This filter is documented in jk-includes/class-jk-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'JKPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $pagelinkedfrom );
		...
		$request                = jk_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source          = jk_remote_retrieve_body( $request );
