<?php
/**
 * Error Protection API: JK_Recovery_Mode_Link_Handler class
 *
 * @package JKPress
 * @since 5.2.0
 */
...
class JK_Recovery_Mode_Link_Service {
	...
	/**
	 * JK_Recovery_Mode_Link_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param JK_Recovery_Mode_Cookie_Service $cookie_service Service to handle setting the recovery mode cookie.
	 * @param JK_Recovery_Mode_Key_Service    $key_service    Service to handle generating recovery mode keys.
	 */
	public function __construct( JK_Recovery_Mode_Cookie_Service $cookie_service, JK_Recovery_Mode_Key_Service $key_service ) {
	...
	/**
	 * Enters recovery mode when the user hits jk-login.php with a valid recovery mode link.
	 *
	 * @since 5.2.0
	 *
	...
	public function handle_begin_link( $ttl ) {
		...
		$url = add_query_arg( 'action', self::LOGIN_ACTION_ENTERED, jk_login_url() );
		jk_redirect( $url );
		die;
	}
