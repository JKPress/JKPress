...
function create_initial_post_types() {
	JK_Post_Type::reset_default_labels();
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'posts',
			'rest_controller_class' => 'JK_REST_Posts_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			'labels'                => array(
				...
				'view_item'      => ( '1' === get_option( 'jk_attachment_pages_enabled' ) ) ? __( 'View Attachment Page' ) : __( 'View Media File' ),
			...
			'show_in_rest'          => true,
			'rest_base'             => 'media',
			'rest_controller_class' => 'JK_REST_Attachments_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'JK_REST_Menu_Items_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'blocks',
			'rest_controller_class' => 'JK_REST_Blocks_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rewrite'                         => false,
			'rest_base'                       => 'templates',
			'rest_controller_class'           => 'JK_REST_Templates_Controller',
			'autosave_rest_controller_class'  => 'JK_REST_Template_Autosaves_Controller',
			'revisions_rest_controller_class' => 'JK_REST_Template_Revisions_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class'           => 'JK_REST_Global_Styles_Controller',
			'revisions_rest_controller_class' => 'JK_REST_Global_Styles_Revisions_Controller',
			'late_route_registration'         => true,
			'capabilities'                    => array(
				'read'                   => 'edit_posts',
				'create_posts'           => 'edit_theme_options',
	...
	remove_post_type_support( 'jk_global_styles', 'autosave' );

	$navigation_post_edit_link = 'site-editor.php?' . build_query(
		array(
			'postId'   => '%s',
			'postType' => 'jk_navigation',
			'canvas'   => 'edit',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'JK_REST_Font_Families_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'JK_REST_Font_Faces_Controller',
			'supports'              => array( 'title' ),
		)
	);
 ...
 *
 * The function works by retrieving the `_jk_attached_file` post meta value.
...
function get_attached_file( $attachment_id, $unfiltered = false ) {
	$file = get_post_meta( $attachment_id, '_jk_attached_file', true );

	// If the file is relative, prepend upload dir.
	if ( $file && ! str_starts_with( $file, '/' ) && ! preg_match( '|^.:\\\|', $file ) ) {
		$uploads = jk_get_upload_dir();
 ...
 * `_jk_attached_file` to store the path of the attachment.
 *
 * @since 2.1.0
 *
 * @param int    $attachment_id Attachment ID.
 * @param string $file          File path for the attachment.
 * @return int|bool Meta ID if the `_jk_attached_file` key didn't exist for the attachment.
...
function update_attached_file( $attachment_id, $file ) {
	...
	$file = _jk_relative_upload_path( $file );
	if ( $file ) {
		return update_post_meta( $attachment_id, '_jk_attached_file', $file );
	} else {
		return delete_post_meta( $attachment_id, '_jk_attached_file' );
	}
}
 ...
 */
function _jk_relative_upload_path( $path ) {
	$new_path = $path;
	 ...
	 * @param string $path     Full path to the file.
	 */
	return apply_filters( '_jk_relative_upload_path', $new_path, $path );
}
 ...
 * context of the inner workings of JKPress, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but JKPress
 ...
 *                       correspond to a JK_Post object, an associative array, or a numeric array,
 *                       respectively. Default OBJECT.
 * @return JK_Post[]|array[]|int[] Array of post objects, arrays, or IDs, depending on `$output`.
 */
 ...
 * @global JK_Post $post Global post object.
 *
 * @param int|JK_Post|null $post   Optional. Post ID or post object. `null`, `false`, `0` and other PHP falsey values
 ...
 * @return JK_Post|array|null Type corresponding to $output on success or null on failure.
 *                            When $output is OBJECT, a `JK_Post` instance is returned.
 */
function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( $post instanceof JK_Post ) {
		$_post = $post;
	} elseif ( is_object( $post ) ) {
		if ( empty( $post->filter ) ) {
			$_post = sanitize_post( $post, 'raw' );
			$_post = new JK_Post( $_post );
		} elseif ( 'raw' === $post->filter ) {
			$_post = new JK_Post( $post );
		} else {
			$_post = JK_Post::get_instance( $post->ID );
		}
	} else {
		$_post = JK_Post::get_instance( $post );
	}
 ...
 * @param int|JK_Post $post    Optional. Post ID or post object. Defaults to global $post.
...
function get_post_status( $post = null ) {
	...
	if ( ! $post instanceof JK_Post || ! isset( $post->filter ) || 'sample' !== $post->filter ) {
		$post = get_post( $post );
	}
	...
	if (
		...
		if (
			...
			$post_status = get_post_meta( $post->post_parent, '_jk_trash_meta_status', true );
		 ...
		 * This is to match the logic in jk_insert_post().
		 *
	 ...
	 * @param JK_Post $post        The post object.
	 */
	return apply_filters( 'get_post_status', $post_status, $post );
}

/**
 * Retrieves all of the JKPress supported post statuses.
 *
...
/**
 * Retrieves all of the JKPress support page statuses.
 *
 ...
 * @since 3.0.0
 *
 * @global stdClass[] $jk_post_statuses Inserts new post status object into the list
 *
...
function register_post_status( $post_status, $args = array() ) {
	global $jk_post_statuses;

	if ( ! is_array( $jk_post_statuses ) ) {
		$jk_post_statuses = array();
	}
	...
	if ( false === $args->label_count ) {
		// phpcs:ignore JKPress.JK.I18n.NonSingularStringLiteralSingular,JKPress.JK.I18n.NonSingularStringLiteralPlural
		$args->label_count = _n_noop( $args->label, $args->label );
	}

	$jk_post_statuses[ $post_status ] = $args;
 ...
 * @since 3.0.0
 *
 * @global stdClass[] $jk_post_statuses List of post statuses.
 *
...
function get_post_status_object( $post_status ) {
	...
	if ( empty( $jk_post_statuses[ $post_status ] ) ) {
		return null;
	}

	return $jk_post_statuses[ $post_status ];
}
 ...
 *                               properties of the global `$jk_post_statuses objects`. Default empty array.
...
function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return jk_filter_object_list( $jk_post_statuses, $args, $operator, $field );
}
 ...
 * @since 2.1.0
 *
 * @param int|JK_Post|null $post Optional. Post ID or post object. Default is global $post.
 ...
 *
 * @since 3.0.0
 * @since 4.6.0 Object returned is now an instance of `JK_Post_Type`.
 *
 * @global array $jk_post_types List of post types.
 *
 * @see register_post_type()
 *
 * @param string $post_type The name of a registered post type.
 * @return JK_Post_Type|null JK_Post_Type object if it exists, null otherwise.
 */
function get_post_type_object( $post_type ) {
	global $jk_post_types;

	if ( ! is_scalar( $post_type ) || empty( $jk_post_types[ $post_type ] ) ) {
		return null;
	}

	return $jk_post_types[ $post_type ];
}
 ...
 * @return string[]|JK_Post_Type[] An array of post type names or objects.
 */
function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return jk_filter_object_list( $jk_post_types, $args, $operator, $field );
}
 ...
 * @since 4.6.0 Post type object returned is now an instance of `JK_Post_Type`.
 ...
 *     @type string       $rest_namespace                  To change the namespace URL of REST API route. Default is jk/v2.
 *     @type string       $rest_controller_class           REST API controller class name. Default is 'JK_REST_Posts_Controller'.
 *     @type string|bool  $autosave_rest_controller_class  REST API controller class name. Default is 'JK_REST_Autosaves_Controller'.
 *     @type string|bool  $revisions_rest_controller_class REST API controller class name. Default is 'JK_REST_Revisions_Controller'.
 ...
 *                                                        'dashicons-chart-pie'. Pass 'none' to leave div.jk-menu-image empty
 ...
 *         @type bool   $with_front Whether the permastruct should be prepended with JK_Rewrite::$front.
 *                                  Default true.
 ...
 * }
 * @return JK_Post_Type|JK_Error The registered post type object on success,
 *                               JK_Error object on failure.
 */
function register_post_type( $post_type, $args = array() ) {
	global $jk_post_types;

	if ( ! is_array( $jk_post_types ) ) {
		$jk_post_types = array();
	}
	...
	if ( empty( $post_type ) || strlen( $post_type ) > 20 ) {
		...
		return new JK_Error( 'post_type_length_invalid', __( 'Post type names must be between 1 and 20 characters in length.' ) );
	}

	$post_type_object = new JK_Post_Type( $post_type, $args );
	$post_type_object->add_supports();
	$post_type_object->add_rewrite_rules();
	$post_type_object->register_meta_boxes();

	$jk_post_types[ $post_type ] = $post_type_object;
	 ...
	 *
	 * @since 3.3.0
	 * @since 4.6.0 Converted the `$post_type` parameter to accept a `JK_Post_Type` object.
	 *
	 * @param string       $post_type        Post type.
	 * @param JK_Post_Type $post_type_object Arguments used to register the post type.
	 */
 ...
 * @return true|JK_Error True on success, JK_Error on failure or if the post type doesn't exist.
 */
function unregister_post_type( $post_type ) {
	global $jk_post_types;

	if ( ! post_type_exists( $post_type ) ) {
		return new JK_Error( 'invalid_post_type', __( 'Invalid post type.' ) );
	}
	...
	if ( $post_type_object->_builtin ) {
		return new JK_Error( 'invalid_post_type', __( 'Unregistering a built-in post type is not allowed' ) );
	}
 ...
 * @access private
 *
 * @param object|JK_Post_Type $post_type_object Post type object.
 * @return object Object with all the labels as member variables.
 */
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = JK_Post_Type::get_default_labels();
 ...
 *
 * @global array $_jk_post_type_features
 *
...
function add_post_type_support( $post_type, $feature, ...$args ) {
	global $_jk_post_type_features;

	$features = (array) $feature;
	foreach ( $features as $feature ) {
		if ( $args ) {
			$_jk_post_type_features[ $post_type ][ $feature ] = $args;
		} else {
			$_jk_post_type_features[ $post_type ][ $feature ] = true;
		}
	}
}
...
function remove_post_type_support( $post_type, $feature ) {
	...
	unset( $_jk_post_type_features[ $post_type ][ $feature ] );
}
...
function get_all_post_type_supports( $post_type ) {
	...
	if ( isset( $_jk_post_type_features[ $post_type ] ) ) {
		return $_jk_post_type_features[ $post_type ];
	}
...
function post_type_supports( $post_type, $feature ) {
	...
	return ( isset( $_jk_post_type_features[ $post_type ][ $feature ] ) );
}
 ...
 * @since 4.5.0
 *
 * @global array $_jk_post_type_features Post type features
 *
...
function get_post_types_by_support( $feature, $operator = 'and' ) {
	...
	return array_keys( jk_filter_object_list( $_jk_post_type_features, $features, $operator ) );
}
...
function set_post_type( $post_id = 0, $post_type = 'post' ) {
	...
	$return    = $jkdb->update( $jkdb->posts, array( 'post_type' => $post_type ), array( 'ID' => $post_id ) );
 ...
 *
 * @param string|JK_Post_Type $post_type Post type name or object.
...
function is_post_type_viewable( $post_type ) {
	 ...
	 * @param JK_Post_Type $post_type   Post type object.
	 */
 ...
 * @since 5.7.0
 *
 * @param int|JK_Post|null $post Optional. Post ID or post object. Defaults to global $post.
 ...
 * {@link https://developer.wordpress.org/reference/classes/jk_query/
 * JK_Query} documentation in the Developer Handbook.
 *
 ...
 *
 * @see JK_Query
 * @see JK_Query::parse_query()
 *
 * @param array $args {
 *     Optional. Arguments to retrieve posts. See JK_Query::parse_query() for all available arguments.
 *
 *     @type int        $numberposts      Total number of posts to retrieve. Is an alias of `$posts_per_page`
 *                                        in JK_Query. Accepts -1 for all. Default 5.
 *     @type int|string $category         Category ID or comma-separated list of IDs (this or any children).
 *                                        Is an alias of `$cat` in JK_Query. Default 0.
 *     @type int[]      $include          An array of post IDs to retrieve, sticky posts will be included.
 *                                        Is an alias of `$post__in` in JK_Query. Default empty array.
 *     @type int[]      $exclude          An array of post IDs not to retrieve. Default empty array.
 *     @type bool       $suppress_filters Whether to suppress filters. Default true.
 * }
 * @return JK_Post[]|int[] Array of post objects or post IDs.
 */
function get_posts( $args = null ) {
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		$incposts                      = jk_parse_id_list( $parsed_args['include'] );
		$parsed_args['posts_per_page'] = count( $incposts );  // Only the number of posts included.
		$parsed_args['post__in']       = $incposts;
	} elseif ( ! empty( $parsed_args['exclude'] ) ) {
		$parsed_args['post__not_in'] = jk_parse_id_list( $parsed_args['exclude'] );
	}

	$parsed_args['ignore_sticky_posts'] = true;
	$parsed_args['no_found_rows']       = true;

	$get_posts = new JK_Query();
...
function add_post_meta( $post_id, $meta_key, $meta_value, $unique = false ) {
	...
	$the_post = jk_is_post_revision( $post_id );
	if ( $the_post ) {
		$post_id = $the_post;
	}
 ...
 * @see sanitize_post_field()
 *
 * @param object|JK_Post|array $post    The post object or array
 * @param string               $context Optional. How to sanitize post fields.
 *                                      Accepts 'raw', 'edit', 'db', 'display',
 *                                      'attribute', or 'js'. Default 'display'.
 * @return object|JK_Post|array The now sanitized post object or array (will be the
 *                              same type as `$post`).
 */
...
/**
 * Returns the cache key for jk_count_posts() based on the passed arguments.
 *
 * @since 3.9.0
 * @access private
 *
 ...
 */
function jk_count_posts( $type = 'post', $perm = '' ) {
	global $jkdb;
	...
	$counts = jk_cache_get( $cache_key, 'counts' );
	if ( false !== $counts ) {
		...
		/** This filter is documented in jk-includes/post.php */
		return apply_filters( 'jk_count_posts', $counts, $type, $perm );
	}

	$query = "SELECT post_status, COUNT( * ) AS num_posts FROM {$jkdb->posts} WHERE post_type = %s";
	...
	$results = (array) $jkdb->get_results( $jkdb->prepare( $query, $type ), ARRAY_A );
	...
	jk_cache_set( $cache_key, $counts, 'counts' );
 ...
 */
function jk_count_attachments( $mime_type = '' ) {
	global $jkdb;
	...
	if ( false === $counts ) {
		$and   = jk_post_mime_type_where( $mime_type );
		$count = $jkdb->get_results( "SELECT post_mime_type, COUNT( * ) AS num_posts FROM $jkdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' $and GROUP BY post_mime_type", ARRAY_A );

		$counts = array();
		foreach ( (array) $count as $row ) {
			$counts[ $row['post_mime_type'] ] = $row['num_posts'];
		}
		$counts['trash'] = $jkdb->get_var( "SELECT COUNT( * ) FROM $jkdb->posts WHERE post_type = 'attachment' AND post_status = 'trash' $and" );

		jk_cache_set( $cache_key, (object) $counts, 'counts' );
	}
	 ...
	 */
	return apply_filters( 'jk_count_attachments', (object) $counts, $mime_type );
}
...
function get_post_mime_types() {
	...
	$ext_types  = jk_get_ext_types();
	$mime_types = jk_get_mime_types();
 ...
 */
function jk_match_mime_types( $wildcard_mime_types, $real_mime_types ) {
	$matches = array();
 ...
 */
function jk_post_mime_type_where( $post_mime_types, $table_alias = '' ) {
	$where     = '';
	$wildcards = array( '', '%', '%/%' );
 ...
 *
 * @global jkdb $jkdb JKPress database abstraction object.
 * @see jk_delete_attachment()
 * @see jk_trash_post()
 *
 * @param int  $post_id      Optional. Post ID. Default 0.
 * @param bool $force_delete Optional. Whether to bypass Trash and force deletion.
 *                           Default false.
 * @return JK_Post|false|null Post data on success, false or null on failure.
 */
function jk_delete_post( $post_id = 0, $force_delete = false ) {
	global $jkdb;

	$post = $jkdb->get_row( $jkdb->prepare( "SELECT * FROM $jkdb->posts WHERE ID = %d", $post_id ) );
	...
	if ( ! $force_delete
		...
		return jk_trash_post( $post_id );
	}

	if ( 'attachment' === $post->post_type ) {
		return jk_delete_attachment( $post_id, $force_delete );
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_Post|false|null $delete       Whether to go forward with deletion.
	...
	/**
	 * Fires before a post is deleted, at the start of jk_delete_post().
	 *
	 * @since 3.2.0
	...
	jk_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	if ( is_post_type_hierarchical( $post->post_type ) ) {
		...
		$children_query = $jkdb->prepare(
			"SELECT * FROM $jkdb->posts WHERE post_parent = %d AND post_type = %s",
			$post_id,
			$post->post_type
		);

		$children = $jkdb->get_results( $children_query );

		if ( $children ) {
			$jkdb->update( $jkdb->posts, $parent_data, $parent_where + array( 'post_type' => $post->post_type ) );
		}
	}

	// Do raw query. jk_get_post_revisions() is filtered.
	$revision_ids = $jkdb->get_col(
		$jkdb->prepare( "SELECT ID FROM $jkdb->posts WHERE post_parent = %d AND post_type = 'revision'", $post_id )
	);

	// Use jk_delete_post (via jk_delete_post_revision) again. Ensures any meta/misplaced data gets cleaned up.
	foreach ( $revision_ids as $revision_id ) {
		jk_delete_post_revision( $revision_id );
	}

	// Point all attachments to this post up one level.
	$jkdb->update( $jkdb->posts, $parent_data, $parent_where + array( 'post_type' => 'attachment' ) );

	jk_defer_comment_counting( true );

	$comment_ids = $jkdb->get_col(
		$jkdb->prepare( "SELECT comment_ID FROM $jkdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC", $post_id )
	);

	foreach ( $comment_ids as $comment_id ) {
		jk_delete_comment( $comment_id, true );
	}

	jk_defer_comment_counting( false );

	$post_meta_ids = $jkdb->get_col(
		$jkdb->prepare( "SELECT meta_id FROM $jkdb->postmeta WHERE post_id = %d ", $post_id )
	);
	...
	$result = $jkdb->delete( $jkdb->posts, array( 'ID' => $post_id ) );
	if ( ! $result ) {
		return false;
	}
	...
	jk_clear_scheduled_hook( 'publish_future_post', array( $post_id ) );

	/**
	 * Fires after a post is deleted, at the conclusion of jk_delete_post().
	 *
	 * @since 3.2.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
 ...
 * @return JK_Post|false|null Post data on success, false or null on failure.
 */
function jk_trash_post( $post_id = 0 ) {
	if ( ! EMPTY_TRASH_DAYS ) {
		return jk_delete_post( $post_id, true );
	}
	 ...
	 * @param string $previous_status The status of the post about to be trashed.
	 */
	do_action( 'jk_trash_post', $post_id, $previous_status );

	add_post_meta( $post_id, '_jk_trash_meta_status', $previous_status );
	add_post_meta( $post_id, '_jk_trash_meta_time', time() );

	$post_updated = jk_update_post(
		array(
			'ID'          => $post_id,
			'post_status' => 'trash',
		)
	...
	jk_trash_post_comments( $post_id );
 ...
 * @return JK_Post|false|null Post data on success, false or null on failure.
 */
function jk_untrash_post( $post_id = 0 ) {
	$post = get_post( $post_id );
	...
	$previous_status = get_post_meta( $post_id, '_jk_trash_meta_status', true );
	 ...
	 * in order to assign the status that the post had before it was trashed. The `jk_untrash_post_set_previous_status()`
	 * function is available for this.
	 *
	 * Prior to JKPress 5.6.0, restored posts were always assigned their original status.
	 *
	 * @since 5.6.0
	 *
	 ...
	 * @param string $previous_status The status of the post at the point where it was trashed.
	 */
	$post_status = apply_filters( 'jk_untrash_post_status', $new_status, $post_id, $previous_status );
	...
	jk_untrash_post_comments( $post_id );
 ...
 */
function jk_trash_post_comments( $post = null ) {
	global $jkdb;
	...
	$comments = $jkdb->get_results( $jkdb->prepare( "SELECT comment_ID, comment_approved FROM $jkdb->comments WHERE comment_post_ID = %d", $post_id ) );
	...
	add_post_meta( $post_id, '_jk_trash_meta_comments_status', $statuses );

	// Set status for all comments to post-trashed.
	$result = $jkdb->update( $jkdb->comments, array( 'comment_approved' => 'post-trashed' ), array( 'comment_post_ID' => $post_id ) );
 ...
 * @return true|void
 */
function jk_untrash_post_comments( $post = null ) {
	global $jkdb;
	...
	$statuses = get_post_meta( $post_id, '_jk_trash_meta_comments_status', true );
	...
	foreach ( $group_by_status as $status => $comments ) {
		...
		$jkdb->query( $jkdb->prepare( "UPDATE $jkdb->comments SET comment_approved = %s WHERE comment_ID IN ($comments_in)", $status ) );
	}

	clean_comment_cache( array_keys( $statuses ) );

	delete_post_meta( $post_id, '_jk_trash_meta_comments_status' );
 ...
 * @since 2.1.0
 *
 * @see jk_get_object_terms()
 *
 * @param int   $post_id Optional. The Post ID. Does not default to the ID of the
 *                       global $post. Default 0.
 * @param array $args    Optional. Category query parameters. Default empty array.
 *                       See JK_Term_Query::__construct() for supported arguments.
 * @return array|JK_Error List of categories. If the `$fields` argument passed via `$args` is 'all' or
 *                        'all_with_object_id', an array of JK_Term objects will be returned. If `$fields`
 *                        is 'ids', an array of category IDs. If `$fields` is 'names', an array of category names.
 *                        JK_Error object if 'category' taxonomy doesn't exist.
 */
function jk_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'ids' );
	$args     = jk_parse_args( $args, $defaults );

	$cats = jk_get_object_terms( $post_id, 'category', $args );
	return $cats;
}
 ...
 *                       See JK_Term_Query::__construct() for supported arguments.
 * @return array|JK_Error Array of JK_Term objects on success or empty array if no tags were found.
 *                        JK_Error object if 'post_tag' taxonomy doesn't exist.
 */
function jk_get_post_tags( $post_id = 0, $args = array() ) {
	return jk_get_post_terms( $post_id, 'post_tag', $args );
}
 ...
 * @param array           $args     {
 *     Optional. Term query parameters. See JK_Term_Query::__construct() for supported arguments.
 *
 *     @type string $fields Term fields to retrieve. Default 'all'.
 * }
 * @return array|JK_Error Array of JK_Term objects on success or empty array if no terms were found.
 *                        JK_Error object if `$taxonomy` doesn't exist.
 */
function jk_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'all' );
	$args     = jk_parse_args( $args, $defaults );

	$tags = jk_get_object_terms( $post_id, $taxonomy, $args );
 ...
 *                       correspond to a JK_Post object or an associative array, respectively.
 *                       Default ARRAY_A.
 * @return array|false Array of recent posts, where the type of each element is determined
 *                     by the `$output` parameter. Empty array on failure.
 */
function jk_get_recent_posts( $args = array(), $output = ARRAY_A ) {
 ...
 *
 * @since 1.0.0
 * @since 2.6.0 Added the `$jk_error` parameter to allow a JK_Error to be returned on failure.
 ...
 * }
 * @param bool  $jk_error         Optional. Whether to return a JK_Error on failure. Default false.
 * @param bool  $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|JK_Error The post ID on success. The value 0 or JK_Error on failure.
 */
function jk_insert_post( $postarr, $jk_error = false, $fire_after_hooks = true ) {
	global $jkdb;
	...
	$postarr = jk_parse_args( $postarr, $defaults );
	...
	if ( ! empty( $postarr['ID'] ) ) {
		...
		if ( is_null( $post_before ) ) {
			if ( $jk_error ) {
				return new JK_Error( 'invalid_post', __( 'Invalid post ID.' ) );
			}
			return 0;
		}
	 ...
	 * the new post being inserted and return 0. If $jk_error is true, a JK_Error
	 ...
	 */
	if ( apply_filters( 'jk_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $jk_error ) {
			return new JK_Error( 'empty_content', __( 'Content, title, and excerpt are empty.' ) );
		} else {
			return 0;
		}
	}
	 ...
	 */
	$post_date = jk_resolve_post_date( $postarr['post_date'], $postarr['post_date_gmt'] );

	if ( ! $post_date ) {
		if ( $jk_error ) {
			return new JK_Error( 'invalid_date', __( 'Invalid date.' ) );
		} else {
			return 0;
		}
	}
	...
	/*
	 * The 'jk_insert_post_parent' filter expects all variables to be present.
	 ...
	 */
	$post_parent = apply_filters( 'jk_insert_post_parent', $post_parent, $post_id, $new_postarr, $postarr );
	...
	if ( 'trash' === $previous_status && 'trash' !== $post_status ) {
		$desired_post_slug = get_post_meta( $post_id, '_jk_desired_post_slug', true );

		if ( $desired_post_slug ) {
			delete_post_meta( $post_id, '_jk_desired_post_slug' );
	...
	if ( 'trash' !== $post_status && $post_name ) {
		...
		if ( $add_trashed_suffix ) {
			jk_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id );
		}
	}

	// When trashing an existing post, change its slug to allow non-trashed posts to use it.
	if ( 'trash' === $post_status && 'trash' !== $previous_status && 'new' !== $previous_status ) {
		$post_name = jk_add_trashed_suffix_to_post_name_for_post( $post_id );
	}

	$post_name = jk_unique_post_slug( $post_name, $post_id, $post_status, $post_type, $post_parent );
	...
	foreach ( $emoji_fields as $emoji_field ) {
		if ( isset( $data[ $emoji_field ] ) ) {
			$charset = $jkdb->get_col_charset( $jkdb->posts, $emoji_field );

			if ( 'utf8' === $charset ) {
				$data[ $emoji_field ] = jk_encode_emoji( $data[ $emoji_field ] );
			}
		}
	}
	...
	if ( 'attachment' === $post_type ) {
		 ...
		 *                                   as originally passed to jk_insert_post().
		 * @param bool  $update              Whether this is an existing attachment post being updated.
		 */
		$data = apply_filters( 'jk_insert_attachment_data', $data, $postarr, $unsanitized_postarr, $update );
	} else {
		/**
		 ...
		 *                                   originally passed to jk_insert_post().
		 * @param bool  $update              Whether this is an existing post being updated.
		 */
		$data = apply_filters( 'jk_insert_post_data', $data, $postarr, $unsanitized_postarr, $update );
	}

	$data  = jk_unslash( $data );
	...
	if ( $update ) {
		...
		if ( false === $jkdb->update( $jkdb->posts, $data, $where ) ) {
			if ( $jk_error ) {
				if ( 'attachment' === $post_type ) {
				...
				return new JK_Error( 'db_update_error', $message, $jkdb->last_error );
			} else {
				return 0;
			}
		}
		...
		if ( ! empty( $import_id ) ) {
			...
			if ( ! $jkdb->get_var( $jkdb->prepare( "SELECT ID FROM $jkdb->posts WHERE ID = %d", $import_id ) ) ) {
				$data['ID'] = $import_id;
			}
		}

		if ( false === $jkdb->insert( $jkdb->posts, $data ) ) {
			if ( $jk_error ) {
				if ( 'attachment' === $post_type ) {
				...
				return new JK_Error( 'db_insert_error', $message, $jkdb->last_error );
			} else {
				return 0;
			}
		}

		$post_id = (int) $jkdb->insert_id;
	...
	if ( empty( $data['post_name'] ) && ! in_array( $data['post_status'], array( 'draft', 'pending', 'auto-draft' ), true ) ) {
		$data['post_name'] = jk_unique_post_slug( sanitize_title( $data['post_title'], $post_id ), $post_id, $data['post_status'], $post_type, $post_parent );

		$jkdb->update( $jkdb->posts, array( 'post_name' => $data['post_name'] ), $where );
		clean_post_cache( $post_id );
	}

	if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
		jk_set_post_categories( $post_id, $post_category );
	}

	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $post_type, 'post_tag' ) ) {
		jk_set_post_tags( $post_id, $postarr['tags_input'] );
	}
	...
	if ( 'auto-draft' !== $post_status ) {
		foreach ( get_object_taxonomies( $post_type, 'object' ) as $taxonomy => $tax_object ) {
			...
			if ( ! empty( $tax_object->default_term ) ) {
				...
				$terms = jk_get_object_terms( $post_id, $taxonomy, array( 'fields' => 'ids' ) );
	...
	if ( ! empty( $postarr['tax_input'] ) ) {
		foreach ( $postarr['tax_input'] as $taxonomy => $tags ) {
			...
			if ( current_user_can( $taxonomy_obj->cap->assign_terms ) ) {
				jk_set_post_terms( $post_id, $tags, $taxonomy );
			}
		}
	}
	...
	if ( ! $update && '' === $current_guid ) {
		$jkdb->update( $jkdb->posts, array( 'guid' => get_permalink( $post_id ) ), $where );
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		...
		if ( ! empty( $postarr['context'] ) ) {
			add_post_meta( $post_id, '_jk_attachment_context', $postarr['context'], true );
		}
	}
	...
	if ( isset( $postarr['_thumbnail_id'] ) ) {
		...
		if ( ! $thumbnail_support && 'attachment' === $post_type && $post_mime_type ) {
			if ( jk_attachment_is( 'audio', $post_id ) ) {
				$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
			} elseif ( jk_attachment_is( 'video', $post_id ) ) {
	...
	if ( ! empty( $postarr['page_template'] ) ) {
		...
		$page_templates      = jk_get_theme()->get_page_templates( $post );

		if ( 'default' !== $postarr['page_template'] && ! isset( $page_templates[ $postarr['page_template'] ] ) ) {
			if ( $jk_error ) {
				return new JK_Error( 'invalid_page_template', __( 'Invalid page template.' ) );
			}

			update_post_meta( $post_id, '_jk_page_template', 'default' );
		} else {
			update_post_meta( $post_id, '_jk_page_template', $postarr['page_template'] );
		}
	}

	if ( 'attachment' !== $postarr['post_type'] ) {
		jk_transition_post_status( $data['post_status'], $previous_status, $post );
	} else {
		if ( $update ) {
			/**
			 ...
			 *
			 * @param int     $post_id      Post ID.
			 * @param JK_Post $post_after   Post object following the update.
			 * @param JK_Post $post_before  Post object before the update.
			 */
	 ...
	 * @param bool    $update  Whether this is an existing post being updated.
	 */
	do_action( 'jk_insert_post', $post_id, $post, $update );

	if ( $fire_after_hooks ) {
		jk_after_insert_post( $post, $update, $post_before );
	}
 ...
 *
 * @since 1.0.0
 * @since 3.5.0 Added the `$jk_error` parameter to allow a JK_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @param array|object $postarr          Optional. Post data. Arrays are expected to be escaped,
 *                                       objects are not. See jk_insert_post() for accepted arguments.
 *                                       Default array.
 * @param bool         $jk_error         Optional. Whether to return a JK_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|JK_Error The post ID on success. The value 0 or JK_Error on failure.
 */
function jk_update_post( $postarr = array(), $jk_error = false, $fire_after_hooks = true ) {
	if ( is_object( $postarr ) ) {
		// Non-escaped post was passed.
		$postarr = get_object_vars( $postarr );
		$postarr = jk_slash( $postarr );
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		return jk_insert_attachment( $postarr, false, 0, $jk_error );
	}
	...
	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $postarr['post_type'], 'post_tag' ) ) {
		...
		if ( $tags && ! is_jk_error( $tags ) ) {
			$tag_names = jk_list_pluck( $tags, 'name' );
		}
	...
	return jk_insert_post( $postarr, $jk_error, $fire_after_hooks );
}
...
function jk_publish_post( $post ) {
	...
	foreach ( get_object_taxonomies( $post->post_type, 'object' ) as $taxonomy => $tax_object ) {
		...
		jk_set_post_terms( $post->ID, array( $default_term_id ), $taxonomy );
	}

	$jkdb->update( $jkdb->posts, array( 'post_status' => 'publish' ), array( 'ID' => $post->ID ) );

	clean_post_cache( $post->ID );

	$old_status        = $post->post_status;
	$post->post_status = 'publish';
	jk_transition_post_status( 'publish', $old_status, $post );

	/** This action is documented in jk-includes/post.php */
	...
	/** This action is documented in jk-includes/post.php */
	do_action( 'jk_insert_post', $post->ID, $post, true );

	jk_after_insert_post( $post, true, $post_before );
}
...
function check_and_publish_future_post( $post ) {
	...
	if ( $time > time() ) {
		jk_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) ); // Clear anything else in the system.
		jk_schedule_single_event( $time, 'publish_future_post', array( $post->ID ) );
		return;
	}

	// jk_publish_post() returns no meaningful value.
	jk_publish_post( $post->ID );
}

/**
 * Uses jk_checkdate to return a valid Gregorian-calendar value for post_date.
 * If post_date is not provided, this first checks post_date_gmt if provided,
 * then falls back to use the current time.
 *
 * For back-compat purposes in jk_insert_post, an empty post_date and an invalid
 ...
 */
function jk_resolve_post_date( $post_date = '', $post_date_gmt = '' ) {
	...
	$valid_date = jk_checkdate( $month, $day, $year, $post_date );
 ...
 */
function jk_unique_post_slug( $slug, $post_id, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ), true )
	 ...
	 * @param int         $post_parent   Post parent ID.
	 */
	$override_slug = apply_filters( 'pre_jk_unique_post_slug', null, $slug, $post_id, $post_status, $post_type, $post_parent );
	if ( null !== $override_slug ) {
		return $override_slug;
	}

	global $jkdb, $jk_rewrite;

	$original_slug = $slug;

	$feeds = $jk_rewrite->feeds;
	if ( ! is_array( $feeds ) ) {
		$feeds = array();
	}
	...
	if ( 'attachment' === $post_type ) {
		...
		$check_sql       = "SELECT post_name FROM $jkdb->posts WHERE post_name = %s AND ID != %d LIMIT 1";
		$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $slug, $post_id ) );
		 ...
		 */
		$is_bad_attachment_slug = apply_filters( 'jk_unique_post_slug_is_bad_attachment_slug', false, $slug );
		...
		if ( $post_name_check
			...
			do {
				...
				$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $alt_post_name, $post_id ) );
				++$suffix;
		 ...
		 */
		$check_sql       = "SELECT post_name FROM $jkdb->posts WHERE post_name = %s AND post_type IN ( %s, 'attachment' ) AND ID != %d AND post_parent = %d LIMIT 1";
		$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $slug, $post_type, $post_id, $post_parent ) );
		 ...
		 * @param int    $post_parent Post parent ID.
		 */
		$is_bad_hierarchical_slug = apply_filters( 'jk_unique_post_slug_is_bad_hierarchical_slug', false, $slug, $post_type, $post_parent );

		if ( $post_name_check
			|| in_array( $slug, $feeds, true ) || 'embed' === $slug
			|| preg_match( "@^($jk_rewrite->pagination_base)?\d+$@", $slug )
			|| $is_bad_hierarchical_slug
		) {
			$suffix = 2;
			do {
				$alt_post_name   = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $alt_post_name, $post_type, $post_id, $post_parent ) );
				++$suffix;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
		...
		$check_sql       = "SELECT post_name FROM $jkdb->posts WHERE post_name = %s AND post_type = %s AND ID != %d LIMIT 1";
		$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $slug, $post_type, $post_id ) );
		 ...
		 * @param string $post_type Post type.
		 */
		$is_bad_flat_slug = apply_filters( 'jk_unique_post_slug_is_bad_flat_slug', false, $slug, $post_type );
		...
		if ( $post_name_check
			...
			do {
				$alt_post_name   = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $jkdb->get_var( $jkdb->prepare( $check_sql, $alt_post_name, $post_type, $post_id ) );
				++$suffix;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
	 ...
	 */
	return apply_filters( 'jk_unique_post_slug', $slug, $post_id, $post_status, $post_type, $post_parent, $original_slug );
}
 ...
 * @return array|false|JK_Error Array of affected term IDs. JK_Error or false on failure.
 */
function jk_add_post_tags( $post_id = 0, $tags = '' ) {
	return jk_set_post_tags( $post_id, $tags, true );
}
 ...
 * @return array|false|JK_Error Array of term taxonomy IDs of affected terms. JK_Error or false on failure.
 */
function jk_set_post_tags( $post_id = 0, $tags = '', $append = false ) {
	return jk_set_post_terms( $post_id, $tags, 'post_tag', $append );
}
 ...
 * @return array|false|JK_Error Array of term taxonomy IDs of affected terms. JK_Error or false on failure.
 */
function jk_set_post_terms( $post_id = 0, $terms = '', $taxonomy = 'post_tag', $append = false ) {
	$post_id = (int) $post_id;
	...
	return jk_set_object_terms( $post_id, $terms, $taxonomy, $append );
}
 ...
 * @return array|false|JK_Error Array of term taxonomy IDs of affected categories. JK_Error or false on failure.
 */
function jk_set_post_categories( $post_id = 0, $post_categories = array(), $append = false ) {
	$post_id     = (int) $post_id;
	...
	return jk_set_post_terms( $post_id, $post_categories, 'category', $append );
}
 ...
 * @param JK_Post $post Post data.
 */
function jk_transition_post_status( $new_status, $old_status, $post ) {
	/**
 ...
 * @since 5.6.0
 *
 * @param int|JK_Post  $post        The post ID or object that has been saved.
 * @param bool         $update      Whether this is an existing post being updated.
 * @param null|JK_Post $post_before Null for new posts, the JK_Post object prior
 *                                  to the update for updated posts.
 */
function jk_after_insert_post( $post, $update, $post_before ) {
	$post = get_post( $post );
	 ...
	 *                                  to the update for updated posts.
	 */
	do_action( 'jk_after_insert_post', $post_id, $post, $update, $post_before );
}
 ...
 *
 * @since 1.5.0
 * @since 4.7.0 `$post` can be a JK_Post object.
...
function add_ping( $post, $uri ) {
	...
	$return = $jkdb->update( $jkdb->posts, array( 'pinged' => $new ), array( 'ID' => $post->ID ) );
	clean_post_cache( $post->ID );
	return $return;
}
 ...
 * @since 4.7.0 `$post` can be a JK_Post object.
 *
 * @param int|JK_Post $post Post ID or object.
...
function trackback_url_list( $tb_list, $post_id ) {
	if ( ! empty( $tb_list ) ) {
		...
		foreach ( (array) $trackback_urls as $tb_url ) {
			$tb_url = trim( $tb_url );
			trackback( $tb_url, jk_unslash( $postdata['post_title'] ), $excerpt, $post_id );
		}
	}
}
...
function get_all_page_ids() {
	...
	$page_ids = jk_cache_get( 'all_page_ids', 'posts' );
	if ( ! is_array( $page_ids ) ) {
		$page_ids = $jkdb->get_col( "SELECT ID FROM $jkdb->posts WHERE post_type = 'page'" );
		jk_cache_add( 'all_page_ids', $page_ids, 'posts' );
	}
 ...
 *
 * @param int|JK_Post $page   Page object or page ID. Passed by reference.
 * @param string      $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                            correspond to a JK_Post object, an associative array, or a numeric array,
 *                            respectively. Default OBJECT.
 * @param string      $filter Optional. How the return value should be filtered. Accepts 'raw',
 *                            'edit', 'db', 'display'. Default 'raw'.
 * @return JK_Post|array|null JK_Post or array on success, null on failure.
 */
 ...
 * @return JK_Post|array|null JK_Post (or array) on success, or null on failure.
 */
function get_page_by_path( $page_path, $output = OBJECT, $post_type = 'page' ) {
	global $jkdb;

	$last_changed = jk_cache_get_last_changed( 'posts' );

	$hash      = md5( $page_path . serialize( $post_type ) );
	$cache_key = "get_page_by_path:$hash:$last_changed";
	$cached    = jk_cache_get( $cache_key, 'post-queries' );
	if ( false !== $cached ) {
	...
	$pages = $jkdb->get_results( $sql, OBJECT_K );
	...
	jk_cache_set( $cache_key, $found_id, 'post-queries' );
 ...
 * @param JK_Post[] $pages   List of page objects from which descendants should be identified.
 * @return JK_Post[] List of page children.
 */
 ...
 * @since 2.0.0
 *
 * @param JK_Post[] $pages   Posts array (passed by reference).
 ...
 *
 * @param JK_Post|object|int $page Optional. Page ID or JK_Post object. Default is global $post.
 * @return string|false Page URI, false on error.
 */
function get_page_uri( $page = 0 ) {
	if ( ! $page instanceof JK_Post ) {
		$page = get_post( $page );
	}
	 ...
	 * @param JK_Post $page Page object.
	 */
 ...
 *
 * @since 1.5.0
 * @since 6.3.0 Use JK_Query internally.
 *
 * @param array|string $args {
 ...
 *                                      Default 'publish'.
 * }
 * @return JK_Post[]|false Array of pages (or hierarchical post type items). Boolean false if the
...
function get_pages( $args = array() ) {
	...
	$query_args = array(
		'orderby'                => 'post_title',
		'order'                  => 'ASC',
		'post__not_in'           => jk_parse_id_list( $exclude ),
		'meta_key'               => $meta_key,
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$hierarchical           = false;
		$query_args['post__in'] = jk_parse_id_list( $parsed_args['include'] );
	}

	if ( ! empty( $parsed_args['authors'] ) ) {
		$post_authors = jk_parse_list( $parsed_args['authors'] );
	 ...
	 * Additionally to `JK_Query`, it has been supporting the `post_modified_gmt` field, so this logic will translate
	 * it to `post_modified` which should result in the same order given the two dates in the fields match.
	 */
	$orderby = jk_parse_list( $parsed_args['sort_column'] );
	$orderby = array_map(
	...
	/**
	 * Filters query arguments passed to JK_Query in get_pages.
	 *
	 * @since 6.3.0
	 *
	 * @param array $query_args  Array of arguments passed to JK_Query.
	...
	if ( ! empty( $parsed_args['exclude_tree'] ) ) {
		$exclude = jk_parse_id_list( $parsed_args['exclude_tree'] );
	 ...
	 * @since 2.1.0
	 *
	 * @param JK_Post[] $pages       Array of page objects.
	 * @param array     $parsed_args Array of get_pages() arguments.
	 */
 ...
 *
 * @since 2.0.0
 * @since 4.7.0 Added the `$jk_error` parameter to allow a JK_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @see jk_insert_post()
 *
 ...
 * @param bool         $jk_error         Optional. Whether to return a JK_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|JK_Error The attachment ID on success. The value 0 or JK_Error on failure.
 */
function jk_insert_attachment( $args, $file = false, $parent_post_id = 0, $jk_error = false, $fire_after_hooks = true ) {
	$defaults = array(
		'file'        => $file,
		'post_parent' => 0,
	);

	$data = jk_parse_args( $args, $defaults );
	...
	return jk_insert_post( $data, $jk_error, $fire_after_hooks );
}
 ...
 * @return JK_Post|false|null Post data on success, false or null on failure.
 */
function jk_delete_attachment( $post_id, $force_delete = false ) {
	global $jkdb;
	...
	$meta         = jk_get_attachment_metadata( $post_id );
	$backup_sizes = get_post_meta( $post->ID, '_jk_attachment_backup_sizes', true );
	...
	/**
	 * Fires before an attachment is deleted, at the start of jk_delete_attachment().
	 *
	 * @since 2.0.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
	...
	jk_delete_object_term_relationships( $post_id, array( 'category', 'post_tag' ) );
	jk_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	$comment_ids = $jkdb->get_col( $jkdb->prepare( "SELECT comment_ID FROM $jkdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC", $post_id ) );
	foreach ( $comment_ids as $comment_id ) {
		jk_delete_comment( $comment_id, true );
	}
	...
	$post_meta_ids = $jkdb->get_col( $jkdb->prepare( "SELECT meta_id FROM $jkdb->postmeta WHERE post_id = %d ", $post_id ) );
	foreach ( $post_meta_ids as $mid ) {
		delete_metadata_by_mid( 'post', $mid );
	}
	...
	jk_delete_attachment_files( $post_id, $meta, $backup_sizes, $file );
 ...
 * @return bool True on success, false on failure.
 */
function jk_delete_attachment_files( $post_id, $meta, $backup_sizes, $file ) {
	global $jkdb;

	$uploadpath = jk_get_upload_dir();
	$deleted    = true;

	if ( ! empty( $meta['thumb'] ) ) {
		// Don't delete the thumb if another attachment uses it.
		if ( ! $jkdb->get_row( $jkdb->prepare( "SELECT meta_id FROM $jkdb->postmeta WHERE meta_key = '_jk_attachment_metadata' AND meta_value LIKE %s AND post_id <> %d", '%' . $jkdb->esc_like( $meta['thumb'] ) . '%', $post_id ) ) ) {
			$thumbfile = str_replace( jk_basename( $file ), $meta['thumb'], $file );

			if ( ! empty( $thumbfile ) ) {
				$thumbfile = path_join( $uploadpath['basedir'], $thumbfile );
				$thumbdir  = path_join( $uploadpath['basedir'], dirname( $file ) );

				if ( ! jk_delete_file_from_directory( $thumbfile, $thumbdir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
		...
		foreach ( $meta['sizes'] as $size => $sizeinfo ) {
			$intermediate_file = str_replace( jk_basename( $file ), $sizeinfo['file'], $file );

			if ( ! empty( $intermediate_file ) ) {
				$intermediate_file = path_join( $uploadpath['basedir'], $intermediate_file );

				if ( ! jk_delete_file_from_directory( $intermediate_file, $intermediate_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! empty( $meta['original_image'] ) ) {
		...
		$original_image = str_replace( jk_basename( $file ), $meta['original_image'], $file );

		if ( ! empty( $original_image ) ) {
			$original_image = path_join( $uploadpath['basedir'], $original_image );

			if ( ! jk_delete_file_from_directory( $original_image, $intermediate_dir ) ) {
				$deleted = false;
			}
		}
	}
	...
	if ( is_array( $backup_sizes ) ) {
		...
		foreach ( $backup_sizes as $size ) {
			...
			if ( ! empty( $del_file ) ) {
				...
				if ( ! jk_delete_file_from_directory( $del_file, $del_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! jk_delete_file_from_directory( $file, $uploadpath['basedir'] ) ) {
		$deleted = false;
	}
 ...
 *     @type string $file       The file path relative to `jk-content/uploads`.
 ...
 * }
 */
function jk_get_attachment_metadata( $attachment_id = 0, $unfiltered = false ) {
	...
	$data = get_post_meta( $attachment_id, '_jk_attachment_metadata', true );
	 ...
	 */
	return apply_filters( 'jk_get_attachment_metadata', $data, $attachment_id );
}
 ...
 */
function jk_update_attachment_metadata( $attachment_id, $data ) {
	$attachment_id = (int) $attachment_id;
	 ...
	 * @param int   $attachment_id Attachment post ID.
	 */
	$data = apply_filters( 'jk_update_attachment_metadata', $data, $post->ID );
	if ( $data ) {
		return update_post_meta( $post->ID, '_jk_attachment_metadata', $data );
	} else {
		return delete_post_meta( $post->ID, '_jk_attachment_metadata' );
	}
}
 ...
 */
function jk_get_attachment_url( $attachment_id = 0 ) {
	global $pagenow;
	...
	$url = '';
	// Get attached file.
	$file = get_post_meta( $post->ID, '_jk_attached_file', true );
	if ( $file ) {
		...
		if ( $uploads && false === $uploads['error'] ) {
			...
			} elseif ( str_contains( $file, 'jk-content/uploads' ) ) {
				// Get the directory name relative to the basedir (back compat for pre-2.7 uploads).
				$url = trailingslashit( $uploads['baseurl'] . '/' . _jk_get_attachment_relative_path( $file ) ) . jk_basename( $file );
			} else {
	...
	if ( is_ssl() && ! is_admin() && 'jk-login.php' !== $pagenow ) {
	 ...
	 * @param int    $attachment_id Attachment post ID.
	 */
	$url = apply_filters( 'jk_get_attachment_url', $url, $post->ID );
 ...
 */
function jk_get_attachment_caption( $post_id = 0 ) {
	$post_id = (int) $post_id;
	$post    = get_post( $post_id );
	 ...
	 * @param int    $post_id Attachment ID.
	 */
	return apply_filters( 'jk_get_attachment_caption', $caption, $post->ID );
}
 ...
 *
 * @since 2.1.0
 * @since 6.1.0 Changed to use jk_get_attachment_image_url().
 *
 * @param int $post_id Optional. Attachment ID. Default is the ID of the global `$post`.
 * @return string|false Thumbnail URL on success, false on failure.
 */
function jk_get_attachment_thumb_url( $post_id = 0 ) {
	$post_id = (int) $post_id;
	 ...
	 */
	$thumbnail_url = jk_get_attachment_image_url( $post_id, 'thumbnail' );
	 ...
	 * @param int    $post_id       Attachment ID.
	 */
	return apply_filters( 'jk_get_attachment_thumb_url', $thumbnail_url, $post_id );
}
 ...
 * @param int|JK_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool True if an accepted type or a matching file extension, false otherwise.
 */
function jk_attachment_is( $type, $post = null ) {
	$post = get_post( $post );
	...
	switch ( $type ) {
		...
		case 'audio':
			return in_array( $ext, jk_get_audio_extensions(), true );

		case 'video':
			return in_array( $ext, jk_get_video_extensions(), true );
 ...
 *
 * @since 2.1.0
 * @since 4.2.0 Modified into wrapper for jk_attachment_is() and
 *              allowed JK_Post object to be passed.
 *
 * @param int|JK_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool Whether the attachment is an image.
 */
function jk_attachment_is_image( $post = null ) {
	return jk_attachment_is( 'image', $post );
}
 ...
 */
function jk_mime_type_icon( $mime = 0, $preferred_ext = '.png' ) {
	if ( ! is_numeric( $mime ) ) {
		$icon = jk_cache_get( "mime_type_icon_$mime" );
	}
	...
	if ( empty( $icon ) ) {
		...
		$icon_files = jk_cache_get( 'icon_files' );
		...
		if ( ! is_array( $icon_files ) ) {
			 ...
			 */
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . JKINC . '/images/media' );
			...
			jk_cache_add( 'icon_files', $icon_files, 'default', 600 );
		}

		$types = array();
		// Icon jk_basename - extension = MIME wildcard.
		foreach ( $icon_files as $file => $uri ) {
			$types[ preg_replace( '/^([^.]*).*$/', '$1', jk_basename( $file ) ) ] =& $icon_files[ $file ];
		}
		...
		$matches            = jk_match_mime_types( array_keys( $types ), $post_mimes );
		...
		foreach ( $matches as $match => $wilds ) {
			foreach ( $wilds as $wild ) {
				...
				$icon = $types[ $wild ];
				if ( ! is_numeric( $mime ) ) {
					jk_cache_add( "mime_type_icon_$mime", $icon );
				}
				break 2;
			}
		}
	 ...
	 *                        the mime type.
	 */
	return apply_filters( 'jk_mime_type_icon', $icon, $mime, $post_id );
}
 ...
 * added to the post meta field ('_jk_old_slug') for storing old slugs for that
 * post.
 *
 ...
 * @param int     $post_id     Post ID.
 * @param JK_Post $post        The post object.
 * @param JK_Post $post_before The previous post object.
 */
function jk_check_for_changed_slugs( $post_id, $post, $post_before ) {
	...
	$old_slugs = (array) get_post_meta( $post_id, '_jk_old_slug' );

	// If we haven't added this old slug before, add it now.
	if ( ! empty( $post_before->post_name ) && ! in_array( $post_before->post_name, $old_slugs, true ) ) {
		add_post_meta( $post_id, '_jk_old_slug', $post_before->post_name );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $post->post_name, $old_slugs, true ) ) {
		delete_post_meta( $post_id, '_jk_old_slug', $post->post_name );
	}
}
 ...
 * added to the post meta field ('_jk_old_date') for storing old dates for that
 * post.
 *
 * The most logically usage of this function is redirecting changed post objects, so
 * that those that linked to an changed post will be redirected to the new post.
 ...
 * @param JK_Post $post_before The previous post object.
 */
function jk_check_for_changed_dates( $post_id, $post, $post_before ) {
	...
	$old_dates = (array) get_post_meta( $post_id, '_jk_old_date' );

	// If we haven't added this old date before, add it now.
	if ( ! empty( $previous_date ) && ! in_array( $previous_date, $old_dates, true ) ) {
		add_post_meta( $post_id, '_jk_old_date', $previous_date );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $new_date, $old_dates, true ) ) {
		delete_post_meta( $post_id, '_jk_old_date', $new_date );
	}
}
...
function get_posts_by_author_sql( $post_type, $full = true, $post_author = null, $public_only = false ) {
	...
	if ( null !== $post_author ) {
		$sql .= $jkdb->prepare( ' AND post_author = %d', $post_author );
	}
...
function _get_last_post_time( $timezone, $field, $post_type = 'any' ) {
	...
	$date = jk_cache_get( $key, 'timeinfo' );
	if ( false !== $date ) {
		return $date;
	}
	...
	if ( 'any' === $post_type ) {
		...
		array_walk( $post_types, array( $jkdb, 'escape_by_ref' ) );
	...
	switch ( $timezone ) {
		case 'gmt':
			$date = $jkdb->get_var( "SELECT post_{$field}_gmt FROM $jkdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
		case 'blog':
			$date = $jkdb->get_var( "SELECT post_{$field} FROM $jkdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
		case 'server':
			$add_seconds_server = gmdate( 'Z' );
			$date               = $jkdb->get_var( "SELECT DATE_ADD(post_{$field}_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $jkdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
	}

	if ( $date ) {
		jk_cache_set( $key, $date, 'timeinfo' );
 ...
 * @since 1.5.1
 *
 * @param JK_Post[] $posts Array of post objects (passed by reference).
 */
function update_post_cache( &$posts ) {
	...
	jk_cache_add_multiple( $data, 'posts' );
}
 ...
 *
 * This function not run if $_jk_suspend_cache_invalidation is not empty. See
 * jk_suspend_cache_invalidation().
 *
 * @since 2.0.0
 *
 * @global bool $_jk_suspend_cache_invalidation
 *
 * @param int|JK_Post $post Post ID or post object to remove from the cache.
 */
function clean_post_cache( $post ) {
	global $_jk_suspend_cache_invalidation;

	if ( ! empty( $_jk_suspend_cache_invalidation ) ) {
		return;
	}
	...
	jk_cache_delete( $post->ID, 'posts' );
	jk_cache_delete( 'post_parent:' . (string) $post->ID, 'posts' );
	jk_cache_delete( $post->ID, 'post_meta' );

	clean_object_term_cache( $post->ID, $post->post_type );

	jk_cache_delete( 'jk_get_archives', 'general' );
	...
	if ( 'page' === $post->post_type ) {
		jk_cache_delete( 'all_page_ids', 'posts' );
	...
	jk_cache_set_posts_last_changed();
}
 ...
 * @since 6.1.0
 *
 * @param JK_Post[] $posts Array of post objects.
 */
function update_post_author_caches( $posts ) {
	...
	$author_ids = jk_list_pluck( $posts, 'post_author' );
...
function update_post_parent_caches( $posts ) {
	$parent_ids = jk_list_pluck( $posts, 'post_parent' );
 ...
 *
 * This function will not run if $_jk_suspend_cache_invalidation is not empty.
 *
 * @since 3.0.0
 *
 * @global bool $_jk_suspend_cache_invalidation
...
function clean_attachment_cache( $id, $clean_terms = false ) {
	...
	jk_cache_delete( $id, 'posts' );
	jk_cache_delete( $id, 'post_meta' );
...
function _transition_post_status( $new_status, $old_status, $post ) {
	...
	if ( 'publish' !== $old_status && 'publish' === $new_status ) {
		...
		if ( '' === get_the_guid( $post->ID ) ) {
			$jkdb->update( $jkdb->posts, array( 'guid' => get_permalink( $post->ID ) ), array( 'ID' => $post->ID ) );
		}
	...
	if ( 'publish' === $new_status || 'publish' === $old_status ) {
		foreach ( array( 'server', 'gmt', 'blog' ) as $timezone ) {
			jk_cache_delete( "lastpostmodified:$timezone", 'timeinfo' );
			jk_cache_delete( "lastpostdate:$timezone", 'timeinfo' );
			jk_cache_delete( "lastpostdate:$timezone:{$post->post_type}", 'timeinfo' );
		}
	}

	if ( $new_status !== $old_status ) {
		jk_cache_delete( _count_posts_cache_key( $post->post_type ), 'counts' );
		jk_cache_delete( _count_posts_cache_key( $post->post_type, 'readable' ), 'counts' );
	}

	// Always clears the hook in case the post status bounced from future to draft.
	jk_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
}
 ...
 *                            jk_transition_post_status() and the default filter for _future_post_hook().
 * @param JK_Post $post       Post object.
 */
function _future_post_hook( $deprecated, $post ) {
	jk_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
	jk_schedule_single_event( strtotime( get_gmt_from_date( $post->post_date ) . ' GMT' ), 'publish_future_post', array( $post->ID ) );
}

/**
 * Hook to schedule pings and enclosures when a post is published.
 *
 * Uses XMLRPC_REQUEST and JK_IMPORTING constants.
 *
 * @since 2.3.0
 * @access private
 *
...
function _publish_post_hook( $post_id ) {
	...
	if ( ! jk_next_scheduled( 'do_pings' ) ) {
		jk_schedule_single_event( time(), 'do_pings' );
	}
}
 ...
 */
function jk_get_post_parent_id( $post = null ) {
	$post = get_post( $post );

	if ( ! $post || is_jk_error( $post ) ) {
		return false;
	}
 ...
 * to the {@see 'jk_insert_post_parent'} filter.
 *
 * @since 3.1.0
 *
 * @see jk_find_hierarchy_loop()
 *
 * @param int $post_parent ID of the parent for the post we're checking.
 * @param int $post_id     ID of the post we're checking.
 * @return int The new post_parent for the post, 0 otherwise.
 */
function jk_check_post_hierarchy_for_loops( $post_parent, $post_id ) {
	...
	$loop = jk_find_hierarchy_loop( 'jk_get_post_parent_id', $post_id, $post_parent );
	if ( ! $loop ) {
 ...
 * @since 3.1.0
 *
 * @param int|JK_Post $post         Post ID or post object where thumbnail should be attached.
...
function set_post_thumbnail( $post, $thumbnail_id ) {
	...
	if ( $post && $thumbnail_id && get_post( $thumbnail_id ) ) {
		if ( jk_get_attachment_image( $thumbnail_id, 'thumbnail' ) ) {
 ...
 * @since 3.3.0
 *
 * @param int|JK_Post $post Post ID or post object from which the thumbnail should be removed.
 * @return bool True on success, false on failure.
 */
 ...
 * @global jkdb $jkdb JKPress database abstraction object.
 */
function jk_delete_auto_drafts() {
	global $jkdb;

	// Cleanup old auto-drafts more than 7 days old.
	$old_posts = $jkdb->get_col( "SELECT ID FROM $jkdb->posts WHERE post_status = 'auto-draft' AND DATE_SUB( NOW(), INTERVAL 7 DAY ) > post_date" );
	foreach ( (array) $old_posts as $delete ) {
		// Force delete.
		jk_delete_post( $delete, true );
	}
}
 ...
 * @since 4.5.0
 *
 * @param JK_Post[] $posts Array of JK_Post objects.
 */
function jk_queue_posts_for_term_meta_lazyload( $posts ) {
	...
	if ( $prime_post_terms ) {
		foreach ( $prime_post_terms as $taxonomy => $post_ids ) {
			$cached_term_ids = jk_cache_get_multiple( $post_ids, "{$taxonomy}_relationships" );
	...
	jk_lazyload_term_meta( $term_ids );
}
...
function _update_term_count_on_transition_post_status( $new_status, $old_status, $post ) {
	...
	foreach ( (array) get_object_taxonomies( $post->post_type ) as $taxonomy ) {
		$tt_ids = jk_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'tt_ids' ) );
		jk_update_term_count( $tt_ids, $taxonomy );
	}
}
...
function _prime_post_caches( $ids, $update_term_cache = true, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_posts = $jkdb->get_results( sprintf( "SELECT $jkdb->posts.* FROM $jkdb->posts WHERE ID IN (%s)", implode( ',', $non_cached_ids ) ) );
...
function _prime_post_parent_id_caches( array $ids ) {
	...
	$cached_data = jk_cache_get_multiple( array_values( $cache_keys ), 'posts' );
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_posts = $jkdb->get_results( sprintf( "SELECT $jkdb->posts.ID, $jkdb->posts.post_parent FROM $jkdb->posts WHERE ID IN (%s)", implode( ',', $non_cached_ids ) ) );
		...
		if ( $fresh_posts ) {
			...
			jk_cache_add_multiple( $post_parent_data, 'posts' );
		}
	}
}
 ...
 */
function jk_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id = 0 ) {
	...
	if ( ! empty( $trashed_posts_with_desired_slug ) ) {
		foreach ( $trashed_posts_with_desired_slug as $_post ) {
			jk_add_trashed_suffix_to_post_name_for_post( $_post );
		}
	}
}
 ...
 * @global jkdb $jkdb JKPress database abstraction object.
 *
 * @param JK_Post $post The post.
 * @return string New slug for the post.
 */
function jk_add_trashed_suffix_to_post_name_for_post( $post ) {
	global $jkdb;
	...
	add_post_meta( $post->ID, '_jk_desired_post_slug', $post->post_name );
	$post_name = _truncate_post_slug( $post->post_name, 191 ) . '__trashed';
	$jkdb->update( $jkdb->posts, array( 'post_name' => $post_name ), array( 'ID' => $post->ID ) );
	clean_post_cache( $post->ID );
	return $post_name;
}
 ...
 * @since 5.0.0
 */
function jk_cache_set_posts_last_changed() {
	jk_cache_set_last_changed( 'posts' );
}
...
function get_available_post_mime_types( $type = 'attachment' ) {
	...
	if ( ! is_array( $mime_types ) ) {
		$mime_types = $jkdb->get_col( $jkdb->prepare( "SELECT DISTINCT post_mime_type FROM $jkdb->posts WHERE post_type = %s AND post_mime_type != ''", $type ) );
	}
 ...
 */
function jk_get_original_image_path( $attachment_id, $unfiltered = false ) {
	if ( ! jk_attachment_is_image( $attachment_id ) ) {
		return false;
	}

	$image_meta = jk_get_attachment_metadata( $attachment_id );
	 ...
	 * @param int    $attachment_id  Attachment ID.
	 */
	return apply_filters( 'jk_get_original_image_path', $original_image, $attachment_id );
}

/**
 * Retrieves the URL to an original attachment image.
 *
 * Similar to `jk_get_attachment_url()` however some images may have been
 ...
 */
function jk_get_original_image_url( $attachment_id ) {
	if ( ! jk_attachment_is_image( $attachment_id ) ) {
		return false;
	}

	$image_url = jk_get_attachment_url( $attachment_id );
	 ...
	 * @param int    $attachment_id      Attachment ID.
	 */
	return apply_filters( 'jk_get_original_image_url', $original_image_url, $attachment_id );
}

/**
 * Filters callback which sets the status of an untrashed post to its previous status.
 *
 * This can be used as a callback on the `jk_untrash_post_status` filter.
 *
 * @since 5.6.0
 *
 * @param string $new_status      The new status of the post being restored.
 ...
 */
function jk_untrash_post_set_previous_status( $new_status, $post_id, $previous_status ) {
 ...
 *
 * @since 5.0.0
 * @since 6.1.0 Moved to jk-includes from jk-admin.
 *
 * @param int|JK_Post $post Post ID or JK_Post object.
...
function use_block_editor_for_post( $post ) {
	 ...
	 * @param JK_Post $post             The post being checked.
	 */
 ...
 *
 * @since 6.3.0 Adds `jk_pattern_sync_status` meta field to the jk_block post type so an unsynced option can be added.
 *
 * @link https://github.com/JKPress/gutenberg/pull/51144
 */
function jk_create_initial_post_meta() {
	register_post_meta(
		'jk_block',
		'jk_pattern_sync_status',
		array(
