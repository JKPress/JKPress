<?php
/**
 * Widget API: JK_Widget_Custom_HTML class
 *
 * @package JKPress
 * @subpackage Widgets
 * @since 4.8.1
 ...
 * @see JK_Widget
 */
class JK_Widget_Custom_HTML extends JK_Widget {
	...
	public function _register_one( $number = -1 ) {
		 ...
		 */
		add_action( 'admin_footer-widgets.php', array( 'JK_Widget_Custom_HTML', 'render_control_template_scripts' ) );

		// Note this action is used to ensure the help text is added to the end.
		add_action( 'admin_head-widgets.php', array( 'JK_Widget_Custom_HTML', 'add_help_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in jk-includes/widgets/class-jk-widget-text.php */
		 ...
		 * @param JK_Widget_Custom_HTML $widget   Current Custom HTML widget instance.
		 */
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['content'] = jk_kses_post( $new_instance['content'] );
		}
		return $instance;
	}
	...
	public function enqueue_admin_scripts() {
		$settings = jk_enqueue_code_editor(
			array(
				'type'       => 'text/html',
				'codemirror' => array(
					'indentUnit' => 2,
		...
		jk_enqueue_script( 'custom-html-widgets' );
		jk_add_inline_script( 'custom-html-widgets', sprintf( 'jk.customHtmlWidgets.idBases.push( %s );', jk_json_encode( $this->id_base ) ) );
		...
		jk_add_inline_script( 'custom-html-widgets', sprintf( 'jk.customHtmlWidgets.init( %s );', jk_json_encode( $settings ) ), 'after' );
		...
		jk_add_inline_script( 'custom-html-widgets', sprintf( 'jQuery.extend( jk.customHtmlWidgets.l10n, %s );', jk_json_encode( $l10n ) ), 'after' );
	}
	 ...
	 *
	 * @since 4.8.1
	 * @since 4.9.0 The form contains only hidden sync inputs. For the control UI, see `JK_Widget_Custom_HTML::render_control_template_scripts()`.
	 *
	 * @see JK_Widget_Custom_HTML::render_control_template_scripts()
	 *
	 * @param array $instance Current instance.
	 */
	public function form( $instance ) {
		$instance = jk_parse_args( (array) $instance, $this->default_instance );
		?>
