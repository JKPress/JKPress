<?php
/**
 * Widget API: JK_Media_Widget class
 *
 * @package JKPress
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see JK_Widget
 */
abstract class JK_Widget_Media extends JK_Widget {
	 ...
	 * @param array  $control_options Optional. Widget control options. See jk_register_widget_control()
	 *                                for information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = jk_parse_args(
			$widget_options,
			array(
		...
		$control_opts = jk_parse_args( $control_options, array() );
	...
	public function _register_one( $number = -1 ) {
		 ...
		 * the 'admin_print_scripts-widgets.php' action in JK_Customize_Widgets::print_scripts().
		 */
		...
		/*
		 * Note that the widgets component in the customizer will also do
		 * the 'admin_footer-widgets.php' action in JK_Customize_Widgets::print_footer_scripts().
		 */
	...
	public function get_instance_schema() {
		 ...
		 * @param JK_Widget_Media $widget Widget object.
		 */
	 ...
	 * @since 4.8.0
	 *
	 * @param int|JK_Post $attachment Attachment post ID or object.
	...
	public function is_attachment_with_mime_type( $attachment, $mime_type ) {
		...
		return jk_attachment_is( $mime_type, $attachment );
	}
	 ...
	 * @since 4.8.0
	 *
	 * @see JK_Widget::widget()
	 *
	 * @param array $args     Display arguments including before_title, after_title, before_widget, and after_widget.
	 * @param array $instance Saved setting from the database.
	 */
	public function widget( $args, $instance ) {
		$instance = jk_parse_args( $instance, jk_list_pluck( $this->get_instance_schema(), 'default' ) );
		...
		/** This filter is documented in jk-includes/widgets/class-jk-widget-pages.php */
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $schema as $field => $field_schema ) {
			...
			if ( is_jk_error( $value ) ) {
				continue; // Handle case when rest_sanitize_value_from_schema() ever returns JK_Error as its phpdoc @return tag indicates.
			}
	 ...
	 * @since 4.8.0
	 *
	 * @see \JK_Widget_Media::render_control_template_scripts() Where the JS template is located.
	 *
	 * @param array $instance Current settings.
	 */
	final public function form( $instance ) {
		$instance_schema = $this->get_instance_schema();
		$instance        = jk_array_slice_assoc(
			jk_parse_args( (array) $instance, jk_list_pluck( $instance_schema, 'default' ) ),
		...
		foreach ( $instance as $name => $value ) : ?>
			<input
				...
				id="<?php echo esc_attr( $this->get_field_id( $name ) ); // Needed specifically by jkWidgets.appendTitle(). ?>"
	...
	public function enqueue_admin_scripts() {
		jk_enqueue_media();
		jk_enqueue_script( 'media-widgets' );
	}
