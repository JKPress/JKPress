<?php
/**
 * Widget API: JK_Widget_Media_Video class
 *
 * @package JKPress
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see JK_Widget
 */
class JK_Widget_Media_Video extends JK_Widget_Media {
	...
	public function __construct() {
		...
		$this->l10n = array_merge(
			...
			array(
				...
				'unsupported_file_type'      => sprintf( __( 'Sorry, the video at the supplied URL cannot be loaded. Please check that the URL is for a supported video file (%s) or stream (e.g. YouTube and Vimeo).' ), '<code>.' . implode( '</code>, <code>.', jk_get_video_extensions() ) . '</code>' ),
			)
		);
	}
	...
	public function get_instance_schema() {
		...
		$schema = array(
			...
			'content' => array(
				'type'                  => 'string',
				'default'               => '',
				'sanitize_callback'     => 'jk_kses_post',
				'description'           => __( 'Tracks (subtitles, captions, descriptions, chapters, or metadata)' ),
				'should_preview_update' => false,
			),
		);

		foreach ( jk_get_video_extensions() as $video_extension ) {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment || preg_match( $youtube_pattern, $src ) || preg_match( $vimeo_pattern, $src ) ) {
			add_filter( 'jk_video_shortcode', array( $this, 'inject_video_max_width_style' ) );

			echo jk_video_shortcode(
				array_merge(
					$instance,
					compact( 'src' )
				),
			...
			remove_filter( 'jk_video_shortcode', array( $this, 'inject_video_max_width_style' ) );
		} else {
			echo $this->inject_video_max_width_style( jk_oembed_get( $src ) );
		}
	}
	...
	public function enqueue_preview_scripts() {
		...
		if ( 'mediaelement' === apply_filters( 'jk_video_shortcode_library', 'mediaelement' ) ) {
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-jk-media-widget-video-preview">
