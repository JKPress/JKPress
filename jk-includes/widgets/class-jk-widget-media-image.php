<?php
/**
 * Widget API: JK_Widget_Media_Image class
 *
 * @package JKPress
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see JK_Widget
 */
class JK_Widget_Media_Image extends JK_Widget_Media {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment ) {
			...
			$image_attributes = array(
				'class' => sprintf( 'image jk-image-%d %s', $attachment->ID, $instance['image_classes'] ),
			...
			if ( 'custom' === $size || ! in_array( $size, array_merge( get_intermediate_image_sizes(), array( 'full' ) ), true ) ) {
				...
				$caption_size = _jk_get_image_size_from_meta( $instance['size'], jk_get_attachment_metadata( $attachment->ID ) );
			...
			$image = jk_get_attachment_image( $attachment->ID, $size, false, $image_attributes );
		...
		if ( 'file' === $instance['link_type'] ) {
			$url = $attachment ? jk_get_attachment_url( $attachment->ID ) : $instance['url'];
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-jk-media-widget-image-fields">
		...
		<script type="text/html" id="tmpl-jk-media-widget-image-preview">
