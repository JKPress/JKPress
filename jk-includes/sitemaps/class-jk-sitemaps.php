...
class JK_Sitemaps {
	 ...
	 * @since 5.5.0
	 *
	 * @var JK_Sitemaps_Renderer
	 */
	public $renderer;

	/**
	 * JK_Sitemaps constructor.
	 *
	 * @since 5.5.0
	 */
	public function __construct() {
		$this->registry = new JK_Sitemaps_Registry();
		$this->renderer = new JK_Sitemaps_Renderer();
		$this->index    = new JK_Sitemaps_Index( $this->registry );
	}
	...
	public function sitemaps_enabled() {
		 ...
		 *
		 * @see JK_Sitemaps::register_rewrites()
		 *
		 * @since 5.5.0
		 *
		 ...
		 */
		return (bool) apply_filters( 'jk_sitemaps_enabled', $is_enabled );
	}
	...
	public function register_sitemaps() {
		$providers = array(
			'posts'      => new JK_Sitemaps_Posts(),
			'taxonomies' => new JK_Sitemaps_Taxonomies(),
			'users'      => new JK_Sitemaps_Users(),
		);

		/* @var JK_Sitemaps_Provider $provider */
	...
	public function register_rewrites() {
		...
		add_rewrite_rule( '^jk-sitemap\.xml$', 'index.php?sitemap=index', 'top' );

		// Register rewrites for the XSL stylesheet.
		add_rewrite_tag( '%sitemap-stylesheet%', '([^?]+)' );
		add_rewrite_rule( '^jk-sitemap\.xsl$', 'index.php?sitemap-stylesheet=sitemap', 'top' );
		add_rewrite_rule( '^jk-sitemap-index\.xsl$', 'index.php?sitemap-stylesheet=index', 'top' );

		// Register routes for providers.
		add_rewrite_rule(
			'^jk-sitemap-([a-z]+?)-([a-z\d_-]+?)-(\d+?)\.xml$',
			'index.php?sitemap=$matches[1]&sitemap-subtype=$matches[2]&paged=$matches[3]',
			'top'
		);
		add_rewrite_rule(
			'^jk-sitemap-([a-z]+?)-(\d+?)\.xml$',
	...
	public function render_sitemaps() {
		...
		if ( $stylesheet_type ) {
			$stylesheet = new JK_Sitemaps_Stylesheet();
	...
	/**
	 * Redirects a URL to the jk-sitemap.xml
	 *
	 * @since 5.5.0
	 * @deprecated 6.7.0 Deprecated in favor of {@see JK_Rewrite::rewrite_rules()}
	 *
	 * @param bool     $bypass Pass-through of the pre_handle_404 filter value.
	 * @param JK_Query $query  The JK_Query object.
	...
	public function redirect_sitemapxml( $bypass, $query ) {
		...
		if ( 'sitemap-xml' === $query->get( 'pagename' )
			...
			jk_safe_redirect( $this->index->get_index_url() );
			exit();
		}
