<?php
/**
 * Network API: JK_Network_Query class
 *
 * @package JKPress
 ...
 * @since 4.6.0
 *
 * @see JK_Network_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class JK_Network_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query JK_Network_Query arguments. See JK_Network_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param JK_Network_Query $query The JK_Network_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_network_query', array( &$this ) );
	}

	/**
	 * Sets up the JKPress query for retrieving networks.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return array|int List of JK_Network objects, a list of network IDs when 'fields' is set to 'ids',
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param JK_Network_Query $query Current instance of JK_Network_Query (passed by reference).
		 */
		 ...
		 *
		 * Return a non-null value to bypass JKPress' default network queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of JK_Network objects.
		 *
		 * Note that if the filter returns an array of network data, it will be assigned
		 * to the `networks` property of the current JK_Network_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_networks` and `max_num_pages` properties of the JK_Network_Query object,
		 * passed to the filter by reference. If JK_Network_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.2.0
		 * @since 5.6.0 The returned array of network data is assigned to the `networks` property
		 *              of the current JK_Network_Query instance.
		 *
		 * @param array|int|null   $network_data Return an array of network data to short-circuit JK's network query,
		...
		$last_changed = jk_cache_get_last_changed( 'networks' );

		$cache_key   = "get_network_ids:$key:$last_changed";
		$cache_value = jk_cache_get( $cache_key, 'network-queries' );
		...
		if ( false === $cache_value ) {
			...
			jk_cache_add( $cache_key, $cache_value, 'network-queries' );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param JK_Network[]     $_networks An array of JK_Network objects.
		 * @param JK_Network_Query $query     Current instance of JK_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to JK_Network instances.
	...
	protected function get_network_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$jkdb->site.id IN ( " . implode( ',', jk_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$jkdb->site.id NOT IN ( " . implode( ',', jk_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $jkdb->prepare( "$jkdb->site.domain = %s", $this->query_vars['domain'] );
		}

		// Parse network domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "$jkdb->site.domain IN ( '" . implode( "', '", $jkdb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse network domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "$jkdb->site.domain NOT IN ( '" . implode( "', '", $jkdb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $jkdb->prepare( "$jkdb->site.path = %s", $this->query_vars['path'] );
		}

		// Parse network path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "$jkdb->site.path IN ( '" . implode( "', '", $jkdb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse network path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "$jkdb->site.path NOT IN ( '" . implode( "', '", $jkdb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			$this->sql_clauses['where']['search'] = $this->get_search_sql(
				...
				array( "$jkdb->site.domain", "$jkdb->site.path" )
			);
		}
		...
		$this->sql_clauses['from']    = "FROM $jkdb->site $join";
		...
		$network_ids = $jkdb->get_col( $this->request );
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param JK_Network_Query $network_query        The `JK_Network_Query` instance.
			 */
			$found_networks_query = apply_filters( 'found_networks_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_networks = (int) $jkdb->get_var( $found_networks_query );
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( 'network__in' === $orderby ) {
			...
			$parsed      = "FIELD( {$jkdb->site}.id, $network__in )";
		} elseif ( 'domain_length' === $orderby || 'path_length' === $orderby ) {
			$field  = substr( $orderby, 0, -7 );
			$parsed = "CHAR_LENGTH($jkdb->site.$field)";
		} elseif ( in_array( $orderby, $allowed_keys, true ) ) {
			$parsed = "$jkdb->site.$orderby";
		}
