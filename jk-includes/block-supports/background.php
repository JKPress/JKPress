 ...
 */
function jk_register_background_support( $block_type ) {
 ...
 */
function jk_render_background_support( $block_content, $block ) {
	...
	if (
		...
		jk_should_skip_block_supports_serialization( $block_type, 'background', 'backgroundImage' ) ||
	...
	$styles = jk_style_engine_get_styles( array( 'background' => $background_styles ) );
...
JK_Block_Supports::get_instance()->register(
	'background',
	array(
		'register_attribute' => 'jk_register_background_support',
	)
);

add_filter( 'render_block', 'jk_render_background_support', 10, 2 );
