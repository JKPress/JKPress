 ...
 */
function jk_register_shadow_support( $block_type ) {
 ...
 */
function jk_apply_shadow_support( $block_type, $block_attributes ) {
	$has_shadow_support = block_has_support( $block_type, 'shadow', false );

	if (
		! $has_shadow_support ||
		jk_should_skip_block_supports_serialization( $block_type, 'shadow' )
	) {
		return array();
	}
	...
	$attributes = array();
	$styles     = jk_style_engine_get_styles( $shadow_block_styles );
...
JK_Block_Supports::get_instance()->register(
	'shadow',
	array(
		'register_attribute' => 'jk_register_shadow_support',
		'apply'              => 'jk_apply_shadow_support',
	)
);
