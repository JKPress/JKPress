 ...
 */
function jk_get_block_style_variation_name_from_class( $class_string ) {
 ...
 */
function jk_resolve_block_style_variation_ref_values( &$variation_data, $theme_json ) {
	foreach ( $variation_data as $key => &$value ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( array_key_exists( 'ref', $value ) ) {
				...
				jk_resolve_block_style_variation_ref_values( $value, $theme_json );
			}
		}
	}
}
 ...
 */
function jk_render_block_style_variation_support_styles( $parsed_block ) {
	$classes    = $parsed_block['attrs']['className'] ?? null;
	$variations = jk_get_block_style_variation_name_from_class( $classes );
	 ...
	 * theme_json data.
	 */
	jk_resolve_block_style_variation_ref_values( $variation_data, $theme_json );

	$variation_instance = jk_unique_id( $variation . '--' );
	...
	$config = array(
		'version' => JK_Theme_JSON::LATEST_SCHEMA,
		'styles'  => array(
	...
	if ( ! is_admin() ) {
		remove_filter( 'jk_theme_json_get_style_nodes', 'jk_filter_out_block_nodes' );
	}

	// Temporarily prevent variation instance from being sanitized while processing theme.json.
	$styles_registry = JK_Block_Styles_Registry::get_instance();
	$styles_registry->register( $parsed_block['blockName'], array( 'name' => $variation_instance ) );

	$variation_theme_json = new JK_Theme_JSON( $config, 'blocks' );
	...
	jk_register_style( 'block-style-variation-styles', false, array( 'jk-block-library', 'global-styles' ) );
	jk_add_inline_style( 'block-style-variation-styles', $variation_styles );
	 ...
	 */
	_jk_array_set( $parsed_block, array( 'attrs', 'className' ), $updated_class_name );
 ...
 * @access private
 *
 * @see jk_render_block_style_variation_support_styles
 *
 ...
 */
function jk_render_block_style_variation_class_name( $block_content, $block ) {
 ...
 * @access private
 */
function jk_enqueue_block_style_variation_styles() {
	jk_enqueue_style( 'block-style-variation-styles' );
}

// Register the block support.
JK_Block_Supports::get_instance()->register( 'block-style-variation', array() );

add_filter( 'render_block_data', 'jk_render_block_style_variation_support_styles', 10, 2 );
add_filter( 'render_block', 'jk_render_block_style_variation_class_name', 10, 2 );
add_action( 'jk_enqueue_scripts', 'jk_enqueue_block_style_variation_styles', 1 );
 ...
 */
function jk_register_block_style_variations_from_theme_json_partials( $variations ) {
	if ( empty( $variations ) ) {
		return;
	}
