 ...
 */
function jk_get_elements_class_name( $block ) {
	return 'jk-elements-' . md5( serialize( $block ) );
}
 ...
 */
function jk_should_add_elements_class_name( $block, $options ) {
	...
	foreach ( $element_color_properties as $element_config ) {
		...
		foreach ( $element_config['paths'] as $path ) {
			if ( null !== _jk_array_get( $elements_style_attributes, $path, null ) ) {
				return true;
			}
		}
	}
 ...
 */
function jk_render_elements_support_styles( $parsed_block ) {
	/*
	 ...
	 *
	 * @see https://github.com/JKPress/gutenberg/pull/59535
	 *
	...
	$block_type           = JK_Block_Type_Registry::get_instance()->get_registered( $parsed_block['blockName'] );
	...
	$skip_link_color_serialization         = jk_should_skip_block_supports_serialization( $block_type, 'color', 'link' );
	$skip_heading_color_serialization      = jk_should_skip_block_supports_serialization( $block_type, 'color', 'heading' );
	$skip_button_color_serialization       = jk_should_skip_block_supports_serialization( $block_type, 'color', 'button' );
	...
	if ( ! jk_should_add_elements_class_name( $parsed_block, $options ) ) {
		return $parsed_block;
	}

	$class_name         = jk_get_elements_class_name( $parsed_block );
	...
	$element_types = array(
		'button'  => array(
			'selector' => ".$class_name .jk-element-button, .$class_name .jk-block-button__link",
			'skip'     => $skip_button_color_serialization,
		),
		'link'    => array(
			'selector'       => ".$class_name a:where(:not(.jk-element-button))",
			'hover_selector' => ".$class_name a:where(:not(.jk-element-button)):hover",
 ...
 * block's markup.
 *
 * @see jk_render_elements_support_styles
 * @since 6.6.0
 *
 ...
 */
function jk_render_elements_class_name( $block_content, $block ) {
...
add_filter( 'render_block', 'jk_render_elements_class_name', 10, 2 );
add_filter( 'render_block_data', 'jk_render_elements_support_styles', 10, 1 );
