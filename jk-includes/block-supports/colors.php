 ...
 */
function jk_register_colors_support( $block_type ) {
	$color_support = false;
 ...
 */
function jk_apply_colors_support( $block_type, $block_attributes ) {
	$color_support = isset( $block_type->supports['color'] ) ? $block_type->supports['color'] : false;

	if (
		is_array( $color_support ) &&
		jk_should_skip_block_supports_serialization( $block_type, 'color' )
	) {
		return array();
	}
	...
	// Text colors.
	if ( $has_text_colors_support && ! jk_should_skip_block_supports_serialization( $block_type, 'color', 'text' ) ) {
	...
	// Background colors.
	if ( $has_background_colors_support && ! jk_should_skip_block_supports_serialization( $block_type, 'color', 'background' ) ) {
	...
	// Gradients.
	if ( $has_gradients_support && ! jk_should_skip_block_supports_serialization( $block_type, 'color', 'gradients' ) ) {
	...
	$attributes = array();
	$styles     = jk_style_engine_get_styles( array( 'color' => $color_block_styles ), array( 'convert_vars_to_classnames' => true ) );
...
JK_Block_Supports::get_instance()->register(
	'colors',
	array(
		'register_attribute' => 'jk_register_colors_support',
		'apply'              => 'jk_apply_colors_support',
	)
);
