 ...
 */
function jk_register_position_support( $block_type ) {
 ...
 */
function jk_render_position_support( $block_content, $block ) {
	...
	$class_name      = jk_unique_id( 'jk-container-' );
	$selector        = ".$class_name";
		...
		foreach ( $sides as $side ) {
			...
			if ( null !== $side_value ) {
				...
				if (
					...
					$side_value = "calc($side_value + var(--jk-admin--admin-bar--position-offset, 0px))";
				}
	...
	if ( ! empty( $position_styles ) ) {
		...
		$content = new JK_HTML_Tag_Processor( $block_content );
		$content->next_tag();
...
JK_Block_Supports::get_instance()->register(
	'position',
	array(
		'register_attribute' => 'jk_register_position_support',
	)
);
add_filter( 'render_block', 'jk_render_position_support', 10, 2 );
