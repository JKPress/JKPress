 ...
 * @since 0.71
 *
 * @global array $jk_cockneyreplace Array of formatted entities for certain common phrases.
...
function wptexturize( $text, $reset = false ) {
	global $jk_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		...
		if ( isset( $jk_cockneyreplace ) ) {
			$cockney        = array_keys( $jk_cockneyreplace );
			$cockneyreplace = array_values( $jk_cockneyreplace );
		} else {
			/*
...
function wpautop( $text, $br = true ) {
	...
	$text = jk_replace_in_html_tags( $text, array( "\n" => ' <!-- wpnl --> ' ) );
	...
	if ( $br ) {
		...
		$text = str_replace( '<JKPreserveNewline />', "\n", $text );
	}
...
function _get_wptexturize_split_regex( $shortcode_regex = '' ) {
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with jk_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 */
function jk_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = jk_html_split( $haystack );
	$changed = false;
 ...
 */
function _jk_specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	$text = (string) $text;
	...
	if ( ! $double_encode ) {
		 ...
		 */
		$text = jk_kses_normalize_entities( $text, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _jk_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function jk_specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	$text = (string) $text;
	...
	// Match the previous behavior of _jk_specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
 ...
 */
function jk_check_invalid_utf8( $text, $strip = false ) {
	$text = (string) $text;
	...
	// phpcs:ignore JKPress.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
...
function sanitize_file_name( $filename ) {
	...
	if ( ! str_contains( $filename, '.' ) ) {
		...
		$filetype   = jk_check_filetype( 'test.' . $filename, $mime_types );
	...
	if ( count( $parts ) <= 2 ) {
		/** This filter is documented in jk-includes/formatting.php */
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = jk_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( str_contains( $content, '&#1' ) ) {
		$content = strtr( $content, $jk_htmltranswinuni );
	}
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function force_balance_tags( $text ) {
	...
	// JK bug fix for comments - in case you REALLY meant to type '< !--'.
	$text = str_replace( '< !--', '<    !--', $text );
	// JK bug fix for LOVE <3 (and other situations with '<' before a number).
	...
	// JK fix for the bug with HTML comments.
...
function _make_clickable_rel_attr( $url ) {
	$rel_parts        = array();
	$scheme           = strtolower( jk_parse_url( $url, PHP_URL_SCHEME ) );
	$nofollow_schemes = array_intersect( jk_allowed_protocols(), array( 'https', 'http' ) );

	// Apply "nofollow" to external links with qualifying URL schemes (mailto:, tel:, etc... shouldn't be followed).
	if ( ! jk_is_internal_link( $url ) && in_array( $scheme, $nofollow_schemes, true ) ) {
		$rel_parts[] = 'nofollow';
	}
 ...
 */
function jk_rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = jk_kses_hair( $matches[1], jk_allowed_protocols() );

	if ( ! empty( $atts['href'] ) && jk_is_internal_link( $atts['href']['value'] ) ) {
 ...
 */
function jk_rel_nofollow( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return jk_rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return jk_slash( $text );
}
 ...
 *
 * @since 2.3.0
 * @deprecated 5.3.0 Use jk_rel_callback()
 *
 * @param array $matches Single match.
 * @return string HTML A Element with `rel="nofollow"`.
 */
function jk_rel_nofollow_callback( $matches ) {
	return jk_rel_callback( $matches, 'nofollow' );
}
 ...
 * @return string Converted content.
 */
function jk_rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return jk_rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return jk_slash( $text );
 ...
 * @return string Converted content.
 */
function jk_targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'jk_targeted_link_rel_callback', $part );
	}
 ...
 */
function jk_targeted_link_rel_callback( $matches ) {
	_deprecated_function( __FUNCTION__, '6.7.0' );
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by jk_kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = jk_kses_hair( $link_html, jk_allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'jk_targeted_link_rel', 'noopener', $link_html );
 ...
 * @deprecated 6.7.0
 */
function jk_init_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
 ...
 * @deprecated 6.7.0
 */
function jk_remove_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="jk-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.71
 *
 * @global string|array $jk_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $jk_smiliessearch;

	if ( ! get_option( 'use_smilies' ) || empty( $jk_smiliessearch ) ) {
		// Return default text.
		return $text;
	}
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] ) {
			$content = preg_replace_callback( $jk_smiliessearch, 'translate_smiley', $content );
		}
 ...
 */
function jk_iso_descrambler( $subject ) {
	...
	return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_jk_iso_convert', $subject );
}
 ...
 */
function _jk_iso_convert( $matches ) {
...
function get_gmt_from_date( $date_string, $format = 'Y-m-d H:i:s' ) {
	$datetime = date_create( $date_string, jk_timezone() );
...
function get_date_from_gmt( $date_string, $format = 'Y-m-d H:i:s' ) {
	...
	return $datetime->setTimezone( jk_timezone() )->format( $format );
}
...
function iso8601_to_datetime( $date_string, $timezone = 'user' ) {
	...
	$datetime    = date_create( $date_string, $jk_timezone ); // Timezone is ignored if input has one.
	...
	if ( 'user' === $timezone ) {
		return $datetime->setTimezone( $jk_timezone )->format( 'Y-m-d H:i:s' );
	}
 ...
 * @param JK_Post|object|int $post Optional. JK_Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function jk_trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		/*
		 * Temporarily unhook jk_filter_content_tags() since any tags
		 * within the excerpt are stripped out. Modifying the tags here
		 * is wasteful and can lead to bugs in the image counting logic.
		 */
		$filter_image_removed = remove_filter( 'the_content', 'jk_filter_content_tags', 12 );
		 ...
		 * which is generally used for the filter callback in JKPress core.
		 */
		if ( $filter_image_removed ) {
			add_filter( 'the_content', 'jk_filter_content_tags', 12 );
		}
		...
		$text         = jk_trim_words( $text, $excerpt_length, $excerpt_more );
	 ...
	 */
	return apply_filters( 'jk_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function jk_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = jk_strip_all_tags( $text );
	$num_words     = (int) $num_words;

	if ( str_starts_with( jk_get_word_count_type(), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
	 ...
	 */
	return apply_filters( 'jk_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *
 * Usually you should prepare queries using jkdb::prepare().
 ...
 *                            Defaults to return value of jk_allowed_protocols().
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' === $_context ) {
		$url = jk_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( '/' === $url[0] ) {
		...
		if ( ! is_array( $protocols ) ) {
			$protocols = jk_allowed_protocols();
		}
		$good_protocol_url = jk_kses_bad_protocol( $url, $protocols );
...
function esc_js( $text ) {
	$safe_text = jk_check_invalid_utf8( $text );
	$safe_text = _jk_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = jk_check_invalid_utf8( $text );
	$safe_text = _jk_specialchars( $safe_text, ENT_QUOTES );
	/**
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function ( $matches ) {
			...
			if ( isset( $matches['non_cdata'] ) ) {
				...
				return _jk_specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _jk_specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
 ...
 */
function jk_make_link_relative( $link ) {
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email':
			$value = $jkdb->strip_invalid_text_for_column( $jkdb->options, 'option_value', $value );
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $jkdb->strip_invalid_text_for_column( $jkdb->options, 'option_value', jk_encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_jk_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The JKPress address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'JKLANG':
			...
			if ( ! is_multisite() && defined( 'JKLANG' ) && '' !== JKLANG && 'en_US' !== JKLANG ) {
				$allowed[] = JKLANG;
			}
 ...
 */
function jk_parse_str( $input_string, &$result ) {
	 ...
	 */
	$result = apply_filters( 'jk_parse_str', $result );
}
 ...
 * @return string Converted text.
 */
function jk_pre_kses_less_than( $content ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'jk_pre_kses_less_than_callback', $content );
}
 ...
 */
function jk_pre_kses_less_than_callback( $matches ) {
 ...
 */
function jk_pre_kses_block_attributes( $content, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `jk_kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'jk_pre_kses_block_attributes', 10 );
...
/**
 * JKPress' implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function jk_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	...
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to jk_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'jk_sprintf', $fragment, $arg );
 ...
 */
function jk_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by jk_sprintf_l().
 ...
 * @return string The excerpt.
 */
function jk_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}

	$str     = jk_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], jk_allowed_protocols(), true ) ?
			$m[3] :
			JK_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. jk_strip_all_tags() will return an empty string.
 *
 * @since 2.9.0
 *
 ...
 */
function jk_strip_all_tags( $text, $remove_breaks = false ) {
	if ( is_null( $text ) ) {
		return '';
	}
	...
	if ( ! is_scalar( $text ) ) {
		 ...
		 * jk_trigger_error() is used to trigger an E_USER_WARNING,
 ...
 * @see jk_check_invalid_utf8()
 * @see jk_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	$filtered = jk_check_invalid_utf8( $str );

	if ( str_contains( $filtered, '<' ) ) {
		$filtered = jk_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = jk_strip_all_tags( $filtered, false );
 ...
 * @return string
 */
function jk_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

// phpcs:disable JKPress.JK.CapitalPDangit.MisspelledInComment,JKPress.JK.CapitalPDangit.MisspelledInText,JKPress.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'jk_title' === $current_filter ) {
		return str_replace( 'Wordpress', 'JKPress', $text );
	}
	...
	return str_replace(
		...
		array( ' JKPress', '&#8216;JKPress', $dblq . 'JKPress', '>JKPress', '(JKPress' ),
		$text
	);
}
// phpcs:enable
 ...
 */
function jk_slash( $value ) {
	if ( is_array( $value ) ) {
		$value = array_map( 'jk_slash', $value );
	}
...
function jk_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'jk_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
 ...
 * @since 6.4.0
 */
function jk_enqueue_emoji_styles() {
	// Back-compat for plugins that disable functionality by unhooking this action.
	$action = is_admin() ? 'admin_print_styles' : 'jk_print_styles';
	...
	$handle       = 'jk-emoji-styles';
	jk_register_style( $handle, false );
	jk_add_inline_style( $handle, $emoji_styles );
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			...
			'wpemoji' => apply_filters( 'script_loader_src', includes_url( "js/jk-emoji.js?$version" ), 'wpemoji' ),
			/** This filter is documented in jk-includes/class-jk-scripts.php */
		...
		$settings['source'] = array(
			/** This filter is documented in jk-includes/class-jk-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/jk-emoji-release.min.js?$version" ), 'concatemoji' ),
		);
	}

	jk_print_inline_script_tag(
		sprintf( 'window._wpemojiSettings = %s;', jk_json_encode( $settings ) ) . "\n" .
			file_get_contents( ABSPATH . JKINC . '/js/jk-emoji-loader' . jk_scripts_get_suffix() . '.js' )
	);
}
 ...
 */
function jk_encode_emoji( $content ) {
	$emoji = _jk_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function jk_staticize_emoji( $text ) {
	if ( ! str_contains( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = jk_encode_emoji( $text );
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] && str_contains( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="jk-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 */
function jk_staticize_emoji_for_email( $mail ) {
	 ...
	 * in jk_mail() - get the `Content-Type` from the headers, if there is one,
	 * then pass it through the {@see 'jk_mail_content_type'} filter, in case
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = jk_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that JKPress recognises.
 */
function _jk_emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to jk-includes/formatting.php from jk-admin/includes/misc.php and added $length param.
 *
