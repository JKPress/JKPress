...
require_once ABSPATH . JKINC . '/class-walker-nav-menu.php';
 ...
 *
 *     @type int|string|JK_Term $menu                 Desired menu. Accepts a menu ID, slug, name, or object.
 *                                                    Default empty.
 ...
 *                                                    Default is 'jk_page_menu'. Set to false for no fallback.
 ...
 */
function jk_nav_menu( $args = array() ) {
	 ...
	 * @see jk_nav_menu()
	 *
	 * @param array $args Array of jk_nav_menu() arguments.
	 */
	$args = apply_filters( 'jk_nav_menu_args', $args );
	$args = (object) $args;

	/**
	 * Filters whether to short-circuit the jk_nav_menu() output.
	 *
	 * Returning a non-null value from the filter will short-circuit jk_nav_menu(),
	 ...
	 * @param stdClass    $args   An object containing jk_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'pre_jk_nav_menu', null, $args );
	...
	$menu = jk_get_nav_menu_object( $args->menu );

	// Get the nav menu based on the theme_location.
	$locations = get_nav_menu_locations();
	if ( ! $menu && $args->theme_location && $locations && isset( $locations[ $args->theme_location ] ) ) {
		$menu = jk_get_nav_menu_object( $locations[ $args->theme_location ] );
	}
	...
	if ( ! $menu && ! $args->theme_location ) {
		...
		foreach ( $menus as $menu_maybe ) {
			$menu_items = jk_get_nav_menu_items( $menu_maybe->term_id, array( 'update_post_term_cache' => false ) );
			if ( $menu_items ) {
				$menu = $menu_maybe;
				break;
			}
	...
	if ( $menu && ! is_jk_error( $menu ) && ! isset( $menu_items ) ) {
	 ...
	 *  - Otherwise, bail.
	 */
	if ( ( ! $menu || is_jk_error( $menu ) || ( isset( $menu_items ) && empty( $menu_items ) && ! $args->theme_location ) )
	...
	if ( $args->container ) {
		 ...
		 */
		$allowed_tags = apply_filters( 'jk_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
	 ...
	 * @param stdClass $args              An object containing jk_nav_menu() arguments.
	 */
	$sorted_menu_items = apply_filters( 'jk_nav_menu_objects', $sorted_menu_items, $args );
	 ...
	 * @param stdClass $args  An object containing jk_nav_menu() arguments.
	 */
	$items = apply_filters( 'jk_nav_menu_items', $items, $args );
	/**
	 ...
	 * @param stdClass $args  An object containing jk_nav_menu() arguments.
	 */
	$items = apply_filters( "jk_nav_menu_{$menu->slug}_items", $items, $args );
	 ...
	 * @param stdClass $args     An object containing jk_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'jk_nav_menu', $nav_menu, $args );
 ...
 */
function _jk_menu_item_classes_by_context( &$menu_items ) {
	global $jk_query, $jk_rewrite;

	$queried_object    = $jk_query->get_queried_object();
	$queried_object_id = (int) $jk_query->queried_object_id;
	...
	if ( $jk_query->is_singular && ! empty( $queried_object->post_type ) && ! is_post_type_hierarchical( $queried_object->post_type ) ) {
		foreach ( (array) get_object_taxonomies( $queried_object->post_type ) as $taxonomy ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				$term_hierarchy = _get_term_hierarchy( $taxonomy );
				$terms          = jk_get_object_terms( $queried_object_id, $taxonomy, array( 'fields' => 'ids' ) );
				if ( is_array( $terms ) ) {
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		if ( $jk_query->is_singular && 'taxonomy' === $menu_item->type
			...
			&& (
				( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
					&& $jk_query->is_home && $home_page_id === (int) $menu_item->object_id )
				|| ( 'post_type' === $menu_item->type && $jk_query->is_singular )
				|| ( 'taxonomy' === $menu_item->type
					&& ( $jk_query->is_category || $jk_query->is_tag || $jk_query->is_tax )
			...
			if ( 'post_type' === $menu_item->type && 'page' === $menu_item->object ) {
				// Back compat classes for pages to match jk_page_menu().
				$classes[] = 'page_item';
			...
			$_indexless_current = untrailingslashit( preg_replace( '/' . preg_quote( $jk_rewrite->index, '/' ) . '$/', '', $current_url ) );
			...
			if ( $raw_item_url && in_array( $item_url, $matches, true ) ) {
				...
				if ( in_array( home_url(), array( untrailingslashit( $current_url ), untrailingslashit( $_indexless_current ) ), true ) ) {
					// Back compat for home link to match jk_page_menu().
					$classes[] = 'current_page_item';
				}
				$active_parent_item_ids[]   = (int) $menu_item->menu_item_parent;
				$active_parent_object_ids[] = (int) $menu_item->post_parent;
		...
		// Back-compat with jk_page_menu(): add "current_page_parent" to static home page link for any non-page query.
		if ( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
			&& empty( $jk_query->is_page ) && $home_page_id === (int) $menu_item->object_id
		) {
 ...
 * JKPress 3.0.0 so this needs to allow for cases in which the filter is
 * called without them.
 *
 ...
 * @param JK_Post        $menu_item The current menu item object.
 * @param stdClass|false $args      An object of jk_nav_menu() arguments. Default false ($args unspecified when filter is called).
 * @param int|false      $depth     Depth of menu item. Default false ($depth unspecified when filter is called).
 * @return string[] Modified nav menu classes.
 */
function jk_nav_menu_remove_menu_item_has_children_class( $classes, $menu_item, $args = false, $depth = false ) {
	/*
