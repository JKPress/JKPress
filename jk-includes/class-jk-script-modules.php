<?php
/**
 * Script Modules API: JK_Script_Modules class.
 *
...
class JK_Script_Modules {
	 ...
	 * when the `jk_head` actions is fired, so it needs to print everything in the
	 * footer.
	 *
	 * @since 6.5.0
	 */
	public function add_hooks() {
		$position = jk_is_block_theme() ? 'jk_head' : 'jk_footer';
		...
		add_action( 'jk_footer', array( $this, 'print_script_module_data' ) );
		add_action( 'admin_print_footer_scripts', array( $this, 'print_script_module_data' ) );
		add_action( 'jk_footer', array( $this, 'print_a11y_script_module_html' ), 20 );
	...
	public function print_import_map() {
		...
		if ( ! empty( $import_map['imports'] ) ) {
			jk_print_inline_script_tag(
				jk_json_encode( $import_map, JSON_HEX_TAG | JSON_HEX_AMP ),
				array(
					'type' => 'importmap',
					'id'   => 'jk-importmap',
				)
	 ...
	 * JKPress version. If $version is set to null, no version is added.
	 ...
	 * form `jk-script-module-data-{$module_id}`.
	 */
	public function print_script_module_data(): void {
		...
		foreach ( array_keys( $modules ) as $module_id ) {
			 ...
			 * tag with an ID of the form `jk-script-module-data-{$module_id}`.
			 *
			 * The data can be read on the client with a pattern like this:
			 *
			 * Example:
			 *
			 *     const dataContainer = document.getElementById( 'jk-script-module-data-MyScriptModuleID' );
			 *     let data = {};
			 *     if ( dataContainer ) {
			 *         try {
			...
			if ( is_array( $data ) && array() !== $data ) {
				...
				jk_print_inline_script_tag(
					...
					array(
						...
						'id'   => "jk-script-module-data-{$module_id}",
					)
				);
			}
		}
