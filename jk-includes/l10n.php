 ...
 *
 * If the locale is not set already, then the JKLANG constant is used if it is
 ...
 */
function get_locale() {
	global $locale, $jk_local_package;

	if ( isset( $locale ) ) {
		/** This filter is documented in jk-includes/l10n.php */
		return apply_filters( 'locale', $locale );
	}

	if ( isset( $jk_local_package ) ) {
		$locale = $jk_local_package;
	}

	// JKLANG was defined in jk-config.
	if ( defined( 'JKLANG' ) ) {
		$locale = JKLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( jk_installing() ) {
			$ms_locale = get_site_option( 'JKLANG' );
		} else {
			$ms_locale = get_option( 'JKLANG' );
			if ( false === $ms_locale ) {
				$ms_locale = get_site_option( 'JKLANG' );
			}
		}
	...
	/**
	 * Filters the locale ID of the JKPress installation.
	 *
	 * @since 1.5.0
	 *
 ...
 * @since 4.7.0
 *
 * @param int|JK_User $user User's ID or a JK_User object. Defaults to current user.
 * @return string The locale of the user.
 */
function get_user_locale( $user = 0 ) {
	$user_object = false;

	if ( 0 === $user && function_exists( 'jk_get_current_user' ) ) {
		$user_object = jk_get_current_user();
	} elseif ( $user instanceof JK_User ) {
		$user_object = $user;
...
function determine_locale() {
	...
	if (
		isset( $GLOBALS['pagenow'] ) && 'jk-login.php' === $GLOBALS['pagenow'] &&
		( ! empty( $_GET['jk_lang'] ) || ! empty( $_COOKIE['jk_lang'] ) )
	) {
		if ( ! empty( $_GET['jk_lang'] ) ) {
			$determined_locale = sanitize_locale_name( $_GET['jk_lang'] );
		} else {
			$determined_locale = sanitize_locale_name( $_COOKIE['jk_lang'] );
		}
	} elseif (
		is_admin() ||
		( isset( $_GET['_locale'] ) && 'user' === $_GET['_locale'] && jk_is_json_request() )
	) {
		$determined_locale = get_user_locale();
	} elseif (
		( ! empty( $_REQUEST['language'] ) || isset( $GLOBALS['jk_local_package'] ) )
		&& jk_installing()
	) {
		if ( ! empty( $_REQUEST['language'] ) ) {
			$determined_locale = sanitize_locale_name( $_REQUEST['language'] );
		} else {
			$determined_locale = $GLOBALS['jk_local_package'];
		}
	}
...
function load_textdomain( $domain, $mofile, $locale = null ) {
	/** @var JK_Textdomain_Registry $jk_textdomain_registry */
	global $l10n, $l10n_unloaded, $jk_textdomain_registry;
	...
	$i18n_controller = JK_Translation_Controller::get_instance();
	...
	foreach ( $translation_files as $file ) {
		...
		if ( $success ) {
			...
			$l10n[ $domain ] = new JK_Translations( $i18n_controller, $domain );

			$jk_textdomain_registry->set( $domain, $locale, dirname( $file ) );
...
function unload_textdomain( $domain, $reloadable = false ) {
	...
	if ( ! $reloadable ) {
		JK_Translation_Controller::get_instance()->unload_textdomain( $domain );
	}
 ...
 *
 * Loads the .mo file in JK_LANG_DIR constant path from JKPress root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', JK_LANG_DIR . "/$locale.mo", $locale );

	if ( ( is_multisite() || ( defined( 'JK_INSTALLING_NETWORK' ) && JK_INSTALLING_NETWORK ) ) && ! file_exists( JK_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', JK_LANG_DIR . "/ms-$locale.mo", $locale );
		return $return;
	}

	if ( is_admin() || jk_installing() || ( defined( 'JK_REPAIRING' ) && JK_REPAIRING ) || doing_action( 'jk_maybe_auto_update' ) ) {
		load_textdomain( 'default', JK_LANG_DIR . "/admin-$locale.mo", $locale );
	}

	if ( is_network_admin() || ( defined( 'JK_INSTALLING_NETWORK' ) && JK_INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', JK_LANG_DIR . "/admin-network-$locale.mo", $locale );
	}
 ...
 *
 * @global JK_Textdomain_Registry $jk_textdomain_registry JKPress Textdomain Registry.
 * @global array<string, JK_Translations|NOOP_Translations> $l10n An array of all currently loaded text domains.
 *
 * @param string       $domain          Unique identifier for retrieving translated strings
 * @param string|false $deprecated      Optional. Deprecated. Use the $plugin_rel_path parameter instead.
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to JK_PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
 * @return bool True when textdomain is successfully loaded, false otherwise.
 */
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	/** @var JK_Textdomain_Registry $jk_textdomain_registry */
	/** @var array<string, JK_Translations|NOOP_Translations> $l10n */
	global $jk_textdomain_registry, $l10n;
	...
	if ( false !== $plugin_rel_path ) {
		$path = JK_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
	...
	$jk_textdomain_registry->set_custom_path( $domain, $path );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `JKMU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = JKMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
function load_script_textdomain( $handle, $domain = 'default', $path = '' ) {
	...
	if ( ! isset( $jk_scripts->registered[ $handle ] ) ) {
		return false;
	}
	...
	$src = $jk_scripts->registered[ $handle ]->src;

	if ( ! preg_match( '|^(https?:)?//|', $src ) && ! ( $jk_scripts->content_url && str_starts_with( $src, $jk_scripts->content_url ) ) ) {
		$src = $jk_scripts->base_url . $src;
	}

	$relative       = false;
	$languages_path = JK_LANG_DIR;

	$src_url     = jk_parse_url( $src );
	$content_url = jk_parse_url( content_url() );
	$plugins_url = jk_parse_url( plugins_url() );
	$site_url    = jk_parse_url( site_url() );
	...
	if (
		...
		/*
		 * Ensure correct languages path when using a custom `JK_PLUGIN_DIR` / `JK_PLUGIN_URL` configuration,
		...
		$languages_path = JK_LANG_DIR . '/' . $dirname;
		...
		$languages_path = JK_LANG_DIR . '/plugins';
		...
		if ( ! isset( $site_url['path'] ) ) {
			...
			// Make the src relative to the JK root.
 ...
 * the translation file from `jk-content/languages`, removing the need
...
function _load_textdomain_just_in_time( $domain ) {
	/** @var JK_Textdomain_Registry $jk_textdomain_registry */
	global $l10n_unloaded, $jk_textdomain_registry;
	...
	if ( ! $jk_textdomain_registry->has( $domain ) ) {
		return false;
	}

	$locale = determine_locale();
	$path   = $jk_textdomain_registry->get( $domain, $locale );
	if ( ! $path ) {
		return false;
	}
	...
	// Themes with their language directory outside of JK_LANG_DIR have a different file name.
 ...
 *
 * The default directory is JK_LANG_DIR.
 *
 * @since 3.0.0
...
function get_available_languages( $dir = null ) {
	global $jk_textdomain_registry;

	$languages = array();

	$path       = is_null( $dir ) ? JK_LANG_DIR : $dir;
	$lang_files = $jk_textdomain_registry->get_language_files_from_path( $path );
 ...
 *
 * Looks in the jk-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 3.7.0
 *
 ...
 */
function jk_get_installed_translations( $type ) {
	global $jk_textdomain_registry;
	...
	$dir = 'core' === $type ? JK_LANG_DIR : JK_LANG_DIR . "/$type";

	if ( ! is_dir( $dir ) ) {
		return array();
	}

	$files = $jk_textdomain_registry->get_language_files_from_path( $dir );
	if ( ! $files ) {
		return array();
	}
	...
	foreach ( $files as $file ) {
		...
		if ( str_ends_with( $file, '.mo' ) ) {
			...
			$language_data[ $textdomain ][ $language ] = jk_get_pomo_file_data( $pofile );
		} else {
			...
			$language_data[ $textdomain ][ $language ] = jk_get_l10n_php_file_data( $file );
		}
	}
 ...
 */
function jk_get_pomo_file_data( $po_file ) {
 ...
 */
function jk_get_l10n_php_file_data( $php_file ) {
 ...
 */
function jk_dropdown_languages( $args = array() ) {

	$parsed_args = jk_parse_args(
		$args,
		array(
			'id'                          => 'locale',
			'name'                        => 'locale',
...
function is_rtl() {
	global $jk_locale;
	if ( ! ( $jk_locale instanceof JK_Locale ) ) {
		return false;
	}
	return $jk_locale->is_rtl();
}
...
function switch_to_locale( $locale ) {
	/* @var JK_Locale_Switcher $jk_locale_switcher */
	global $jk_locale_switcher;

	if ( ! $jk_locale_switcher ) {
		return false;
	}

	return $jk_locale_switcher->switch_to_locale( $locale );
}
...
function switch_to_user_locale( $user_id ) {
	...
	return $jk_locale_switcher->switch_to_user_locale( $user_id );
}
...
function restore_previous_locale() {
	...
	return $jk_locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $jk_locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $jk_locale_switcher->is_switched();
}
 ...
 */
function jk_get_list_item_separator() {
	global $jk_locale;

	if ( ! ( $jk_locale instanceof JK_Locale ) ) {
		// Default value of JK_Locale::get_list_item_separator().
		/* translators: Used between list items, there is a space after the comma. */
		return __( ', ' );
	}

	return $jk_locale->get_list_item_separator();
}
 ...
 */
function jk_get_word_count_type() {
	global $jk_locale;

	if ( ! ( $jk_locale instanceof JK_Locale ) ) {
		// Default value of JK_Locale::get_word_count_type().
		return 'words';
	}

	return $jk_locale->get_word_count_type();
}
...
function has_translation( string $singular, string $textdomain = 'default', ?string $locale = null ): bool {
	return JK_Translation_Controller::get_instance()->has_translation( $singular, $textdomain, $locale );
}
