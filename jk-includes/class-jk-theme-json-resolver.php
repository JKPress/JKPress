...
class JK_Theme_JSON_Resolver {
	...
	protected static function read_json_file( $file_path ) {
		if ( $file_path ) {
			...
			$decoded_file = jk_json_file_decode( $file_path, array( 'associative' => true ) );
	...
	protected static function translate( $theme_json, $domain = 'default' ) {
		if ( null === static::$i18n_schema ) {
			$i18n_schema         = jk_json_file_decode( __DIR__ . '/theme-i18n.json' );
	 ...
	 * @since 5.8.0
	 *
	 * @return JK_Theme_JSON Entity that holds core data.
	 */
	public static function get_core_data() {
		...
		/**
		 * Filters the default data provided by JKPress for global styles & settings.
		 *
		 * @since 6.1.0
		 *
		 * @param JK_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'jk_theme_json_data_default', new JK_Theme_JSON_Data( $config, 'default' ) );

		/*
		 * Backward compatibility for extenders returning a JK_Theme_JSON_Data
		 * compatible class that is not a JK_Theme_JSON_Data object.
		 */
		if ( $theme_json instanceof JK_Theme_JSON_Data ) {
			static::$core = $theme_json->get_theme_json();
		} else {
			$config       = $theme_json->get_data();
			static::$core = new JK_Theme_JSON( $config, 'default' );
		}
	 ...
	 * }
	 * @return JK_Theme_JSON Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		$options = jk_parse_args( $options, array( 'with_supports' => true ) );

		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			$jk_theme        = jk_get_theme();
			$theme_json_file = $jk_theme->get_file_path( 'theme.json' );
			if ( is_readable( $theme_json_file ) ) {
				$theme_json_data = static::read_json_file( $theme_json_file );
				$theme_json_data = static::translate( $theme_json_data, $jk_theme->get( 'TextDomain' ) );
			} else {
				$theme_json_data = array( 'version' => JK_Theme_JSON::LATEST_SCHEMA );
			}
			...
			jk_register_block_style_variations_from_theme_json_partials( $variations );
			 ...
			 * @param JK_Theme_JSON_Data $theme_json Class to access and update the underlying data.
			 */
			$theme_json = apply_filters( 'jk_theme_json_data_theme', new JK_Theme_JSON_Data( $theme_json_data, 'theme' ) );
			...
			if ( $theme_json instanceof JK_Theme_JSON_Data ) {
				...
				$config        = $theme_json->get_data();
				static::$theme = new JK_Theme_JSON( $config );
			}

			if ( $jk_theme->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_file = $jk_theme->parent()->get_file_path( 'theme.json' );
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					$parent_theme_json_data = static::read_json_file( $parent_theme_json_file );
					$parent_theme_json_data = static::translate( $parent_theme_json_data, $jk_theme->parent()->get( 'TextDomain' ) );
					$parent_theme           = new JK_Theme_JSON( $parent_theme_json_data );
		 ...
		 */
		$theme_support_data = JK_Theme_JSON::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		if ( ! jk_theme_has_theme_json() ) {
			 ...
			 * themes. See https://github.com/JKPress/gutenberg/issues/59989.
			 */
		...
		$with_theme_supports = new JK_Theme_JSON( $theme_support_data );
	...
	public static function get_block_data() {
		...
		$config = array( 'version' => JK_Theme_JSON::LATEST_SCHEMA );
		foreach ( $blocks as $block_name => $block_type ) {
		 ...
		 * @param JK_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'jk_theme_json_data_blocks', new JK_Theme_JSON_Data( $config, 'blocks' ) );
		...
		if ( $theme_json instanceof JK_Theme_JSON_Data ) {
			...
			$config         = $theme_json->get_data();
			static::$blocks = new JK_Theme_JSON( $config, 'blocks' );
		}
	 ...
	 * @since 5.9.0
	 *
	 * @param JK_Theme $theme              The theme object. If empty, it
	 ...
	 */
	public static function get_user_data_from_jk_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		 ...
		 *
		 * Since jk_theme_has_theme_json() only supports the active
		 * theme, the extra condition for whether $theme is the active theme is
		 * present here.
		 */
		if ( $theme->get_stylesheet() === get_stylesheet() && ! jk_theme_has_theme_json() ) {
			return array();
		}

		$user_cpt         = array();
		$post_type_filter = 'jk_global_styles';
		...
		$global_style_query = new JK_Query();
		$recent_posts       = $global_style_query->query( $args );
		if ( count( $recent_posts ) === 1 ) {
			$user_cpt = get_object_vars( $recent_posts[0] );
		} elseif ( $create_post ) {
			$cpt_post_id = jk_insert_post(
				array(
					'post_content' => '{"version": ' . JK_Theme_JSON::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					'post_title'   => 'Custom Styles', // Do not make string translatable, see https://core.trac.wordpress.org/ticket/54518.
					'post_type'    => $post_type_filter,
					'post_name'    => sprintf( 'jk-global-styles-%s', urlencode( $stylesheet ) ),
					'tax_input'    => array(
						'jk_theme' => array( $stylesheet ),
					),
				),
				true
			);
			if ( ! is_jk_error( $cpt_post_id ) ) {
	 ...
	 *
	 * @return JK_Theme_JSON Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		$config   = array();
		$user_cpt = static::get_user_data_from_jk_global_styles( jk_get_theme() );
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				jk_trigger_error( __METHOD__, 'Error when decoding a theme.json schema for user data. ' . json_last_error_msg() );
				/**
				 ...
				 * @param JK_Theme_JSON_Data $theme_json Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'jk_theme_json_data_user', new JK_Theme_JSON_Data( $config, 'custom' ) );
				...
				if ( $theme_json instanceof JK_Theme_JSON_Data ) {
					...
					$config = $theme_json->get_data();
					return new JK_Theme_JSON( $config, 'custom' );
				}
			}
		...
		/** This filter is documented in jk-includes/class-jk-theme-json-resolver.php */
		$theme_json = apply_filters( 'jk_theme_json_data_user', new JK_Theme_JSON_Data( $config, 'custom' ) );
		...
		if ( $theme_json instanceof JK_Theme_JSON_Data ) {
			...
			$config       = $theme_json->get_data();
			static::$user = new JK_Theme_JSON( $config, 'custom' );
		}
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.wordpress.org/reference/classes/jk_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.wordpress.org/reference/classes/jk_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.wordpress.org/reference/classes/jk_theme_json_resolver/get_user_data/ get_user_data},
	...
	public static function get_user_global_styles_post_id() {
		...
		$user_cpt = static::get_user_data_from_jk_global_styles( jk_get_theme(), true );
	 ...
	 * @deprecated 6.2.0 Use jk_theme_has_theme_json() instead.
	 *
	 * @return bool
	 */
	public static function theme_has_support() {
		_deprecated_function( __METHOD__, '6.2.0', 'jk_theme_has_theme_json()' );

		return jk_theme_has_theme_json();
	}
	...
	public static function get_style_variations( $scope = 'theme' ) {
		...
		foreach ( $variation_files as $path => $file ) {
			...
			if ( is_array( $decoded_file ) && static::style_variation_has_scope( $decoded_file, $scope ) ) {
				$translated = static::translate( $decoded_file, jk_get_theme()->get( 'TextDomain' ) );
				$variation  = ( new JK_Theme_JSON( $translated ) )->get_raw_data();
	 ...
	 *
	 * @param JK_Theme_JSON $theme_json A theme json instance.
	 * @return array An array of resolved paths.
	 */
	public static function get_resolved_theme_uris( $theme_json ) {
		$resolved_theme_uris = array();

		if ( ! $theme_json instanceof JK_Theme_JSON ) {
		 ...
		 * See: JK_Font_Face_Resolver::to_theme_file_uri.
		 */
		$placeholder = 'file:./';
		...
		if (
			...
			$file_type          = jk_check_filetype( $background_image_url );
	 ...
	 *
	 * @param JK_Theme_JSON $theme_json A theme json instance.
	 * @return JK_Theme_JSON Theme merged with resolved paths, if any found.
	 */
	public static function resolve_theme_file_uris( $theme_json ) {
		...
		foreach ( $resolved_urls as $resolved_url ) {
			...
			_jk_array_set( $resolved_theme_json_data, $path, $resolved_url['href'] );
		}

		return new JK_Theme_JSON( $resolved_theme_json_data );
	}
	...
	private static function inject_variations_from_block_style_variation_files( $data, $variations ) {
		...
		foreach ( $variations as $variation ) {
			...
			$variation_name = $variation['slug'] ?? _jk_to_kebab_case( $variation['title'] );
			...
			foreach ( $variation['blockTypes'] as $block_type ) {
				...
				_jk_array_set( $data, $path, $variation['styles'] );
			}
		}
	...
	private static function inject_variations_from_block_styles_registry( $data ) {
		$registry = JK_Block_Styles_Registry::get_instance();
		...
		foreach ( $styles as $block_type => $variations ) {
			foreach ( $variations as $variation_name => $variation ) {
				...
				$path = array( 'styles', 'blocks', $block_type, 'variations', $variation_name );
				_jk_array_set( $data, $path, $variation['style_data'] );
			}
		}
