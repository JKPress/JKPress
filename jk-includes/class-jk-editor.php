<?php
/**
 * Facilitates adding of the JKPress editor as used on the Write and Edit screens.
 *
 * @package JKPress
 * @since 3.3.0
 *
 * Private, not included by default. See jk_editor() in jk-includes/general-template.php.
 */
...
final class _WP_Editors {
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the jk_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 */
		$settings = apply_filters( 'jk_editor_settings', $settings, $editor_id );
		...
		if ( self::$this_tinymce ) {
			if ( str_contains( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'jk_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['jk_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' jk-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : jk_default_editor();
				...
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce"' . $html_active . ' class="jk-switch-editor switch-tmce"' .
					' data-jk-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html"' . $tmce_active . ' class="jk-switch-editor switch-html"' .
					' data-jk-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'jk-core-ui jk-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="jk-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			jk_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="jk-' . $editor_id_attr . '-editor-tools" class="jk-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) ) {
					require ABSPATH . 'jk-admin/includes/media.php';
				}

				echo '<div id="jk-' . $editor_id_attr . '-media-buttons" class="jk-media-buttons">';
			...
			echo '<div class="jk-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor',
			'<div id="jk-' . $editor_id_attr . '-editor-container" class="jk-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings( $editor_id, $set ) {
		if ( empty( self::$first_init ) ) {
			if ( is_admin() ) {
				...
				add_action( 'jk_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'jk_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
				add_action( 'jk_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/jk-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in JKPress should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 * @since 5.3.0 The `$editor_id` parameter was added.
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as jk_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = realpath( JK_CONTENT_DIR . $path . '/langs/' );
				...
				if ( ! empty( $mce_external_plugins ) ) {
					$settings['external_plugins'] = jk_json_encode( $mce_external_plugins );
				}
				 ...
				 * Plugins that use jk_editor() on the front-end can decide whether to add the theme stylesheet
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			jk_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			jk_enqueue_script( 'quicktags' );
			jk_enqueue_style( 'buttons' );
		}

		if ( $default_scripts || in_array( 'wplink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			jk_enqueue_script( 'wplink' );
			jk_enqueue_script( 'jquery-ui-autocomplete' );
		}

		if ( self::$has_medialib ) {
			add_thickbox();
			jk_enqueue_script( 'media-upload' );
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'jk_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add jk-includes/css/editor.css.
		jk_enqueue_style( 'editor-buttons' );
		...
		if ( is_admin() ) {
			...
			add_action( 'jk_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
			add_action( 'jk_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			/*
			 * In production all plugins are loaded (they are in jk-editor.js.gz).
			 * The 'wpview', 'wpdialogs', and 'media' TinyMCE plugins are not initialized by default.
			 * Can be added from js by using the 'jk-before-tinymce-init' event.
			 */
		...
		window.jk = window.jk || {};
		window.jk.editor = window.jk.editor || {};
		window.jk.editor.getDefaultSettings = function() {
			return {
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.jk-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.jk-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.jk-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'jk-mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
			...
			'jk_lang_attr'                 => get_bloginfo( 'language' ),
			'jk_shortcut_labels'           => jk_json_encode( $shortcut_labels ),
		);
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/wordpress/jk-content.css?$version" );
	 ...
	 */
	public static function jk_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'jk_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return jk_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . jk_json_encode( $mce_translation ) . ");\n" .
	 ...
	 * sure that JKPress uses the uncompressed TinyMCE file if a theme is defined.
	...
	public static function force_uncompressed_tinymce() {
		...
		$jk_scripts->remove( 'jk-tinymce' );
		jk_register_tinymce_scripts( $jk_scripts, true );
	}
	...
	public static function print_tinymce_scripts() {
		...
		jk_print_scripts( array( 'jk-tinymce' ) );

		echo "<script type='text/javascript'>\n" . self::jk_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_jk_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/jk-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'jk_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			var initialize  = function() {
				...
				for ( id in tinyMCEPreInit.mceInit ) {
					...
					if (
						...
						if ( ! window.jkActiveEditor ) {
							window.jkActiveEditor = id;
						}
					}
				}
			}
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.jk-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
				} else {
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_jk_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @deprecated 4.3.0
	 */
	public static function jk_fullscreen_html() {
	 ...
	 * }
	 */
	public static function jk_link_query( $args = array() ) {
		 ...
		 *
		 * @see JK_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of JK_Query arguments.
		 */
		$query = apply_filters( 'jk_link_query_args', $query );
		 ...
		 * @param array $query  An array of JK_Query arguments.
		 */
		$results = apply_filters( 'jk_link_query', $results, $query );
	 ...
	 * @since 3.1.0
	 */
	public static function jk_link_dialog() {
		// Run once.
		...
		// `display: none` is required here, see #JK27605.
		?>
		<div id="jk-link-backdrop" style="display: none"></div>
		<div id="jk-link-wrap" class="jk-core-ui" style="display: none" role="dialog" aria-modal="true" aria-labelledby="link-modal-title">
		<form id="jk-link" tabindex="-1">
		<?php jk_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="jk-link-close"><span class="screen-reader-text">
			<?php
		...
		<div id="link-selector">
			<div id="link-options">
				...
				<div>
					...
					<input id="jk-link-url" type="text" aria-describedby="wplink-enter-url" /></label>
				</div>
				<div class="jk-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="jk-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="jk-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			...
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						...
						<input type="search" id="jk-link-search" class="link-search-field" autocomplete="off" aria-describedby="wplink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="jk-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="jk-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="jk-link-submit" name="jk-link-submit">
			</div>
		</div>
		</form>
		</div>
