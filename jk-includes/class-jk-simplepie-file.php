<?php
/**
 * Feed API: JK_SimplePie_File class
 *
 * @package JKPress
 * @subpackage Feed
 * @since 4.7.0
...
class JK_SimplePie_File extends SimplePie\File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			if ( SimplePie\Misc::get_default_useragent() !== $this->useragent ) { // Use default JK user agent unless custom has been specified.
				$args['user-agent'] = $this->useragent;
			}

			$res = jk_safe_remote_request( $url, $args );

			if ( is_jk_error( $res ) ) {
				$this->error   = 'JK HTTP Error: ' . $res->get_error_message();
				$this->success = false;

			} else {
				$this->headers = jk_remote_retrieve_headers( $res );

				/*
				 * SimplePie expects multiple headers to be stored as a comma-separated string,
				 * but `jk_remote_retrieve_headers()` returns them as an array, so they need
				 * to be converted.
				 *
				...
				$this->body        = jk_remote_retrieve_body( $res );
				$this->status_code = jk_remote_retrieve_response_code( $res );
			}
		} else {
			$this->error   = '';
			$this->success = false;
