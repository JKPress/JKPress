...
class JK_Query {
	 ...
	 *
	 * @since 3.1.0
	 * @var JK_Tax_Query|null A taxonomy query instance.
	 */
	public $tax_query;
	 ...
	 *
	 * @since 3.2.0
	 * @var JK_Meta_Query A meta query instance.
	 */
	 ...
	 *
	 * @since 3.7.0
	 * @var JK_Date_Query A date query instance.
	 */
	 ...
	 *
	 * @since 1.5.0
	 * @var JK_Term|JK_Post_Type|JK_Post|JK_User|null
	 */
	 ...
	 *
	 * The query flags are what page info JKPress was able to figure out.
	 *
	 * @since 2.0.0
	 */
	 ...
	 *     @type array           $date_query             An associative array of JK_Date_Query arguments.
	 *                                                   See JK_Date_Query::__construct().
	 *     @type int             $day                    Day of the month. Default empty. Accepts numbers 1-31.
	 *     @type bool            $exact                  Whether to search by exact keyword. Default false.
	 *     @type string          $fields                 Post fields to query for. Accepts:
	 *                                                   - '' Returns an array of complete post objects (`JK_Post[]`).
	 ...
	 *                                                   See JK_Meta_Query::__construct() for accepted values and default value.
	 ...
	 *                                                   See JK_Meta_Query::__construct() for accepted values and default value.
	 *     @type array           $meta_query             An associative array of JK_Meta_Query arguments.
	 *                                                   See JK_Meta_Query::__construct() for accepted values.
	 ...
	 *                                                   the 'jk_query_search_exclusion_prefix' filter.
	 ...
	 *     @type array           $tax_query              An associative array of JK_Tax_Query arguments.
	 *                                                   See JK_Tax_Query::__construct().
	...
	public function parse_query( $query = '' ) {
		if ( ! empty( $query ) ) {
			$this->init();
			$this->query      = jk_parse_args( $query );
		...
		if ( ( '' !== $qv['attachment'] ) || ! empty( $qv['attachment_id'] ) ) {
			...
			if ( $qv['day'] ) {
				if ( ! $this->is_date ) {
					...
					if ( $qv['monthnum'] && $qv['year'] && ! jk_checkdate( $qv['monthnum'], $qv['day'], $qv['year'], $date ) ) {
						$qv['error'] = '404';
					} else {
						$this->is_day  = true;
						$this->is_date = true;
		...
		if ( ! ( $this->is_singular || $this->is_archive || $this->is_search || $this->is_feed
				|| ( jk_is_serving_rest_request() && $this->is_main_query() )
		...
		if ( $this->is_home && 'page' === get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
			$_query = jk_parse_args( $this->query );
		...
		if ( '' !== $qv['pagename'] ) {
			...
			if ( isset( $this->queried_object_id ) && get_option( 'jk_page_for_privacy_policy' ) == $this->queried_object_id ) {
		...
		if ( $qv['page_id'] ) {
			...
			if ( get_option( 'jk_page_for_privacy_policy' ) == $qv['page_id'] ) {
				$this->is_privacy_policy = true;
			}
		}
		 ...
		 * @since 1.5.0
		 *
		 * @param JK_Query $query The JK_Query instance (passed by reference).
		 */
	...
	public function parse_tax_query( &$q ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $q[ $t->query_var ] ) ) {
				...
				if ( ! empty( $t->rewrite['hierarchical'] ) ) {
					$q[ $t->query_var ] = jk_basename( $q[ $t->query_var ] );
				}
		...
		$this->tax_query = new JK_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param JK_Query $query The JK_Query instance.
		 */
	...
	protected function parse_search( &$q ) {
		...
		/**
		 * Filters the columns to search in a JK_Query search.
		 *
		 ...
		 * @param JK_Query $query          The current JK_Query instance.
		 */
		 ...
		 */
		$exclusion_prefix = apply_filters( 'jk_query_search_exclusion_prefix', '-' );
		...
		foreach ( $q['search_terms'] as $term ) {
			...
			if ( $n && ! $exclude ) {
				$like                        = '%' . $jkdb->esc_like( $term ) . '%';
				$q['search_orderby_title'][] = $jkdb->prepare( "{$jkdb->posts}.post_title LIKE %s", $like );
			}

			$like = $n . $jkdb->esc_like( $term ) . $n;

			$search_columns_parts = array();
			foreach ( $search_columns as $search_column ) {
				$search_columns_parts[ $search_column ] = $jkdb->prepare( "({$jkdb->posts}.$search_column $like_op %s)", $like );
			}

			if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
				$search_columns_parts['attachment'] = $jkdb->prepare( "(sq1.meta_value $like_op %s)", $like );
			}
		...
		if ( ! empty( $search ) ) {
			...
			if ( ! is_user_logged_in() ) {
				$search .= " AND ({$jkdb->posts}.post_password = '') ";
			}
		}
	...
	protected function get_search_stopwords() {
		 ...
		 */
		$this->stopwords = apply_filters( 'jk_search_stopwords', $stopwords );
		return $this->stopwords;
	}
	...
	protected function parse_search_order( &$q ) {
		...
		if ( $q['search_terms_count'] > 1 ) {
			...
			if ( $like ) {
				$search_orderby .= $jkdb->prepare( "WHEN {$jkdb->posts}.post_title LIKE %s THEN 1 ", $like );
			}
			...
			if ( $like ) {
				$search_orderby .= $jkdb->prepare( "WHEN {$jkdb->posts}.post_excerpt LIKE %s THEN 4 ", $like );
				$search_orderby .= $jkdb->prepare( "WHEN {$jkdb->posts}.post_content LIKE %s THEN 5 ", $like );
			}
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			...
			case 'menu_order':
			case 'comment_count':
				$orderby_clause = "{$jkdb->posts}.{$orderby}";
				break;
			case 'rand':
				$orderby_clause = 'RAND()';
				break;
			...
			case 'post__in':
				if ( ! empty( $this->query_vars['post__in'] ) ) {
					$orderby_clause = "FIELD({$jkdb->posts}.ID," . implode( ',', array_map( 'absint', $this->query_vars['post__in'] ) ) . ')';
				}
				break;
			case 'post_parent__in':
				if ( ! empty( $this->query_vars['post_parent__in'] ) ) {
					$orderby_clause = "FIELD( {$jkdb->posts}.post_parent," . implode( ', ', array_map( 'absint', $this->query_vars['post_parent__in'] ) ) . ' )';
				}
				break;
			case 'post_name__in':
				if ( ! empty( $this->query_vars['post_name__in'] ) ) {
					...
					$orderby_clause       = "FIELD( {$jkdb->posts}.post_name," . $post_name__in_string . ' )';
				}
				break;
			default:
				if ( array_key_exists( $orderby, $meta_clauses ) ) {
					...
					$orderby_clause = "{$jkdb->posts}.post_" . sanitize_key( $orderby );
				}
	...
	public function get_posts() {
		 ...
		 * like is_main_query() test against the global $jk_query instance, not the passed one.
		 *
		 * @since 2.0.0
		 *
		 * @param JK_Query $query The JK_Query instance (passed by reference).
		 ...
		 */
		$this->allow_query_attachment_by_filename = apply_filters( 'jk_allow_query_attachment_by_filename', false );
		remove_all_filters( 'jk_allow_query_attachment_by_filename' );

		// Parse meta query.
		$this->meta_query = new JK_Meta_Query();
		...
		switch ( $q['fields'] ) {
			case 'ids':
				$fields = "{$jkdb->posts}.ID";
				break;
			case 'id=>parent':
				$fields = "{$jkdb->posts}.ID, {$jkdb->posts}.post_parent";
				break;
			default:
				$fields = "{$jkdb->posts}.*";
		}
		...
		if ( '' !== $q['menu_order'] ) {
			$where .= " AND {$jkdb->posts}.menu_order = " . $q['menu_order'];
		}
		// The "m" parameter is meant for months but accepts datetimes of varying specificity.
		if ( $q['m'] ) {
			$where .= " AND YEAR({$jkdb->posts}.post_date)=" . substr( $q['m'], 0, 4 );
			if ( strlen( $q['m'] ) > 5 ) {
				$where .= " AND MONTH({$jkdb->posts}.post_date)=" . substr( $q['m'], 4, 2 );
			}
			if ( strlen( $q['m'] ) > 7 ) {
				$where .= " AND DAYOFMONTH({$jkdb->posts}.post_date)=" . substr( $q['m'], 6, 2 );
			}
			if ( strlen( $q['m'] ) > 9 ) {
				$where .= " AND HOUR({$jkdb->posts}.post_date)=" . substr( $q['m'], 8, 2 );
			}
			if ( strlen( $q['m'] ) > 11 ) {
				$where .= " AND MINUTE({$jkdb->posts}.post_date)=" . substr( $q['m'], 10, 2 );
			}
			if ( strlen( $q['m'] ) > 13 ) {
				$where .= " AND SECOND({$jkdb->posts}.post_date)=" . substr( $q['m'], 12, 2 );
			}
		}
		...
		if ( $date_parameters ) {
			$date_query = new JK_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $q['date_query'] ) ) {
			$this->date_query = new JK_Date_Query( $q['date_query'] );
		...
		if ( '' !== $q['title'] ) {
			$where .= $jkdb->prepare( " AND {$jkdb->posts}.post_title = %s", stripslashes( $q['title'] ) );
		}

		// Parameters related to 'post_name'.
		if ( '' !== $q['name'] ) {
			$q['name'] = sanitize_title_for_query( $q['name'] );
			$where    .= " AND {$jkdb->posts}.post_name = '" . $q['name'] . "'";
		} elseif ( '' !== $q['pagename'] ) {
			...
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || empty( $page_for_posts ) || ( $reqpage != $page_for_posts ) ) {
				$q['pagename'] = sanitize_title_for_query( jk_basename( $q['pagename'] ) );
				$q['name']     = $q['pagename'];
				$where        .= " AND ({$jkdb->posts}.ID = '$reqpage')";
			...
			$q['attachment'] = sanitize_title_for_query( jk_basename( $q['attachment'] ) );
			$q['name']       = $q['attachment'];
			$where          .= " AND {$jkdb->posts}.post_name = '" . $q['attachment'] . "'";
		} elseif ( is_array( $q['post_name__in'] ) && ! empty( $q['post_name__in'] ) ) {
			$q['post_name__in'] = array_map( 'sanitize_title_for_query', $q['post_name__in'] );
			$post_name__in      = "'" . implode( "','", $q['post_name__in'] ) . "'";
			$where             .= " AND {$jkdb->posts}.post_name IN ($post_name__in)";
		}
		...
		if ( $q['p'] ) {
			$where .= " AND {$jkdb->posts}.ID = " . $q['p'];
		} elseif ( $q['post__in'] ) {
			$post__in = implode( ',', array_map( 'absint', $q['post__in'] ) );
			$where   .= " AND {$jkdb->posts}.ID IN ($post__in)";
		} elseif ( $q['post__not_in'] ) {
			$post__not_in = implode( ',', array_map( 'absint', $q['post__not_in'] ) );
			$where       .= " AND {$jkdb->posts}.ID NOT IN ($post__not_in)";
		}

		if ( is_numeric( $q['post_parent'] ) ) {
			$where .= $jkdb->prepare( " AND {$jkdb->posts}.post_parent = %d ", $q['post_parent'] );
		} elseif ( $q['post_parent__in'] ) {
			$post_parent__in = implode( ',', array_map( 'absint', $q['post_parent__in'] ) );
			$where          .= " AND {$jkdb->posts}.post_parent IN ($post_parent__in)";
		} elseif ( $q['post_parent__not_in'] ) {
			$post_parent__not_in = implode( ',', array_map( 'absint', $q['post_parent__not_in'] ) );
			$where              .= " AND {$jkdb->posts}.post_parent NOT IN ($post_parent__not_in)";
		}

		if ( $q['page_id'] ) {
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || ( get_option( 'page_for_posts' ) != $q['page_id'] ) ) {
				$q['p'] = $q['page_id'];
				$where  = " AND {$jkdb->posts}.ID = " . $q['page_id'];
			}
		}
		...
		if ( ! $q['suppress_filters'] ) {
			/**
			 * Filters the search SQL that is used in the WHERE clause of JK_Query.
			 *
			 * @since 3.0.0
			 *
			 * @param string   $search Search SQL for WHERE clause.
			 * @param JK_Query $query  The current JK_Query object.
			 */
		...
		if ( ! $this->is_singular ) {
			...
			$clauses = $this->tax_query->get_sql( $jkdb->posts, 'ID' );
		...
		if ( ! empty( $q['author__not_in'] ) ) {
			...
			$where         .= " AND {$jkdb->posts}.post_author NOT IN ($author__not_in) ";
		} elseif ( ! empty( $q['author__in'] ) ) {
			$author__in = implode( ',', array_map( 'absint', array_unique( (array) $q['author__in'] ) ) );
			$where     .= " AND {$jkdb->posts}.post_author IN ($author__in) ";
		}
		...
		if ( '' !== $q['author_name'] ) {
			...
			$whichauthor .= " AND ({$jkdb->posts}.post_author = " . absint( $q['author'] ) . ')';
		}
		...
		if ( isset( $q['comment_count'] ) ) {
			...
			if ( isset( $q['comment_count']['value'] ) ) {
				...
				$where .= $jkdb->prepare( " AND {$jkdb->posts}.comment_count {$q['comment_count']['compare']} %d", $q['comment_count']['value'] );
			}
		}
		...
		if ( isset( $q['post_mime_type'] ) && '' !== $q['post_mime_type'] ) {
			$whichmimetype = jk_post_mime_type_where( $q['post_mime_type'], $jkdb->posts );
		}
		$where .= $search . $whichauthor . $whichmimetype;

		if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
			$join .= " LEFT JOIN {$jkdb->postmeta} AS sq1 ON ( {$jkdb->posts}.ID = sq1.post_id AND sq1.meta_key = '_jk_attached_file' )";
		}

		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'post', $jkdb->posts, 'ID', $this );
			$join   .= $clauses['join'];
			$where  .= $clauses['where'];
		}
		...
		if ( empty( $q['orderby'] ) ) {
			...
			if ( isset( $q['orderby'] ) && ( is_array( $q['orderby'] ) || false === $q['orderby'] ) ) {
				...
				$orderby = "{$jkdb->posts}.post_date " . $q['order'];
			}
		...
		if ( isset( $q['post_password'] ) ) {
			$where .= $jkdb->prepare( " AND {$jkdb->posts}.post_password = %s", $q['post_password'] );
			if ( empty( $q['perm'] ) ) {
				$q['perm'] = 'readable';
			}
		} elseif ( isset( $q['has_password'] ) ) {
			$where .= sprintf( " AND {$jkdb->posts}.post_password %s ''", $q['has_password'] ? '!=' : '=' );
		}

		if ( ! empty( $q['comment_status'] ) ) {
			$where .= $jkdb->prepare( " AND {$jkdb->posts}.comment_status = %s ", $q['comment_status'] );
		}

		if ( ! empty( $q['ping_status'] ) ) {
			$where .= $jkdb->prepare( " AND {$jkdb->posts}.ping_status = %s ", $q['ping_status'] );
		}
		...
		if ( 'any' === $post_type ) {
			...
			if ( empty( $in_search_post_types ) ) {
				...
				$post_type_where = " AND {$jkdb->posts}.post_type IN ('" . implode( "', '", array_map( 'esc_sql', $in_search_post_types ) ) . "')";
			}
		} elseif ( ! empty( $post_type ) && is_array( $post_type ) ) {
			// Sort post types to ensure same cache key generation.
			sort( $post_type );
			$post_type_where = " AND {$jkdb->posts}.post_type IN ('" . implode( "', '", esc_sql( $post_type ) ) . "')";
		} elseif ( ! empty( $post_type ) ) {
			$post_type_where  = $jkdb->prepare( " AND {$jkdb->posts}.post_type = %s", $post_type );
			$post_type_object = get_post_type_object( $post_type );
		} elseif ( $this->is_attachment ) {
			$post_type_where  = " AND {$jkdb->posts}.post_type = 'attachment'";
			$post_type_object = get_post_type_object( 'attachment' );
		} elseif ( $this->is_page ) {
			$post_type_where  = " AND {$jkdb->posts}.post_type = 'page'";
			$post_type_object = get_post_type_object( 'page' );
		} else {
			$post_type_where  = " AND {$jkdb->posts}.post_type = 'post'";
		...
		if ( $skip_post_status ) {
			...
			if ( in_array( 'any', $q_status, true ) ) {
				foreach ( get_post_stati( array( 'exclude_from_search' => true ) ) as $status ) {
					if ( ! in_array( $status, $q_status, true ) ) {
						$e_status[] = "{$jkdb->posts}.post_status <> '$status'";
					}
				}
			} else {
				foreach ( get_post_stati() as $status ) {
					if ( in_array( $status, $q_status, true ) ) {
						if ( 'private' === $status ) {
							$p_status[] = "{$jkdb->posts}.post_status = '$status'";
						} else {
							$r_status[] = "{$jkdb->posts}.post_status = '$status'";
						}
					}
				}
			}
			...
			if ( ! empty( $r_status ) ) {
				if ( ! empty( $q['perm'] ) && 'editable' === $q['perm'] && ! current_user_can( $edit_others_cap ) ) {
					$statuswheres[] = "({$jkdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $r_status ) . '))';
				} else {
			...
			if ( ! empty( $p_status ) ) {
				if ( ! empty( $q['perm'] ) && 'readable' === $q['perm'] && ! current_user_can( $read_private_cap ) ) {
					$statuswheres[] = "({$jkdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $p_status ) . '))';
				} else {
					$statuswheres[] = '(' . implode( ' OR ', $p_status ) . ')';
				}
			}
			if ( $post_status_join ) {
				$join .= " LEFT JOIN {$jkdb->posts} AS p2 ON ({$jkdb->posts}.post_parent = p2.ID) ";
				foreach ( $statuswheres as $index => $statuswhere ) {
					$statuswheres[ $index ] = "($statuswhere OR ({$jkdb->posts}.post_status = 'inherit' AND " . str_replace( $jkdb->posts, 'p2', $statuswhere ) . '))';
				}
			}
			...
			if ( ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					$type_where = '(' . $jkdb->prepare( "{$jkdb->posts}.post_type = %s AND (", $queried_post_type );

					// Public statuses.
					$public_statuses = get_post_stati( array( 'public' => true ) );
					$status_clauses  = array();
					foreach ( $public_statuses as $public_status ) {
						$status_clauses[] = "{$jkdb->posts}.post_status = '$public_status'";
					}
					...
					if ( $this->is_admin ) {
						...
						foreach ( $admin_all_statuses as $admin_all_status ) {
							$type_where .= " OR {$jkdb->posts}.post_status = '$admin_all_status'";
						}
					}

					// Add private states that are visible to current user.
					if ( is_user_logged_in() && $queried_post_type_object instanceof JK_Post_Type ) {
						...
						foreach ( $private_statuses as $private_status ) {
							$type_where .= current_user_can( $read_private_cap ) ? " \nOR {$jkdb->posts}.post_status = '$private_status'" : " \nOR ({$jkdb->posts}.post_author = $user_id AND {$jkdb->posts}.post_status = '$private_status')";
						}
					}
		...
		if ( $this->is_comment_feed && ! $this->is_singular ) {
			if ( $this->is_archive || $this->is_search ) {
				$cjoin    = "JOIN {$jkdb->posts} ON ( {$jkdb->comments}.comment_post_ID = {$jkdb->posts}.ID ) $join ";
				$cwhere   = "WHERE comment_approved = '1' $where";
				$cgroupby = "{$jkdb->comments}.comment_id";
			} else { // Other non-singular, e.g. front.
				$cjoin    = "JOIN {$jkdb->posts} ON ( {$jkdb->comments}.comment_post_ID = {$jkdb->posts}.ID )";
			...
			$comments_request = "SELECT $distinct {$jkdb->comments}.comment_ID FROM {$jkdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$key          = md5( $comments_request );
			$last_changed = jk_cache_get_last_changed( 'comment' ) . ':' . jk_cache_get_last_changed( 'posts' );

			$cache_key   = "comment_feed:$key:$last_changed";
			$comment_ids = jk_cache_get( $cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $jkdb->get_col( $comments_request );
				jk_cache_add( $cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );

			// Convert to JK_Comment.
			/** @var JK_Comment[] */
			...
			$join     = '';
			if ( $post_ids ) {
				$where = "AND {$jkdb->posts}.ID IN ($post_ids) ";
			} else {
				$where = 'AND 0';
			}
		}
		 ...
		 * See https://github.com/JKPress/wordpress-develop/pull/6393#issuecomment-2088217429
		 */
		$old_request =
			"SELECT $found_rows $distinct $fields
					 FROM {$jkdb->posts} $join
					 WHERE 1=1 $where
					 $groupby
					 $orderby
					 $limits";
		 ...
		 *
		 * Return a non-null value to bypass JKPress' default post queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_posts` and `max_num_pages` properties of the JK_Query object,
		 * passed to the filter by reference. If JK_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 4.6.0
		 *
		 * @param JK_Post[]|int[]|null $posts Return an array of post data to short-circuit JK's query,
		 *                                    or null to allow JK to run its normal queries.
		 * @param JK_Query             $query The JK_Query instance (passed by reference).
		 */
		...
		$cacheable_field_values = array(
			"{$jkdb->posts}.*",
			"{$jkdb->posts}.ID, {$jkdb->posts}.post_parent",
			"{$jkdb->posts}.ID",
		);
		...
		if ( $q['cache_results'] && $id_query_is_cacheable ) {
			$new_request = str_replace( $fields, "{$jkdb->posts}.*", $this->request );
			$cache_key   = $this->generate_cache_key( $q, $new_request );

			$cache_found = false;
			if ( null === $this->posts ) {
				$cached_results = jk_cache_get( $cache_key, 'post-queries', false, $cache_found );
				...
				if ( $cached_results ) {
					...
					if ( 'ids' === $q['fields'] ) {
						...
						/** @var int[] */
						$post_parents = jk_cache_get_multiple( $post_parent_cache_keys, 'posts' );
		...
		if ( 'ids' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $jkdb->get_col( $this->request );
			}
			...
			if ( $q['cache_results'] && $id_query_is_cacheable ) {
				...
				jk_cache_set( $cache_key, $cache_value, 'post-queries' );
			}
		...
		if ( 'id=>parent' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $jkdb->get_results( $this->request );
			}
			...
			jk_cache_add_multiple( $post_parents_cache, 'posts' );
		...
		$is_unfiltered_query = $old_request == $this->request && "{$jkdb->posts}.*" === $fields;

		if ( null === $this->posts ) {
			$split_the_query = (
				$is_unfiltered_query
				&& (
					jk_using_ext_object_cache()
			...
			if ( $split_the_query ) {
				...
				$this->request =
					"SELECT $found_rows $distinct {$jkdb->posts}.ID
					 FROM {$jkdb->posts} $join
					 WHERE 1=1 $where
					 $groupby
					 $orderby
				...
				$post_ids = $jkdb->get_col( $this->request );
		...
		if ( $q['cache_results'] && $id_query_is_cacheable && ! $cache_found ) {
			$post_ids = jk_list_pluck( $this->posts, 'ID' );
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			/** This filter is documented in jk-includes/query.php */
			...
			$comments_request = "SELECT {$jkdb->comments}.comment_ID FROM {$jkdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$comment_key          = md5( $comments_request );
			$comment_last_changed = jk_cache_get_last_changed( 'comment' );

			$comment_cache_key = "comment_feed:$comment_key:$comment_last_changed";
			$comment_ids       = jk_cache_get( $comment_cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $jkdb->get_col( $comments_request );
				jk_cache_add( $comment_cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param JK_Post  $post_preview  The Post object.
				 * @param JK_Query $query         The JK_Query instance (passed by reference).
				 */
		 ...
		 * of the type JK_Post and are filtered.
		 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
		...
		if ( $q['lazy_load_term_meta'] ) {
			jk_queue_posts_for_term_meta_lazyload( $this->posts );
		}
	...
	private function set_found_posts( $q, $limits ) {
		...
		if ( ! empty( $limits ) ) {
			...
			$this->found_posts = (int) $jkdb->get_var( $found_posts_query );
		} else {
	...
	/**
	 * Iterates current comment index and returns JK_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return JK_Comment Comment object.
	 */
	...
	/**
	 * Sets up the JKPress query by parsing query string.
	 *
	 * @since 1.5.0
	 *
	 * @see JK_Query::parse_query() for all available arguments.
	 *
	 ...
	 * @since 1.5.0
	 *
	 * @return JK_Term|JK_Post_Type|JK_Post|JK_User|null The queried object.
	 */
	public function get_queried_object() {
		...
		if ( $this->is_category || $this->is_tag || $this->is_tax ) {
			...
			if ( ! empty( $term ) && ! is_jk_error( $term ) ) {
	 ...
	 * Constructor.
	 *
	 * Sets up the JKPress query, if parameter is not empty.
	 *
	 * @since 1.5.0
	 *
	 * @see JK_Query::parse_query() for all available arguments.
	 ...
	 * @since 3.1.0
	 *
	 * @see JK_Query::is_category()
	 * @see JK_Query::is_tag()
	 * @see JK_Query::is_author()
	 * @see JK_Query::is_date()
	 * @see JK_Query::is_post_type_archive()
	 * @see JK_Query::is_tax()
	 *
	...
	public function is_tax( $taxonomy = '', $term = '' ) {
		...
		$tax_array      = array_intersect( array_keys( $jk_taxonomies ), (array) $taxonomy );
	 ...
	 *
	 * Otherwise the same as {@see JK_Query::is_home()}.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * Depends on the site's "Change your Privacy Policy page" Privacy Settings 'jk_page_for_privacy_policy'.
	 *
	...
	public function is_privacy_policy() {
		if ( get_option( 'jk_page_for_privacy_policy' )
			&& $this->is_page( get_option( 'jk_page_for_privacy_policy' ) )
		) {
			return true;
		} else {
			return false;
	 ...
	 * @since 3.1.0
	 *
	 * @see JK_Query::is_single()
	 * @see JK_Query::is_singular()
	 *
	 ...
	 * @global int     $numpages
	 *
	 * @param JK_Post|object|int $post JK_Post instance or Post ID/object.
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param JK_Post  $post  The Post object (passed by reference).
		 * @param JK_Query $query The current Query object (passed by reference).
		 */
	...
	public function generate_postdata( $post ) {
		...
		if ( str_contains( $content, '<!--nextpage-->' ) ) {
			...
			$content = str_replace( '<!-- jk:nextpage -->', '', $content );
			$content = str_replace( '<!-- /jk:nextpage -->', '', $content );
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$placeholder = $jkdb->placeholder_escape();
		array_walk_recursive(
			$args,
			/*
			 * Replace jkdb placeholders with the string used in the database
			 ...
			 */
			static function ( &$value ) use ( $jkdb, $placeholder ) {
				if ( is_string( $value ) && str_contains( $value, $placeholder ) ) {
					$value = $jkdb->remove_placeholder_escape( $value );
				}
			}
		);
		...
		// Replace jkdb placeholder in the SQL statement used by the cache key.
		$sql = $jkdb->remove_placeholder_escape( $sql );
		...
		return "jk_query:$key:$last_changed";
	}
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See jk_queue_posts_for_term_meta_lazyload().
	 *
	 * @param mixed $check
	 * @param int   $term_id
	 * @return mixed
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See jk_lazyload_comment_meta().
	 *
	 * @param mixed $check
