 ...
 * @since 4.7.0
 *
 * @global array $_jk_additional_image_sizes
 *
 * @return array Additional images size data.
 */
function jk_get_additional_image_sizes() {
	global $_jk_additional_image_sizes;

	if ( ! $_jk_additional_image_sizes ) {
		$_jk_additional_image_sizes = array();
	}

	return $_jk_additional_image_sizes;
}
...
function image_constrain_size_for_editor( $width, $height, $size = 'medium', $context = null ) {
	...
	$_jk_additional_image_sizes = jk_get_additional_image_sizes();
	...
	} elseif ( ! empty( $_jk_additional_image_sizes ) && in_array( $size, array_keys( $_jk_additional_image_sizes ), true ) ) {
		$max_width  = (int) $_jk_additional_image_sizes[ $size ]['width'];
		$max_height = (int) $_jk_additional_image_sizes[ $size ]['height'];
	...
	return jk_constrain_dimensions( $width, $height, $max_width, $max_height );
}
...
function image_downsize( $id, $size = 'medium' ) {
	$is_image = jk_attachment_is_image( $id );
	...
	$img_url          = jk_get_attachment_url( $id );
	$meta             = jk_get_attachment_metadata( $id );
	$width            = 0;
	$height           = 0;
	$is_intermediate  = false;
	$img_url_basename = jk_basename( $img_url );
	...
	if ( $intermediate ) {
		...
		$thumbfile = str_replace( jk_basename( $imagefile ), jk_basename( $meta['thumb'] ), $imagefile );

		if ( file_exists( $thumbfile ) ) {
			$info = jk_getimagesize( $thumbfile );

			if ( $info ) {
				$img_url         = str_replace( $img_url_basename, jk_basename( $thumbfile ), $img_url );
				$width           = $info[0];
				$height          = $info[1];
				$is_intermediate = true;
			}
 ...
 * @since 2.9.0
 *
 * @global array $_jk_additional_image_sizes Associative array of additional image sizes.
 *
...
function add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	...
	$_jk_additional_image_sizes[ $name ] = array(
		'width'  => absint( $width ),
...
function has_image_size( $name ) {
	$sizes = jk_get_additional_image_sizes();
...
function remove_image_size( $name ) {
	...
	if ( isset( $_jk_additional_image_sizes[ $name ] ) ) {
		unset( $_jk_additional_image_sizes[ $name ] );
		return true;
	}
 ...
 * parameters are: what JKPress will use for the class, the Attachment ID,
...
function get_image_tag( $id, $alt, $title, $align, $size = 'medium' ) {
	...
	$class      = 'align' . esc_attr( $align ) . ' size-' . esc_attr( $size_class ) . ' jk-image-' . $id;
 ...
 * }
 */
function jk_constrain_dimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
	if ( ! $max_width && ! $max_height ) {
	 ...
	 */
	return apply_filters( 'jk_constrain_dimensions', array( $w, $h ), $current_width, $current_height, $max_width, $max_height );
}

/**
 * Retrieves calculated resize dimensions for use in JK_Image_Editor.
 *
...
function image_resize_dimensions( $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = jk_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}

	if ( jk_fuzzy_number_match( $new_w, $orig_w ) && jk_fuzzy_number_match( $new_h, $orig_h ) ) {
		 ...
		 */
		$proceed = (bool) apply_filters( 'jk_image_resize_identical_dimensions', false, $orig_w, $orig_h );
...
function image_make_intermediate_size( $file, $width, $height, $crop = false ) {
	if ( $width || $height ) {
		$editor = jk_get_image_editor( $file );

		if ( is_jk_error( $editor ) || is_jk_error( $editor->resize( $width, $height, $crop ) ) ) {
			return false;
		}

		$resized_file = $editor->save();

		if ( ! is_jk_error( $resized_file ) && $resized_file ) {
 ...
 */
function jk_image_matches_ratio( $source_width, $source_height, $target_width, $target_height ) {
	/*
	 * To test for varying crops, we constrain the dimensions of the larger image
	 * to the dimensions of the smaller image and see if they match.
	 */
	if ( $source_width > $target_width ) {
		$constrained_size = jk_constrain_dimensions( $source_width, $source_height, $target_width );
		$expected_size    = array( $target_width, $target_height );
	} else {
		$constrained_size = jk_constrain_dimensions( $target_width, $target_height, $source_width );
		$expected_size    = array( $source_width, $source_height );
	}

	// If the image dimensions are within 1px of the expected size, we consider it a match.
	$matched = ( jk_fuzzy_number_match( $constrained_size[0], $expected_size[0] ) && jk_fuzzy_number_match( $constrained_size[1], $expected_size[1] ) );
...
function image_get_intermediate_size( $post_id, $size = 'thumbnail' ) {
	$imagedata = jk_get_attachment_metadata( $post_id );
		...
		foreach ( $imagedata['sizes'] as $_size => $data ) {
			...
			if ( $data['width'] >= $size[0] && $data['height'] >= $size[1] ) {
				...
				if ( 0 === $size[0] || 0 === $size[1] ) {
					$same_ratio = jk_image_matches_ratio( $data['width'], $data['height'], $imagedata['width'], $imagedata['height'] );
				} else {
					$same_ratio = jk_image_matches_ratio( $data['width'], $data['height'], $size[0], $size[1] );
				}
	...
	if ( empty( $data['path'] ) && ! empty( $data['file'] ) && ! empty( $imagedata['file'] ) ) {
		$file_url     = jk_get_attachment_url( $post_id );
...
function get_intermediate_image_sizes() {
	...
	$additional_sizes = jk_get_additional_image_sizes();
 ...
 *
 * @since 5.3.0
 * @uses jk_get_additional_image_sizes()
 * @uses get_intermediate_image_sizes()
 *
 * @return array[] Associative array of arrays of image sub-size information,
 *                 keyed by image size name.
 */
function jk_get_registered_image_subsizes() {
	$additional_sizes = jk_get_additional_image_sizes();
	$all_sizes        = array();
 ...
 * }
 */
function jk_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	...
	if ( ! $image ) {
		...
		if ( $icon ) {
			$src = jk_mime_type_icon( $attachment_id, '.svg' );

			if ( $src ) {
				/** This filter is documented in jk-includes/post.php */
				$icon_dir = apply_filters( 'icon_dir', ABSPATH . JKINC . '/images/media' );

				$src_file = $icon_dir . '/' . jk_basename( $src );

				list( $width, $height ) = jk_getimagesize( $src_file );
	 ...
	 */
	return apply_filters( 'jk_get_attachment_image_src', $image, $attachment_id, $size, $icon );
}
 ...
 *                                       Default determined by {@see jk_get_loading_optimization_attributes()}.
 ...
 */
function jk_get_attachment_image( $attachment_id, $size = 'thumbnail', $icon = false, $attr = '' ) {
	$html  = '';
	$image = jk_get_attachment_image_src( $attachment_id, $size, $icon );
	...
	if ( $image ) {
		...
		$default_attr = array(
			...
			'alt'   => trim( strip_tags( get_post_meta( $attachment_id, '_jk_attachment_image_alt', true ) ) ),
		);

		/**
		 * Filters the context in which jk_get_attachment_image() is used.
		 *
		 * @since 6.3.0
		 *
		 * @param string $context The context. Default 'jk_get_attachment_image'.
		 */
		$context = apply_filters( 'jk_get_attachment_image_context', 'jk_get_attachment_image' );
		$attr    = jk_parse_args( $attr, $default_attr );

		$loading_attr              = $attr;
		$loading_attr['width']     = $width;
		$loading_attr['height']    = $height;
		$loading_optimization_attr = jk_get_loading_optimization_attributes(
			'img',
			$loading_attr,
			$context
		);
		...
		if ( empty( $attr['srcset'] ) ) {
			...
			if ( is_array( $image_meta ) ) {
				...
				$srcset     = jk_calculate_image_srcset( $size_array, $src, $image_meta, $attachment_id );
				$sizes      = jk_calculate_image_sizes( $size_array, $src, $image_meta, $attachment_id );
		...
		$add_auto_sizes = apply_filters( 'jk_img_tag_add_auto_sizes', true );
		...
		if (
			...
			'lazy' === $attr['loading'] &&
			isset( $attr['sizes'] ) &&
			! jk_sizes_attribute_includes_valid_auto( $attr['sizes'] )
		) {
		 ...
		 *                                 See jk_get_attachment_image().
		 * @param JK_Post      $attachment Image attachment post.
		 * @param string|int[] $size       Requested image size. Can be any registered image size name, or
		 *                                 an array of width and height values in pixels (in that order).
		 */
		$attr = apply_filters( 'jk_get_attachment_image_attributes', $attr, $attachment, $size );
	 ...
	 *                                    See jk_get_attachment_image().
	 */
	return apply_filters( 'jk_get_attachment_image', $html, $attachment_id, $size, $icon, $attr );
}
 ...
 */
function jk_get_attachment_image_url( $attachment_id, $size = 'thumbnail', $icon = false ) {
	$image = jk_get_attachment_image_src( $attachment_id, $size, $icon );
 ...
 */
function _jk_get_attachment_relative_path( $file ) {
	...
	if ( str_contains( $dirname, 'jk-content/uploads' ) ) {
		// Get the directory name relative to the upload directory (back compat for pre-2.7 uploads).
		$dirname = substr( $dirname, strpos( $dirname, 'jk-content/uploads' ) + 18 );
 ...
 * }
 */
function _jk_get_image_size_from_meta( $size_name, $image_meta ) {
	if ( 'full' === $size_name ) {
		return array(
 ...
 * @since 4.4.0
 *
 * @see jk_calculate_image_srcset()
 *
 * @param int          $attachment_id Image attachment ID.
 * @param string|int[] $size          Optional. Image size. Accepts any registered image size name, or an array of
 *                                    width and height values in pixels (in that order). Default 'medium'.
 * @param array|null   $image_meta    Optional. The image meta data as returned by 'jk_get_attachment_metadata()'.
 *                                    Default null.
 * @return string|false A 'srcset' value string or false.
 */
function jk_get_attachment_image_srcset( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = jk_get_attachment_image_src( $attachment_id, $size );
	...
	return jk_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
}
 ...
 * @param array  $image_meta    The image meta data as returned by 'jk_get_attachment_metadata()'.
 * @param int    $attachment_id Optional. The image attachment ID. Default 0.
 * @return string|false The 'srcset' attribute value. False on error or when only one source exists.
 */
function jk_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	/**
	 ...
	 */
	$image_meta = apply_filters( 'jk_calculate_image_srcset_meta', $image_meta, $size_array, $image_src, $attachment_id );
	...
	$image_basename = jk_basename( $image_meta['file'] );

	/*
	 * JKPress flattens animated GIFs into one frame when generating intermediate sizes.
	...
	$dirname = _jk_get_attachment_relative_path( $image_meta['file'] );
	...
	/*
	 * Images that have been edited in JKPress after being uploaded will
	 * contain a unique hash. Look for that hash and use it later to filter
	 * out images that are leftovers from previous versions.
	 */
	$image_edited = preg_match( '/-e[0-9]{13}/', jk_basename( $image_src ), $image_edit_hash );
	...
	foreach ( $image_sizes as $image ) {
		...
		if ( jk_image_matches_ratio( $image_width, $image_height, $image['width'], $image['height'] ) ) {
	 ...
	 */
	$sources = apply_filters( 'jk_calculate_image_srcset', $sources, $size_array, $image_src, $image_meta, $attachment_id );
 ...
 */
function jk_get_attachment_image_sizes( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = jk_get_attachment_image_src( $attachment_id, $size );
	...
	return jk_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
}
 ...
 * @return string|false A valid source size value for use in a 'sizes' attribute or false.
 */
function jk_calculate_image_sizes( $size, $image_src = null, $image_meta = null, $attachment_id = 0 ) {
	$width = 0;
	...
	if ( is_array( $size ) ) {
		...
		if ( is_array( $image_meta ) ) {
			$size_array = _jk_get_image_size_from_meta( $size, $image_meta );
			if ( $size_array ) {
	...
	/**
	 * Filters the output of 'jk_calculate_image_sizes()'.
	 *
	 * @since 4.4.0
	 *
	 ...
	 * @param array|null   $image_meta    The image meta data as returned by jk_get_attachment_metadata() or null.
	 * @param int          $attachment_id Image attachment ID of the original image or 0.
	 */
	return apply_filters( 'jk_calculate_image_sizes', $sizes, $size, $image_src, $image_meta, $attachment_id );
}
 ...
 * @param array  $image_meta     The attachment meta data as returned by 'jk_get_attachment_metadata()'.
 * @param int    $attachment_id  Optional. The image attachment ID. Default 0.
 * @return bool Whether the image meta is for this image file.
 */
function jk_image_file_matches_image_meta( $image_location, $image_meta, $attachment_id = 0 ) {
	$match = false;
	 ...
	 * @param int    $attachment_id  The image attachment ID or 0 if not supplied.
	 */
	return apply_filters( 'jk_image_file_matches_image_meta', $match, $image_location, $image_meta, $attachment_id );
}
 ...
 */
function jk_image_src_get_dimensions( $image_src, $image_meta, $attachment_id = 0 ) {
	$dimensions = false;

	// Is it a full size image?
	if (
		isset( $image_meta['file'] ) &&
		str_contains( $image_src, jk_basename( $image_meta['file'] ) )
	) {
		$dimensions = array(
	...
	if ( ! $dimensions && ! empty( $image_meta['sizes'] ) ) {
		$src_filename = jk_basename( $image_src );
	...
	/**
	 * Filters the 'jk_image_src_get_dimensions' value.
	 *
	 * @since 5.7.0
	 *
	 ...
	 *                                   'jk_get_attachment_metadata()'.
	 * @param int         $attachment_id The image attachment ID. Default 0.
	 */
	return apply_filters( 'jk_image_src_get_dimensions', $dimensions, $image_src, $image_meta, $attachment_id );
}
 ...
 */
function jk_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id ) {
	...
	if ( preg_match( '/-e[0-9]{13}/', $image_meta['file'], $img_edit_hash )
		&& ! str_contains( jk_basename( $image_src ), $img_edit_hash[0] )
	) {
		return $image;
	}
	...
	if ( $width && $height ) {
		...
		$size_array = jk_image_src_get_dimensions( $image_src, $image_meta, $attachment_id );
		if ( ! $size_array ) {
			return $image;
		}
	}

	$srcset = jk_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
	...
	if ( $srcset ) {
		...
		if ( ! $sizes ) {
			$sizes = jk_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
		}
	}
 ...
 */
function jk_lazy_loading_enabled( $tag_name, $context ) {
	/*
	 ...
	 *                         or the function name from where this was called.
	 */
	return (bool) apply_filters( 'jk_lazy_loading_enabled', $default, $tag_name, $context );
}
 ...
 *
 * @see jk_img_tag_add_width_and_height_attr()
 * @see jk_img_tag_add_srcset_and_sizes_attr()
 * @see jk_img_tag_add_loading_optimization_attrs()
 * @see jk_iframe_tag_add_loading_attr()
 *
 ...
 */
function jk_filter_content_tags( $content, $context = null ) {
	if ( null === $context ) {
		$context = current_filter();
	}

	$add_iframe_loading_attr = jk_lazy_loading_enabled( 'iframe', $context );
	...
	foreach ( $matches as $match ) {
		...
		switch ( $tag_name ) {
			case 'img':
				if ( preg_match( '/jk-image-([0-9]+)/i', $tag, $class_id ) ) {
	...
	foreach ( $matches as $match ) {
		...
		if ( isset( $images[ $match[0] ] ) ) {
			...
			if ( $attachment_id > 0 && ! str_contains( $filtered_image, ' width=' ) && ! str_contains( $filtered_image, ' height=' ) ) {
				$filtered_image = jk_img_tag_add_width_and_height_attr( $filtered_image, $context, $attachment_id );
			}

			// Add 'srcset' and 'sizes' attributes if applicable.
			if ( $attachment_id > 0 && ! str_contains( $filtered_image, ' srcset=' ) ) {
				$filtered_image = jk_img_tag_add_srcset_and_sizes_attr( $filtered_image, $context, $attachment_id );
			}

			// Add loading optimization attributes if applicable.
			$filtered_image = jk_img_tag_add_loading_optimization_attrs( $filtered_image, $context );

			// Adds 'auto' to the sizes attribute if applicable.
			$filtered_image = jk_img_tag_add_auto_sizes( $filtered_image );
			 ...
			 */
			$filtered_image = apply_filters( 'jk_content_img_tag', $filtered_image, $context, $attachment_id );
		...
		if ( isset( $iframes[ $match[0] ] ) ) {
			...
			if ( $add_iframe_loading_attr && ! str_contains( $filtered_iframe, ' loading=' ) ) {
				$filtered_iframe = jk_iframe_tag_add_loading_attr( $filtered_iframe, $context );
			}
 ...
 */
function jk_img_tag_add_auto_sizes( string $image ): string {
	/**
	 ...
	 */
	if ( ! apply_filters( 'jk_img_tag_add_auto_sizes', true ) ) {
		return $image;
	}

	$processor = new JK_HTML_Tag_Processor( $image );
	 ...
	 * Per JKPress Core itself, lazy-loaded images should always have a width attribute.
	...
	if ( jk_sizes_attribute_includes_valid_auto( $sizes ) ) {
		return $image;
	}
 ...
 */
function jk_sizes_attribute_includes_valid_auto( string $sizes_attr ): bool {
 ...
 */
function jk_print_auto_sizes_contain_css_fix() {
	/** This filter is documented in jk-includes/media.php */
	$add_auto_sizes = apply_filters( 'jk_img_tag_add_auto_sizes', true );
	if ( ! $add_auto_sizes ) {
		return;
 ...
 */
function jk_img_tag_add_loading_optimization_attrs( $image, $context ) {
	 ...
	 */
	$optimization_attrs = jk_get_loading_optimization_attributes(
		'img',
		array(
			'src'           => $src,
			'width'         => $width,
	...
	$loading_attrs_enabled = jk_lazy_loading_enabled( 'img', $context );
	...
	if ( empty( $loading_val ) && $loading_attrs_enabled ) {
		...
		if ( isset( $optimization_attrs['loading'] ) && ! $filtered_loading_attr ) {
			 ...
			 *
			 * The same warning is present in `jk_get_loading_optimization_attributes()`, and here it
 ...
 */
function jk_img_tag_add_width_and_height_attr( $image, $context, $attachment_id ) {
	$image_src         = preg_match( '/src="([^"]+)"/', $image, $match_src ) ? $match_src[1] : '';
	list( $image_src ) = explode( '?', $image_src );
	 ...
	 */
	$add = apply_filters( 'jk_img_tag_add_width_and_height_attr', true, $image, $context, $attachment_id );
 ...
 */
function jk_img_tag_add_srcset_and_sizes_attr( $image, $context, $attachment_id ) {
	/**
	 ...
	 * @param int    $attachment_id The image attachment ID.
	 */
	$add = apply_filters( 'jk_img_tag_add_srcset_and_sizes_attr', true, $image, $context, $attachment_id );

	if ( true === $add ) {
		$image_meta = jk_get_attachment_metadata( $attachment_id );
		return jk_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id );
	}
 ...
 */
function jk_iframe_tag_add_loading_attr( $iframe, $context ) {
	/*
	...
	$optimization_attrs = jk_get_loading_optimization_attributes(
		...
		array(
			 ...
			 * TODO: Use JK_HTML_Tag_Processor to extract actual values once support is
			 * added.
			 */
	 ...
	 */
	$value = apply_filters( 'jk_iframe_tag_add_loading_attr', $value, $iframe, $context );
...
/**
 * Adds a 'jk-post-image' class to post thumbnails. Internal use only.
 *
 ...
 * @return string[] Modified array of attributes including the new 'jk-post-image' class.
 */
function _jk_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' jk-post-image';
	return $attr;
}

/**
 * Adds '_jk_post_thumbnail_class_filter' callback to the 'jk_get_attachment_image_attributes'
 ...
 * @param string[] $attr Array of thumbnail attributes including src, class, alt, title, keyed by attribute name.
 */
function _jk_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'jk_get_attachment_image_attributes', '_jk_post_thumbnail_class_filter' );
}

/**
 * Removes the '_jk_post_thumbnail_class_filter' callback from the 'jk_get_attachment_image_attributes'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 2.9.0
 ...
 * @param string[] $attr Array of thumbnail attributes including src, class, alt, title, keyed by attribute name.
 */
function _jk_post_thumbnail_class_filter_remove( $attr ) {
	remove_filter( 'jk_get_attachment_image_attributes', '_jk_post_thumbnail_class_filter' );
}

/**
 * Overrides the context used in {@see jk_get_attachment_image()}. Internal use only.
 *
 * Uses the {@see 'begin_fetch_post_thumbnail_html'} and {@see 'end_fetch_post_thumbnail_html'}
 * action hooks to dynamically add/remove itself so as to only filter post thumbnails.
 *
 ...
 */
function _jk_post_thumbnail_context_filter( $context ) {
	return 'the_post_thumbnail';
}

/**
 * Adds the '_jk_post_thumbnail_context_filter' callback to the 'jk_get_attachment_image_context'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 6.3.0
 * @access private
 */
function _jk_post_thumbnail_context_filter_add() {
	add_filter( 'jk_get_attachment_image_context', '_jk_post_thumbnail_context_filter' );
}

/**
 * Removes the '_jk_post_thumbnail_context_filter' callback from the 'jk_get_attachment_image_context'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 6.3.0
 * @access private
 */
function _jk_post_thumbnail_context_filter_remove() {
	remove_filter( 'jk_get_attachment_image_context', '_jk_post_thumbnail_context_filter' );
}

add_shortcode( 'jk_caption', 'img_caption_shortcode' );
...
function img_caption_shortcode( $attr, $content = '' ) {
	...
	if ( ! isset( $attr['caption'] ) ) {
		...
		$attr['caption'] = jk_kses( $attr['caption'], 'post' );
	}
	...
	$class = trim( 'jk-caption ' . $atts['align'] . ' ' . $atts['class'] );
	...
	if ( $html5 ) {
		$html = sprintf(
			...
			sprintf(
				'<figcaption %sclass="jk-caption-text">%s</figcaption>',
				$caption_id,
				$atts['caption']
			)
		);
 ...
 * @since 4.1.0 Added attribute to `jk_get_attachment_link()` to output `aria-describedby`.
 ...
 * @since 5.1.0 Code cleanup for JKCS 1.0.0 coding standards.
...
function gallery_shortcode( $attr ) {
	...
	if ( is_feed() ) {
		...
		foreach ( $attachments as $att_id => $attachment ) {
			if ( ! empty( $atts['link'] ) ) {
				if ( 'none' === $atts['link'] ) {
					$output .= jk_get_attachment_image( $att_id, $atts['size'], false, $attr );
				} else {
					$output .= jk_get_attachment_link( $att_id, $atts['size'], false );
				}
			} else {
				$output .= jk_get_attachment_link( $att_id, $atts['size'], true );
			}
			$output .= "\n";
		}
		return $output;
	...
	$valid_tags = jk_kses_allowed_html( 'post' );
	...
	if ( apply_filters( 'use_default_gallery_style', ! $html5 ) ) {
		...
		<style{$type_attr}>
			...
			/* see gallery_shortcode() in jk-includes/media.php */
		</style>\n\t\t";
	}
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( ! empty( $atts['link'] ) && 'file' === $atts['link'] ) {
			$image_output = jk_get_attachment_link( $id, $atts['size'], false, false, false, $attr );
		} elseif ( ! empty( $atts['link'] ) && 'none' === $atts['link'] ) {
			$image_output = jk_get_attachment_image( $id, $atts['size'], false, $attr );
		} else {
			$image_output = jk_get_attachment_link( $id, $atts['size'], true, false, false, $attr );
		}

		$image_meta = jk_get_attachment_metadata( $id );
		...
		if ( $captiontag && trim( $attachment->post_excerpt ) ) {
			$output .= "
				<{$captiontag} class='jk-caption-text gallery-caption' id='$selector-$id'>
 ...
 * @since 3.9.0
 */
function jk_underscore_playlist_templates() {
	?>
<script type="text/html" id="tmpl-jk-playlist-current-item">
	<# if ( data.thumb && data.thumb.src ) { #>
		<img src="{{ data.thumb.src }}" alt="" />
	<# } #>
	<div class="jk-playlist-caption">
		<span class="jk-playlist-item-meta jk-playlist-item-title">
		...
		<# if ( data.meta.album ) { #><span class="jk-playlist-item-meta jk-playlist-item-album">{{ data.meta.album }}</span><# } #>
		<# if ( data.meta.artist ) { #><span class="jk-playlist-item-meta jk-playlist-item-artist">{{ data.meta.artist }}</span><# } #>
	</div>
</script>
<script type="text/html" id="tmpl-jk-playlist-item">
	<div class="jk-playlist-item">
		<a class="jk-playlist-caption" href="{{ data.src }}">
			{{ data.index ? ( data.index + '. ' ) : '' }}
			<# if ( data.caption ) { #>
				{{ data.caption }}
			<# } else { #>
				<# if ( data.artists && data.meta.artist ) { #>
					<span class="jk-playlist-item-title">
						<?php
						/* translators: %s: Playlist item title. */
						printf( _x( '&#8220;%s&#8221;', 'playlist item title' ), '{{{ data.title }}}' );
						?>
					</span>
					<span class="jk-playlist-item-artist"> &mdash; {{ data.meta.artist }}</span>
				<# } else { #>
					<span class="jk-playlist-item-title">{{{ data.title }}}</span>
				<# } #>
			<# } #>
		</a>
		<# if ( data.meta.length_formatted ) { #>
		<div class="jk-playlist-item-length">{{ data.meta.length_formatted }}</div>
		<# } #>
	</div>
</script>
	<?php
 ...
 */
function jk_playlist_scripts( $type ) {
	jk_enqueue_style( 'jk-mediaelement' );
	jk_enqueue_script( 'jk-playlist' );
	?>
<!--[if lt IE 9]><script>document.createElement('<?php echo esc_js( $type ); ?>');</script><![endif]-->
	<?php
	add_action( 'jk_footer', 'jk_underscore_playlist_templates', 0 );
	add_action( 'admin_footer', 'jk_underscore_playlist_templates', 0 );
}
 ...
 * a collection of JKPress audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
 ...
 */
function jk_playlist_shortcode( $attr ) {
	global $content_width;
	$post = get_post();
	...
	if ( is_feed() ) {
		$output = "\n";
		foreach ( $attachments as $att_id => $attachment ) {
			$output .= jk_get_attachment_link( $att_id ) . "\n";
		}
		return $output;
	}
	...
	$data = array(
		...
		'tracklist'    => jk_validate_boolean( $atts['tracklist'] ),
		'tracknumbers' => jk_validate_boolean( $atts['tracknumbers'] ),
		'images'       => jk_validate_boolean( $atts['images'] ),
		'artists'      => jk_validate_boolean( $atts['artists'] ),
	);

	$tracks = array();
	foreach ( $attachments as $attachment ) {
		$url   = jk_get_attachment_url( $attachment->ID );
		$ftype = jk_check_filetype( $url, jk_get_mime_types() );
		$track = array(
			'src'         => $url,
			'type'        => $ftype['type'],
		...
		$track['meta'] = array();
		$meta          = jk_get_attachment_metadata( $attachment->ID );
		if ( ! empty( $meta ) ) {

			foreach ( jk_get_attachment_id3_keys( $attachment ) as $key => $label ) {
				if ( ! empty( $meta[ $key ] ) ) {
		...
		if ( $atts['images'] ) {
			...
			if ( ! empty( $thumb_id ) ) {
				list( $src, $width, $height ) = jk_get_attachment_image_src( $thumb_id, 'full' );
				$track['image']               = compact( 'src', 'width', 'height' );
				list( $src, $width, $height ) = jk_get_attachment_image_src( $thumb_id, 'thumbnail' );
				$track['thumb']               = compact( 'src', 'width', 'height' );
			} else {
				$src            = jk_mime_type_icon( $attachment->ID, '.svg' );
				$width          = 48;
				$height         = 64;
				$track['image'] = compact( 'src', 'width', 'height' );
				$track['thumb'] = compact( 'src', 'width', 'height' );
	...
	if ( 1 === $instance ) {
		 ...
		 */
		do_action( 'jk_playlist_scripts', $atts['type'], $atts['style'] );
	}
	?>
<div class="jk-playlist jk-<?php echo $safe_type; ?>-playlist jk-playlist-<?php echo $safe_style; ?>">
	<?php if ( 'audio' === $atts['type'] ) : ?>
		<div class="jk-playlist-current-item"></div>
	<?php endif; ?>
	<<?php echo $safe_type; ?> controls="controls" preload="none" width="<?php echo (int) $theme_width; ?>"
		...
		?>
	></<?php echo $safe_type; ?>>
	<div class="jk-playlist-next"></div>
	<div class="jk-playlist-prev"></div>
	<noscript>
	<ol>
		<?php
		foreach ( $attachments as $att_id => $attachment ) {
			printf( '<li>%s</li>', jk_get_attachment_link( $att_id ) );
		}
		?>
	</ol>
	</noscript>
	<script type="application/json" class="jk-playlist-script"><?php echo jk_json_encode( $data ); ?></script>
</div>
	<?php
	return ob_get_clean();
}
add_shortcode( 'playlist', 'jk_playlist_shortcode' );
 ...
 */
function jk_mediaelement_fallback( $url ) {
	/**
	 ...
	 */
	return apply_filters( 'jk_mediaelement_fallback', sprintf( '<a href="%1$s">%1$s</a>', esc_url( $url ) ), $url );
}
 ...
 */
function jk_get_audio_extensions() {
	/**
	 ...
	 */
	return apply_filters( 'jk_audio_extensions', array( 'mp3', 'ogg', 'flac', 'm4a', 'wav' ) );
}
 ...
 * @since 3.9.0
 *
 * @param JK_Post $attachment The current attachment, provided for context.
 * @param string  $context    Optional. The context. Accepts 'edit', 'display'. Default 'display'.
 * @return string[] Key/value pairs of field keys to labels.
 */
function jk_get_attachment_id3_keys( $attachment, $context = 'display' ) {
	$fields = array(
		'artist' => __( 'Artist' ),
		'album'  => __( 'Album' ),
	);
	 ...
	 * @param JK_Post $attachment Attachment object.
	 * @param string  $context    The context. Accepts 'edit', 'display'. Default 'display'.
	 */
	return apply_filters( 'jk_get_attachment_id3_keys', $fields, $attachment, $context );
}
/**
 * Builds the Audio shortcode output.
 *
 * This implements the functionality of the Audio Shortcode for displaying
 * JKPress mp3s in a post.
 *
 * @since 3.6.0
 *
 * @param array  $attr {
 ...
 *     @type string $class    The 'class' attribute for the `<audio>` element. Default 'jk-audio-shortcode'.
 *     @type string $style    The 'style' attribute for the `<audio>` element. Default 'width: 100%;'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display audio.
 */
function jk_audio_shortcode( $attr, $content = '' ) {
	 ...
	 * @param array  $attr     Attributes of the shortcode. See {@see jk_audio_shortcode()}.
	 * @param string $content  Shortcode content.
	 * @param int    $instance Unique numeric ID of this audio shortcode instance.
	 */
	$override = apply_filters( 'jk_audio_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = jk_get_audio_extensions();
	$defaults_atts = array(
		'src'      => '',
		'loop'     => '',
		'autoplay' => '',
		'preload'  => 'none',
		'class'    => 'jk-audio-shortcode',
		'style'    => 'width: 100%;',
	);
	...
	$primary = false;
	if ( ! empty( $atts['src'] ) ) {
		$type = jk_check_filetype( $atts['src'], jk_get_mime_types() );

		if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
			return sprintf( '<a class="jk-embedded-audio" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
		}
		...
		foreach ( $default_types as $ext ) {
			if ( ! empty( $atts[ $ext ] ) ) {
				$type = jk_check_filetype( $atts[ $ext ], jk_get_mime_types() );
	...
	if ( ! $primary ) {
		...
		$audio       = reset( $audios );
		$atts['src'] = jk_get_attachment_url( $audio->ID );
	 ...
	 */
	$library = apply_filters( 'jk_audio_shortcode_library', 'mediaelement' );
	 ...
	 */
	$atts['class'] = apply_filters( 'jk_audio_shortcode_class', $atts['class'], $atts );

	$html_atts = array(
		'class'    => $atts['class'],
		'id'       => sprintf( 'audio-%d-%d', $post_id, $instance ),
		'loop'     => jk_validate_boolean( $atts['loop'] ),
		'autoplay' => jk_validate_boolean( $atts['autoplay'] ),
	...
	foreach ( $default_types as $fallback ) {
		if ( ! empty( $atts[ $fallback ] ) ) {
			...
			$type  = jk_check_filetype( $atts[ $fallback ], jk_get_mime_types() );
	...
	if ( 'mediaelement' === $library ) {
		$html .= jk_mediaelement_fallback( $fileurl );
	}
	 ...
	 * @param string $library Media library used for the audio shortcode.
	 */
	return apply_filters( 'jk_audio_shortcode', $html, $atts, $audio, $post_id, $library );
}
add_shortcode( 'audio', 'jk_audio_shortcode' );
 ...
 */
function jk_get_video_extensions() {
	/**
	 ...
	 */
	return apply_filters( 'jk_video_extensions', array( 'mp4', 'm4v', 'webm', 'ogv', 'flv' ) );
}
 ...
 *                            Default 'jk-video-shortcode'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display video.
 */
function jk_video_shortcode( $attr, $content = '' ) {
	global $content_width;
	$post_id = get_post() ? get_the_ID() : 0;
	 ...
	 * @since 3.6.0
	 *
	 * @see jk_video_shortcode()
	 *
	 * @param string $html     Empty variable to be replaced with shortcode markup.
	 * @param array  $attr     Attributes of the shortcode. See {@see jk_video_shortcode()}.
	 * @param string $content  Video shortcode content.
	 * @param int    $instance Unique numeric ID of this video shortcode instance.
	 */
	$override = apply_filters( 'jk_video_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = jk_get_video_extensions();
	$defaults_atts = array(
		'src'      => '',
		'poster'   => '',
		'loop'     => '',
	...
	if ( ! empty( $atts['src'] ) ) {
		...
		if ( ! $is_youtube && ! $is_vimeo ) {
			...
			if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
				return sprintf( '<a class="jk-embedded-video" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
			}
		}

		if ( $is_vimeo ) {
			jk_enqueue_script( 'mediaelement-vimeo' );
		}
	...
	if ( ! $primary ) {
		...
		$video       = reset( $videos );
		$atts['src'] = jk_get_attachment_url( $video->ID );
		if ( empty( $atts['src'] ) ) {
			return;
		}
	 ...
	 */
	$library = apply_filters( 'jk_video_shortcode_library', 'mediaelement' );
	if ( 'mediaelement' === $library && did_action( 'init' ) ) {
		jk_enqueue_style( 'jk-mediaelement' );
		jk_enqueue_script( 'jk-mediaelement' );
		jk_enqueue_script( 'mediaelement-vimeo' );
	...
	if ( 'mediaelement' === $library ) {
		if ( $is_youtube ) {
			...
			$parsed_vimeo_url = jk_parse_url( $atts['src'] );
	 ...
	 */
	$atts['class'] = apply_filters( 'jk_video_shortcode_class', $atts['class'], $atts );
	...
	$html_atts = array(
		...
		'loop'     => jk_validate_boolean( $atts['loop'] ),
		'autoplay' => jk_validate_boolean( $atts['autoplay'] ),
		'muted'    => jk_validate_boolean( $atts['muted'] ),
		'preload'  => $atts['preload'],
	);
	...
	$output = sprintf( '<div style="%s" class="jk-video">%s</div>', $width_rule, $html );
	 ...
	 * @param string $library Media library used for the video shortcode.
	 */
	return apply_filters( 'jk_video_shortcode', $output, $atts, $video, $post_id, $library );
}
add_shortcode( 'video', 'jk_video_shortcode' );
...
function get_adjacent_image_link( $prev = true, $size = 'thumbnail', $text = false ) {
	...
	if ( $attachments ) {
		...
		if ( isset( $attachments[ $k ] ) ) {
			...
			$output        = jk_get_attachment_link( $attachment_id, $size, true, false, $text, $attr );
		}
	}
 ...
 *                                     Default is 'names'.
 * @return string[]|JK_Taxonomy[] List of taxonomies or taxonomy names. Empty array on failure.
 */
 ...
 *                       Default 'names'.
 * @return string[]|JK_Taxonomy[] Array of names or objects of registered taxonomies for attachments.
 */
 ...
 *                                False on failure.
 */
function jk_imagecreatetruecolor( $width, $height ) {
 ...
 * }
 */
function jk_expand_dimensions( $example_width, $example_height, $max_width, $max_height ) {
	$example_width  = (int) $example_width;
	$example_height = (int) $example_height;
	$max_width      = (int) $max_width;
	$max_height     = (int) $max_height;

	return jk_constrain_dimensions( $example_width * 1000000, $example_height * 1000000, $max_width, $max_height );
}
 ...
 */
function jk_max_upload_size() {
	$u_bytes = jk_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = jk_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
...
/**
 * Returns a JK_Image_Editor instance and loads file into it.
 *
 * @since 3.5.0
 *
 ...
 *                     Default empty array.
 * @return JK_Image_Editor|JK_Error The JK_Image_Editor object on success,
 *                                  a JK_Error object otherwise.
 */
function jk_get_image_editor( $path, $args = array() ) {
	$args['path'] = $path;

	// If the mime type is not set in args, try to extract and set it from the file.
	if ( ! isset( $args['mime_type'] ) ) {
		$file_info = jk_check_filetype( $args['path'] );
	...
	if ( isset( $args['mime_type'] ) ) {
		$output_format = jk_get_image_editor_output_format( $path, $args['mime_type'] );
		if ( isset( $output_format[ $args['mime_type'] ] ) ) {
			$args['output_mime_type'] = $output_format[ $args['mime_type'] ];
		}
	}

	$implementation = _jk_image_editor_choose( $args );

	if ( $implementation ) {
		$editor = new $implementation( $path );
		$loaded = $editor->load();

		if ( is_jk_error( $loaded ) ) {
			return $loaded;
		}
	...
	return new JK_Error( 'image_no_editor', __( 'No editor could be selected.' ) );
}
 ...
 */
function jk_image_editor_supports( $args = array() ) {
	return (bool) _jk_image_editor_choose( $args );
}
 ...
 */
function _jk_image_editor_choose( $args = array() ) {
	require_once ABSPATH . JKINC . '/class-jk-image-editor.php';
	require_once ABSPATH . JKINC . '/class-jk-image-editor-gd.php';
	require_once ABSPATH . JKINC . '/class-jk-image-editor-imagick.php';
	require_once ABSPATH . JKINC . '/class-avif-info.php';
	/**
	 ...
	 *                                'JK_Image_Editor_Imagick', 'JK_Image_Editor_GD'.
	 */
	$implementations = apply_filters( 'jk_image_editors', array( 'JK_Image_Editor_Imagick', 'JK_Image_Editor_GD' ) );

	$editors = jk_cache_get( 'jk_image_editor_choose', 'image_editor' );
	...
	jk_cache_set( 'jk_image_editor_choose', $editors, 'image_editor', DAY_IN_SECONDS );
 ...
 * @since 3.4.0
 */
function jk_plupload_default_settings() {
	$jk_scripts = jk_scripts();

	$data = $jk_scripts->get_data( 'jk-plupload', 'data' );
	if ( $data && str_contains( $data, '_jkPluploadSettings' ) ) {
		return;
	}

	$max_upload_size    = jk_max_upload_size();
	...
	if ( ! jk_image_editor_supports( array( 'mime_type' => 'image/webp' ) ) ) {
		$defaults['webp_upload_error'] = true;
	}

	// Check if AVIF images can be edited.
	if ( ! jk_image_editor_supports( array( 'mime_type' => 'image/avif' ) ) ) {
		$defaults['avif_upload_error'] = true;
	}

	// Check if HEIC images can be edited.
	if ( ! jk_image_editor_supports( array( 'mime_type' => 'image/heic' ) ) ) {
	...
	$params['_jknonce'] = jk_create_nonce( 'media-form' );
	...
	$script = 'var _jkPluploadSettings = ' . jk_json_encode( $settings ) . ';';

	if ( $data ) {
		$script = "$data\n$script";
	}

	$jk_scripts->add_data( 'jk-plupload', 'data', $script );
}
 ...
 * @since 3.5.0
 *
 * @param int|JK_Post $attachment Attachment ID or object.
 * @return array|void {
 ...
 *     @type string $icon                  Icon URL of the attachment (e.g. /jk-includes/images/media/archive.png).
 ...
 *     @type string $url                   Direct URL to the attachment file (from jk-content).
 *     @type int    $width                 If the attachment is an image, represents the width of the image in pixels.
 * }
 */
function jk_prepare_attachment_for_js( $attachment ) {
	$attachment = get_post( $attachment );
	...
	$attachment_url = jk_get_attachment_url( $attachment->ID );
	$base_url       = str_replace( jk_basename( $attachment_url ), '', $attachment_url );

	$response = array(
		'id'            => $attachment->ID,
		'title'         => $attachment->post_title,
		'filename'      => jk_basename( get_attached_file( $attachment->ID ) ),
		'url'           => $attachment_url,
		'link'          => get_attachment_link( $attachment->ID ),
		'alt'           => get_post_meta( $attachment->ID, '_jk_attachment_image_alt', true ),
		...
		'type'          => $type,
		'subtype'       => $subtype,
		'icon'          => jk_mime_type_icon( $attachment->ID, '.svg' ),
	...
	$author = new JK_User( $attachment->post_author );
	...
	if ( isset( $meta['filesize'] ) ) {
		...
		$bytes = jk_filesize( $attached_file );
	} else {
		$bytes = '';
	}
	...
	$context             = get_post_meta( $attachment->ID, '_jk_attachment_context', true );
	$response['context'] = ( $context ) ? $context : '';

	if ( current_user_can( 'edit_post', $attachment->ID ) ) {
		$response['nonces']['update'] = jk_create_nonce( 'update-post_' . $attachment->ID );
		$response['nonces']['edit']   = jk_create_nonce( 'image_editor-' . $attachment->ID );
		$response['editLink']         = get_edit_post_link( $attachment->ID, 'raw' );
	}

	if ( current_user_can( 'delete_post', $attachment->ID ) ) {
		$response['nonces']['delete'] = jk_create_nonce( 'delete-post_' . $attachment->ID );
	}
	...
	if ( $meta && ( 'image' === $type || ! empty( $meta['sizes'] ) ) ) {
		...
		if ( 'image' === $type ) {
			if ( ! empty( $meta['original_image'] ) ) {
				$response['originalImageURL']  = jk_get_original_image_url( $attachment->ID );
				$response['originalImageName'] = jk_basename( jk_get_original_image_path( $attachment->ID ) );
			}
	...
	if ( $meta && ( 'audio' === $type || 'video' === $type ) ) {
		...
		$response['meta'] = array();
		foreach ( jk_get_attachment_id3_keys( $attachment, 'js' ) as $key => $label ) {
		...
		if ( ! empty( $id ) ) {
			list( $src, $width, $height ) = jk_get_attachment_image_src( $id, 'full' );
			$response['image']            = compact( 'src', 'width', 'height' );
			list( $src, $width, $height ) = jk_get_attachment_image_src( $id, 'thumbnail' );
	 ...
	 * @since 3.5.0
	 *
	 * @param array       $response   Array of prepared attachment data. See {@see jk_prepare_attachment_for_js()}.
	 * @param JK_Post     $attachment Attachment object.
	 * @param array|false $meta       Array of attachment meta data, or false if there is none.
	 */
	return apply_filters( 'jk_prepare_attachment_for_js', $response, $attachment, $meta );
}
 ...
 *
 *     @type int|JK_Post $post Post ID or post object.
 * }
 */
function jk_enqueue_media( $args = array() ) {
	// Enqueue me just once per page, please.
	if ( did_action( 'jk_enqueue_media' ) ) {
		return;
	}

	global $content_width, $jkdb, $jk_locale;
	...
	$exts      = array_merge( jk_get_audio_extensions(), jk_get_video_extensions() );
	...
	if ( null === $show_audio_playlist ) {
		$show_audio_playlist = $jkdb->get_var(
			"SELECT ID
			FROM $jkdb->posts
	...
	if ( null === $show_video_playlist ) {
		$show_video_playlist = $jkdb->get_var(
			"SELECT ID
			FROM $jkdb->posts
			WHERE post_type = 'attachment'
	...
	foreach ( $months as $month_year ) {
		$month_year->text = sprintf(
			...
			__( '%1$s %2$d' ),
			$jk_locale->get_month( $month_year->month ),
			$month_year->year
		);
	}
	...
	$settings = array(
		...
		'mimeTypes'         => jk_list_pluck( get_post_mime_types(), 0 ),
		/** This filter is documented in jk-admin/includes/media.php */
		'captions'          => ! apply_filters( 'disable_captions', '' ),
		'nonce'             => array(
			'sendToEditor'           => jk_create_nonce( 'media-send-to-editor' ),
			'setAttachmentThumbnail' => jk_create_nonce( 'set-attachment-thumbnail' ),
		),
		'post'              => array(
			'id' => 0,
		),
	...
	if ( isset( $args['post'] ) ) {
		...
		$settings['post'] = array(
			'id'    => $post->ID,
			'nonce' => jk_create_nonce( 'update-post_' . $post->ID ),
		);

		$thumbnail_support = current_theme_supports( 'post-thumbnails', $post->post_type ) && post_type_supports( $post->post_type, 'thumbnail' );
		if ( ! $thumbnail_support && 'attachment' === $post->post_type && $post->post_mime_type ) {
			if ( jk_attachment_is( 'audio', $post ) ) {
				$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
			} elseif ( jk_attachment_is( 'video', $post ) ) {
	 ...
	 */
	jk_enqueue_script( 'media-editor' );
	jk_localize_script( 'media-views', '_jkMediaViewsL10n', $strings );

	jk_enqueue_script( 'media-audiovideo' );
	jk_enqueue_style( 'media-views' );
	if ( is_admin() ) {
		jk_enqueue_script( 'mce-view' );
		jk_enqueue_script( 'image-edit' );
	}
	jk_enqueue_style( 'imgareaselect' );
	jk_plupload_default_settings();

	require_once ABSPATH . JKINC . '/media-template.php';
	add_action( 'admin_footer', 'jk_print_media_templates' );
	add_action( 'jk_footer', 'jk_print_media_templates' );
	add_action( 'customize_controls_print_footer_scripts', 'jk_print_media_templates' );

	/**
	 * Fires at the conclusion of jk_enqueue_media().
	 *
	 * @since 3.5.0
	 */
	do_action( 'jk_enqueue_media' );
}
 ...
 * @param int|JK_Post $post Optional. Post ID or JK_Post object. Default is global $post.
 * @return JK_Post[] Array of media attached to the given post.
 */
function get_attached_media( $type, $post = 0 ) {
	 ...
	 * @since 3.6.0
	 *
	 * @param JK_Post[] $children Array of media attached to the given post.
	...
	if ( has_block( 'gallery', $post->post_content ) ) {
		...
		while ( $block = array_shift( $post_blocks ) ) {
			...
			if ( $has_inner_blocks && $html ) {
				$block_html  = jk_list_pluck( $block['innerBlocks'], 'innerHTML' );
			...
			if ( $has_inner_blocks ) {
				$attrs = jk_list_pluck( $block['innerBlocks'], 'attrs' );
				$ids   = jk_list_pluck( $attrs, 'id' );

				foreach ( $ids as $id ) {
					$url = jk_get_attachment_url( $id );
...
function get_post_galleries_images( $post = 0 ) {
	...
	return jk_list_pluck( $galleries, 'src' );
}
 ...
 * @param JK_Post $attachment Attachment object.
 */
function jk_maybe_generate_attachment_metadata( $attachment ) {
	...
	$meta          = jk_get_attachment_metadata( $attachment_id );

	if ( empty( $meta ) && file_exists( $file ) ) {
		$_meta = get_post_meta( $attachment_id );
		$_lock = 'jk_generating_att_' . $attachment_id;

		if ( ! array_key_exists( '_jk_attachment_metadata', $_meta ) && ! get_transient( $_lock ) ) {
			set_transient( $_lock, $file );
			jk_update_attachment_metadata( $attachment_id, jk_generate_attachment_metadata( $attachment_id, $file ) );
...
function attachment_url_to_postid( $url ) {
	 ...
	 * - null to indicate JKPress should proceed with the lookup.
	 *
	...
	$sql = $jkdb->prepare(
		"SELECT post_id, meta_value FROM $jkdb->postmeta WHERE meta_key = '_jk_attached_file' AND meta_value = %s",
		$path
	);

	$results = $jkdb->get_results( $sql );
	$post_id = null;
...
function wpview_media_sandbox_styles() {
	...
	$wpmediaelement = includes_url( "js/mediaelement/jk-mediaelement.css?$version" );
 ...
 */
function jk_register_media_personal_data_exporter( $exporters ) {
	$exporters['wordpress-media'] = array(
		'exporter_friendly_name' => __( 'JKPress Media' ),
		'callback'               => 'jk_media_personal_data_exporter',
	);
 ...
 * }
 */
function jk_media_personal_data_exporter( $email_address, $page = 1 ) {
	...
	foreach ( (array) $post_query->posts as $post ) {
		$attachment_url = jk_get_attachment_url( $post->ID );
 ...
 *
 * These sizes are meant to enhance the way JKPress displays images on the front-end on larger,
 ...
 * @access private
 */
function _jk_add_additional_image_sizes() {
 ...
 */
function jk_show_heic_upload_error( $plupload_settings ) {
	// Check if HEIC images can be edited.
	if ( ! jk_image_editor_supports( array( 'mime_type' => 'image/heic' ) ) ) {
 ...
 */
function jk_getimagesize( $filename, ?array &$image_info = null ) {
	...
	$image_mime_type = jk_get_image_mime( $filename );
	...
	if ( 'image/webp' === $image_mime_type ) {
		$webp_info = jk_get_webp_info( $filename );
	...
	if ( 'image/avif' === $image_mime_type ) {
		$avif_info = jk_get_avif_info( $filename );
	...
	if ( jk_is_heic_image_mime_type( $image_mime_type ) ) {
		$editor = jk_get_image_editor( $filename );

		if ( is_jk_error( $editor ) ) {
			return false;
		}

		// If the editor for HEICs is Imagick, use it to get the image size.
		if ( $editor instanceof JK_Image_Editor_Imagick ) {
 ...
 * }
 */
function jk_get_avif_info( $filename ) {
	$results = array(
		'width'        => false,
		'height'       => false,
		'bit_depth'    => false,
	...
	if ( 'image/avif' !== jk_get_image_mime( $filename ) ) {
		return $results;
	}
 ...
 * }
 */
function jk_get_webp_info( $filename ) {
	$width  = false;
	$height = false;
	$type   = false;

	if ( 'image/webp' !== jk_get_image_mime( $filename ) ) {
 ...
 */
function jk_get_loading_optimization_attributes( $tag_name, $attr, $context ) {
	global $jk_query;
	 ...
	 * @param string      $context       Context for the element for which the loading optimization attribute is requested.
	 */
	$loading_attrs = apply_filters( 'pre_jk_get_loading_optimization_attributes', false, $tag_name, $attr, $context );
	...
	if ( 'template' === $context ) {
		/** This filter is documented in jk-includes/media.php */
		return apply_filters( 'jk_get_loading_optimization_attributes', $loading_attrs, $tag_name, $attr, $context );
	}
	...
	if ( null === $maybe_in_viewport ) {
		$header_enforced_contexts = array(
			'template_part_' . JK_TEMPLATE_PART_AREA_HEADER => true,
		 ...
		 */
		$header_enforced_contexts = apply_filters( 'jk_loading_optimization_force_header_contexts', $header_enforced_contexts );
		...
		if ( isset( $header_enforced_contexts[ $context ] ) ) {
			 ...
			 */
			$content_media_count = jk_increase_content_media_count( 0 );
			$increase_count      = true;

			// If the count so far is below the threshold, `loading` attribute is omitted.
			if ( $content_media_count < jk_omit_loading_attr_threshold() ) {
				$maybe_in_viewport = true;
			} else {
				$maybe_in_viewport = false;
			}
			...
			$jk_query->before_loop && $jk_query->is_main_query()
			/*
	...
	if ( $maybe_in_viewport ) {
		$loading_attrs = jk_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr );
	} else {
		// Only add `loading="lazy"` if the feature is enabled.
		if ( jk_lazy_loading_enabled( $tag_name, $context ) ) {
	 ...
	 */
	if ( $increase_count ) {
		jk_increase_content_media_count();
	} elseif ( $maybe_increase_count ) {
		/** This filter is documented in jk-includes/media.php */
		$jk_min_priority_img_pixels = apply_filters( 'jk_min_priority_img_pixels', 50000 );

		if ( $jk_min_priority_img_pixels <= $attr['width'] * $attr['height'] ) {
			jk_increase_content_media_count();
		}
	}
 ...
 *
 * This function runs the {@see 'jk_omit_loading_attr_threshold'} filter, which uses a default threshold value of 3.
 ...
 */
function jk_omit_loading_attr_threshold( $force = false ) {
	...
	if ( ! isset( $omit_threshold ) || $force ) {
		 ...
		 */
		$omit_threshold = apply_filters( 'jk_omit_loading_attr_threshold', 3 );
	}
 ...
 */
function jk_increase_content_media_count( $amount = 1 ) {
 ...
 */
function jk_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that JKPress core would apply on its
		 * own.
		 */
		if ( 'high' === $attr['fetchpriority'] ) {
			$loading_attrs['fetchpriority'] = 'high';
			jk_high_priority_element_flag( false );
		}
	...
	if ( ! jk_high_priority_element_flag() ) {
		return $loading_attrs;
	}
 ...
 */
function jk_high_priority_element_flag( $value = null ) {
 ...
 */
function jk_get_image_editor_output_format( $filename, $mime_type ) {
	$output_format = array(
	 ...
	 *
	 * @see JK_Image_Editor::get_output_format()
	 *
	 * @since 5.8.0
