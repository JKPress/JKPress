...
/**
 * Checks whether JKPress should replace old HTTP URLs to the site with their HTTPS counterpart.
 *
 * If a JKPress site had its URL changed from HTTP to HTTPS, by default this will return `true`, causing JKPress to
 * add frontend filters to replace insecure site URLs that may be present in older database content. The
 * {@see 'jk_should_replace_insecure_home_url'} filter can be used to modify that behavior.
 *
 * @since 5.7.0
 *
 * @return bool True if insecure URLs should replaced, false otherwise.
 */
function jk_should_replace_insecure_home_url() {
	$should_replace_insecure_home_url = jk_is_using_https()
		&& get_option( 'https_migration_required' )
		// For automatic replacement, both 'home' and 'siteurl' need to not only use HTTPS, they also need to be using
		// the same domain.
		&& jk_parse_url( home_url(), PHP_URL_HOST ) === jk_parse_url( site_url(), PHP_URL_HOST );

	/**
	 * Filters whether JKPress should replace old HTTP URLs to the site with their HTTPS counterpart.
	 *
	 * If a JKPress site had its URL changed from HTTP to HTTPS, by default this will return `true`. This filter can
	 ...
	 */
	return apply_filters( 'jk_should_replace_insecure_home_url', $should_replace_insecure_home_url );
}
 ...
 * determined via {@see jk_should_replace_insecure_home_url()}.
 *
 * @since 5.7.0
 *
 ...
 */
function jk_replace_insecure_home_url( $content ) {
	if ( ! jk_should_replace_insecure_home_url() ) {
		return $content;
	}
 ...
 *
 * If this update does not result in JKPress recognizing that the site is now using HTTPS (e.g. due to constants
 ...
 */
function jk_update_urls_to_https() {
 ...
 */
function jk_update_https_migration_required( $old_url, $new_url ) {
	// Do nothing if JKPress is being installed.
	if ( jk_installing() ) {
		return;
	}
