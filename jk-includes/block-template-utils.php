...
// Define constants for supported jk_template_part_area taxonomy.
if ( ! defined( 'JK_TEMPLATE_PART_AREA_HEADER' ) ) {
	define( 'JK_TEMPLATE_PART_AREA_HEADER', 'header' );
}
if ( ! defined( 'JK_TEMPLATE_PART_AREA_FOOTER' ) ) {
	define( 'JK_TEMPLATE_PART_AREA_FOOTER', 'footer' );
}
if ( ! defined( 'JK_TEMPLATE_PART_AREA_SIDEBAR' ) ) {
	define( 'JK_TEMPLATE_PART_AREA_SIDEBAR', 'sidebar' );
}
if ( ! defined( 'JK_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) {
	define( 'JK_TEMPLATE_PART_AREA_UNCATEGORIZED', 'uncategorized' );
}
 ...
 *
 *     @type string $jk_template      Theme-relative directory name for block templates.
 *     @type string $jk_template_part Theme-relative directory name for block template parts.
 * }
 */
function get_block_theme_folders( $theme_stylesheet = null ) {
	$theme = jk_get_theme( (string) $theme_stylesheet );
	if ( ! $theme->exists() ) {
		// Return the default folders if the theme doesn't exist.
		return array(
			'jk_template'      => 'templates',
			'jk_template_part' => 'parts',
		);
	}
...
function get_allowed_block_template_part_areas() {
	$default_area_definitions = array(
		array(
			'area'        => JK_TEMPLATE_PART_AREA_UNCATEGORIZED,
		...
		array(
			'area'        => JK_TEMPLATE_PART_AREA_HEADER,
		...
		array(
			'area'        => JK_TEMPLATE_PART_AREA_FOOTER,
	 ...
	 * }
	 */
	return apply_filters( 'default_jk_template_part_areas', $default_area_definitions );
}
...
function _filter_block_template_part_area( $type ) {
	...
	$warning_message = sprintf(
		...
		__( '"%1$s" is not a supported jk_template_part area value and has been added as "%2$s".' ),
		$type,
		JK_TEMPLATE_PART_AREA_UNCATEGORIZED
	);
	jk_trigger_error( __FUNCTION__, $warning_message );
	return JK_TEMPLATE_PART_AREA_UNCATEGORIZED;
}
 ...
 * @access private
 *
 * @param string $template_type Template type. Either 'jk_template' or 'jk_template_part'.
 * @param string $slug          Template slug.
 * @return array|null {
 *     Array with template metadata if $template_type is one of 'jk_template' or 'jk_template_part',
 *     null otherwise.
 *
 ...
 *     @type string   $area      Template area. Only for 'jk_template_part'.
 *     @type string   $title     Optional. Template title.
 *     @type string[] $postTypes Optional. List of post types that the template supports. Only for 'jk_template'.
 * }
 */
function _get_block_template_file( $template_type, $slug ) {
	if ( 'jk_template' !== $template_type && 'jk_template_part' !== $template_type ) {
		return null;
	}
	...
	foreach ( $themes as $theme_slug => $theme_dir ) {
		...
		if ( file_exists( $file_path ) ) {
			...
			if ( 'jk_template_part' === $template_type ) {
				return _add_block_template_part_area_info( $new_template_item );
			}

			if ( 'jk_template' === $template_type ) {
 ...
 *     @type string   $area         A 'jk_template_part_area' taxonomy value to filter by (for 'jk_template_part' template type only).
...
function _add_block_template_info( $template_item ) {
	...
	$theme_data = jk_get_theme_data_custom_templates();
...
function _add_block_template_part_area_info( $template_info ) {
	if ( jk_theme_has_theme_json() ) {
		$theme_data = jk_get_theme_data_template_parts();
	}
	...
	if ( isset( $theme_data[ $template_info['slug'] ]['area'] ) ) {
		...
		$template_info['area'] = JK_TEMPLATE_PART_AREA_UNCATEGORIZED;
	}
 ...
 * @param string $template_type Template type. Either 'jk_template' or 'jk_template_part'.
 * @return JK_Block_Template Template.
 */
function _build_block_template_result_from_file( $template_file, $template_type ) {
	$default_template_types = get_default_block_template_types();
	$theme                  = get_stylesheet();

	$template                 = new JK_Block_Template();
	...
	if ( 'jk_template' === $template_type ) {
		$registered_template = JK_Block_Templates_Registry::get_instance()->get_by_slug( $template_file['slug'] );
	...
	if ( 'jk_template' === $template_type && isset( $default_template_types[ $template_file['slug'] ] ) ) {
		$template->description = $default_template_types[ $template_file['slug'] ]['description'];
		$template->title       = $default_template_types[ $template_file['slug'] ]['title'];
		$template->is_custom   = false;
	}

	if ( 'jk_template' === $template_type && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'jk_template_part' === $template_type && isset( $template_file['area'] ) ) {
		$template->area = $template_file['area'];
	}

	if ( 'jk_template_part' === $template->type ) {
		/*
 ...
 * @param JK_Block_Template $template  Template to mutate adding the description and title computed.
 * @return bool Returns true if the referenced post was found and false otherwise.
 */
function _jk_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, JK_Block_Template $template ) {
	...
	$posts_with_same_title_query = new JK_Query( $args );
 ...
 */
function _jk_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, JK_Block_Template $template ) {
 ...
 * @access private
 *
 * @param JK_Post $post  Template post.
 ...
 * @return JK_Block_Template|JK_Error Template or error object.
 */
function _build_block_template_object_from_post_object( $post, $terms = array(), $meta = array() ) {
	if ( empty( $terms['jk_theme'] ) ) {
		return new JK_Error( 'template_missing_theme', __( 'No theme is defined for this template.' ) );
	}
	$theme = $terms['jk_theme'];
	...
	$template->is_custom      = empty( $meta['is_jk_suggestion'] );
	$template->author         = $post->post_author;
	$template->modified       = $post->post_modified;

	if ( 'jk_template' === $post->post_type && $has_theme_file && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'jk_template' === $post->post_type && isset( $default_template_types[ $template->slug ] ) ) {
		$template->is_custom = false;
	}

	if ( 'jk_template_part' === $post->post_type && isset( $terms['jk_template_part_area'] ) ) {
		$template->area = $terms['jk_template_part_area'];
	}
...
function _build_block_template_result_from_post( $post ) {
	$post_id = jk_is_post_revision( $post );
	if ( ! $post_id ) {
		$post_id = $post;
	}
	...
	$terms = get_the_terms( $parent_post, 'jk_theme' );
	...
	$terms = array(
		'jk_theme' => $terms[0]->name,
	);

	if ( 'jk_template_part' === $parent_post->post_type ) {
		$type_terms = get_the_terms( $parent_post, 'jk_template_part_area' );
		if ( ! is_jk_error( $type_terms ) && false !== $type_terms ) {
			$terms['jk_template_part_area'] = $type_terms[0]->name;
		}
	}

	$meta = array(
		'origin'           => get_post_meta( $parent_post->ID, 'origin', true ),
		'is_jk_suggestion' => get_post_meta( $parent_post->ID, 'is_jk_suggestion', true ),
	);

	$template = _build_block_template_object_from_post_object( $post, $terms, $meta );

	if ( is_jk_error( $template ) ) {
		return $template;
	}

	// Check for a block template without a description and title or with a title equal to the slug.
	if ( 'jk_template' === $parent_post->post_type && empty( $template->description ) && ( empty( $template->title ) || $template->title === $template->slug ) ) {
		$matches = array();
		...
		if ( preg_match( '/(author|page|single|tag|category|taxonomy)-(.+)/', $template->slug, $matches ) ) {
			...
			switch ( $type ) {
				case 'author':
					...
					break;
				case 'page':
					_jk_build_title_and_description_for_single_post_type_block_template( 'page', $slug_remaining, $template );
					break;
				case 'single':
					...
					foreach ( $post_types as $post_type ) {
						...
						if ( 0 === strncmp( $slug_remaining, $post_type . '-', $post_type_length ) ) {
							...
							$found = _jk_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, $template );
					...
					break;
				case 'tag':
					_jk_build_title_and_description_for_taxonomy_block_template( 'post_tag', $slug_remaining, $template );
					break;
				case 'category':
					_jk_build_title_and_description_for_taxonomy_block_template( 'category', $slug_remaining, $template );
					break;
				case 'taxonomy':
					...
					foreach ( $taxonomies as $taxonomy ) {
						...
						if ( 0 === strncmp( $slug_remaining, $taxonomy . '-', $taxonomy_length ) ) {
							...
							$found = _jk_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, $template );
	...
	if ( 'jk_template' === $post->post_type ) {
		$registered_template = JK_Block_Templates_Registry::get_instance()->get_by_slug( $template->slug );
		if ( $registered_template ) {
			$template->plugin      = $registered_template->plugin;
			$template->origin      =
	...
	if ( 'jk_template_part' === $template->type ) {
		$existing_ignored_hooked_blocks = get_post_meta( $post->ID, '_jk_ignored_hooked_blocks', true );
 ...
 *
 *     @type string[] $slug__in  List of slugs to include.
 *     @type int      $jk_id     Post ID of customized template.
 *     @type string   $area      A 'jk_template_part_area' taxonomy value to filter by (for 'jk_template_part' template type only).
 *     @type string   $post_type Post type to get the templates for.
 * }
 * @param string $template_type Template type. Either 'jk_template' or 'jk_template_part'.
 * @return JK_Block_Template[] Array of block templates.
 */
function get_block_templates( $query = array(), $template_type = 'jk_template' ) {
	/**
	 * Filters the block templates array before the query takes place.
	 *
	 * Return a non-null value to bypass the JKPress queries.
	 *
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template[]|null $block_templates Return an array of block templates to short-circuit the default query,
	...
	if ( 'jk_template_part' === $template_type && isset( $query['area'] ) ) {
		$jk_query_args['tax_query'][]           = array(
			'taxonomy' => 'jk_template_part_area',
			'field'    => 'name',
			'terms'    => $query['area'],
		);
		$jk_query_args['tax_query']['relation'] = 'AND';
	}

	if ( ! empty( $query['slug__in'] ) ) {
		$jk_query_args['post_name__in']  = $query['slug__in'];
		$jk_query_args['posts_per_page'] = count( array_unique( $query['slug__in'] ) );
	}

	// This is only needed for the regular templates/template parts post type listing and editor.
	if ( isset( $query['jk_id'] ) ) {
		$jk_query_args['p'] = $query['jk_id'];
	} else {
		$jk_query_args['post_status'] = 'publish';
	}

	$template_query = new JK_Query( $jk_query_args );
	$query_result   = array();
	...
	if ( ! isset( $query['jk_id'] ) ) {
		 ...
		 */
		$query['slug__not_in'] = jk_list_pluck( $query_result, 'slug' );
		...
		if ( 'jk_template' === $template_type ) {
			...
			$registered_templates          = JK_Block_Templates_Registry::get_instance()->get_by_query( $query );
	 ...
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template[] $query_result Array of found block templates.
	 * @param array               $query {
	 *     Arguments to retrieve templates. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
	 ...
	 *     @type string   $post_type Post type to get the templates for.
	 * }
	 * @param string              $template_type jk_template or jk_template_part.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'jk_template' or 'jk_template_part'.
 *                              Default 'jk_template'.
 * @return JK_Block_Template|null Template.
 */
function get_block_template( $id, $template_type = 'jk_template' ) {
	/**
	 ...
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template|null $block_template Return block template object to short-circuit the default query,
	 *                                               or null to allow JK to run its normal queries.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'jk_template' or 'jk_template_part'.
	 */
	 ...
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template|null $block_template The found block template, or null if there isn't one.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'jk_template' or 'jk_template_part'.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'jk_template' or 'jk_template_part'.
 *                              Default 'jk_template'.
 * @return JK_Block_Template|null The found block template, or null if there isn't one.
 */
function get_block_file_template( $id, $template_type = 'jk_template' ) {
	/**
	 * Filters the block template object before the theme file discovery takes place.
	 *
	 * Return a non-null value to bypass the JKPress theme file discovery.
	 *
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template|null $block_template Return block template object to short-circuit the default query,
	...
	$parts = explode( '//', $id, 2 );
	if ( count( $parts ) < 2 ) {
		/** This filter is documented in jk-includes/block-template-utils.php */
	...
	$block_template = JK_Block_Templates_Registry::get_instance()->get_by_slug( $slug );
	 ...
	 * @since 5.9.0
	 *
	 * @param JK_Block_Template|null $block_template The found block template, or null if there is none.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'jk_template' or 'jk_template_part'.
	 */
	return apply_filters( 'get_block_file_template', $block_template, $id, $template_type );
...
function block_template_part( $part ) {
	$template_part = get_block_template( get_stylesheet() . '//' . $part, 'jk_template_part' );
 ...
 */
function jk_is_theme_directory_ignored( $path ) {
 ...
 *
 * @return JK_Error|string Path of the ZIP file or error on failure.
 */
function jk_generate_block_templates_export_file() {
	$jk_version = jk_get_jk_version();

	if ( ! class_exists( 'ZipArchive' ) ) {
		return new JK_Error( 'missing_zip_package', __( 'Zip Export not supported.' ) );
	}

	$obscura    = jk_generate_password( 12, false, false );
	$theme_name = basename( get_stylesheet() );
	$filename   = get_temp_dir() . $theme_name . $obscura . '.zip';

	$zip = new ZipArchive();
	if ( true !== $zip->open( $filename, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
		return new JK_Error( 'unable_to_create_zip', __( 'Unable to open export file (archive) for writing.' ) );
	}
	...
	$theme_path = jk_normalize_path( get_stylesheet_directory() );
	...
	foreach ( $theme_files as $file ) {
		...
		if ( ! $file->isDir() ) {
			...
			$file_path     = jk_normalize_path( $file );
			$relative_path = substr( $file_path, strlen( $theme_path ) + 1 );

			if ( ! jk_is_theme_directory_ignored( $relative_path ) ) {
	...
	$template_parts = get_block_templates( array(), 'jk_template_part' );
	...
	$tree = JK_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) );
	// Merge with user data.
	$tree->merge( JK_Theme_JSON_Resolver::get_user_data() );

	$theme_json_raw = $tree->get_data();
	// If a version is defined, add a schema.
	if ( $theme_json_raw['version'] ) {
		$theme_json_version = 'jk/' . substr( $jk_version, 0, 3 );
		$schema             = array( '$schema' => 'https://schemas.jk.org/' . $theme_json_version . '/theme.json' );
		$theme_json_raw     = array_merge( $schema, $theme_json_raw );
	}

	// Convert to a string.
	$theme_json_encoded = jk_json_encode( $theme_json_raw, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function get_template_hierarchy( $slug, $is_custom = false, $template_prefix = '' ) {
	if ( 'index' === $slug ) {
		/** This filter is documented in jk-includes/template.php */
 ...
 *
 * Given an object that represents a `jk_template` or `jk_template_part` post object
 ...
 * @param JK_REST_Request $deprecated Deprecated. Not used.
 * @return stdClass|JK_Error The updated object representing a template or template part.
 */
function inject_ignored_hooked_blocks_metadata_attributes( $changes, $deprecated = null ) {
	...
	if ( empty( $changes->ID ) ) {
		...
		$type_terms        = get_the_terms( $changes->ID, 'jk_theme' );
		$terms['jk_theme'] = ! is_jk_error( $type_terms ) && ! empty( $type_terms ) ? $type_terms[0]->name : null;
	}

	// Required for the JK_Block_Template. Update the post object with the current time.
	...
	if ( 'jk_template_part' === $post->post_type && ! isset( $terms['jk_template_part_area'] ) ) {
		$area_terms                     = get_the_terms( $changes->ID, 'jk_template_part_area' );
		$terms['jk_template_part_area'] = ! is_jk_error( $area_terms ) && ! empty( $area_terms ) ? $area_terms[0]->name : null;
	}

	$template = _build_block_template_object_from_post_object( new JK_Post( $post ), $terms, $meta );

	if ( is_jk_error( $template ) ) {
		return $template;
	}

	if ( 'jk_template_part' === $post->post_type ) {
		$attributes                     = array();
		$existing_ignored_hooked_blocks = isset( $post->ID ) ? get_post_meta( $post->ID, '_jk_ignored_hooked_blocks', true ) : '';
		...
		if ( ! empty( $ignored_hooked_blocks ) ) {
			...
			$changes->meta_input['_jk_ignored_hooked_blocks'] = jk_json_encode( $ignored_hooked_blocks );
		}
	} else {
