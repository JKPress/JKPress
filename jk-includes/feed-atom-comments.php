...
<feed
	...
	<?php
		/** This action is documented in jk-includes/feed-atom.php */
		do_action( 'atom_ns' );
...
>
	...
	if ( is_singular() ) {
		...
		printf( ent2ncr( __( 'Comments for %s' ) ), get_jk_title_rss() );
	}
	?>
	</title>
...
while ( have_comments() ) :
	...
	<entry>
		...
		if ( ! is_singular() ) {
			...
			/** This filter is documented in jk-includes/feed.php */
		...
		if ( '0' === $comment->comment_parent ) : // This comment is top-level.
			 ...
			 * but JK doesn't create them for comments (unlike posts).
