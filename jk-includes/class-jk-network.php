 ...
 *
 * This class is most useful in JKPress multi-network installations where the
...
class JK_Network {
	 ...
	 * @return JK_Network|false The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = jk_cache_get( $network_id, 'networks' );

		if ( false === $_network ) {
			$_network = $jkdb->get_row( $jkdb->prepare( "SELECT * FROM {$jkdb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_jk_error( $_network ) ) {
				$_network = -1;
			}

			jk_cache_add( $network_id, $_network, 'networks' );
		}
		...
		return new JK_Network( $_network );
	}

	/**
	 * Creates a new JK_Network object.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @param JK_Network|object $network A network object.
	 */
	...
	private function get_main_site_id() {
		 ...
		 * @param JK_Network $network      The network object for which the main site was detected.
		 */
	...
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		 ...
		 * an object from jk_get_network().
		 *
		 * @since 3.9.0
		 *
		 * @param null|false|JK_Network $network  Network value to return by path. Default null
