...
final class JK_Theme implements ArrayAccess {
	...
	private static $default_themes = array(
		'classic'           => 'JKPress Classic',
		'default'           => 'JKPress Default',
	...
	/**
	 * Absolute path to the theme root, usually jk-content/themes
	 *
	 * @since 3.4.0
	 * @var string
	 */
	...
	/**
	 * URL to the theme root, usually an absolute URL to jk-content/themes
	 *
	 * @since 3.4.0
	 * @var string
	 */
	 ...
	 *
	 * Default is false. Can be set with the {@see 'jk_cache_themes_persistently'} filter.
	 *
	 * @since 3.4.0
	 * @var bool
	 */
	 ...
	 * @param JK_Theme|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'jk_cache_themes_persistently', false, 'JK_Theme' );
			if ( self::$persistently_cache ) {
				jk_cache_add_global_groups( 'themes' );
				if ( is_int( self::$persistently_cache ) ) {
					self::$cache_expiration = self::$persistently_cache;
				}
			} else {
				jk_cache_add_non_persistent_groups( 'themes' );
			}
		}
		...
		if ( ! in_array( $theme_root, (array) $jk_theme_directories, true )
			&& in_array( dirname( $theme_root ), (array) $jk_theme_directories, true )
		) {
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) ) {
				...
				$this->errors = new JK_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			}
			...
			$this->headers['Name']        = $this->stylesheet;
			$this->errors                 = new JK_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template               = $this->stylesheet;
			$this->block_theme            = false;
			$this->block_template_folders = $this->default_template_folders;
			$this->cache_add(
			 ...
			 * Properly identify default themes that are inside a directory within jk-content/themes.
			 */
		...
		if ( ! $this->template ) {
			...
			if ( ! $this->is_block_theme() && ! file_exists( $theme_path . '/index.php' ) ) {
				...
				$this->errors = new JK_Error( 'theme_no_index', $error_message );
				$this->cache_add(
					'theme',
					array(
						'block_template_folders' => $this->get_block_template_folders(),
		...
		if ( ! is_array( $cache )
			 ...
			 * jk-content/themes/directory-of-themes/*
			 */
			...
			if ( '.' !== $parent_dir
				...
				$this->parent = new JK_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template !== $this->stylesheet ) {
			...
			if ( $_child instanceof JK_Theme && $_child->template === $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new JK_Error(
			...
			$this->parent = new JK_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}

		if ( jk_paused_themes()->get( $this->stylesheet ) && ( ! is_jk_error( $this->errors ) || ! isset( $this->errors->errors['theme_paused'] ) ) ) {
			$this->errors = new JK_Error( 'theme_paused', __( 'This theme failed to load properly and was paused within the admin backend.' ) );
		}
	 ...
	 * @since 3.4.0
	 *
	 * @return JK_Error|false JK_Error if there are errors, or false.
	 */
	public function errors() {
		return is_jk_error( $this->errors ) ? $this->errors : false;
	}
	 ...
	 * @since 3.4.0
	 *
	 * @return JK_Theme|false Parent theme, or false if the active theme is not a child theme.
	 */
	 ...
	 * @return bool Return value from jk_cache_add()
	 */
	private function cache_add( $key, $data ) {
		return jk_cache_add( $key . '-' . $this->cache_hash, $data, 'themes', self::$cache_expiration );
	}
	...
	private function cache_get( $key ) {
		return jk_cache_get( $key . '-' . $this->cache_hash, 'themes' );
	}
	...
	public function cache_delete() {
		foreach ( array( 'theme', 'screenshot', 'headers', 'post_templates' ) as $key ) {
			jk_cache_delete( $key . '-' . $this->cache_hash, 'themes' );
		}
		$this->template               = null;
	 ...
	 * get_template() takes into account where JKPress actually located the theme and
	...
	private function sanitize_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Name':
				...
				$value = jk_kses( $value, $header_tags );
				break;
			case 'Author':
			...
			case 'Description':
				...
				$value = jk_kses( $value, $header_tags_with_a );
				break;
			case 'ThemeURI':
			case 'AuthorURI':
	...
	private function markup_header( $header, $value, $translate ) {
		switch ( $header ) {
			...
			case 'Tags':
				static $comma = null;
				if ( ! isset( $comma ) ) {
					$comma = jk_get_list_item_separator();
				}
	 ...
	 *
	 * This is typically the absolute path to jk-content/themes.
	 *
	 * @since 3.4.0
	 *
	 ...
	 *
	 * This is typically the absolute URL to jk-content/themes. This forms the basis
	 * for all other URLs returned by JK_Theme, so we pass it to the public function
	...
	public function get_post_templates() {
		...
		if ( current_theme_supports( 'block-templates' ) ) {
			$block_templates = get_block_templates( array(), 'jk_template' );
	...
	public function get_page_templates( $post = null, $post_type = 'page' ) {
		 ...
		 * @param JK_Theme     $theme          The theme object.
		 * @param JK_Post|null $post           The post being edited, provided for context, or null.
	...
	/**
	 * Determines the latest JKPress default theme that is installed.
	 *
	 * This hits the filesystem.
	 *
	 * @since 4.4.0
	 *
	 * @return JK_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	...
	public static function get_allowed_on_site( $blog_id = null ) {
		...
		/** This filter is documented in jk-includes/class-jk-theme.php */
		return (array) apply_filters( 'site_allowed_themes', $allowed_themes[ $blog_id ], $blog_id );
	}
	...
	public function get_block_template_folders() {
		...
		if ( file_exists( $stylesheet_directory . '/block-templates' ) || file_exists( $stylesheet_directory . '/block-template-parts' ) ) {
			$this->block_template_folders = array(
				'jk_template'      => 'block-templates',
				'jk_template_part' => 'block-template-parts',
			);
		}
		return $this->block_template_folders;
	}
		...
		foreach ( $files as $file ) {
			...
			foreach ( $properties_to_parse as $property ) {
				if ( ! empty( $pattern[ $property ] ) ) {
					$pattern[ $property ] = array_filter( jk_parse_list( (string) $pattern[ $property ] ) );
				} else {
	...
	private function get_pattern_cache() {
		...
		$pattern_data = get_site_transient( 'jk_theme_files_patterns-' . $this->cache_hash );
	...
	private function set_pattern_cache( array $patterns ) {
		 ...
		 */
		$cache_expiration = (int) apply_filters( 'jk_theme_files_cache_ttl', self::$cache_expiration, 'theme_block_patterns' );
		...
		if ( $cache_expiration <= 0 ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					'<code>jk_theme_files_cache_ttl</code>'
				),
				'6.6.0'
			);
		...
		set_site_transient( 'jk_theme_files_patterns-' . $this->cache_hash, $pattern_data, $cache_expiration );
	}
	...
	public function delete_pattern_cache() {
		delete_site_transient( 'jk_theme_files_patterns-' . $this->cache_hash );
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param JK_Theme[] $themes Array of theme objects to sort (passed by reference).
	 */
	public static function sort_by_name( &$themes ) {
		if ( str_starts_with( get_user_locale(), 'en_' ) ) {
			uasort( $themes, array( 'JK_Theme', '_name_sort' ) );
		} else {
			foreach ( $themes as $key => $theme ) {
				$theme->translate_header( 'Name', $theme->headers['Name'] );
			}
			uasort( $themes, array( 'JK_Theme', '_name_sort_i18n' ) );
		}
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param JK_Theme $a First theme.
	 * @param JK_Theme $b Second theme.
