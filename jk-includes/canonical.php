<?php
/**
 * Canonical API to handle JKPress Redirecting
 *
 ...
 * page/post previews, JK admin, Trackbacks, robots.txt, favicon.ico, searches,
 * or on POST requests.
 *
 * Will also attempt to find the correct link when a user enters a URL that does
 * not exist based on exact JKPress query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $jk_rewrite, $is_IIS, $jk_query, $jkdb, $jk;
	...
	/*
	 * If we're not in jk-admin and the post has been published and preview nonce
	 * is non-existent or invalid then no need for preview in query.
	 */
	if ( is_preview() && get_query_var( 'p' ) && 'publish' === get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			|| ! isset( $_GET['preview_nonce'] )
			|| ! jk_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] )
		) {
			$jk_query->is_preview = false;
		}
	}
	...
	if ( is_singular() && $jk_query->post_count < 1 && $post_id ) {

		$vars = $jkdb->get_results( $jkdb->prepare( "SELECT post_type, post_parent FROM $jkdb->posts WHERE ID = %d", $post_id ) );
	...
	// These tests give us a JK-generated permalink.
	if ( is_404() ) {
		...
		if ( $year && $month && $day ) {
			...
			if ( ! jk_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) ) {
			...
			if ( $jk_query->queried_object instanceof JK_Post ) {
				$post_id = $jk_query->queried_object->ID;
			} elseif ( $jk_query->post ) {
				$post_id = $jk_query->post->ID;
			}
	...
	} elseif ( is_object( $jk_rewrite ) && $jk_rewrite->using_permalinks() ) {

		// Rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101.
		if ( is_attachment()
			&& ! array_diff( array_keys( $jk->query_vars ), array( 'attachment', 'attachment_id' ) )
			&& ! $redirect_url
		) {
			...
			$redirect_url = get_permalink( $jk_query->get_queried_object_id() );
			$redirect_obj = get_post( $jk_query->get_queried_object_id() );
			...
			if ( false !== $author
				&& $jkdb->get_var( $jkdb->prepare( "SELECT ID FROM $jkdb->posts WHERE $jkdb->posts.post_author = %d AND $jkdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) )
			) {
			...
			if ( $term_count <= 1 && ! empty( $obj->term_id ) ) {
				...
				if ( $tax_url && ! is_jk_error( $tax_url ) ) {
					if ( ! empty( $redirect['query'] ) ) {
						...
						$rewrite_vars = array_diff( array_keys( $jk_query->query ), array_keys( $_GET ) );
		...
		} elseif ( is_single() && str_contains( $jk_rewrite->permalink_structure, '%category%' ) ) {
			$category_name = get_query_var( 'category_name' );

			if ( $category_name ) {
				$category = get_category_by_path( $category_name );

				if ( ! $category || is_jk_error( $category )
					|| ! has_term( $category->term_id, 'category', $jk_query->get_queried_object_id() )
				) {
					$redirect_url = get_permalink( $jk_query->get_queried_object_id() );
					$redirect_obj = get_post( $jk_query->get_queried_object_id() );
				}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				...
				if ( is_front_page() ) {
					$redirect_url .= user_trailingslashit( "$jk_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'sitemap' ) ) {
			...
			while ( preg_match( "#/$jk_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] )
				|| preg_match( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+)?$#', $redirect['path'] )
				|| preg_match( "#/{$jk_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] )
			) {
				// Strip off any existing paging.
				$redirect['path'] = preg_replace( "#/$jk_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] );
				// Strip off feed endings.
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] );
				// Strip off any existing comment paging.
				$redirect['path'] = preg_replace( "#/{$jk_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] );
			}

			$addl_path    = '';
			$default_feed = get_default_feed();

			if ( is_feed() && in_array( $feed, $jk_rewrite->feeds, true ) ) {
				...
				$old_feed_files = array(
					'jk-atom.php'         => 'atom',
					'jk-commentsrss2.php' => 'comments_rss2',
					'jk-feed.php'         => $default_feed,
					'jk-rdf.php'          => 'rdf',
					'jk-rss.php'          => 'rss2',
					'jk-rss2.php'         => 'rss2',
				);
			...
			if ( $paged > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( ! is_single() ) {
						...
						if ( $paged > 1 ) {
							$addl_path .= user_trailingslashit( "$jk_rewrite->pagination_base/$paged", 'paged' );
						}
					}
				} elseif ( $paged > 1 ) {
			...
			if ( get_option( 'page_comments' )
				...
				$addl_path  = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' );
				$addl_path .= user_trailingslashit( $jk_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' );

				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			// Strip off trailing /index.php/.
			$redirect['path'] = preg_replace( '|/' . preg_quote( $jk_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] );
			$redirect['path'] = user_trailingslashit( $redirect['path'] );

			if ( ! empty( $addl_path )
				&& $jk_rewrite->using_index_permalinks()
				&& ! str_contains( $redirect['path'], '/' . $jk_rewrite->index . '/' )
			) {
				$redirect['path'] = trailingslashit( $redirect['path'] ) . $jk_rewrite->index . '/';
			}
		...
		if ( 'jk-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in jk-login.php */
				$redirect_url = apply_filters( 'jk_signup_location', network_site_url( 'jk-signup.php' ) );
			} else {
				$redirect_url = jk_registration_url();
			}
	...
	if ( ! $jk_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $jk_rewrite->index . '/', '/', $redirect['path'] );
	}

	// Trailing slashes.
	if ( is_object( $jk_rewrite ) && $jk_rewrite->using_permalinks()
	...
	if ( $redirect_obj instanceof JK_Post ) {
...
function redirect_guess_404_permalink() {
	...
	if ( get_query_var( 'name' ) ) {
		...
		if ( $strict_guess ) {
			$where = $jkdb->prepare( 'post_name = %s', get_query_var( 'name' ) );
		} else {
			$where = $jkdb->prepare( 'post_name LIKE %s', $jkdb->esc_like( get_query_var( 'name' ) ) . '%' );
		}
		...
		if ( get_query_var( 'post_type' ) ) {
			if ( is_array( get_query_var( 'post_type' ) ) ) {
				...
				$where .= $jkdb->prepare( ' AND post_type = %s', get_query_var( 'post_type' ) );
			}
		} else {
		...
		if ( get_query_var( 'year' ) ) {
			$where .= $jkdb->prepare( ' AND YEAR(post_date) = %d', get_query_var( 'year' ) );
		}
		if ( get_query_var( 'monthnum' ) ) {
			$where .= $jkdb->prepare( ' AND MONTH(post_date) = %d', get_query_var( 'monthnum' ) );
		}
		if ( get_query_var( 'day' ) ) {
			$where .= $jkdb->prepare( ' AND DAYOFMONTH(post_date) = %d', get_query_var( 'day' ) );
		}

		// phpcs:ignore JKPress.DB.PreparedSQL.InterpolatedNotPrepared
		$post_id = $jkdb->get_var( "SELECT ID FROM $jkdb->posts WHERE $where AND post_status IN ('" . implode( "', '", esc_sql( $publicly_viewable_statuses ) ) . "')" );
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /jk-admin.
 * Visiting /login redirects to /jk-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global JK_Rewrite $jk_rewrite JKPress rewrite component.
 */
function jk_redirect_admin_locations() {
	global $jk_rewrite;

	if ( ! ( is_404() && $jk_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'jk-admin', 'relative' ),
	...
	if ( in_array( untrailingslashit( $_SERVER['REQUEST_URI'] ), $admins, true ) ) {
		jk_redirect( admin_url() );
		exit;
	}

	$logins = array(
		home_url( 'jk-login.php', 'relative' ),
