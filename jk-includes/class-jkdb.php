 ...
 * By default, JKPress uses this class to instantiate the global $jkdb object, providing
 * access to the JKPress database.
 *
 * It is possible to replace this class with your own by setting the $jkdb global variable
 * in jk-content/db.php file to your class. The jkdb class will still be included, so you can
 * extend it or simply use your own.
 *
 * @link https://developer.wordpress.org/reference/classes/jkdb/
 *
 * @since 0.71
 */
...
class jkdb {
	 ...
	 *
	 * Default is to show errors if both JK_DEBUG and JK_DEBUG_DISPLAY evaluate to true.
	 *
	 * @since 0.71
	 *
	 * @var bool
	 ...
	 * @since 3.9.0
	 *
	 * @see jkdb::check_connection()
	 * @var int
	 */
	protected $reconnect_retries = 5;

	/**
	 * JKPress table prefix.
	 *
	 * You can set this to have multiple JKPress installations in a single database.
	...
	/**
	 * List of JKPress per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see jkdb::tables()
	...
	/**
	 * List of deprecated JKPress tables.
	 *
	...
	/**
	 * List of deprecated JKPress Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see jkdb::tables()
	...
	/**
	 * JKPress Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * JKPress Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 *
	 * Columns not listed here default to %s. Initialized during JK load.
	 ...
	 *
	 * Set in jkdb::db_connect() to true, by default. This is used when checking
	 * against the required MySQL version for JKPress. Normally, a replacement
	...
	/**
	 * Backward compatibility, where jkdb::prepare() has not quoted formatted/argnum placeholders.
	 *
	 * This is often used for table/field names (before %i was supported), and sometimes string formatting, e.g.
	 *
	 *     $jkdb->prepare( 'WHERE `%1$s` = "%2$s something %3$s" OR %1$s = "%4$-10s"', 'field_1', 'a', 'b', 'c' );
	 *
	 * But it's risky, e.g. forgetting to add quotes, resulting in SQL Injection vulnerabilities:
	 *
	 *     $jkdb->prepare( 'WHERE (id = %1s) OR (id = %2$s)', $_GET['id'], $_GET['id'] ); // ?id=id
	 *
	 * This feature is preserved while plugin authors update their code to use safer approaches:
	 *
	 *     $_GET['key'] = 'a`b';
	 *
	 *     $jkdb->prepare( 'WHERE %1s = %s',        $_GET['key'], $_GET['value'] ); // WHERE a`b = 'value'
	 *     $jkdb->prepare( 'WHERE `%1$s` = "%2$s"', $_GET['key'], $_GET['value'] ); // WHERE `a`b` = "value"
	 *
	 *     $jkdb->prepare( 'WHERE %i = %s',         $_GET['key'], $_GET['value'] ); // WHERE `a``b` = 'value'
	 *
	 ...
	 *
	 *     $jkdb->prepare( 'WHERE %1$s = "%2$-10s"', 'my_field', 'my_value' );
	 ...
	 *
	 *     $jkdb->prepare( 'WHERE id = %1$s', $_GET['id'] ); // ?id=id
	 *
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		// jk-config.php creation will manually connect when ready.
	...
	/**
	 * Changes the current SQL mode, and ensures its JKPress compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the JKPress tables.
	 *
	 * @since 2.5.0
	 *
	 * @param string $prefix          Alphanumeric name for the new prefix.
	 * @param bool   $set_table_names Optional. Whether the table names, e.g. jkdb::$posts,
	 *                                should be updated or not. Default true.
	 * @return string|JK_Error Old prefix or JK_Error on error.
	 */
	public function set_prefix( $prefix, $set_table_names = true ) {

		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new JK_Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	...
	/**
	 * Returns an array of JKPress tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the JKPress users
	 ...
	 *
	 * @uses jkdb::$tables
	 * @uses jkdb::$old_tables
	 * @uses jkdb::$global_tables
	 * @uses jkdb::$ms_global_tables
	 * @uses jkdb::$old_ms_global_tables
	 *
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">JKPress support forums</a>.' ),
	 ...
	 *
	 * Use esc_sql() or jkdb::prepare() instead.
	 *
	 * @since 2.8.0
	 * @deprecated 3.6.0 Use jkdb::prepare()
	 * @see jkdb::prepare()
	 * @see esc_sql()
	 *
	 * @param string $data
	...
	public function _weak_escape( $data ) {
		if ( func_num_args() === 1 && function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __METHOD__, '3.6.0', 'jkdb::prepare() or esc_sql()' );
		}
	...
	public function _real_escape( $data ) {
		...
		if ( $this->dbh ) {
			...
			jk_load_translations_early();
			/* translators: %s: Database access abstraction class, usually jkdb or a class extending jkdb. */
	 ...
	 *              Check support via `jkdb::has_cap( 'identifier_placeholders' )`.
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of JKPress core, e.g. by HyperDB, in which case
		...
		while ( $key < $split_query_count ) {
			...
			if ( 'f' === $type && true === $this->allow_unsafe_unquoted_parameters
				...
				/*
				 * Before JK 6.2 the "force floats to be locale-unaware" RegEx didn't
	 ...
	 *
	 * Use this only before jkdb::prepare() or esc_sql(). Reversing the order is very bad for security.
	 *
	 * Example Prepared Statement:
	 *
	 *     $wild = '%';
	 *     $find = 'only 43% of planets';
	 *     $like = $wild . $jkdb->esc_like( $find ) . $wild;
	 *     $sql  = $jkdb->prepare( "SELECT * FROM $jkdb->posts WHERE post_content LIKE %s", $like );
	 *
	 * Example Escape Chain:
	 *
	 *     $sql  = esc_sql( $jkdb->esc_like( $input ) );
	 *
	 * @since 4.0.0
	 *
	 ...
	 *                Call jkdb::prepare() or jkdb::_real_escape() next.
	 */
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'JKPress database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'JKPress database error %1$s for query %2$s', $str, $this->last_query );
		}
	 ...
	 * jkdb::hide_errors() should be used instead for hiding errors.
	 *
	 * @since 0.71
	 *
	 * @see jkdb::hide_errors()
	 *
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Set the MySQLi error reporting off because JKPress handles its own.
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: jk-config.php, 2: Database host. */
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			 ...
			 */
			if ( $this->reconnect_retries === $tries && JK_DEBUG ) {
				error_reporting( $error_reporting );
			}
		...
		/*
		 * If template_redirect has already happened, it's too late for jk_die()/dead_db().
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'JKPress database error: Could not perform query because it contains invalid data.' );
	...
	public function placeholder_escape() {
		...
		if ( ! $placeholder ) {
			...
			// Old JK installs may not have AUTH_SALT defined.
	 ...
	 *                                specified in jkdb::$field_types. Default null.
	 ...
	 *
	 * This is a helper method for jkdb's CRUD methods, which take field/value pairs
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'JKPress database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'JKPress database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	/**
	 * Prepares arrays of value/format pairs as passed to jkdb CRUD methods.
	 *
	 * @since 4.2.0
	 *
	 * @param array           $data   Array of values keyed by their field names.
	...
	/**
	 * Adds field charsets to field/value/format arrays generated by jkdb::process_field_formats().
	 *
	 * @since 4.2.0
	 *
	 * @param array $data {
	 *     Array of values and formats keyed by their field names,
	 *     as it comes from the jkdb::process_field_formats() method.
	 *
	 *     @type array ...$0 {
	 ...
	 *     as it comes from the jkdb::process_field_charsets() method.
	 *
	 *     @type array ...$0 {
	 *         Value, format, and charset for this field.
	 *
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|JK_Error Table character set, JK_Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|JK_Error|null $charset The character set to use, JK_Error object
		...
		if ( ! $results ) {
			return new JK_Error( 'wpdb_get_table_charset_failure', __( 'Could not retrieve table charset.' ) );
		}
	 ...
	 * @return string|false|JK_Error Column character set as a string. False if the column has
	 *                               no character set. JK_Error object if there was an error.
	 */
	public function get_col_charset( $table, $column ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|null|false|JK_Error $charset The character set to use. Default null.
		 * @param string                     $table   The name of the table being checked.
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|JK_Error {
	 *     Array of column length information, false if the column has no length (for
	 *     example, numeric column), JK_Error object if there was an error.
	 *
	 *     @type string $type   One of 'byte' or 'char'.
	 *     @type int    $length The column length.
	 * }
	 ...
	 * @return array|JK_Error The $data parameter, with invalid characters removed from each value.
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				return new JK_Error( 'wpdb_strip_invalid_text_failure', __( 'Could not strip invalid text.' ) );
			}
	 ...
	 * @return string|JK_Error The converted query, or a JK_Error object if the conversion fails.
	 */
	 ...
	 * @return string|JK_Error The converted string, or a JK_Error object if the conversion fails.
	 */
	...
	protected function get_table_from_query( $query ) {
		...
		// SHOW TABLE STATUS and SHOW TABLES WHERE Name = 'jk_posts'
		if ( preg_match( '/^\s*SHOW\s+(?:TABLE\s+STATUS|(?:FULL\s+)?TABLES).+WHERE\s+Name\s*=\s*("|\')((?:[0-9a-zA-Z$_.-]|[\xC2-\xDF][\x80-\xBF])+)\\1/is', $query, $maybe ) ) {
			return $maybe[2];
		}

		/*
		 * SHOW TABLE STATUS LIKE and SHOW TABLES LIKE 'jk\_123\_%'
		 * This quoted LIKE operand seldom holds a full table name.
		 * It is usually a pattern for matching a prefix so we just
		 * strip the trailing % and unescape the _ to get 'jk_123_'
	 ...
	 *
	 * Will not die if jkdb::$show_errors is false.
	 *
	 * @since 1.5.0
	 *
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( 'JK_Error', false ) ) {
				$this->error = new JK_Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: JKPress version number, 2: Minimum required MySQL version number. */
			return new JK_Error( 'database_version', sprintf( __( '<strong>Error:</strong> JKPress %1$s requires MySQL %2$s or higher' ), $jk_version, $required_mysql_version ) );
		}
	}
	 ...
	 *
	 * Called when JKPress is generating the table scheme.
	 *
	 * Use `jkdb::has_cap( 'collation' )`.
	 *
	 * @since 2.5.0
	 * @deprecated 3.5.0 Use jkdb::has_cap()
	 *
	 * @return bool True if collation is supported, false if not.
	 */
	public function supports_collation() {
		_deprecated_function( __FUNCTION__, '3.5.0', 'jkdb::has_cap( \'collation\' )' );
	...
	/**
	 * Determines whether the database or JKDB supports a particular feature.
	 *
	 * Capability sniffs for the database server and current version of JKDB.
	 *
	 * Database sniffs are based on the version of MySQL the site is using.
	 *
	 * JKDB sniffs are added as new features are introduced to allow theme and plugin
	 * developers to determine feature support. This is to account for drop-ins which may
	 * introduce feature support at a different time to JKPress.
	 *
	 * @since 2.7.0
	...
	public function has_cap( $db_cap ) {
		...
		switch ( strtolower( $db_cap ) ) {
			...
			case 'identifier_placeholders': // @since 6.2.0
				/*
				 * As of JKPress 6.2, jkdb::prepare() supports identifiers via '%i',
	...
	/**
	 * Retrieves a comma-separated list of the names of the functions that called jkdb.
	 *
	 * @since 2.5.0
	 *
	...
	public function get_caller() {
		return jk_debug_backtrace_summary( __CLASS__ );
	}
