<?php
/**
 * Dependencies API: JK_Scripts class
 *
 * @since 2.6.0
 *
 * @package JKPress
 ...
 * @see JK_Dependencies
 */
class JK_Scripts extends JK_Dependencies {
	/**
	...
	public function init() {
		/**
		 * Fires when the JK_Scripts instance is initialized.
		 *
		 * @since 2.6.0
		 *
		 * @param JK_Scripts $jk_scripts JK_Scripts instance (passed by reference).
		 */
		do_action_ref_array( 'jk_default_scripts', array( &$this ) );
	}
	...
	public function print_scripts_l10n( $handle, $display = true ) {
		_deprecated_function( __FUNCTION__, '3.3.0', 'JK_Scripts::print_extra_script()' );
	...
	public function print_extra_script( $handle, $display = true ) {
		...
		jk_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-extra" ) );
	...
	public function do_item( $handle, $group = false ) {
		...
		if ( $translations ) {
			$translations = jk_get_inline_script_tag( $translations, array( 'id' => "{$handle}-js-translations" ) );
		}
		...
		if ( $intended_strategy ) {
			$attr['data-jk-strategy'] = $intended_strategy;
		}
		$tag  = $translations . $ie_conditional_prefix . $before_script;
		$tag .= jk_get_script_tag( $attr );
	...
	public function print_inline_script( $handle, $position = 'after', $display = true ) {
		_deprecated_function( __METHOD__, '6.3.0', 'JK_Scripts::get_inline_script_data() or JK_Scripts::get_inline_script_tag()' );
	...
	public function get_inline_script_tag( $handle, $position = 'after' ) {
		...
		return jk_get_inline_script_tag( $js, compact( 'id' ) );
	}
	...
	public function localize( $handle, $object_name, $l10n ) {
		...
		if ( ! is_array( $l10n ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: 1: $l10n, 2: jk_add_inline_script() */
		...
		$script = "var $object_name = " . jk_json_encode( $l10n ) . ';';
	...
	public function set_translations( $handle, $domain = 'default', $path = '' ) {
		...
		if ( ! in_array( 'jk-i18n', $obj->deps, true ) ) {
			$obj->deps[] = 'jk-i18n';
		}
...
( function( domain, translations ) {
	...
	jk.i18n.setLocaleData( localeData, domain );
} )( "{$domain}", {$json_translations} );
JS;

		if ( $display ) {
			jk_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-translations" ) );
		}
	...
	public function in_default_dir( $src ) {
		...
		if ( str_starts_with( $src, '/' . JKINC . '/js/l10n' ) ) {
			return false;
		}
	...
	/**
	 * This overrides the add_data method from JK_Dependencies, to support normalizing of $args.
	 *
	 * @since 6.3.0
	 *
	 * @param string $handle Name of the item. Should be unique.
