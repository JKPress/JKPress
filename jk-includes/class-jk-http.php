...
if ( ! class_exists( 'WpOrg\Requests\Autoload' ) ) {
	require ABSPATH . JKINC . '/Requests/src/Autoload.php';

	WpOrg\Requests\Autoload::register();
	WpOrg\Requests\Requests::set_certificate_path( ABSPATH . JKINC . '/certificates/ca-bundle.crt' );
}
...
class JK_Http {
	 ...
	 *                                             Default 'JKPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through jk_http_validate_url().
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . JKINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be dropped it in the JK temp dir and its name will
	 ...
	 *     Array of response data, or a JK_Error instance upon error.
	 *
	 ...
	 *     }
	 *     @type JK_HTTP_Cookie[]                                  $cookies       Array of cookies set by the server.
	 *     @type string|null                                       $filename      Optional. Filename of the response.
	 *     @type JK_HTTP_Requests_Response|null                    $http_response Response object.
	 * }
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			 ...
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param string $user_agent JKPress user agent string.
			 * @param string $url        The request URL.
			 */
			'user-agent'          => apply_filters( 'http_headers_useragent', 'JKPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url ),
			/**
			 * Filters whether to pass URLs through jk_http_validate_url() in an HTTP request.
			 *
			 * @since 3.6.0
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param bool   $pass_url Whether to pass URLs through jk_http_validate_url(). Default false.
			 * @param string $url      The request URL.
			 */
			...
			'decompress'          => true,
			'sslverify'           => true,
			'sslcertificates'     => ABSPATH . JKINC . '/certificates/ca-bundle.crt',
			'stream'              => false,
			'filename'            => null,
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|JK_Error $response    A preemptive return value of an HTTP request. Default false.
		...
		if ( function_exists( 'jk_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
				$url = jk_http_validate_url( $url );
			}
			if ( $url ) {
				$url = jk_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $parsed_url['scheme'] ) ) {
			$response = new JK_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
			/** This action is documented in jk-includes/class-jk-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}

		if ( $this->block_request( $url ) ) {
			$response = new JK_Error( 'http_request_not_executed', __( 'User has blocked requests through HTTP.' ) );
			/** This action is documented in jk-includes/class-jk-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}

		// If we are streaming to a file but no filename was given drop it in the JK temp dir
		...
		if ( $parsed_args['stream'] ) {
			...
			if ( ! jk_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				$response = new JK_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
				/** This action is documented in jk-includes/class-jk-http.php */
				do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
				return $response;
			}
		...
		// JK allows passing in headers as a string, weirdly.
		if ( ! is_array( $parsed_args['headers'] ) ) {
			$processed_headers      = JK_Http::processHeaders( $parsed_args['headers'] );
		...
		$options = array(
			...
			'hooks'     => new JK_HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		if ( function_exists( 'jk_kses_bad_protocol' ) && $parsed_args['reject_unsafe_urls'] ) {
		...
		if ( ! empty( $parsed_args['cookies'] ) ) {
			$options['cookies'] = JK_Http::normalize_cookies( $parsed_args['cookies'] );
		}
		...
		try {
			...
			$http_response = new JK_HTTP_Requests_Response( $requests_response, $parsed_args['filename'] );
			$response      = $http_response->to_array();

			// Add the original object to the array.
			$response['http_response'] = $http_response;
		} catch ( WpOrg\Requests\Exception $e ) {
			$response = new JK_Error( 'http_request_failed', $e->getMessage() );
		}
		 ...
		 * @since 2.8.0
		 *
		 * @param array|JK_Error $response    HTTP response or JK_Error object.
	...
	public static function normalize_cookies( $cookies ) {
		...
		foreach ( $cookies as $name => $value ) {
			if ( $value instanceof JK_Http_Cookie ) {
	...
	public static function validate_redirects( $location ) {
		if ( ! jk_http_validate_url( $location ) ) {
			throw new WpOrg\Requests\Exception( __( 'A valid URL was not provided.' ), 'jk_http.redirect_failed_validation' );
		}
	}
	 ...
	 *
	 * @since 3.2.0
	 * @deprecated 5.1.0 Use JK_Http::request()
	 * @see JK_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
	 * @return array|JK_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A JK_Error instance upon error.
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new JK_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
	 ...
	 * @return array|JK_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A JK_Error instance upon error. See JK_Http::response() for details.
	 */
	 ...
	 * }
	 */
	public static function processResponse( $response ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		$response = explode( "\r\n\r\n", $response, 2 );
	 ...
	 *     @type JK_Http_Cookie[] $cookies    If the original headers contain the 'Set-Cookie' key,
	 *                                        an array containing `JK_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new JK_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to JK_Http_Cookie instances,
	 ...
	 */
	public static function buildCookieHeader( &$r ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		if ( ! empty( $r['cookies'] ) ) {
			// Upgrade any name => value cookie pairs to JK_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) ) {
					$r['cookies'][ $name ] = new JK_Http_Cookie(
						array(
							'name'  => $name,
							'value' => $value,
						)
	 ...
	 */
	public static function chunkTransferDecode( $body ) { // phpcs:ignore JKPress.NamingConventions.ValidFunctionName.MethodNameInvalid
	 ...
	 *
	 * You block external URL requests by defining `JK_HTTP_BLOCK_EXTERNAL` as true in your `jk-config.php`
	 * file and this will only allow localhost and your site to make requests. The constant
	 * `JK_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `JK_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.wordpress.org` will allow for all subdomains of `wordpress.org` to be contacted.
	 *
	 * @since 2.8.0
	 *
	 * @link https://core.trac.wordpress.org/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.wordpress.org/ticket/14636 Allow wildcard domains in JK_ACCESSIBLE_HOSTS
	 *
	...
	public function block_request( $uri ) {
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', JK_ACCESSIBLE_HOSTS );

			if ( str_contains( JK_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 *
	 * @deprecated 4.4.0 Use jk_parse_url()
	 * @see jk_parse_url()
	 *
	...
	protected static function parse_url( $url ) {
		_deprecated_function( __METHOD__, '4.4.0', 'jk_parse_url()' );
		return jk_parse_url( $url );
	}
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = jk_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = jk_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|JK_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a JK_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new JK_Error( 'http_request_failed', __( 'Too many redirects.' ) );
		}
		...
		$redirect_location = JK_Http::make_absolute_url( $redirect_location, $url );
		...
		return jk_remote_request( $redirect_location, $args );
	}
