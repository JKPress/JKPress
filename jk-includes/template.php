...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof JK_Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
 ...
 */
function jk_set_template_globals() {
	global $jk_stylesheet_path, $jk_template_path;

	$jk_stylesheet_path = get_stylesheet_directory();
	$jk_template_path   = get_template_directory();
}
 ...
 * jk-includes/theme-compat so that themes which inherit from a parent theme
...
function locate_template( $template_names, $load = false, $load_once = true, $args = array() ) {
	...
	if ( ! isset( $jk_stylesheet_path ) || ! isset( $jk_template_path ) ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists( $jk_stylesheet_path . '/' . $template_name ) ) {
			$located = $jk_stylesheet_path . '/' . $template_name;
			break;
		} elseif ( $is_child_theme && file_exists( $jk_template_path . '/' . $template_name ) ) {
			$located = $jk_template_path . '/' . $template_name;
			break;
		} elseif ( file_exists( ABSPATH . JKINC . '/theme-compat/' . $template_name ) ) {
			$located = ABSPATH . JKINC . '/theme-compat/' . $template_name;
			break;
		}
	}
...
/**
 * Requires the template file with JKPress environment.
 *
 * The globals are set up for the template file to ensure that the JKPress
...
function load_template( $_template_file, $load_once = true, $args = array() ) {
	global $posts, $post, $jk_did_header, $jk_query, $jk_rewrite, $jkdb, $jk_version, $jk, $id, $comment, $user_ID;

	if ( is_array( $jk_query->query_vars ) ) {
		/*
		 ...
		 */
		// phpcs:ignore JKPress.PHP.DontExtract.extract_extract
		extract( $jk_query->query_vars, EXTR_SKIP );
	}
	 ...
	 */
	do_action( 'jk_before_load_template', $_template_file, $load_once, $args );
	 ...
	 * @param array  $args           Additional arguments passed to the template.
	 */
	do_action( 'jk_after_load_template', $_template_file, $load_once, $args );
}
