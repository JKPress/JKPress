...
function generate_block_asset_handle( $block_name, $field_name, $index = 0 ) {
	if ( str_starts_with( $block_name, 'core/' ) ) {
		$asset_handle = str_replace( 'core/', 'jk-block-', $block_name );
...
function get_block_asset_url( $path ) {
	...
	if ( ! $wpinc_path_norm ) {
		$wpinc_path_norm = jk_normalize_path( realpath( ABSPATH . JKINC ) );
	}
	...
	if ( ! isset( $template_paths_norm[ $template ] ) ) {
		$template_paths_norm[ $template ] = jk_normalize_path( realpath( get_template_directory() ) );
	}
	...
	if ( is_child_theme() ) {
		...
		if ( ! isset( $template_paths_norm[ $stylesheet ] ) ) {
			$template_paths_norm[ $stylesheet ] = jk_normalize_path( realpath( get_stylesheet_directory() ) );
		}
...
function register_block_script_module_id( $metadata, $field_name, $index = 0 ) {
	...
	$module_asset_path     = jk_normalize_path(
		realpath( $module_asset_raw_path )
	);

	$module_path_norm = jk_normalize_path( realpath( $path . '/' . $module_path ) );
...
function register_block_script_handle( $metadata, $field_name, $index = 0 ) {
	...
	$script_asset_path     = jk_normalize_path(
	...
	if ( jk_script_is( $script_handle, 'registered' ) ) {
		return $script_handle;
	}

	$script_path_norm    = jk_normalize_path( realpath( $path . '/' . $script_path ) );
	...
	if ( ! empty( $metadata['textdomain'] ) && in_array( 'jk-i18n', $script_dependencies, true ) ) {
		jk_set_script_translations( $script_handle, $metadata['textdomain'] );
	}
...
function register_block_style_handle( $metadata, $field_name, $index = 0 ) {
	...
	if ( jk_style_is( $style_handle_name, 'registered' ) ) {
	...
	if ( $is_core_block && ! jk_should_load_separate_core_block_assets() ) {
		return false;
	}
	...
	$style_path_norm = jk_normalize_path( realpath( dirname( $metadata['file'] ) . '/' . $style_path ) );
	$style_uri       = get_block_asset_url( $style_path_norm );

	$version = ! $is_core_block && isset( $metadata['version'] ) ? $metadata['version'] : false;
	$result  = jk_register_style(
		$style_handle_name,
		$style_uri,
		array(),
		$version
	...
	if ( $style_uri ) {
		jk_style_add_data( $style_handle_name, 'path', $style_path_norm );
		...
		if ( is_rtl() && file_exists( $rtl_file ) ) {
			jk_style_add_data( $style_handle_name, 'rtl', 'replace' );
			jk_style_add_data( $style_handle_name, 'suffix', $suffix );
			jk_style_add_data( $style_handle_name, 'path', $rtl_file );
		}
	}
...
function get_block_metadata_i18n_schema() {
	...
	if ( ! isset( $i18n_block_schema ) ) {
		$i18n_block_schema = jk_json_file_decode( __DIR__ . '/block-i18n.json' );
	}
 ...
 */
function jk_register_block_metadata_collection( $path, $manifest ) {
	JK_Block_Metadata_Registry::register_collection( $path, $manifest );
}
 ...
 *                               of `JK_Block_Type`. See JK_Block_Type::__construct() for information
 *                               on accepted arguments. Default empty array.
 * @return JK_Block_Type|false The registered block type on success, or false on failure.
 */
function register_block_type_from_metadata( $file_or_folder, $args = array() ) {
	...
	$is_core_block        = str_starts_with( $file_or_folder, ABSPATH . JKINC );
	$metadata_file_exists = $is_core_block || file_exists( $metadata_file );
	$registry_metadata    = JK_Block_Metadata_Registry::get_metadata( $file_or_folder );

	if ( $registry_metadata ) {
		$metadata = $registry_metadata;
	} elseif ( $metadata_file_exists ) {
		$metadata = jk_json_file_decode( $metadata_file, array( 'associative' => true ) );
	} else {
		$metadata = array();
	}
	...
	$metadata['file'] = $metadata_file_exists ? jk_normalize_path( realpath( $metadata_file ) ) : null;
	...
	if ( ! empty( $metadata['render'] ) ) {
		$template_path = jk_normalize_path(
			realpath(
	...
	if ( ! empty( $metadata['variations'] ) && is_string( $metadata['variations'] ) ) {
		$variations_path = jk_normalize_path(
			realpath(
				dirname( $metadata['file'] ) . '/' .
	...
	return JK_Block_Type_Registry::get_instance()->register(
		$metadata['name'],
		$settings
	);
}
 ...
 *
 * @param string|JK_Block_Type $block_type Block type name including namespace, or alternatively
 *                                         a path to the JSON file with metadata definition for the block,
 *                                         or a path to the folder where the `block.json` file is located,
 *                                         or a complete JK_Block_Type instance.
 *                                         In case a JK_Block_Type is provided, the $args parameter will be ignored.
 * @param array                $args       Optional. Array of block type arguments. Accepts any public property
 *                                         of `JK_Block_Type`. See JK_Block_Type::__construct() for information
 *                                         on accepted arguments. Default empty array.
 *
...
function register_block_type( $block_type, $args = array() ) {
	...
	return JK_Block_Type_Registry::get_instance()->register( $block_type, $args );
}
 ...
 * @since 5.0.0
 *
 * @param string|JK_Block_Type $name Block type name including namespace, or alternatively
 *                                   a complete JK_Block_Type instance.
 * @return JK_Block_Type|false The unregistered block type on success, or false on failure.
 */
function unregister_block_type( $name ) {
	return JK_Block_Type_Registry::get_instance()->unregister( $name );
}
 ...
 * @see parse_blocks()
 *
 * @param int|string|JK_Post|null $post Optional. Post content, post ID, or post object.
 *                                      Defaults to global $post.
 * @return bool Whether the post has blocks.
 */
function has_blocks( $post = null ) {
	if ( ! is_string( $post ) ) {
		$jk_post = get_post( $post );

		if ( ! $jk_post instanceof JK_Post ) {
			return false;
		}

		$post = $jk_post->post_content;
	}

	return str_contains( (string) $post, '<!-- jk:' );
}
...
function has_block( $block_name, $post = null ) {
	 ...
	 * This matches behavior for JKPress 5.0.0 - 5.3.0 in matching blocks by
	...
	$has_block = str_contains( $post, '<!-- jk:' . $block_name . ' ' );
	...
	if ( ! $has_block ) {
		...
		if ( $serialized_block_name !== $block_name ) {
			$has_block = str_contains( $post, '<!-- jk:' . $serialized_block_name . ' ' );
		}
	}
...
function get_dynamic_block_names() {
	...
	$block_types = JK_Block_Type_Registry::get_instance()->get_all_registered();
...
function get_hooked_blocks() {
	...
	$hooked_blocks = array();
	foreach ( $block_types as $block_type ) {
		if ( ! ( $block_type instanceof JK_Block_Type ) || ! is_array( $block_type->block_hooks ) ) {
			continue;
		}
 ...
 * @param JK_Block_Template|JK_Post|array $context             The block template, template part, or pattern that the anchor block belongs to.
 * @return string
 */
function insert_hooked_blocks( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	 ...
	 * @param JK_Block_Template|JK_Post|array $context            The block template, template part, post object,
 ...
 * @param JK_Block_Template|JK_Post|array|null $context  A block template, template part, post object, or pattern
		...
		foreach ( $relative_positions as $relative_position => $hooked_block_types ) {
			foreach ( $hooked_block_types as $index => $hooked_block_type ) {
				$hooked_block_type_definition =
					JK_Block_Type_Registry::get_instance()->get_registered( $hooked_block_type );
...
/**
 * Updates the jk_postmeta with the list of ignored hooked blocks
 * where the inner blocks are stored as post content.
 *
 * @since 6.6.0
 * @since 6.8.0 Support non-`jk_navigation` post types.
 * @access private
 *
...
function update_ignored_hooked_blocks_postmeta( $post ) {
	...
	if ( 'jk_navigation' === $post->post_type ) {
		$wrapper_block_type = 'core/navigation';
	} elseif ( 'jk_block' === $post->post_type ) {
	...
	$context          = new JK_Post( $context ); // Convert to JK_Post object.
	...
	if ( ! empty( $ignored_hooked_blocks ) ) {
		...
		$post->meta_input['_jk_ignored_hooked_blocks'] = json_encode( $ignored_hooked_blocks );
	}
 ...
 * @param JK_Block_Template|JK_Post|array $context       A block template, template part, post object,
...
function serialize_block_attributes( $block_attributes ) {
	$encoded_attributes = jk_json_encode( $block_attributes, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function get_comment_delimited_block_content( $block_name, $block_attributes, $block_content ) {
	...
	if ( empty( $block_content ) ) {
		return sprintf( '<!-- jk:%s %s/-->', $serialized_block_name, $serialized_attributes );
	}
 ...
 *
 * @param array $block {
 *     An associative array of a single parsed block object. See JK_Block_Parser_Block.
 *
 ...
 * @see serialize_block()
 *
 * @param array    $block         An associative array of a single parsed block object. See JK_Block_Parser_Block.
...
function resolve_pattern_blocks( $blocks ) {
	...
	while ( $i < count( $blocks ) ) {
		if ( 'core/pattern' === $blocks[ $i ]['blockName'] ) {
			...
			$registry = JK_Block_Patterns_Registry::get_instance();
 ...
 *
 * @param array[]  $blocks        An array of parsed blocks. See JK_Block_Parser_Block.
 * @param callable $pre_callback  Callback to run on each block in the tree before it is traversed and serialized.
 *                                It is called with the following arguments: &$block, $parent_block, $previous_block.
 *                                Its string return value will be prepended to the serialized block markup.
 * @param callable $post_callback Callback to run on each block in the tree after it is traversed and serialized.
 ...
 * @since 5.3.1
 *
 * @param JK_Block_Parser_Block $block             The parsed block object.
...
function filter_block_kses_value( $value, $allowed_html, $allowed_protocols = array(), $block_context = null ) {
	if ( is_array( $value ) ) {
		...
		return jk_kses( $value, $allowed_html, $allowed_protocols );
	}
 ...
 * @since 5.0.0
 *
 * @global JK_Post $post The post to edit.
 *
 * @param array $parsed_block {
 *     An associative array of the block being rendered. See JK_Block_Parser_Block.
 *
 *     @type string   $blockName    Name of block.
 *     @type array    $attrs        Attributes from block comment delimiters.
 *     @type array[]  $innerBlocks  List of inner blocks. An array of arrays that
...
function render_block( $parsed_block ) {
	 ...
	 *                                  inner blocks were found.
	 * }
	 * @param JK_Block|null $parent_block If this is a nested block, a reference to the parent block.
	 */
	...
	$block = new JK_Block( $parsed_block, $context );
...
function parse_blocks( $content ) {
	 ...
	 */
	$parser_class = apply_filters( 'block_parser_class', 'JK_Block_Parser' );
 ...
 *                                          See JK_Block_Styles_Registry::register().
 * @return bool True if the block style was registered with success and false otherwise.
 */
function register_block_style( $block_name, $style_properties ) {
	return JK_Block_Styles_Registry::get_instance()->register( $block_name, $style_properties );
}
...
function unregister_block_style( $block_name, $block_style_name ) {
	return JK_Block_Styles_Registry::get_instance()->unregister( $block_name, $block_style_name );
}
 ...
 *
 * @param JK_Block_Type $block_type    Block type to check for support.
...
function block_has_support( $block_type, $feature, $default_value = false ) {
	...
	if ( $block_type instanceof JK_Block_Type ) {
		if ( is_array( $feature ) && count( $feature ) === 1 ) {
			$feature = $feature[0];
		}

		if ( is_array( $feature ) ) {
			$block_support = _jk_array_get( $block_type->supports, $feature, $default_value );
 ...
 */
function jk_migrate_old_typography_shape( $metadata ) {
	...
	foreach ( $typography_keys as $typography_key ) {
		...
		if ( null !== $support_for_key ) {
			...
			_jk_array_set( $metadata['supports'], array( 'typography', $typography_key ), $support_for_key );
...
/**
 * Helper function that constructs a JK_Query args array from
 ...
 *
 * @return array Returns the constructed JK_Query arguments.
 */
function build_query_vars_from_query_block( $block, $page ) {
	...
	if ( isset( $block->context['query'] ) ) {
		...
		if ( isset( $block->context['query']['sticky'] ) && ! empty( $block->context['query']['sticky'] ) ) {
			...
			if ( 'only' === $block->context['query']['sticky'] ) {
				 ...
				 * Logic should be used before hand to determine if JK_Query should be used in the event that the array
	...
	/**
	 * Filters the arguments which will be passed to `JK_Query` for the Query Loop Block.
	 *
	 * Anything to this filter should be compatible with the `JK_Query` API to form
	 ...
	 * @since 6.1.0
	 *
	 * @param array    $query Array containing parameters for `JK_Query` as parsed by the block context.
	 * @param JK_Block $block Block instance.
	 * @param int      $page  Current query's page.
	 */
...
function get_query_pagination_arrow( $block, $is_next ) {
	...
	if ( ! empty( $block->context['paginationArrow'] ) && array_key_exists( $block->context['paginationArrow'], $arrow_map ) && ! empty( $arrow_map[ $block->context['paginationArrow'] ] ) ) {
		...
		$arrow_classes   = "jk-block-query-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
	...
	if ( get_option( 'page_comments' ) === '1' || get_option( 'page_comments' ) === true ) {
		...
		if ( $per_page > 0 ) {
			...
			if ( $page ) {
				...
				$max_num_pages = (int) ( new JK_Comment_Query( $comment_args ) )->max_num_pages;
				if ( 0 !== $max_num_pages ) {
...
function get_comments_pagination_arrow( $block, $pagination_type = 'next' ) {
	...
	if ( ! empty( $block->context['comments/paginationArrow'] ) && ! empty( $arrow_map[ $block->context['comments/paginationArrow'] ][ $pagination_type ] ) ) {
		...
		$arrow_classes   = "jk-block-comments-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
		return "<span class='$arrow_classes' aria-hidden='true'>$arrow</span>";
	}
	return null;
}
 ...
 */
function _jk_filter_post_meta_footnotes( $footnotes ) {
	...
	foreach ( $footnotes_decoded as $footnote ) {
		if ( ! empty( $footnote['content'] ) && ! empty( $footnote['id'] ) ) {
			$footnotes_sanitized[] = array(
				...
				'content' => jk_unslash( jk_filter_post_kses( jk_slash( $footnote['content'] ) ) ),
			);
		}
	}
	return jk_json_encode( $footnotes_sanitized );
}
 ...
 * @since 6.3.2
 */
function _jk_footnotes_kses_init_filters() {
	add_filter( 'sanitize_post_meta_footnotes', '_jk_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _jk_footnotes_remove_filters() {
	remove_filter( 'sanitize_post_meta_footnotes', '_jk_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _jk_footnotes_kses_init() {
	_jk_footnotes_remove_filters();
	if ( ! current_user_can( 'unfiltered_html' ) ) {
		_jk_footnotes_kses_init_filters();
	}
}
 ...
 */
function _jk_footnotes_force_filtered_html_on_import_filter( $arg ) {
