 ...
 * @since 3.9.0
 */
function jk_underscore_audio_template() {
	$audio_types = jk_get_audio_extensions();
	?>
...
>
	...
	<source src="{{ data.model.src }}" type="{{ jk.media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>
	...
	<source src="{{ data.model.<?php echo $type; ?> }}" type="{{ jk.media.view.settings.embedMimes[ '<?php echo $type; ?>' ] }}" />
	<# } #>
		<?php
	endforeach;
	?>
 ...
 * @since 3.9.0
 */
function jk_underscore_video_template() {
	$video_types = jk_get_video_extensions();
	?>
<#  var w_rule = '', classes = [],
		w, h, settings = jk.media.view.settings,
...
#>
<div style="{{ w_rule }}" class="jk-video">
<video controls
	class="jk-video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
	$props = array(
 ...
 * @since 3.5.0
 */
function jk_print_media_templates() {
	$class = 'media-modal jk-core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<?php elseif ( is_multisite() && ! is_upload_space_available() ) : ?>
			<div class="upload-ui">
				...
				<?php
				/** This action is documented in jk-admin/includes/media.php */
	...
	<script type="text/html" id="tmpl-attachment-details-two-column">
		<div class="attachment-media-view {{ data.orientation }}">
			...
			if ( isset( $_GET['error'] ) && 'deprecated' === $_GET['error'] ) {
				jk_admin_notice(
					__( 'The Edit Media screen is deprecated as of JKPress 6.3. Please use the Media Library instead.' ),
					array(
						'id'                 => 'message',
			...
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<# if ( 'audio' === data.type ) { #>
				<div class="jk-media-wrapper jk-audio">
					<audio style="visibility: hidden" controls class="jk-audio-shortcode" width="100%" preload="none">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( jk.media.view.settings.contentWidth ) {
						w_rule = 'width: ' + jk.media.view.settings.contentWidth + 'px;';
					}
				#>
				<div style="{{ w_rule }}" class="jk-media-wrapper jk-video">
					<video controls="controls" class="jk-video-shortcode" preload="metadata"
		...
		<div class="attachment-info">
			...
			<div class="actions">
				<# if ( data.link ) { #>
					<?php
					$view_media_text = ( '1' === get_option( 'jk_attachment_pages_enabled' ) ) ? __( 'View attachment page' ) : __( 'View media file' );
					?>
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<span class="setting">
			...
			<select id="gallery-settings-link-to" class="link-to"
				...
				<option value="post" <# if ( ! jk.media.galleryDefaults.link || 'post' === jk.media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_html_e( 'Attachment Page' ); ?>
				</option>
				<option value="file" <# if ( 'file' === jk.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_html_e( 'Media File' ); ?>
				</option>
				<option value="none" <# if ( 'none' === jk.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php echo esc_html_x( 'None', 'Media item link option' ); ?>
				</option>
			</select>
		</span>
			...
			<select id="gallery-settings-columns" class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i; ?> == jk.media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
			...
			<div class="embed-media-settings">
				<div class="column-settings">
					...
					<# if ( data.attachment ) { #>
						...
						<# } #>
							<div class="custom-size jk-clearfix<# if ( data.model.size !== 'custom' ) { #> hidden<# } #>">
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: jk.media.view.settings.embedMimes.mp3,
			ogg: jk.media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = jk_get_audio_extensions(); ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: jk.media.view.settings.embedMimes.mp4,
			ogv: jk.media.view.settings.embedMimes.ogv,
			webm: jk.media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = jk_get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-video-settings">
				<div class="jk-video-holder">
				<#
				...
				<?php jk_underscore_video_template(); ?>
				...
				<# _.each( data.attachments, function( attachment, index ) { #>
					<dl class="gallery-item">
						...
						<# if ( attachment.caption ) { #>
							<dd class="jk-caption-text gallery-caption">
