...
final class JK_Comment {
	...
	/**
	 * Retrieves a JK_Comment instance.
	 *
	 * @since 4.4.0
	 *
	 ...
	 * @return JK_Comment|false Comment object, otherwise false.
	 */
	public static function get_instance( $id ) {
		...
		$_comment = jk_cache_get( $comment_id, 'comment' );

		if ( ! $_comment ) {
			$_comment = $jkdb->get_row( $jkdb->prepare( "SELECT * FROM $jkdb->comments WHERE comment_ID = %d LIMIT 1", $comment_id ) );

			if ( ! $_comment ) {
				return false;
			}

			jk_cache_add( $_comment->comment_ID, $_comment, 'comment' );
		}

		return new JK_Comment( $_comment );
	}
	 ...
	 * }
	 * @return JK_Comment[] Array of `JK_Comment` objects.
	 */
	public function get_children( $args = array() ) {
		...
		$_args           = jk_parse_args( $args, $defaults );
	 ...
	 *
	 * Used by `JK_Comment_Query` when bulk-filling descendants.
	 *
	 * @since 4.4.0
	 *
	 * @param JK_Comment $child Child comment.
	 */
	public function add_child( JK_Comment $child ) {
	 ...
	 * @return JK_Comment|false Returns the comment object if found, otherwise false.
	 */
