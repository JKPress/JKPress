/**
 * @output jk-includes/js/mce-view.js
 */
...
( function( window, jk, shortcode, $ ) {
	...
	jk.mce.views = {
		 ...
		 * @param {Object} extend An object to extend jk.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = jk.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		 ...
		 *
		 * @return {jk.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {jk.mce.View} The view instance or undefined.
		 */
	 ...
	 * @param {Object} options Options.
	 */
	jk.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	jk.mce.View.extend = Backbone.View.extend;

	_.extend( jk.mce.View.prototype, /** @lends jk.mce.View.prototype */{
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/wordpress/jk-content.css' ) === -1 ) {
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					jk.mce.views.createInstance( type, text, match.options, force ).render();
...
} )( window, window.jk, window.jk.shortcode, window.jQuery );

/*
 * The JKPress core TinyMCE views.
...
} )( window, window.jk.mce.views, window.jk.media, window.jQuery );
