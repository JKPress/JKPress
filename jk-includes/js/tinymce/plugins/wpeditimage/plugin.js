...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
	...
	editor.addButton( 'jk_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'jk_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'jkAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.jk-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:jk_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:jk_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			return '<div class="mceTemp"><dl id="' + id + '" class="jk-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="jk-caption-dt">'+ img +'</dt><dd class="jk-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+jk-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /jk-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^jk-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'jk-image-', '' ), 10 );
		...
		// Extract caption.
		captionBlock = dom.getParents( imageNode, '.jk-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.jk-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( $imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'jk-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.jk-caption', captionNode );
				...
				dd = dom.select( '.jk-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="jk-caption-dt"></dt><dd class="jk-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.jk-caption-dt' ).append( node );
		...
		if ( jk.media.events ) {
			jk.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		jk.media.events.trigger( 'editor:frame-create', { frame: frame } );
		...
		frame.on( 'close', function() {
			 ...
			 */
			imageNode = editor.$( 'img[data-jk-editing]' )
			imageNode.removeAttr( 'data-jk-editing' );
		});
	...
	editor.on( 'init', function() {
		...
		// Prevent IE11 from making dl.jk-caption resizable.
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+.
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.jk-caption' ) ) {
					// Hide the thick border with resize handles around dl.jk-caption.
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'jk-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.jk-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.jk-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'jkAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.jk-caption' );
	...
	editor.jkSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.jkGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__jk-temp-img-id"]' ).removeAttr( 'id' );
		}
	});

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.jkSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.jkGetImgCaption( event.content );
		}
	});
	...
	// Add to editor.jk.
	editor.jk = editor.jk || {};
	editor.jk.isPlaceholder = isPlaceholder;
