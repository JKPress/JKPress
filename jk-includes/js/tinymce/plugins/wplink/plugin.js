( function( tinymce ) {
	tinymce.ui.Factory.add( 'JKLinkPreview', tinymce.ui.Control.extend( {
		url: '#',
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="jk-link-preview">' +
	...
	tinymce.ui.Factory.add( 'JKLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="jk-link-input">' +
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		var speak = ( typeof window.jk !== 'undefined' && window.jk.a11y && window.jk.a11y.speak ) ? window.jk.a11y.speak : function() {};
		var hasLinkError = false;
		var __ = window.jk.i18n.__;
		var _n = window.jk.i18n._n;
		var sprintf = window.jk.i18n.sprintf;
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '_jk_link_placeholder' ) {
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_jk_link_placeholder"' ) > -1 ) {
					return text;
				}
		...
		editor.on( 'preinit', function() {
			if ( editor.jk && editor.jk._createToolbar ) {
				toolbar = editor.jk._createToolbar( [
					'jk_link_preview',
					'jk_link_edit',
					'jk_link_remove'
				], true );
				...
				if ( typeof window.jkLink !== 'undefined' ) {
					editButtons.push( 'jk_link_advanced' );
				}

				editToolbar = editor.jk._createToolbar( editButtons, true );

				editToolbar.on( 'show', function() {
					if ( typeof window.jkLink === 'undefined' || ! window.jkLink.modalOpen ) {
						window.setTimeout( function() {
							...
							if ( element ) {
								if ( ! element.value && selection && typeof window.jkLink !== 'undefined' ) {
									element.value = window.jkLink.getUrlFromSelection( selection );
								}
				...
				editToolbar.on( 'hide', function() {
					if ( ! editToolbar.scrolling ) {
						editor.execCommand( 'jk_link_cancel' );
					}
				} );
			}
		} );

		editor.addCommand( 'JK_Link', function() {
			if ( tinymce.Env.ie && tinymce.Env.ie < 10 && typeof window.jkLink !== 'undefined' ) {
				window.jkLink.open( editor.id );
				return;
			}
			...
			if ( ! linkNode ) {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_jk_link_placeholder' } );

				linkNode = editor.$( 'a[href="_jk_link_placeholder"]' )[0];
				editor.nodeChanged();
			}
		...
		editor.addCommand( 'jk_link_apply', function() {
			if ( editToolbar.scrolling ) {
				return;
			}
			...
			if ( typeof window.jkLinkL10n !== 'undefined' && ! hasLinkError ) {
				speak( window.jkLinkL10n.linkInserted );
			}
		} );

		editor.addCommand( 'jk_link_cancel', function() {
			inputInstance.reset();
		...
		editor.addCommand( 'jk_unlink', function() {
			editor.execCommand( 'unlink' );
			editToolbar.tempHide = false;
			editor.execCommand( 'jk_link_cancel' );
		} );

		// JK default shortcuts.
		editor.addShortcut( 'access+a', '', 'JK_Link' );
		editor.addShortcut( 'access+s', '', 'jk_unlink' );
		// The "de-facto standard" shortcut, see #27305.
		editor.addShortcut( 'meta+k', '', 'JK_Link' );
		...
		editor.addButton( 'jk_link_preview', {
			type: 'JKLinkPreview',
		...
		editor.addButton( 'jk_link_input', {
			type: 'JKLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-jk-autocomplete-' + ui.item.ID );
							/*
						...
						select: function( event, ui ) {
							...
							if ( 9 === event.keyCode && typeof window.jkLinkL10n !== 'undefined' ) {
								// Audible confirmation message when a link has been selected.
								speak( window.jkLinkL10n.linkSelected );
							}
						...
						var fallbackTitle = ( typeof window.jkLinkL10n !== 'undefined' ) ? window.jkLinkL10n.noTitle : '',
							title = item.title ? item.title : fallbackTitle;

						return $( '<li role="option" id="mce-jk-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="jk-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'jk_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.on( 'wptoolbar', function( event ) {
			...
			if ( linkNode ) {
				...
				if ( href === '_jk_link_placeholder' || edit ) {
					if ( href !== '_jk_link_placeholder' && ! inputInstance.getURL() ) {
					...
					if ( $linkNode.attr( 'data-wplink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.jk-link-preview a' ).addClass( 'wplink-url-error' );
					} else {
						toolbar.$el.find( '.jk-link-preview a' ).removeClass( 'wplink-url-error' );
						hasLinkError = false;
					}
				}
		...
		editor.addButton( 'jk_link_edit', {
			tooltip: 'Edit|button', // '|button' is not displayed, only used for context.
			icon: 'dashicon dashicons-edit',
			cmd: 'JK_Link'
		} );

		editor.addButton( 'jk_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'jk_unlink'
		} );

		editor.addButton( 'jk_link_advanced', {
			...
			onclick: function() {
				if ( typeof window.jkLink !== 'undefined' ) {
					...
					window.jkLink.open( editor.id, url, text );
		...
		editor.addButton( 'jk_link_apply', {
			tooltip: 'Apply',
