( function( tinymce ) {
	tinymce.PluginManager.add( 'wpemoji', function( editor ) {
		...
		if ( ! jk || ! jk.emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-jk-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			jk.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after jk.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-jk-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-jk-emoji' ) ) {
