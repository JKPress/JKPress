...
( function( tinymce ) {
	tinymce.PluginManager.add( 'wpview', function( editor ) {
		...
		// Set this here as jk-tinymce.js may be loaded too early.
		var jk = window.jk;

		if ( ! jk || ! jk.mce || ! jk.mce.views ) {
			return {
				getView: noop
			};
		}
		...
		editor.on( 'init', function() {
			...
			if ( MutationObserver ) {
				new MutationObserver( function() {
					editor.fire( 'jk-body-class-change' );
				} )
			...
			// Pass on body class name changes from the editor to the jkView iframes.
			editor.on( 'jk-body-class-change', function() {
		...
		editor.on( 'beforesetcontent', function( event ) {
			...
			event.content = jk.mce.views.setMarkers( event.content, editor );
		} );
		...
		editor.addButton( 'jk_view_edit', {
			...
			onclick: function() {
				...
				if ( isView( node ) ) {
					jk.mce.views.edit( editor, node );
				}
			}
		} );

		editor.addButton( 'jk_view_remove', {
			tooltip: 'Remove',
		...
		editor.jk = editor.jk || {};
		editor.jk.getView = noop;
		editor.jk.setViewCursor = noop;
