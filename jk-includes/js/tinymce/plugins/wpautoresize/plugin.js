...
// Forked for JKPress so it can be turned on/off after loading.
...
tinymce.PluginManager.add( 'wpautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'jk-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'jk-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'jk-autoresize' );
	...
	function off() {
		...
		if ( ! settings.jk_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'jk-autoresize' );
	...
	editor.addCommand( 'jkAutoResize', resize );

	// On/off.
	editor.addCommand( 'jkAutoResizeOn', on );
	editor.addCommand( 'jkAutoResizeOff', off );
});
