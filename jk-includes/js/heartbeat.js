 ...
 * @see jk_ajax_nopriv_heartbeat(), jk_ajax_heartbeat()
 *
 ...
 *
 * @since 3.6.0
 * @output jk-includes/js/heartbeat.js
 */
...
( function( $, window, undefined ) {
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.jk-heartbeat', function() {
			...
			$(window).on( 'pagehide.jk-heartbeat', function() {
		...
		function setErrorState( error, status ) {
			...
			if ( error ) {
				...
				if ( trigger && ! hasConnectionError() ) {
					...
					jk.hooks.doAction( 'heartbeat.connection-lost', error, status );
				}
			}
		}
		...
		function clearErrorState() {
			...
			if ( hasConnectionError() ) {
				...
				jk.hooks.doAction( 'heartbeat.connection-restored' );
			}
		}
		...
		function connect() {
			...
			jk.hooks.doAction( 'heartbeat.send', heartbeatData );
			...
			settings.xhr = $.ajax({
				...
				if ( response.nonces_expired ) {
					...
					jk.hooks.doAction( 'heartbeat.nonces-expired' );
				}
				...
				// Update the Rest API nonce if set and jk-api loaded.
				if ( response.rest_nonce && typeof window.jkApiSettings === 'object' ) {
					window.jkApiSettings.nonce = response.rest_nonce;
					// This nonce is required for api-fetch through heartbeat.tick.
					// delete response.rest_nonce;
				}

				$document.trigger( 'heartbeat-tick', [response, textStatus, jqXHR] );
				jk.hooks.doAction( 'heartbeat.tick', response, textStatus, jqXHR );
				...
				jk.hooks.doAction( 'heartbeat.error', jqXHR, textStatus, error );
			});
		}
		...
		function userIsActive() {
			...
			$document.off( '.jk-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.jk-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.jk-heartbeat-active keyup.jk-heartbeat-active touchend.jk-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.jk-heartbeat-active keyup.jk-heartbeat-active touchend.jk-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 *
		 * @since 3.6.0
		 * @memberOf jk.heartbeat.prototype
		 *
		 ...
		 * the third argument is 'true'. Use `jk.heartbeat.isQueued('handle')` to see if
	...
	/**
	 * Ensure the global `jk` object exists.
	 *
	 * @namespace jk
	 */
	window.jk = window.jk || {};
	 ...
	 *
	 * @namespace jk.heartbeat
	 * @type {Heartbeat}
	 */
	window.jk.heartbeat = new Heartbeat();
