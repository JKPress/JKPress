 ...
 *
 * @output jk-includes/js/customize-preview.js
 */
(function( exports, $ ){
	 ...
	 */
	api.Preview = api.Messenger.extend(/** @lends jk.customize.Preview.prototype */{
		/**
		...
		handleLinkClick: function( event ) {
			...
			if ( ! api.isLinkPreviewable( link[0] ) ) {
				jk.a11y.speak( api.settings.l10n.linkUnpreviewable );
		...
		handleFormSubmit: function( event ) {
			...
			if ( 'GET' !== form.prop( 'method' ).toUpperCase() || ! api.isLinkPreviewable( urlParser ) ) {
				jk.a11y.speak( api.settings.l10n.formUnpreviewable );
				event.preventDefault();
				return;
			}
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		// Skip jk login and signup pages.
		if ( /\/jk-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/jk-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/jk-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		// Ignore links with class jk-playlist-caption.
		if ( $element.hasClass( 'jk-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'jk-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /jk-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'jk-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'jk-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the jk-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
