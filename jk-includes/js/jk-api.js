/**
 * @output jk-includes/js/jk-api.js
 */
...
(function( window, undefined ) {
	...
	/**
	 * Initialize the JK_API.
	 */
	function JK_API() {
		/** @namespace jk.api.models */
		this.models = {};
		/** @namespace jk.api.collections */
		this.collections = {};
		/** @namespace jk.api.views */
		this.views = {};
	}
	...
	window.jk            = window.jk || {};
	/** @namespace jk.api */
	jk.api               = jk.api || new JK_API();
	jk.api.versionString = jk.api.versionString || 'jk/v2/';
...
(function( window, undefined ) {
	...
	/** @namespace jk.api */
	jk.api = jk.api || {};
	/** @namespace jk.api.utils */
	jk.api.utils = jk.api.utils || {};
	 ...
	 */
	jk.api.getModelByRoute = function( route ) {
		return _.find( jk.api.models, function( model ) {
	 ...
	 */
	jk.api.getCollectionByRoute = function( route ) {
		return _.find( jk.api.collections, function( collection ) {
	 ...
	 * @param {Date} date.
	 */
	jk.api.utils.parseISO8601 = function( date ) {
	 ...
	 */
	jk.api.utils.getRootUrl = function() {
	 ...
	 */
	jk.api.utils.capitalize = function( str ) {
	 ...
	 */
	jk.api.utils.capitalizeAndCamelCaseDashes = function( str ) {
		if ( _.isUndefined( str ) ) {
			return str;
		}
		str = jk.api.utils.capitalize( str );

		return jk.api.utils.camelCaseDashes( str );
	};

	/**
	 * Helper function to camel case the letter after dashes, removing the dashes.
	 */
	jk.api.utils.camelCaseDashes = function( str ) {
	 ...
	 * @param {string}  [versionString] Version string, defaults to `jk.api.versionString`.
	 * @param {boolean} [reverse]       Whether to reverse the order when extracting the route part. Optional, default false.
	 */
	jk.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		var routeParts;

		part = part || 1;
		versionString = versionString || jk.api.versionString;
	 ...
	 */
	jk.api.utils.extractParentName = function( route ) {
		var name,
	 ...
	 *                                to add the args to.
	 */
	jk.api.utils.decorateFromRoute = function( routeEndpoints, modelInstance ) {
	 ...
	 */
	jk.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. JKPress expects dates in UTC.
				 ...
				 *
				 * JKPress returns 'date' and 'date_modified' in the timezone of the server as well as
				...
				getDate: function( field ) {
					...
					return new Date( jk.api.utils.parseISO8601( theISODate ) );
				}
			},
			...
			buildModelGetter = function( parentModel, modelId, modelName, embedSourcePoint, embedCheckField ) {
				...
				getModel = new jk.api.models[ modelName ]( attributes );
			...
			buildCollectionGetter = function( parentModel, collectionName, embedSourcePoint, embedIndex ) {
				 ...
				 *
				 * @return {Deferred.promise} promise Resolves to a jk.api.collections[ collectionName ]
				 * collection.
				 */
				...
				getObjects = new jk.api.collections[ collectionName ]( properties, classProperties );
			...
			TagsMixin = {
				...
				getTags: function() {
					var tagIds = this.get( 'tags' ),
						tags  = new jk.api.collections.Tags();
				...
				setTags: function( tags ) {
					...
					if ( _.isArray( tags ) ) {
						...
						// Get all the tags.
						allTags = new jk.api.collections.Tags();
						allTags.fetch( {
							data:    { per_page: 100 },
							success: function( alltags ) {
								...
								_.each( tags, function( tag ) {
									newTag = new jk.api.models.Tag( alltags.findWhere( { slug: tag } ) );
								...
								tags = new jk.api.collections.Tags( newTags );
			...
			CategoriesMixin = {
				...
				getCategories: function() {
					var categoryIds = this.get( 'categories' ),
						categories  = new jk.api.collections.Categories();
				...
				setCategories: function( categories ) {
					...
					if ( _.isArray( categories ) ) {
						...
						allCategories = new jk.api.collections.Categories();
						allCategories.fetch( {
							data:    { per_page: 100 },
							success: function( allcats ) {
								...
								_.each( categories, function( category ) {
									newCategory = new jk.api.models.Category( allcats.findWhere( { slug: category } ) );
								...
								categories = new jk.api.collections.Categories( newCategories );
			...
			FeaturedMediaMixin = {
				getFeaturedMedia: function() {
					return buildModelGetter( this, this.get( 'featured_media' ), 'Media', 'jk:featuredmedia', 'source_url' );
				}
			};
...
(function() {
	 ...
	 */
	jk.api.JKApiBaseModel = Backbone.Model.extend(
		/** @lends JKApiBaseModel.prototype  */
		{
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-JK-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-JK-Nonce' );
	 ...
	 */
	jk.api.models.Schema = jk.api.JKApiBaseModel.extend(
		/** @lends Schema.prototype  */
		{
			...
			initialize: function( attributes, options ) {
				...
				jk.api.JKApiBaseModel.prototype.initialize.call( model, attributes, options );

				model.apiRoot = options.apiRoot || jkApiSettings.root;
				model.versionString = options.versionString || jkApiSettings.versionString;
			},
...
( function() {
	 ...
	 */
	jk.api.JKApiBaseCollection = Backbone.Collection.extend(
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-jk-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-jk-total' ), 10 );
						}
...
( function() {
	...
	var Endpoint, initializedDeferreds = {},
		jkApiSettings = window.jkApiSettings || {};
	...
	// If jkApiSettings is unavailable, try the default.
	if ( _.isEmpty( jkApiSettings ) ) {
		jkApiSettings.root = window.location.origin + '/jk-json/';
	}

	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		defaults: {
			apiRoot: jkApiSettings.root,
			versionString: jk.api.versionString,
			nonce: null,
			schema: null,
			models: {},
			collections: {}
		...
		initialize: function() {
			...
			model.schemaModel = new jk.api.models.Schema( null, {
			...
			if ( model.get( 'schema' ) ) {
				...
				( _.isUndefined( jkApiSettings.cacheSchema ) || jkApiSettings.cacheSchema ) &&
				sessionStorage.getItem( 'jk-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'jk-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( jkApiSettings.cacheSchema ) || jkApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'jk-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
		...
		constructFromSchema: function() {
			 ...
			 *
			 * Localizing a variable jkApiSettings.mapping will over-ride the default mapping options.
			 *
			 */
			mapping = jkApiSettings.mapping || {
				models: {
					'Categories':      'Category',
					'Comments':        'Comment',
					'Pages':           'Page',
			...
			modelRoutes      = [];
			collectionRoutes = [];
			schemaRoot       = routeModel.get( 'apiRoot' ).replace( jk.api.utils.getRootUrl(), '' );
			loadingObjects   = {};
			...
			_.each( modelRoutes, function( modelRoute ) {
				...
				var modelClassName,
					routeName  = jk.api.utils.extractRoutePart( modelRoute.index, 2, routeModel.get( 'versionString' ), true ),
					parentName = jk.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), false ),
					routeEnd   = jk.api.utils.extractRoutePart( modelRoute.index, 1, routeModel.get( 'versionString' ), true );
				...
				if ( '' !== parentName && parentName !== routeName ) {
					modelClassName = jk.api.utils.capitalizeAndCamelCaseDashes( parentName ) + jk.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = jk.api.JKApiBaseModel.extend( {
					...
					modelClassName = jk.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName = mapping.models[ modelClassName ] || modelClassName;
					loadingObjects.models[ modelClassName ] = jk.api.JKApiBaseModel.extend( {
				...
				jk.api.utils.decorateFromRoute(
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						...
						parentName = jk.api.utils.extractRoutePart( collectionRoute.index, 1, routeModel.get( 'versionString' ), false );

				// If the collection has a parent in its route, add that to its class name.
				if ( '' !== parentName && parentName !== routeName && routeModel.get( 'versionString' ) !== parentName ) {

					collectionClassName = jk.api.utils.capitalizeAndCamelCaseDashes( parentName ) + jk.api.utils.capitalizeAndCamelCaseDashes( routeName );
					...
					loadingObjects.collections[ collectionClassName ] = jk.api.JKApiBaseCollection.extend( {
					...
					collectionClassName = jk.api.utils.capitalizeAndCamelCaseDashes( routeName );
					modelClassName      = mapping.models[ collectionClassName ] || collectionClassName;
					collectionClassName = mapping.collections[ collectionClassName ] || collectionClassName;
					loadingObjects.collections[ collectionClassName ] = jk.api.JKApiBaseCollection.extend( {
				...
				// Add defaults to the new model, pulled form the endpoint.
				jk.api.utils.decorateFromRoute( collectionRoute.route.endpoints, loadingObjects.collections[ collectionClassName ] );
			} );

			// Add mixins and helpers for each of the models.
			_.each( loadingObjects.models, function( model, index ) {
				loadingObjects.models[ index ] = jk.api.utils.addMixinsAndHelpers( model, index, loadingObjects );
			} );
	...
	jk.api.endpoints = new Backbone.Collection();

	/**
	 * Initialize the jk-api, optionally passing the API root.
	 *
	 * @param {Object} [args]
	 * @param {string} [args.nonce] The nonce. Optional, defaults to jkApiSettings.nonce.
	 * @param {string} [args.apiRoot] The api root. Optional, defaults to jkApiSettings.root.
	 * @param {string} [args.versionString] The version string. Optional, defaults to jkApiSettings.root.
	 * @param {Object} [args.schema] The schema. Optional, will be fetched from API if not provided.
	 */
	jk.api.init = function( args ) {
		var endpoint, attributes = {}, deferred, promise;

		args                      = args || {};
		attributes.nonce          = _.isString( args.nonce ) ? args.nonce : ( jkApiSettings.nonce || '' );
		attributes.apiRoot        = args.apiRoot || jkApiSettings.root || '/jk-json';
		attributes.versionString  = args.versionString || jkApiSettings.versionString || 'jk/v2/';
		attributes.schema         = args.schema || null;
		attributes.modelEndpoints = args.modelEndpoints || [ 'me', 'settings' ];
		if ( ! attributes.schema && attributes.apiRoot === jkApiSettings.root && attributes.versionString === jkApiSettings.versionString ) {
			attributes.schema = jkApiSettings.schema;
		}

		if ( ! initializedDeferreds[ attributes.apiRoot + attributes.versionString ] ) {

			// Look for an existing copy of this endpoint.
			endpoint = jk.api.endpoints.findWhere( { 'apiRoot': attributes.apiRoot, 'versionString': attributes.versionString } );
			if ( ! endpoint ) {
			...
			endpoint.schemaConstructed.done( function( resolvedEndpoint ) {
				jk.api.endpoints.add( resolvedEndpoint );

				// Map the default endpoints, extending any already present items (including Schema model).
				jk.api.models      = _.extend( jk.api.models, resolvedEndpoint.get( 'models' ) );
				jk.api.collections = _.extend( jk.api.collections, resolvedEndpoint.get( 'collections' ) );
	...
	// The jk.api.init function returns a promise that will resolve with the endpoint once it is ready.
	jk.api.loadPromise = jk.api.init();
