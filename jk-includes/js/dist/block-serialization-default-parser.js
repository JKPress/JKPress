 ...
 */
const tokenizer = /<!--\s+(\/)?jk:([a-z][a-z0-9_-]*\/)?([a-z][a-z0-9_-]*)\s+({(?:(?=([^}]+|}+(?=})|(?!}\s+\/?-->)[^])*)\5|[^]*?)}\s+)?(\/)?-->/g;
 ...
 * Input post:
 * ```html
 * <!-- jk:columns {"columns":3} -->
 * <div class="jk-block-columns has-3-columns"><!-- jk:column -->
 ...
 *             }
 *         ],
 *         innerHTML: '\n<div class="jk-block-columns has-3-columns">\n\n\n\n</div>\n'
 *     }
 * ];
 * ```
...
(window.jk = window.jk || {}).blockSerializationDefaultParser = __webpack_exports__;
/******/ })()
;