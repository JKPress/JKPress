 ...
 */
const getFormatTypes = (0,external_jk_data_namespaceObject.createSelector)(state => Object.values(state.formatTypes), state => [state.formatTypes]);
...
;// external ["jk","escapeHtml"]
const external_jk_escapeHtml_namespaceObject = window["jk"]["escapeHtml"];
...
function get_format_type_getFormatType(name) {
  return (0,external_jk_data_namespaceObject.select)(store).getFormatType(name);
}
...
function createElementHTML({
  ...
  let attributeString = '';
  for (const key in attributes) {
    if (!(0,external_jk_escapeHtml_namespaceObject.isValidAttributeName)(key)) {
      continue;
    }
    attributeString += ` ${key}="${(0,external_jk_escapeHtml_namespaceObject.escapeAttribute)(attributes[key])}"`;
  }
  if (object) {
...
function createChildrenHTML(children = []) {
  return children.map(child => {
    ...
    return child.text === undefined ? createElementHTML(child) : (0,external_jk_escapeHtml_namespaceObject.escapeEditableHTML)(child.text);
  }).join('');
}
...
function toFormat({
  ...
  if (attributes && attributes.class) {
    formatType = (0,external_jk_data_namespaceObject.select)(store).getFormatTypeForClassName(attributes.class);
    if (formatType) {
  ...
  if (!formatType) {
    formatType = (0,external_jk_data_namespaceObject.select)(store).getFormatTypeForBareElement(tagName);
  }
  if (!formatType) {
    return attributes ? {
      type: tagName,
 ...
 *
 * @param {string}   name     Format name.
 * @param {JKFormat} settings Format settings.
 *
 * @return {JKFormat|undefined} The format, if it has been successfully
...
function registerFormatType(name, settings) {
  ...
  if ((0,external_jk_data_namespaceObject.select)(store).getFormatType(settings.name)) {
  ...
  if (settings.className === null) {
    const formatTypeForBareElement = (0,external_jk_data_namespaceObject.select)(store).getFormatTypeForBareElement(settings.tagName);
    ...
    const formatTypeForClassName = (0,external_jk_data_namespaceObject.select)(store).getFormatTypeForClassName(settings.className);
  ...
  (0,external_jk_data_namespaceObject.dispatch)(store).addFormatTypes(settings);
  return settings;
}
...
function toggleFormat(value, format) {
  if (getActiveFormat(value, format.type)) {
    ...
    if (format.title) {
      ...
      (0,external_jk_a11y_namespaceObject.speak)((0,external_jk_i18n_namespaceObject.sprintf)((0,external_jk_i18n_namespaceObject.__)('%s removed.'), format.title), 'assertive');
    }
  ...
  if (format.title) {
    // translators: %s: title of the formatting control
    (0,external_jk_a11y_namespaceObject.speak)((0,external_jk_i18n_namespaceObject.sprintf)((0,external_jk_i18n_namespaceObject.__)('%s applied.'), format.title), 'assertive');
  }
...
/** @typedef {import('./register-format-type').JKFormat} JKFormat */
 ...
 * @param {string} name Format name.
 *
 * @return {JKFormat|undefined} The previous format value, if it has
 *                                        been successfully unregistered;
 *                                        otherwise `undefined`.
 */
function unregisterFormatType(name) {
  const oldFormat = (0,external_jk_data_namespaceObject.select)(store).getFormatType(name);
  if (!oldFormat) {
    window.console.error(`Format ${name} is not registered.`);
    return;
  }
  (0,external_jk_data_namespaceObject.dispatch)(store).removeFormatTypes(name);
  return oldFormat;
}
 ...
 * @param {JKFormat}               $1.settings The format type's settings.
 *
...
function useAnchorRef({
  ...
  external_jk_deprecated_default()('`useAnchorRef` hook', {
    since: '6.1',
...
function useAnchor({
  ...
  const [anchor, setAnchor] = (0,external_jk_element_namespaceObject.useState)(() => getAnchor(editableContentElement, tagName, className));
  const wasActive = (0,external_jk_compose_namespaceObject.usePrevious)(isActive);
...
function useDefaultStyle() {
  return (0,external_jk_element_namespaceObject.useCallback)(element => {
    if (!element) {
      return;
    }
...
/* harmony default export */ const format_boundaries = (props => element => {
  function onKeyDown(event) {
    ...
    shiftKey || altKey || metaKey || ctrlKey || keyCode !== external_jk_keycodes_namespaceObject.LEFT && keyCode !== external_jk_keycodes_namespaceObject.RIGHT) {
      return;
    }
    const {
      record,
    ...
    const reverseKey = direction === 'rtl' ? external_jk_keycodes_namespaceObject.RIGHT : external_jk_keycodes_namespaceObject.LEFT;
...
/* harmony default export */ const event_listeners_delete = (props => element => {
  function onKeyDown(event) {
    ...
    if (keyCode !== external_jk_keycodes_namespaceObject.DELETE && keyCode !== external_jk_keycodes_namespaceObject.BACKSPACE) {
      return;
    }
...
function useEventListeners(props) {
  const propsRef = (0,external_jk_element_namespaceObject.useRef)(props);
  propsRef.current = props;
  const refEffects = (0,external_jk_element_namespaceObject.useMemo)(() => allEventListeners.map(refEffect => refEffect(propsRef)), [propsRef]);
  return (0,external_jk_compose_namespaceObject.useRefEffect)(element => {
...
function useRichText({
  ...
  const [, forceRender] = (0,external_jk_element_namespaceObject.useReducer)(() => ({}));
  const ref = (0,external_jk_element_namespaceObject.useRef)();
  ...
  const _valueRef = (0,external_jk_element_namespaceObject.useRef)(value);
  const recordRef = (0,external_jk_element_namespaceObject.useRef)();
  ...
  const hadSelectionUpdateRef = (0,external_jk_element_namespaceObject.useRef)(false);
  ...
  const didMountRef = (0,external_jk_element_namespaceObject.useRef)(false);
  ...
  const mergedRefs = (0,external_jk_compose_namespaceObject.useMergeRefs)([ref, useDefaultStyle(), useBoundaryStyle({
    record: recordRef
  }), useEventListeners({
    record: recordRef,
    handleChange,
    ...
    onSelectionChange,
    forceRender
  }), (0,external_jk_compose_namespaceObject.useRefEffect)(() => {
    applyFromProps();
...
(window.jk = window.jk || {}).richText = __webpack_exports__;
/******/ })()
;