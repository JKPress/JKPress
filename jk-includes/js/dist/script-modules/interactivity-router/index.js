...
const {
  ...
  populateServerData,
  batch
} = (0,interactivity_namespaceObject.privateApis)('I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of JKPress.');
...
const isValidLink = ref => ref && ref instanceof window.HTMLAnchorElement && ref.href && (!ref.target || ref.target === '_self') && ref.origin === window.location.origin && !ref.pathname.startsWith('/jk-admin') && !ref.pathname.startsWith('/jk-login.php') && !ref.getAttribute('href').startsWith('#') && !new URL(ref.href).searchParams.has('_jknonce');
...
function a11ySpeak(messageKey) {
  if (!hasLoadedNavigationTextsData) {
    ...
    const content = document.getElementById('jk-script-module-data-@wordpress/interactivity-router')?.textContent;
    if (content) {
      try {
