...
const immutableError = () => {
  throw new Error('Please use `data-jk-bind` to modify the attributes of an element.');
};
 ...
 * with `jk_interactivity_data_jk_context()`, including the corresponding
 ...
 * `jk_interactivity_state()`. When using `actions.navigate()`, this object is
 ...
 *
 * ```html
 * <div data-jk-interactive="counter">
 *   <button
 *     data-jk-text="state.double"
 *     data-jk-on--click="actions.increment"
 *   >
 *     0
 *   </button>
 * </div>
...
const parseServerData = (dom = document) => {
  ...
  (_dom$getElementById = dom.getElementById('jk-script-module-data-@wordpress/interactivity')) !== null && _dom$getElementById !== void 0 ? _dom$getElementById :
  // Legacy form
  dom.getElementById('jk-interactivity-data');
 ...
 * ```js
 * directive(
 *   'alert', // Name without the `data-jk-` prefix.
 ...
 * the `data-jk-alert` directive will have the `onclick` event handler, e.g.,
 *
 * ```html
 * <div data-jk-interactive="messages">
 *   <button data-jk-alert="state.alert">Click me!</button>
 * </div>
 * ```
 ...
 *
 * ```html
 * <div data-jk-interactive="myblock">
 *   <button
 *     data-jk-color--text="state.text"
 *     data-jk-color--background="state.background"
 *   >Click me!</button>
 * </div>
 * ```
 *
 ...
 * ```
 *
 * @param name             Directive name, without the `data-jk-` prefix.
...
/* harmony default export */ const directives = (() => {
    ...
    const contextStack = T(() => {
      ...
      if (defaultEntry) {
        ...
        if (!isPlainObject(value)) {
          warn(`The value of data-jk-context in "${namespace}" store must be a valid stringified JSON object.`);
        }
  ...
  // data-jk-on-window--[event]
  directive('on-window', getGlobalEventDirective('window'));
  // data-jk-on-document--[event]
  directive('on-document', getGlobalEventDirective('document'));

  // data-jk-on-async-window--[event]
  directive('on-async-window', getGlobalAsyncEventDirective('window'));
  // data-jk-on-async-document--[event]
  directive('on-async-document', getGlobalAsyncEventDirective('document'));

  // data-jk-class--[classname]
  directive('class', ({
    directives: {
      class: classNames
    },
...
// ${p} must be a prefix string, like 'jk'.
...
const requiredConsent = 'I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of JKPress.';
