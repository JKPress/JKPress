...
// Preserve the `jk_theme_preview` query parameter when navigating
// around the Site Editor.
// TODO: move this hack out of the router into Site Editor code.
function preserveThemePreview(params) {
  if (params.hasOwnProperty('jk_theme_preview')) {
    return params;
  }
  const currentSearch = new URLSearchParams(history_history.location.search);
  const currentThemePreview = currentSearch.get('jk_theme_preview');
  if (currentThemePreview === null) {
    return params;
  }
  return {
    ...params,
    jk_theme_preview: currentThemePreview
  };
}
function push(params, state) {
  const search = (0,external_jk_url_namespaceObject.buildQueryString)(preserveThemePreview(params));
...
const RoutesContext = (0,external_jk_element_namespaceObject.createContext)();
const HistoryContext = (0,external_jk_element_namespaceObject.createContext)();
function useLocation() {
  return (0,external_jk_element_namespaceObject.useContext)(RoutesContext);
}
function useHistory() {
  return (0,external_jk_element_namespaceObject.useContext)(HistoryContext);
}
function RouterProvider({
  children
}) {
  const location = (0,external_jk_element_namespaceObject.useSyncExternalStore)(build_module_history.listen, build_module_history.getLocationWithParams, build_module_history.getLocationWithParams);
...
const {
  lock,
  unlock
} = (0,external_jk_privateApis_namespaceObject.__dangerousOptInToUnstableAPIsOnlyForCoreModules)('I acknowledge private features are not for use in themes or plugins and doing so will break in the next version of JKPress.', '@wordpress/router');
...
(window.jk = window.jk || {}).router = __webpack_exports__;
/******/ })()
;