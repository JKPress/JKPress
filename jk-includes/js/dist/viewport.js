...
const addDimensionsEventListener = (breakpoints, operators) => {
   ...
   */
  const setIsMatching = (0,external_jk_compose_namespaceObject.debounce)(() => {
    const values = Object.fromEntries(queries.map(([key, query]) => [key, query.matches]));
    (0,external_jk_data_namespaceObject.dispatch)(store).setIsMatching(values);
  }, 0, {
    leading: true
  });
...
const withViewportMatch = queries => {
  ...
  const useViewPortQueriesResult = () => Object.fromEntries(queryEntries.map(([key, query]) => {
    ...
    return [key, (0,external_jk_compose_namespaceObject.useViewportMatch)(breakpointName, operator)];
  }));
  return (0,external_jk_compose_namespaceObject.createHigherOrderComponent)(WrappedComponent => {
    return (0,external_jk_compose_namespaceObject.pure)(props => {
 ...
 * @return {Function} Higher-order component.
 */
const ifViewportMatches = query => (0,external_jk_compose_namespaceObject.createHigherOrderComponent)((0,external_jk_compose_namespaceObject.compose)([with_viewport_match({
  isViewportMatch: query
}), (0,external_jk_compose_namespaceObject.ifCondition)(props => props.isViewportMatch)]), 'ifViewportMatches');
...
(window.jk = window.jk || {}).viewport = __webpack_exports__;
/******/ })()
;