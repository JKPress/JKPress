...
/**
 * Creates a persistence layer that stores data in JKPress user meta via the
 * REST API.
 *
 ...
 *
 * @return {Object} A persistence layer for JKPress user meta.
 */
function create({
  preloadedData,
  localStorageRestoreKey = 'JK_PREFERENCES_RESTORE_DATA',
  requestDebounceMS = 2500
} = {}) {
  let cache = preloadedData;
  const debouncedApiFetch = debounceAsync((external_jk_apiFetch_default()), requestDebounceMS);
  async function get() {
    if (cache) {
      return cache;
    }
    const user = await external_jk_apiFetch_default()({
      path: '/jk/v2/users/me?context=edit'
    });
...
function getLegacyData(userId) {
  const key = `JK_DATA_USER_${userId}`;
 ...
 *
 * This function is used internally by JKPress in an inline script, so
...
function __unstableCreatePersistenceLayer(serverData, userId) {
  const localStorageRestoreKey = `JK_PREFERENCES_USER_${userId}`;
...
(window.jk = window.jk || {}).preferencesPersistence = __webpack_exports__;
/******/ })()
;