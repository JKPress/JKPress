...
;// external ["jk","reduxRoutine"]
const external_jk_reduxRoutine_namespaceObject = window["jk"]["reduxRoutine"];
var external_jk_reduxRoutine_default = /*#__PURE__*/__webpack_require__.n(external_jk_reduxRoutine_namespaceObject);
;// external ["jk","compose"]
...
const {
  lock,
  unlock
} = (0,external_jk_privateApis_namespaceObject.__dangerousOptInToUnstableAPIsOnlyForCoreModules)('I acknowledge private features are not for use in themes or plugins and doing so will break in the next version of JKPress.', '@wordpress/data');
...
/** @typedef {import('./registry').JKDataRegistry} JKDataRegistry */

/**
 * Creates a middleware handling resolvers cache invalidation.
 *
 * @param {JKDataRegistry} registry  Registry for which to create the middleware.
...
function getIsResolving(state, selectorName, args) {
  external_jk_deprecated_default()('jk.data.select( store ).getIsResolving', {
    since: '6.6',
    version: '6.8',
    alternative: 'jk.data.select( store ).getResolutionState'
  });
...
function instantiateReduxStore(key, options, registry, thunkArgs) {
  ...
  const middlewares = [resolvers_cache_middleware(registry, key), promise_middleware, external_jk_reduxRoutine_default()(normalizedControls), createThunkMiddleware(thunkArgs)];
  ...
  return createStore(enhancedReducer, {
    root: initialState
  }, (0,external_jk_compose_namespaceObject.compose)(enhancers));
}
...
/**
 * @typedef {Object} JKDataRegistry An isolated orchestrator of store registrations.
 *
...
/**
 * @typedef {Object} JKDataPlugin An object of registry function overrides.
 *
 ...
 *
 * @return {JKDataRegistry} Data registry.
 */
function createRegistry(storeConfigs = {}, parent = null) {
  ...
  const subscribe = (listener, storeNameOrDescriptor) => {
    ...
    // See https://github.com/JKPress/gutenberg/pull/27466 for more info.
    if (!parent) {
      return emitter.subscribe(listener);
    }
  ...
  function registerGenericStore(name, store) {
    external_jk_deprecated_default()('jk.data.registerGenericStore', {
      since: '5.9',
      alternative: 'jk.data.register( storeDescriptor )'
    });
...
/** @typedef {import('../../registry').JKDataPlugin} JKDataPlugin */

/**
 * @typedef {Object} JKDataPersistencePluginOptions Persistence plugin options.
 *
 ...
 * @type {string}
 */
const DEFAULT_STORAGE_KEY = 'JK_DATA';
 ...
 *
 * @param {JKDataPersistencePluginOptions} options Plugin options.
 *
 ...
 *
 * @param {JKDataRegistry}                  registry      Data registry.
 * @param {?JKDataPersistencePluginOptions} pluginOptions Plugin options.
 *
 * @return {JKDataPlugin} Data plugin.
 */
...
const Context = (0,external_jk_element_namespaceObject.createContext)(default_registry);
const {
  Consumer,
  Provider
} = Context;
...
function useRegistry() {
  return (0,external_jk_element_namespaceObject.useContext)(Context);
}
...
const context_Context = (0,external_jk_element_namespaceObject.createContext)(false);
const {
...
function useAsyncMode() {
  return (0,external_jk_element_namespaceObject.useContext)(context_Context);
}
...
const renderQueue = (0,external_jk_priorityQueue_namespaceObject.createQueue)();
...
function Store(registry, suspense) {
  ...
  return (mapSelect, isAsync) => {
    function updateValue() {
      ...
      if (!external_jk_isShallowEqual_default()(lastMapResult, mapResult)) {
...
function useMappingSelect(suspense, mapSelect, deps) {
  ...
  const store = (0,external_jk_element_namespaceObject.useMemo)(() => Store(registry, suspense), [registry, suspense]);

  // These are "pass-through" dependencies from the parent hook,
  // and the parent should catch any hook rule violations.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const selector = (0,external_jk_element_namespaceObject.useCallback)(mapSelect, deps);
  const {
    subscribe,
    getValue
  } = store(selector, isAsync);
  const result = (0,external_jk_element_namespaceObject.useSyncExternalStore)(subscribe, getValue, getValue);
  (0,external_jk_element_namespaceObject.useDebugValue)(result);
  return result;
}
...
function useSelect(mapSelect, deps) {
  ...
  const staticSelectModeRef = (0,external_jk_element_namespaceObject.useRef)(staticSelectMode);
...
const withSelect = mapSelectToProps => (0,external_jk_compose_namespaceObject.createHigherOrderComponent)(WrappedComponent => (0,external_jk_compose_namespaceObject.pure)(ownProps => {
...
const useDispatchWithMap = (dispatchMap, deps) => {
  const registry = useRegistry();
  const currentDispatchMapRef = (0,external_jk_element_namespaceObject.useRef)(dispatchMap);
  (0,external_jk_compose_namespaceObject.useIsomorphicLayoutEffect)(() => {
...
const withDispatch = mapDispatchToProps => (0,external_jk_compose_namespaceObject.createHigherOrderComponent)(WrappedComponent => ownProps => {
...
const withRegistry = (0,external_jk_compose_namespaceObject.createHigherOrderComponent)(OriginalComponent => props => /*#__PURE__*/(0,external_ReactJSXRuntime_namespaceObject.jsx)(RegistryConsumer, {
...
(window.jk = window.jk || {}).data = __webpack_exports__;
/******/ })()
;