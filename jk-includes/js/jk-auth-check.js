 ...
 *
 * @output jk-includes/js/jk-auth-check.js
 */
...
( function( $ ) {
	 ...
	 */
	function show() {
		var parent = $( '#jk-auth-check' ),
			form = $( '#jk-auth-check-form' ),
			noframe = wrap.find( '.jk-auth-fallback-expired' ),
			frame, loaded = false;

		if ( form.length ) {
			// Add unload confirmation to counter (frame-busting) JS redirects.
			$( window ).on( 'beforeunload.jk-auth-check', function( event ) {
				event.originalEvent.returnValue = window.jk.i18n.__( 'Your session has expired. You can log in again from this page or go to the login page.' );
			});

			frame = $( '<iframe id="jk-auth-check-frame" frameborder="0">' ).attr( 'title', noframe.text() );
	...
	function hide() {
		...
		$( window ).off( 'beforeunload.jk-auth-check' );

		// When on the Edit Post screen, speed up heartbeat
		// after the user logs in to quickly refresh nonces.
		if ( ( adminpage === 'post-php' || adminpage === 'post-new-php' ) && jk && jk.heartbeat ) {
			jk.heartbeat.connectNow();
		}

		wrap.fadeOut( 200, function() {
			wrap.addClass( 'hidden' ).css( 'display', '' );
			$( '#jk-auth-check-frame' ).remove();
	...
	$( function() {
		 ...
		 * @since 3.6.0
		 */
		wrap = $( '#jk-auth-check-wrap' );
		wrap.find( '.jk-auth-check-close' ).on( 'click', function() {
			hide();
			setShowTimeout();
		});
	}).on( 'heartbeat-tick.jk-auth-check', function( e, data ) {
		if ( 'jk-auth-check' in data ) {
			if ( ! data['jk-auth-check'] && wrap.hasClass( 'hidden' ) && ! tempHidden ) {
				show();
			} else if ( data['jk-auth-check'] && ! wrap.hasClass( 'hidden' ) ) {
				hide();
			}
		}
	});
