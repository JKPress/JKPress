/* global pluploadL10n, plupload, _jkPluploadSettings */
...
( function( exports, $ ) {
	...
	if ( typeof _jkPluploadSettings === 'undefined' ) {
		return;
	}
	 ...
	 * JKPress back end and the JKPress media experience.
	 *
	 * @class
	 * @memberOf jk
	 * @alias jk.Uploader
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__jk-uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-jk-upload-attachment-id:\s*(\d+)/i );
			...
			if ( times && times > 4 ) {
				...
				$.ajax({
					...
					data: {
						...
						_jknonce: _jkPluploadSettings.defaults.multipart_params._jknonce,
						attachment_id: id,
		 ...
		 * and display errors. @see jk.Uploader.errors.
		 *
		 * @param {string}        message Error message.
		 * @param {object}        data    Error data from Plupload.
		 * @param {plupload.File} file    File that was uploaded.
		...
		fileUploaded = function( up, file, response ) {
			...
			jk.media.model.Attachment.get( response.data.id, file.attachment );
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.jk-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.jk-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.jk-uploader, drop.jk-uploader', function() {
				/*
		...
		this.uploader.bind( 'FilesAdded', function( up, files ) {
			_.each( files, function( file ) {
				...
				attributes = _.extend({
					...
					filename:  file.name,
					menuOrder: 0,
					uploadedTo: jk.media.model.settings.post.id
				...
				file.attachment = jk.media.model.Attachment.create( attributes );
	...
	$.extend( Uploader, _jkPluploadSettings );
	...
	$.extend( Uploader.prototype, /** @lends jk.Uploader.prototype */{
		/**
		...
		/**
		 * Make a few internal event callbacks available on the jk.Uploader object
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'jk-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="jk-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'jk-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
	...
	Uploader.queue = new jk.media.model.Attachments( [], { query: false });
