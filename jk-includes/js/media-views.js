...
var Selection = jk.media.model.Selection,
	Library = jk.media.controller.Library,
	CollectionAdd;

/**
 * jk.media.controller.CollectionAdd
 *
 ...
 *
 * @class
 * @augments jk.media.controller.Library
 ...
 * @param {jk.media.model.Attachments} [attributes.library]                 The attachments collection to browse.
 ...
 */
CollectionAdd = Library.extend(/** @lends jk.media.controller.CollectionAdd.prototype */{
	defaults: _.defaults( {
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', jk.media.query({ type: this.get('type') }) );
		}
...
var Library = jk.media.controller.Library,
	l10n = jk.media.view.l10n,
	$ = jQuery,
	CollectionEdit;

/**
 * jk.media.controller.CollectionEdit
 *
 ...
 * @param {jk.media.model.Attachments} [attributes.library]              The attachments collection to edit.
 ...
 *                                                                       If none supplied, defaults to jk.media.view.Attachment.EditLibrary.
 * @param {string}                     attributes.type                   The collection's media type. (e.g. 'video').
 * @param {string}                     attributes.collectionType         The collection type. (e.g. 'playlist').
 */
CollectionEdit = Library.extend(/** @lends jk.media.controller.CollectionEdit.prototype */{
	defaults: {
		multiple:         false,
		sortable:         true,
		date:             false,
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', new jk.media.model.Selection() );
		}
		// The single `Attachment` view to be used in the `Attachments` view.
		if ( ! this.get('AttachmentView') ) {
			this.set( 'AttachmentView', jk.media.view.Attachment.EditLibrary );
		}
		Library.prototype.initialize.apply( this, arguments );
	},
	...
	activate: function() {
		...
		this.get('library').observe( jk.Uploader.queue );
	...
	deactivate: function() {
		...
		this.get('library').unobserve( jk.Uploader.queue );
	 ...
	 * @since 3.9.0
	 *
	 * @param {jk.media.view.attachmentsBrowser} The attachments browser view.
	 */
	renderSettings: function( attachmentsBrowserView ) {
		...
		if ( dragInfoText ) {
			attachmentsBrowserView.toolbar.set( 'dragInfo', new jk.media.View({
...
var l10n = jk.media.view.l10n,
	Cropper;

/**
 * jk.media.controller.Cropper
 *
 ...
 * @augments Backbone.Model
 */
Cropper = jk.media.controller.State.extend(/** @lends jk.media.controller.Cropper.prototype */{
	defaults: {
		id:          'cropper',
	...
	createCropContent: function() {
		this.cropperView = new jk.media.view.Cropper({
			controller: this,
	...
	createCropToolbar: function() {
		...
		this.frame.toolbar.set( new jk.media.view.Toolbar(toolbarOptions) );
	},
	...
	doCrop: function( attachment ) {
		return jk.ajax.post( 'custom-header-crop', _.extend(
			{},
...
var Controller = jk.media.controller,
 ...
 * @since 4.3.0
 *
 * @constructs jk.media.controller.CustomizeImageCropper
 * @memberOf jk.media.controller
 * @augments jk.media.controller.CustomizeImageCropper.Cropper
 * @inheritDoc
 */
CustomizeImageCropper = Controller.Cropper.extend(/** @lends jk.media.controller.CustomizeImageCropper.prototype */{
	/**
	...
	doCrop: function( attachment ) {
		...
		return jk.ajax.post( 'crop-image', {
			jk_customize: 'on',
			nonce: attachment.get( 'nonces' ).edit,
			id: attachment.get( 'id' ),
			context: control.id,
 ...
 *
 * @param {object}                    attributes                      The attributes hash passed to the state.
 * @param {jk.media.model.Attachment} attributes.model                The attachment.
 ...
 */
EditImage = jk.media.controller.State.extend(/** @lends jk.media.controller.EditImage.prototype */{
	defaults: {
		id:      'edit-image',
	...
	toolbar: function() {
		...
		frame.toolbar.set( new jk.media.view.Toolbar({
			controller: frame,
			items: {
				back: {
					style: 'primary',
 ...
 */
Embed = jk.media.controller.State.extend(/** @lends jk.media.controller.Embed.prototype */{
	defaults: {
		id:       'embed',
	 ...
	 *
	 * @fires jk.media.controller.Embed#scan
	 */
	scan: function() {
		var scanners,
			embed = this,
...
var Attachment = jk.media.model.Attachment,
	Library = jk.media.controller.Library,
	l10n = jk.media.view.l10n,
	FeaturedImage;

/**
 * jk.media.controller.FeaturedImage
 *
 ...
 */
FeaturedImage = Library.extend(/** @lends jk.media.controller.FeaturedImage.prototype */{
	defaults: _.defaults({
		id:            'featured-image',
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', jk.media.query({ type: 'image' }) );
		}
	...
	updateSelection: function() {
		var selection = this.get('selection'),
			id = jk.media.view.settings.post.featuredImageId,
			attachment;
...
/**
 * jk.media.controller.GalleryAdd
 *
 ...
 *                                                                          Defaults to false because for this state, because the library of the Edit Gallery state is the selection.
 */
GalleryAdd = Library.extend(/** @lends jk.media.controller.GalleryAdd.prototype */{
	defaults: _.defaults({
		id:            'gallery-library',
...
/**
 * jk.media.controller.GalleryEdit
 *
 ...
 * @param {jk.media.model.Attachments} [attributes.library]               The collection of attachments in the gallery.
 *                                                                        If one is not supplied, an empty media.model.Selection collection is created.
 * @param {boolean}                    [attributes.multiple=false]        Whether multi-select is enabled.
 * @param {boolean}                    [attributes.searchable=false]      Whether the library is searchable.
 * @param {boolean}                    [attributes.sortable=true]         Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
 ...
 *                                                                        If none supplied, defaults to jk.media.view.Attachment.EditLibrary.
 */
GalleryEdit = Library.extend(/** @lends jk.media.controller.GalleryEdit.prototype */{
	defaults: {
		id:               'gallery-edit',
	 ...
	 * @since 3.5.0
	 *
	 * @param {jk.media.view.Frame} browser The file browser.
	 *
	 * @return {void}
	 */
	gallerySettings: function( browser ) {
		...
		browser.sidebar.set({
			gallery: new jk.media.view.Settings.Gallery({
				controller: this,
...
/**
 * jk.media.controller.ImageDetails
 *
 ...
 * @param {jk.media.model.Attachment} attributes.image                   The image's model.
 ...
 */
ImageDetails = State.extend(/** @lends jk.media.controller.ImageDetails.prototype */{
	defaults: _.defaults({
		id:       'image-details',
 ...
 * @param {jk.media.model.Selection|object} [attributes.selection]               A collection to contain attachment selections within the state.
 ...
 * @param {boolean}                         [attributes.syncSelection=true]      Whether the Attachments selection should be persisted from the last state.
 */
Library = jk.media.controller.State.extend(/** @lends jk.media.controller.Library.prototype */{
	defaults: {
		id:                 'library',
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', jk.media.query() );
		}

		if ( ! ( selection instanceof jk.media.model.Selection ) ) {
			props = selection;
			...
			this.set( 'selection', new jk.media.model.Selection( null, {
	...
	activate: function() {
		...
		jk.Uploader.queue.on( 'add', this.uploading, this );
	...
	deactivate: function() {
		...
		jk.Uploader.queue.off( null, null, this );
	},
	...
	resetDisplays: function() {
		var defaultProps = jk.media.view.settings.defaultProps;
		this._displays = [];
	 ...
	 * @since 3.5.0
	 *
	 * @param {jk.media.model.Attachment} attachment
	...
	canEmbed: function( attachment ) {
		...
		return _.contains( jk.media.view.settings.embedExts, attachment.get('filename').split('.').pop() );
	},
...
_.extend( Library.prototype, jk.media.selectionSync );
...
/**
 * jk.media.controller.MediaLibrary
 *
 * @memberOf jk.media.controller
 *
 * @class
...
MediaLibrary = Library.extend(/** @lends jk.media.controller.MediaLibrary.prototype */{
	defaults: _.defaults({
		// Attachments browser defaults. @see media.view.AttachmentsBrowser
		filterable:      'uploaded',
	...
	initialize: function( options ) {
		...
		this.set( 'library', jk.media.query({ type: this.type }) );
	...
	activate: function() {
		...
		if ( jk.media.frame.lastMime ) {
			this.set( 'library', jk.media.query({ type: jk.media.frame.lastMime }) );
			delete jk.media.frame.lastMime;
		}
		Library.prototype.activate.apply( this, arguments );
	}
});
...
_.extend( Region.prototype,/** @lends jk.media.controller.Region.prototype */{
	/**
	 * Activate a mode.
	 *
	 * @since 3.5.0
	 ...
	 *
	 * @return {jk.media.controller.Region} Returns itself to allow chaining.
	 */
	mode: function( mode ) {
		if ( ! mode ) {
			return this._mode;
		 ...
		 *
		 * @event jk.media.controller.Region#deactivate
		 */
		 ...
		 *
		 * @event jk.media.controller.Region#activate
		 */
	...
	render: function( mode ) {
		 ...
		 *
		 * @event jk.media.controller.Region#create
		 * @type {object}
		 ...
		 * Region view creation takes place in an event callback on the frame.
		 *
		 * @event jk.media.controller.Region#render
		 * @type {object}
		 */
	 ...
	 * @return {jk.Backbone.Subviews} Subviews is returned to allow chaining.
	 */
	 ...
	 *
	 * @param {string} event
	 * @return {undefined|jk.media.controller.Region} Returns itself to allow chaining.
	 */
...
/**
 * jk.media.controller.ReplaceImage
 *
 ...
 * @param {boolean}                    [attributes.syncSelection=true]      Whether the Attachments selection should be persisted from the last state.
 */
ReplaceImage = Library.extend(/** @lends jk.media.controller.ReplaceImage.prototype */{
	defaults: _.defaults({
		id:            'replace-image',
...
/**
 * jk.media.controller.SiteIconCropper
 *
 ...
 *
 * @class
 * @augments jk.media.controller.Cropper
 * @augments jk.media.controller.State
 * @augments Backbone.Model
 */
SiteIconCropper = Controller.Cropper.extend(/** @lends jk.media.controller.SiteIconCropper.prototype */{
	activate: function() {
		this.frame.on( 'content:create:crop', this.createCropContent, this );
		this.frame.on( 'close', this.removeCropper, this );
		this.set('selection', new Backbone.Collection(this.frame._selection.single));
	...
	createCropContent: function() {
		this.cropperView = new jk.media.view.SiteIconCropper({
			controller: this,
			attachment: this.get('selection').first()
		});
		this.cropperView.on('image-loaded', this.createCropToolbar, this);
...
/**
 * jk.media.controller.StateMachine
 *
...
_.extend( StateMachine.prototype, Backbone.Events,/** @lends jk.media.controller.StateMachine.prototype */{
	/**
	 * Fetch a state.
	 *
	 ...
	 *
	 * @param {string} id
	 * @return {jk.media.controller.State} Returns a State model from
	 ...
	 * @param {string} id
	 *
	 * @fires jk.media.controller.State#deactivate
	 * @fires jk.media.controller.State#activate
	 *
	 * @return {jk.media.controller.StateMachine} Returns itself to allow chaining.
	 */
...
_.each([ 'on', 'off', 'trigger' ], function( method ) {
	/**
	 * @function on
	 * @memberOf jk.media.controller.StateMachine
	 * @instance
	 * @return {jk.media.controller.StateMachine} Returns itself to allow chaining.
	 */
	/**
 ...
 * @augments Backbone.Model
 */
var State = Backbone.Model.extend(/** @lends jk.media.controller.State.prototype */{
	/**
	 * Constructor.
	 *
	 * @since 3.5.0
...
var View = jk.media.View,
	AttachmentCompat;

/**
 * jk.media.view.AttachmentCompat
 *
 ...
 */
AttachmentCompat = View.extend(/** @lends jk.media.view.AttachmentCompat.prototype */{
	tagName:   'form',
	...
	/**
	 * @return {jk.media.view.AttachmentCompat} Returns itself to allow chaining.
	 */
	dispose: function() {
...
/**
 * jk.media.view.AttachmentFilters
 *
 * @memberOf jk.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
AttachmentFilters = jk.media.View.extend(/** @lends jk.media.view.AttachmentFilters.prototype */{
	tagName:   'select',
...
/**
 * jk.media.view.AttachmentFilters.All
 *
 * @memberOf jk.media.view.AttachmentFilters
 *
 * @class
 * @augments jk.media.view.AttachmentFilters
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
All = jk.media.view.AttachmentFilters.extend(/** @lends jk.media.view.AttachmentFilters.All.prototype */{
	createFilters: function() {
		var filters = {},
			uid = window.userSettings ? parseInt( window.userSettings.uid, 10 ) : 0;

		_.each( jk.media.view.settings.mimeTypes || {}, function( text, key ) {
			filters[ key ] = {
				text: text,
				props: {
					status:  null,
		...
		if ( jk.media.view.settings.post.id ) {
			filters.uploaded = {
				text:  l10n.uploadedToThisPost,
				props: {
					status:  null,
					type:    null,
					uploadedTo: jk.media.view.settings.post.id,
					orderby: 'menuOrder',
					order:   'ASC',
					author:  null
				},
		...
		if ( jk.media.view.settings.mediaTrash &&
 ...
 * @augments Backbone.View
 */
DateFilter = jk.media.view.AttachmentFilters.extend(/** @lends jk.media.view.AttachmentFilters.Date.prototype */{
	id: 'media-attachment-date-filters',

	createFilters: function() {
		var filters = {};
		_.each( jk.media.view.settings.months || {}, function( value, index ) {
			filters[ index ] = {
				text: value.text,
				props: {
					year: value.year,
...
/**
 * jk.media.view.AttachmentFilters.Uploaded
 *
 * @memberOf jk.media.view.AttachmentFilters
 *
 * @class
 ...
 * @augments Backbone.View
 */
Uploaded = jk.media.view.AttachmentFilters.extend(/** @lends jk.media.view.AttachmentFilters.Uploaded.prototype */{
	createFilters: function() {
		var type = this.model.get('type'),
			types = jk.media.view.settings.mimeTypes,
			uid = window.userSettings ? parseInt( window.userSettings.uid, 10 ) : 0,
			text;
 ...
 * @augments Backbone.View
 */
Attachment = View.extend(/** @lends jk.media.view.Attachment.prototype */{
	tagName:   'li',
	className: 'attachment',
	template:  jk.template('attachment'),
	...
	/**
	 * @return {jk.media.view.Attachment} Returns itself to allow chaining.
	 */
	dispose: function() {
		var selection = this.options.selection;
...
_.each({
	 ...
	 * @memberOf jk.media.view.Attachment
	 * @instance
	 *
	 * @param {Backbone.Model} model
	 * @param {string} value
...
/* global ClipboardJS */
var Attachment = jk.media.view.Attachment,
	l10n = jk.media.view.l10n,
	$ = jQuery,
	Details,
	__ = jk.i18n.__;

Details = Attachment.extend(/** @lends jk.media.view.Attachment.Details.prototype */{
	tagName:   'div',
	className: 'attachment-details',
	template:  jk.template('attachment-details'),
	 ...
	 */
	 copyAttachmentDetailsURLClipboard: function() {
		...
		clipboard.on( 'success', function( event ) {
			...
			jk.a11y.speak( __( 'The file URL has been copied to your clipboard' ) );
		} );
	 },
	 ...
	 * @since 3.5.0
	 *
	 * @constructs jk.media.view.Attachment.Details
	 * @augments jk.media.view.Attachment
	 *
	 * @return {void}
	 */
	initialize: function() {
	 ...
	 * @since 4.0.0
	 *
	 * @fires jk.media.controller.MediaLibrary#attachment:details:shift-tab
	 * @fires jk.media.controller.MediaLibrary#attachment:keydown:arrow
	 *
	...
	render: function() {
		...
		jk.media.mixin.removeAllPlayers();
		this.$( 'audio, video' ).each( function (i, elem) {
			var el = jk.media.view.MediaDetails.prepareSrc( elem );
			new window.MediaElementPlayer( el, jk.media.mixin.mejsSettings );
		} );
	}
});
...
/**
 * jk.media.view.Attachment.EditLibrary
 *
 * @memberOf jk.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var EditLibrary = jk.media.view.Attachment.extend(/** @lends jk.media.view.Attachment.EditLibrary.prototype */{
	buttons: {
		close: true
	}
});
...
/**
 * jk.media.view.Attachment.EditSelection
 *
 * @memberOf jk.media.view.Attachment
 *
 * @class
 * @augments jk.media.view.Attachment.Selection
 * @augments jk.media.view.Attachment
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
var EditSelection = jk.media.view.Attachment.Selection.extend(/** @lends jk.media.view.Attachment.EditSelection.prototype */{
	buttons: {
		close: true
	}
});
...
/**
 * jk.media.view.Attachment.Library
 *
 * @memberOf jk.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Library = jk.media.view.Attachment.extend(/** @lends jk.media.view.Attachment.Library.prototype */{
	buttons: {
		check: true
	}
});
...
/**
 * jk.media.view.Attachment.Selection
 *
 * @memberOf jk.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Selection = jk.media.view.Attachment.extend(/** @lends jk.media.view.Attachment.Selection.prototype */{
...
var View = jk.media.View,
	$ = jQuery,
	Attachments,
	infiniteScrolling = jk.media.view.settings.infiniteScrolling;

Attachments = View.extend(/** @lends jk.media.view.Attachments.prototype */{
	tagName:   'ul',
	...
	initialize: function() {
		...
		_.defaults( this.options, {
			...
			refreshSensitivity: jk.media.isTouchDevice ? 300 : 200,
			refreshThreshold:   3,
			AttachmentView:     jk.media.view.Attachment,
			sortable:           false,
			resize:             true,
	 ...
	 * @param {jk.media.model.Attachment} attachment
	 *
	 * @return {jk.media.View} The created view.
	 */
...
var View = jk.media.View,
	mediaTrash = jk.media.view.settings.mediaTrash,
	l10n = jk.media.view.l10n,
	$ = jQuery,
	AttachmentsBrowser,
	infiniteScrolling = jk.media.view.settings.infiniteScrolling,
	__ = jk.i18n.__,
	sprintf = jk.i18n.sprintf;

/**
 * jk.media.view.AttachmentsBrowser
 *
 * @memberOf jk.media.view
 *
 * @class
 ...
 */
AttachmentsBrowser = View.extend(/** @lends jk.media.view.AttachmentsBrowser.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		_.defaults( this.options, {
			...
			display: false,
			sidebar: true,
			AttachmentView: jk.media.view.Attachment.Library
		});
	...
	/**
	 * Updates the `jk.a11y.speak()` ARIA live region with a message to communicate
	...
	announceSearchResults: _.debounce( function() {
		...
		if ( this.collection.mirroring && this.collection.mirroring.args.s ) {
			...
			if ( 0 === count ) {
				jk.a11y.speak( l10n.noMediaTryNewSearch );
				return;
			}

			if ( this.collection.hasMore() ) {
				jk.a11y.speak( mediaFoundHasMoreResultsMessage.replace( '%d', count ) );
				return;
			}

			jk.a11y.speak( l10n.mediaFound.replace( '%d', count ) );
		}
	}, 200 ),
	...
	/**
	 * @return {jk.media.view.AttachmentsBrowser} Returns itself to allow chaining.
	 */
	dispose: function() {
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'media-toolbar jk-filter';
		}

		/**
		* @member {jk.media.view.Toolbar}
		*/
		this.toolbar = new jk.media.view.Toolbar( toolbarOptions );

		this.views.add( this.toolbar );

		this.toolbar.set( 'spinner', new jk.media.view.Spinner({
			priority: -20
		}) );
		...
		if ( showFilterByType || this.options.date ) {
			 ...
			 */
			this.toolbar.set( 'filters-heading', new jk.media.view.Heading( {
				priority:   -100,
		...
		if ( showFilterByType ) {
			...
			this.toolbar.set( 'filtersLabel', new jk.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.toolbar.set( 'filters', new jk.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				}).render() );
			} else {
				Filters = new jk.media.view.AttachmentFilters.All({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			LibraryViewSwitcher = View.extend({
				...
				template: jk.template( 'media-library-view-switcher')
			});
			...
			this.toolbar.set( 'dateFilterLabel', new jk.media.view.Label({
			...
			this.toolbar.set( 'dateFilter', new jk.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.toolbar.set( 'selectModeToggleButton', new jk.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.toolbar.set( 'deleteSelectedButton', new jk.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.toolbar.set( 'deleteSelectedPermanentlyButton', new jk.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
		...
		if ( this.options.search ) {
			...
			this.toolbar.set( 'searchLabel', new jk.media.view.Label({
			...
			this.toolbar.set( 'search', new jk.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
	...
	createUploader: function() {
		this.uploader = new jk.media.view.UploaderInline({
			controller: this.controller,
			status:     false,
	...
	createAttachmentsWrapperView: function() {
		this.attachmentsWrapper = new jk.media.View( {
	...
	createAttachments: function() {
		this.attachments = new jk.media.view.Attachments({
			controller:           this.controller,
			collection:           this.collection,
			selection:            this.options.selection,
			model:                this.model,
	...
	createLoadMoreView: function() {
		...
		this.loadMoreButton = new jk.media.view.Button( {
			text: __( 'Load more' ),
		...
		this.loadMoreSpinner = new jk.media.view.Spinner();

		this.loadMoreJumpToFirst = new jk.media.view.Button( {
	...
	createAttachmentsHeading: function() {
		this.attachmentsHeading = new jk.media.view.Heading( {
	...
	createSidebar: function() {
		var options = this.options,
			...
			sidebar = this.sidebar = new jk.media.view.Sidebar({
				controller: this.controller
			});
		...
		if ( this.controller.uploader ) {
			sidebar.set( 'uploads', new jk.media.view.UploaderStatus({
				controller: this.controller,
				priority:   40
			}) );
		}
	...
	createSingle: function() {
		...
		sidebar.set( 'details', new jk.media.view.Attachment.Details({
			controller: this.controller,
			model:      single,
			priority:   80
		}) );

		sidebar.set( 'compat', new jk.media.view.AttachmentCompat({
			controller: this.controller,
			model:      single,
			priority:   120
		}) );
		...
		if ( this.options.display ) {
			sidebar.set( 'display', new jk.media.view.Settings.AttachmentDisplay({
				controller:   this.controller,
...
var Attachments = jk.media.view.Attachments,
	Selection;

/**
 * jk.media.view.Attachments.Selection
 *
 * @memberOf jk.media.view.Attachments
 *
 * @class
 * @augments jk.media.view.Attachments
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
Selection = Attachments.extend(/** @lends jk.media.view.Attachments.Selection.prototype */{
	events: {},
	initialize: function() {
		_.defaults( this.options, {
			sortable:   false,
			...
			// The single `Attachment` view to be used in the `Attachments` view.
			AttachmentView: jk.media.view.Attachment.Selection
		});
		// Call 'initialize' directly on the parent class.
 ...
 * @augments Backbone.View
 */
ButtonGroup = jk.media.View.extend(/** @lends jk.media.view.ButtonGroup.prototype */{
	tagName:   'div',
	className: 'button-group button-large media-button-group',

	initialize: function() {
		/**
		 * @member {jk.media.view.Button[]}
		 */
		this.buttons = _.map( this.options.buttons || [], function( button ) {
			if ( button instanceof Backbone.View ) {
				return button;
			} else {
				return new jk.media.view.Button( button ).render();
			}
		});
	...
	/**
	 * @return {jk.media.view.ButtonGroup}
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
var Button = jk.media.View.extend(/** @lends jk.media.view.Button.prototype */{
	tagName:    'button',
	...
	/**
	 * @return {jk.media.view.Button} Returns itself to allow chaining.
	 */
	render: function() {
...
var View = jk.media.View,
	UploaderStatus = jk.media.view.UploaderStatus,
	l10n = jk.media.view.l10n,
	$ = jQuery,
	Cropper;

/**
 * jk.media.view.Cropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop an image.
 *
 * Takes imgAreaSelect options from
 * jk.customize.HeaderControl.calculateImageSelectOptions via
 * jk.customize.HeaderControl.openMM.
 *
 * @memberOf jk.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
Cropper = View.extend(/** @lends jk.media.view.Cropper.prototype */{
	className: 'crop-content',
	template: jk.template('crop-content'),
	initialize: function() {
	...
	onError: function() {
		...
		this.views.add( '.upload-errors', new jk.media.view.UploaderStatusError({
			filename: UploaderStatus.prototype.filename(filename),
			message: window._jkMediaViewsL10n.cropError
		}), { at: 0 });
	}
});
 ...
 * @augments Backbone.View
 */
EditImage = View.extend(/** @lends jk.media.view.EditImage.prototype */{
	className: 'image-editor',
	template: jk.template('image-editor'),
 ...
 * @augments Backbone.View
 */
var Embed = jk.media.View.extend(/** @lends jk.media.view.Ember.prototype */{
	className: 'media-embed',

	initialize: function() {
		/**
		 * @member {jk.media.view.EmbedUrl}
		 */
		this.url = new jk.media.view.EmbedUrl({
			controller: this.controller,
	...
	refresh: function() {
		...
		if ( 'image' === type ) {
			constructor = jk.media.view.EmbedImage;
		} else if ( 'link' === type ) {
			constructor = jk.media.view.EmbedLink;
		} else {
			return;
		}
 ...
 * @augments Backbone.View
 */
EmbedImage = AttachmentDisplay.extend(/** @lends jk.media.view.EmbedImage.prototype */{
	className: 'embed-media-settings',
	template:  jk.template('embed-image-settings'),
 ...
 * @augments Backbone.View
 */
EmbedLink = jk.media.view.Settings.extend(/** @lends jk.media.view.EmbedLink.prototype */{
	className: 'embed-link-settings',
	template:  jk.template('embed-link-settings'),
	...
	updateoEmbed: _.debounce( function() {
		...
		this.fetch();
	}, jk.media.controller.Embed.sensitivity ),
	...
	fetch: function() {
		...
		this.dfd = jk.apiRequest({
			url: jk.media.view.settings.oEmbedProxyUrl,
			data: {
				url: url,
 ...
 * @augments Backbone.View
 */
EmbedUrl = View.extend(/** @lends jk.media.view.EmbedUrl.prototype */{
	tagName:   'span',
	className: 'embed-url',
	...
	/**
	 * @return {jk.media.view.EmbedUrl} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
var FocusManager = jk.media.View.extend(/** @lends jk.media.view.FocusManager.prototype */{
	 ...
	 * prevents the `jk.a11y.speak()` ARIA live regions to work as they're outside
 ...
 * @memberOf jk.media.view
 *
 * @see jk.media.controller.State
 * @see jk.media.controller.Region
 *
 * @class
 * @augments jk.media.View
 * @augments jk.Backbone.View
 ...
 */
var Frame = jk.media.View.extend(/** @lends jk.media.view.Frame.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			mode: [ 'select' ]
		});
	...
	_createRegions: function() {
		...
		_.each( this.regions, function( region ) {
			this[ region ] = new jk.media.controller.Region({
				view:     this,
				id:       region,
	 ...
	 *
	 * @see jk.media.controller.State
	 * @see jk.media.controller.StateMachine
	 *
	 * @fires jk.media.controller.State#ready
	 */
	_createStates: function() {
		// Create the default `states` collection.
		this.states = new Backbone.Collection( null, {
			model: jk.media.controller.State
		});
	 ...
	 *
	 * @return {jk.media.view.Frame} Returns itself to allow chaining.
	 */
	reset: function() {
	...
	deactivateMode: function( mode ) {
		 ...
		 *
		 * @event jk.media.view.Frame#{mode}:deactivate
		 */
...
_.extend( Frame.prototype, jk.media.controller.StateMachine.prototype );
...
/**
 * jk.media.view.MediaFrame.ImageDetails
 *
 ...
 * @mixes jk.media.controller.StateMachine
 */
ImageDetails = Select.extend(/** @lends jk.media.view.MediaFrame.ImageDetails.prototype */{
	defaults: {
		id:      'image',
		url:     '',
		menu:    'image-details',
	...
	initialize: function( options ) {
		this.image = new jk.media.model.PostImage( options.metadata );
		this.options.selection = new jk.media.model.Selection( this.image.attachment, { multiple: false } );
	...
	createStates: function() {
		this.states.add([
			new jk.media.controller.ImageDetails({
				image: this.image,
				editable: false
			}),
			new jk.media.controller.ReplaceImage({
				id: 'replace-image',
				library: jk.media.query( { type: 'image' } ),
				image: this.image,
				multiple:  false,
			...
			new jk.media.controller.EditImage( {
				image: this.image,
				selection: this.options.selection
			} )
		]);
	...
	imageDetailsContent: function( options ) {
		options.view = new jk.media.view.ImageDetails({
			controller: this,
	...
	editImageContent: function() {
		...
		view = new jk.media.view.EditImage( { model: model, controller: this } ).render();
			...
			items: {
				select: {
					...
					click: function() {
						...
						// Not sure if we want to use jk.media.string.image which will create a shortcode or
						// perhaps jk.html.string to at least to build the <img />.
 ...
 * @mixes jk.media.controller.StateMachine
 */
Post = Select.extend(/** @lends jk.media.view.MediaFrame.Post.prototype */{
	initialize: function() {
		this.counts = {
			audio: {
				count: jk.media.view.settings.attachmentCounts.audio,
				state: 'playlist'
			},
			video: {
				count: jk.media.view.settings.attachmentCounts.video,
	...
	createStates: function() {
		...
		this.states.add([
			...
			new Library({
				...
				toolbar:    'main-insert',
				filterable: 'all',
				library:    jk.media.query( options.library ),
			...
			new Library({
				...
				library:  jk.media.query( _.defaults({
					type: 'image'
				}, options.library ) )
			}),

			// Embed states.
			new jk.media.controller.Embed( { metadata: options.metadata } ),

			new jk.media.controller.EditImage( { model: options.editImage } ),

			// Gallery states.
			new jk.media.controller.GalleryEdit({
				library: options.selection,
				editing: options.editing,
				menu:    'gallery'
			}),

			new jk.media.controller.GalleryAdd(),
			...
			// Playlist states.
			new jk.media.controller.CollectionEdit({
				type: 'audio',
				collectionType: 'playlist',
				title:          l10n.editPlaylistTitle,
				SettingsView:   jk.media.view.Settings.Playlist,
				library:        options.selection,
				editing:        options.editing,
				menu:           'playlist',
			...
			new jk.media.controller.CollectionAdd({
				type: 'audio',
				collectionType: 'playlist',
		...
		if ( jk.media.view.settings.post.featuredImageId ) {
			this.states.add( new jk.media.controller.FeaturedImage() );
		}
	},
	...
	bindHandlers: function() {
		...
		if ( typeof checkCounts !== 'undefined' ) {
			this.listenTo( jk.media.model.Attachments.all, 'change:type', this.mediaTypeCounts );
		}
	...
	// Menus.
	/**
	 * @param {jk.Backbone.View} view
	 */
	mainMenu: function( view ) {
		view.set({
			'library-separator': new jk.media.View({
				className:  'separator',
				priority:   100,
				attributes: {
					role: 'presentation'
	...
	embedContent: function() {
		var view = new jk.media.view.Embed({
			controller: this,
			model:      this.state()
		}).render();
	...
	editSelectionContent: function() {
		...
		view = new jk.media.view.AttachmentsBrowser({
			controller: this,
			...
			AttachmentView: jk.media.view.Attachments.EditSelection
		}).render();
	...
	editImageContent: function() {
		var image = this.state().get('image'),
			view = new jk.media.view.EditImage( { model: image, controller: this } ).render();
	...
	selectionStatusToolbar: function( view ) {
		...
		view.set( 'selection', new jk.media.view.Selection({
			controller: this,
	...
	mainInsertToolbar: function( view ) {
		...
		view.set( 'insert', {
			 ...
			 * @ignore
			 *
			 * @fires jk.media.controller.State#insert
			 */
			click: function() {
	...
	mainGalleryToolbar: function( view ) {
		...
		view.set( 'gallery', {
			...
			click: function() {
				...
				edit.set( 'library', new jk.media.model.Selection( models, {
	...
	mainEmbedToolbar: function( toolbar ) {
		toolbar.view = new jk.media.view.Toolbar.Embed({
			controller: this
		});
	},
		...
		this.toolbar.set( new jk.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires jk.media.controller.State#update
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state();
		...
		this.toolbar.set( new jk.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires jk.media.controller.State#reset
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state(),
...
var MediaFrame = jk.media.view.MediaFrame,
	l10n = jk.media.view.l10n,
	Select;

/**
 * jk.media.view.MediaFrame.Select
 *
 ...
 * @mixes jk.media.controller.StateMachine
 */
Select = MediaFrame.extend(/** @lends jk.media.view.MediaFrame.Select.prototype */{
	initialize: function() {
		// Call 'initialize' directly on the parent class.
	...
	createSelection: function() {
		...
		if ( ! (selection instanceof jk.media.model.Selection) ) {
			this.options.selection = new jk.media.model.Selection( selection, {
				multiple: this.options.multiple
			});
		}

		this._selection = {
			attachments: new jk.media.model.Attachments(),
			difference: []
		};
	},
	...
	createStates: function() {
		...
		this.states.add([
			// Main states.
			new jk.media.controller.Library({
				library:   jk.media.query( options.library ),
	 ...
	 *
	 * @param {jk.media.view.Router} routerView
	 */
	 ...
	 *
	 * @param {jk.media.controller.Region} contentRegion
	 */
	browseContent: function( contentRegion ) {
		...
		// Browse our library of attachments.
		contentRegion.view = new jk.media.view.AttachmentsBrowser({
			controller: this,
	...
	uploadContent: function() {
		this.$el.removeClass( 'hide-toolbar' );
		this.content.set( new jk.media.view.UploaderInline({
			controller: this
		}) );
	},
	 ...
	 * @param {Object} [options={}]
	 * @this jk.media.controller.Region
	 */
	createSelectToolbar: function( toolbar, options ) {
		options = options || this.options.button || {};
		options.controller = this;

		toolbar.view = new jk.media.view.Toolbar.Select( options );
	}
});
 ...
 * @augments Backbone.View
 */
var Heading = jk.media.View.extend( {
	tagName: function() {
 ...
 * @augments Backbone.View
 */
var Iframe = jk.media.View.extend(/** @lends jk.media.view.Iframe.prototype */{
	className: 'media-iframe',
	/**
	 * @return {jk.media.view.Iframe} Returns itself to allow chaining.
	 */
	render: function() {
		this.views.detach();
 ...
 * @augments Backbone.View
 */
ImageDetails = AttachmentDisplay.extend(/** @lends jk.media.view.ImageDetails.prototype */{
	className: 'image-details',
	template:  jk.template('image-details'),
 ...
 * @augments Backbone.View
 */
var Label = jk.media.View.extend(/** @lends jk.media.view.Label.prototype */{
	tagName: 'label',
...
var Frame = jk.media.view.Frame,
	l10n = jk.media.view.l10n,
	$ = jQuery,
	MediaFrame;

/**
 * jk.media.view.MediaFrame
 *
 ...
 * @mixes jk.media.controller.StateMachine
 */
MediaFrame = Frame.extend(/** @lends jk.media.view.MediaFrame.prototype */{
	className: 'media-frame',
	template:  jk.template('media-frame'),
	...
	initialize: function() {
		...
		this.$el.addClass('jk-core-ui');

		// Initialize modal container view.
		if ( this.options.modal ) {
			this.modal = new jk.media.view.Modal({
				controller: this,
		...
		if ( jk.Uploader.limitExceeded || ! jk.Uploader.browser.supported ) {
			this.options.uploader = false;
		}

		// Initialize window-wide uploader.
		if ( this.options.uploader ) {
			this.uploader = new jk.media.view.UploaderWindow({
				controller: this,
				uploader: {
	...
	/**
	 * @return {jk.media.view.MediaFrame} Returns itself to allow chaining.
	 */
	render: function() {
	...
	createMenu: function( menu ) {
		menu.view = new jk.media.view.Menu({
			controller: this,
	...
	createToolbar: function( toolbar ) {
		toolbar.view = new jk.media.view.Toolbar({
			controller: this
		});
	},
	/**
	...
	createRouter: function( router ) {
		router.view = new jk.media.view.Router({
			controller: this,
	...
	createIframeStates: function( options ) {
		var settings = jk.media.view.settings,
			tabs = settings.tabs,
	...
	iframeContent: function( content ) {
		...
		content.view = new jk.media.view.Iframe({
			controller: this
		});
	},
	...
	iframeMenu: function( view ) {
		...
		_.each( jk.media.view.settings.tabs, function( title, id ) {
			views[ 'iframe:' + id ] = {
 ...
 * @augments Backbone.View
 */
MenuItem = jk.media.View.extend(/** @lends jk.media.view.MenuItem.prototype */{
	tagName:   'button',
	...
	/**
	 * @return {jk.media.view.MenuItem} returns itself to allow chaining.
	 */
	render: function() {
		var options = this.options,
...
var MenuItem = jk.media.view.MenuItem,
	PriorityList = jk.media.view.PriorityList,
	Menu;
 ...
 *
 * @class
 * @augments jk.media.view.PriorityList
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
Menu = PriorityList.extend(/** @lends jk.media.view.Menu.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		...
		this.focusManager = new jk.media.view.FocusManager( {
			el:   this.el,
			mode: 'tabsNavigation'
		} );
 ...
 * @augments Backbone.View
 */
Modal = jk.media.View.extend(/** @lends jk.media.view.Modal.prototype */{
	tagName:  'div',
	template: jk.template('media-modal'),
	...
	/**
	 * @return {jk.media.view.Modal} Returns itself to allow chaining.
	 */
	attach: function() {
 ...
 * @augments Backbone.View
 */
var PriorityList = jk.media.View.extend(/** @lends jk.media.view.PriorityList.prototype */{
	tagName:   'div',
	...
	/**
	 * @param {string} id
	 * @param {jk.media.View|Object} view
	 * @param {Object} options
	 * @return {jk.media.view.PriorityList} Returns itself to allow chaining.
	 */
	...
	/**
	 * @param {string} id
	 * @return {jk.media.view.PriorityList}
	 */
	unset: function( id ) {
		var view = this.get( id );
	...
	toView: function( options ) {
		return new jk.media.View( options );
	}
});
...
/**
 * jk.media.view.RouterItem
 *
 * @memberOf jk.media.view
 *
 * @class
 * @augments jk.media.view.MenuItem
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
var RouterItem = jk.media.view.MenuItem.extend(/** @lends jk.media.view.RouterItem.prototype */{
	/**
...
var Menu = jk.media.view.Menu,
	Router;

/**
 * jk.media.view.Router
 *
 * @memberOf jk.media.view
 *
 * @class
 * @augments jk.media.view.Menu
 * @augments jk.media.view.PriorityList
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
Router = Menu.extend(/** @lends jk.media.view.Router.prototype */{
	tagName:   'div',
	className: 'media-router',
	property:  'contentMode',
	ItemView:  jk.media.view.RouterItem,
	region:    'router',
 ...
 * @augments Backbone.View
 */
Search = jk.media.View.extend(/** @lends jk.media.view.Search.prototype */{
	tagName:   'input',
	className: 'search',
	...
	/**
	 * @return {jk.media.view.Search} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
Selection = jk.media.View.extend(/** @lends jk.media.view.Selection.prototype */{
	tagName:   'div',
	className: 'media-selection',
	template:  jk.template('media-selection'),
	...
	initialize: function() {
		...
		/**
		 * @member {jk.media.view.Attachments.Selection}
		 */
		this.attachments = new jk.media.view.Attachments.Selection({
			controller: this.controller,
			collection: this.collection,
 ...
 * @augments Backbone.View
 */
Settings = View.extend(/** @lends jk.media.view.Settings.prototype */{
	events: {
	...
	/**
	 * @return {jk.media.view.Settings} Returns itself to allow chaining.
	 */
	render: function() {
		View.prototype.render.apply( this, arguments );
...
/**
 * jk.media.view.Settings.AttachmentDisplay
 *
 * @memberOf jk.media.view.Settings
 *
 * @class
 * @augments jk.media.view.Settings
 * @augments jk.media.View
 ...
 * @augments Backbone.View
 */
AttachmentDisplay = Settings.extend(/** @lends jk.media.view.Settings.AttachmentDisplay.prototype */{
	className: 'attachment-display-settings',
	template:  jk.template('attachment-display-settings'),
	...
	/**
	 * @return {jk.media.view.AttachmentDisplay} Returns itself to allow chaining.
	 */
	render: function() {
		var attachment = this.options.attachment;
		if ( attachment ) {
...
/**
 * jk.media.view.Settings.Gallery
 *
 * @memberOf jk.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Gallery = jk.media.view.Settings.extend(/** @lends jk.media.view.Settings.Gallery.prototype */{
	className: 'collection-settings gallery-settings',
	template:  jk.template('gallery-settings')
});
...
/**
 * jk.media.view.Settings.Playlist
 *
 * @memberOf jk.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Playlist = jk.media.view.Settings.extend(/** @lends jk.media.view.Settings.Playlist.prototype */{
	className: 'collection-settings playlist-settings',
	template:  jk.template('playlist-settings')
});
 ...
 * @augments Backbone.View
 */
var Sidebar = jk.media.view.PriorityList.extend(/** @lends jk.media.view.Sidebar.prototype */{
...
/**
 * jk.media.view.SiteIconCropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop a Site Icon.
 *
 * Takes imgAreaSelect options from
 * jk.customize.SiteIconControl.calculateImageSelectOptions.
 *
 * @memberOf jk.media.view
 *
 * @class
 * @augments jk.media.view.Cropper
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
SiteIconCropper = View.Cropper.extend(/** @lends jk.media.view.SiteIconCropper.prototype */{
	...
	addSidebar: function() {
		this.sidebar = new jk.media.view.Sidebar({
			controller: this.controller
		});

		this.sidebar.set( 'preview', new jk.media.view.SiteIconPreview({
			controller: this.controller,
			attachment: this.options.attachment
		}) );
 ...
 * @augments Backbone.View
 */
SiteIconPreview = View.extend(/** @lends jk.media.view.SiteIconPreview.prototype */{
	className: 'site-icon-preview-crop-modal',
	template: jk.template( 'site-icon-preview-crop' ),
 ...
 * @augments Backbone.View
 */
var Spinner = jk.media.View.extend(/** @lends jk.media.view.Spinner.prototype */{
	tagName:   'span',
	className: 'spinner',
	 ...
	 * @since 3.9.0
	 *
	 * @return {jk.media.view.Spinner} The spinner.
	 */
	show: function() {
 ...
 * @augments Backbone.View
 */
Toolbar = View.extend(/** @lends jk.media.view.Toolbar.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		...
		this.primary   = new jk.media.view.PriorityList();
		this.secondary = new jk.media.view.PriorityList();
		this.tertiary  = new jk.media.view.PriorityList();
	...
	/**
	 * @return {jk.media.view.Toolbar} Returns itself to allow chaining
	 */
	dispose: function() {
		if ( this.selection ) {
	...
	set: function( id, view, options ) {
		...
		if ( _.isObject( id ) ) {
			...
			if ( ! ( view instanceof Backbone.View ) ) {
				...
				view = new jk.media.view.Button( view ).render();
			}
...
var Select = jk.media.view.Toolbar.Select,
	l10n = jk.media.view.l10n,
	Embed;

/**
 * jk.media.view.Toolbar.Embed
 *
 * @memberOf jk.media.view.Toolbar
 *
 * @class
 * @augments jk.media.view.Toolbar.Select
 * @augments jk.media.view.Toolbar
 * @augments jk.media.View
 * @augments jk.Backbone.View
 * @augments Backbone.View
 */
Embed = Select.extend(/** @lends jk.media.view.Toolbar.Embed.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			text: l10n.insertIntoPost,
			requires: false
...
var Toolbar = jk.media.view.Toolbar,
	l10n = jk.media.view.l10n,
	Select;

/**
 * jk.media.view.Toolbar.Select
 *
 * @memberOf jk.media.view.Toolbar
 *
 * @class
 ...
 * @augments Backbone.View
 */
Select = Toolbar.extend(/** @lends jk.media.view.Toolbar.Select.prototype */{
	initialize: function() {
		var options = this.options;
...
/**
 * Creates a dropzone on JK editor instances (elements with .jk-editor-wrap)
 * and relays drag'n'dropped files to a media workflow.
 *
 * jk.media.view.EditorUploader
 *
 * @memberOf jk.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
EditorUploader = View.extend(/** @lends jk.media.view.EditorUploader.prototype */{
	tagName:   'div',
	className: 'uploader-editor',
	template:  jk.template( 'uploader-editor' ),
	...
	render: function() {
		...
		View.prototype.render.apply( this, arguments );
		$( '.jk-editor-wrap' ).each( _.bind( this.attach, this ) );
		return this;
	},
	...
	drop: function( event ) {
		...
		$wrap = $( event.target ).parents( '.jk-editor-wrap' );
		if ( $wrap.length > 0 && $wrap[0].id ) {
			window.jkActiveEditor = $wrap[0].id.slice( 3, -5 );
		}

		if ( ! this.workflow ) {
			this.workflow = jk.media.editor.open( window.jkActiveEditor, {
				frame:    'post',
				state:    'insert',
				title:    l10n.addMedia,
				multiple: true
 ...
 * @augments Backbone.View
 */
UploaderInline = View.extend(/** @lends jk.media.view.UploaderInline.prototype */{
	tagName:   'div',
	className: 'uploader-inline',
	template:  jk.template('uploader-inline'),
	...
	initialize: function() {
		...
		if ( _.isUndefined( this.options.postId ) ) {
			this.options.postId = jk.media.view.settings.post.id;
		}

		if ( this.options.status ) {
			this.views.set( '.upload-inline-status', new jk.media.view.UploaderStatus({
				controller: this.controller
			}) );
		}
	},
	...
	/**
	 * @return {jk.media.view.UploaderInline} Returns itself to allow chaining.
	 */
	dispose: function() {
		if ( this.disposing ) {
			/**
	...
	/**
	 * @return {jk.media.view.UploaderInline}
	 */
	ready: function() {
...
/**
 * jk.media.view.UploaderStatusError
 *
 * @memberOf jk.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
var UploaderStatusError = jk.media.View.extend(/** @lends jk.media.view.UploaderStatusError.prototype */{
	className: 'upload-error',
	template:  jk.template('uploader-status-error')
});
 ...
 * @augments Backbone.View
 */
UploaderStatus = View.extend(/** @lends jk.media.view.UploaderStatus.prototype */{
	className: 'media-uploader-status',
	template:  jk.template('uploader-status'),
	...
	initialize: function() {
		this.queue = jk.Uploader.queue;
		this.queue.on( 'add remove reset', this.visibility, this );
		this.queue.on( 'add remove reset change:percent', this.progress, this );
		this.queue.on( 'add remove reset change:uploading', this.info, this );

		this.errors = jk.Uploader.errors;
		this.errors.reset();
		this.errors.on( 'add remove reset', this.visibility, this );
		this.errors.on( 'add', this.error, this );
	},
	/**
	 * @return {jk.media.view.UploaderStatus}
	 */
	dispose: function() {
		jk.Uploader.queue.off( null, null, this );
		/**
	...
	error: function( error ) {
		var statusError = new jk.media.view.UploaderStatusError( {
		...
		_.delay( function() {
			...
			jk.a11y.speak( error.get( 'message' ), 'assertive' );
		}, 1000 );
	},
 ...
 */
UploaderWindow = jk.media.View.extend(/** @lends jk.media.view.UploaderWindow.prototype */{
	tagName:   'div',
	className: 'uploader-window',
	template:  jk.template('uploader-window'),
	...
	ready: function() {
		var postId = jk.media.view.settings.post.id,
			dropzone;
		...
		this.uploader = new jk.Uploader( this.options.uploader );
	...
	hide: function() {
		...
		jk.media.transition( $el ).done( function() {
 ...
 * @augments Backbone.View
 */
var View = jk.Backbone.View.extend(/** @lends jk.media.View.prototype */{
	constructor: function( options ) {
		if ( options && options.controller ) {
			this.controller = options.controller;
		}
		jk.Backbone.View.apply( this, arguments );
	},
	/**
	 ...
	 *
	 * @return {jk.media.View} Returns itself to allow chaining.
	 */
	dispose: function() {
		/*
		 * Undelegating events, removing events from the model, and
	...
	remove: function() {
		 ...
		 * call 'remove' directly on the parent class
		 */
		return jk.Backbone.View.prototype.remove.apply( this, arguments );
	}
});
...
/************************************************************************/
/**
 * @output jk-includes/js/media-views.js
 */
...
l10n = media.view.l10n = window._jkMediaViewsL10n || {};
