/**
 * @output jk-includes/js/customize-preview-nav-menus.js
 */

/* global _jkCustomizePreviewNavMenusExports */

/** @namespace jk.customize.navMenusPreview */
jk.customize.navMenusPreview = jk.customize.MenusCustomizerPreview = ( function( $, _, jk, api ) {
	'use strict';
	...
	if ( 'undefined' !== typeof _jkCustomizePreviewNavMenusExports ) {
		_.extend( self.data, _jkCustomizePreviewNavMenusExports );
	}
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				 ...
				 * being exported to the frontend by the _is_valid_nav_menu_item filter in jk_get_nav_menu_items(),
			...
			/*
			 * Ensure that jk_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of jk_nav_menu().
		 *
		 * @memberOf jk.customize.navMenusPreview
		 * @alias jk.customize.navMenusPreview.NavMenuInstancePartial
		 *
		 * @class
		 * @augments jk.customize.selectiveRefresh.Partial
		 * @since 4.5.0
		 */
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(/** @lends jk.customize.navMenusPreview.NavMenuInstancePartial.prototype */{
			 ...
			 *
			 * @since 4.5.0
			 * @param {jk.customize.Value|string} setting  - Object or ID.
			 ...
			 *
			 * @inheritdoc
			 * @param {jk.customize.selectiveRefresh.Placement} placement
			 */
			renderContent: function( placement ) {
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-menu-refreshed', [ {
						...
						jkNavArgs: placement.context, // @deprecated
						jkNavMenuArgs: placement.context,
		 ...
		 * @since 4.5.0
		 *
		 * @param {jk.customize.Value} setting
		 * @param {Object}             [options]
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no jk_nav_menu() instances that refer to the theme location, do full refresh.
