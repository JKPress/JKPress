/**
 * @output jk-includes/js/customize-selective-refresh.js
 */

/* global jQuery, JSON, _customizePartialRefreshExports, console */

/** @namespace jk.customize.selectiveRefresh */
jk.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @memberOf jk.customize.selectiveRefresh
	 *
	 * @see PHP class JK_Customize_Partial.
	 *
	 * @class
	 * @augments jk.customize.Class
	 * @since 4.5.0
	 */
	Partial = self.Partial = api.Class.extend(/** @lends jk.customize.SelectiveRefresh.Partial.prototype */{
		 ...
		 * @since 4.5.0
		 *
		 * @param {jk.customize.Value|string} setting  ID or object for setting.
		 ...
		 * @since 4.5.0
		 *
		 * @this {jk.customize.selectiveRefresh.Partial}
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( jk.emoji && jk.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = jk.emoji.parse( content );
				}
			...
			/*
			 * Note that the 'jk_audio_shortcode_library' and 'jk_video_shortcode_library' filters
			 * will determine whether or not jk.mediaelement is loaded and whether it will
	 ...
	 * @since 4.5.0
	 */
	self.Placement = Placement = api.Class.extend(/** @lends jk.customize.selectiveRefresh.prototype */{

		/**
		 * The partial with which the container is associated.
		 *
		 * @param {jk.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	 ...
	 * @since 4.5.0
	 *
	 * @type {Object.<string, jk.customize.selectiveRefresh.Partial>}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @see jk.customize.previewer.query()
	 *
	 * @return {Object}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @type {Object<string, { deferred: jQuery.Promise, partial: jk.customize.selectiveRefresh.Partial }>}
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param {jk.customize.selectiveRefresh.Partial} partial
	 * @return {jQuery.Promise}
	 */
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = jk.ajax.send( null, {
					data: data,
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to jk.customize
		 * will only include the changed setting object as an argument, not including the
		 * new value or the old value.
		 *
		 * @since 4.5.0
		 * @this {jk.customize.Setting}
		 *
		 ...
		 *
		 * @since 4.5.0
		 * @this {jk.customize.Values}
		 *
		 * @param {jk.customize.Setting} setting
		 */
