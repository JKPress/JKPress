/**
 * @output jk-includes/js/jk-pointer.js
 */

/**
 * Initializes the jk-pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;

	$.widget('jk.pointer',/** @lends $.widget.jk.pointer.prototype */{
		options: {
			pointerClass: 'jk-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var button = $('<a class="close" href="#"></a>').text( jk.i18n.__( 'Dismiss' ) );
		...
		/**
		 * A class that represents a JKPress pointer.
		 *
		 * @since 3.3.0
		 * @private
		 *
		 * @constructs $.widget.jk.pointer
		 */
		_create: function() {
			var positioning,
				family;

			this.content = $('<div class="jk-pointer-content"></div>');
			this.arrow   = $('<div class="jk-pointer-arrow"><div class="jk-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="jk-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /jk-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'jk-pointer-' + edge );
		},
