/**
 * @output jk-includes/js/jk-custom-header.js
 */
...
(function( window, settings ) {
			...
			if ( this.supportsVideo() ) {
				for ( var id in this.handlers ) {
					...
					if ( 'test' in handler && handler.test( settings ) ) {
						...
						trigger( document, 'jk-custom-header-video-loaded' );
						break;
					}
				}
			}
	...
	BaseHandler.prototype = {
		...
		initialize: function( settings ) {
			...
			this.container = document.getElementById( 'jk-custom-header' );
			this.button = button;

			button.setAttribute( 'type', 'button' );
			button.setAttribute( 'id', 'jk-custom-header-video-button' );
			button.setAttribute( 'class', 'jk-custom-header-video-button jk-custom-header-video-play' );
			...
			this.container.addEventListener( 'play', function() {
				button.className = 'jk-custom-header-video-button jk-custom-header-video-play';
				button.innerHTML = settings.l10n.pause;
				if ( 'a11y' in window.jk ) {
					window.jk.a11y.speak( settings.l10n.playSpeak);
				}
			});

			this.container.addEventListener( 'pause', function() {
				button.className = 'jk-custom-header-video-button jk-custom-header-video-pause';
				button.innerHTML = settings.l10n.play;
				if ( 'a11y' in window.jk ) {
					window.jk.a11y.speak( settings.l10n.pauseSpeak);
				}
			});
	 ...
	 * @class
	 */
	NativeHandler = BaseHandler.extend(/** @lends jk.NativeHandler.prototype */{
		/**
		...
		ready: function() {
			...
			video.id = 'jk-custom-header-video';
			video.autoplay = true;
			video.loop = true;
			video.muted = true;
	 ...
	 * @memberOf jk
	 *
	 * @class jk.YouTubeHandler
	 */
	YouTubeHandler = BaseHandler.extend(/** @lends jk.YouTubeHandler.prototype */{
		/**
	...
	window.jk.customHeader = new CustomHeader();
	document.addEventListener( 'DOMContentLoaded', window.jk.customHeader.initialize.bind( window.jk.customHeader ), false );

	// Selective refresh support in the Customizer.
	if ( 'customize' in window.jk ) {
		window.jk.customize.selectiveRefresh.bind( 'render-partials-response', function( response ) {
			if ( 'custom_header_settings' in response ) {
				settings = response.custom_header_settings;
			}
		});

		window.jk.customize.selectiveRefresh.bind( 'partial-content-rendered', function( placement ) {
			if ( 'custom_header' === placement.partial.id ) {
				window.jk.customHeader.initialize();
			}
		});
	}

})( window, window._jkCustomHeaderSettings || {} );
