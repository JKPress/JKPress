 ...
 * Quicktags
 *
 * This is the HTML editor in JKPress. It can be attached to any textarea and will
 ...
 *
 * @output jk-includes/js/quicktags.js
 */
...
/* global adminpage, jkActiveEditor, quicktagsL10n, jkLink, prompt, edButtons */
...
(function(){
	...
	var qt = window.QTags = function(settings) {
		...
		wrap = document.getElementById( 'jk-' + id + '-wrap' );
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'jk_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(jkActiveEditor), event;
	...
	qt.Button.prototype.html = function(idPrefix) {
		var active, on, jk,
			...
			dfw = ( jk = window.jk ) && jk.editor && jk.editor.dfw;
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( jk = window.jk ) || ! jk.editor || ! jk.editor.dfw ) {
			return;
		}

		window.jk.editor.dfw.toggle();
	};
