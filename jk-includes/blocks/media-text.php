...
function render_block_core_media_text( $attributes, $content ) {
	...
	$unique_id          = 'jk-block-media-text__media-' . jk_unique_id();

	$block_tag_processor = new JK_HTML_Tag_Processor( $content );
	$block_query         = array(
		'tag_name'   => 'div',
		'class_name' => 'jk-block-media-text',
	);
	...
	$media_tag_processor   = new JK_HTML_Tag_Processor( $content );
	$wrapping_figure_query = array(
		'tag_name'   => 'figure',
		'class_name' => 'jk-block-media-text__media',
	);
	...
	$image_tag       = '<img class="jk-block-media-text__featured_image">';
	$content         = preg_replace(
		'/(<figure\s+id="' . preg_quote( $unique_id, '/' ) . '"\s+class="jk-block-media-text__media"\s*>)/',
		'$1' . $image_tag,
		$content
	);

	$image_tag_processor = new JK_HTML_Tag_Processor( $content );
	if ( $image_tag_processor->next_tag(
		...
		if ( $image_tag_processor->next_tag(
			array(
				'tag_name'   => 'img',
				'class_name' => 'jk-block-media-text__featured_image',
			)
		) ) {
			$image_tag_processor->set_attribute( 'src', esc_url( $current_featured_image ) );
			$image_tag_processor->set_attribute( 'class', 'jk-image-' . get_post_thumbnail_id() . ' size-' . $media_size_slug );
			$image_tag_processor->set_attribute( 'alt', trim( strip_tags( get_post_meta( get_post_thumbnail_id(), '_jk_attachment_image_alt', true ) ) ) );
			if ( $image_fill ) {
