...
function render_block_core_query_pagination_next( $attributes, $content, $block ) {
	...
	if ( isset( $block->context['query']['inherit'] ) && $block->context['query']['inherit'] ) {
		...
		global $jk_query;
		if ( $max_page > $jk_query->max_num_pages ) {
			$max_page = $jk_query->max_num_pages;
		}
		$content = get_next_posts_link( $label, $max_page );
		remove_filter( 'next_posts_link_attributes', $filter_link_attributes );
	} elseif ( ! $max_page || $max_page > $page ) {
		$custom_query           = new JK_Query( build_query_vars_from_query_block( $block, $page ) );
	...
	if ( $enhanced_pagination && isset( $content ) ) {
		...
		if ( $p->next_tag(
			array(
				'tag_name'   => 'a',
				'class_name' => 'jk-block-query-pagination-next',
			)
		) ) {
			$p->set_attribute( 'data-jk-key', 'query-pagination-next' );
			$p->set_attribute( 'data-jk-on--click', 'core/query::actions.navigate' );
			$p->set_attribute( 'data-jk-on-async--mouseenter', 'core/query::actions.prefetch' );
			$p->set_attribute( 'data-jk-watch', 'core/query::callbacks.prefetch' );
