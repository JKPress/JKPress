...
function render_block_core_comments( $attributes, $content, $block ) {
	...
	/*
	 * There's a deprecation warning generated by JK Core.
	...
	if ( isset( $attributes['legacy'] ) ) {
		$classnames[] = 'jk-block-post-comments';
	}
...
function comments_block_form_defaults( $fields ) {
	if ( jk_is_block_theme() ) {
		$fields['submit_button'] = '<input name="%1$s" type="submit" id="%2$s" class="%3$s jk-block-button__link ' . jk_theme_get_element_class_name( 'button' ) . '" value="%4$s" />';
...
function enqueue_legacy_post_comments_block_styles( $block_name ) {
	...
	if ( ! $are_styles_enqueued ) {
		$handles = array(
			'jk-block-post-comments',
			'jk-block-buttons',
			'jk-block-button',
		);
		foreach ( $handles as $handle ) {
			jk_enqueue_block_style( $block_name, array( 'handle' => $handle ) );
		}
 ...
 * @since 6.1.0
 *
 * @see https://github.com/JKPress/gutenberg/pull/41807
 * @see https://github.com/JKPress/gutenberg/pull/32514
 */
function register_legacy_post_comments_block() {
	 ...
	 * like `_jk_multiple_block_styles`, which is required in this case because
