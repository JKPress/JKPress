...
function render_block_core_post_terms( $attributes, $content, $block ) {
	...
	if ( is_jk_error( $post_terms ) || empty( $post_terms ) ) {
		return '';
	}
	...
	if ( isset( $attributes['prefix'] ) && $attributes['prefix'] ) {
		$prefix .= '<span class="jk-block-post-terms__prefix">' . $attributes['prefix'] . '</span>';
	}

	$suffix = '</div>';
	if ( isset( $attributes['suffix'] ) && $attributes['suffix'] ) {
		$suffix = '<span class="jk-block-post-terms__suffix">' . $attributes['suffix'] . '</span>' . $suffix;
	}
	...
	return get_the_term_list(
		...
		$attributes['term'],
		jk_kses_post( $prefix ),
		'<span class="jk-block-post-terms__separator">' . esc_html( $separator ) . '</span>',
		jk_kses_post( $suffix )
	);
}
