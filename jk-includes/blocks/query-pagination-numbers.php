...
function render_block_core_query_pagination_numbers( $attributes, $content, $block ) {
	...
	if ( isset( $block->context['query']['inherit'] ) && $block->context['query']['inherit'] ) {
		...
		$total         = ! $max_page || $max_page > $jk_query->max_num_pages ? $jk_query->max_num_pages : $max_page;
		$paginate_args = array(
			'prev_next' => false,
			'total'     => $total,
		);
		...
		$block_query = new JK_Query( build_query_vars_from_query_block( $block, $page ) );
		// `paginate_links` works with the global $jk_query, so we have to
		// temporarily switch it with our custom query.
		$prev_jk_query = $jk_query;
		$jk_query      = $block_query;
		$total         = ! $max_page || $max_page > $jk_query->max_num_pages ? $jk_query->max_num_pages : $max_page;
		$paginate_args = array(
			'base'      => '%_%',
			'format'    => "?$page_key=%#%",
		...
		if ( 1 !== $page ) {
			 ...
			 *
			 * TODO: After two JK versions (starting from the JK version the core patch landed),
		...
		$content = paginate_links( $paginate_args );
		jk_reset_postdata(); // Restore original Post Data.
		$jk_query = $prev_jk_query;
	}
	...
	if ( $enhanced_pagination ) {
		...
		while ( $p->next_tag(
			...
			if ( null === $p->get_attribute( 'data-jk-key' ) ) {
				$p->set_attribute( 'data-jk-key', 'index-' . $tag_index++ );
			}
			if ( 'A' === $p->get_tag() ) {
