...
function render_block_core_template_part( $attributes ) {
	...
	if ( isset( $attributes['slug'] ) && get_stylesheet() === $theme ) {
		...
		$template_part_query = new JK_Query(
			array(
				'post_type'           => 'jk_template_part',
				'post_status'         => 'publish',
		...
		if ( $template_part_post ) {
			 ...
			 * @param array   $attributes         The block attributes.
			 * @param JK_Post $template_part_post The template part post object.
			...
			if ( 0 === validate_file( $attributes['slug'] ) ) {
				$block_template = get_block_file_template( $template_part_id, 'jk_template_part' );
				...
				$block_template_file = _get_block_template_file( 'jk_template_part', $attributes['slug'] );
	...
	// JK_DEBUG_DISPLAY must only be honored when JK_DEBUG. This precedent
	// is set in `jk_debug_mode()`.
	$is_debug = JK_DEBUG && JK_DEBUG_DISPLAY;
	...
	$content = jk_filter_content_tags( $content, "template_part_{$area}" );
