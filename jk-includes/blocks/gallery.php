...
function block_core_gallery_render( $attributes, $content ) {
	// Adds a style tag for the --jk--style--unstable-gallery-gap var.
	...
	if ( is_array( $gap ) ) {
		foreach ( $gap as $key => $value ) {
			...
			if ( is_string( $value ) && str_contains( $value, 'var:preset|spacing|' ) ) {
				...
				$slug            = _jk_to_kebab_case( substr( $value, $index_to_splice ) );
				$value           = "var(--jk--preset--spacing--$slug)";
			}
		...
		if ( is_string( $gap ) && str_contains( $gap, 'var:preset|spacing|' ) ) {
			...
			$slug            = _jk_to_kebab_case( substr( $gap, $index_to_splice ) );
			$gap             = "var(--jk--preset--spacing--$slug)";
		}
	}

	$unique_gallery_classname = jk_unique_id( 'jk-block-gallery-' );
	$processed_content        = new JK_HTML_Tag_Processor( $content );
	$processed_content->next_tag();
	$processed_content->add_class( $unique_gallery_classname );

	// --gallery-block--gutter-size is deprecated. --jk--style--gallery-gap-default should be used by themes that want to set a default
	// gap on the gallery.
	$fallback_gap = 'var( --jk--style--gallery-gap-default, var( --gallery-block--gutter-size, var( --jk--style--block-gap, 0.5em ) ) )';
	...
	$gallery_styles = array(
		array(
			'selector'     => ".jk-block-gallery.{$unique_gallery_classname}",
			'declarations' => array(
				'--jk--style--unstable-gallery-gap' => $gap_column,
				'gap'                               => $gap_value,
			),
		),
	);
	...
	// The JK_HTML_Tag_Processor class calls get_updated_html() internally
	 ...
	 *
	 * @see: https://github.com/JKPress/gutenberg/pull/58733
	 */
	if ( empty( $attributes['randomOrder'] ) ) {
		return $updated_content;
	}

	// This pattern matches figure elements with the `jk-block-image` class to
