 ...
 */
function get_jk_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
	 ...
	 * @param string $deprecated Unused.
	 */
	return apply_filters( 'get_jk_title_rss', jk_get_document_title(), $deprecated );
}
 ...
 * @param string $deprecated Unused.
 */
function jk_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
		/* translators: %s: 'document_title_separator' filter name. */
		_deprecated_argument( __FUNCTION__, '4.4.0', sprintf( __( 'Use the %s filter instead.' ), '<code>document_title_separator</code>' ) );
	}
	 ...
	 * @since 4.4.0 The `$sep` parameter was deprecated and renamed to `$deprecated`.
	 *
	 * @see get_jk_title_rss()
	 *
	 * @param string $jk_title_rss The current blog title.
	 * @param string $deprecated   Unused.
	 */
	echo apply_filters( 'jk_title_rss', get_jk_title_rss(), $deprecated );
}
 ...
 * @since 2.5.0
 *
 * @param int|JK_Comment $comment_id Optional comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * @param int|JK_Comment $comment Optional. Comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * In the case of JKPress, text is defined as containing no markup,
...
function prep_atom_text_construct( $data ) {
	...
	if ( ! function_exists( 'xml_parser_create' ) ) {
		jk_trigger_error( '', __( "PHP's XML extension is not available. Please contact your hosting provider to enable PHP's XML extension." ) );
...
function get_self_link() {
	...
	return set_url_scheme( 'http://' . $domain . jk_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
/**
 * Gets the UTC time of the most recently modified post from JK_Query.
 *
...
function get_feed_build_date( $format ) {
	...
	if ( ! empty( $jk_query ) && $jk_query->have_posts() ) {
		// Extract the post modified times from the posts.
		$modified_times = jk_list_pluck( $jk_query->posts, 'post_modified_gmt' );

		// If this is a comment feed, check those objects too.
		if ( $jk_query->is_comment_feed() && $jk_query->comment_count ) {
			// Extract the comment modified times from the comments.
			$comment_times = jk_list_pluck( $jk_query->comments, 'comment_date_gmt' );
 ...
 * @return SimplePie\SimplePie|JK_Error SimplePie object on success or JK_Error object on failure.
 */
function fetch_feed( $url ) {
	...
	$feed->set_sanitize_class( 'JK_SimplePie_Sanitize_KSES' );
	/*
	 * We must manually overwrite $feed->sanitize because SimplePie's constructor
	 * sets it before we have a chance to set the sanitization class.
	 */
	$feed->sanitize = new JK_SimplePie_Sanitize_KSES();

	// Register the cache handler using the recommended method for SimplePie 1.3 or later.
	if ( method_exists( 'SimplePie_Cache', 'register' ) ) {
		SimplePie_Cache::register( 'jk_transient', 'JK_Feed_Cache_Transient' );
		$feed->set_cache_location( 'jk_transient' );
	} else {
		// Back-compat for SimplePie 1.2.x.
		require_once ABSPATH . JKINC . '/class-jk-feed-cache.php';
		$feed->set_cache_class( 'JK_Feed_Cache' );
	}

	$feed->set_file_class( 'JK_SimplePie_File' );

	$feed->set_feed_url( $url );
	/** This filter is documented in jk-includes/class-jk-feed-cache-transient.php */
	$feed->set_cache_duration( apply_filters( 'jk_feed_cache_transient_lifetime', 12 * HOUR_IN_SECONDS, $url ) );
	 ...
	 */
	do_action_ref_array( 'jk_feed_options', array( &$feed, $url ) );

	$feed->init();
	$feed->set_output_encoding( get_bloginfo( 'charset' ) );

	if ( $feed->error() ) {
		return new JK_Error( 'simplepie-error', $feed->error() );
	}
