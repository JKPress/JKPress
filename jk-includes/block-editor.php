 ...
 *
 * @param JK_Post|JK_Block_Editor_Context $post_or_block_editor_context The current post object or
...
function get_block_categories( $post_or_block_editor_context ) {
	...
	$block_editor_context = $post_or_block_editor_context instanceof JK_Post ?
		new JK_Block_Editor_Context(
			array(
	 ...
	 * @param JK_Block_Editor_Context $block_editor_context The current block editor context.
	 */
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array[] $block_categories Array of categories for block types.
		 * @param JK_Post $post             Post being loaded.
		 */
...
function get_allowed_block_types( $block_editor_context ) {
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 * @param bool|string[] $allowed_block_types Array of block type slugs, or boolean to enable/disable all.
		 *                                           Default true (all registered block types supported)
		 * @param JK_Post       $post                The post resource data.
		 */
...
function get_default_block_editor_settings() {
	...
	// jk_max_upload_size() can be expensive, so only call it when relevant for the current user.
	$max_upload_size = 0;
	...
	$all_sizes        = jk_get_registered_image_subsizes();
	...
	$default_editor_styles_file = ABSPATH . JKINC . '/css/dist/block-editor/default-editor-styles.css';
 ...
 * @access private
 *
 * @global JK_Styles  $jk_styles  The JK_Styles current instance.
 * @global JK_Scripts $jk_scripts The JK_Scripts current instance.
 *
 * @return array {
 ...
 * }
 */
function _jk_get_iframed_editor_assets() {
	global $jk_styles, $jk_scripts;

	// Keep track of the styles and scripts instance to restore later.
	$current_jk_styles  = $jk_styles;
	$current_jk_scripts = $jk_scripts;
	 ...
	 */
	$jk_styles->registered  = $current_jk_styles->registered;
	$jk_scripts->registered = $current_jk_scripts->registered;
	 ...
	 */
	$jk_styles->done =
		jk_theme_has_theme_json() ? array( 'jk-reset-editor-styles' ) : array();

	jk_enqueue_script( 'jk-polyfill' );
	// Enqueue the `editorStyle` handles for all core block, and dependencies.
	jk_enqueue_style( 'jk-edit-blocks' );

	if ( current_theme_supports( 'jk-block-styles' ) ) {
	 ...
	 */
	$has_emoji_styles = has_action( 'jk_print_styles', 'print_emoji_styles' );
	if ( $has_emoji_styles ) {
		remove_action( 'jk_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	jk_print_styles();
	jk_print_font_faces();
	jk_print_font_faces_from_style_variations();
	$styles = ob_get_clean();

	if ( $has_emoji_styles ) {
		add_action( 'jk_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	jk_print_head_scripts();
	jk_print_footer_scripts();
	$scripts = ob_get_clean();

	// Restore the original instances.
	$jk_styles  = $current_jk_styles;
	$jk_scripts = $current_jk_scripts;
 ...
 */
function jk_get_first_block( $blocks, $block_name ) {
	foreach ( $blocks as $block ) {
		if ( $block_name === $block['blockName'] ) {
			return $block;
		}
		if ( ! empty( $block['innerBlocks'] ) ) {
			$found_block = jk_get_first_block( $block['innerBlocks'], $block_name );
 ...
 */
function jk_get_post_content_block_attributes() {
	global $post_ID;
	...
	if ( ! empty( $current_template ) ) {
		...
		$post_content_block = jk_get_first_block( $template_blocks, 'core/post-content' );
...
function get_block_editor_settings( array $custom_settings, $block_editor_context ) {
	...
	foreach ( $presets as $preset_style ) {
		$actual_css = jk_get_global_stylesheet( array( $preset_style['css'] ) );
		if ( '' !== $actual_css ) {
	...
	if ( jk_theme_has_theme_json() ) {
		...
		$actual_css    = jk_get_global_stylesheet( array( $block_classes['css'] ) );
		if ( '' !== $actual_css ) {
		 ...
		 */
		$global_styles[] = array(
			'css'            => jk_get_global_stylesheet( array( 'custom-css' ) ),
			'__unstableType' => 'user',
			'isGlobalStyles' => true,
		);
	} else {
	...
	$editor_settings['__experimentalFeatures'] = jk_get_global_settings();
	...
	$editor_settings['__unstableResolvedAssets']         = _jk_get_iframed_editor_assets();
	$editor_settings['__unstableIsBlockBasedTheme']      = jk_is_block_theme();
	...
	$post_content_block_attributes = jk_get_post_content_block_attributes();
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array   $editor_settings Default editor settings.
		 * @param JK_Post $post            Post being edited.
		 */
...
function block_editor_rest_api_preload( array $preload_paths, $block_editor_context ) {
	global $post, $jk_scripts, $jk_styles;
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param (string|string[])[] $preload_paths Array of paths to preload.
		 * @param JK_Post             $selected_post Post being edited.
		 */
	...
	/*
	 * Ensure the global $post, $jk_scripts, and $jk_styles remain the same after
	...
	$backup_jk_scripts  = ! empty( $jk_scripts ) ? clone $jk_scripts : $jk_scripts;
	$backup_jk_styles   = ! empty( $jk_styles ) ? clone $jk_styles : $jk_styles;
	...
	// Restore the global $post, $jk_scripts, and $jk_styles as they were before API preloading.
	$post       = $backup_global_post;
	$jk_scripts = $backup_jk_scripts;
	$jk_styles  = $backup_jk_styles;

	jk_add_inline_script(
		'jk-api-fetch',
		sprintf(
			'jk.apiFetch.use( jk.apiFetch.createPreloadingMiddleware( %s ) );',
	...
	if ( $editor_styles && current_theme_supports( 'editor-styles' ) ) {
		foreach ( $editor_styles as $style ) {
			if ( preg_match( '~^(https?:)?//~', $style ) ) {
				$response = jk_remote_get( $style );
				if ( ! is_jk_error( $response ) ) {
					$styles[] = array(
						'css'            => jk_remote_retrieve_body( $response ),
						'__unstableType' => 'theme',
