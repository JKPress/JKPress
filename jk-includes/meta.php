 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various JKPress object types. Metadata
...
require ABSPATH . JKINC . '/class-jk-metadata-lazyloader.php';
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	$mid = (int) $jkdb->insert_id;

	jk_cache_delete( $object_id, $meta_type . '_meta' );
...
function update_metadata( $meta_type, $object_id, $meta_key, $meta_value, $prev_value = '' ) {
	...
	$meta_ids = $jkdb->get_col( $jkdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $jkdb->update( $table, $data, $where );
	if ( ! $result ) {
		return false;
	}
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	$query = $jkdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( ! $delete_all ) {
		$query .= $jkdb->prepare( " AND $type_column = %d", $object_id );
	}

	if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
		$query .= $jkdb->prepare( ' AND meta_value = %s', $meta_value );
	}

	$meta_ids = $jkdb->get_col( $query );
	if ( ! count( $meta_ids ) ) {
		return false;
	}
	...
	if ( $delete_all ) {
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$object_ids = $jkdb->get_col( $jkdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s", $meta_key, $meta_value ) );
		} else {
			$object_ids = $jkdb->get_col( $jkdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
		}
	}
	...
	jk_cache_delete_multiple( $data, $meta_type . '_meta' );
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = jk_cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! jk_is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in jk-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $jkdb->get_row( $jkdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in jk-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $jkdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result ) {
			return false;
		}
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		$result = (bool) $jkdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$cache          = array();
	$cache_values   = jk_cache_get_multiple( $object_ids, $cache_key );
	...
	$meta_list = $jkdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	jk_cache_add_multiple( $data, $cache_key );
 ...
 * @since 4.5.0
 *
 * @return JK_Metadata_Lazyloader Metadata lazyloader queue.
 */
function jk_metadata_lazyloader() {
	static $jk_metadata_lazyloader;

	if ( null === $jk_metadata_lazyloader ) {
		$jk_metadata_lazyloader = new JK_Metadata_Lazyloader();
	}

	return $jk_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new JK_Meta_Query( $meta_query );
...
function _get_meta_table( $type ) {
	...
	if ( empty( $jkdb->$table_name ) ) {
		return false;
	}

	return $jkdb->$table_name;
}
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$jk_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $jk_meta_keys ) || ! isset( $jk_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $jk_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $jk_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $jk_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $jk_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $jk_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $jk_meta_keys[ $object_type ] ) ) {
		unset( $jk_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $jk_meta_keys ) || ! isset( $jk_meta_keys[ $object_type ] ) || ! isset( $jk_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $jk_meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function _jk_register_meta_args_allowed_list( $args, $default_args ) {
