...
/**
 * Taxonomy API: JK_Term_Query class.
 *
 * @package JKPress
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see JK_Term_Query::__construct() for accepted arguments.
 */
...
class JK_Term_Query {
	 ...
	 *                                                   - 'all' Returns an array of complete term objects (`JK_Term[]`).
	 *                                                   - 'all_with_object_id' Returns an array of term objects
	 *                                                     with the 'object_id' param (`JK_Term[]`). Works only
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query JK_Term_Query arguments. See JK_Term_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		...
		$query = jk_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param JK_Term_Query $query Current instance of JK_Term_Query.
		 */
	 ...
	 * JK_Term_Query::get_terms() for details.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return JK_Term[]|int[]|string[]|string Array of terms, or number of terms as numeric string
	 ...
	 *
	 * The following will result in an array of `JK_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	...
	public function get_terms() {
		 ...
		 * @since 4.6.0
		 *
		 * @param JK_Term_Query $query Current instance of JK_Term_Query (passed by reference).
		 */
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', jk_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree      = jk_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( jk_parse_id_list( $exclude ), $exclusions );
		}
		...
		if ( ! empty( $args['name__like'] ) ) {
			$this->sql_clauses['where']['name__like'] = $jkdb->prepare(
				't.name LIKE %s',
				'%' . $jkdb->esc_like( $args['name__like'] ) . '%'
			);
		}

		if ( ! empty( $args['description__like'] ) ) {
			$this->sql_clauses['where']['description__like'] = $jkdb->prepare(
				'tt.description LIKE %s',
				'%' . $jkdb->esc_like( $args['description__like'] ) . '%'
			);
		}
		...
		$join .= " INNER JOIN $jkdb->term_taxonomy AS tt ON t.term_id = tt.term_id";

		if ( ! empty( $this->query_vars['object_ids'] ) ) {
			$join    .= " INNER JOIN {$jkdb->term_relationships} AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$distinct = 'DISTINCT';
		}
		...
		$this->sql_clauses['from']    = "FROM $jkdb->terms AS t $join";
		 ...
		 *
		 * Return a non-null value to bypass JKPress' default term queries.
		 *
		 * @since 5.3.0
		 *
		 * @param array|null    $terms Return an array of term data to short-circuit JK's term query,
		 *                             or null to allow JK queries to run normally.
		 * @param JK_Term_Query $query The JK_Term_Query instance, passed by reference.
		 */
			...
			if ( false !== $cache ) {
				if ( 'ids' === $_fields ) {
					...
					|| ( 'all' === $_fields && $args['pad_counts'] || $fields_is_filtered )
						...
						$term_ids = jk_list_pluck( $cache, 'term_id' );
					} else {
		...
		if ( 'count' === $_fields ) {
			$count = $jkdb->get_var( $this->request ); // phpcs:ignore JKPress.DB.PreparedSQL.NotPrepared
			if ( $args['cache_results'] ) {
				jk_cache_set( $cache_key, $count, 'term-queries' );
			}
			return $count;
		}

		$terms = $jkdb->get_results( $this->request ); // phpcs:ignore JKPress.DB.PreparedSQL.NotPrepared

		if ( empty( $terms ) ) {
			if ( $args['cache_results'] ) {
				jk_cache_add( $cache_key, array(), 'term-queries' );
			}
			return array();
		}
		...
		if ( $args['update_term_meta_cache'] ) {
			$term_ids = jk_list_pluck( $term_objects, 'term_id' );
		...
		if ( 'all_with_object_id' === $_fields && ! empty( $args['object_ids'] ) ) {
			...
			$term_cache = jk_list_pluck( $term_objects, 'term_id' );
		}

		if ( $args['cache_results'] ) {
			jk_cache_add( $cache_key, $term_cache, 'term-queries' );
		}
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', jk_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 6.0.0
	 *
	 * @param JK_Term[] $term_objects Array of term objects.
	 * @param string    $_fields      Field to format.
	 *
	 * @return JK_Term[]|int[]|string[] Array of terms / strings / ints depending on field requested.
	 */
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to JK_Term_Query.
	...
	protected function get_search_sql( $search ) {
		...
		return $jkdb->prepare( '((t.name LIKE %s) OR (t.slug LIKE %s))', $like, $like );
	}
	 ...
	 * @return JK_Term[] Array of `JK_Term` objects.
	 */
	 ...
	 * @global jkdb $jkdb JKPress database abstraction object.
	 *
	 * @param array  $args JK_Term_Query arguments.
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$cache_args = jk_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) );
