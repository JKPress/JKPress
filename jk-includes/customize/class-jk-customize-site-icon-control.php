<?php
/**
 * Customize API: JK_Customize_Site_Icon_Control class
 *
 * @package JKPress
 ...
 * @since 4.3.0
 *
 * @see JK_Customize_Cropped_Image_Control
 */
class JK_Customize_Site_Icon_Control extends JK_Customize_Cropped_Image_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		add_action( 'customize_controls_print_styles', 'jk_site_icon', 99 );
	}
							...
							<img src="{{ data.attachment.sizes.full ? data.attachment.sizes.full.url : data.attachment.url }}" class="app-icon-preview" alt="{{
								data.attachment.alt ?
									jk.i18n.sprintf(
										...
										echo jk_json_encode( __( 'App icon preview: Current image: %s' ) )
										?>
										,
										data.attachment.alt
									) :
									jk.i18n.sprintf(
										...
										echo jk_json_encode( __( 'App icon preview: The current image has no alternative text. The file name is: %s' ) );
										?>
										,
									...
									<img src="{{ data.attachment.sizes.full ? data.attachment.sizes.full.url : data.attachment.url }}" class="browser-icon-preview" alt="{{
										data.attachment.alt ?
											jk.i18n.sprintf(
												<?php
												/* translators: %s: The selected image alt text. */
												echo jk_json_encode( __( 'Browser icon preview: Current image: %s' ) );
												?>
												,
												data.attachment.alt
											) :
											jk.i18n.sprintf(
												<?php
												/* translators: %s: The selected image filename. */
												echo jk_json_encode( __( 'Browser icon preview: The current image has no alternative text. The file name is: %s' ) );
												?>
												,
												data.attachment.filename
											)
