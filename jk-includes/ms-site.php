 ...
 *                                Passed to the `jk_initialize_site` hook.
 * }
 * @return int|JK_Error The new site's ID on success, or error object on failure.
 */
function jk_insert_site( array $data ) {
	global $jkdb;
	...
	$prepared_data = jk_prepare_site_data( $data, $defaults );
	if ( is_jk_error( $prepared_data ) ) {
		return $prepared_data;
	}

	if ( false === $jkdb->insert( $jkdb->blogs, $prepared_data ) ) {
		return new JK_Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $jkdb->last_error );
	}

	$site_id = (int) $jkdb->insert_id;
	...
	if ( ! $new_site ) {
		return new JK_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param JK_Site $new_site New site object.
	 */
	do_action( 'jk_insert_site', $new_site );
	 ...
	 */
	do_action( 'jk_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'jkmu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// JKLANG was passed with `$meta` to the `jkmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'JKLANG', $meta ) ) {
			$meta['JKLANG'] = get_network_option( $new_site->network_id, 'JKLANG' );
		}

		/*
		 * Rebuild the data expected by the `jkmu_new_blog` hook prior to 5.1.0 using allowed keys.
		 * The `$allowed_data_fields` matches the one used in `jkmu_create_blog()`.
		 */
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see 'jk_initialize_site'} instead.
		 *
 ...
 * @param array $data    Site data to update. See {@see jk_insert_site()} for the list of supported keys.
 * @return int|JK_Error The updated site's ID on success, or error object on failure.
 */
function jk_update_site( $site_id, array $data ) {
	global $jkdb;

	if ( empty( $site_id ) ) {
		return new JK_Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new JK_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = jk_prepare_site_data( $data, $defaults, $old_site );
	if ( is_jk_error( $data ) ) {
		return $data;
	}

	if ( false === $jkdb->update( $jkdb->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new JK_Error( 'db_update_error', __( 'Could not update site in the database.' ), $jkdb->last_error );
	}
	 ...
	 *
	 * @param JK_Site $new_site New site object.
	 * @param JK_Site $old_site Old site object.
	 */
	do_action( 'jk_update_site', $new_site, $old_site );
 ...
 * @return JK_Site|JK_Error The deleted site object on success, or error object on failure.
 */
function jk_delete_site( $site_id ) {
	global $jkdb;
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `JK_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param JK_Error $errors   Error object to add validation errors to.
	 * @param JK_Site  $old_site The site object to be deleted.
	 */
	do_action( 'jk_validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param JK_Site $old_site Deleted site object.
	 */
	do_action( 'jk_uninitialize_site', $old_site );

	if ( is_site_meta_supported() ) {
		$blog_meta_ids = $jkdb->get_col( $jkdb->prepare( "SELECT meta_id FROM $jkdb->blogmeta WHERE blog_id = %d ", $old_site->id ) );
		foreach ( $blog_meta_ids as $mid ) {
			delete_metadata_by_mid( 'blog', $mid );
		}
	}

	if ( false === $jkdb->delete( $jkdb->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new JK_Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $jkdb->last_error );
	}
	 ...
	 * @param JK_Site $old_site Deleted site object.
	 */
	do_action( 'jk_delete_site', $old_site );
 ...
 * @since 4.6.0
 *
 * @param JK_Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return JK_Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof JK_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new JK_Site( $site );
	} else {
		$_site = JK_Site::get_instance( $site );
	}
 ...
 * @since 6.3.0 Use jk_lazyload_site_meta() for lazy-loading of site meta.
 *
...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $jkdb->get_results( sprintf( "SELECT * FROM $jkdb->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore JKPress.DB.PreparedSQL.NotPrepared

		update_site_cache( $fresh_sites, false );
	}

	if ( $update_meta_cache ) {
		jk_lazyload_site_meta( $ids );
	}
}
 ...
 */
function jk_lazyload_site_meta( array $site_ids ) {
	if ( empty( $site_ids ) ) {
		return;
	}
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	jk_cache_add_multiple( $site_data, 'sites' );
	jk_cache_add_multiple( $blog_details_data, 'blog-details' );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'jk_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'jk_check_site_meta_support_prefilter' );
	}
 ...
 *
 * @see JK_Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See JK_Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return JK_Site[]|int[]|int List of JK_Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                             or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new JK_Site_Query();
 ...
 *                               See {@see jk_insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param JK_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|JK_Error Site data ready for a database transaction, or JK_Error in case a validation
 *                        error occurred.
 */
function jk_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see jk_insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'jk_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( jk_parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `JK_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param JK_Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see jk_insert_site()}
	 *                               for the included data.
	 * @param JK_Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'jk_validate_site_data', $errors, $data, $old_site );
 ...
 */
function jk_normalize_site_data( $data ) {
 ...
 * @since 5.1.0
 *
 * @param JK_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see jk_insert_site()}
 *                               for the included data.
 * @param JK_Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function jk_validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = jk_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
 ...
 *
 * @param int|JK_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|JK_Error True on success, or error object on failure.
 */
function jk_initialize_site( $site_id, array $args = array() ) {
	global $jkdb, $jk_roles;
	...
	if ( ! $site ) {
		return new JK_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( jk_is_site_initialized( $site ) ) {
		return new JK_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param JK_Site    $site    Site that is being initialized.
	 * @param JK_Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'jk_initialize_site_args', $args, $site, $network );

	$orig_installing = jk_installing();
	if ( ! $orig_installing ) {
		jk_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'blogname'    => jk_unslash( $args['title'] ),
				'admin_email' => '',
				...
				'JKLANG'      => get_network_option( $network->id, 'JKLANG' ),
			),
			$args['options']
		)
	);
	...
	populate_roles();
	$jk_roles = new JK_Roles();

	// Populate metadata for the site.
	populate_site_meta( $site->id, $args['meta'] );

	// Remove all permissions that may exist for the site.
	$table_prefix = $jkdb->get_blog_prefix();
	delete_metadata( 'user', 0, $table_prefix . 'user_level', null, true );   // Delete all.
	delete_metadata( 'user', 0, $table_prefix . 'capabilities', null, true ); // Delete all.

	// Install default site content.
	jk_install_defaults( $args['user_id'] );
 ...
 * @return true|JK_Error True on success, or error object on failure.
 */
function jk_uninitialize_site( $site_id ) {
	global $jkdb;
	...
	if ( ! jk_is_site_initialized( $site ) ) {
		return new JK_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 */
	$drop_tables = apply_filters( 'jkmu_drop_tables', $tables, $site->id );

	foreach ( (array) $drop_tables as $table ) {
		$jkdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore JKPress.DB.PreparedSQL.NotPrepared
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. See {@see jk_upload_dir()}.
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'jkmu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable JKPress.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable JKPress.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 */
function jk_is_site_initialized( $site_id ) {
	global $jkdb;
	 ...
	 */
	$pre = apply_filters( 'pre_jk_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', 'jk_switch_roles_and_user', 1 );
		switch_to_blog( $site_id );
	}

	$suppress = $jkdb->suppress_errors();
	$result   = (bool) $jkdb->get_results( "DESCRIBE {$jkdb->posts}" );
	$jkdb->suppress_errors( $suppress );

	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', 'jk_switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param JK_Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a JK_Site object exists even when the site has been deleted.
		$blog = new JK_Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	...
	jk_cache_delete( $blog_id, 'sites' );
	jk_cache_delete( $blog_id, 'site-details' );
	jk_cache_delete( $blog_id, 'blog-details' );
	jk_cache_delete( $blog_id . 'short', 'blog-details' );
	jk_cache_delete( $domain_path_key, 'blog-lookup' );
	jk_cache_delete( $domain_path_key, 'blog-id-cache' );
	jk_cache_delete( $blog_id, 'blog_meta' );
	 ...
	 * @param JK_Site $blog            Site object.
 ...
 * @since 5.1.0
 *
 * @param JK_Site      $new_site The site object that has been inserted, updated or deleted.
 * @param JK_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function jk_maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		jk_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id !== $old_site->network_id ) {
		jk_maybe_update_network_site_counts( $new_site->network_id );
		jk_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param JK_Site      $new_site The site object after the update.
 * @param JK_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function jk_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new JK_Site( new stdClass() );
	}
 ...
 *
 * @param JK_Site $new_site The site object after the update.
 * @param JK_Site $old_site The site object prior to the update.
 */
function jk_maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 *                          for compatibility reasons. Accepts '1' or '0'.
 */
function jk_update_blog_public_option_on_site_update( $site_id, $is_public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! jk_is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function jk_cache_set_sites_last_changed() {
	jk_cache_set_last_changed( 'sites' );
}
 ...
 */
function jk_check_site_meta_support_prefilter( $check ) {
	if ( ! is_site_meta_supported() ) {
		/* translators: %s: Database table name. */
		_doing_it_wrong( __FUNCTION__, sprintf( __( 'The %s table is not installed. Please run the network database upgrade.' ), $GLOBALS['jkdb']->blogmeta ), '5.1.0' );
		return false;
	}
