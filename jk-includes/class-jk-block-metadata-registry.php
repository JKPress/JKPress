 ...
 *
 * The JK_Block_Metadata_Registry allows plugins to register metadata for large
 ...
 * @since 6.7.0
 */
class JK_Block_Metadata_Registry {
	...
	/**
	 * Stores the JKPress 'jk-includes' directory path.
	 *
	 * @since 6.7.0
	 * @var string|null
	 */
	...
	/**
	 * Stores the normalized JKPress plugin directory path.
	 *
	 * @since 6.7.0
	 * @var string|null
	 */
	 ...
	 * @since 6.7.0
	 *
	 * @param string $path     The absolute base path for the collection ( e.g., JK_PLUGIN_DIR . '/my-plugin/blocks/' ).
	...
	public static function register_collection( $path, $manifest ) {
		$path = jk_normalize_path( rtrim( $path, '/' ) );
		...
		if ( str_starts_with( $path, $plugin_dir ) ) {
			...
			// If it's neither a plugin directory path nor within 'jk-includes', the path is invalid.
			_doing_it_wrong(
				__METHOD__,
				__( 'Block metadata collections can only be registered for a specific plugin. The provided path is neither a core path nor a valid plugin path.' ),
				'6.7.0'
	...
	private static function find_collection_path( $file_or_folder ) {
		...
		$path = jk_normalize_path( rtrim( $file_or_folder, '/' ) );
	 ...
	 *
	 * For example:
	 * - Path: '/jk-content/plugins/my-plugin/blocks/example/block.json'
	 *   Identifier: 'example'
	 * - Path: '/jk-content/plugins/my-plugin/blocks/another-block'
	 *   Identifier: 'another-block'
	 *
	 * This default behavior matches the standard JKPress block structure.
	 *
	 * @since 6.7.0
	 *
	...
	/**
	 * Gets the JKPress 'jk-includes' directory path.
	 *
	 * @since 6.7.0
	 *
	 * @return string The JKPress 'jk-includes' directory path.
	 */
	private static function get_wpinc_dir() {
		if ( ! isset( self::$wpinc_dir ) ) {
			self::$wpinc_dir = jk_normalize_path( ABSPATH . JKINC );
		}
		return self::$wpinc_dir;
	}

	/**
	 * Gets the normalized JKPress plugin directory path.
	 *
	 * @since 6.7.0
	 *
	 * @return string The normalized JKPress plugin directory path.
	 */
	private static function get_plugin_dir() {
		if ( ! isset( self::$plugin_dir ) ) {
			self::$plugin_dir = jk_normalize_path( JK_PLUGIN_DIR );
		}
