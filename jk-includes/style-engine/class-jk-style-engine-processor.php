<?php
/**
 * Style Engine: JK_Style_Engine_Processor class
 *
 * @package JKPress
...
class JK_Style_Engine_Processor {

	/**
	 * A collection of Style Engine Store objects.
	 *
	 * @since 6.1.0
	 * @var JK_Style_Engine_CSS_Rules_Store[]
	 */
	 ...
	 * @since 6.1.0
	 *
	 * @param JK_Style_Engine_CSS_Rules_Store $store The store to add.
	 * @return JK_Style_Engine_Processor Returns the object to allow chaining methods.
	 */
	public function add_store( $store ) {
		if ( ! $store instanceof JK_Style_Engine_CSS_Rules_Store ) {
			_doing_it_wrong(
				__METHOD__,
				__( '$store must be an instance of JK_Style_Engine_CSS_Rules_Store' ),
				'6.1.0'
			);
			return $this;
		}
	 ...
	 *
	 * @param JK_Style_Engine_CSS_Rule|JK_Style_Engine_CSS_Rule[] $css_rules A single, or an array of,
	 *                                                                       JK_Style_Engine_CSS_Rule objects
	...
	public function get_css( $options = array() ) {
		...
		foreach ( $this->css_rules as $rule ) {
			// See class JK_Style_Engine_CSS_Rule for the get_css method.
	...
	private function combine_rules_selectors() {
		...
		foreach ( $this->css_rules as $rule ) {
			...
			ksort( $declarations );
			$selectors_json[ $rule->get_selector() ] = jk_json_encode( $declarations );
		}
		...
		foreach ( $selectors_json as $selector => $json ) {
			...
			$this->css_rules[ $duplicate_selectors ] = new JK_Style_Engine_CSS_Rule( $duplicate_selectors, $declarations );
		}
	}
}
