...
function get_option( $option, $default_value = false ) {
	...
	if ( isset( $deprecated_keys[ $option ] ) && ! jk_installing() ) {
	...
	if ( ! jk_installing() ) {
		...
		if ( isset( $alloptions[ $option ] ) ) {
			...
			$value = jk_cache_get( $option, 'options' );

			if ( false === $value ) {
				// Prevent non-existent options from triggering multiple queries.
				$notoptions = jk_cache_get( 'notoptions', 'options' );

				// Prevent non-existent `notoptions` key from triggering multiple key lookups.
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
					jk_cache_set( 'notoptions', $notoptions, 'options' );
				...
				$row = $jkdb->get_row( $jkdb->prepare( "SELECT option_value FROM $jkdb->options WHERE option_name = %s LIMIT 1", $option ) );

				// Has to be get_row() instead of get_var() because of funkiness with 0, false, null values.
				if ( is_object( $row ) ) {
					$value = $row->option_value;
					jk_cache_add( $option, $value, 'options' );
				} else { // Option does not exist, so we must cache its non-existence.
					$notoptions[ $option ] = true;
					jk_cache_set( 'notoptions', $notoptions, 'options' );

					/** This filter is documented in jk-includes/option.php */
					return apply_filters( "default_option_{$option}", $default_value, $option, $passed_default );
				}
			}
		}
 ...
 */
function jk_prime_option_caches( $options ) {
	global $jkdb;

	$alloptions     = jk_load_alloptions();
	$cached_options = jk_cache_get_multiple( $options, 'options' );
	$notoptions     = jk_cache_get( 'notoptions', 'options' );
	if ( ! is_array( $notoptions ) ) {
		$notoptions = array();
	}
	...
	$results = $jkdb->get_results(
		$jkdb->prepare(
			sprintf(
				"SELECT option_name, option_value FROM $jkdb->options WHERE option_name IN (%s)",
	...
	jk_cache_set_multiple( $options_found, 'options' );
 ...
 */
function jk_prime_option_caches_by_group( $option_group ) {
	global $new_allowed_options;

	if ( isset( $new_allowed_options[ $option_group ] ) ) {
		jk_prime_option_caches( $new_allowed_options[ $option_group ] );
	}
}
...
function get_options( $options ) {
	jk_prime_option_caches( $options );
 ...
 *               was updated.
 */
function jk_set_option_autoload_values( array $options ) {
	global $jkdb;
	...
	foreach ( $options as $option => $autoload ) {
		jk_protect_special_option( $option ); // Ensure only valid options can be passed.
	 ...
	 */
	// phpcs:ignore JKPress.DB.PreparedSQL.InterpolatedNotPrepared,JKPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	$options_to_update = $jkdb->get_col( $jkdb->prepare( "SELECT option_name FROM $jkdb->options $where", $where_args ) );
	...
	foreach ( $grouped_options as $autoload => $options ) {
		...
		$success = $jkdb->query(
			$jkdb->prepare(
				"UPDATE $jkdb->options SET autoload = %s WHERE option_name IN (" . implode( ',', array_fill( 0, count( $grouped_options[ $autoload ] ), '%s' ) ) . ')',
				array_merge(
					array( $autoload ),
	...
	if ( $grouped_options['on'] ) {
		jk_cache_delete_multiple( $grouped_options['on'], 'options' );
		jk_cache_delete( 'alloptions', 'options' );
	} elseif ( $grouped_options['off'] ) {
		$alloptions = jk_load_alloptions( true );
		...
		jk_cache_set( 'alloptions', $alloptions, 'options' );
	}
 ...
 *
 * This is a wrapper for {@see jk_set_option_autoload_values()}, which can be used to set different autoload values for
 * each option at once.
 *
 * @since 6.4.0
 * @since 6.7.0 The autoload values 'yes' and 'no' are deprecated.
 *
 * @see jk_set_option_autoload_values()
 *
 * @param string[] $options  List of option names. Expected to not be SQL-escaped.
 * @param bool     $autoload Autoload value to control whether to load the options when JKPress starts up.
 *                           For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                           deprecated.
 * @return array Associative array of all provided $options as keys and boolean values for whether their autoload value
 *               was updated.
 */
function jk_set_options_autoload( array $options, $autoload ) {
	return jk_set_option_autoload_values(
 ...
 *
 * This is a wrapper for {@see jk_set_option_autoload_values()}, which can be used to set the autoload value for
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when JKPress starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
 * @return bool True if the autoload value was modified, false otherwise.
 */
function jk_set_option_autoload( $option, $autoload ) {
	$result = jk_set_option_autoload_values( array( $option => $autoload ) );
...
/**
 * Protects JKPress special option from being modified.
 *
 ...
 * @param string $option Option name.
 */
function jk_protect_special_option( $option ) {
	if ( 'alloptions' === $option || 'notoptions' === $option ) {
		jk_die(
			sprintf(
				/* translators: %s: Option name. */
				__( '%s is a protected JK option and may not be modified' ),
				esc_html( $option )
			)
		);
	}
 ...
 */
function jk_load_alloptions( $force_cache = false ) {
	global $jkdb;
	 ...
	 * jk_load_alloptions(), returning that value instead.
	 *
	 * @since 6.2.0
	 *
	 ...
	 */
	$alloptions = apply_filters( 'pre_jk_load_alloptions', null, $force_cache );
	if ( is_array( $alloptions ) ) {
		return $alloptions;
	}

	if ( ! jk_installing() || ! is_multisite() ) {
		$alloptions = jk_cache_get( 'alloptions', 'options', $force_cache );
	} else {
		$alloptions = false;
	}
	...
	if ( ! $alloptions ) {
		$suppress      = $jkdb->suppress_errors();
		$alloptions_db = $jkdb->get_results( "SELECT option_name, option_value FROM $jkdb->options WHERE autoload IN ( '" . implode( "', '", esc_sql( jk_autoload_values_to_autoload() ) ) . "' )" );

		if ( ! $alloptions_db ) {
			$alloptions_db = $jkdb->get_results( "SELECT option_name, option_value FROM $jkdb->options" );
		}
		$jkdb->suppress_errors( $suppress );
		...
		if ( ! jk_installing() || ! is_multisite() ) {
			...
			jk_cache_add( 'alloptions', $alloptions, 'options' );
		}
	}
 ...
 *
 * If site is not multisite, then call jk_prime_option_caches().
 *
 * @since 6.6.0
 *
 * @see jk_prime_network_option_caches()
 *
 * @param string[] $options An array of option names to be loaded.
 */
function jk_prime_site_option_caches( array $options ) {
	jk_prime_network_option_caches( null, $options );
}
 ...
 * @param string[] $options    An array of option names to be loaded.
 */
function jk_prime_network_option_caches( $network_id, array $options ) {
	global $jkdb;
	...
	$cached_options = jk_cache_get_multiple( array_values( $cache_keys ), $cache_group );

	$notoptions_key = "$network_id:notoptions";
	$notoptions     = jk_cache_get( $notoptions_key, $cache_group );
	...
	$results      = $jkdb->get_results(
		$jkdb->prepare(
			sprintf(
				"SELECT meta_key, meta_value FROM $jkdb->sitemeta WHERE meta_key IN (%s) AND site_id = %s",
				implode( ',', array_fill( 0, count( $options_to_prime ), '%s' ) ),
				'%d'
			),
			$query_args
	...
	jk_cache_set_multiple( $data, $cache_group );
	// If all options were found, no need to update `notoptions` cache.
	if ( count( $options_found ) === count( $options_to_prime ) ) {
		return;
	}
	...
	// Only update the cache if it was modified.
	if ( $update_notoptions ) {
		jk_cache_set( $notoptions_key, $notoptions, $cache_group );
	}
}
 ...
 * @since 6.6.0 Uses jk_prime_network_option_caches().
 *
 * @param int $network_id Optional. Network ID of network for which to prime network options cache. Defaults to current network.
 */
function jk_load_core_site_options( $network_id = null ) {
	if ( ! is_multisite() || jk_installing() ) {
		return;
	}
	$core_options = array( 'site_name', 'siteurl', 'active_sitewide_plugins', '_site_transient_timeout_theme_roots', '_site_transient_theme_roots', 'site_admins', 'can_compress_scripts', 'global_terms_enabled', 'ms_files_rewriting', 'JKLANG' );

	jk_prime_network_option_caches( $network_id, $core_options );
}
 ...
 * @param bool|null $autoload Optional. Whether to load the option when JKPress starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in JKPress.
 ...
 *                            For non-existent options, the default is null, which means JKPress will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	...
	jk_protect_special_option( $option );
	...
	if ( null !== $autoload ) {
		$update_args['autoload'] = jk_determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	} else {
		// Retrieve the current autoload value to reevaluate it in case it was set automatically.
		$raw_autoload = $jkdb->get_var( $jkdb->prepare( "SELECT autoload FROM $jkdb->options WHERE option_name = %s LIMIT 1", $option ) );
		$allow_values = array( 'auto-on', 'auto-off', 'auto' );
		if ( in_array( $raw_autoload, $allow_values, true ) ) {
			$autoload = jk_determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	...
	$result = $jkdb->update( $jkdb->options, $update_args, array( 'option_name' => $option ) );
	if ( ! $result ) {
		return false;
	}
	...
	if ( ! jk_installing() ) {
		if ( ! isset( $update_args['autoload'] ) ) {
			...
			if ( isset( $alloptions[ $option ] ) ) {
				...
				jk_cache_set( $option, $serialized_value, 'options' );
			}
		} elseif ( in_array( $update_args['autoload'], jk_autoload_values_to_autoload(), true ) ) {
			// Delete the individual cache, then set in alloptions cache.
			jk_cache_delete( $option, 'options' );
 ...
 * aren't adding a protected JKPress option. Care should be taken to not name
 ...
 *                              JKPress. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means JKPress will determine the autoload value.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = null ) {
	...
	$result = $jkdb->query( $jkdb->prepare( "INSERT INTO `$jkdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $serialized_value, $autoload ) );
	if ( ! $result ) {
		return false;
	}

	if ( ! jk_installing() ) {
		if ( in_array( $autoload, jk_autoload_values_to_autoload(), true ) ) {
			$alloptions            = jk_load_alloptions( true );
			$alloptions[ $option ] = $serialized_value;
			jk_cache_set( 'alloptions', $alloptions, 'options' );
		} else {
	...
	$notoptions = jk_cache_get( 'notoptions', 'options' ); // Yes, again... we need it to be fresh.
...
/**
 * Removes an option by name. Prevents removal of protected JKPress options.
 *
 * @since 1.2.0
 *
 * @global jkdb $jkdb JKPress database abstraction object.
...
function delete_option( $option ) {
	...
	$row = $jkdb->get_row( $jkdb->prepare( "SELECT autoload FROM $jkdb->options WHERE option_name = %s", $option ) );
	if ( is_null( $row ) ) {
		return false;
	}
	...
	$result = $jkdb->delete( $jkdb->options, array( 'option_name' => $option ) );

	if ( ! jk_installing() ) {
		if ( in_array( $row->autoload, jk_autoload_values_to_autoload(), true ) ) {
			$alloptions = jk_load_alloptions( true );
 ...
 */
function jk_determine_option_autoload_value( $option, $value, $serialized_value, $autoload ) {
	 ...
	 */
	$autoload = apply_filters( 'jk_default_autoload_value', null, $option, $value, $serialized_value );
	if ( is_bool( $autoload ) ) {
 ...
 */
function jk_filter_default_autoload_value_via_option_size( $autoload, $option, $value, $serialized_value ) {
	/**
	 ...
	 * @param string $option          The name of the option.
	 */
	$max_option_size = (int) apply_filters( 'jk_max_autoloaded_option_size', 150000, $option );
...
function delete_transient( $transient ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$result = jk_cache_delete( $transient, 'transient' );
	} else {
...
function get_transient( $transient ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$value = jk_cache_get( $transient, 'transient' );
	} else {
		...
		if ( ! jk_installing() ) {
			...
			if ( ! isset( $alloptions[ $transient_option ] ) ) {
				...
				jk_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$result = jk_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
		$transient_option  = '_transient_' . $transient;
		jk_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function delete_expired_transients( $force_db = false ) {
	...
	if ( ! $force_db && jk_using_ext_object_cache() ) {
		return;
	}

	$jkdb->query(
		$jkdb->prepare(
			"DELETE a, b FROM {$jkdb->options} a, {$jkdb->options} b
			WHERE a.option_name LIKE %s
			AND a.option_name NOT LIKE %s
			AND b.option_name = CONCAT( '_transient_timeout_', SUBSTRING( a.option_name, 12 ) )
			AND b.option_value < %d",
			$jkdb->esc_like( '_transient_' ) . '%',
			$jkdb->esc_like( '_transient_timeout_' ) . '%',
			time()
		)
	);
	...
	if ( ! is_multisite() ) {
		...
		$jkdb->query(
			$jkdb->prepare(
				...
				AND b.option_value < %d",
				$jkdb->esc_like( '_site_transient_' ) . '%',
				$jkdb->esc_like( '_site_transient_timeout_' ) . '%',
				time()
			)
		);
		...
		$jkdb->query(
			$jkdb->prepare(
				"DELETE a, b FROM {$jkdb->sitemeta} a, {$jkdb->sitemeta} b
...
function jk_user_settings() {
	...
	if ( isset( $_COOKIE[ 'jk-settings-' . $user_id ] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE[ 'jk-settings-' . $user_id ] );
		...
		if ( isset( $_COOKIE[ 'jk-settings-time-' . $user_id ] ) ) {
			$current = (int) preg_replace( '/[^0-9]/', '', $_COOKIE[ 'jk-settings-time-' . $user_id ] );
		}
	...
	setcookie( 'jk-settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	setcookie( 'jk-settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	$_COOKIE[ 'jk-settings-' . $user_id ] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return jk_set_all_user_settings( $all_user_settings );
}
 ...
 *                   Null if the current user is not a member of the site.
 */
function jk_set_all_user_settings( $user_settings ) {
	global $_updated_user_settings;
...
function delete_all_user_settings() {
	...
	setcookie( 'jk-settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
...
function get_network_option( $network_id, $option, $default_value = false ) {
	...
	// Prevent non-existent options from triggering multiple queries.
	$notoptions_key = "$network_id:notoptions";
	$notoptions     = jk_cache_get( $notoptions_key, 'site-options' );
	...
	if ( ! is_multisite() ) {
		...
		$value     = jk_cache_get( $cache_key, 'site-options' );

		if ( ! isset( $value ) || false === $value ) {
			$row = $jkdb->get_row( $jkdb->prepare( "SELECT meta_value FROM $jkdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $network_id ) );

			// Has to be get_row() instead of get_var() because of funkiness with 0, false, null values.
			if ( is_object( $row ) ) {
				$value = $row->meta_value;
				$value = maybe_unserialize( $value );
				jk_cache_set( $cache_key, $value, 'site-options' );
			} else {
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
				}
				...
				$notoptions[ $option ] = true;
				jk_cache_set( $notoptions_key, $notoptions, 'site-options' );
...
function add_network_option( $network_id, $option, $value ) {
	...
	if ( ! is_multisite() ) {
		...
		// This option exists now.
		$notoptions = jk_cache_get( $notoptions_key, 'site-options' ); // Yes, again... we need it to be fresh.
...
function delete_network_option( $network_id, $option ) {
	...
	if ( ! is_multisite() ) {
		...
		$row = $jkdb->get_row( $jkdb->prepare( "SELECT meta_id FROM {$jkdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $network_id ) );
		if ( is_null( $row ) || ! $row->meta_id ) {
			return false;
		}
		$cache_key = "$network_id:$option";
		jk_cache_delete( $cache_key, 'site-options' );
...
function delete_site_transient( $transient ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$result = jk_cache_delete( $transient, 'site-transient' );
	} else {
...
function get_site_transient( $transient ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$value = jk_cache_get( $transient, 'site-transient' );
	} else {
		...
		if ( ! in_array( $transient, $no_timeout, true ) ) {
			...
			jk_prime_site_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_site_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( jk_using_ext_object_cache() || jk_installing() ) {
		$result = jk_cache_set( $transient, $value, 'site-transient', $expiration );
	} else {
		$transient_timeout = '_site_transient_timeout_' . $transient;
		$option            = '_site_transient_' . $transient;
		jk_prime_site_option_caches( array( $option, $transient_timeout ) );
...
/**
 * Registers default settings available in JKPress.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in JKPress.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'JKPress locale code.' ),
			'default'      => 'en_US',
		)
	);
 ...
 *
 * @global array $new_allowed_options
 * @global array $jk_registered_settings
 *
...
function register_setting( $option_group, $option_name, $args = array() ) {
	global $new_allowed_options, $jk_registered_settings;
	...
	if ( ! is_array( $jk_registered_settings ) ) {
		$jk_registered_settings = array();
	}
	...
	$jk_registered_settings[ $option_name ] = $args;
}
...
function unregister_setting( $option_group, $option_name, $deprecated = '' ) {
	...
	if ( isset( $jk_registered_settings[ $option_name ] ) ) {
		// Remove the sanitize callback if one was set during registration.
		if ( ! empty( $jk_registered_settings[ $option_name ]['sanitize_callback'] ) ) {
			remove_filter( "sanitize_option_{$option_name}", $jk_registered_settings[ $option_name ]['sanitize_callback'] );
		}

		// Remove the default filter if a default was provided during registration.
		if ( array_key_exists( 'default', $jk_registered_settings[ $option_name ] ) ) {
		...
		unset( $jk_registered_settings[ $option_name ] );
	}
}
...
function get_registered_settings() {
	global $jk_registered_settings;

	if ( ! is_array( $jk_registered_settings ) ) {
		return array();
	}

	return $jk_registered_settings;
}
 ...
 */
function jk_autoload_values_to_autoload() {
	 ...
	 */
	$filtered_values = apply_filters( 'jk_autoload_values_to_autoload', $autoload_values );
