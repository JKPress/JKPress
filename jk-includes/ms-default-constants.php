 ...
 * jk-includes/ms-files.php (jk-content/blogs.php in MU).
 *
 * @since 3.0.0
 */
function ms_upload_constants() {
	...
	if ( ! defined( 'UPLOADBLOGSDIR' ) ) {
		define( 'UPLOADBLOGSDIR', 'jk-content/blogs.dir' );
	}

	/*
	 * Note, the main site in a post-MU network uses jk-content/uploads.
	 * This is handled in jk_upload_dir() by ignoring UPLOADS for this case.
	 */
	if ( ! defined( 'UPLOADS' ) ) {
		...
		if ( 'jk-content/blogs.dir' === UPLOADBLOGSDIR && ! defined( 'BLOGUPLOADDIR' ) ) {
			define( 'BLOGUPLOADDIR', JK_CONTENT_DIR . '/blogs.dir/' . $site_id . '/files/' );
		}
	}
}
...
function ms_file_constants() {
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'JKMU_SENDFILE' ) ) {
		define( 'JKMU_SENDFILE', false );
	}
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'JKMU_ACCEL_REDIRECT' ) ) {
		define( 'JKMU_ACCEL_REDIRECT', false );
	}
}
...
function ms_subdomain_constants() {
	...
	if ( $subdomain_error ) {
		$vhost_deprecated = sprintf(
			/* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: jk-config.php, 4: is_subdomain_install() */
